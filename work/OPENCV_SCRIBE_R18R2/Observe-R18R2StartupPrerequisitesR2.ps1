...
foreach($x in @($c.reviewCases)){
 $id=[string]$x.physicalIdentity;$d=Join-Path ([string]$c.proposalRoot) $id;$s=Join-Path $d 'scribe'
 $bf=Join-Path $s 'BF_SCRIBE_ORIENTED_DETECTOR_INPUT.png'
 $df=Join-Path $s 'DF_SCRIBE_ORIENTED_DETECTOR_INPUT.png'
 $rows+=[pscustomobject]@{physicalIdentity=$id;directoryPresent=(Test-Path -LiteralPath $d -PathType Container);scribeDirectoryPresent=(Test-Path -LiteralPath $s -PathType Container);bfPresent=(Test-Path -LiteralPath $bf -PathType Leaf);dfPresent=(Test-Path -LiteralPath $df -PathType Leaf)}
}
$missing=@($rows|Where-Object{-not $_.directoryPresent -or -not $_.scribeDirectoryPresent -or -not $_.bfPresent -or -not $_.dfPresent})
...
[ordered]@{
 schema='argos_opencv_scribe_r18r2_startup_prerequisite_observation_r2_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R18R2_STARTUP_PREREQUISITES_R2_OBSERVED';computerName=$env:COMPUTERNAME
