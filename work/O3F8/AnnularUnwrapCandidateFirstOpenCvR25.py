#!/usr/bin/env python3
"""R25 review-only successor to the R24 candidate-first annular detector.
...
EXPECTED_INFERENCE_ROOT = Path(r"C:\O3F16U25P2_CANDIDATE_DRAFT1")

TRACE_SCHEMA = "argos_ocv03_annular_candidate_first_r25_native_trace_v1"
POPULATION_SCHEMA = "argos_ocv03_annular_candidate_first_r25_neutral_population_v1"
SUMMARY_SCHEMA = "argos_ocv03_annular_candidate_first_r25_inference_v1"
FREEZE_SCHEMA = "argos_ocv03_annular_candidate_first_r25_freeze_manifest_v1"
GATE_SCHEMA = "argos_ocv03_annular_candidate_first_r25_inference_gate_v1"
EVALUATION_SCHEMA = "argos_ocv03_annular_candidate_first_r25_post_label_evaluation_v1"
...
EXPECTED_MAXIMUM_LOGICAL_OUTPUT_BYTES = 18 * 1024 ** 3
OUTPUT_SAFETY_RESERVE_BYTES = 16 * 1024 ** 3
...
CAPACITY_OBSERVED_R24_TRACE_COUNT = 10_449
CAPACITY_PROJECTED_R25_P1_P2_BYTES = 3_776_434_938
CAPACITY_PROJECTED_R25_TOTAL_TRIPLE_CANDIDATES_BYTES = 11_398_075_518
...
def require_output_write_reserve(path: Path, planned_logical_bytes: int) -> None:
    ...
    need(
        ...
        f"R25 output write would cross the 16 GiB safety reserve: {path}",
    )
...
def enable_fresh_output_compression(root: Path, free_bytes_before: int) -> dict[str, Any]:
    need(root.is_dir() and not any(root.iterdir()), "R25 compression target is not a fresh empty directory")
    ...
    need(
        ...
        "Pinned compact utility failed before R25 child output: "
        + completed.stderr[-512:].decode("utf-8", errors="replace"),
    )
    attributes = int(getattr(root.stat(), "st_file_attributes", 0))
    need(attributes & FILE_ATTRIBUTE_COMPRESSED, "Fresh R25 root lacks the required NTFS compressed attribute")
    need(not any(root.iterdir()), "Pinned compact utility created unexpected R25 root content")
    return {
        "state": "PASS_R25_FRESH_RESULT_ROOT_NTFS_COMPRESSION_ENABLED",
        "root": str(root),
        ...
        "capacityProjection": {
            "basis": "EXACT_PRESERVED_R24_P1_P2_TRACE_AND_ASSET_INVENTORY_REENCODED_AS_R25",
            "observedR24TraceCount": CAPACITY_OBSERVED_R24_TRACE_COUNT,
            "projectedR25P1P2LogicalBytes": CAPACITY_PROJECTED_R25_P1_P2_BYTES,
            "projectedR25LogicalBytesIfRemainingCohortsAddTwiceObservedCandidates": (
                CAPACITY_PROJECTED_R25_TOTAL_TRIPLE_CANDIDATES_BYTES
            ),
...
def preflight_output_layout(root: Path, identities: Iterable[str], evaluation: bool = False) -> None:
    components = {
        ...
        "hotspot",
        "evaluation",
        "POST2_R25_POSTFREEZE_SAME_ANGLE_NATIVE_COMPARISON.png",
...
def configure_pinned_dependency_paths() -> None:
    ...
    for name, path in requested.items():
        ...
        if existing:
            need(Path(existing).resolve() == path.resolve(), f"{name} differs from the frozen R25 dependency path")
        os.environ[name] = str(path)


configure_pinned_dependency_paths()
require_exact_file(R21_PATH, R21_SHA256, "R21 engine")
SPEC = importlib.util.spec_from_file_location("argos_annular_r21_for_r25", R21_PATH)
...
def candidate_graph_trace(
    ...
    need(component.size > 0 and core_columns, "R25 candidate component is empty")
        ...
        base.update(
            {
                ...
                "gradientNormalAlignmentPostContourMeasurement": {
                    "state": "NOT_REQUIRED_FOR_R25_NATIVE_RAW_POLARITY_CONTOUR",
                    "selectionRole": False,
                },
...
def analyze_native_strip(
    ...
    need(strip.dtype == np.uint8 and strip.ndim == 2, "R25 strip must be uint8 grayscale")
    need(offsets.ndim == 1 and offsets.size == strip.shape[0], "R25 strip/offset geometry differs")
    need(predecessor_measured.shape == (strip.shape[1],), "R25 predecessor hold width differs")
    ...
    need(inner_circle["radius"] > 0.0, "R25 inner circle radius is non-positive")
    inner_path = r18.circle_ray_offsets(base_fit, inner_circle)
    spacing_error = float(
        np.max(np.abs((circle["outerPath"] - inner_path) - r18.EDGE_ZONE_INWARD_PX))
    )
    need(spacing_error <= 1.0e-4, "R25 yellow circle is not exactly 20 px inward")
    ...
    return {
        ...
        "discovery": discovery,
        "evidence": {
            "algorithm": "R25_CANDIDATE_FIRST_CHANNEL_LOCAL_NATIVE_SUPPORT_GRAPH",
...
def render_channel(
    ...
    need(bool(np.all(~changed | declared)), "R25 full overlay changed pixels outside declared masks")
    need(int(np.count_nonzero(normal_mask)) > 0, "R25 normal brightness trace rendered empty")
    stem = safe_stem(identity) + "_" + channel.lower()
    trace_context_asset = write_json_new(
        root / f"{stem}_trace_context.json",
        {
            "schema": "argos_ocv03_annular_candidate_first_r25_channel_trace_context_v1",
            "identity": identity,
            "channel": channel,
...
def channel_summary(
    ...
    return {
        ...
        "r25DiagnosticCyanGeometryVerified": analysis["cyanGeometryVerified"],
        "r25DiagnosticCyanGeometryVerificationState": analysis["cyanGeometryVerificationState"],
...
def process_post2(
    ...
    for ordinal, member in enumerate(source_job["inputs"], 1):
        ...
        pair["r25DiagnosticStateBeforeInheritedAuthority"] = pair["state"]
        pair["r25DiagnosticResolvedPairsRemainUnowned"] = pair["resolvedPairs"]
        ...
        results.append(
            {
                "ordinal": ordinal,
                "identity": identity,
                "state": "DIAGNOSTIC_ONLY_R25_POST2_CANDIDATE_FIRST_CONTOURS_COMPLETE",
                "channels": rows,
                "pairDiagnostic": pair,
        ...
        results.append(
            {
                ...
                "safeId": identity,
                "state": "DIAGNOSTIC_ONLY_R25_HOTSPOT_CANDIDATE_FIRST_CONTOURS_COMPLETE",
                "channels": rows,
                "pairDiagnostic": pair,
            }
        )
...
def evaluate_hotspot_regression(
    ...
    for current, prior in zip(results, oracle["results"]):
        ...
        for channel in ("BF", "DF"):
            ...
            if old_accepted:
                new_channel["contourAuthorityState"] = "DIAGNOSTIC_ONLY_R25_NATIVE_CONTOUR_REGRESSION"
            else:
            ...
            new_channel["r25MeasurementCannotClearPredecessorR21Hold"] = True
            held_preserved = bool(
                old_accepted
                or (
            ...
            channel_rows[channel] = {
                ...
                "r25CyanGeometryVerified": bool(new_channel["cyanGeometryVerified"]),
    ...
    return {
        "state": (
            "PASS_R25_HOTSPOT_REGRESSION_WITH_EXISTING_HOLDS_RETAINED"
            if all(
            ...
            else "HOLD_R25_HOTSPOT_REGRESSION_DIFFERENCE"
        ),
        "caseCount": len(rows),
...
def post2_semantic_checks(results: list[dict[str, Any]]) -> dict[str, bool]:
    ...
    slot17_broad_held = bool(
        ...
        and not slot17["pairDiagnostic"].get("r25DiagnosticResolvedPairsRemainUnowned")
...
def run_inference(args: argparse.Namespace) -> int:
    for supplied, expected, label in (
        ...
        need(str(supplied).upper() == expected, f"Supplied {label} pin differs from R25")
    ...
    need(code_root == PROJECT_ROOT.resolve(), "R25 code root differs from its exact checkout")
    need(
        output.is_absolute()
        ...
        "R25 inference output must be a fresh short C: root",
    )
    ...
    need(
        ...
        "Fresh R25 root lacks its 18 GiB logical-output allowance plus 16 GiB safety reserve",
    )
    output.mkdir()
    output_storage = enable_fresh_output_compression(output, free_bytes_before)
    neutral_root = output / "neutral"
    neutral_root.mkdir()
    need(directory_is_ntfs_compressed(neutral_root), "R25 neutral child did not inherit NTFS compression")
    ...
    freeze = {
        ...
        "state": "PASS_R25_COMPLETE_NEUTRAL_POPULATION_FILESET_FROZEN",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
    ...
    summary = {
        ...
        "state": "COMPLETE_DIAGNOSTIC_ONLY_R25_NEUTRAL_POPULATION_FROZEN",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
        "engine": file_record(Path(__file__).resolve()),
    ...
    gate = {
        "schema": GATE_SCHEMA,
        "state": (
            "PASS_R25_NEUTRAL_POPULATION_FROZEN_WITH_SEMANTIC_HOLDS_RETAINED"
            if evaluation_authorized
            else "HOLD_R25_NEUTRAL_POPULATION_PRELABEL_GATE_FAILURE"
        ),
...
def run_evaluation(args: argparse.Namespace) -> int:
    need(args.scorer_labels_sha256.upper() == SCORER_LABELS_SHA256, "Scorer-label pin differs from R25")
    output = Path(args.output_root)
    ...
    need(
        ...
        and not output.exists()
        and len(str(output)) + 128 < 200,
        "R25 evaluation output must be a fresh short C: child/root",
    )
    need(args.engine_sha256.upper() == sha256_file(Path(__file__).resolve()), "R25 source changed after inference freeze")
    need(
        directory_is_ntfs_compressed(EXPECTED_INFERENCE_ROOT),
        "Frozen R25 inference root lost its verified NTFS compressed attribute",
    )
    ...
    summary = load_json_pinned(summary_path, args.inference_summary_sha256, "R25 inference summary")
    gate = load_json_pinned(gate_path, args.inference_gate_sha256, "R25 inference gate")
    population = load_json_pinned(population_path, args.neutral_population_sha256, "R25 neutral population")
    freeze = load_json_pinned(freeze_path, args.freeze_manifest_sha256, "R25 freeze manifest")
    need(
        summary.get("schema") == SUMMARY_SCHEMA
        and summary.get("state") == "COMPLETE_DIAGNOSTIC_ONLY_R25_NEUTRAL_POPULATION_FROZEN"
        and summary.get("scorerLabelJsonParsed") is False
        and summary.get("candidatePopulationFrozenBeforeScorerLabelParse") is True,
        "R25 inference summary is not a clean pre-label freeze",
    )
    need(
        gate.get("schema") == GATE_SCHEMA
        and str(gate.get("state", "")).startswith("PASS_R25_NEUTRAL_POPULATION_FROZEN")
        ...
        "R25 inference integrity gate does not authorize post-freeze evaluation",
    )
    need(
        ...
        "R25 neutral population was not frozen before label evaluation",
    )
    need(
        freeze.get("schema") == FREEZE_SCHEMA
        and freeze.get("state") == "PASS_R25_COMPLETE_NEUTRAL_POPULATION_FILESET_FROZEN",
        "R25 freeze manifest is not PASS",
    )
    need(
        ...
        "R25 gate does not bind the supplied frozen files",
    )
    ...
    need(
        ...
        "R25 source changed during frozen-population rehash",
    )
    ...
    output.mkdir()
    need(directory_is_ntfs_compressed(output), "R25 evaluation child did not inherit NTFS compression")
    evaluation_storage = {
        "state": "PASS_R25_EVALUATION_CHILD_INHERITED_NTFS_COMPRESSION",
        "root": str(output),
        "directoryCompressedAttributeVerifiedBeforeChildWrites": True,
        "logicalFileBytesAndSha256UnaffectedByFilesystemCompression": True,
        "jsonEncoding": "UTF8_COMPACT_SORTED_KEYS_STREAMED_FINAL_LF",
    ...
    sheet_asset = write_png_new(output / "POST2_R25_POSTFREEZE_SAME_ANGLE_NATIVE_COMPARISON.png", sheet)
    evaluation = {
        "schema": EVALUATION_SCHEMA,
        "state": "HOLD_R25_POSTFREEZE_OPERATOR_VISUAL_REVIEW_REQUIRED",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
    ...
    gate_payload = {
        "schema": "argos_ocv03_annular_candidate_first_r25_post_label_evaluation_gate_v1",
        "state": "HOLD_R25_OPERATOR_VISUAL_REVIEW_READY_NO_PACKAGE",
...
def run_self_check() -> int:
    ...
    need(
        ...
        "R25 file-backed output-footprint allowance and reserve do not close",
    )
    ...
    need(
        ...
        "R25 streamed compact JSON encoding differs from its exact logical byte contract",
    )
    for forbidden in (
        "cyclic_" + "open_1d(",
        ...
        need(forbidden not in source, f"Forbidden R25 candidate operation is present: {forbidden}")
    ...
    print(json.dumps({"state": "PASS_R25_MINIMAL_SOURCE_SELF_CHECK", "engineSha256": sha256_file(Path(__file__).resolve())}, separators=(",", ":")))
    return 0
...
def main() -> int:
    ...
    raise RuntimeError(f"Unknown R25 command: {args.command}")
