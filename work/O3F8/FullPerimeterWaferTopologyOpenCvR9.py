#!/usr/bin/env python3
"""R9 review-only split-method full-perimeter wafer-notch detector.
...
are not accepted.  When R8 has zero eligible pairs, R9 applies the frozen O3P8
DF-seeded local-BF corroboration method without changing either R8 channel.
"""
...
def o3p8_df_seeded_local_bf_recovery(
    ...
    for seed in seeds:
        ...
        width_passed, angle_difference, passed = O3P8.evaluate_candidate(bf, df_candidate, recovery_job)
        rows.append(
            {
                "seedId": seed["seedId"],
...
def validate_job(path: Path, output_root: Path, verify_source_metadata: bool) -> dict[str, Any]:
    ...
    require(max(len(str(leaf)) for leaf in planned_leaves) < 200, "Planned R9 output path is unsafe.")
    require(max(len(leaf.name) for leaf in planned_leaves) <= 80, "Planned R9 output component is unsafe.")
    crop = job["crop"]
...
def process_job(job_path: Path, output_root: Path) -> dict[str, Any]:
    ...
    for pair_id in sorted(grouped):
        ...
        if recovery is not None and recovery.get("selected") is not None:
            recovery["selected"]["assets"] = render_o3p8_recovery_assets(
                output_root, pair_id, bf_image, base_result["bf"]["fit"], crop, cfg, recovery["selected"]
            )
        ...
        results.append(
            {
                ...
                "selectedReviewOnlyO3p8DfSeededLocalBfNotch": None if recovery is None else recovery.get("selected"),
    ...
    manifest = {
        ...
        "channelMethods": {"BF": "TOP_CONNECTED_TOPOLOGY_FULL_360", "DF": "OUTER_EDGE_RADIAL_FULL_360"},
        "engineProvenance": {
            "r9Sha256": sha256_file(Path(__file__).resolve()),
