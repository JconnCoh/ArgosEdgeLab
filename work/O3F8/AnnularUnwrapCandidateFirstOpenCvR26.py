#!/usr/bin/env python3
"""R26 review-only successor to the held R25 candidate-first detector.
...
traces every candidate from channel-local native transition support, records
irregular responses without coercing them
...
EXPECTED_INFERENCE_ROOT = Path(r"C:\O3F16U26P2_CANDIDATE_DRAFT1")

TRACE_SCHEMA = "argos_ocv03_annular_candidate_first_r26_native_trace_v1"
POPULATION_SCHEMA = "argos_ocv03_annular_candidate_first_r26_neutral_population_v1"
SUMMARY_SCHEMA = "argos_ocv03_annular_candidate_first_r26_inference_v1"
FREEZE_SCHEMA = "argos_ocv03_annular_candidate_first_r26_freeze_manifest_v1"
GATE_SCHEMA = "argos_ocv03_annular_candidate_first_r26_inference_gate_v1"
EVALUATION_SCHEMA = "argos_ocv03_annular_candidate_first_r26_post_label_evaluation_v1"
...
CAPACITY_OBSERVED_R26_INTERRUPTION_BASIN_UPPER_BOUND = 1_163
CAPACITY_PROJECTED_R26_MAXIMUM_OUTPUT_BYTES = EXPECTED_MAXIMUM_LOGICAL_OUTPUT_BYTES
...
def require_output_write_reserve(path: Path, planned_logical_bytes: int) -> None:
    ...
    need(
        ...
        f"R26 output write would cross the 8 GiB safety reserve: {path}",
    )
...
def enable_fresh_output_compression(root: Path, free_bytes_before: int) -> dict[str, Any]:
    need(root.is_dir() and not any(root.iterdir()), "R26 compression target is not a fresh empty directory")
    ...
    need(
        ...
        "Pinned compact utility failed before R26 child output: "
        + completed.stderr[-512:].decode("utf-8", errors="replace"),
    )
    attributes = int(getattr(root.stat(), "st_file_attributes", 0))
    need(attributes & FILE_ATTRIBUTE_COMPRESSED, "Fresh R26 root lacks the required NTFS compressed attribute")
    need(not any(root.iterdir()), "Pinned compact utility created unexpected R26 root content")
    return {
        "state": "PASS_R26_FRESH_RESULT_ROOT_NTFS_COMPRESSION_ENABLED",
        "root": str(root),
        ...
        "capacityProjection": {
            "basis": "EXACT_FROZEN_R25_GENERATED_BYTES_AND_TRACE_COUNT_WITH_HASH_LOCKED_R26_INTERRUPTION_BASIN_PROBE_UPPER_BOUND",
            ...
            "observedR26InterruptionBasinUpperBound": CAPACITY_OBSERVED_R26_INTERRUPTION_BASIN_UPPER_BOUND,
            "r26MaximumLogicalOutputAllowanceBytes": CAPACITY_PROJECTED_R26_MAXIMUM_OUTPUT_BYTES,
...
def preflight_output_layout(root: Path, identities: Iterable[str], evaluation: bool = False) -> None:
    components = {
        ...
        "hotspot",
        "evaluation",
        "POST2_R26_POSTFREEZE_SAME_ANGLE_NATIVE_COMPARISON.png",
...
def configure_pinned_dependency_paths() -> None:
    ...
    for name, path in requested.items():
        ...
        if existing:
            need(Path(existing).resolve() == path.resolve(), f"{name} differs from the frozen R26 dependency path")
        os.environ[name] = str(path)


configure_pinned_dependency_paths()
require_exact_file(R21_PATH, R21_SHA256, "R21 engine")
SPEC = importlib.util.spec_from_file_location("argos_annular_r21_for_r26", R21_PATH)
...
def candidate_graph_trace(
    ...
    need(component.size > 0 and core_columns, "R26 candidate component is empty")
    ...
    base: dict[str, Any] = {
        ...
        "strictCoreSeedIds": list(component_info.get("strictCoreSeedIds", [])),
        ...
        base.update(
            {
                ...
                "rawComponentTransitionWitness": {
                    "population": "ALL_RENDERED_R18_NATIVE_TRANSITION_COMPONENT_NODES",
                    "count": int(point_native_raw.size),
                    "rawOutsideInMinimum": float(np.min(point_native_raw)),
                    "rawOutsideInP10": float(np.percentile(point_native_raw, 10.0)),
                    "rawOutsideInMedian": float(np.median(point_native_raw)),
                    ...
                    "directUnblurredRawOutsideInMinimum": float(np.min(point_direct_raw)),
                    "directUnblurredRawOutsideInP10": float(np.percentile(point_direct_raw, 10.0)),
                    "directUnblurredRawOutsideInMedian": float(np.median(point_direct_raw)),
                    "directUnblurredRawPolaritySupportedFraction": float(
                        np.mean(point_direct_raw >= float(r18.MINIMUM_RAW_POLARITY))
                    ),
                },
                "gradientNormalAlignmentPostContourMeasurement": {
                    "state": "NOT_REQUIRED_FOR_R26_NATIVE_RAW_POLARITY_CONTOUR",
                    "selectionRole": False,
                },
    ...
    manufactured = native_evidence_qualified and not morphology_reasons
    classification = (
    ...
    record = {
        **base,
        ...
        "pairingEligible": manufactured and not authority_reasons,
        ...
        "reachableShoulderPathCountSaturationValue": PATH_COUNT_SATURATION,
        "selectedTransitionWitness": {
            "population": "R18_RAW_AND_ENHANCED_NATIVE_TRANSITION_WITNESSES_WITH_DIRECT_UNBLURRED_DIAGNOSTIC",
            ...
            "directUnblurredRawOutsideInMinimum": float(np.min(selected_direct_raw)),
            "directUnblurredRawOutsideInP10": float(np.percentile(selected_direct_raw, 10.0)),
            "directUnblurredRawOutsideInMedian": float(np.median(selected_direct_raw)),
            "directUnblurredRawPolaritySupportedFraction": float(
                np.mean(selected_direct_raw >= float(r18.MINIMUM_RAW_POLARITY))
            ),
        },
        "gradientNormalAlignmentPostContourMeasurement": {
            "state": "NOT_REQUIRED_FOR_R26_NATIVE_RAW_POLARITY_CONTOUR",
...
def discover_raw_candidate_components(
    ...
    # R26 uses only the frozen strict threshold for seed lineage.  The lower
...
def analyze_native_strip(
    ...
    need(strip.dtype == np.uint8 and strip.ndim == 2, "R26 strip must be uint8 grayscale")
    need(offsets.ndim == 1 and offsets.size == strip.shape[0], "R26 strip/offset geometry differs")
    need(predecessor_measured.shape == (strip.shape[1],), "R26 predecessor hold width differs")
    ...
    need(inner_circle["radius"] > 0.0, "R26 inner circle radius is non-positive")
    inner_path = r18.circle_ray_offsets(base_fit, inner_circle)
    spacing_error = float(
        np.max(np.abs((circle["outerPath"] - inner_path) - r18.EDGE_ZONE_INWARD_PX))
    )
    need(spacing_error <= 1.0e-4, "R26 yellow circle is not exactly 20 px inward")
    ...
    return {
        ...
        "discovery": discovery,
        "evidence": {
            "algorithm": "R26_INTERRUPTION_BASIN_CHANNEL_LOCAL_NATIVE_SUPPORT_GRAPH",
            ...
            "directUnblurredRawContrastRole": "DIAGNOSTIC_TELEMETRY_NOT_COORDINATE_VETO",
...
def render_channel(
    ...
    need(bool(np.all(~changed | declared)), "R26 full overlay changed pixels outside declared masks")
    need(int(np.count_nonzero(normal_mask)) > 0, "R26 normal brightness trace rendered empty")
    stem = safe_stem(identity) + "_" + channel.lower()
    trace_context_asset = write_json_new(
        root / f"{stem}_trace_context.json",
        {
            "schema": "argos_ocv03_annular_candidate_first_r26_channel_trace_context_v1",
            "identity": identity,
            "channel": channel,
    ...
    for candidate in analysis["candidateTraces"]:
        ...
        trace_payload = {
            ...
            "candidateCoordinateEncoding": (
                "SPARSE_REPRESENTATIVE_PATH_AND_COMPLETE_SUPPORT_GRAPH_NATIVE_NODES_IN_CANDIDATE_RECORD"
            ),
...
def channel_summary(
    ...
    return {
        ...
        "r26DiagnosticCyanGeometryVerified": analysis["cyanGeometryVerified"],
        "r26DiagnosticCyanGeometryVerificationState": analysis["cyanGeometryVerificationState"],
...
def process_post2(
    ...
    for ordinal, member in enumerate(source_job["inputs"], 1):
        ...
        pair["r26DiagnosticStateBeforeInheritedAuthority"] = pair["state"]
        pair["r26DiagnosticResolvedPairsRemainUnowned"] = pair["resolvedPairs"]
        ...
        results.append(
            {
                "ordinal": ordinal,
                "identity": identity,
                "state": "DIAGNOSTIC_ONLY_R26_POST2_CANDIDATE_FIRST_CONTOURS_COMPLETE",
                "channels": rows,
                "pairDiagnostic": pair,
        ...
        results.append(
            {
                ...
                "safeId": identity,
                "state": "DIAGNOSTIC_ONLY_R26_HOTSPOT_CANDIDATE_FIRST_CONTOURS_COMPLETE",
                "channels": rows,
                "pairDiagnostic": pair,
            }
        )
...
def evaluate_hotspot_regression(
    ...
    for current, prior in zip(results, oracle["results"]):
        ...
        for channel in ("BF", "DF"):
            ...
            if old_accepted:
                new_channel["contourAuthorityState"] = "DIAGNOSTIC_ONLY_R26_NATIVE_CONTOUR_REGRESSION"
            else:
            ...
            new_channel["r26MeasurementCannotClearPredecessorR21Hold"] = True
            held_preserved = bool(
                old_accepted
                or (
            ...
            channel_rows[channel] = {
                ...
                "r26CyanGeometryVerified": bool(new_channel["cyanGeometryVerified"]),
    ...
    return {
        "state": (
            "PASS_R26_HOTSPOT_REGRESSION_WITH_EXISTING_HOLDS_RETAINED"
            if all(
            ...
            else "HOLD_R26_HOTSPOT_REGRESSION_DIFFERENCE"
        ),
        "caseCount": len(rows),
    ...
    try:
        ...
        for window_key in ("leftShoulderWindow", "rightShoulderWindow"):
            ...
            window_flags = np.asarray(
                [bool(entry.get("nativeNormalTraceSupported")) for entry in entries],
                dtype=bool,
            )
            ...
            if bool(window.get("passed")) != expected_window_pass:
                return False
        if not (
        ...
        evidence_qualified = bool(
            ...
            and float(candidate["rawSupportedFraction"]) >= MINIMUM_PATH_COVERAGE_FRACTION
        ...
        if not (
            ...
            return False
        if bool(candidate.get("pairingEligible")) and candidate.get("authorityHoldReasons"):
            return False
...
def post2_semantic_checks(results: list[dict[str, Any]]) -> dict[str, bool]:
    ...
    slot03_df_real = len(slot03_df_rows) == 1 and complete_native(slot03_df_rows[0])
    slot03_bf_real = len(slot03_bf_rows) == 1 and complete_native(slot03_bf_rows[0])
    ...
    slot17_df_real = len(slot17_df_rows) == 1 and complete_native(slot17_df_rows[0])
    ...
    slot17_broad_held = bool(
        len(slot17_broad) == 1
        and not slot17["pairDiagnostic"].get("r26DiagnosticResolvedPairsRemainUnowned")
...
def run_inference(args: argparse.Namespace) -> int:
    for supplied, expected, label in (
        ...
        need(str(supplied).upper() == expected, f"Supplied {label} pin differs from R26")
    ...
    need(code_root == PROJECT_ROOT.resolve(), "R26 code root differs from its exact checkout")
    need(
        output.is_absolute()
        ...
        "R26 inference output must be a fresh short C: root",
    )
    ...
    need(
        ...
        "Fresh R26 root lacks its 4 GiB logical-output allowance plus 8 GiB safety reserve",
    )
    output.mkdir()
    output_storage = enable_fresh_output_compression(output, free_bytes_before)
    neutral_root = output / "neutral"
    neutral_root.mkdir()
    need(directory_is_ntfs_compressed(neutral_root), "R26 neutral child did not inherit NTFS compression")
    ...
    freeze = {
        ...
        "state": "PASS_R26_COMPLETE_NEUTRAL_POPULATION_FILESET_FROZEN",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
    ...
    summary = {
        ...
        "state": "COMPLETE_DIAGNOSTIC_ONLY_R26_NEUTRAL_POPULATION_FROZEN",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
        "engine": file_record(Path(__file__).resolve()),
    ...
    gate = {
        "schema": GATE_SCHEMA,
        "state": (
            "PASS_R26_NEUTRAL_POPULATION_FROZEN_WITH_SEMANTIC_HOLDS_RETAINED"
            if evaluation_authorized
            else "HOLD_R26_NEUTRAL_POPULATION_PRELABEL_GATE_FAILURE"
        ),
...
def run_evaluation(args: argparse.Namespace) -> int:
    need(args.scorer_labels_sha256.upper() == SCORER_LABELS_SHA256, "Scorer-label pin differs from R26")
    output = Path(args.output_root)
    ...
    need(
        ...
        and not output.exists()
        and len(str(output)) + 128 < 200,
        "R26 evaluation output must be a fresh short C: child/root",
    )
    need(args.engine_sha256.upper() == sha256_file(Path(__file__).resolve()), "R26 source changed after inference freeze")
    need(
        directory_is_ntfs_compressed(EXPECTED_INFERENCE_ROOT),
        "Frozen R26 inference root lost its verified NTFS compressed attribute",
    )
    ...
    summary = load_json_pinned(summary_path, args.inference_summary_sha256, "R26 inference summary")
    gate = load_json_pinned(gate_path, args.inference_gate_sha256, "R26 inference gate")
    population = load_json_pinned(population_path, args.neutral_population_sha256, "R26 neutral population")
    freeze = load_json_pinned(freeze_path, args.freeze_manifest_sha256, "R26 freeze manifest")
    need(
        summary.get("schema") == SUMMARY_SCHEMA
        and summary.get("state") == "COMPLETE_DIAGNOSTIC_ONLY_R26_NEUTRAL_POPULATION_FROZEN"
        and summary.get("scorerLabelJsonParsed") is False
        and summary.get("candidatePopulationFrozenBeforeScorerLabelParse") is True,
        "R26 inference summary is not a clean pre-label freeze",
    )
    need(
        gate.get("schema") == GATE_SCHEMA
        and str(gate.get("state", "")).startswith("PASS_R26_NEUTRAL_POPULATION_FROZEN")
        ...
        "R26 inference integrity gate does not authorize post-freeze evaluation",
    )
    need(
        ...
        "R26 neutral population was not frozen before label evaluation",
    )
    need(
        freeze.get("schema") == FREEZE_SCHEMA
        and freeze.get("state") == "PASS_R26_COMPLETE_NEUTRAL_POPULATION_FILESET_FROZEN",
        "R26 freeze manifest is not PASS",
    )
    need(
        ...
        "R26 gate does not bind the supplied frozen files",
    )
    ...
    need(
        ...
        "R26 source changed during frozen-population rehash",
    )
    ...
    output.mkdir()
    need(directory_is_ntfs_compressed(output), "R26 evaluation child did not inherit NTFS compression")
    evaluation_storage = {
        "state": "PASS_R26_EVALUATION_CHILD_INHERITED_NTFS_COMPRESSION",
        "root": str(output),
        "directoryCompressedAttributeVerifiedBeforeChildWrites": True,
        "logicalFileBytesAndSha256UnaffectedByFilesystemCompression": True,
        "jsonEncoding": "UTF8_COMPACT_SORTED_KEYS_STREAMED_FINAL_LF",
    ...
    sheet_asset = write_png_new(output / "POST2_R26_POSTFREEZE_SAME_ANGLE_NATIVE_COMPARISON.png", sheet)
    evaluation = {
        "schema": EVALUATION_SCHEMA,
        "state": "HOLD_R26_POSTFREEZE_OPERATOR_VISUAL_REVIEW_REQUIRED",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
    ...
    gate_payload = {
        "schema": "argos_ocv03_annular_candidate_first_r26_post_label_evaluation_gate_v1",
        "state": "HOLD_R26_OPERATOR_VISUAL_REVIEW_READY_NO_PACKAGE",
...
def run_self_check() -> int:
    ...
    need(
        CAPACITY_PROJECTED_R26_MAXIMUM_OUTPUT_BYTES
        == EXPECTED_MAXIMUM_LOGICAL_OUTPUT_BYTES
        and CAPACITY_OBSERVED_R26_INTERRUPTION_BASIN_UPPER_BOUND
        ...
        "R26 file-backed output-footprint allowance and reserve do not close",
    )
    ...
    need(
        ...
        "R26 streamed compact JSON encoding differs from its exact logical byte contract",
    )
    for forbidden in (
        "cyclic_" + "open_1d(",
        ...
        need(forbidden not in source, f"Forbidden R26 candidate operation is present: {forbidden}")
    ...
    need(
        ...
        and candidate_metric_schema_valid(overflow_trace["record"]),
        "Synthetic unwrapped-overflow fallback did not preserve the full native component",
    )
    print(json.dumps({"state": "PASS_R26_MINIMAL_SOURCE_SELF_CHECK", "engineSha256": sha256_file(Path(__file__).resolve())}, separators=(",", ":")))
    return 0
...
def main() -> int:
    ...
    raise RuntimeError(f"Unknown R26 command: {args.command}")
