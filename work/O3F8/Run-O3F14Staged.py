#!/usr/bin/env python3
"""Run the frozen O3F8/R11 draft through the O3F14 GATE and aliased DEV6 stages."""
...
LOCAL_GATE = HERE / "Test-O3F14R11SeedAngles.py"
...
LOCAL_GATE_SHA256 = "03EBD2130C17FEA0DDDEB36F3A5E75DAA0014440734C7767690E2A47348435AA"
...
def alias_plan_projection(plans: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "state": "PASS_O3F14_ALL_SIX_ALIAS_PLANS_PREVALIDATED",
        "caseCount": len(cases),
...
def load_base() -> Any:
    ...
    if _BASE is None:
        spec = importlib.util.spec_from_file_location("argos_o3f14_frozen_runner_base", BASE_RUNNER)
...
def assert_package_pins() -> None:
    for path, expected, label in (
        ...
        (LOCAL_GATE, LOCAL_GATE_SHA256, "R11 focused seed-angle regression"),
...
def self_test() -> None:
    ...
    print('{"state":"PASS_O3F14_STAGED_RUNNER_SELF_TEST","mutationsPerformed":false}')
...
def run_gate(output_root: Path) -> dict[str, Any]:
    ...
    local_output = output_root / "R11_SEED_ANGLE_GATE.json"
    synthetic_root = output_root / "R11_INHERITED_SYNTHETIC"
    ...
    commands = [
        ...
        execute(
            "INHERITED",
            [str(RUNTIME), "-I", "-B", str(R11), "--synthetic-gate", "--output-root", str(synthetic_root)],
            360,
            output_root,
            env,
        ),
    ...
    passed = (
        ...
        and local.get("state") == "PASS_O3F14_R11_SEED_ANGLE_REGRESSION"
        and local.get("r10Sha256") == "0EEEE7A396E918AF44082EC0930871A6A94C1FBB59D3F5CEF1AC34D1273745FA"
        and local.get("r11Sha256") == R11_SHA256
        and local.get("testSha256") == LOCAL_GATE_SHA256
        and local.get("anglePaths")
        == {
            "physicalSeedClassExact": True,
            "physicalAnglesExact": True,
            "dfOnlySeedClassExact": True,
            "dfOnlyAnglesExact": True,
            "bothPathsExecuteWithoutLegacyKeyError": True,
        }
        and local.get("decisionSemantics")
        == {
            "physicalUniquePositivePasses": True,
            "dfOnlyUniquePositivePasses": True,
            "twoDistinctEligibleSeedsRemainAmbiguous": True,
            "ambiguousClassesPreserved": True,
            "ineligibleSeedRemainsNegative": True,
        }
    ...
    summary = {
        "schema": "argos_ocv03_o3f14_r11_gate_result_v1",
        "state": "COMPLETE_O3F14_GATE" if passed else "HOLD_O3F14_GATE",
        "stage": "GATE",
...
def validate_gate(path: Path | None, expected_sha256: str | None) -> dict[str, Any]:
    ...
    need(
        gate.get("schema") == "argos_ocv03_o3f14_r11_gate_result_v1"
        and gate.get("state") == "COMPLETE_O3F14_GATE"
        ...
        and gate.get("r11Sha256") == R11_SHA256
    ...
    local_path = path.parent / "R11_SEED_ANGLE_GATE.json"
    inherited_path = path.parent / "R11_INHERITED_SYNTHETIC" / "SYNTHETIC_GATE.json"
...
def result_projection(observed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "r11Invoked": bool(symmetric["invoked"]),
...
def run_aliased_r11_child(
    ...
    job["revision"] = f"O3F14_R11_DEV6_{ordinal:04d}"
    job["inputs"] = [
        {
    ...
    with owned_case_alias(plan, alias_evidence):
        ...
        child = subprocess.run(
            [str(RUNTIME), "-I", "-B", str(R11), "--run", "--job", str(job_path), "--output-root", str(case_root)],
            capture_output=True,
            text=True,
            timeout=600,
            env=env,
...
def run_dev6(
    ...
    expected_provenance = {
        # R11 is a four-field-forwarding patch. Its frozen detector-manifest
        # schema retains the inherited R10 key; the O3F14 runner projects the
        # actual R11 hash under fresh r11Sha256 fields in its own results.
        "r10Sha256": R11_SHA256,
        "r9PredecessorSha256": R9_SHA256,
        "r8PredecessorSha256": R8_SHA256,
        "r6Sha256": R6_SHA256,
        "topologySha256": TOPOLOGY_SHA256,
    ...
    for ordinal, row in enumerate(selected, 1):
        ...
        try:
            ...
            job, job_path, case_root, child = run_aliased_r11_child(
                ordinal,
                row,
                canonical_fixed,
                jobs,
            ...
            need(child.returncode == 0, f"R11 child exit {child.returncode}: {child.stderr[-1200:]}")
            manifest_path = case_root / "MANIFEST.json"
            manifest, manifest_sha = read_json_and_sha256(manifest_path)
            need(isinstance(manifest.get("results"), list) and len(manifest["results"]) == 1, "R11 manifest is not one pair")
            observed = manifest["results"][0]
            need(str(observed["pairId"]) == safe_id, "R11 result pair identity changed")
            need(str(observed["baselineR8State"]) == str(row["r8State"]), "R11 baseline state differs from frozen R8 state")
            need(
                base.r8_decision_projection(observed) == base.r8_decision_projection(prior["priorResult"]),
                "R11 changed inherited R8 decision evidence",
            )
            need(str(manifest["revision"]) == job["revision"] and int(manifest["inputCount"]) == 2, "R11 manifest binding changed")
            need(manifest.get("engineProvenance") == expected_provenance, "R11 engine provenance changed")
            need(sha256(job_path) == str(manifest["jobSha256"]).upper(), "R11 manifest job hash changed")
            need(Path(str(manifest["jobPath"])).resolve(strict=False) == job_path.resolve(strict=False), "R11 manifest job path changed")
            need(manifest.get("sourceMutationPerformed") is False and manifest.get("providerActivated") is False, "R11 exceeded review-only authority")
            ...
            detailed = {
                ...
                "finalState": "HOLD_O3F14_R11_PROVIDER_ERROR",
                "r11Invoked": False,
                ...
                "eligibleHypotheses": [],
                "execution": "HOLD_R11_CHILD",
                "error": error,
            }
            results.append(detailed)
            compact_results.append(dict(detailed))
    counts = Counter(str(row["finalState"]) for row in results)
    executed_count = sum(row["execution"] == "PASS_R11_CHILD" for row in results)
    new_hold_count = sum(row["execution"] == "HOLD_R11_CHILD" for row in results)
    ...
    compact_alias_evidence = {
        "state": (
            "COMPLETE_O3F14_Q_ALIAS_LIFECYCLE"
            if alias_lifecycle_complete
            else "HOLD_O3F14_Q_ALIAS_LIFECYCLE_INCOMPLETE"
        ),
        "aliasDrive": ALIAS_DRIVE,
        "aliasAnchorKind": "CASE_SLOT_ROOT",
        "sourceAliasPlanSha256": SOURCE_ALIAS_PLAN_SHA256,
    ...
    summary = {
        ...
        "state": "COMPLETE_O3F14_DEV6" if clean_completion else "HOLD_O3F14_DEV6_EXECUTION",
        "stage": "DEV6",
        "runnerSha256": sha256(Path(__file__).resolve()),
        "selectedCount": 6,
...
def main() -> int:
    ...
    if args.stage == "GATE":
        ...
        return 0 if result["state"] == "COMPLETE_O3F14_GATE" else 2
    result = run_dev6(
    ...
    print(json.dumps(result, separators=(",", ":")))
    return 0 if result["state"] == "COMPLETE_O3F14_DEV6" else 2
