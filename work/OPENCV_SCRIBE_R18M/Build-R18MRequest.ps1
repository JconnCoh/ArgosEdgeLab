...
function Require-Pin([string]$Path, [string]$Sha256, [string]$State = '') {
    Require (Test-Path -LiteralPath $Path -PathType Leaf) "R18M build dependency absent: $Path"
    Require ((Get-Sha256 $Path) -eq $Sha256) "R18M build dependency changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($State)) {
        $value = Get-Content -Raw -LiteralPath $Path | ConvertFrom-Json
        Require ([string]$value.state -eq $State) "R18M build dependency state changed: $Path"
    }
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 32) {
    Require (-not (Test-Path -LiteralPath $Path)) "R18M build create-new JSON exists: $Path"
...
$revision = 'R18M_FULL_KLARF_JBOD_REVIEW_ONLY_20260904A'
$payloadRevision = 'R18J_FULL_KLARF_JBOD_REVIEW_ONLY_20260904A'
$entrypoint = Join-Path $PSScriptRoot 'Invoke-R18MCorpusLaunch.ps1'
...
$pathGatePath = Join-Path $PSScriptRoot 'R18M_PATH_GATE.json'
...
$finalGatePath = Join-Path $PSScriptRoot 'R18M_FINAL_PACKAGE_GATE.json'
...
$entrypointSha = '52249A9DF21D0B9D655A970B6E098771EB4838B8F56F56BCBFA27685591A7C54'
$payloadManifestSha = 'BF2D0E809915ADEB503E2B7ADEFB3DCAC30903025D851B26067C9FB5B90DC57D'
$definitionSha = '162701F3D7CF286C6AD82B08755F7A2D5005806663F6B2837974FD902760603C'
...
Require (Test-Path -LiteralPath $localSource -PathType Container) 'R18M local rehearsal source absent.'
Require (Test-Path -LiteralPath $localProposals -PathType Container) 'R18M local rehearsal proposal root absent.'
...
Require ([string]$preactionResult.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'R18M preaction gate changed.'
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Require ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH' -and [string]$definition.entryPoint -eq 'payload/Invoke-R18MCorpusLaunch.ps1') 'R18M maintenance route changed.'
Require (@($definition.changes).Count -eq 1 -and [string]$definition.changes[0].installedSha256 -eq $entrypointSha -and [bool]$definition.changes[0].allowCreate) 'R18M maintenance change changed.'
Require (@($definition.changes[0].approvedPredecessorSha256).Count -eq 1 -and [string]$definition.changes[0].approvedPredecessorSha256[0] -eq $entrypointSha) 'R18M create-only idempotent installed-hash boundary changed.'
Require (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R18M process/task action cardinality changed.'
Require ([string]$definition.sourceProcessingContract.proposalRoot -eq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals') 'R18M observed proposal root changed.'
Require ([bool]$definition.reviewOnly -and -not [bool]$definition.productionRoutingEnabled -and -not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed) 'R18M authority changed.'
$payloadManifest = Get-Content -Raw -LiteralPath $payloadManifestPath | ConvertFrom-Json
Require ([string]$payloadManifest.schema -eq 'argos_opencv_scribe_r18j_payload_manifest_v1' -and [string]$payloadManifest.revision -eq $payloadRevision -and @($payloadManifest.files).Count -eq 23) 'R18M inherited payload manifest shape changed.'
...
foreach ($file in $payloadFiles) {
    ...
    Require ((Get-Item -LiteralPath $source).Length -eq [int64]$file.bytes) "R18M payload length changed: $($file.sourcePath)"
}
...
try {
    ...
    Require ($matches.Count -eq 1 -and $matches[0].HasPrivateKey) 'R18M signer certificate or private key changed.'
...
foreach ($path in @($stageRoot,$readyRoot,$stageZip,$verifyRoot,$finalRoot,$finalPartial,$finalGatePath)) {
    Require (-not (Test-Path -LiteralPath $path)) "R18M build fresh output exists: $path"
}
if ($Preflight) {
    [ordered]@{schema='argos_opencv_scribe_r18m_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R18M_BUILD_PREFLIGHT';requestId=$requestId;payloadFileCount=25;entrypointSha256=$entrypointSha;payloadManifestSha256=$payloadManifestSha;definitionSha256=$definitionSha;localGateSha256=$localGateSha;pathGateSha256=$pathGateSha;preactionSha256=$preactionSha;signerThumbprint=$thumbprint;mutationsPerformed=$false;targetExecuted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}

[void](New-Item -ItemType Directory -Path (Join-Path $readyRoot 'payload\files'))
Copy-Item -LiteralPath $entrypoint -Destination (Join-Path $readyRoot 'payload\Invoke-R18MCorpusLaunch.ps1')
...
$manifestFiles.Add([ordered]@{path='payload/Invoke-R18MCorpusLaunch.ps1';bytes=[int64](Get-Item -LiteralPath $entrypoint).Length;sha256=$entrypointSha})
...
Require ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18M signed package verification failed.'
...
Require ($extracted.Count -eq 27) 'R18M extracted final ZIP file count changed.'
foreach ($row in $manifestFiles) {
    $path = Join-Path $verifyRoot ([string]$row.path).Replace('/','\')
    Require-Pin $path ([string]$row.sha256)
    Require ((Get-Item -LiteralPath $path).Length -eq [int64]$row.bytes) "R18M extracted payload length changed: $($row.path)"
}
...
Require ([string]$exactPackageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18M extracted signed package verification failed.'
$packagedPreflight = (& (Join-Path $verifyRoot 'payload\Invoke-R18MCorpusLaunch.ps1') -Preflight -Rehearsal -PayloadRoot (Join-Path $verifyRoot 'payload') -WorkRoot 'C:\R18MFW' -OutputRoot 'C:\R18MFO' -SourceRoot $localSource -ProposalRoot $localProposals -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localRefs -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
Require ([string]$packagedPreflight.state -eq 'PASS_R18J_CORPUS_LAUNCH_PREFLIGHT' -and -not [bool]$packagedPreflight.mutationsPerformed -and -not [bool]$packagedPreflight.processStarted) 'R18M exact packaged entrypoint preflight failed.'
...
$finalGate = [ordered]@{
    schema='argos_opencv_scribe_r18m_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
    state='PASS_R18M_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip=('work/OPENCV_SCRIBE_R18M/final/'+$zipName);
