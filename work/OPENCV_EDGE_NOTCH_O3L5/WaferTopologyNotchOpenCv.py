#!/usr/bin/env python3
"""Top-connected wafer topology and outer-contour notch localization."""
...
def overlay(enhanced: np.ndarray, edge: np.ndarray, unindented: np.ndarray, found: list[dict[str, Any]], state: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for rank, item in enumerate(found[:3]):
        ...
        cv2.line(out, (center, 0), (center, height - 1), color, 3 if rank == 0 else 2, cv2.LINE_8)
        cv2.line(mask, (center, 0), (center, height - 1), 255, 3 if rank == 0 else 2, cv2.LINE_8)
        cv2.drawMarker(out, (tip, int(round(edge[tip]))), (255, 0, 255), cv2.MARKER_CROSS, 18, 3, cv2.LINE_8)
        cv2.drawMarker(mask, (tip, int(round(edge[tip]))), 255, cv2.MARKER_CROSS, 18, 3, cv2.LINE_8)
    legend = "RED notch mouth center | MAGENTA tip | CYAN top-connected wafer contour | GREEN baseline"
...
def validate(job_path: Path, output_root: Path) -> tuple[dict[str, Any], Path]:
    ...
    require(job.get("schema") == "argos_ocv03_wafer_topology_notch_job_v1" and bool(job.get("reviewOnly")), "Job schema/authority changed.")
    ...
    cfg = job["config"]
    require(float(cfg["minimumNotchDepthPx"]) == 7.0 and float(cfg["waferDistanceThreshold"]) >= 2.5, "Decision gates weakened.")
    return job, root
...
def process(job_path: Path, output_root: Path) -> dict[str, Any]:
    ...
    for row in sorted(job["inputs"], key=lambda item: str(item["id"])):
        ...
        elif len(found) > 1 and float(found[1]["score"]) >= float(cfg["ambiguityScoreRatio"]) * float(found[0]["score"]):
        ...
        for item in found:
            ...
            ci = int(round(center))
            item["mouthCenterAngleDegrees"] = angle(center, float(edge[ci]), 1000, float(row["inwardY"]), float(row["radiusPx"]), float(row["cropCenterAngleDegrees"]))
            item["tipAngleDegrees"] = angle(float(tip), float(edge[tip]), 1000, float(row["inwardY"]), float(row["radiusPx"]), float(row["cropCenterAngleDegrees"]))
    ...
    manifest = {
        "schema": "argos_ocv03_wafer_topology_notch_manifest_v1", "revision": str(job["revision"]), "state": "PASS_O3L5_WAFER_TOPOLOGY_REVIEW_RENDERED",
        "topConnectedWaferTopologyUsed": True, "internalDieHolesFilled": True, "jsonCandidateCenterConsumed": False, "thresholdRelaxationPerformed": False,
    ...
    return {"schema": "argos_ocv03_wafer_topology_notch_command_v1", "state": manifest["state"], "manifest": str(manifest_path), "manifestSha256": digest(manifest_path), "imageInputCount": 6, "assetFileCount": 24, "imageBytesEmittedToStdout": False, "reviewOnly": True}
...
def main() -> int:
    ...
    result = {
        "schema": "argos_ocv03_wafer_topology_notch_preflight_v1", "state": "PASS_O3L5_WAFER_TOPOLOGY_PREFLIGHT", "revision": str(job["revision"]),
