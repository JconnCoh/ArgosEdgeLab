...
$expectedProcessorCreatedUtc = [DateTime]::Parse('2026-08-25T15:38:27.716Z').ToUniversalTime()
...
try {
    ...
    if ($residentBefore.Count -ne 3) { throw "Endpoint/response resident process cardinality changed before action: $($residentBefore.Count)" }
    ...
    do {
        ...
        if ($receiverTaskAfter.state -eq 'Running' -and $receiverAfter.Count -eq 2 -and $listenerAfter.Count -eq 1) { break }
    } while ([DateTime]::UtcNow -lt $deadline)

    if ($receiverTaskAfter.state -ne 'Running') { throw "Receiver task failed to remain Running: $($receiverTaskAfter.state)" }
    if ($receiverAfter.Count -ne 2) { throw "Receiver process cardinality after start is $($receiverAfter.Count), expected 2." }
