...
function Write-TerminalResult {
    ...
    [pscustomobject]@{
        schema = 'argos_o2d10_argos_tunnel_terminal_v2'
...
if ([string]$invocation.schema -ne 'argos_o2d10_argos_tunnel_invocation_v2') {
...
if (-not [bool]$invocation.requireWsManIdentify) {
    throw 'WS-Man Identify may not be disabled.'
}
...
foreach ($requiredCommand in @('Get-NetTCPConnection', 'Get-CimInstance', 'Start-Process', 'Stop-Process', 'Test-WSMan')) {
...
if ($Preflight) {
    Write-TerminalResult -State 'PASS_O2D10_ARGOS_TUNNEL_R2_PREFLIGHT' -MutationsPerformed $false -Detail ([pscustomobject]@{
...
try {
    ...
    $identify = Test-WSMan -ComputerName '127.0.0.1' -Port $localPort -ErrorAction Stop
    if ([string]::IsNullOrWhiteSpace([string]$identify.ProtocolVersion) -or
        [string]::IsNullOrWhiteSpace([string]$identify.ProductVendor) -or
        [string]::IsNullOrWhiteSpace([string]$identify.ProductVersion)) {
        throw 'Fresh tunnel WS-Man Identify response is incomplete.'
    }

    Write-TerminalResult -State 'PASS_O2D10_ARGOS_TUNNEL_R2_STARTED_AND_WSMAN_IDENTIFIED' -MutationsPerformed $true -Detail ([pscustomobject]@{
        tunnelProcessId = [int]$newTunnelRows[0].ProcessId
        tunnelCommandLine = [string]$newTunnelRows[0].CommandLine
        localPort = $localPort
        targetHost = $targetHost
        targetPort = $targetPort
        wsManProtocolVersion = [string]$identify.ProtocolVersion
        wsManProductVendor = [string]$identify.ProductVendor
        wsManProductVersion = [string]$identify.ProductVersion
    ...
    Write-TerminalResult -State 'FAIL_O2D10_ARGOS_TUNNEL_R2_START_OR_WSMAN_ROLLED_BACK' -MutationsPerformed ([bool]($rollbackIds.Count -gt 0)) -Detail ([pscustomobject]@{
        message = $_.Exception.Message
        rolledBackProcessIds = $rollbackIds
        newPortListenerRemaining = [bool](@(Get-NetTCPConnection -State Listen -LocalPort $localPort -ErrorAction SilentlyContinue).Count -gt 0)
        predecessorPortPreserved = [bool](@(Get-NetTCPConnection -State Listen -LocalPort $preservedPredecessorPort -ErrorAction SilentlyContinue).Count -eq 1)
    })
    exit 1
}
