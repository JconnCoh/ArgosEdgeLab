...
Require ($root -ceq 'C:\O3F15L4D2PK') 'O3F15L4D2 package root changed.'

$contractPath = Join-Path $d2 'O3F15L4D2_DIAGNOSTIC_CONTRACT.json'
$definitionPath = Join-Path $d2 'MAINTENANCE_DEFINITION.json'
$intentPath = Join-Path $d2 'O3F15L4D2_RECOVERY_INTENT.json'
$intentGatePath = Join-Path $d2 'O3F15L4D2_RECOVERY_INTENT_GATE.json'
$routeInventoryPath = Join-Path $d2 'O3F15L4D2_ROUTE_CAPABILITY_INVENTORY.json'
...
Require ([string]$definition.schema -ceq 'argos_maintenance_definition_v1' -and [string]$definition.revision -ceq 'O3F15L4D2') 'D2 maintenance definition identity changed.'
$isDraft = [string]$contract.lifecycle -ceq 'DRAFT' -and [string]$contract.state -ceq 'DRAFT_O3F15L4D2_METADATA_DIAGNOSTIC_CONTRACT' -and [string]$definition.lifecycle -ceq 'DRAFT' -and [string]$definition.state -ceq 'DRAFT_O3F15L4D2_MAINTENANCE_DEFINITION'
$isFrozen = [string]$contract.lifecycle -ceq 'FROZEN' -and [string]$contract.state -ceq 'FROZEN_O3F15L4D2_METADATA_DIAGNOSTIC_CONTRACT' -and [string]$definition.lifecycle -ceq 'FROZEN' -and [string]$definition.state -ceq 'FROZEN_O3F15L4D2_MAINTENANCE_DEFINITION'
...
Require ([string]$definition.entrypoint -ceq 'Invoke-O3F15L4D2.ps1' -and [int]$definition.timeoutSeconds -eq 630 -and [int64]$definition.maximumResultBytes -eq 8388608) 'D2 maintenance execution bounds changed.'
...
$sourceMap = [ordered]@{
    'Invoke-O3F15L4D2.ps1' = 'work/OPENCV_EDGE_NOTCH_O3F15L4D2/Invoke-O3F15L4D2.ps1'
    'O3F15L4D2DiagnosticFixture.py' = 'work/OPENCV_EDGE_NOTCH_O3F15L4D2/O3F15L4D2DiagnosticFixture.py'
    'Run-O3F15L4FrontReconcile.py' = 'work/OPENCV_EDGE_NOTCH_O3F15L4/Run-O3F15L4FrontReconcile.py'
    'Test-O3F15L4PathHolds.py' = 'work/OPENCV_EDGE_NOTCH_O3F15L4/Test-O3F15L4PathHolds.py'
...
$sources = @($sourceRows.ToArray()) + @([pscustomobject]@{ name='O3F15L4D2_DIAGNOSTIC_CONTRACT.json'; source=$contractPath; bytes=[int64](Get-Item -LiteralPath $contractPath).Length; sha256=$contractHash })
...
$buildGateTarget = Join-Path $partial 'O3F15L4D2_BUILD_GATE.json'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_ocv03_o3f15l4d2_build_preflight_v1'
        state=$(if ($isFrozen) { 'PASS_O3F15L4D2_BUILD_PREFLIGHT_FROZEN' } else { 'PASS_O3F15L4D2_BUILD_PREFLIGHT_DRAFT' })
...
$gate = [ordered]@{
    schema='argos_ocv03_o3f15l4d2_build_gate_v1'
    createdUtc=[DateTime]::UtcNow.ToString('o')
    state='PASS_O3F15L4D2_UNSIGNED_METADATA_DIAGNOSTIC_PACKAGE_BUILT'
    lifecycle='FROZEN'
    buildCount=1
    buildRoot=$ready
    payloadPinCount=17
