...
$buildGatePath = Join-Path $buildRoot 'O3F15L4D2_BUILD_GATE.json'
...
Require ([string]$buildGate.state -ceq 'PASS_O3F15L4D2_UNSIGNED_METADATA_DIAGNOSTIC_PACKAGE_BUILT' -and [int]$buildGate.buildCount -eq 1 -and [int]$buildGate.payloadPinCount -eq 17 -and [int]$buildGate.payloadFileCount -eq 18) 'D2 build evidence changed.'
Require ([string]$contract.lifecycle -ceq 'FROZEN' -and [string]$contract.state -ceq 'FROZEN_O3F15L4D2_METADATA_DIAGNOSTIC_CONTRACT') 'D2 signing requires the once-frozen contract.'
Require ([string]$definition.lifecycle -ceq 'FROZEN' -and [string]$definition.state -ceq 'FROZEN_O3F15L4D2_MAINTENANCE_DEFINITION') 'D2 signing requires the once-frozen definition.'
...
Require ([string]$definition.entrypoint -ceq 'Invoke-O3F15L4D2.ps1' -and [int]$definition.timeoutSeconds -eq 630 -and -not [bool]$definition.automaticRetry) 'D2 signed entrypoint/timeout/retry contract changed.'
...
$finalRoot = Join-Path $PSScriptRoot 'final_o3f15l4d2'
$plannedZip = Join-Path $finalRoot ($plannedRequestId + '.ready.zip')
$signGatePath = Join-Path $finalRoot 'O3F15L4D2_SIGN_GATE.json'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_ocv03_o3f15l4d2_sign_preflight_v1'
        state='PASS_O3F15L4D2_SIGN_PREFLIGHT'
...
$gate = [ordered]@{
    schema='argos_ocv03_o3f15l4d2_sign_gate_v1'
    createdUtc=[DateTime]::UtcNow.ToString('o')
    state='PASS_O3F15L4D2_SIGNED_METADATA_DIAGNOSTIC_PACKAGE'
    requestId=$requestId
