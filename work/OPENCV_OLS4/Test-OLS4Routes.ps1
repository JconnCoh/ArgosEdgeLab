...
if([string]$invocation.schema-ne'argos_ols4_route_invocation_v1'){throw 'OLS4 route invocation schema mismatch.'}
...
if($paths.Count-lt30-or$paths.Count-gt128-or@($paths|Sort-Object -Unique).Count-ne$paths.Count){throw 'OLS4 route path set is incomplete or duplicated.'}
$rows=New-Object Collections.Generic.List[object]
foreach($path in $paths){
    $one=& $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson|ConvertFrom-Json
    if([string]$one.state-ne'PASS_PATH_BUDGET'-or@($one.candidates).Count-ne1){throw "OLS4 route path budget failed: $path"}
...
$result=[ordered]@{
    schema='argos_ols4_complete_route_gate_v1'
    createdUtc=[DateTime]::UtcNow.ToString('o')
    state=if($Preflight){'PASS_OLS4_COMPLETE_ROUTE_PREFLIGHT'}else{'PASS_OLS4_COMPLETE_ROUTE_GATE'}
...
foreach($property in @('requestZipPath','requestManifestPath','requestSignaturePath')){$path=[IO.Path]::GetFullPath([string]$invocation.$property);if(-not(Test-Path -LiteralPath $path -PathType Leaf)){throw "OLS4 exact route artifact is missing: $path"}}
...
if(-not$result.exactFinalZipExtractionPassed-or-not$result.exactFinalZipSignaturePassed){throw 'OLS4 exact final ZIP verification is not PASS.'}
$outputPath=[IO.Path]::GetFullPath([string]$invocation.outputPath)
if(Test-Path -LiteralPath $outputPath){throw 'OLS4 route gate output already exists.'}
