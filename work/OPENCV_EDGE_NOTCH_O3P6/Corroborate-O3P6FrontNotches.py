#!/usr/bin/env python3
"""Corroborate full-360 R6 seeds while isolating candidate-local contour insufficiency."""
...
JOB_SCHEMA = "argos_ocv03_o3p6_front_notch_corroboration_job_v1"
...
def load_job(path: Path) -> tuple[dict[str, Any], Path, Path, Path, Path, Path]:
    ...
    require(job.get("schema") == JOB_SCHEMA, "O3P6 job schema changed.")
    for field in (
        "trainingEligible",
        "xmlEligible",
        "productionEligible",
        ...
        require(job.get(field) is False, f"Forbidden O3P6 authority changed: {field}")
    require(job.get("reviewOnly") is True, "O3P6 must remain review-only.")
    require(
        job.get("candidateLocalTopologyErrors") == sorted(CANDIDATE_LOCAL_TOPOLOGY_ERRORS),
        "O3P6 candidate-local exception contract changed.",
    )
    ...
    require(runtime_root.is_dir() and output_path.parent.is_dir(), "O3P6 runtime or output parent is absent.")
    require(
        not output_path.exists() and not output_path.with_name(output_path.name + ".partial").exists(),
        "O3P6 output must be create-new.",
    )
    for record, label in (
        ...
        require(sha256_file(resolved) == str(record["sha256"]).upper(), f"O3P6 {label} changed.")
    require(
        json.loads(runtime_gate.read_text(encoding="utf-8")).get("state") == "PASS_O3P2_LOCAL_RUNTIME_INSTALLED",
        "O3P6 runtime gate is not PASS.",
    )
    module_under_root(str(cv2.__file__), runtime_root, "OpenCV")
    module_under_root(str(np.__file__), runtime_root, "NumPy")
    require(cv2.__version__ == str(job["expectedOpenCvVersion"]), "O3P6 OpenCV version changed.")
    require(np.__version__ == str(job["expectedNumpyVersion"]), "O3P6 NumPy version changed.")
...
def rejected_channel(seed: dict[str, Any], channel: str, message: str) -> dict[str, Any]:
    candidate = seed[f"{channel.lower()}Candidate"]
    return {
        "channel": channel,
        "cropCenterAngleDegrees": float(seed["seedAngleDegrees"]) if candidate is None else float(candidate["centerAngleDegrees"]),
        ...
        "feature": None,
        "topologyToRadialDepthRatio": None,
        "channelCorroborated": False,
...
def refine_channel(
    ...
    require(clean.shape == (inward + outward, width), "O3P6 crop shape changed.")
    ...
    try:
        ...
        if message not in CANDIDATE_LOCAL_TOPOLOGY_ERRORS:
            raise
        return rejected_channel(seed, channel, message)
    complete = (
...
def process_identity(row: dict[str, Any], topology: Any, renderer: Any, job: dict[str, Any]) -> dict[str, Any]:
    ...
    require(1 <= len(seeds) <= int(job["corroboration"]["maximumSeedCountPerWafer"]), "O3P6 seed count is outside the bound.")
    ...
    for channel in ("BF", "DF"):
        ...
        require(source_path.stat().st_size == int(source["bytes"]), f"O3P6 {channel} byte count changed: {source_path}")
        require(sha256_file(source_path) == str(source["sha256"]).upper(), f"O3P6 {channel} source hash changed: {source_path}")
        image = cv2.imread(str(source_path), cv2.IMREAD_GRAYSCALE)
        require(image is not None, f"O3P6 {channel} decode failed: {source_path}")
        channel_result = seed_result[channel.lower()]
        require(image.shape == (int(channel_result["heightPx"]), int(channel_result["widthPx"])), f"O3P6 {channel} dimensions changed.")
        for seed in seeds:
            refinements[seed["seedId"]][channel.lower()] = refine_channel(
                image,
                dict(channel_result["fit"]),
                seed,
                channel,
                topology,
                renderer,
    ...
    for seed in seeds:
        ...
        passed = (
            ...
            and angle_difference <= float(job["corroboration"]["maximumBfDfRefinedAngleDifferenceDegrees"])
        )
        result = {
            "seedId": seed["seedId"],
    ...
    return {
        ...
        "candidateLocalTopologyInsufficiencyCount": sum(
            ...
            for channel in ("bf", "df")
            if result[channel]["topologyObservationState"] == "REJECTED_CANDIDATE_LOCAL_TOPOLOGY_INSUFFICIENCY"
        ),
        "selected": selected,
        "seeds": output_rows,
...
def run(job_path: Path, job: dict[str, Any], runtime_root: Path, topology_path: Path, renderer_path: Path, output_path: Path) -> dict[str, Any]:
    topology = load_module("argos_o3p6_topology", topology_path)
    renderer = load_module("argos_o3p6_crop", renderer_path)
    rows = [process_identity(row, topology, renderer, job) for row in job["inputs"]]
    require(len(rows) == int(job["expectedInputCount"]), "O3P6 input count changed.")
    result = {
        "schema": "argos_ocv03_o3p6_front_notch_corroboration_result_v1",
        "revision": str(job["revision"]),
        "createdUtc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "state": "COMPLETE_O3P6_POST2_TOPOLOGY_CORROBORATION_REVIEW_ONLY",
        ...
        "candidateLocalTopologyErrors": sorted(CANDIDATE_LOCAL_TOPOLOGY_ERRORS),
    ...
    return {
        ...
        "candidateLocalTopologyInsufficiencyCount": sum(row["candidateLocalTopologyInsufficiencyCount"] for row in rows),
    }
...
def main() -> int:
    ...
    for row in job["inputs"]:
        ...
        for channel in ("bf", "df"):
            ...
            require(source.stat().st_size == int(row[channel]["bytes"]), f"O3P6 source byte count changed: {source}")
            require(sha256_file(source) == str(row[channel]["sha256"]).upper(), f"O3P6 source hash changed: {source}")
    if arguments.preflight:
        result = {
            "state": "PASS_O3P6_FRONT_NOTCH_CORROBORATION_PREFLIGHT",
            "jobSha256": sha256_file(job_path),
