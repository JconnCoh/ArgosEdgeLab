...
$endpoint = Join-Path $PSScriptRoot 'Invoke-O3D3HotspotEndpoint.ps1'
$generator = Join-Path $PSScriptRoot 'New-O3D3RehearsalFixtures.py'
$payloadRoot = Join-Path $project 'work\PATTERNED_FIDUCIAL_INVENTORY\tools'
$job = Join-Path $PSScriptRoot 'O3D3_HOTSPOT_JOB.json'
$fixtureManifest = Join-Path $PSScriptRoot 'O3D3_LIVE_CONTRACT_FIXTURE.json'
...
$workRoot = 'C:\A35\w'
$outputRoot = 'C:\A35\o'
$badWorkRoot = 'C:\A35\bw'
$badOutputRoot = 'C:\A35\bo'
$gatePath = Join-Path $PSScriptRoot 'O3D3_ENDPOINT_REHEARSAL_GATE.json'

$endpointSha = 'AF51CBAEC86F70B2427415B1301A64177C90732D097089031A8B459C6166A1C2'
$generatorSha = '49B5B802D7F2A65932F5B2A3281DEE3986764FD7DD3262FC6986FAD9F6E1B6B2'
$jobSha = '4F7C5809CFB16D9D120196331CC31EF6F4C4B0CF43EE70379591E61392DD88E2'
...
function Assert-PathBudget([string]$Path, [int]$Reserve = 32) {
    ...
    Assert-True (($full.Length + $Reserve) -lt 200) "O3D3 test path is unsafe: $full"
    Assert-True ($longest -le 80) "O3D3 test component is unsafe: $full"
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 16) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "O3D3 test create-new JSON exists: $Path"
...
foreach ($pin in @(
    ...
    Assert-True (Test-Path -LiteralPath $pin.path -PathType Leaf) "O3D3 test dependency absent: $($pin.path)"
    Assert-True ((Get-Sha256 $pin.path) -eq $pin.sha) "O3D3 test dependency changed: $($pin.path)"
}
foreach ($path in @($testRoot,$sourceRoot,$workRoot,$outputRoot,$badWorkRoot,$badOutputRoot,$gatePath)) {
    Assert-PathBudget $path 32
    Assert-True (-not (Test-Path -LiteralPath $path)) "O3D3 test fresh target exists: $path"
}
Assert-True (-not (Test-Path -LiteralPath 'F:\') -and $null -eq (Get-PSDrive -Name F -ErrorAction SilentlyContinue)) 'O3D3 test requires unused F:.'

$tokens = $null
$parserErrors = $null
[void][Management.Automation.Language.Parser]::ParseFile($endpoint, [ref]$tokens, [ref]$parserErrors)
Assert-True (@($parserErrors).Count -eq 0) 'O3D3 endpoint Windows PowerShell parser failed.'
$endpointText = [IO.File]::ReadAllText($endpoint)
Assert-True ($endpointText.Contains("`$installationSha = '1289EC11F092233D3AAB5ACF416B0212F1874F698F9BBC2B474829939BCDF596'")) 'O3D3 installed-runtime live self-pin changed.'
Assert-True ($endpointText.Contains("`$liveJobSha = '4F7C5809CFB16D9D120196331CC31EF6F4C4B0CF43EE70379591E61392DD88E2'")) 'O3D3 live job self-pin changed.'
Assert-True ($endpointText.Contains('WaitForExit(780000)')) 'O3D3 child timeout changed.'

if ($Preflight) {
    [ordered]@{
        schema='argos_o3d3_endpoint_test_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O3D3_ENDPOINT_TEST_PREFLIGHT'
...
Assert-True ($LASTEXITCODE -eq 0) ('O3D3 fixture generator failed: ' + $generatorText.Trim())
$generatorGate = ($generatorText.Trim() | ConvertFrom-Json)
Assert-True ([string]$generatorGate.state -eq 'PASS_O3D3_REHEARSAL_FIXTURES_CREATED') 'O3D3 fixture generator terminal state changed.'
...
Assert-True ([string]$endpointPreflight.state -eq 'PASS_O3D3_ENDPOINT_PREFLIGHT' -and [bool]$endpointPreflight.rehearsal) 'O3D3 endpoint preflight failed.'
Assert-True (-not [bool]$endpointPreflight.sourceImageBytesRead -and -not [bool]$endpointPreflight.sourceHashesComputed -and -not [bool]$endpointPreflight.pixelsDecoded -and -not [bool]$endpointPreflight.mutationsPerformed) 'O3D3 endpoint preflight mutated or read source bytes.'
...
Assert-True ([string]$liveFixture.state -eq 'PASS_O3D3_LIVE_CONTRACT_FIXTURE' -and [bool]$liveFixture.liveJobAssertionExecuted -and [bool]$liveFixture.liveInstallationAssertionExecuted) 'O3D3 live self-pin function fixture failed.'
...
Assert-True ([string]$normal.state -eq 'PASS_O3D3_HOTSPOT_EDGE_NOTCH_EXECUTED' -and [int]$normal.inputCount -eq 1 -and [int]$normal.verifiedSourceCount -eq 2) 'O3D3 normal rehearsal failed.'
Assert-True (@($normal.rows).Count -eq 1 -and [bool]$normal.rows[0].manufacturedNotchSelectedForReview) 'O3D3 normal rehearsal did not select the manufactured synthetic notch.'
Assert-True ([bool]$normal.sourceAliasRemoved -and [bool]$normal.processorIdentityUnchanged -and -not [bool]$normal.rotationAuthorityGranted) 'O3D3 normal rehearsal runtime boundary changed.'
...
Assert-True $badFailed 'O3D3 injected source hash mismatch did not fail closed.'
Assert-True (-not (Test-Path -LiteralPath $badWorkRoot) -and -not (Test-Path -LiteralPath $badOutputRoot) -and -not (Test-Path -LiteralPath 'F:\')) 'O3D3 injected failure left a live work/output root or alias.'

$resultGate = [ordered]@{
    schema='argos_o3d3_endpoint_rehearsal_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O3D3_ENDPOINT_REHEARSAL';revision='O3D3_20260827T162000000Z_62629419'
