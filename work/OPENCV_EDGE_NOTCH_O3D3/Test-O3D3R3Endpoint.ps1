...
$endpoint=Join-Path $PSScriptRoot 'Invoke-O3D3R3HotspotEndpoint.ps1'
$generator=Join-Path $PSScriptRoot 'New-O3D3R3RehearsalFixtures.py'
$payloadRoot=Join-Path $project 'work\PATTERNED_FIDUCIAL_INVENTORY\tools'
$job=Join-Path $PSScriptRoot 'O3D3R3_HOTSPOT_JOB.json'
$fixtureManifest=Join-Path $PSScriptRoot 'O3D3R3_LIVE_CONTRACT_FIXTURE.json'
...
$testRoot='C:\A37';$sourceRoot='C:\A37\s';$workRoot='C:\A37\w';$outputRoot='C:\A37\o';$badWorkRoot='C:\A37\bw';$badOutputRoot='C:\A37\bo'
$gatePath=Join-Path $PSScriptRoot 'O3D3R3_ENDPOINT_REHEARSAL_GATE.json'

$endpointSha='9A99F1EB253A5063E7911E89D0E0A10A487A7D412284B6823CCE81523A1D1606'
$generatorSha='FB822178DCEEE96E8DACA061DF8E3946DE7D0AC9873420F120F1D7833E72C5FE'
$jobSha='C8D011BA7D350BE7C228EDBC21F6BFD2386A7D8BF7526B4948AC94AA7BA389F5'
$fixtureManifestSha='E9BC43D8487C50C309B940EFD79C58C982F67B26F5AC2A99BBC46C9FD752B8C8'
...
function Assert-Path([string]$Path){$full=[IO.Path]::GetFullPath($Path);$parts=@($full.Split([char[]]@('\','/'),[StringSplitOptions]::RemoveEmptyEntries));$longest=if($parts.Count){[int](($parts|ForEach-Object{$_.Length}|Measure-Object -Maximum).Maximum)}else{0};Assert-True (($full.Length+32)-lt 200) "Unsafe O3D3R3 test path: $full";Assert-True ($longest-le80) "Unsafe O3D3R3 test component: $full"}
function Write-NewJson([string]$Path,[object]$Value,[int]$Depth=16){Assert-True (-not(Test-Path -LiteralPath $Path)) "Create-new O3D3R3 JSON exists: $Path";[IO.File]::WriteAllText($Path,(($Value|ConvertTo-Json -Depth $Depth)+[Environment]::NewLine),(New-Object Text.UTF8Encoding($false)))}
...
foreach($pin in $pins){Assert-True (Test-Path -LiteralPath $pin.path -PathType Leaf) "Missing O3D3R3 dependency: $($pin.path)";Assert-True ((Get-Sha $pin.path)-eq$pin.sha) "Changed O3D3R3 dependency: $($pin.path)"}
foreach($path in @($testRoot,$sourceRoot,$workRoot,$outputRoot,$badWorkRoot,$badOutputRoot,$gatePath)){Assert-Path $path;Assert-True (-not(Test-Path -LiteralPath $path)) "O3D3R3 fresh target exists: $path"}
Assert-True (-not(Test-Path -LiteralPath 'F:\')-and$null-eq(Get-PSDrive -Name F -ErrorAction SilentlyContinue)) 'O3D3R3 requires unused F:.'

$tokens=$null;$errors=$null;[void][Management.Automation.Language.Parser]::ParseFile($endpoint,[ref]$tokens,[ref]$errors);Assert-True (@($errors).Count-eq0) 'O3D3R3 endpoint parser failed.'
...
Assert-True ($fixtureSchemaMatch.Success) 'O3D3R3 endpoint fixture-schema assertion absent.'
Assert-True ($fixtureSchemaMatch.Groups[1].Value-eq[string]$fixture.schema) 'O3D3R3 endpoint/fixture schema literals differ.'
Assert-True ($endpointText.Contains("[string]`$result.schema -eq 'argos_native_frontside_wafer_pose_opencv_v2'")) 'O3D3R3 exact result schema assertion changed.'
Assert-True ($endpointText.Contains("[string]`$summary.schema -eq 'argos_native_frontside_wafer_pose_opencv_v2_summary'")) 'O3D3R3 exact summary schema assertion changed.'

if($Preflight){[ordered]@{schema='argos_o3d3r3_endpoint_test_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O3D3R3_ENDPOINT_TEST_PREFLIGHT';endpointSha256=$endpointSha;generatorSha256=$generatorSha;jobSha256=$jobSha;fixtureSchema=[string]$fixture.schema;fixtureSchemaEqualityPassed=$true;resultSchemaEqualityPassed=$true;summarySchemaEqualityPassed=$true;windowsPowerShellMajor=$PSVersionTable.PSVersion.Major;windowsPowerShellMinor=$PSVersionTable.PSVersion.Minor;testRoot=$testRoot;sourceImageBytesRead=$false;pixelsDecoded=$false;mutationsPerformed=$false;targetExecuted=$false;providerActivated=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 8;return}

$liveText=& $endpoint -Preflight -LiveContractFixtureManifest $fixtureManifest|Out-String;$live=$liveText|ConvertFrom-Json
Assert-True ([string]$live.state-eq'PASS_O3D3R3_LIVE_CONTRACT_FIXTURE') 'O3D3R3 live-contract fixture failed.'
...
Assert-True ($LASTEXITCODE-eq0) ('O3D3R3 fixture generation failed: '+$generatorText.Trim());$generatorGate=$generatorText.Trim()|ConvertFrom-Json;Assert-True ([string]$generatorGate.state-eq'PASS_O3D3R3_REHEARSAL_FIXTURES_CREATED') 'O3D3R3 fixture state changed.'
...
Assert-True ([string]$pre.state-eq'PASS_O3D3R3_ENDPOINT_PREFLIGHT'-and-not[bool]$pre.sourceImageBytesRead-and-not[bool]$pre.mutationsPerformed) 'O3D3R3 endpoint preflight changed.'
...
Assert-True ([string]$normal.state-eq'PASS_O3D3R3_HOTSPOT_EDGE_NOTCH_EXECUTED'-and[int]$normal.inputCount-eq1-and[int]$normal.verifiedSourceCount-eq2) 'O3D3R3 normal rehearsal failed.'
Assert-True (@($normal.rows).Count-eq1-and[bool]$normal.rows[0].manufacturedNotchSelectedForReview-and[bool]$normal.sourceAliasRemoved-and[bool]$normal.processorIdentityUnchanged) 'O3D3R3 normal result boundary changed.'
