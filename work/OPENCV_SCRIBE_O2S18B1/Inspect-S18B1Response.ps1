...
if(-not$Preflight){throw 'S18B1 response inspection is preflight-only.'}
...
Assert-ExactCondition((Get-Item -LiteralPath $sourceZip).Length-le 4194304)'S18B1 response exceeds 4 MiB.'
...
try{
    ...
    Assert-ExactCondition($entryNames.Count-eq4 -and @(Compare-Object $expectedEntries $entryNames).Count-eq0)'S18B1 response entry set changed.'
...
Assert-ExactCondition([string]$manifest.schema-eq'argos_project_portal_response_manifest_v1' -and [string]$manifest.requestId-eq$ExpectedRequestId -and [string]$manifest.sourceRole-eq'JBOD' -and [string]$manifest.state-eq'PASS_DATA_PULL')'S18B1 signed response identity or state changed.'
Assert-ExactCondition([string]$manifest.signerThumbprint-eq'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC' -and [bool]$manifest.reviewOnly -and -not[bool]$manifest.trainingEligible -and -not[bool]$manifest.xmlEligible -and -not[bool]$manifest.productionEligible -and -not[bool]$manifest.productionRoutingEnabled -and -not[bool]$manifest.credentialsIncluded)'S18B1 signed response authority changed.'
Assert-ExactCondition(@($manifest.files).Count-eq2 -and @(Compare-Object @('DATA_PULL_PAYLOAD.zip','RESULT.json') @($manifest.files.path)).Count-eq0)'S18B1 manifest file set changed.'
foreach($record in @($manifest.files)){$recordBytes=if([string]$record.path-eq'DATA_PULL_PAYLOAD.zip'){$payloadBytes}else{$resultBytes};Assert-ExactCondition(@($recordBytes).Count-eq[int64]$record.bytes -and (Get-ExactBytesSha256 ([byte[]]$recordBytes))-eq[string]$record.sha256)"S18B1 declared response file changed: $($record.path)"}
...
Assert-ExactCondition($signatureValid -and $certificate.Thumbprint-eq'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC')'S18B1 response signature failed.'
$proposalEntry='data/JBOD_PROCESSOR_REVIEW/identity/proposals/62619-433_20260824005735_Slot18/SCRIBE_PROPOSAL.json'
$summaryEntry='data/JBOD_PROCESSOR_REVIEW/identity/proposals/62619-433_20260824005735_Slot18/scribe/multi_channel/MULTI_CHANNEL_READER_SUMMARY.json'
