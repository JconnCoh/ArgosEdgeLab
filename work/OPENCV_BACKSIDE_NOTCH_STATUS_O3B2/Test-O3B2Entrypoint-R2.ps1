...
$fixtureRoot='C:\O3B2U'
$gatePath=Join-Path $root 'O3B2_ENTRYPOINT_REHEARSAL_GATE_R2.json'
...
function New-Case([string]$Name,[bool]$TargetInstalled=$false,[bool]$CorruptWorker=$false){
    ...
    if($TargetInstalled){Copy-Item -LiteralPath $config -Destination (Join-Path $portal 'config\endpoint_jbod.json')}else{[IO.File]::WriteAllText((Join-Path $portal 'config\endpoint_jbod.json'),(($evidence.endpointConfig|ConvertTo-Json -Depth 20)+[Environment]::NewLine),(New-Object Text.UTF8Encoding($false)))}
