#!/usr/bin/env python3
"""No-image fixture for the exact O3F11 endpoint orchestration rehearsal."""
...
def fail_if(stage: str) -> None:
    if os.environ.get("ARGOS_O3F11_FIXTURE_FAIL_STAGE", "").upper() == stage:
        raise RuntimeError(f"INJECTED_O3F11_{stage}_FAILURE")


def gate(root: Path) -> dict[str, object]:
    fail_if("GATE")
    summary = {
        "schema": "argos_ocv03_o3f11_gate_result_v1",
    ...
    return {"state": "COMPLETE_O3F9_GATE", "stage": "GATE", "summarySha256": sha256(target), "commands": []}
...
def dev6(root: Path, prerequisite: Path, expected_sha256: str) -> dict[str, object]:
    ...
    for index in range(1, 7):
        ...
        manifest = {"schema": "argos_ocv03_full_perimeter_topology_manifest_v2", "revision": f"O3F11_FIXTURE_{index:04d}", "results": [observed]}
    ...
    summary = {
        "schema": "argos_ocv03_o3f11_staged_result_v1",
    ...
    target = root / "SUMMARY.json"
    write_new(target, summary)
    return {key: summary[key] for key in ("state", "stage", "selectedCount", "executedCount", "newProviderHoldCount", "stateCounts", "results")} | {"summarySha256": sha256(target)}
...
def main() -> int:
    ...
    if args.stage == "SELF_TEST":
        print(json.dumps({"state": "PASS_O3F9_STAGED_RUNNER_SELF_TEST", "mutationsPerformed": False}, separators=(",", ":")))
        return 0
