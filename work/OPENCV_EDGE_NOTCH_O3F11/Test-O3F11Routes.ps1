...
function Write-O3F11Json([string]$Path, [object]$Value) { Assert-O3F11 (-not (Test-Path -LiteralPath $Path)) "O3F11 route gate exists: $Path"; [IO.File]::WriteAllText($Path, (($Value | ConvertTo-Json -Depth 12) + [Environment]::NewLine), (New-Object Text.UTF8Encoding($false))) }
...
$outputPath = Join-Path (Join-Path $PSScriptRoot 'final_o3f11') 'O3F11_PREPUBLICATION_PATH_GATE.json'
...
foreach ($path in @(
    (Join-Path $project "work/OPENCV_EDGE_NOTCH_O3F11/final_o3f11/$zipName"),
    ...
    'D:/O3F9G11/SUMMARY.json.partial', 'D:/O3F9G11.failed/SUMMARY.json',
    'D:/O3F9D11/cases/C0006/0123456789abcdef_df_r10_recovery_enhanced.png', 'D:/O3F9D11/SUMMARY.json.partial', 'D:/O3F9D11.failed/SUMMARY.json',
...
Write-O3F11Json $outputPath $result
