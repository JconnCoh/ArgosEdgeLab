...
def engine_at(path: Path):
    spec = importlib.util.spec_from_file_location("o3l8", path)
...
def analyze(engine, image: np.ndarray, geometry: np.ndarray, cfg: dict):
    ...
    state = "NONE" if not found else ("AMBIGUOUS" if len(found) > 1 else "PRIMARY")
...
def main() -> int:
    ...
    gate = {
        "schema": "argos_ocv03_o3l8_synthetic_gate_v1",
        "state": "PASS_O3L8_WAFER_TOPOLOGY_AXIS_SYNTHETIC_GATE",
        "caseCount": len(cases),
        "cases": cases,
