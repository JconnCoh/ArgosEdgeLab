...
function Write-NewBytes([string]$Path, [byte[]]$Bytes) {
    Require (-not (Test-Path -LiteralPath $Path)) "O3F15L3 create-new path exists: $Path"
...
$buildGatePath = Join-Path $PSScriptRoot 'O3F15L3_BUILD_GATE.json'
...
foreach ($path in @($definitionPath, $buildGatePath, $identityPath, $certificatePath, $verifier)) {
    Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L3 signing dependency absent: $path"
}
$buildGate = Get-Content -LiteralPath $buildGatePath -Raw | ConvertFrom-Json
$definition = Get-Content -LiteralPath $definitionPath -Raw | ConvertFrom-Json
Require ([string]$buildGate.state -ceq 'PASS_O3F15L3_UNSIGNED_PREFLIGHT_DIAGNOSTIC_PACKAGE_BUILT' -and [string]$buildGate.definitionSha256 -ceq (Sha $definitionPath)) 'O3F15L3 build evidence changed.'
Require ([string]$definition.schema -ceq 'argos_ocv03_o3f15l3_maintenance_definition_v1' -and [string]$definition.state -ceq 'FROZEN_FOR_SIGNING' -and [string]$definition.entryPoint -ceq 'payload/Invoke-O3F15L3.ps1') 'O3F15L3 definition is not frozen for signing.'
Require (@($definition.changes).Count -eq 1 -and @($definition.entryPointMutations).Count -eq 0 -and @($definition.entryPointOutputs).Count -eq 0 -and @($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'O3F15L3 signed authority bounds changed.'
Require ([string]::Join('|', @($definition.sourceProcessingContract.exactChildArguments)) -ceq '-I|-B|Run-O3F15FrontReconcile.py|PREFLIGHT' -and -not [bool]$definition.sourceProcessingContract.selfTestAllowed -and -not [bool]$definition.sourceProcessingContract.gateAllowed -and -not [bool]$definition.sourceProcessingContract.runAllowed -and -not [bool]$definition.sourceProcessingContract.imageBytesReadAllowed) 'O3F15L3 signed child boundary changed.'
$change = @($definition.changes)[0]
$carrierHash = '6B7CC6E643265545297BA4DED1E6B37AB262349572194B13F79264EF77D8DCE4'
Require ([string]$change.source -ceq 'payload/OCV03_NotchReviewOpenCvV1.py' -and [string]$change.destination -ceq 'C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/OCV03_NotchReviewOpenCvV1.py' -and [string]$change.installedSha256 -ceq $carrierHash -and @($change.approvedPredecessorSha256).Count -eq 1 -and [string]$change.approvedPredecessorSha256[0] -ceq $carrierHash -and -not [bool]$change.allowCreate) 'O3F15L3 same-bytes carrier contract changed.'
$actual = @(Get-ChildItem -LiteralPath $payloadRoot -File | Sort-Object Name)
Require ($actual.Count -eq [int]$buildGate.payloadFileCount) 'O3F15L3 payload cardinality changed.'
foreach ($row in @($buildGate.payloadFiles)) {
    $path = Join-Path $payloadRoot ([string]$row.path)
    Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L3 payload absent: $($row.path)"
    Require ((Get-Item -LiteralPath $path).Length -eq [int64]$row.bytes -and (Sha $path) -ceq [string]$row.sha256) "O3F15L3 payload changed: $($row.path)"
}
$identity = Get-Content -LiteralPath $identityPath -Raw | ConvertFrom-Json
$thumbprint = ([string]$identity.thumbprint).Replace(' ', '').ToUpperInvariant()
$certificate = Get-Item -LiteralPath ("Cert:\CurrentUser\My\$thumbprint") -ErrorAction Stop
Require ([bool]$certificate.HasPrivateKey) 'O3F15L3 signer private key unavailable.'
...
Require ([string]$pathGate.state -ceq 'PASS_PATH_BUDGET') 'O3F15L3 final package path gate failed.'
foreach ($path in @($finalRoot, $signedRoot, $signGatePath)) {
    Require (-not (Test-Path -LiteralPath $path)) "O3F15L3 create-new signing target exists: $path"
}

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ocv03_o3f15l3_sign_preflight_v1'
        state = 'PASS_O3F15L3_SIGN_PREFLIGHT'
...
Require ($files.Count -eq $actual.Count) 'O3F15L3 signed payload cardinality changed.'
$manifest = [ordered]@{
...
$gate = [ordered]@{
    schema = 'argos_ocv03_o3f15l3_sign_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O3F15L3_SIGNED_PREFLIGHT_DIAGNOSTIC_PACKAGE'
    requestId = $requestId
    finalRoot = $finalRoot
    packagePath = $ready
    packageZipPath = $zip
