...
$project=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'));$sourceRoot=Join-Path $project 'work\OPENCV_BACKSIDE_NOTCH_O3B10';$root='C:\R32C953L1PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R32C953L1_BUILD_GATE.json'
$sources=[ordered]@{
 'Invoke-R32C953L1.ps1'=@((Join-Path $PSScriptRoot 'Invoke-R32C953L1.ps1'),'3ECC2767028C54DAD9A50225032E06976EBCBEF4C9F05EE818710D37746F3577')
 'Run-R32Frozen953Corpus.py'=@((Join-Path $PSScriptRoot 'Run-R32Frozen953Corpus.py'),'31A24CAD936117A3046357FAC58B32BCCA8850C113452BB0E9EC0188910C387D')
 'Test-RunR32Frozen953Corpus.py'=@((Join-Path $PSScriptRoot 'Test-RunR32Frozen953Corpus.py'),'A8887BC5B6CFF6C67A6489DB47856B770704F4A0E54A709345F3DF345E161B35')
 'R32C953_LAUNCH_CONTRACT.json'=@((Join-Path $PSScriptRoot 'R32C953_LAUNCH_CONTRACT.json'),'805DEECB384199D6BC1F8B0261EF0FC8AF3A1141B18C07F147D1FED7230906FC')
 'Run-OpenCvKlarfCorpusR2.py'=@((Join-Path $sourceRoot 'Run-OpenCvKlarfCorpusR2.py'),'20102DD8502EEC798BE1199B1B074922D24A8AE8343A180762EA1CD78BB8EFF6')
 ...
 'Test-BacksideNotchOpenCvR32.py'=@((Join-Path $PSScriptRoot 'Test-BacksideNotchOpenCvR32.py'),'A59F9C85D8AB210FAEA78F458342AEE29D1B38E460EFEE9A700085D2AB6B49D2')
 ...
 'Detect-BacksideNotchOpenCvR32.py'=@((Join-Path $sourceRoot 'Detect-BacksideNotchOpenCvR32.py'),'2E9D19DDCCCA751C21C545AF5E2B6AB62596E86891374AB0E13C84BEDEA48012')
}
...
foreach($name in $sources.Keys){$path=[string]$sources[$name][0];if(-not(Test-Path -LiteralPath $path -PathType Leaf)-or(Sha $path)-ne[string]$sources[$name][1]){throw "R32C953L1 source absent or changed: $name"}}
if($Preflight){[ordered]@{schema='argos_o3b21_r32c953l1_build_preflight_v1';state='PASS_R32C953L1_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=$sources.Count;currentPairCount=978;excludedPairCount=25;expectedPairCount=953;syntheticDetectorTestCount=95;selectionTestCount=7;targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json;return}
if(Test-Path -LiteralPath $root){throw 'R32C953L1 build root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R32C953L1 build gate exists.'};[void](New-Item -ItemType Directory -Path $payload -Force);foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$actual=@(Get-ChildItem -LiteralPath $payload -File|Sort-Object Name);if($actual.Count-ne$sources.Count){throw 'R32C953L1 payload cardinality failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=1048576;entryPoint='payload/Invoke-R32C953L1.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_R32C953L1_FRESH_EXACT_953_BACKSIDE_CORPUS_LAUNCHED'}};Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$payloadFiles=@($actual|ForEach-Object{[ordered]@{path=$_.Name;bytes=$_.Length;sha256=Sha $_.FullName}});$gate=[ordered]@{schema='argos_o3b21_r32c953l1_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R32C953L1_UNSIGNED_EXACT_953_LAUNCH_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadFiles=$payloadFiles;entrySha256=Sha (Join-Path $payload 'Invoke-R32C953L1.ps1');selectionWrapperSha256=$sources['Run-R32Frozen953Corpus.py'][1];selectionContractSha256=$sources['R32C953_LAUNCH_CONTRACT.json'][1];runnerSha256=$sources['Run-OpenCvKlarfCorpusR2.py'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR32.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');currentPairCount=978;excludedPairCount=25;expectedPairCount=953;syntheticDetectorTestCount=95;selectionTestCount=7;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false};Write-NewJson $gatePath $gate;$gate|ConvertTo-Json -Depth 10
