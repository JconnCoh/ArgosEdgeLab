...
$manifestPath = Join-Path $PSScriptRoot 'R21M1_MISSING_ONLY_CASES.json'
$manifestHash = '56419244A465B6E45D7409649BB7810BFEF474E26CD6812E7108907741F8270A'
$frozenPath = Join-Path $PSScriptRoot 'R18_REGRESSION_CASES.json'

Require ($env:COMPUTERNAME -eq 'A1025645101') 'R21M1 missing-only evidence reached the wrong computer.'
foreach ($pin in @(
    ...
    foreach ($bfCandidate in @($detector.bf.candidates)) {
        foreach ($dfCandidate in @($detector.df.candidates)) {
            ...
            if ($distance -lt [double]$manifest.newControlSelection.minimumAdjacentDistanceDegrees -or
                $distance -gt [double]$manifest.newControlSelection.maximumAdjacentDistanceDegrees -or
                $distance -ge $adjacentDistance) { continue }
            $record = ItemForDiagnostic ([string]$row.diagnosticRoot)
...
else { $allCases.Add($soleHolder); $seen[$soleHolder.id] = $true }
if ($null -eq $adjacent) { $selectionHolds.Add('HOLD_NO_EXACT_R20_NOTCH_ADJACENT_PAIRED_HOLDER_CONTROL_FOUND') }
elseif (-not $seen.ContainsKey($adjacent.id)) { $allCases.Add($adjacent); $seen[$adjacent.id] = $true }
else { $selectionHolds.Add('HOLD_NEW_CONTROL_SELECTION_NOT_DISTINCT') }

Require ($selectionHolds.Count -eq 0) ('R21M1 new control selection failed: ' + (@($selectionHolds) -join '; '))
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R21M1 all-case cardinality changed.'
$selectedCases = @(
    foreach ($ordinal in ([int]$manifest.caseSelection.firstOrdinalInclusive)..([int]$manifest.caseSelection.lastOrdinalInclusive)) {
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R21M1 selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R21M1 hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R21M1 new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R21M1 output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r21m1_missing_only_preflight_v1'
        state = 'PASS_O3B21_R21M1_MISSING_ONLY_PREFLIGHT'
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R21M1 detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R21M1 detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R21M1 detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R21M1 result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R21M1 evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r21m1_missing_only_real_image_evidence_v1'
    state = 'PASS_O3B21_R21M1_MISSING_ONLY_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R21M1_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '0-18'
...
'PASS_O3B21_R21M1_MISSING_ONLY_EVIDENCE_EXECUTED'
