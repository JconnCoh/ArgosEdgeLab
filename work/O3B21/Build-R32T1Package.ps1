...
$project=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'));$sourceRoot=Join-Path $project 'work\OPENCV_BACKSIDE_NOTCH_O3B10';$root='C:\R32T1PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R32T1_BUILD_GATE.json'
$sources=[ordered]@{
 'Invoke-R32T1.ps1'=@((Join-Path $PSScriptRoot 'Invoke-R32T1.ps1'),'FAA1C2C2066CF60BCE1B98AD9542A2E1CB42DD848ED27D6DF093D59ACAF8A662')
 'Run-R32TargetedValidation.py'=@((Join-Path $PSScriptRoot 'Run-R32TargetedValidation.py'),'80AA946E0FA9FD8C09E37FB715F5D9CE73DCBFA485EC890D390AFC3A1DC6FF0D')
 'R32T1_CONTRACT.json'=@((Join-Path $PSScriptRoot 'R32T1_CONTRACT.json'),'8CA83D75B3337013FC7FF582B332CE26D7CD5DF400A332BED0461CEE372E4C2E')
...
foreach($name in $sources.Keys){$path=[string]$sources[$name][0];if(-not(Test-Path -LiteralPath $path -PathType Leaf)-or(Sha $path)-ne[string]$sources[$name][1]){throw "R32T1 source absent or changed: $name"}}
if($Preflight){[ordered]@{schema='argos_o3b21_r32t1_build_preflight_v1';state='PASS_R32T1_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=$sources.Count;realImageCaseCount=1;syntheticTestCount=95;maximumTotalSeconds=240;targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json;return}
if(Test-Path -LiteralPath $root){throw 'R32T1 build root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R32T1 build gate exists.'};[void](New-Item -ItemType Directory -Path $payload -Force);foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$actual=@(Get-ChildItem -LiteralPath $payload -File|Sort-Object Name);if($actual.Count-ne$sources.Count){throw 'R32T1 payload cardinality failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=16777216;entryPoint='payload/Invoke-R32T1.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R32T1_ONE_EXECUTION_COMPLETE'}};Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$payloadFiles=@($actual|ForEach-Object{[ordered]@{path=$_.Name;bytes=$_.Length;sha256=Sha $_.FullName}});$gate=[ordered]@{schema='argos_o3b21_r32t1_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R32T1_UNSIGNED_ONE_CASE_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadFiles=$payloadFiles;entrySha256=Sha (Join-Path $payload 'Invoke-R32T1.ps1');runnerSha256=$sources['Run-R32TargetedValidation.py'][1];contractSha256=$sources['R32T1_CONTRACT.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR32.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');realImageCaseCount=1;syntheticTestCount=95;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false};Write-NewJson $gatePath $gate;$gate|ConvertTo-Json -Depth 10
