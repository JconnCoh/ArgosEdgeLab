...
$root='C:\R27B32R2PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R27B32R2_BUILD_GATE.json'
$entrySource=Join-Path $PSScriptRoot 'r27b32r2_generated\Invoke-R27B32R2.ps1'
$carrier='C:\R21P5\payload\C.json'
$sources=[ordered]@{
 'R27B32R2_CASES.json'=@((Join-Path $PSScriptRoot 'R27B32R2_CASES.json'),'3E2F5DF2F5E97FA1CA82527DC3D235F61FCBF4245C8D09B44BC6D6140A41B8D0')
...
$entrySourceSha='2B541C873B3FAD5738B40CE4D8175A371F84CD9FA9BACE0A4F802A5F0CA0596C'
$expectedEntrySha='2B541C873B3FAD5738B40CE4D8175A371F84CD9FA9BACE0A4F802A5F0CA0596C'
...
if(-not(Test-Path -LiteralPath $entrySource -PathType Leaf)-or(Sha $entrySource)-ne$entrySourceSha){throw 'R27B32R2 entry source absent or changed.'}
foreach($name in $sources.Keys){$source=[string]$sources[$name][0];$hash=[string]$sources[$name][1];if(-not(Test-Path -LiteralPath $source -PathType Leaf)-or(Sha $source)-ne$hash){throw "R27B32R2 dependency absent or changed: $name"}}
...
if($Preflight){[ordered]@{schema='argos_o3b21_r27b32r2_build_preflight_v1';state='PASS_R27B32R2_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=16;entrySha256=$entrySha;selectedOrdinals=@(0..31);targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5;return}
if($entrySha-ne$expectedEntrySha){throw "R27B32R2 entry is not frozen: $entrySha"};if(Test-Path -LiteralPath $root){throw 'R27B32R2 root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R27B32R2 gate exists.'}
[void](New-Item -ItemType Directory -Path $payload -Force)
[IO.File]::WriteAllBytes((Join-Path $payload 'Invoke-R27B32R2.ps1'),$bytes)
foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$expected=@('Invoke-R27B32R2.ps1')+@($sources.Keys);$expected=@($expected|Sort-Object)
...
if(@(Compare-Object -ReferenceObject $expected -DifferenceObject $actual).Count -ne 0){throw 'R27B32R2 exact clean payload inventory failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=33554432;entryPoint='payload/Invoke-R27B32R2.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R27B32R2_ALL_32_BATCHED_REGRESSION'}}
Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$gate=[ordered]@{schema='argos_o3b21_r27b32r2_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R27B32R2_UNSIGNED_ALL_32_BATCH_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadRelativePaths=$actual;entrySha256=Sha (Join-Path $payload 'Invoke-R27B32R2.ps1');casesSha256=$sources['R27B32R2_CASES.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR27.py'][1];frozenR25Sha256=$sources['Detect-BacksideNotchOpenCvR25.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');selectedCaseCount=32;completedCaseRerunCount=32;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false}
