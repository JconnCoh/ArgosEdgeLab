...
$root='C:\R25H22APK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R25H22A_BUILD_GATE.json'
$entrySource=Join-Path $PSScriptRoot 'r25h22a_generated\Invoke-R25H22A.ps1'
$carrier='C:\R21P5\payload\C.json'
$sources=[ordered]@{
 'R25H22A_CASES.json'=@((Join-Path $PSScriptRoot 'R25H22A_CASES.json'),'3EE1153E11D5FAD2A196028AAACA57967AEE75CD1B4370FA8A78F34AA45E600D')
...
$entrySourceSha='E0D462FAE34AE0FD729D515CD5D855953F7F9B5E730E7BF9A830B2B9FF7ECB35'
$expectedEntrySha='E0D462FAE34AE0FD729D515CD5D855953F7F9B5E730E7BF9A830B2B9FF7ECB35'
...
if(-not(Test-Path -LiteralPath $entrySource -PathType Leaf)-or(Sha $entrySource)-ne$entrySourceSha){throw 'R25H22A entry source absent or changed.'}
foreach($name in $sources.Keys){$source=[string]$sources[$name][0];$hash=[string]$sources[$name][1];if(-not(Test-Path -LiteralPath $source -PathType Leaf)-or(Sha $source)-ne$hash){throw "R25H22A dependency absent or changed: $name"}}
...
if($Preflight){[ordered]@{schema='argos_o3b21_r25h22a_build_preflight_v1';state='PASS_R25H22A_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=14;entrySha256=$entrySha;selectedOrdinals=@(10,11,12,13,14,15,16,17,18);targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5;return}
if($entrySha-ne$expectedEntrySha){throw "R25H22A entry is not frozen: $entrySha"};if(Test-Path -LiteralPath $root){throw 'R25H22A root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R25H22A gate exists.'}
[void](New-Item -ItemType Directory -Path $payload -Force)
[IO.File]::WriteAllBytes((Join-Path $payload 'Invoke-R25H22A.ps1'),$bytes)
foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$expected=@('Invoke-R25H22A.ps1')+@($sources.Keys);$expected=@($expected|Sort-Object)
...
if(@(Compare-Object -ReferenceObject $expected -DifferenceObject $actual).Count -ne 0){throw 'R25H22A exact clean payload inventory failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=67108864;entryPoint='payload/Invoke-R25H22A.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R25H22A_MISSING_ONLY_EVIDENCE_EXECUTED'}}
Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$gate=[ordered]@{schema='argos_o3b21_r25h22a_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R25H22A_UNSIGNED_CLEAN_HOLD_PARTITION_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadRelativePaths=$actual;entrySha256=Sha (Join-Path $payload 'Invoke-R25H22A.ps1');casesSha256=$sources['R25H22A_CASES.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR25.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');selectedCaseCount=9;completedCaseRerunCount=0;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false}
