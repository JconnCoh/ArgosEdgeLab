...
$root='C:\R25T1PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R25T1_BUILD_GATE.json'
$entrySource=Join-Path $PSScriptRoot 'r25t1_generated\Invoke-R25T1.ps1'
$carrier='C:\R21P5\payload\C.json'
$sources=[ordered]@{
 'R25T1_TARGET_CASES.json'=@((Join-Path $PSScriptRoot 'R25T1_TARGET_CASES.json'),'68B81540B095C5B15E3BBA908639BD181E48BB53DD7B46D0BE60CB36FA432F18')
...
$entrySourceSha='A582090C45C9B37FF4B0FBE8B07E50959F9023A6FDE6B44556070744F1FD3E47'
$expectedEntrySha='A582090C45C9B37FF4B0FBE8B07E50959F9023A6FDE6B44556070744F1FD3E47'
...
if(-not(Test-Path -LiteralPath $entrySource -PathType Leaf)-or(Sha $entrySource)-ne$entrySourceSha){throw 'R25T1 entry source absent or changed.'}
foreach($name in $sources.Keys){$source=[string]$sources[$name][0];$hash=[string]$sources[$name][1];if(-not(Test-Path -LiteralPath $source -PathType Leaf)-or(Sha $source)-ne$hash){throw "R25T1 dependency absent or changed: $name"}}
...
if($Preflight){[ordered]@{schema='argos_o3b21_r25t1_build_preflight_v1';state='PASS_R25T1_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=14;entrySha256=$entrySha;selectedOrdinals=@(24,25,26,27,29,30,31,8);targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5;return}
if($entrySha-ne$expectedEntrySha){throw "R25T1 entry is not frozen: $entrySha"};if(Test-Path -LiteralPath $root){throw 'R25T1 root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R25T1 gate exists.'}
[void](New-Item -ItemType Directory -Path $payload -Force)
[IO.File]::WriteAllBytes((Join-Path $payload 'Invoke-R25T1.ps1'),$bytes)
foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$expected=@('Invoke-R25T1.ps1')+@($sources.Keys);$expected=@($expected|Sort-Object)
...
if(@(Compare-Object -ReferenceObject $expected -DifferenceObject $actual).Count -ne 0){throw 'R25T1 exact clean payload inventory failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=67108864;entryPoint='payload/Invoke-R25T1.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R25T1_MISSING_ONLY_EVIDENCE_EXECUTED'}}
Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$gate=[ordered]@{schema='argos_o3b21_r25t1_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R25T1_UNSIGNED_CLEAN_EIGHT_CASE_TARGET_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadRelativePaths=$actual;entrySha256=Sha (Join-Path $payload 'Invoke-R25T1.ps1');casesSha256=$sources['R25T1_TARGET_CASES.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR25.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');selectedCaseCount=8;completedCaseRerunCount=0;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false}
