...
$manifestPath=Join-Path $PSScriptRoot 'R27B32_CASES.json'
$manifestHash='8BA92F3351D21A3E58AD05C12574C4ECDF37F7BE7C38056C9A118F9F64567DA4'
...
if($PackageLeafPreflight){
    ...
    Require ([string]$m.outputRoot-eq'D:/R27B32') 'Package output root changed.'
    ...
    [ordered]@{schema='argos_o3b21_r27b32_package_leaf_preflight_v1';state='PASS_R27B32_EXACT_PACKAGED_ENTRY_MANIFEST_LEAF';manifestSha256=Sha $manifestPath;detectorSha256=Sha $engine;selectedCaseCount=32;maximumConcurrentChildren=3;imageDecoded=$false;processStarted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5 -Compress
    return
}

Require ($env:COMPUTERNAME-eq'A1025645101') 'R27B32 batched evidence reached the wrong computer.'
...
Require ($allCases.Count-eq32) 'R27B32 all-case cardinality changed.'
...
Require ($selectedCases.Count-eq32) 'R27B32 selected cardinality changed.'
Require (@($selectedCases|Where-Object{$_.group-eq'FROZEN_R20_CONTROL'}).Count-eq10) 'R27B32 frozen-control cardinality changed.'
Require (@($selectedCases|Where-Object{$_.group-eq'R20_CURRENT_HOLD'}).Count-eq22) 'R27B32 hold cardinality changed.'
...
Require (-not(Test-Path -LiteralPath $outputRoot)) "Create-new R27B32 output exists: $outputRoot"
if($Preflight){[ordered]@{schema='argos_o3b21_r27b32_preflight_v1';state='PASS_O3B21_R27B32_BATCHED_PREFLIGHT';outputRoot=$outputRoot;selectedCaseCount=32;frozenControlCount=10;r20CurrentHoldCount=22;maximumConcurrentChildren=3;imageDecoded=$false;sourceHashingPerformed=$false;processStarted=$false;mutationsPerformed=$false;reviewOnly=$true}|ConvertTo-Json -Depth 6 -Compress;return}
...
try{
    while($pending.Count-gt0-or$running.Count-gt0){
        while($pending.Count-gt0-and$running.Count-lt[int]$manifest.runtimeBoundary.maximumConcurrentChildren){
            ...
            $process=New-Object Diagnostics.Process;$process.StartInfo=$start;Require $process.Start() "R27B32 detector did not start: $($case.id)"
        ...
        foreach($run in @($running)){
            if(-not$run.Process.HasExited){if($run.Timer.Elapsed.TotalSeconds-ge[int]$manifest.runtimeBoundary.maximumPerCaseSeconds){try{$run.Process.Kill()}catch{};throw "R27B32 detector case timed out: $($run.Case.id)"};continue}
            $run.Process.WaitForExit();$run.Timer.Stop();Require ($run.Process.ExitCode-eq0) ("R27B32 detector failed: $($run.Case.id): "+$run.Stderr.Result)
            $resultPath=Join-Path $run.CaseRoot 'RESULT.json';Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R27B32 result absent: $($run.Case.id)";$detector=Get-Content -LiteralPath $resultPath -Raw|ConvertFrom-Json
            ...
            foreach($asset in @(@('BF_REVIEW',(Join-Path $run.CaseRoot 'BF_review.jpg')),@('DF_REVIEW',(Join-Path $run.CaseRoot 'DF_review.jpg')),@('BF_HOLDER_MASK',(Join-Path $run.CaseRoot 'BF_holder_exclusion.png')),@('DF_HOLDER_MASK',(Join-Path $run.CaseRoot 'DF_holder_exclusion.png')))){
                Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R27B32 evidence asset absent: $($run.Case.id) $($asset[0])";$item=Get-Item -LiteralPath $asset[1];$record=[pscustomobject][ordered]@{role=$asset[0];path=$item.FullName;bytes=$item.Length;sha256=Sha $item.FullName};$assets.Add($record);$allAssetBytes+=[int64]$item.Length
            }
...
}catch{foreach($run in @($running)){try{if(-not$run.Process.HasExited){$run.Process.Kill()}}catch{};try{$run.Process.Dispose()}catch{}};throw}
$overall.Stop();$results=@(0..31|ForEach-Object{$resultByOrdinal[$_]});Require (@($results|Where-Object{$null-ne$_}).Count-eq32) 'R27B32 completed result cardinality failed.'
$expected=@($manifest.targetGate.expectedPairedCandidateCountsByOrdinal)
foreach($result in $results){$ordinal=[int]$result.ordinal;Require ([int]$result.detector.pairedCandidateCount-eq[int]$expected[$ordinal]) "R27B32 exact paired cardinality failed at ordinal $ordinal";Require (-not[bool]$result.detector.knownNotchLocationConsumed) "R27B32 notch location consumed at ordinal $ordinal";Require (-not[bool]$result.detector.sourceMutationPerformed) "R27B32 source mutation reported at ordinal $ordinal"}
$o23=$resultByOrdinal[23];Require ([string]$o23.detector.bf.dfGeometryBfFullPerimeterCompensation.state-eq[string]$manifest.targetGate.o23RequiredCompensationState) 'R27B32 O23 compensation state failed.'
Require ([string]$resultByOrdinal[8].detector.bf.bfShallowDepthRatioNegativeControl.state-eq[string]$manifest.targetGate.ordinal8RequiredNegativeControlState) 'R27B32 ordinal 8 negative control changed.'
...
Require ([string]$o11bf.sha256-eq[string]$manifest.targetGate.ordinal11RequiredBfHolderMaskSha256) 'R27B32 ordinal 11 BF holder mask changed.';Require ([string]$o11df.sha256-eq[string]$manifest.targetGate.ordinal11RequiredDfHolderMaskSha256) 'R27B32 ordinal 11 DF holder mask changed.'
