...
$root='C:\R26O23PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R26O23_BUILD_GATE.json'
$entrySource=Join-Path $PSScriptRoot 'r26o23_generated\Invoke-R26O23.ps1'
$carrier='C:\R21P5\payload\C.json'
$sources=[ordered]@{
 'R26O23_TARGET_CASE.json'=@((Join-Path $PSScriptRoot 'R26O23_TARGET_CASE.json'),'CFF942B395789E93C002D5DF57CCA60BB78A962D8CBE8863A11B2FE7FC020553')
...
$entrySourceSha='85BDFA8383A61AB9A2167E432956E99DBBDBA381E4D2D17A860F502D84165952'
$expectedEntrySha='85BDFA8383A61AB9A2167E432956E99DBBDBA381E4D2D17A860F502D84165952'
...
if(-not(Test-Path -LiteralPath $entrySource -PathType Leaf)-or(Sha $entrySource)-ne$entrySourceSha){throw 'R26O23 entry source absent or changed.'}
foreach($name in $sources.Keys){$source=[string]$sources[$name][0];$hash=[string]$sources[$name][1];if(-not(Test-Path -LiteralPath $source -PathType Leaf)-or(Sha $source)-ne$hash){throw "R26O23 dependency absent or changed: $name"}}
...
if($Preflight){[ordered]@{schema='argos_o3b21_r26o23_build_preflight_v1';state='PASS_R26O23_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=15;entrySha256=$entrySha;selectedOrdinals=@(23);targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5;return}
if($entrySha-ne$expectedEntrySha){throw "R26O23 entry is not frozen: $entrySha"};if(Test-Path -LiteralPath $root){throw 'R26O23 root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R26O23 gate exists.'}
[void](New-Item -ItemType Directory -Path $payload -Force)
[IO.File]::WriteAllBytes((Join-Path $payload 'Invoke-R26O23.ps1'),$bytes)
foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$expected=@('Invoke-R26O23.ps1')+@($sources.Keys);$expected=@($expected|Sort-Object)
...
if(@(Compare-Object -ReferenceObject $expected -DifferenceObject $actual).Count -ne 0){throw 'R26O23 exact clean payload inventory failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=33554432;entryPoint='payload/Invoke-R26O23.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R26O23_TARGET_EVIDENCE_EXECUTED'}}
Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$gate=[ordered]@{schema='argos_o3b21_r26o23_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R26O23_UNSIGNED_SINGLE_CASE_TARGET_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadRelativePaths=$actual;entrySha256=Sha (Join-Path $payload 'Invoke-R26O23.ps1');casesSha256=$sources['R26O23_TARGET_CASE.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR26.py'][1];frozenR25Sha256=$sources['Detect-BacksideNotchOpenCvR25.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');selectedCaseCount=1;completedCaseRerunCount=0;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false}
