...
$manifestPath = Join-Path $PSScriptRoot 'R25T1_TARGET_CASES.json'
$manifestHash = '68B81540B095C5B15E3BBA908639BD181E48BB53DD7B46D0BE60CB36FA432F18'
...
if ($PackageLeafPreflight) {
    ...
    Require ([string]$packageManifest.outputRoot -eq 'D:/R25T1') 'Package output root changed.'
    Require ([int]$packageManifest.caseSelection.selectedCount -eq 8) 'Package target count changed.'
    Require ((@($packageManifest.caseSelection.selectedOrdinals | ForEach-Object { [int]$_ }) -join ',') -eq '24,25,26,27,29,30,31,8') 'Package target ordinals changed.'
    [ordered]@{
        schema = 'argos_o3b21_r25t1_package_leaf_preflight_v1'
        state = 'PASS_R25T1_EXACT_PACKAGED_ENTRY_MANIFEST_LEAF'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R25T1 missing-only evidence reached the wrong computer.'
foreach ($pin in @(
...
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R25T1 all-case cardinality changed.'
$selectedCases = @(
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R25T1 selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R25T1 hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R25T1 new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R25T1 output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r24f10_missing_only_preflight_v1'
        state = 'PASS_O3B21_R25T1_MISSING_ONLY_PREFLIGHT'
        ...
        completedOrdinalsExcluded = '0-7,9-23,28,32-33'
        imageDecoded = $false
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R25T1 detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R25T1 detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R25T1 detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R25T1 result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R25T1 evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r25t1_target_real_image_evidence_v1'
    state = 'PASS_O3B21_R25T1_MISSING_ONLY_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R25T1_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '0-7,9-23,28,32-33'
...
'PASS_O3B21_R25T1_MISSING_ONLY_EVIDENCE_EXECUTED'
