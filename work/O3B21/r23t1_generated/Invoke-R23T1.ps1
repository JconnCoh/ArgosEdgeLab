...
$engine = Join-Path $PSScriptRoot 'Detect-BacksideNotchOpenCvR23.py'
$engineHash = 'AAE38F93C7C1FE16E0967713A3773E33D61488E4D02BE6794E2811624D6DCE4C'
...
$configPath = Join-Path $PSScriptRoot 'BACKSIDE_NOTCH_CONFIG_R11.json'
$configHash = '60498159EA39BBA209F97414977A12CF5F275D4C5559B8EC0A80E83BBBCEDC49'
$manifestPath = Join-Path $PSScriptRoot 'R23T1_TARGET_CASES.json'
$manifestHash = '47DBFB09C4946A34E0CFF0559D79628A4292C3EA4FC3F971739AD66DC4B184F5'
$frozenPath = Join-Path $PSScriptRoot 'R18_REGRESSION_CASES.json'

Require ($env:COMPUTERNAME -eq 'A1025645101') 'R23T1 missing-only evidence reached the wrong computer.'
foreach ($pin in @(
...
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R23T1 all-case cardinality changed.'
$selectedCases = @(
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R23T1 selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R23T1 hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R23T1 new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R23T1 output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r23t1_missing_only_preflight_v1'
        state = 'PASS_O3B21_R23T1_MISSING_ONLY_PREFLIGHT'
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R23T1 detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R23T1 detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R23T1 detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R23T1 result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R23T1 evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r23t1_missing_only_real_image_evidence_v1'
    state = 'PASS_O3B21_R23T1_MISSING_ONLY_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R23T1_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '0-18'
...
'PASS_O3B21_R23T1_MISSING_ONLY_EVIDENCE_EXECUTED'
