...
$root='C:\R25NA2PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R25NA2_BUILD_GATE.json'
$entrySource=Join-Path $PSScriptRoot 'r25na2_generated\Invoke-R25NA2.ps1';$carrier='C:\R21P5\payload\C.json'
$sources=[ordered]@{
...
$entrySourceSha='173C9C236351F22B60F66B67E96C76E8B3F586ECA83F89E6CEFCA76B111771EA';$expectedEntrySha=$entrySourceSha
...
if(-not(Test-Path -LiteralPath $entrySource -PathType Leaf)-or(Sha $entrySource)-ne$entrySourceSha){throw 'R25NA2 entry source absent or changed.'}
foreach($name in $sources.Keys){$source=[string]$sources[$name][0];$hash=[string]$sources[$name][1];if(-not(Test-Path -LiteralPath $source -PathType Leaf)-or(Sha $source)-ne$hash){throw "R25NA2 dependency absent or changed: $name"}}
...
if($Preflight){[ordered]@{schema='argos_o3b21_r25na2_build_preflight_v1';state='PASS_R25NA2_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=16;entrySha256=$entrySha;frozenCandidateCount=24;reusedOrdinalCount=17;missingOrdinalCount=7;targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5;return}
if($entrySha-ne$expectedEntrySha){throw "R25NA2 entry is not frozen: $entrySha"};if(Test-Path -LiteralPath $root){throw 'R25NA2 root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R25NA2 gate exists.'}
[void](New-Item -ItemType Directory -Path $payload -Force);[IO.File]::WriteAllBytes((Join-Path $payload 'Invoke-R25NA2.ps1'),$bytes)
foreach($name in $sources.Keys){[IO.File]::Copy([string]$sources[$name][0],(Join-Path $payload $name),$false)}
$expected=@('Invoke-R25NA2.ps1')+@($sources.Keys);$expected=@($expected|Sort-Object);$actual=@(Get-ChildItem -LiteralPath $payload -Recurse -File|ForEach-Object{$_.FullName.Substring($payload.Length+1).Replace('\','/')}|Sort-Object)
if(@(Compare-Object -ReferenceObject $expected -DifferenceObject $actual).Count-ne 0){throw 'R25NA2 exact clean payload inventory failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=8388608;entryPoint='payload/Invoke-R25NA2.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R25NA2_MISSING7_SELECTOR_EXECUTED'}}
Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$gate=[ordered]@{schema='argos_o3b21_r25na2_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R25NA2_UNSIGNED_REUSE17_MISSING7_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadRelativePaths=$actual;entrySha256=Sha (Join-Path $payload 'Invoke-R25NA2.ps1');casesSha256=$sources['R25NA1_CASES.json'][1];selectorSha256=$sources['R25_NA1_LEXICAL_PASS_SOURCE_DISCOVERY_SELECTOR.json'][1];selectionSha256=$sources['R25_NA1_LEXICAL_PASS_SOURCE_DISCOVERY_RESULT.json'][1];sourceFreezeSha256=$sources['R25_NA1_EXACT_SOURCE_RECORD_FREEZE.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR25.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');frozenCandidateCount=24;reusedOrdinalCount=17;missingOrdinalCount=7;completedR25CaseRerunCount=0;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false}
