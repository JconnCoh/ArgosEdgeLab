...
$root='C:\R30VAL1PK';$payload=Join-Path $root 'payload';$gatePath=Join-Path $PSScriptRoot 'R30VAL1_BUILD_GATE.json'
$sources=[ordered]@{
 'Invoke-R30VAL1.ps1'=@((Join-Path $PSScriptRoot 'Invoke-R30VAL1.ps1'),'332B33B52A10E787EE9980F3C4E40714B993DE3164A981F3059D8882857722A7')
 ...
 'Run-R30ValidationBatch.py'=@((Join-Path $PSScriptRoot 'Run-R30ValidationBatch.py'),'CD87CB23EACC0CF45A0B565DE8B77613222E5840B19B37EA92DC2A0442C636BC')
 'R30VAL1_CONTRACT.json'=@((Join-Path $PSScriptRoot 'R30VAL1_CONTRACT.json'),'B4E4A64253AECB61575EF1C76068094DBE12DFEDA6A53A768912144150C88DB5')
...
foreach($name in $sources.Keys){$path=[string]$sources[$name][0];if(-not(Test-Path -LiteralPath $path -PathType Leaf)-or(Sha $path)-ne[string]$sources[$name][1]){throw "R30VAL1 source absent or changed: $name"}}
if($Preflight){[ordered]@{schema='argos_o3b21_r30val1_build_preflight_v1';state='PASS_R30VAL1_BUILD_PREFLIGHT';buildRoot=$root;payloadFileCount=$sources.Count;caseCount=70;syntheticTestCount=59;maximumTotalSeconds=1200;targetExecuted=$false;mutationsPerformed=$false}|ConvertTo-Json;return}
if(Test-Path -LiteralPath $root){throw 'R30VAL1 build root exists.'};if(Test-Path -LiteralPath $gatePath){throw 'R30VAL1 build gate exists.'}
...
$actual=@(Get-ChildItem -LiteralPath $payload -File|Sort-Object Name);if($actual.Count-ne$sources.Count){throw 'R30VAL1 payload cardinality failed.'}
$definition=[ordered]@{targetRole='JBOD';jobClass='MAINTENANCE_PATCH';maxResultBytes=33554432;entryPoint='payload/Invoke-R30VAL1.ps1';changes=@([ordered]@{source='payload/C.json';destination='C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/PROCESSOR_CONFIG.json';approvedPredecessorSha256=@($sources['C.json'][1]);installedSha256=$sources['C.json'][1];allowCreate=$false});allowedTaskActions=@();rehearsal=[ordered]@{requiredState='PASS_O3B21_R30VAL1_70_EXECUTIONS_COMPLETE'}}
Write-NewJson (Join-Path $root 'DEFINITION.json') $definition
$gate=[ordered]@{schema='argos_o3b21_r30val1_build_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R30VAL1_UNSIGNED_70_CASE_PACKAGE_BUILT';root=$root;payloadFileCount=$actual.Count;payloadRelativePaths=@($actual.Name);entrySha256=Sha (Join-Path $payload 'Invoke-R30VAL1.ps1');runnerSha256=$sources['Run-R30ValidationBatch.py'][1];contractSha256=$sources['R30VAL1_CONTRACT.json'][1];detectorSha256=$sources['Detect-BacksideNotchOpenCvR30.py'][1];configSha256=$sources['BACKSIDE_NOTCH_CONFIG_R13.json'][1];definitionSha256=Sha (Join-Path $root 'DEFINITION.json');caseCount=70;frozenCaseCount=32;currentPatternedFrontCount=25;currentUnpatternedFrontCount=12;syntheticTestCount=59;sameBytesCarrier=$true;installedSemanticChange=$false;taskOrProcessActionCount=0;pythonCacheLeafCount=0;signed=$false;published=$false;targetExecuted=$false;mutationsPerformed=$false}
