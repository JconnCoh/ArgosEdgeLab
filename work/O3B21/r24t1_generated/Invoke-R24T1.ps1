...
$manifestPath = Join-Path $PSScriptRoot 'R24T1_TARGET_CASES.json'
$manifestHash = 'B5A629BE005950E1C66112BAFD8F5923F24DC55AEBB7D70432418D365F35F2B3'
$frozenPath = Join-Path $PSScriptRoot 'R18_REGRESSION_CASES.json'

Require ($env:COMPUTERNAME -eq 'A1025645101') 'R24T1 missing-only evidence reached the wrong computer.'
foreach ($pin in @(
...
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R24T1 all-case cardinality changed.'
$selectedCases = @(
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R24T1 selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R24T1 hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R24T1 new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R24T1 output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r24t1_missing_only_preflight_v1'
        state = 'PASS_O3B21_R24T1_MISSING_ONLY_PREFLIGHT'
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R24T1 detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R24T1 detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R24T1 detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R24T1 result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R24T1 evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r24t1_missing_only_real_image_evidence_v1'
    state = 'PASS_O3B21_R24T1_MISSING_ONLY_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R24T1_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '0-18'
...
'PASS_O3B21_R24T1_MISSING_ONLY_EVIDENCE_EXECUTED'
