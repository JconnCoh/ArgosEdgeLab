...
$predecessorRoot='D:\R25NA1';$outputRoot='D:\R25NA2';Require (Test-Path -LiteralPath $predecessorRoot -PathType Container) 'R25NA1 predecessor output root is absent.';Require (-not(Test-Path -LiteralPath $outputRoot)) "Create-new R25NA2 output exists: $outputRoot"
for($ordinal=0;$ordinal-lt 24;$ordinal++){$ordinalText='{0:D2}'-f$ordinal;$oldJob=Join-Path $predecessorRoot ('J'+$ordinalText+'.json');$oldResult=Join-Path (Join-Path $predecessorRoot ('O'+$ordinalText)) 'RESULT.json';if($ordinal-lt 17){Require (Test-Path -LiteralPath $oldJob -PathType Leaf) "Reusable job absent: $ordinalText";Require (Test-Path -LiteralPath $oldResult -PathType Leaf) "Reusable result absent: $ordinalText"}else{Require (-not(Test-Path -LiteralPath $oldJob)) "Missing ordinal job unexpectedly exists: $ordinalText";Require (-not(Test-Path -LiteralPath $oldResult)) "Missing ordinal result unexpectedly exists: $ordinalText"}}
if($Preflight){[ordered]@{schema='argos_o3b21_r25na2_preflight_v1';state='PASS_O3B21_R25NA2_REUSE17_MISSING7_PREFLIGHT';predecessorRoot=$predecessorRoot;outputRoot=$outputRoot;candidateCount=24;reusedOrdinalCount=17;missingOrdinalCount=7;canonicalSourceJsonCharacters=$canonical.Length;canonicalSourceJsonSha256=BytesSha $canonicalBytes;imageDecoded=$false;sourceHashingPerformed=$false;processStarted=$false;mutationsPerformed=$false;reviewOnly=$true}|ConvertTo-Json -Depth 6 -Compress;return}

[void](New-Item -ItemType Directory -Path $outputRoot);$config=Get-Content -LiteralPath $configPath -Raw|ConvertFrom-Json
$overall=[Diagnostics.Stopwatch]::StartNew();$results=New-Object Collections.Generic.List[object];$eligible=$null;$ownedChildProcessCount=0
for($ordinal=0;$ordinal-lt$cases.Count;$ordinal++){
    ...
    $ordinalText='{0:D2}'-f$ordinal;$reused=$ordinal-lt 17;$caseRoot=Join-Path $(if($reused){$predecessorRoot}else{$outputRoot}) ('O'+$ordinalText);$jobPath=Join-Path $(if($reused){$predecessorRoot}else{$outputRoot}) ('J'+$ordinalText+'.json');$elapsedSeconds=$null;$detectorStdout=''
    if(-not$reused){$job=[ordered]@{bf=$case.bf;df=$case.df;bfSha256=$bfHash;dfSha256=$dfHash;output=$caseRoot;radialEngine=[string]$config.radialEngine;radialEngineSha256=[string]$config.radialEngineSha256;radialParameters=$config.radialParameters;maximumDimension=[int]$manifest.maximumDimension};[IO.File]::WriteAllText($jobPath,($job|ConvertTo-Json -Depth 20),[Text.UTF8Encoding]::new($false));$start=New-Object Diagnostics.ProcessStartInfo;$start.FileName=$python;$start.Arguments=('-B "{0}" --job "{1}"'-f$engine,$jobPath);$start.WorkingDirectory=$PSScriptRoot;$start.UseShellExecute=$false;$start.CreateNoWindow=$true;$start.RedirectStandardOutput=$true;$start.RedirectStandardError=$true;$process=New-Object Diagnostics.Process;$process.StartInfo=$start;$timer=[Diagnostics.Stopwatch]::StartNew();Require $process.Start() "R25NA2 detector did not start: $($case.id)";$ownedChildProcessCount++;$stdout=$process.StandardOutput.ReadToEndAsync();$stderr=$process.StandardError.ReadToEndAsync();if(-not$process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds*1000))){try{$process.Kill()}catch{};throw "R25NA2 detector case timed out: $($case.id)"};$timer.Stop();Require ($process.ExitCode-eq 0) ("R25NA2 detector failed: $($case.id): "+$stderr.Result);$elapsedSeconds=[Math]::Round($timer.Elapsed.TotalSeconds,3);$detectorStdout=$stdout.Result.Trim()}
    $resultPath=Join-Path $caseRoot 'RESULT.json';Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R25NA2 result absent: $($case.id)";$detector=Get-Content -LiteralPath $resultPath -Raw|ConvertFrom-Json
    $assets=New-Object Collections.Generic.List[object];foreach($asset in @(@('BF_REVIEW',(Join-Path $caseRoot 'BF_review.jpg')),@('DF_REVIEW',(Join-Path $caseRoot 'DF_review.jpg')),@('BF_HOLDER_MASK',(Join-Path $caseRoot 'BF_holder_exclusion.png')),@('DF_HOLDER_MASK',(Join-Path $caseRoot 'DF_holder_exclusion.png')))){Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R25NA2 evidence asset absent: $($case.id) $($asset[0])";$i=Get-Item -LiteralPath $asset[1];$assets.Add([pscustomobject][ordered]@{role=$asset[0];path=$i.FullName;bytes=$i.Length;sha256=Sha $i.FullName})}
    ...
    $record=[pscustomobject][ordered]@{ordinal=$ordinal;id=$case.id;reusedPredecessorOutput=$reused;bfSha256=$bfHash;dfSha256=$dfHash;sourceRecordPath=$case.sourceRecordPath;elapsedSeconds=$elapsedSeconds;eligibility=$evaluation;pairedCandidateCount=[int]$detector.pairedCandidateCount;resultPath=$resultPath;resultSha256=Sha $resultPath;assets=$assets;detectorStdout=$detectorStdout};$results.Add($record)
    if([bool]$evaluation.eligible){$eligible=$record;break}
}
$overall.Stop();$summary=[ordered]@{schema='argos_ocv03_o3b21_r25na2_notch_adjacent_control_evidence_v1';state=if($null-ne$eligible){'PASS_R25NA2_LEXICAL_FIRST_ELIGIBLE_CONTROL_FOUND'}else{'HOLD_R25NA2_NO_ELIGIBLE_CONTROL_IN_FROZEN_24'};gateState='PASS_R25NA2_FROZEN_SELECTOR_EXECUTION_COMPLETE';predecessorRoot=$predecessorRoot;outputRoot=$outputRoot;frozenCandidateCount=24;reusedCandidateCount=@($results|Where-Object{$_.reusedPredecessorOutput}).Count;newCandidateCount=@($results|Where-Object{-not$_.reusedPredecessorOutput}).Count;evaluatedCandidateCount=$results.Count;stoppedAfterFirstEligible=$null-ne$eligible;eligibleOrdinal=if($null-ne$eligible){$eligible.ordinal}else{$null};elapsedSeconds=[Math]::Round($overall.Elapsed.TotalSeconds,3);rastersEmbedded=$false;results=$results;selectorRelaxedAfterResults=$false;automaticHoldClearancePerformed=$false;sourceMutationPerformed=$false;sourceDeletionPerformed=$false;existingTaskOrProcessActionPerformed=$false;ownedChildProcessCount=$ownedChildProcessCount;providerActivationPerformed=$false;reviewOnly=$true;trainingEligible=$false;xmlEligible=$false;productionEligible=$false};$summaryPath=Join-Path $outputRoot 'SUMMARY.json';[IO.File]::WriteAllText($summaryPath,($summary|ConvertTo-Json -Depth 32),[Text.UTF8Encoding]::new($false));$summary|ConvertTo-Json -Depth 32 -Compress
'PASS_O3B21_R25NA2_MISSING7_SELECTOR_EXECUTED'
