...
$engine = Join-Path $PSScriptRoot 'Detect-BacksideNotchOpenCvR26.py'
$engineHash = '05534929ECCCB18EA8E2E68A66CF33FA7AAF6B43CA80B6BBBD1970C3946FC1D6'
...
$manifestPath = Join-Path $PSScriptRoot 'R26O23_TARGET_CASE.json'
$manifestHash = 'CFF942B395789E93C002D5DF57CCA60BB78A962D8CBE8863A11B2FE7FC020553'
...
if ($PackageLeafPreflight) {
    ...
    Require ([string]$packageManifest.outputRoot -eq 'D:/R26O23') 'Package output root changed.'
    ...
    [ordered]@{
        schema = 'argos_o3b21_r26o23_package_leaf_preflight_v1'
        state = 'PASS_R26O23_EXACT_PACKAGED_ENTRY_MANIFEST_LEAF'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R26O23 target evidence reached the wrong computer.'
foreach ($pin in @(
...
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R26O23 all-case cardinality changed.'
$selectedCases = @(
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R26O23 selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R26O23 hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R26O23 new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R26O23 output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r26o23_target_preflight_v1'
        state = 'PASS_O3B21_R26O23_TARGET_PREFLIGHT'
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R26O23 detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R26O23 detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R26O23 detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R26O23 result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R26O23 evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r26o23_target_real_image_evidence_v1'
    state = 'PASS_O3B21_R26O23_TARGET_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R26O23_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '0-22,24-31'
...
'PASS_O3B21_R26O23_TARGET_EVIDENCE_EXECUTED'
