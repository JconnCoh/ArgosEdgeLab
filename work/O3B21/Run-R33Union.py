#!/usr/bin/env python3
"""Run an exact case manifest plus explicit sentinels through one frozen detector."""
...
def execute(case: dict, args, config: dict, output: Path) -> dict:
    ...
    try:
        ...
        if ordinal == 92:
            ...
            if not angle_ok or (multi_pair or {}).get("state") != "PASS_UNIQUE_BOTH_CHANNELS_EXTERIOR_CLEAR_PAIR":
        ...
        if "expectedMeanAngleDegrees" in case:
            ...
            if not (angle_ok and mode_ok and ratio_ok):
                invariant_failures.append("R33 sentinel identity or mode changed")
        return {
...
def main() -> int:
    ...
    summary = {
        "schema": "argos_r33_union_result_v1",
        "state": "PASS_R33_UNION_301_OF_301" if not mismatches else "HOLD_R33_UNION_MISMATCH",
