...
$manifestPath=Join-Path $PSScriptRoot 'R27M12_CASES.json'
$manifestHash='192FF3569EF4AF9FAE14110FACE9F2D0A800169F1CAE590FFF5096D516FEABE9'
...
if($PackageLeafPreflight){
    ...
    Require ([string]$m.outputRoot-eq'D:/R27M12') 'Package output root changed.'
    Require ([int]$m.caseSelection.selectedCount-eq12) 'Package batch count changed.'
    Require ([int]$m.runtimeBoundary.maximumConcurrentChildren-eq3) 'Package concurrency changed.'
    [ordered]@{schema='argos_o3b21_r27m12_package_leaf_preflight_v1';state='PASS_R27M12_EXACT_PACKAGED_ENTRY_MANIFEST_LEAF';manifestSha256=Sha $manifestPath;detectorSha256=Sha $engine;selectedCaseCount=12;selectedOrdinals=@(20..31);maximumConcurrentChildren=3;imageDecoded=$false;processStarted=$false;mutationsPerformed=$false}|ConvertTo-Json -Depth 5 -Compress
    return
}

Require ($env:COMPUTERNAME-eq'A1025645101') 'R27M12 batched evidence reached the wrong computer.'
...
Require ($allCases.Count-eq32) 'R27M12 all-case cardinality changed.'
...
Require ($selectedCases.Count-eq12) 'R27M12 selected cardinality changed.'
Require (@($selectedCases|Where-Object{$_.group-eq'FROZEN_R20_CONTROL'}).Count-eq0) 'R27M12 frozen-control cardinality changed.'
Require (@($selectedCases|Where-Object{$_.group-eq'R20_CURRENT_HOLD'}).Count-eq12) 'R27M12 hold cardinality changed.'
...
Require (-not(Test-Path -LiteralPath $outputRoot)) "Create-new R27M12 output exists: $outputRoot"
if($Preflight){[ordered]@{schema='argos_o3b21_r27m12_preflight_v1';state='PASS_O3B21_R27M12_MISSING_ONLY_PREFLIGHT';outputRoot=$outputRoot;selectedCaseCount=12;selectedOrdinals=@(20..31);completedCaseRerunCount=0;maximumConcurrentChildren=3;imageDecoded=$false;sourceHashingPerformed=$false;processStarted=$false;mutationsPerformed=$false;reviewOnly=$true}|ConvertTo-Json -Depth 6 -Compress;return}
...
try{
    while($pending.Count-gt0-or$running.Count-gt0){
        while($pending.Count-gt0-and$running.Count-lt[int]$manifest.runtimeBoundary.maximumConcurrentChildren){
            ...
            $process=New-Object Diagnostics.Process;$process.StartInfo=$start;Require $process.Start() "R27M12 detector did not start: $($case.id)"
        ...
        foreach($run in $running.ToArray()){
            if(-not$run.Process.HasExited){if($run.Timer.Elapsed.TotalSeconds-ge[int]$manifest.runtimeBoundary.maximumPerCaseSeconds){try{$run.Process.Kill()}catch{};throw "R27M12 detector case timed out: $($run.Case.id)"};continue}
            $run.Process.WaitForExit();$run.Timer.Stop();Require ($run.Process.ExitCode-eq0) ("R27M12 detector failed: $($run.Case.id): "+$run.Stderr.Result)
            $resultPath=Join-Path $run.CaseRoot 'RESULT.json';Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R27M12 result absent: $($run.Case.id)";$detector=Get-Content -LiteralPath $resultPath -Raw|ConvertFrom-Json
            ...
            foreach($asset in @(@('BF_REVIEW',(Join-Path $run.CaseRoot 'BF_review.jpg')),@('DF_REVIEW',(Join-Path $run.CaseRoot 'DF_review.jpg')),@('BF_HOLDER_MASK',(Join-Path $run.CaseRoot 'BF_holder_exclusion.png')),@('DF_HOLDER_MASK',(Join-Path $run.CaseRoot 'DF_holder_exclusion.png')))){
                Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R27M12 evidence asset absent: $($run.Case.id) $($asset[0])";$item=Get-Item -LiteralPath $asset[1];$record=[pscustomobject][ordered]@{role=$asset[0];path=$item.FullName;bytes=$item.Length;sha256=Sha $item.FullName};$assets.Add($record);$allAssetBytes+=[int64]$item.Length
            }
...
$overall.Stop();$results=@($manifest.caseSelection.selectedOrdinals|ForEach-Object{$resultByOrdinal[[int]$_]});Require (@($results|Where-Object{$null-ne$_}).Count-eq12) 'R27M12 completed result cardinality failed.'
$expected=@($manifest.targetGate.expectedPairedCandidateCountsByOrdinal)
foreach($result in $results){$ordinal=[int]$result.ordinal;Require ([int]$result.detector.pairedCandidateCount-eq[int]$expected[$ordinal]) "R27M12 exact paired cardinality failed at ordinal $ordinal";Require (-not[bool]$result.detector.knownNotchLocationConsumed) "R27M12 notch location consumed at ordinal $ordinal";Require (-not[bool]$result.detector.sourceMutationPerformed) "R27M12 source mutation reported at ordinal $ordinal"}
$o23=$resultByOrdinal[23];Require ([string]$o23.detector.bf.dfGeometryBfFullPerimeterCompensation.state-eq[string]$manifest.targetGate.o23RequiredCompensationState) 'R27M12 O23 compensation state failed.'
...
[ordered]@{schema='argos_ocv03_o3b21_r27m12_missing_only_real_image_evidence_v1';state='PASS_O3B21_R27M12_MISSING_12_REGRESSION';gateState='PASS_O3B21_R27M12_EXACT_MISSING_12_AND_O23_GATE';outputRoot=$outputRoot;caseCount=12;selectedOrdinals=@(20..31);completedCaseRerunCount=0;maximumConcurrentChildren=3;elapsedSeconds=[Math]::Round($overall.Elapsed.TotalSeconds,3);allAssetBytes=$allAssetBytes;o23ReturnedRasterBytes=$o23AssetBytes;o23RasterBytesEmbedded=$returnO23;results=$results;sourceMutationPerformed=$false;sourceDeletionPerformed=$false;existingTaskOrProcessActionPerformed=$false;ownedChildProcessCount=12;providerActivationPerformed=$false;fresh953CorpusExecuted=$false;reviewOnly=$true;trainingEligible=$false;xmlEligible=$false;productionEligible=$false}|ConvertTo-Json -Depth 64 -Compress
'PASS_O3B21_R27M12_MISSING_12_REGRESSION'
