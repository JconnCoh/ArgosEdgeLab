...
$engine = Join-Path $PSScriptRoot 'Detect-BacksideNotchOpenCvR25.py'
$engineHash = '6A7977E4DAFE692FCE6E7DE4740C94EE66D5F79ECD62FDF190CB5EE8E4862274'
...
$manifestPath = Join-Path $PSScriptRoot 'R25H22A_CASES.json'
$manifestHash = '3EE1153E11D5FAD2A196028AAACA57967AEE75CD1B4370FA8A78F34AA45E600D'
...
if ($PackageLeafPreflight) {
    ...
    Require ([string]$packageManifest.outputRoot -eq 'D:/R25H22A') 'Package output root changed.'
    Require ([int]$packageManifest.caseSelection.selectedCount -eq 9) 'Package hold-partition count changed.'
    Require ((@($packageManifest.caseSelection.selectedOrdinals | ForEach-Object { [int]$_ }) -join ',') -eq '10,11,12,13,14,15,16,17,18') 'Package hold-partition ordinals changed.'
    [ordered]@{
        schema = 'argos_o3b21_r25h22a_package_leaf_preflight_v1'
        state = 'PASS_R25H22A_EXACT_PACKAGED_ENTRY_MANIFEST_LEAF'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R25H22A missing-only evidence reached the wrong computer.'
foreach ($pin in @(
...
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R25H22A all-case cardinality changed.'
$selectedCases = @(
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R25H22A selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R25H22A hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R25H22A new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R25H22A output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r25h22a_hold_partition_preflight_v1'
        state = 'PASS_O3B21_R25H22A_MISSING_ONLY_PREFLIGHT'
        ...
        completedOrdinalsExcluded = '0-9,19-31'
        imageDecoded = $false
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R25H22A detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R25H22A detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R25H22A detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R25H22A result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R25H22A evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r25h22a_hold_partition_real_image_evidence_v1'
    state = 'PASS_O3B21_R25H22A_MISSING_ONLY_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R25H22A_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '0-9,19-31'
...
'PASS_O3B21_R25H22A_MISSING_ONLY_EVIDENCE_EXECUTED'
