...
$manifestPath = Join-Path $PSScriptRoot 'R25F10_CASES.json'
$manifestHash = '6CE25A52AF96A6AD0B2188EBF87535954B1FF8414C6721A655AE0DA9FD553258'
...
if ($PackageLeafPreflight) {
    ...
    Require ([string]$packageManifest.outputRoot -eq 'D:/R25F10') 'Package output root changed.'
    ...
    [ordered]@{
        schema = 'argos_o3b21_r25f10_package_leaf_preflight_v1'
        state = 'PASS_R25F10_EXACT_PACKAGED_ENTRY_MANIFEST_LEAF'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R25F10 missing-only evidence reached the wrong computer.'
foreach ($pin in @(
...
Require ($allCases.Count -eq [int]$manifest.caseSelection.allCaseCount) 'R25F10 all-case cardinality changed.'
$selectedCases = @(
...
Require ($selectedCases.Count -eq [int]$manifest.caseSelection.selectedCount) 'R25F10 selected cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -eq 'R20_CURRENT_HOLD' }).Count -eq [int]$manifest.caseSelection.r20CurrentHoldCount) 'R25F10 hold cardinality changed.'
Require (@($selectedCases | Where-Object { $_.group -like 'NEW_*' }).Count -eq [int]$manifest.caseSelection.newControlCount) 'R25F10 new-control cardinality changed.'
...
Require (-not (Test-Path -LiteralPath $outputRoot)) "Create-new R25F10 output exists: $outputRoot"
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o3b21_r25f10_frozen_ten_preflight_v1'
        state = 'PASS_O3B21_R25F10_MISSING_ONLY_PREFLIGHT'
...
foreach ($case in $selectedCases) {
    ...
    Require $process.Start() "R25F10 detector did not start: $($case.id)"
    $stdout = $process.StandardOutput.ReadToEndAsync()
    $stderr = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(([int]$manifest.runtimeBoundary.maximumPerCaseSeconds * 1000))) {
        try { $process.Kill() } catch {}
        throw "R25F10 detector case timed out: $($case.id)"
    }
    $caseTimer.Stop()
    Require ($process.ExitCode -eq 0) ("R25F10 detector failed: $($case.id): " + $stderr.Result)
    $resultPath = Join-Path $caseRoot 'RESULT.json'
    Require (Test-Path -LiteralPath $resultPath -PathType Leaf) "R25F10 result absent: $($case.id)"
    ...
    foreach ($asset in @(
        ...
        Require (Test-Path -LiteralPath $asset[1] -PathType Leaf) "R25F10 evidence asset absent: $($case.id) $($asset[0])"
...
[ordered]@{
    schema = 'argos_ocv03_o3b21_r25f10_frozen_ten_real_image_evidence_v1'
    state = 'PASS_O3B21_R25F10_MISSING_ONLY_EVIDENCE_EXECUTED'
    gateState = 'PASS_O3B21_R25F10_EXACT_CARDINALITY_GATE'
    outputRoot = $outputRoot
    caseCount = $selectedCases.Count
    selectedOrdinals = @($selectedCases | ForEach-Object { [int]$_.ordinal })
    completedOrdinalsExcluded = '10-31'
...
'PASS_O3B21_R25F10_MISSING_ONLY_EVIDENCE_EXECUTED'
