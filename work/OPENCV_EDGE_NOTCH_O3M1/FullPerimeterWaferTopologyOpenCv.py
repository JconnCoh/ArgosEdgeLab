#!/usr/bin/env python3
"""Review-only full-perimeter wafer-notch topology detector.
...
def patterned_wafer(radius: int, features: list[tuple[float, float, float, bool]]) -> np.ndarray:
    image = np.zeros((1024, 1024), dtype=np.uint8)
    cv2.circle(image, (512, 512), radius, 180, cv2.FILLED, cv2.LINE_AA)
    for offset in range(-radius + 20, radius - 20, 38):
        cv2.line(image, (512 - radius, 512 + offset), (512 + radius, 512 + offset), 48, 5, cv2.LINE_8)
        cv2.line(image, (512 + offset, 512 - radius), (512 + offset, 512 + radius), 48, 5, cv2.LINE_8)
    disk = np.zeros_like(image)
    cv2.circle(disk, (512, 512), radius, 255, cv2.FILLED)
    image[disk == 0] = 0
    for angle, width, depth, irregular in features:
        radians, half = math.radians(angle), math.radians(width / 2.0)
        mouths = [
            (int(round(512 + math.cos(radians + delta) * radius)), int(round(512 + math.sin(radians + delta) * radius)))
            for delta in (-half, half)
        ]
        tip_angle = radians + (math.radians(width * 0.22) if irregular else 0.0)
        tip = (int(round(512 + math.cos(tip_angle) * (radius - depth))), int(round(512 + math.sin(tip_angle) * (radius - depth))))
...
def synthetic_gate(output_root: Path) -> dict[str, Any]:
    ...
    crop = {"widthPx": 600, "inwardPx": 240, "outwardPx": 120, "stepDegrees": 7.5}
    cfg = {
        "clahe": 2.5, "exteriorStartY": 285, "exteriorEndY": 350, "minimumExteriorScale": 3.0,
        "waferDistanceThreshold": 2.5, "dieStreetCloseKernelPx": 17, "topContactRowsPx": 12,
        "minimumTopContactPixels": 60, "minimumWaferAreaPx": 50000, "maximumInwardPx": 120,
        "maximumOutwardPx": 50, "supportSampleOffsetPx": 8, "minimumContourCoverage": 0.95,
        "maximumInterpolatedGapPx": 18, "patternSuppressionWidthPx": 11, "minimumNotchDepthPx": 20.0,
        "noiseSigmaThreshold": 4.5, "candidateJoinWidthPx": 13, "minimumNotchWidthPx": 10,
    ...
    for case_id, bf_features, df_features, expected_count, expected_angle in cases:
        bf_image = patterned_wafer(420, bf_features)
        df_image = patterned_wafer(438, df_features)
