...
$routeGatePath = Join-Path $workRoot 'OEL1_COMPLETE_ROUTE_GATE.json'
$publishGatePath = Join-Path $workRoot 'OEL1_PUBLISH_GATE.json'
$pathTool = Join-Path $project 'utilities\Confirm-ArgosPathBudget.ps1'
$shareRoot = '\\shm-cifs\Department\DE-1302_FAB_BE_Engineering\60_Saw_VI_Sort\600_General\Joshua.conn\AVI_Images\Argos\Uploads\InspectionRevs'

foreach ($path in @($source, $finalGatePath, $routeGatePath, $pathTool)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "OEL1 publish input is missing: $path" }
}
if (Test-Path -LiteralPath $publishGatePath) { throw "OEL1 publish gate already exists: $publishGatePath" }
...
if ([string]$finalGate.state -ne 'PASS_OEL1_FINAL_PACKAGE_GATE' -or [string]$finalGate.requestId -ne $requestId -or [string]$finalGate.requestZipSha256 -ne $expectedSha -or [int64]$finalGate.requestZipBytes -ne $expectedBytes -or -not [bool]$finalGate.publicationRequiresCompleteRouteGate -or [bool]$finalGate.publicationAuthorized) { throw 'OEL1 final package gate contract changed.' }
if ([string]$routeGate.state -ne 'PASS_OEL1_COMPLETE_ROUTE_GATE' -or [string]$routeGate.requestId -ne $requestId -or [string]$routeGate.requestZipSha256 -ne $expectedSha -or -not [bool]$routeGate.exactFinalZipExtractionPassed -or -not [bool]$routeGate.exactFinalZipSignaturePassed) { throw 'OEL1 complete route gate contract changed.' }
if ([string]$finalGate.requestManifestSha256 -ne [string]$routeGate.requestManifestSha256 -or [string]$finalGate.requestSignatureSha256 -ne [string]$routeGate.requestSignatureSha256 -or [string]$finalGate.oldEndpointWorkerSha256 -ne [string]$routeGate.endpointWorkerPredecessorSha256 -or [string]$finalGate.targetEndpointWorkerSha256 -ne [string]$routeGate.endpointWorkerTargetSha256) { throw 'OEL1 final-package and complete-route evidence do not describe the same exact request.' }
...
try {
    ...
    if ([string]$pathGate.state -ne 'PASS_PATH_BUDGET') { throw "OEL1 publish path gate failed: $($pathGate.state)" }
    foreach ($path in @($upload, $ready)) { if (Test-Path -LiteralPath $path) { throw "Refusing existing OEL1 share artifact: $path" } }

    if ($Preflight) {
        [ordered]@{
            schema = 'argos_oel1_publish_preflight_v1'
            createdUtc = [DateTime]::UtcNow.ToString('o')
            state = 'PASS_OEL1_PUBLISH_PREFLIGHT'
            requestId = $requestId
    ...
    if ((Get-Item -LiteralPath $upload).Length -ne $expectedBytes -or (Get-FileHash -LiteralPath $upload -Algorithm SHA256).Hash -ne $expectedSha) { throw 'OEL1 uploaded request ZIP hash mismatch.' }
    Move-Item -LiteralPath $upload -Destination $ready -ErrorAction Stop
    if ((Get-Item -LiteralPath $ready).Length -ne $expectedBytes -or (Get-FileHash -LiteralPath $ready -Algorithm SHA256).Hash -ne $expectedSha) { throw 'OEL1 published request ZIP hash mismatch.' }
    $gate = [ordered]@{
        schema = 'argos_oel1_publish_gate_v1'
        publishedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OEL1_EXACT_SIGNED_REQUEST_PUBLISHED_CREATE_NEW'
