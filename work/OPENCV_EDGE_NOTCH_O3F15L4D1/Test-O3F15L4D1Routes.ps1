...
function Write-NewJson([string]$Path, [object]$Value) {
    Require (-not (Test-Path -LiteralPath $Path)) "O3F15L4D1 create-new route gate exists: $Path"
...
function New-BoundaryPath([int]$Length) {
    ...
    Require ($remaining -ge 1) 'O3F15L4D1 boundary path target is too short.'
    $value + ('x' * $remaining)
}
function Test-Boundary([int]$EffectiveLength, [string]$PowerShell, [string]$PathTool, [int]$Reserve) {
    ...
    Require ($exitCode -eq $expectedExit -and [string]$result.state -ceq $expectedState -and [int]$result.candidates[0].effectiveLength -eq $EffectiveLength -and [bool]$result.metadataOnly -and -not [bool]$result.fileContentRead) "O3F15L4D1 path boundary failed: $EffectiveLength"
...
Require ([string]$PSVersionTable.PSEdition -ceq 'Desktop' -and [int]$PSVersionTable.PSVersion.Major -eq 5 -and [int]$PSVersionTable.PSVersion.Minor -eq 1) 'O3F15L4D1 route gate requires exact Windows PowerShell 5.1.'
...
$outputPath = Join-Path $finalRoot 'O3F15L4D1_PREPUBLICATION_PATH_GATE.json'
$pathTool = Join-Path $project 'utilities\Confirm-ArgosPathBudget.ps1'
$windowsPowerShell = Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe'
foreach ($path in @($contractPath,$signGatePath,$queueGatePath,$pathTool,$windowsPowerShell)) { Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L4D1 route dependency absent: $path" }
...
Require ([string]$contract.schema -ceq 'argos_ocv03_o3f15l4d1_diagnostic_contract_v1' -and [string]$contract.state -ceq 'FROZEN_FOR_BUILD') 'O3F15L4D1 route contract changed.'
Require ([string]$signGate.state -ceq 'PASS_O3F15L4D1_SIGNED_PREFLIGHT_DIAGNOSTIC_PACKAGE') 'O3F15L4D1 route sign gate changed.'
Require ((Sha $queueGatePath) -ceq '170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D' -and [string]$queueGate.state -ceq 'PASS_JBOD_PORTAL_QUEUE_RECOVERY_V2_1_REHEARSAL' -and [int]$queueGate.checkCount -eq 16 -and @($queueGate.checks | Where-Object { [string]$_.state -cne 'PASS' }).Count -eq 0) 'O3F15L4D1 inherited queue-safety evidence changed.'
...
foreach ($name in $requiredQueueChecks) { Require (@($queueGate.checks | Where-Object { [string]$_.name -ceq $name -and [string]$_.state -ceq 'PASS' }).Count -eq 1) "O3F15L4D1 inherited queue check absent: $name" }
...
Require ([IO.Path]::GetFullPath([string]$signGate.finalRoot) -ceq [IO.Path]::GetFullPath($finalRoot) -and [IO.Path]::GetDirectoryName([IO.Path]::GetFullPath([string]$signGate.packageZipPath)) -ceq [IO.Path]::GetFullPath($finalRoot)) 'O3F15L4D1 final ZIP is not adjacent to its path gate.'
...
Require (Test-Path -LiteralPath $manifestPath -PathType Leaf) 'O3F15L4D1 signed manifest absent.'
$manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
Require ((Sha $manifestPath) -ceq [string]$signGate.manifestSha256 -and [string]$manifest.requestId -ceq [string]$signGate.requestId -and @($manifest.files).Count -eq 16 -and [int64]$manifest.maxResultBytes -eq 1048576) 'O3F15L4D1 signed route manifest changed.'
...
foreach ($path in @(
    ...
    "U:/ProjectPortalRO/responses/$responseZip", "C:/O3F15L4D1C/$responseZip",
    "C:/O3F15L4D1C/$responseName/PORTAL_RESPONSE_MANIFEST.json", "C:/O3F15L4D1C/$responseName/MAINTENANCE.stdout.txt",
    "C:/O3F15L4D1C/$responseName/MAINTENANCE.stderr.txt", "C:/O3F15L4D1C/$responseName/RESULT.json"
...
Require ([string]$actionableGate.state -ceq 'PASS_PATH_BUDGET' -and @($actionableGate.candidates).Count -eq $actionable.Count) 'O3F15L4D1 actionable round-trip path gate failed.'
...
Require ([int]$longest.effectiveLength -lt 200 -and $componentMaximum -le 80) 'O3F15L4D1 actionable route exceeded a safe path bound.'
$boundaries = @(199,200,229,230 | ForEach-Object { Test-Boundary $_ $windowsPowerShell $pathTool 32 })
$result = [ordered]@{
    schema = 'argos_ocv03_o3f15l4d1_complete_route_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_O3F15L4D1_COMPLETE_ROUTE_PREFLIGHT' } else { 'PASS_O3F15L4D1_SIGNED_AND_PATH_GATED' })
    requestId = $requestId
    finalRoot = $finalRoot
    ...
    expandedSigningRootIsShortPhysicalRoot = $packagePath.StartsWith('C:\O3F15L4D1PK\signed\',[StringComparison]::OrdinalIgnoreCase)
...
Require ([bool]$result.expandedSigningRootIsShortPhysicalRoot) 'O3F15L4D1 expanded signing tree is not on its short physical root.'
