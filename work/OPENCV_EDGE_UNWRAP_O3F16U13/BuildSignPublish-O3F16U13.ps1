...
function New-Json([string]$Path, [object]$Value) {
    Require (-not (Test-Path -LiteralPath $Path)) "O3F16U13 create-new JSON exists: $Path"
...
Require ([string]$PSVersionTable.PSEdition -ceq 'Desktop' -and [int]$PSVersionTable.PSVersion.Major -eq 5 -and [int]$PSVersionTable.PSVersion.Minor -eq 1) 'O3F16U13 package utility requires Windows PowerShell 5.1.'

$project = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'))
$packageRoot = 'C:\O3F16U13PK1'
$unsignedPayload = Join-Path $packageRoot 'payload'
$signedRoot = Join-Path $packageRoot 'signed'
$finalRoot = Join-Path $PSScriptRoot 'final_o3f16u13'
$signGatePath = Join-Path $PSScriptRoot 'O3F16U13_SIGN_GATE.json'
$routeGatePath = Join-Path $finalRoot 'O3F16U13_PREPUBLICATION_PATH_GATE.json'
$attemptPath = Join-Path $PSScriptRoot 'O3F16U13_PUBLISH_ATTEMPT.json'
$publishGatePath = Join-Path $PSScriptRoot 'O3F16U13_PUBLISH_GATE.json'
$invokeSource = Join-Path $PSScriptRoot 'Invoke-O3F16U13.ps1'
$r13Source = Join-Path $project 'work\O3F8\AnnularUnwrapDiagnosticOpenCvR13.py'
...
$priorTerminalGate = Join-Path $project 'work\OPENCV_EDGE_UNWRAP_O3F16U12\O3F16U12_PULL_TERMINAL_VISUAL_GATE.json'
...
$invokeHash = 'ABE77E0744D6A844F7DCD084019CCBDA9FFA88CDB0BB5B9F66397EFF4A60E8AD'
...
foreach ($pin in @(
    ...
    @($priorTerminalGate, 'F41EA5ADDAC954299971BD733775D4D023F97F8519826DBA10531F63D62214F4')
)) {
    Require (Test-Path -LiteralPath $pin[0] -PathType Leaf) "O3F16U13 dependency absent: $($pin[0])"
    Require ((Sha $pin[0]) -ceq $pin[1]) "O3F16U13 dependency hash changed: $($pin[0])"
}
foreach ($path in @($identityPath, $certificatePath, $verifier)) {
    Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F16U13 signing dependency absent: $path"
}
$prior = Get-Content -LiteralPath $priorTerminalGate -Raw | ConvertFrom-Json
Require ([string]$prior.pullRequestId -ceq 'REQ_20260904T133028436Z_0AF28A2F2529' -and [string]$prior.pullResponseId -ceq 'R_C6592AF3282C_20260904133455935_5ccc11ad' -and [bool]$prior.pullResponseSignatureVerified) 'O3F16U13 prior serial request is not signed-terminal U12 DATA_PULL.'
$parentRoute = Get-Content -LiteralPath $parentRouteGate -Raw | ConvertFrom-Json
Require ([string]$parentRoute.state -ceq 'PASS_O3F8R13T5_COMPLETE_ROUTE_PATH_R4_GATE' -and [int]$parentRoute.maximumEffectiveLength -lt 200 -and [int]$parentRoute.maximumComponentLength -le 80) 'O3F16U13 inherited route gate changed.'
...
$plannedCandidates = @(
    ...
    $signedRoot,
    $finalRoot,
    (Join-Path $plannedReady 'payload\AnnularUnwrapDiagnosticOpenCvR13.py'),
...
Require ([string]$pathCheck.state -ceq 'PASS_PATH_BUDGET') 'O3F16U13 package/publication path preflight failed.'
$maximumPlannedEffectiveLength = [int](($pathCheck.candidates | Measure-Object -Property effectiveLength -Maximum).Maximum)

if ($Preflight) {
    Require (-not (Test-Path -LiteralPath $packageRoot)) 'O3F16U13 package root already exists.'
    Require (-not (Test-Path -LiteralPath $finalRoot)) 'O3F16U13 final root already exists.'
    Require (-not (Test-Path -LiteralPath $signGatePath)) 'O3F16U13 sign gate already exists.'
    [ordered]@{
        schema = 'argos_ocv03_o3f16u13_build_sign_publish_preflight_v1'
        state = 'PASS_O3F16U13_BUILD_SIGN_PUBLISH_PREFLIGHT'
        r13Sha256 = $r13Hash
        payloadFileCount = 3
        sourcePayloadPinsPassed = $true
        requestedCaseCount = 4
        inheritedRouteGateSha256 = Sha $parentRouteGate
        priorSignedTerminalRequestId = [string]$prior.pullRequestId
...
if ($BuildSign) {
    foreach ($path in @($packageRoot, $finalRoot, $signGatePath)) {
        Require (-not (Test-Path -LiteralPath $path)) "O3F16U13 create-new build target exists: $path"
    }
    ...
    [IO.File]::Copy($invokeSource, (Join-Path $unsignedPayload 'Invoke-O3F16U13.ps1'), $false)
    [IO.File]::Copy($r13Source, (Join-Path $unsignedPayload 'AnnularUnwrapDiagnosticOpenCvR13.py'), $false)
    ...
    Require ($actual.Count -eq 3) 'O3F16U13 unsigned payload cardinality changed.'
    Require ((Sha (Join-Path $unsignedPayload 'Invoke-O3F16U13.ps1')) -ceq $invokeHash) 'O3F16U13 copied entrypoint changed.'
    Require ((Sha (Join-Path $unsignedPayload 'AnnularUnwrapDiagnosticOpenCvR13.py')) -ceq $r13Hash) 'O3F16U13 copied detector changed.'
    Require ((Sha (Join-Path $unsignedPayload 'OCV03_NotchReviewOpenCvV1.py')) -ceq $carrierHash) 'O3F16U13 copied carrier changed.'
    $leaf = & (Join-Path $unsignedPayload 'Invoke-O3F16U13.ps1') -PackageLeafPreflight | ConvertFrom-Json
    Require ([string]$leaf.state -ceq 'PASS_O3F16U13_PACKAGE_LEAVES') 'O3F16U13 assembled package-leaf preflight failed.'
    ...
    Require ([bool]$certificate.HasPrivateKey) 'O3F16U13 signer private key unavailable.'
    ...
    $manifest = [ordered]@{
        ...
        files = $files
        entryPoint = 'payload/Invoke-O3F16U13.ps1'
        changes = @([ordered]@{
        ...
        entryPointMutations = @(
            [ordered]@{action = 'COPY_EXACT_R13_TO_EXISTING_JBOD_DEVELOPMENT_ROOT'; target = 'D:/O3F15L4E5RT/AnnularUnwrapDiagnosticOpenCvR13.py'},
            [ordered]@{action = 'RUN_FOUR_CASE_REVIEW_ONLY_ANNULAR_DIAGNOSTIC_FOREGROUND'; target = 'D:/O3F16U13'},
            [ordered]@{action = 'CREATE_PORTAL_READABLE_RESULT_ARCHIVE'; target = 'D:/KLARFExport/_ArgosReview/O3F16U13_20260904.zip'}
        )
        entryPointOutputs = @(
            [ordered]@{path = 'D:/O3F16U13/SUMMARY.json'; requiredAtCompletion = $true},
            [ordered]@{path = 'D:/KLARFExport/_ArgosReview/O3F16U13_20260904.zip'; requiredAtCompletion = $true}
        )
        sourceProcessingContract = [ordered]@{
            side = 'FRONT'; expectedPairCount = 4; detectorRevision = 'R13'; detectorSha256 = $r13Hash
            outputRoot = 'D:/O3F16U13'; sourceMutationAllowed = $false; selectorThresholdRelaxationAllowed = $false
        ...
        allowedTaskActions = @()
        allowedProcessActions = @('START_ONE_BOUNDED_OWNED_O3F16U13_DETECTOR_CHILD', 'TERMINATE_ONLY_NEWLY_OWNED_CHILD_ON_TIMEOUT')
    ...
    $routeGate = [ordered]@{
        schema = 'argos_ocv03_o3f16u13_prepublication_path_gate_v1'; state = 'PASS_O3F16U13_COMPLETE_ROUTE_PATH_GATE'
    ...
    $signGate = [ordered]@{
        schema = 'argos_ocv03_o3f16u13_sign_gate_v1'; state = 'PASS_O3F16U13_SIGNED_FOUR_CASE_PACKAGE'
        ...
        r13Sha256 = $r13Hash; requestedCaseCount = 4; signed = $true; published = $false
...
Require (Test-Path -LiteralPath $signGatePath -PathType Leaf) 'O3F16U13 sign gate is absent.'
Require (Test-Path -LiteralPath $routeGatePath -PathType Leaf) 'O3F16U13 route gate is absent.'
Require (-not (Test-Path -LiteralPath $attemptPath)) 'O3F16U13 publication attempt already exists; retry forbidden.'
Require (-not (Test-Path -LiteralPath $publishGatePath)) 'O3F16U13 publication gate already exists.'
...
Require ([string]$sign.state -ceq 'PASS_O3F16U13_SIGNED_FOUR_CASE_PACKAGE' -and [string]$route.state -ceq 'PASS_O3F16U13_COMPLETE_ROUTE_PATH_GATE') 'O3F16U13 signed or route gate changed.'
$zipPath = [string]$sign.packageZipPath
Require (Test-Path -LiteralPath $zipPath -PathType Leaf) 'O3F16U13 signed ZIP is absent.'
Require ((Sha $zipPath) -ceq [string]$sign.packageZipSha256 -and (Sha $routeGatePath) -ceq [string]$sign.routeGateSha256) 'O3F16U13 signed package evidence changed.'
...
Require ([string]$drive.DisplayRoot -ceq $expectedShare -and [string]$disk.ProviderName -ceq $expectedShare -and [int]$disk.DriveType -eq 4) 'O3F16U13 persistent U: mapping changed.'
Require (Test-Path -LiteralPath $requestRoot -PathType Container) 'O3F16U13 request root unavailable.'
$pending = @(Get-ChildItem -LiteralPath $requestRoot -File | Where-Object { $_.Name -cmatch '\.ready\.zip(?:\.upload)?$' })
Require ($pending.Count -eq 0) ('O3F16U13 blocked by pending request: ' + (($pending | ForEach-Object { $_.Name }) -join ', '))
$requestId = [string]$sign.requestId
$readyPath = Join-Path $requestRoot ($requestId + '.ready.zip')
$uploadPath = $readyPath + '.upload'
Require (-not (Test-Path -LiteralPath $readyPath) -and -not (Test-Path -LiteralPath $uploadPath)) 'O3F16U13 request path collision.'
$attempt = [ordered]@{
    schema = 'argos_ocv03_o3f16u13_publish_attempt_v1'; state = 'STARTED_O3F16U13_SINGLE_PUBLICATION_ATTEMPT'
...
Require ((Get-Item -LiteralPath $uploadPath).Length -eq [int64]$sign.packageZipBytes -and (Sha $uploadPath) -ceq [string]$sign.packageZipSha256) 'O3F16U13 upload verification failed; no retry authorized.'
$transition = @(Get-ChildItem -LiteralPath $requestRoot -File | Where-Object { $_.Name -cmatch '\.ready\.zip(?:\.upload)?$' })
Require ($transition.Count -eq 1 -and $transition[0].FullName.Equals($uploadPath, [StringComparison]::OrdinalIgnoreCase)) 'O3F16U13 queue changed before atomic publication.'
[IO.File]::Move($uploadPath, $readyPath)
$publishGate = [ordered]@{
    schema = 'argos_ocv03_o3f16u13_publish_gate_v1'; state = 'PASS_O3F16U13_PUBLISHED_EXACTLY_ONCE_AWAITING_SIGNED_TERMINAL_RESPONSE'
    ...
    priorSignedTerminalRequestId = [string]$prior.pullRequestId; priorSignedTerminalResponseId = [string]$prior.pullResponseId
