...
$expectedInvocation = Join-Path $PSScriptRoot 'O3F15L4D2_PUBLISH_P2_INVOCATION.json'
...
Assert-True ([string](Get-RequiredProperty $invocation 'schema') -ceq 'argos_ocv03_o3f15l4d2_publish_p2_invocation_v1') 'D2 P2 publication invocation schema changed.'
Assert-True ([string](Get-RequiredProperty $invocation 'state') -ceq 'FROZEN_O3F15L4D2_EXACTLY_ONCE_PUBLICATION_P2_INVOCATION') 'D2 P2 publication invocation is not frozen.'
...
Assert-True ([string]$routeGate.schema -ceq 'argos_ocv03_o3f15l4d2_complete_route_path_r4_gate_v1' -and [string]$routeGate.state -ceq 'PASS_O3F15L4D2_COMPLETE_ROUTE_PATH_R4_GATE' -and [string]$routeGate.requestId -ceq $requestId -and [string]$routeGate.signedPackageZipSha256 -ceq $sourceHash -and [int]$routeGate.publicationCountMaximum -eq 1 -and -not [bool]$routeGate.requestRetryAuthorized) 'D2 R4 prepublication route gate changed.'
...
Assert-True ($publicationGatePath.Equals((Join-Path $projectRoot 'work\OPENCV_EDGE_NOTCH_O3F15L4D2\O3F15L4D2_PUBLISH_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication gate path changed.'
