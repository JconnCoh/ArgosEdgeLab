...
function Get-O3F13TextSha256([string]$Value) { $algorithm=[Security.Cryptography.SHA256]::Create(); try { return [BitConverter]::ToString($algorithm.ComputeHash([Text.Encoding]::UTF8.GetBytes($Value))).Replace('-','').ToLowerInvariant() } finally { $algorithm.Dispose() } }
function Get-O3F13SafeId([string]$Identity) { $clean=[regex]::Replace($Identity,'[^A-Za-z0-9_.-]+','_').Trim([char[]]@('_','.')); $suffix=(Get-O3F13TextSha256 $Identity).Substring(0,10); if([string]::IsNullOrWhiteSpace($clean)){return $suffix}; $prefix=$(if($clean.Length-gt42){$clean.Substring(0,42)}else{$clean}); return ($prefix+'_'+$suffix) }
function Write-O3F13Json([string]$Path, [object]$Value) { Assert-O3F13 (-not (Test-Path -LiteralPath $Path)) "O3F13 route gate exists: $Path"; [IO.File]::WriteAllText($Path, (($Value | ConvertTo-Json -Depth 12) + [Environment]::NewLine), (New-Object Text.UTF8Encoding($false))) }
...
$outputPath = Join-Path (Join-Path $PSScriptRoot 'final_o3f13') 'O3F13_PREPUBLICATION_PATH_GATE.json'
...
Assert-O3F13 ((Get-O3F13Hash $aliasPlanPath) -eq [string]$spec.sourceAliasPlanSha256 -and [string]$aliasPlan.state -eq 'FROZEN_FOR_BUILD' -and [int]$aliasPlan.caseCount -eq 6 -and [int]$aliasPlan.sourceLeafCount -eq 12) 'O3F13 route source-alias plan changed.'
...
Assert-O3F13 ($canonicalSources.Count -eq 12 -and $aliasSources.Count -eq 12 -and @($canonicalSources | Sort-Object -Unique).Count -eq 12 -and @($aliasSources | Sort-Object -Unique).Count -eq 12) 'O3F13 route source path cardinality changed.'
$safeIdRows = New-Object Collections.Generic.List[object]
$exactDev6Leaves = New-Object Collections.Generic.List[string]
foreach($devRoot in @('D:/O3F9D13','D:/O3F9D13.failed')) {
    ...
    foreach($case in @($aliasPlan.cases)) {
        ...
        $safeId=Get-O3F13SafeId $identity
        $stem=$safeId.ToLowerInvariant().Replace('-','')
        $digest=(Get-O3F13TextSha256 $safeId).Substring(0,16)
        $caseRoot=("$devRoot/cases/C{0:D4}" -f $ordinal)
        if($devRoot-eq'D:/O3F9D13'){$safeIdRows.Add([pscustomobject]@{ordinal=$ordinal;identity=$identity;safeId=$safeId;stem=$stem;digest=$digest})}
...
Assert-O3F13 ($safeIds.Count-eq6-and@($safeIds.safeId|Sort-Object -Unique).Count-eq6-and[string]$safeIds[4].safeId-eq'PatternedFront_Lot_62616-131_62616-131_202_aae0de7fdf') 'O3F13 exact O3F6 safe-ID derivation changed.'
$exactGateLeaves=@('SUMMARY.json.partial','R10_SYMMETRIC_GATE.json','LOCAL.stdout.txt','LOCAL.stderr.txt','INHERITED.stdout.txt','INHERITED.stderr.txt','R10_INHERITED_SYNTHETIC/upper_right_315_df_c01_enhanced.png','R10_INHERITED_SYNTHETIC/SYNTHETIC_GATE.json.partial')|ForEach-Object{"D:/O3F9G13/$_","D:/O3F9G13.failed/$_"}
...
foreach ($path in @(
    (Join-Path $project "work/OPENCV_EDGE_NOTCH_O3F13/final_o3f13/$zipName"),
    ...
    'D:/O3F9G13/SUMMARY.json', 'D:/O3F9G13.failed/SUMMARY.json',
    'D:/O3F9D13/SUMMARY.json', 'D:/O3F9D13.failed/SUMMARY.json',
    ...
    "U:/ProjectPortalRO/responses/$responseName.zip", "C:/A13C/$responseName.zip",
    "C:/A13C/$responseName/PORTAL_RESPONSE_MANIFEST.json", "C:/A13C/$responseName/MAINTENANCE.stdout.txt"
)) { $paths.Add([string]$path) }
$pathRows = New-Object Collections.Generic.List[object]
$pathTool = Join-Path $project 'utilities\Confirm-ArgosPathBudget.ps1'
foreach ($path in $paths.ToArray()) { $one = & $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json; Assert-O3F13 ([string]$one.state -eq 'PASS_PATH_BUDGET') "O3F13 route path failed: $path"; $row=@($one.candidates)[0]; $pathRows.Add([pscustomobject]@{path=[string]$row.path;effectiveLength=[int]$row.effectiveLength;longestComponentLength=[int]$row.longestComponentLength;state=[string]$row.disposition}) }
$windowsPowerShell = Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe'
Assert-O3F13 (Test-Path -LiteralPath $windowsPowerShell -PathType Leaf) 'O3F13 Windows PowerShell 5.1 path gate host is absent.'
foreach ($path in $canonicalSources) {
    ...
    Assert-O3F13 ($exit -eq 1 -and [string]$one.state -eq 'SHORT_ALIAS_REQUIRED_BEFORE_WRITE_OR_LAUNCH') "O3F13 canonical source did not require its frozen alias: $path"
    $row = @($one.candidates)[0]
    Assert-O3F13 ([int]$row.effectiveLength -ge 200 -and [int]$row.effectiveLength -lt 230 -and [int]$row.longestComponentLength -le 80) "O3F13 canonical source crossed an unhandled path boundary: $path"
...
foreach ($path in $aliasSources) {
    ...
    Assert-O3F13 ([string]$one.state -eq 'PASS_PATH_BUDGET') "O3F13 actionable source alias path failed: $path"
    $row = @($one.candidates)[0]
...
Assert-O3F13 ($rows.Count -ge 75 -and @($rows.path | Sort-Object -Unique).Count -eq $rows.Count) 'O3F13 complete route cardinality changed.'
...
Write-O3F13Json $outputPath $result
