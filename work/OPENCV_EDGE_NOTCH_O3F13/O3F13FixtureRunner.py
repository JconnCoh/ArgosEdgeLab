#!/usr/bin/env python3
"""No-image fixture for the exact O3F13 endpoint orchestration rehearsal."""
...
def fail_if(stage: str) -> None:
    if os.environ.get("ARGOS_O3F13_FIXTURE_FAIL_STAGE", "").upper() == stage:
        raise RuntimeError(f"INJECTED_O3F13_{stage}_FAILURE")


def timeout_with_owned_alias() -> None:
    if os.environ.get("ARGOS_O3F13_FIXTURE_FAIL_STAGE", "").upper() != "DEV6_TIMEOUT_ALIAS":
        return
    subst_text = os.environ.get("ARGOS_O3F13_FIXTURE_SUBST_PATH", "")
    anchor_text = os.environ.get("ARGOS_O3F13_FIXTURE_ALIAS_ANCHOR", "")
    ...
    if not subst.is_absolute() or not subst.is_file() or not anchor.is_absolute() or not anchor.is_dir():
        raise RuntimeError("O3F13 timeout-alias fixture environment is invalid")
    ...
    def mappings() -> dict[str, str]:
        ...
        if child.returncode != 0 or child.stderr:
            raise RuntimeError("O3F13 timeout-alias subst query failed")
        result: dict[str, str] = {}
    ...
    if "Q:" in mappings():
        raise RuntimeError("O3F13 timeout-alias Q: is already mapped")
    ...
    if create.returncode != 0 or create.stderr:
        raise RuntimeError("O3F13 timeout-alias creation failed")
    ...
    if target is None or os.path.normcase(os.path.normpath(target)) != os.path.normcase(os.path.normpath(str(anchor))):
        raise RuntimeError("O3F13 timeout-alias target verification failed")
    time.sleep(3600)  # Endpoint timeout/backstop must own cleanup for this injected case.
    raise RuntimeError("O3F13 timeout-alias fixture unexpectedly resumed")


def gate(root: Path) -> dict[str, object]:
    fail_if("GATE")
    summary = {
        "schema": "argos_ocv03_o3f13_gate_result_v1",
        "state": "COMPLETE_O3F12_GATE",
        "stage": "GATE",
        "fixtureOnly": True,
    ...
    return {"state": "COMPLETE_O3F12_GATE", "stage": "GATE", "summarySha256": sha256(target), "commands": []}
...
def dev6(root: Path, prerequisite: Path, expected_sha256: str) -> dict[str, object]:
    ...
    mode = os.environ.get("ARGOS_O3F13_FIXTURE_DEV6_MODE", "COMPLETE").upper()
    hold_result = mode in {"HOLD", "EXIT0_HOLD", "EXIT3_HOLD", "MALFORMED", "TWO_OBJECTS", "STDERR"}
    results: list[dict[str, object]] = []
    state_counts: dict[str, int] = {}
    for index in range(1, 7):
        if hold_result and index == 6:
            state = "HOLD_O3F12_R10_PROVIDER_ERROR"
            state_counts[state] = state_counts.get(state, 0) + 1
            results.append({"ordinal": index, "identity": f"fixture_identity_{index:02d}", "safeId": f"fixture_{index:02d}", "priorR8State": "HOLD_NO_BF_TOPOLOGY_DF_RADIAL_NOTCH", "baselineState": None, "inheritedR9State": None, "finalState": state, "r10Invoked": False, "physicalClusterCount": 0, "selectedClusterDirections": [], "dfSeedCount": 0, "dfSeedEligibleCount": 0, "bfSeedCount": 0, "bfHypothesisCount": 0, "bfHypothesisEligibleCount": 0, "eligibleHypotheses": [], "execution": "HOLD_R10_CHILD", "error": "INJECTED_O3F13_STRUCTURED_PROVIDER_HOLD"})
            continue
        observed = fixture_observation(index)
        manifest_path = root / "cases" / f"C{index:04d}" / "MANIFEST.json"
        manifest = {"schema": "argos_ocv03_full_perimeter_topology_manifest_v2", "revision": f"O3F13_FIXTURE_{index:04d}", "results": [observed]}
    ...
    summary = {
        "schema": "argos_ocv03_o3f13_staged_result_v1",
        "state": "HOLD_O3F12_DEV6_EXECUTION" if hold_result else "COMPLETE_O3F12_DEV6",
        "stage": "DEV6",
        "selectedCount": 6,
...
def main() -> int:
    ...
    if args.stage == "SELF_TEST":
        print(json.dumps({"state": "PASS_O3F12_STAGED_RUNNER_SELF_TEST", "mutationsPerformed": False}, separators=(",", ":")))
        return 0
    ...
    mode = os.environ.get("ARGOS_O3F13_FIXTURE_DEV6_MODE", "COMPLETE").upper() if args.stage == "DEV6" else "COMPLETE"
    if mode == "MALFORMED":
        print('{not-json')
        ...
        print(json.dumps(result, separators=(",", ":")))
    if mode == "STDERR":
        print("INJECTED_O3F13_STDERR", file=os.sys.stderr)
    if mode == "EXIT0_HOLD":
        return 0
