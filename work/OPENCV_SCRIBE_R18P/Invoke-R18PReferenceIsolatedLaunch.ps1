...
param(
    ...
    [switch]$Rehearsal,
    [string]$PayloadRoot = '',
    [string]$WorkRoot = 'D:\A2\w\ocv\R18P1',
    [string]$OutputRoot = 'D:\A2\o\ocv\R18P1',
...
$manifestSha = 'B5E6296C8266542CB34B53F53033858948F7259A7BA5B8D06ED594947F7B7EF6'
...
$runnerSha = '5B6AA224E844FCEA22DEC6A9D10C863437F039CA73EC258C263DF539905791D0'
...
$payloadManifestPath = Join-Path $payload 'R18P_PAYLOAD_MANIFEST.json'
$partial = $WorkRoot + '.partial'
$failed = $WorkRoot + '.failed'
$runtimeConfiguration = Join-Path $WorkRoot 'RUNTIME_CONFIGURATION.json'
$runner = Join-Path $WorkRoot 'OPENCV_SCRIBE_R18P\Run-R18PReferenceIsolatedCorpus.py'
$cohortPath = Join-Path $WorkRoot 'OPENCV_SCRIBE_R18P\R18P_REVIEW_COHORT.json'
...
function Get-CorpusRows {
    return @(Get-CimInstance Win32_Process -Filter "Name='python.exe' OR Name='pythonw.exe'" -ErrorAction Stop |
        Where-Object { [string]$_.CommandLine -like '*Run-R18PReferenceIsolatedCorpus.py*' } |
        Sort-Object ProcessId |
        ForEach-Object { [pscustomobject]@{processId=[uint32]$_.ProcessId;creationDate=[string]$_.CreationDate;commandLine=[string]$_.CommandLine} })
}
...
Require ([string]$manifest.schema -eq 'argos_opencv_scribe_r18p_payload_manifest_v1' -and [string]$manifest.revision -eq $revision) 'R18P payload manifest contract changed.'
...
Require ($files.Count -eq 24) 'R18P payload file cardinality changed.'
...
$cohortSource = Join-Path $payload 'files\OPENCV_SCRIBE_R18P\R18P_REVIEW_COHORT.json'
...
Require ((Get-Sha256 $cohortSource) -eq $cohortSha) 'R18P cohort hash changed.'
Require ([string]$cohort.schema -eq 'argos_opencv_scribe_r18p_review_cohort_v1' -and [string]$cohort.revision -eq $revision) 'R18P cohort contract changed.'
Require ([int]$cohort.caseCount -eq 20 -and @($cohort.reviewCases).Count -eq 20) 'R18P cohort cardinality changed.'
Require ([bool]$cohort.authority.reviewOnly -and -not [bool]$cohort.authority.identityAcceptanceAuthorized -and -not [bool]$cohort.authority.automaticReferenceAdmissionAuthorized -and -not [bool]$cohort.authority.trainingAuthorized -and -not [bool]$cohort.authority.activationAuthorized -and -not [bool]$cohort.authority.xmlAuthorized -and -not [bool]$cohort.authority.productionAuthorized) 'R18P cohort authority changed.'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_opencv_scribe_r18p_launch_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
        state='PASS_R18P_REFERENCE_ISOLATED_LAUNCH_PREFLIGHT';revision=$revision;rehearsal=[bool]$Rehearsal;
        payloadFileCount=$files.Count;payloadManifestSha256=$manifestSha;pythonSha256=$ExpectedPythonSha256;
        referenceBundleSha256=$refsSha;cohortSha256=$cohortSha;cohortCaseCount=20;proposalRoot=$ProposalRoot;workRoot=$WorkRoot;
...
try {
    ...
    $launch = [ordered]@{
        schema='argos_opencv_scribe_r18p_reference_isolated_launch_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
        state='PASS_R18P_REFERENCE_ISOLATED_WORKER_STARTED';revision=$revision;computerName=$env:COMPUTERNAME;
        ...
        cropSweepSha256=$cropSweepSha;runnerSha256=$runnerSha;cohortSha256=$cohortSha;cohortCaseCount=20;baseReferenceManifestSha256=$baseManifestSha;
    ...
    if ((Test-Path -LiteralPath $OutputRoot -PathType Container) -and -not (Test-Path -LiteralPath $failurePath)) {
        Write-JsonNew $failurePath ([ordered]@{schema='argos_opencv_scribe_r18p_launch_failure_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='HOLD_R18P_REFERENCE_ISOLATED_LAUNCH_FAILURE';detail=$detail;ownedChildTerminated=$true;automaticRetryAllowed=$false;sourceMutationPerformed=$false;identityAccepted=$false;reviewOnly=$true;productionRoutingEnabled=$false}) 8
    }
    throw
}
