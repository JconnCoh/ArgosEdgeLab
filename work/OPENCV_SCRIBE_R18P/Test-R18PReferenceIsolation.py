#!/usr/bin/env python3
"""Non-image gate for generic reference isolation and cohort integrity."""
...
def load_runner(path: Path) -> Any:
    spec = importlib.util.spec_from_file_location("r18p_reference_isolation_gate", path)
...
def main() -> int:
    ...
    engine_rows = [
        row for row in payload_manifest.get("files", [])
    ...
    if len(engine_paths) < 3:
        raise AssertionError("Packaged Python engine set is unexpectedly incomplete.")
    ...
    if int(cohort["caseCount"]) != len(cases) or len(cases) > 25:
        raise AssertionError("Cohort cardinality is not bounded and exact.")
    ...
    if len(set(pairs)) != len(pairs):
        raise AssertionError("Cohort contains duplicate source-image pairs.")
    ...
    for case, pair in zip(cases, pairs):
        ...
        case_lineage = runner.physical_lineage_key(str(case["physicalIdentity"]))
        retained_lineages = set()
        for reference in filtered[0]:
            try:
                retained_lineages.add(runner.physical_lineage_key(reference.physical_identity))
            except ValueError:
                pass
    ...
    print(json.dumps({
        "schema": "argos_opencv_scribe_r18p_reference_isolation_local_gate_v1",
        "state": "PASS_R18P_REFERENCE_ISOLATION_LOCAL_GATE",
        "caseCount": len(cases),
