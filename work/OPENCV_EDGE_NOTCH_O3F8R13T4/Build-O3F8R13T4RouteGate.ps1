...
$outputPath=Join-Path $PSScriptRoot 'final_o3f8r13t4\O3F8R13T4_PREPUBLICATION_PATH_R4_GATE.json'
...
Need ([string]$sign.state-eq'PASS_O3F8R13T4_SIGNED_TARGETED_PACKAGE'-and[int]$sign.payloadFileCount-eq28) 'Target sign gate changed'
...
$manifest=Get-Content -Raw -LiteralPath $manifestPath|ConvertFrom-Json;Need (@($manifest.files).Count-eq28) 'Signed payload cardinality changed'
...
foreach($row in @($old.routePaths)){
    $path=([string]$row.path).Replace($oldId,$newId).Replace('O3F15L4E3','O3F8R13T4').Replace('o3f15l4e3','o3f8r13t4')
