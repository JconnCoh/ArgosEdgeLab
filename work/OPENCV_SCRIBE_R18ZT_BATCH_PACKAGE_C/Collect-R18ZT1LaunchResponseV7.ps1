...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v7' -and
    [string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION_V7' -and [string]$invocation.lifecycle -ceq 'FROZEN') 'R18ZT publication invocation is not frozen; collection refused.'
...
Require ([string]$applyIntent.schema -ceq 'argos_opencv_scribe_r18zt1_publication_apply_intent_v7' -and
    [string]$applyIntent.state -ceq 'FROZEN_R18ZT1V7_APPLY_INTENT_BEFORE_EXTERNAL_WRITE' -and
...
if ($publishGateExists) {
    ...
    Require ([string]$publishGate.schema -ceq 'argos_opencv_scribe_r18zt1_publish_gate_v7' -and
        [string]$publishGate.state -ceq 'PASS_R18ZT1V7_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' -and
...
$preflightResult = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_launch_response_collection_gate_v7'
    collectedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V7_ONE_SIGNED_LAUNCH_RESPONSE_PREFLIGHT' } else { 'PASS_R18ZT1V7_SIGNED_LAUNCH_RESPONSE_COLLECTED' })
