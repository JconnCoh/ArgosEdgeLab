...
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZT_C12_REM.json'
...
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZT_C12_RAW.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZT_C12_CLONE.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZT_PATH_PLAN_GATE_V3.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZT_C12_PS.json'
...
$preactionPath = Join-Path $PSScriptRoot 'R18ZT_C12_PRE.json'
$collisionGatePath = Join-Path $PSScriptRoot 'R18ZT_REQUEST_ID_COLLISION_GATE_V3.json'
$collisionScannerPath = Join-Path $PSScriptRoot 'Test-R18ZT1RequestIdUniquenessV3.ps1'
...
$testRoot = 'C:\R18ZT1C4T'
$testWorkRoot = 'C:\R18ZT1C4TW'
$testOutputRoot = 'C:\R18ZT1C4TO'
$stageRoot = 'C:\R18ZT1C4P'
$readyRoot = Join-Path $stageRoot ($requestId + '.ready')
$stageZip = Join-Path $stageRoot ($requestId + '.ready.zip')
$verifyRoot = 'C:\R18ZT1C4V'
...
$collisionGate = $null
if ($Build) { $collisionGate = Read-BoundGate $collisionGatePath 'PASS_R18ZT1_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS' }

$builderPath = [IO.Path]::GetFullPath($MyInvocation.MyCommand.Path)
$builderSha = Get-Sha256 $builderPath
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV9.ps1' = $builderSha
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/R18ZT_C12_RAW.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @([pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Invoke-R18ZTBatchLaunchV3.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256}, [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV9.ps1';sha=$builderSha}, [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Test-R18ZT1RequestIdUniquenessV3.ps1';sha=(Get-Sha256 $collisionScannerPath)})) {
...
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV9.ps1' })
...
if ($Build) {
    Require ([string]$collisionGate.schema -ceq 'argos_r18zt1_request_id_uniqueness_gate_v1' -and [string]$collisionGate.requestId -ceq $requestId -and [string]$collisionGate.scannerSha256 -ceq (Get-Sha256 $collisionScannerPath)) 'R18ZT collision gate identity changed.'
...
$expectedLeafSetSha = 'AB10C93FDA4A920FD87EF1FB4C7F7140B3D3F7EC8FD9D49978FE7C553A67DC9E'
...
$stagedPreflight = (& (Join-Path $activePayload 'Invoke-R18ZTBatchLaunch.ps1') -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $activePayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\R18ZT1C4_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
Require ([string]$stagedPreflight.state -eq 'PASS_R18ZT_STATIC_PACKAGE_PREFLIGHT' -and -not [bool]$stagedPreflight.targetWritesPerformed -and -not [bool]$stagedPreflight.processStarted -and -not [bool]$stagedPreflight.completionClaimed -and -not [bool]$stagedPreflight.sourceImageBytesHashed -and -not [bool]$stagedPreflight.pixelsDecoded) 'R18ZT staged static launcher preflight failed.'
...
$actualLeafSetSha = $expectedLeafSetSha
...
$extractedPreflightText = (& $windowsPowerShell51 -NoProfile -ExecutionPolicy Bypass -File $extractedLauncher -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $extractedPayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\R18ZT1C4_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String)
...
Require ([string]$extractedPreflight.state -eq 'PASS_R18ZT_STATIC_PACKAGE_PREFLIGHT' -and -not [bool]$extractedPreflight.targetWritesPerformed -and -not [bool]$extractedPreflight.processStarted -and -not [bool]$extractedPreflight.completionClaimed -and -not [bool]$extractedPreflight.sourceImageBytesHashed -and -not [bool]$extractedPreflight.pixelsDecoded) 'R18ZT extracted launcher Windows PowerShell 5.1 preflight failed.'
