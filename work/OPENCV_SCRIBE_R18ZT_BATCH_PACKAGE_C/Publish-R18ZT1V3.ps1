...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZT_PUBLICATION_INVOCATION_V3.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v3') 'R18ZT publication invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION_V3') 'R18ZT publication invocation is not frozen; execution refused.'
...
Require (-not (Test-Path -LiteralPath $publishGatePath)) 'R18ZT V3 publication gate already exists; republish refused.'
Require (-not (Test-Path -LiteralPath $publishGatePartialPath)) 'R18ZT V3 publication gate partial already exists; repeat publication refused.'
Require (-not (Test-Path -LiteralPath $applyIntentPath)) 'R18ZT V3 apply intent already exists; repeat publication refused.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publication_apply_intent_v3'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZT1V3_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
...
Require (-not (Test-Path -LiteralPath $publicationCollisionGatePath)) 'R18ZT publication collision gate already exists; repeat publication refused.'
...
Require ([string]$collision.requestId -ceq 'REQ_R18ZT1' -and
...
if ($Preflight) {
    ...
    Require ([string]$collision.schema -ceq [string]$invocation.collision.expectedGateSchema -and
        ...
        -not [bool]$collision.zipPayloadOrImageMembersRead -and @($collision.queueTaskProcessActions).Count -eq 0 -and
        -not [bool]$collision.mutationsPerformed) 'R18ZT fresh collision scan did not prove zero collisions in all accessible namespaces.'
    ...
    Require (Test-Path -LiteralPath $publicationCollisionGatePath -PathType Leaf) 'R18ZT publication collision gate was not created.'
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publish_gate_v3'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V3_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZT1V3_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_R18ZT1'
