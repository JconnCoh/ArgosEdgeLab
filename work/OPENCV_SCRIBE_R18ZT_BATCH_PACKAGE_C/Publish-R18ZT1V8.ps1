...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZT_PUBLICATION_INVOCATION_V8.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v8') 'R18ZT publication invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION_V8') 'R18ZT publication invocation is not frozen; execution refused.'
...
Require ([string]$postFreezeContract.path -ceq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/R18ZT_P8_FINAL_TOOL.json') 'R18ZT post-freeze tooling-gate path changed.'
...
Require ([string]$priorTerminal.state -ceq 'PASS_R18W4S21_SIGNED_TERMINAL_RESPONSE_CHECKPOINT_GATE' -and
    [string]$priorTerminal.terminalResponse.state -ceq 'PASS_DATA_PULL' -and
    [string]$priorTerminal.terminalResponse.requestId -ceq 'REQ_S21_20260905212322_S9RHWN0X00G4K59QB7Q1120VDR' -and
    [bool]$priorTerminal.terminalResponse.signedResponseVerified) 'The prior same-endpoint request lacks its pinned signed terminal response.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publication_apply_intent_v8'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZT1V8_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publish_gate_v8'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V8_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZT1V8_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_R18ZT1'
