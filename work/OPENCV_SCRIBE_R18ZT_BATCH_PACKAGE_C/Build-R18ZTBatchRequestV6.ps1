...
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZT_C9_REMEDIATION.json'
...
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZT_C9_RAW.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZT_C9_CLONE.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZT_PATH_PLAN_GATE_V3.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZT_C9_PS.json'
...
$preactionPath = Join-Path $PSScriptRoot 'R18ZT_C9_PRE.json'
$collisionGatePath = Join-Path $PSScriptRoot 'R18ZT_REQUEST_ID_COLLISION_GATE_V2.json'
$collisionScannerPath = Join-Path $PSScriptRoot 'Test-R18ZT1RequestIdUniqueness.ps1'
...
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV6.ps1' = $builderSha
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Test-R18ZT1RequestIdUniqueness.ps1' = Get-Sha256 $collisionScannerPath
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/R18ZT_C9_RAW.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @([pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Invoke-R18ZTBatchLaunchV3.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256}, [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV6.ps1';sha=$builderSha}, [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Test-R18ZT1RequestIdUniqueness.ps1';sha=(Get-Sha256 $collisionScannerPath)})) {
...
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV6.ps1' })
