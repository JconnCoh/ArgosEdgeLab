...
if ($Preflight) {
    [ordered]@{
        schema='argos_r18zt1_request_id_uniqueness_preflight_v3'
...
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZT_P8_COLLISION.json'))
Assert-True ($resolvedOutput.Equals($expectedOutput, [StringComparison]::OrdinalIgnoreCase)) 'Uniqueness gate output path changed.'
...
$result = [ordered]@{
    schema='argos_r18zt1_request_id_uniqueness_gate_v3'
    checkedUtc=[DateTime]::UtcNow.ToString('o')
    state=$(if($scanPassed){'PASS_R18ZT1_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS'}else{'FAIL_R18ZT1_REQUEST_ID_COLLISION_OR_SCAN_ERROR'})
    requestId=$requestId
    bindingRecordSha256=$bindingRecordSha256
    scannerSha256=$scannerSha256
