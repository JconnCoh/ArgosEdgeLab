...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZT_PUBLICATION_INVOCATION_V7.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v7') 'R18ZT publication invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION_V7') 'R18ZT publication invocation is not frozen; execution refused.'
...
# pre-invocation P7 artifact without a circular expected hash.
$postFreezeContract = $invocation.postFreezeToolingGate
Require ([string]$postFreezeContract.path -ceq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/R18ZT_P7_FINAL_TOOL.json') 'R18ZT post-freeze tooling-gate path changed.'
...
Require (-not (Test-Path -LiteralPath $publishGatePath)) 'R18ZT V7 publication gate already exists; republish refused.'
Require (-not (Test-Path -LiteralPath $publishGatePartialPath)) 'R18ZT V7 publication gate partial already exists; repeat publication refused.'
Require (-not (Test-Path -LiteralPath $applyIntentPath)) 'R18ZT V7 apply intent already exists; repeat publication refused.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publication_apply_intent_v7'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZT1V7_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publish_gate_v7'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V7_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZT1V7_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_R18ZT1'
