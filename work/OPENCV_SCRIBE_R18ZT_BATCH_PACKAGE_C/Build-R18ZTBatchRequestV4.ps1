...
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZT_C7_REMEDIATION.json'
...
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZT_C7_RAW.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZT_C7_CLONE.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZT_PATH_PLAN_GATE_V3.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZT_POWERSHELL_SAFETY_GATE_V3.json'
...
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_R18ZT_SIGNED_UNPUBLISHED_PACKAGE_V3.json'
...
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV4.ps1' = $builderSha
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/R18ZT_C7_RAW.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @([pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Invoke-R18ZTBatchLaunchV3.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256}, [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV4.ps1';sha=$builderSha})) {
...
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_C/Build-R18ZTBatchRequestV4.ps1' })
