...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v8' -and
    [string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION_V8' -and [string]$invocation.lifecycle -ceq 'FROZEN') 'R18ZT publication invocation is not frozen; collection refused.'
...
Require ([string]$applyIntent.schema -ceq 'argos_opencv_scribe_r18zt1_publication_apply_intent_v8' -and
    [string]$applyIntent.state -ceq 'FROZEN_R18ZT1V8_APPLY_INTENT_BEFORE_EXTERNAL_WRITE' -and
...
if ($publishGateExists) {
    ...
    Require ([string]$publishGate.schema -ceq 'argos_opencv_scribe_r18zt1_publish_gate_v8' -and
        [string]$publishGate.state -ceq 'PASS_R18ZT1V8_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' -and
...
if (-not $publishGateExists) {
    ...
    Require ($readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZT1.ready.zip' -and
        $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_R18ZT1.ready.zip' -and
        $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZT1.ready.zip.upload') 'R18ZT missing-gate recovery route leaves changed.'
    $routeRows = @(
...
$preflightResult = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_launch_response_collection_gate_v8'
    collectedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V8_ONE_SIGNED_LAUNCH_RESPONSE_PREFLIGHT' } else { 'PASS_R18ZT1V8_SIGNED_LAUNCH_RESPONSE_COLLECTED' })
