...
if([string]$invocation.schema-ne'argos_o3c1_route_invocation_v1'){throw 'O3C1 route invocation schema mismatch.'}
...
if($paths.Count-lt30-or$paths.Count-gt128-or@($paths|Sort-Object -Unique).Count-ne$paths.Count){throw 'O3C1 route path set is incomplete or duplicated.'}
$rows=New-Object Collections.Generic.List[object]
foreach($path in $paths){
    $one=& $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson|ConvertFrom-Json
    if([string]$one.state-ne'PASS_PATH_BUDGET'-or@($one.candidates).Count-ne1){throw "O3C1 route path budget failed: $path"}
...
$result=[ordered]@{
    schema='argos_o3c1_complete_route_gate_v1'
    createdUtc=[DateTime]::UtcNow.ToString('o')
    state=if($Preflight){'PASS_O3C1_COMPLETE_ROUTE_PREFLIGHT'}else{'PASS_O3C1_COMPLETE_ROUTE_GATE'}
...
foreach($property in @('requestZipPath','requestManifestPath','requestSignaturePath')){$path=[IO.Path]::GetFullPath([string]$invocation.$property);if(-not(Test-Path -LiteralPath $path -PathType Leaf)){throw "O3C1 exact route artifact is missing: $path"}}
...
if(-not$result.exactFinalZipExtractionPassed-or-not$result.exactFinalZipSignaturePassed){throw 'O3C1 exact final ZIP verification is not PASS.'}
$outputPath=[IO.Path]::GetFullPath([string]$invocation.outputPath)
if(Test-Path -LiteralPath $outputPath){throw 'O3C1 route gate output already exists.'}
