#!/usr/bin/env python3
"""Run the locked 15-case full OpenCV R6 grid-localization parity gate."""
...
def load_engine(path: Path):
    specification = importlib.util.spec_from_file_location("argos_opencv_scribe_r6_full_gate", path)
...
def main() -> int:
    ...
    if engine.sha256_file(engine_path) != ENGINE_SHA256:
        raise RuntimeError("R6 engine hash changed.")
    ...
    gate = {
        "schema": "argos_opencv_scribe_r6_full_localization_gate_v1",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
        "state": "PASS_OCV02_R6_FULL_LOCALIZATION_AND_RECOGNITION_15_OF_15" if passed else "FAIL_OCV02_R6_FULL_LOCALIZATION_AND_RECOGNITION",
        "engine": {
