...
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 16) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "O2D20 test create-new JSON exists: $Path"
...
$endpoint = Join-Path $PSScriptRoot 'Invoke-O2D20ScribeEndpoint.ps1'
$engine = Join-Path $project 'work\OPENCV_SCRIBE_V1R5\ArgosOpenCvScribeV1R5.py'
$liveJob = Join-Path $PSScriptRoot 'O2D20_SLOT22_JOB.json'
...
$sampleDf = $sampleBf
$selfPinGate = Join-Path $PSScriptRoot 'O2D20_SELF_PIN_AND_LIVE_BRANCH_GATE.json'
$root = 'C:\O2D20T_5B7ADC1B'
$payload = Join-Path $root 'p'
$input = Join-Path $root 'i'
$normal = Join-Path $root 'n'
$failure = Join-Path $root 'f'
$gatePath = Join-Path $PSScriptRoot 'O2D20_ENTRYPOINT_TEST_GATE_R2.json'
...
foreach ($path in @($endpoint,$engine,$liveJob,$bundle,(Join-Path $runtime 'python.exe'),$sampleBf,$sampleDf,$selfPinGate)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "O2D20 test dependency absent: $path"
}
Assert-True ((Get-Sha256 $endpoint) -eq '4A0A3DDD28EB22ED788BE4EA373BF490A27B6BA88FC5421D08F6F04794E9079D') 'O2D20 endpoint changed.'
Assert-True ((Get-Sha256 $engine) -eq 'F61F5954A77E6F730A2BF0D110A468535C4595D25DB21AFFE1573EF08B8139AB') 'O2D20 test engine changed.'
Assert-True ((Get-Sha256 $bundle) -eq '56DF00E37A195E7BC3E026E4950DFB5A0AA7E7AF49A6FF39020B071840CCFBD6') 'O2D20 test bundle changed.'
Assert-True ((Get-Sha256 $sampleBf) -eq $bfSha -and (Get-Sha256 $sampleDf) -eq $dfSha) 'O2D20 test sample changed.'
Assert-True ((Get-Sha256 $selfPinGate) -eq $selfPinGateSha) 'O2D20 self-pin gate changed.'
$selfPin = Get-Content -Raw -LiteralPath $selfPinGate | ConvertFrom-Json
Assert-True ([string]$selfPin.state -eq 'PASS_O2D20_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [int]$selfPin.pinMatchCount -eq 6 -and @($selfPin.liveBranchCases | Where-Object { [string]$_.state -eq 'PASS' }).Count -eq 3) 'O2D20 self-pin/live-branch contract changed.'
foreach ($path in @($root,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "O2D20 test target exists: $path" }
$tokens = $null
$parserErrors = $null
[void][Management.Automation.Language.Parser]::ParseFile($endpoint, [ref]$tokens, [ref]$parserErrors)
Assert-True (@($parserErrors).Count -eq 0) 'O2D20 endpoint Windows PowerShell parser failed.'

if ($Preflight) {
    [ordered]@{schema='argos_o2d20_entrypoint_test_preflight_v2';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D20_ENTRYPOINT_TEST_PREFLIGHT_R2';endpointSha256=Get-Sha256 $endpoint;engineSha256=Get-Sha256 $engine;bundleSha256=Get-Sha256 $bundle;selfPinGateSha256=$selfPinGateSha;mutationsPerformed=$false;processStarted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 6
    return
}
...
try {
    ...
    $job.revision = 'OCV02_O2D20_REHEARSAL_73C073D0'
    $job.jobId = 'O2D20_REHEARSAL_73C073D0'
    ...
    $jobPath = Join-Path $payload 'O2D20_SLOT22_JOB.json'
    ...
    Assert-True ([string]$endpointPreflight.state -eq 'PASS_O2D20_ENDPOINT_PREFLIGHT' -and -not [bool]$endpointPreflight.processStarted -and -not [bool]$endpointPreflight.mutationsPerformed) 'O2D20 endpoint preflight changed.'
    ...
    Assert-True ([string]$normalResult.state -eq 'PASS_O2D20_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED') 'O2D20 normal rehearsal state changed.'
    Assert-True (-not [string]::IsNullOrWhiteSpace([string]$normalResult.imageFirstString) -and [string]$normalResult.checksumState -ne 'NOT_EVALUATED' -and [int]$normalResult.localization.autoLocalizedPromotedCandidateCount -gt 0) 'O2D20 normal rehearsal did not evaluate an auto-localized scribe.'
    Assert-True (@($normalResult.holds | Where-Object { [string]$_.code -eq 'SCRIBE_REFERENCE_COVERAGE_HOLD' }).Count -eq 1) 'O2D20 normal rehearsal coverage hold changed.'
    Assert-True ([bool]$normalResult.upstreamNotchHoldDidNotSkipScribe -and [string]$normalResult.upstreamProposalState -eq 'SCRIBE_IDENTITY_CONFIRMATION_HOLD') 'O2D20 upstream-hold boundary changed.'
    Assert-True ([bool]$normalResult.sourceAliasRemoved -and -not (Test-Path -LiteralPath 'X:\')) 'O2D20 source alias was not removed.'
    Assert-True (-not [bool]$normalResult.taskOrProcessRestarted -and -not [bool]$normalResult.providerActivated -and -not [bool]$normalResult.productionEligible) 'O2D20 normal rehearsal authority changed.'
    ...
    Assert-True ($caught -and -not (Test-Path -LiteralPath (Join-Path $failure 'w')) -and -not (Test-Path -LiteralPath (Join-Path $failure 'o'))) 'O2D20 injected source-hash failure did not fail before write.'

    $gate = [ordered]@{
        schema='argos_o2d20_entrypoint_test_gate_v2';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D20_ENTRYPOINT_TEST_GATE_R2';revision='O2D20_20260827T020900000Z_5B7ADC1B'
    ...
    if (Test-Path -LiteralPath $root) {
        ...
        Assert-True ($resolved -eq 'C:\O2D20T_5B7ADC1B') 'O2D20 test cleanup root changed.'
