...
foreach ($dependency in @($sourceZip, $certificate, $verifier)) {
    Assert-True (Test-Path -LiteralPath $dependency -PathType Leaf) "O2D20 response dependency absent: $dependency"
}
Assert-True ((Get-Item -LiteralPath $sourceZip).Length -eq $expectedBytes) 'O2D20 response ZIP byte count changed.'
Assert-True ((Get-Sha256 $sourceZip) -eq $expectedSha256) 'O2D20 response ZIP hash changed.'
...
try {
    ...
    Assert-True ($names.Count -eq $expectedNames.Count) 'O2D20 response ZIP entry count changed.'
    $nameDifference = @(Compare-Object -ReferenceObject $expectedNames -DifferenceObject $names)
    Assert-True ($nameDifference.Count -eq 0) 'O2D20 response ZIP entry names changed.'
    $manifestEntry = $zip.GetEntry('PORTAL_RESPONSE_MANIFEST.json')
    Assert-True ($null -ne $manifestEntry -and $manifestEntry.Length -le 65536) 'O2D20 response manifest entry is absent or unbounded.'
...
Assert-True ([string]$manifest.requestId -eq $requestId) 'O2D20 response request identity changed.'
Assert-True ([string]$manifest.responseId -eq $responseId) 'O2D20 response identity changed.'
Assert-True ([string]$manifest.sourceRole -eq 'JBOD' -and [string]$manifest.state -eq 'PASS_MAINTENANCE_PATCH') 'O2D20 response terminal state changed.'

$preflightResult = [ordered]@{
    schema = 'argos_o2d20_response_collection_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O2D20_EXACT_RESPONSE_COLLECTION_PREFLIGHT'
    requestId = $requestId
...
foreach ($path in @($tempRoot, $archivePath, $extractionRoot)) {
    Assert-True (-not (Test-Path -LiteralPath $path)) "O2D20 create-new collection target exists: $path"
}
...
try {
    ...
    Assert-True ((Get-Sha256 $tempZip) -eq $expectedSha256) 'O2D20 temporary response copy changed.'
    ...
    Assert-True ([string]$verification.State -eq 'PASS_SIGNED_PORTAL_RESPONSE') 'O2D20 signed response verification failed.'
    Assert-True ([string]$verification.RequestId -eq $requestId -and [string]$verification.EndpointState -eq 'PASS_MAINTENANCE_PATCH') 'O2D20 signed response correlation or terminal state changed.'
    Assert-True ([string]$verification.SignerThumbprint -eq 'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC') 'O2D20 JBOD signer changed.'

    [void][IO.Directory]::CreateDirectory($archiveDir)
    [IO.File]::Copy($tempZip, $archivePath, $false)
    $archiveCreated = $true
    Assert-True ((Get-Sha256 $archivePath) -eq $expectedSha256) 'O2D20 archived response changed.'
    ...
    Assert-True ([string]$finalVerification.State -eq 'PASS_SIGNED_PORTAL_RESPONSE' -and [string]$finalVerification.EndpointState -eq 'PASS_MAINTENANCE_PATCH') 'O2D20 final collected response verification failed.'

    [ordered]@{
        schema = 'argos_o2d20_exact_response_collection_gate_v1'
        collectedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_O2D20_EXACT_SIGNED_TERMINAL_RESPONSE_COLLECTED'
        requestId = $requestId
        responseId = $responseId
    ...
    if (Test-Path -LiteralPath $tempRoot -PathType Container) {
        ...
        Assert-True ($resolvedTemp -eq 'C:\O2D20R_5B7ADC1B') 'O2D20 temporary cleanup root changed.'
