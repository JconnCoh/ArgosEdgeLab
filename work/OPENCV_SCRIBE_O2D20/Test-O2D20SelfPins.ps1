...
$endpoint = Join-Path $root 'Invoke-O2D20ScribeEndpoint.ps1'
$engine = Join-Path $project 'work\OPENCV_SCRIBE_V1R5\ArgosOpenCvScribeV1R5.py'
$job = Join-Path $root 'O2D20_SLOT22_JOB.json'
...
$gatePath = Join-Path $root 'O2D20_SELF_PIN_AND_LIVE_BRANCH_GATE.json'
...
function Get-LiteralPin([string]$Text, [string]$Name) {
    ...
    Assert-True ($matches.Count -eq 1) "O2D20 endpoint requires exactly one literal pin: $Name"
...
foreach ($path in @($endpoint,$engine,$job,$ols6Path,$o2d5InvocationPath,$continuityPath,$goodFixture,$badJobFixture,$badInstallationFixture)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "O2D20 self-pin prerequisite absent: $path"
}
$endpointText = Get-Content -Raw -LiteralPath $endpoint
Assert-True ((Get-Sha256 (Join-Path $project 'work\OPENCV_SCRIBE_O2D13\Invoke-O2D13ScribeEndpoint.ps1')) -eq $expectedEndpointSourceSha256) 'O2D20 approved O2D13 endpoint source changed.'
Assert-True ([regex]::Matches($endpointText, [regex]::Escape('Assert-LiveSelfPins -JobPath $jobSource -ExpectedJobSha256 $jobSha -InstallationPath $installation -ExpectedInstallationSha256 $installationSha')).Count -eq 1) 'O2D20 normal live self-pin call changed.'
Assert-True ([regex]::Matches($endpointText, [regex]::Escape('Assert-LiveSelfPins -JobPath ([string]$fixture.jobPath)')).Count -eq 1) 'O2D20 fixture self-pin call changed.'

$ols6 = Get-Content -Raw -LiteralPath $ols6Path | ConvertFrom-Json
$slot22 = @($ols6.hashes | Where-Object { [string]$_.slot -eq 'Slot22' })
Assert-True ($slot22.Count -eq 2) 'O2D20 OLS6 Slot22 source row cardinality changed.'
$bfRows = @($slot22 | Where-Object { [string]$_.channel -eq 'Brightfield' })
$dfRows = @($slot22 | Where-Object { [string]$_.channel -eq 'Darkfield' })
Assert-True ($bfRows.Count -eq 1 -and $dfRows.Count -eq 1) 'O2D20 OLS6 Slot22 channel cardinality changed.'
...
Assert-True ($installationMatches.Count -eq 1) 'O2D20 continuity installation pin cardinality changed.'
...
foreach ($name in @('engineSha','jobSha','refsSha','installationSha','bfSha','dfSha')) {
    ...
    Assert-True ($observed -eq $expectedValue) "O2D20 endpoint self-pin mismatch: $name"
    $rows.Add([pscustomobject]@{id=$name;endpointValue=$observed;evidenceValue=$expectedValue;match=$true})
}

if ($Preflight) {
    [ordered]@{
        schema='argos_o2d20_self_pin_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D20_SELF_PIN_PREFLIGHT'
...
Assert-True (-not (Test-Path -LiteralPath $gatePath)) "O2D20 create-new self-pin gate exists: $gatePath"
$good = & $endpoint -Preflight -LiveContractFixtureManifest $goodFixture | ConvertFrom-Json
Assert-True ([string]$good.state -eq 'PASS_O2D20_LIVE_CONTRACT_FIXTURE' -and [bool]$good.jobAssertionExecuted -and [bool]$good.installationAssertionExecuted -and [bool]$good.mutationsPerformed -eq $false) 'O2D20 positive live-contract fixture failed.'
$badJobRefused = $false
try { & $endpoint -Preflight -LiveContractFixtureManifest $badJobFixture | Out-Null }
catch { $badJobRefused = $_.Exception.Message -eq 'O2D20 live job changed.' }
Assert-True $badJobRefused 'O2D20 bad-job live-contract fixture was not refused by the exact job assertion.'
$badInstallationRefused = $false
try { & $endpoint -Preflight -LiveContractFixtureManifest $badInstallationFixture | Out-Null }
catch { $badInstallationRefused = $_.Exception.Message -eq 'O2D20 runtime installation changed.' }
Assert-True $badInstallationRefused 'O2D20 bad-installation live-contract fixture was not refused by the exact installation assertion.'

$gate = [ordered]@{
    schema='argos_o2d20_self_pin_and_live_branch_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D20_SELF_PIN_AND_LIVE_BRANCH_GATE'
