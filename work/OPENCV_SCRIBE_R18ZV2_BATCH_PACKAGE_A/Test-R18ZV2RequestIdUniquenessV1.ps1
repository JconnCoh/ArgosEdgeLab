...
$requestId = 'REQ_RZV2'
$bindingRecordSha256 = 'DF4376B6A6B06A8384D6B8A66E2A904CCFF727BAF827DB92C57C1C0774F3DBE3'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_r18zv2_request_id_uniqueness_preflight_v1'
        checkedUtc=[DateTime]::UtcNow.ToString('o')
        state='PASS_R18ZV2_REQUEST_ID_UNIQUENESS_PREFLIGHT'
        requestId=$requestId
...
$preSignatureOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV2_REQUEST_ID_COLLISION_GATE_V2.json'))
$prePublicationOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV2_P8_COLLISION.json'))
Assert-True (
...
$result = [ordered]@{
    schema='argos_r18zv2_request_id_uniqueness_gate_v1'
    checkedUtc=[DateTime]::UtcNow.ToString('o')
    state=$(if($scanPassed){'PASS_R18ZV2_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS'}else{'FAIL_R18ZV2_REQUEST_ID_COLLISION_OR_SCAN_ERROR'})
    requestId=$requestId
    bindingRecordSha256=$bindingRecordSha256
    scannerSha256=$scannerSha256
