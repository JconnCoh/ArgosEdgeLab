...
$s='argos_r18zv2_endpoint_consumer_observation_v2'
try{
...
$rows+=@{taskPath=$t.TaskPath;taskName=$t.TaskName;state=[string]$t.State;lastRunTime=[string]$i.LastRunTime;lastTaskResult=$i.LastTaskResult;nextRunTime=[string]$i.NextRunTime;missedRuns=$i.NumberOfMissedRuns;action=$a}
}
