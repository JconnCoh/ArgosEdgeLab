...
function Assert-ExactLauncherChange([object[]]$Changes, [string]$LauncherSha256, [string]$Label) {
    ...
    Require ([string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZV2.ps1') "$Label launcher destination changed."
...
$requestId = 'REQ_RZV2'
$revision = 'R18ZV2_EXISTING_ORIENTED_CROPS_BATCH_CACHE_SEAM_CORRECTION_20260911A'
$runnerRevision = 'R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260908D'
$bindingPath = Join-Path $PSScriptRoot 'R18ZV2_FROZEN_PACKAGE_BINDINGS_V1.json'
$launcherPath = Join-Path $PSScriptRoot 'Invoke-R18ZV2BatchLaunchV1.ps1'
$payloadManifestPath = Join-Path $PSScriptRoot 'R18ZV2_PAYLOAD_MANIFEST.json'
$definitionPath = Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION_R18ZV2.json'
$configurationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A\R18ZV2_BATCH_CONFIGURATION.json'
$designPath = Join-Path $PSScriptRoot 'R18ZV2_BATCH_PACKAGE_DESIGN_V1.json'
$importSmokePath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B\Test-R18ZTPackagedImportClosure.py'
$payloadCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZV2_A1_CLONE_REMEDIATION.json'
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZV2_A1_CLONE_REMEDIATION.json'
$publisherCloneRemediationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A\R18ZV2_A1_CLONE_REMEDIATION.json'
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZV2_A1_CLONE_RAW_GATE_LOCKED_V2.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZV2_A1_CLONE_GATE_LOCKED_V2.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZV2_PATH_PLAN_GATE.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZV2_POWERSHELL_SAFETY_GATE_V3.json'
$scienceGatePath = Join-Path $PSScriptRoot 'R18ZV2_SCIENCE_GATE.json'
$junctionGatePath = Join-Path $PSScriptRoot 'R18ZV2_PROPOSAL_JUNCTION_GATE.json'
$recoveryIntentPath = Join-Path $PSScriptRoot 'R18ZV2_RECOVERY_INTENT_V3.json'
$recoveryGatePath = Join-Path $PSScriptRoot 'R18ZV2_RECOVERY_INTENT_GATE_V3.json'
$preactionPath = Join-Path $PSScriptRoot 'R18ZV2_PREACTION_GATE_V2.json'
$collisionGatePath = Join-Path $PSScriptRoot 'R18ZV2_REQUEST_ID_COLLISION_GATE_V2.json'
$collisionScannerPath = Join-Path $PSScriptRoot 'Test-R18ZV2RequestIdUniquenessV1.ps1'
...
$testRoot = 'C:\RZV2A1T'
$testWorkRoot = 'C:\RZV2A1TW'
$testOutputRoot = 'C:\RZV2A1TO'
$stageRoot = 'C:\RZV2A1P'
$readyRoot = Join-Path $stageRoot ($requestId + '.ready')
$stageZip = Join-Path $stageRoot ($requestId + '.ready.zip')
$verifyRoot = 'C:\RZV2A1V'
$finalRoot = Join-Path $PSScriptRoot 'final'
$finalPartial = Join-Path $PSScriptRoot 'final.partial'
$zipName = $requestId + '.ready.zip'
$finalGatePath = Join-Path $PSScriptRoot 'R18ZV2_FINAL_PACKAGE_GATE.json'
$productionWorkRoot = 'D:\A2\w\ocv\R18ZV2'
$productionOutputRoot = 'D:\A2\o\ocv\R18ZV2'
$productionProposalAlias = 'D:\A2\w\ocv\R18ZV2\p'
$canonicalProposalRoot = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals'
$productionInstalledLauncher = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZV2.ps1'
$bindingSha = 'DF4376B6A6B06A8384D6B8A66E2A904CCFF727BAF827DB92C57C1C0774F3DBE3'

Require-Pin $bindingPath $bindingSha -1 'FROZEN_R18ZV2_PACKAGE_A_V1_BINDINGS'
$binding = Get-Content -LiteralPath $bindingPath -Raw | ConvertFrom-Json
Require ([string]$binding.schema -eq 'argos_opencv_scribe_r18zv2_package_bindings_v1' -and [string]$binding.requestId -eq $requestId -and [string]$binding.packageRevision -eq $revision -and [string]$binding.runnerRevision -eq $runnerRevision) 'R18ZV2 binding changed.'
...
Require ([string]$boundLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$boundLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZV2\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$boundLongestOutput.basePathLength -eq 62 -and [int]$boundLongestOutput.suffixReserveCharacters -eq 52 -and [int]$boundLongestOutput.effectivePathLength -eq 114 -and [int]$boundLongestOutput.effectiveComponentLength -eq 68) 'R18ZT exact longest atomic case-output binding changed.'
$boundConservativeTemp = $binding.pathContract.launcherConservativeProviderTempBudget
Require ([string]$boundConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$boundConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZV2\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$boundConservativeTemp.basePathLength -eq 74 -and [int]$boundConservativeTemp.baseComponentLength -eq 28 -and [int]$boundConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$boundConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$boundConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$boundConservativeTemp.trueAtomicWriteTarget) 'R18ZT conservative provider-temp launcher budget changed.'
...
$requiredArtifactNames = @(
    ...
    'heldOutZDiagnosticGate','r18zuReviewBatchScienceGate','approvedSourceGuard','priorRequestTerminalRouteGate','r18zuProvider','r18zv1DevelopmentFreeze','r18zv1Slot21Job','r18zv1Slot21Result','r18zv1Slot21CompactGate','r18zv1Slot21CorrectionGate','collectorFailure','collectorRemediationBinding','packageAPreflightWithdrawal','batchAdapter','adapterQualificationGate'
)
...
$cloneGate = Read-BoundGate $cloneGatePath 'PASS_R18ZV2_PACKAGE_A_CLONE_LITERAL_GATE'
$pathGate = Read-BoundGate $pathGatePath 'PASS_R18ZV2_PACKAGE_A_PATH_BUDGET'
$powerShellGate = Read-BoundGate $powerShellGatePath 'PASS_R18ZV2_PACKAGE_A_POWERSHELL_SAFETY'
$scienceGate = Read-BoundGate $scienceGatePath 'PASS_R18ZV2_BATCH_SCIENCE_AND_ADAPTER_EQUIVALENCE_GATE'
$junctionGate = Read-BoundGate $junctionGatePath 'PASS_R18ZV2_PROPOSAL_JUNCTION_CREATION_AND_ROLLBACK'
...
$collisionGate = $null
if ($Build) { $collisionGate = Read-BoundGate $collisionGatePath 'PASS_R18ZV2_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS' }

$builderPath = [IO.Path]::GetFullPath($MyInvocation.MyCommand.Path)
$builderSha = Get-Sha256 $builderPath
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Build-R18ZV2BatchRequestV1.ps1' = $builderSha
    'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Collect-R18ZV2LaunchResponseV1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZV2LaunchResponseV1.ps1')
    'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Invoke-R18ZV2BatchLaunchV1.ps1' = Get-Sha256 $launcherPath
    'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Publish-R18ZV2V2.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZV2V2.ps1')
    'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Test-R18ZV2RequestIdUniquenessV1.ps1' = Get-Sha256 $collisionScannerPath
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/R18ZV2_A1_CLONE_RAW_GATE_LOCKED_V2.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @(
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Invoke-R18ZV2BatchLaunchV1.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Build-R18ZV2BatchRequestV1.ps1';sha=$builderSha},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Publish-R18ZV2V2.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZV2V2.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Collect-R18ZV2LaunchResponseV1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZV2LaunchResponseV1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Test-R18ZV2RequestIdUniquenessV1.ps1';sha=(Get-Sha256 $collisionScannerPath)}
)) {
...
foreach ($sciencePin in @('runnerNonImageTestGate','canonicalZDevelopmentGate','r18zuNonImageRegressionGate','heldOutZDiagnosticGate','r18zuReviewBatchScienceGate','priorRequestTerminalRouteGate','batchAdapter','adapterQualificationGate')) {
    $bindingRow = @($artifactRows | Where-Object { [string]$_.name -eq $sciencePin })[0]
    $scienceRows = @($scienceGate.artifacts | Where-Object { [string]$_.name -eq $sciencePin })
...
$priorRoute = Get-Content -LiteralPath $resolvedArtifacts['priorRequestTerminalRouteGate'] -Raw | ConvertFrom-Json
Require ([string]$priorRoute.state -ceq 'PASS_R18ZV1_PREDECESSOR_TERMINAL_FAILED_BEFORE_PROVIDER_ENTRY_NO_RETRY' -and [string]$priorRoute.priorRequestId -ceq 'REQ_RZV1' -and [string]$priorRoute.successorRequestId -ceq 'REQ_RZV2' -and
    [bool]$priorRoute.conclusion.priorEndpointRequestTerminal -and [bool]$priorRoute.conclusion.signedTerminalResponseReturnedToShare -and
    [bool]$priorRoute.conclusion.returnRouteHealthy -and [bool]$priorRoute.signedEndpointTerminalResponse.signedResponseVerified -and
    [string]$priorRoute.signedEndpointTerminalResponse.endpointState -ceq 'PASS_MAINTENANCE_PATCH' -and [bool]$priorRoute.conclusion.backgroundBatchReachedTerminalIndexAndAggregate -and
    -not [bool]$priorRoute.conclusion.backgroundBatchScientificallySucceeded -and -not [bool]$priorRoute.conclusion.ocrAccuracyEvaluated -and
    [bool]$priorRoute.conclusion.failureOccurredBeforePublicProviderEntry -and [int]$priorRoute.boundedBatchTerminalObservation.caseIndex.heldBeforeProviderEntry -eq 1001 -and
    -not [bool]$priorRoute.conclusion.priorRetryAuthorized -and -not [bool]$priorRoute.externalMutationPerformed -and
    -not [bool]$priorRoute.imageBytesRead) 'R18ZV1 predecessor terminal/failure gate changed.'
$adapterGate = Get-Content -LiteralPath $resolvedArtifacts['adapterQualificationGate'] -Raw | ConvertFrom-Json
Require ([string]$adapterGate.state -ceq 'PASS_R18ZV2_BATCH_ADAPTER_EXACT_SEAM_ALGORITHM_UNCHANGED' -and [string]$adapterGate.requestId -ceq $requestId -and
    ...
    -not [bool]$adapterGate.tests.imageBytesRead -and -not [bool]$adapterGate.tests.providerRunPerformed) 'R18ZV2 adapter qualification changed.'
...
Require ([string]$pathLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$pathLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZV2\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$pathLongestOutput.basePathLength -eq 62 -and [int]$pathLongestOutput.suffixReserveCharacters -eq 52 -and [int]$pathLongestOutput.effectivePathLength -eq 114 -and [int]$pathLongestOutput.effectiveComponentLength -eq 68) 'R18ZT path-gate true atomic longest-output fact changed.'
$pathConservativeTemp = $pathGate.launcherConservativeProviderTempBudget
Require ([string]$pathConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$pathConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZV2\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$pathConservativeTemp.basePathLength -eq 74 -and [int]$pathConservativeTemp.baseComponentLength -eq 28 -and [int]$pathConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$pathConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$pathConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$pathConservativeTemp.trueAtomicWriteTarget) 'R18ZT path-gate conservative provider-temp fact changed.'
...
Require ([string]$recoveryIntent.state -eq 'FROZEN_R18ZV2_PACKAGE_A_RECOVERY_INTENT' -and [string]$recoveryIntent.requestId -eq $requestId -and [string]$recoveryIntent.bindingRecordSha256 -eq $bindingSha -and -not [bool]$recoveryIntent.mutation.publicationAuthorized -and -not [bool]$recoveryIntent.mutation.automaticRetryAuthorized) 'R18ZV1 recovery intent changed.'
...
$bindingDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/R18ZV2_FROZEN_PACKAGE_BINDINGS_V1.json' })
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/Build-R18ZV2BatchRequestV1.ps1' })
...
if ($Build) {
    Require ([string]$collisionGate.schema -ceq 'argos_r18zv2_request_id_uniqueness_gate_v1' -and [string]$collisionGate.requestId -ceq $requestId -and [string]$collisionGate.scannerSha256 -ceq (Get-Sha256 $collisionScannerPath) -and [string]$collisionGate.scanPurpose -ceq 'PRE_SIGNATURE') 'R18ZV2 collision gate changed.'
...
foreach ($outputPin in @(
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV2\LAUNCH.json';schema='argos_opencv_scribe_r18zt_batch_launch_v2'},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV2\RUNNING.json';schema=[string]$binding.outputSchemas.progress},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV2\COMPLETE.json';schema=[string]$binding.outputSchemas.complete},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV2\STATUS.json';schema=[string]$binding.outputSchemas.status}
)) {
...
$runningOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZV2\RUNNING.json' })[0]
...
$failureOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZV2\FAILURE.json' })[0]
...
$expectedLeafSetSha = '7B8600D2E033CC9BCA195FE2423C3FB1E8B41B5EB66B39BB91ADCC2F3657BE5F'
...
if ($Preflight) {
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zv2_batch_build_preflight_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZV2_PACKAGE_A_BUILD_PREFLIGHT'
        requestId = $requestId
...
$stagedPackageHashClosure = [ordered]@{
    state = 'PASS_R18ZV2_STAGED_PACKAGED_HASH_CLOSURE_NO_IMAGE'
...
if ($Test) {
    ...
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zv2_batch_unsigned_test_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZV2_PACKAGE_A_UNSIGNED_TEST'
        requestId = $requestId
        bindingRecordSha256 = $bindingSha
...
$extractedRunner = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZT_BATCH\Run-R18ZTExistingOrientedCrops.py'
...
Require ((Get-Sha256 $extractedRunner) -eq $runnerSha -and (Get-Sha256 $extractedProvider) -eq $providerSha -and (Get-Sha256 $extractedWrappedProvider) -eq $wrappedProviderSha -and
...
$routeGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv2_complete_route_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZV2_COMPLETE_ROUTE_GATE_SIGNED_UNPUBLISHED'
    requestId = $requestId
    bindingRecordSha256 = $bindingSha
...
$finalGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv2_final_package_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZV2_SIGNED_UNPUBLISHED_PACKAGE_GATE'
    requestId = $requestId
    revision = $revision
    bindingRecordSha256 = $bindingSha
    requestZip = 'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/final/REQ_RZV2.ready.zip'
