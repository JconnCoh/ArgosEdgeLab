...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV2_PUBLICATION_INVOCATION.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zv2_publication_invocation_v1') 'R18ZV2 invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZV2_PUBLICATION_INVOCATION_V1') 'R18ZV2 invocation is not frozen.'
Require ([string]$invocation.requestId -ceq 'REQ_RZV2') 'R18ZT publication request identity changed.'
...
Require ([string]$postFreezeContract.path -ceq 'work/OPENCV_SCRIBE_R18ZV2_BATCH_PACKAGE_A/R18ZV2_P8_FINAL_TOOL.json') 'R18ZT post-freeze tooling-gate path changed.'
...
Require ([string]$postFreezeTooling.schema -ceq [string]$postFreezeContract.expectedSchema -and
    ...
    [string]$postFreezeTooling.requestId -ceq 'REQ_RZV2' -and
...
Require ([string]$invocation.packageBinding.expectedSchema -ceq 'argos_opencv_scribe_r18zv2_package_bindings_v1') 'Fresh R18ZV2 package binding schema declaration changed.'
Require ([string]$binding.schema -ceq [string]$invocation.packageBinding.expectedSchema) 'Fresh package binding schema changed.'
Require ([string]$binding.state -ceq [string]$invocation.packageBinding.expectedState -and [string]$binding.requestId -ceq 'REQ_RZV2') 'Fresh package binding identity/state changed.'
...
foreach ($consumer in @($invocation.bindingConsumers)) {
    ...
    Require ([string](Get-NestedValue $consumerObject ([string]$consumer.requestIdProperty) "Binding consumer $($consumer.dependency)") -ceq 'REQ_RZV2') "Binding consumer request ID changed: $($consumer.dependency)"
}
...
Require ($bindingAdapterRows.Count -eq 1 -and $scienceAdapterRows.Count -eq 1 -and
    ...
    [string]$scienceAdapterGateRows[0].sha256 -ceq [string]$adapterQualificationDependency.sha256) 'R18ZV2 adapter dependency pins changed.'
Require ([string]$adapterQualification.state -ceq 'PASS_R18ZV2_BATCH_ADAPTER_EXACT_SEAM_ALGORITHM_UNCHANGED' -and
    ...
    -not [bool]$adapterQualification.tests.providerRunPerformed) 'R18ZV2 batch adapter qualification changed.'
$predecessorDependency = Get-ValidatedDependency 'priorRequestTerminalRouteGate'
$predecessorGate = Read-JsonFile ([string]$predecessorDependency.path) 1048576
Require ([string]$predecessorGate.state -ceq 'PASS_R18ZV1_PREDECESSOR_TERMINAL_FAILED_BEFORE_PROVIDER_ENTRY_NO_RETRY' -and
    [bool]$predecessorGate.conclusion.priorEndpointRequestTerminal -and
    [bool]$predecessorGate.conclusion.signedTerminalResponseReturnedToShare -and
    [bool]$predecessorGate.conclusion.returnRouteHealthy -and
    -not [bool]$predecessorGate.conclusion.backgroundBatchScientificallySucceeded -and
    [bool]$predecessorGate.conclusion.failureOccurredBeforePublicProviderEntry -and
    -not [bool]$predecessorGate.conclusion.priorRetryAuthorized -and
    [string]$predecessorGate.successorRequestId -ceq 'REQ_RZV2') 'R18ZV1 predecessor terminal/failure boundary changed.'
...
try {
    ...
    Require ([string]$manifest.schema -ceq 'argos_project_portal_request_manifest_v1' -and [string]$manifest.requestId -ceq 'REQ_RZV2' -and
    ...
    Require ([string]$change.source -ceq 'payload/Invoke-R18ZTBatchLaunch.ps1' -and
        [string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZV2.ps1' -and
...
Require ($requestRoot -ceq 'U:\ProjectPortalRO\requests' -and $responseRoot -ceq 'U:\ProjectPortalRO\responses' -and
    $readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZV2.ready.zip' -and $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZV2.ready.zip.upload' -and
    $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_RZV2.ready.zip') 'R18ZT exact publication route changed.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv2_publication_apply_intent_v1'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZV2_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
    requestId = 'REQ_RZV2'
...
Require ([string]$collision.requestId -ceq 'REQ_RZV2' -and
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv2_publish_gate_v1'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZV2_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZV2_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_RZV2'
