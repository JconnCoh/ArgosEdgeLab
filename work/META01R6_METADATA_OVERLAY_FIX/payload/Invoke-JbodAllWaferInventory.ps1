...
if ([string]::IsNullOrWhiteSpace($MetadataSnapshotRoot)) {
    $processorConfigPath = Join-Path $StateRoot 'PROCESSOR_CONFIG.json'
    if (Test-Path -LiteralPath $processorConfigPath -PathType Leaf) {
        $processorConfig = Get-Content -LiteralPath $processorConfigPath -Raw | ConvertFrom-Json
        if (($processorConfig.PSObject.Properties.Name -contains 'metadataSnapshotRoot') -and
            -not [string]::IsNullOrWhiteSpace([string]$processorConfig.metadataSnapshotRoot)) {
            $MetadataSnapshotRoot = [string]$processorConfig.metadataSnapshotRoot
        }
    }
}
if ([string]::IsNullOrWhiteSpace($MetadataSnapshotRoot)) {
    $MetadataSnapshotRoot = Join-Path $StateRoot 'metadata\verified'
}
$script:MetadataOverlayPath = Join-Path ([IO.Path]::GetFullPath($MetadataSnapshotRoot)) 'ACTIVE_VERIFIED_METADATA_OVERLAY.json'
