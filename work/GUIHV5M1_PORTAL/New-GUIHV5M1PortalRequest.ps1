...
$outputRoot = 'C:\G5MZ'
$requestId = 'REQ_G5M1_0901_A1'
$utf8 = New-Object Text.UTF8Encoding($false)

$expectedDefinitionSha256 = '9F0F50C69EFD5E8AF63FEDD0901E2702BCD5B8D78449D42A84E286162AAE8E99'
...
$expectedPayloadHashes = @{
    'payload/Apply-GUIHV5M1DirectGuiPatch.ps1' = '83386150E0AC4942FCFC4807DF2CCFC03D24BA53FF2CECE7C3EE2F5055126359'
...
foreach ($requiredPath in @($definitionPath, $payloadRoot, $identityPath, $publicCertificatePath, $pathTool)) {
    Assert-True (Test-Path -LiteralPath $requiredPath) "GUIHV5M1 signing input is missing: $requiredPath"
}
Assert-True ((Get-Sha256 $definitionPath) -eq $expectedDefinitionSha256) 'GUIHV5M1 maintenance definition changed.'
Assert-True ((Get-Sha256 $identityPath) -eq $expectedIdentitySha256) 'GUIHV5M1 signer identity state changed.'
Assert-True ((Get-Sha256 $publicCertificatePath) -eq $expectedPublicCertificateSha256) 'GUIHV5M1 signer public certificate changed.'
...
Assert-True ($payloadFiles.Count -eq $expectedPayloadHashes.Count) 'GUIHV5M1 signing payload count changed.'
foreach ($row in $payloadFiles) {
    Assert-True ($expectedPayloadHashes.ContainsKey([string]$row.path)) "Unexpected GUIHV5M1 payload: $($row.path)"
    Assert-True ([string]$expectedPayloadHashes[[string]$row.path] -eq [string]$row.sha256) "GUIHV5M1 payload changed: $($row.path)"
}
...
Assert-True ([string]$definition.targetRole -eq 'JBOD') 'GUIHV5M1 target role changed.'
Assert-True ([string]$definition.jobClass -eq 'MAINTENANCE_PATCH') 'GUIHV5M1 job class changed.'
Assert-True ([string]$definition.entryPoint -eq 'payload/Apply-GUIHV5M1DirectGuiPatch.ps1') 'GUIHV5M1 entry point changed.'
Assert-True ([int64]$definition.maxResultBytes -eq 1048576) 'GUIHV5M1 result bound changed.'
Assert-True ($changes.Count -eq 4) 'GUIHV5M1 change count changed.'
Assert-True ($allowedTaskActions.Count -eq 3) 'GUIHV5M1 task-action count changed.'
Assert-True ([string]$definition.rehearsal.requiredState -eq 'PASS_GUIHV5M1_SCRIBE_HOLD_PROJECTION_PRODUCED_VALIDATED_AND_TRAY_ACTIVATED') 'GUIHV5M1 rehearsal state changed.'
Assert-True ($allowedTaskActions[0] -eq 'STOP_IF_RUNNING:ArgosEdgeLab.AllWaferMonitor.ReviewOnly.V2') 'GUIHV5M1 tray stop action changed.'
Assert-True ($allowedTaskActions[1] -eq 'START_ALWAYS:ArgosEdgeLab.AllWaferMonitor.ReviewOnly.V2') 'GUIHV5M1 tray start action changed.'
Assert-True ($allowedTaskActions[2] -eq 'CLOSE_IF_RUNNING:ArgosEdgeLab.JbodCompositeAccepted.V1_2.exe') 'GUIHV5M1 viewer-close action changed.'

$identity = Get-Content -Raw -LiteralPath $identityPath | ConvertFrom-Json
$thumbprint = ([string]$identity.thumbprint).Replace(' ', '').ToUpperInvariant()
$certificate = Get-Item -LiteralPath ("Cert:\CurrentUser\My\$thumbprint") -ErrorAction Stop
Assert-True ([bool]$certificate.HasPrivateKey) 'GUIHV5M1 signer private key is unavailable.'
$publicCertificate = New-Object Security.Cryptography.X509Certificates.X509Certificate2($publicCertificatePath)
Assert-True ($publicCertificate.Thumbprint.Replace(' ', '').ToUpperInvariant() -eq $thumbprint) 'GUIHV5M1 public certificate thumbprint changed.'
...
foreach ($freshPath in @($outputRoot, $partialRoot, $readyRoot)) {
    Assert-True (-not (Test-Path -LiteralPath $freshPath)) "Fresh GUIHV5M1 signing path required: $freshPath"
}
$plannedPaths = @(
...
foreach ($candidate in $plannedPaths) {
    ...
    Assert-True ([string]$pathResult.state -eq 'PASS_PATH_BUDGET') "GUIHV5M1 signing path gate failed: $candidate"
    $pathRows += $pathResult.candidates[0]
}

if ($Preflight) {
    [ordered]@{
        schema='argos_guihv5m1_portal_signing_preflight_v1'; createdUtc=[DateTime]::UtcNow.ToString('o')
        state='PASS_GUIHV5M1_PORTAL_SIGNING_PREFLIGHT'; requestId=$requestId; payloadFileCount=$payloadFiles.Count
...
Assert-True $signatureValid 'GUIHV5M1 signed manifest verification failed.'
Move-Item -LiteralPath $partialRoot -Destination $readyRoot

[ordered]@{
    schema='argos_guihv5m1_portal_signing_result_v1'; createdUtc=[DateTime]::UtcNow.ToString('o')
    state='PASS_GUIHV5M1_PORTAL_SIGNED_REQUEST'; requestId=$requestId; packagePath=$readyRoot
