...
$scribeQueueSha256 = '9374F6ABE5555EDCF6E607EFC836B01745F3977712C95966F3E2D0677B4A45AF'
...
function Assert-PathBudget {
    ...
    if (($fullPath.Length + 32) -ge 200 -or $maximumComponentLength -gt 80) {
        throw "GUIHV5M1 path budget refused: $fullPath"
    }
...
elseif ($Preflight -and -not [string]::IsNullOrWhiteSpace($InvocationManifest)) {
    ...
    $fixturePath = [Environment]::GetEnvironmentVariable('ARGOS_GUIHV5M1_REHEARSAL_MANIFEST', 'Process')
}
...
if ($isFixture) {
    ...
    if ([string]$fixture.schema -ne 'argos_guihv5m1_direct_patch_rehearsal_v1' -or -not [bool]$fixture.rehearsal) {
        throw 'GUIHV5M1 rehearsal fixture changed.'
    }
...
$payloadRows = @(
    foreach ($spec in $targetSpecs) {
        ...
        if (-not (Test-Path -LiteralPath $payloadPath -PathType Leaf)) {
            throw "GUIHV5M1 payload is missing: $($spec.payloadName)"
        }
        if ((Get-Sha256 -Path $payloadPath) -ne [string]$spec.targetSha256) {
            throw "GUIHV5M1 payload hash changed: $($spec.payloadName)"
        }

        $destination = Assert-PathBudget -Path (Join-Path $script:InstallRoot $spec.relativePath)
        if (-not (Test-Path -LiteralPath $destination -PathType Leaf)) {
            throw "GUIHV5M1 destination is missing: $($spec.relativePath)"
        }

        $installedSha256 = Get-Sha256 -Path $destination
        if (@($spec.approvedPredecessors) -notcontains $installedSha256) {
            throw "GUIHV5M1 destination predecessor is unapproved: $($spec.relativePath)"
        }
...
function Invoke-HoldProjectionCheck {
    ...
    $start.Arguments = '--hold-projection-check 1532 190 3'
...
function Assert-ScribeProjectionOutput {
    ...
    if ($beforeUnrelated.Count -ne 1342 -or $afterUnrelated.Count -ne 1342 -or
        ...
        throw 'GUIHV5M1 unrelated hold rows changed.'
    }
    ...
    $expectedCounts = @{
        ...
        'PROPOSAL_READY_OPERATOR_CONFIRMATION_REQUIRED' = 3
        'HOLD_HUMAN_VISIBLE_NONCANONICAL_CHECKSUM' = 5
        'SCRIBE_CONFIRMED_INSITE_LOOKUP_PENDING' = 1
    }
    foreach ($state in $expectedCounts.Keys) {
        ...
        if ($row.Count -ne 1 -or [int]$row[0].Count -ne [int]$expectedCounts[$state]) {
            throw "GUIHV5M1 queue-state split changed: $state"
        }
    }
    if (@($scribeRows | Where-Object {
        ...
        throw 'GUIHV5M1 exact scribe queue projection is incomplete.'
    }
    if ([int]$ReadinessAfter.scribeHoldQueueJoinRows -ne 380 -or
        ...
        throw 'GUIHV5M1 dashboard readiness queue-join evidence changed.'
    }
...
$preflightResult = [ordered]@{
    schema = 'argos_guihv5m1_scribe_hold_projection_maintenance_patch_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_GUIHV5M1_SCRIBE_HOLD_PROJECTION_MAINTENANCE_PATCH_PREFLIGHT'
    requestId = $requestId
    rehearsal = $isFixture
...
try {
    ...
    if ((Get-Sha256 -Path $dashboardPriorPath) -ne $dashboardManifestSha256 -or
        ...
        throw 'GUIHV5M1 signed producer-output predecessors changed.'
    }
    ...
    if ([string]$readinessAfter.state -ne 'DASHBOARD_CATALOG_READY_REVIEW_ONLY' -or
        ...
        throw 'GUIHV5M1 installed dashboard producer did not emit a fresh terminal revision.'
    }
    ...
    if ($isFixture -and [bool](Get-OptionalValue -Object $fixture -Name 'failAfterTargetVerification' -Default $false)) {
        throw 'INJECTED_GUIHV5M1_POST_PRODUCER_VALIDATION_FAILURE'
    }
    ...
    if ($processorAfter.Count -ne $processorBefore.Count) {
        throw 'Exact processor selector count changed during GUIHV5M1.'
    }
    if ($processorAfter.Count -eq 1 -and (
        ...
        throw 'Exact processor selector identity changed during GUIHV5M1.'
    }
    ...
    if ($processorTaskAfter.Count -ne 1 -or [string]$processorTaskAfter[0].state -ne [string]$processorTaskRows[0].state) {
        throw 'Processor selector task state changed during GUIHV5M1.'
    }
    if ((Get-Sha256 -Path $dashboardUpdaterPath) -ne $dashboardUpdaterSha256) {
        throw 'Dashboard updater changed during GUIHV5M1.'
    }

    foreach ($row in $priorRows) {
        if ((Get-Sha256 -Path $row.destination) -ne [string]$row.targetSha256) {
            throw "GUIHV5M1 target changed after tray activation: $([string]$row.relativePath)"
        }
    }

    [ordered]@{
        schema = 'argos_guihv5m1_scribe_hold_projection_maintenance_patch_result_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_GUIHV5M1_SCRIBE_HOLD_PROJECTION_PRODUCED_VALIDATED_AND_TRAY_ACTIVATED'
        requestId = $requestId
        rehearsal = $isFixture
        targetFiles = @(
            $priorRows | ForEach-Object {
        ...
        try {
            ...
            if ($dashboardMutationStarted) {
                ...
                if ((Get-Sha256 -Path $dashboardManifestPath) -ne $dashboardManifestSha256 -or
                    ...
                    throw 'GUIHV5M1 producer-output rollback verification failed.'
                }
            }
    ...
    $failureState = if ($rollbackVerified) {
        'FAIL_GUIHV5M1_SCRIBE_HOLD_PROJECTION_ROLLED_BACK_BEFORE_TRAY_RESTART'
    }
    else {
        'FAIL_GUIHV5M1_SCRIBE_HOLD_PROJECTION_RECOVERY_FAILED'
    }

    [ordered]@{
        schema = 'argos_guihv5m1_scribe_hold_projection_maintenance_patch_failure_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = $failureState
        requestId = $requestId
        rehearsal = $isFixture
    ...
    if ($rollbackVerified) {
        throw "GUIHV5M1 failed; all endpoint and producer-output predecessors were restored before tray relaunch. $originalFailure"
    }

    throw "GUIHV5M1 failed and recovery failed. Original: $originalFailure Recovery: $recoveryFailure"
}
