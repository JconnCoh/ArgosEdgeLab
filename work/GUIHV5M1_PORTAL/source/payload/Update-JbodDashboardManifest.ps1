...
foreach ($catalogAcquisition in @($catalog.acquisitions)) {
    ...
    if($currentRows.Count-eq0){
        ...
        if($holdReason-eq'HOLD_SCRIBE_CONFIRMATION_REQUIRED_BEFORE_DETECTOR'){
            $heldRow.scribeQueueState=$scribeState;$heldRow.scribeNextAction=$scribeNextAction
            $heldRow.scribeProposalSource=$scribeProposalSource;$heldRow.scribeActionability=$scribeActionability
        }
        $held.Add($heldRow)
