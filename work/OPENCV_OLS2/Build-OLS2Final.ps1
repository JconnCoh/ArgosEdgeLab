...
$requestId = 'REQ_OLS2'
$sourceRoot = Join-Path $workRoot 'signed_short\REQ_OLS2.ready'
$finalRoot = Join-Path $workRoot 'final'
$partialRoot = Join-Path $workRoot 'final.partial'
$zipName = 'REQ_OLS2.ready.zip'
...
$signedSourceGatePath = Join-Path $workRoot 'OLS2_SIGNED_SOURCE_GATE.json'
$providerGatePath = Join-Path $workRoot 'OLS2_PROVIDER_TEST_GATE_R3.json'
$entrypointGatePath = Join-Path $workRoot 'OLS2_ENTRYPOINT_TEST_GATE_R4.json'
$inheritanceGatePath = Join-Path $workRoot 'OLS2_WORKER_INHERITANCE_GATE.json'
$cloneGatePath = Join-Path $workRoot 'OLS2_CLONE_GATE.json'
...
$expectedEntrypointSha = '0CFA53516B5CD9CD3AAC6BF9316C4EF2847CBCA8A234202AF5E8CF942DDFD7DE'
...
$expectedDefinitionSha = '368BF24441571C6DBD6169805E63571EC94BD4E1223D34A5FD91CF2DA071F0E6'
$expectedProviderGateSha = 'C98500E5CD005B62E2B1F4683B8E5B4304B3AEC26C948497C7A851B86A87015D'
$expectedEntrypointGateSha = '430044256071EDE491B4CB3CFC36E0BEE6023BAD6D85DCD6F5D9CA6E58EB653E'
$expectedInheritanceGateSha = '58092C6586DEA26DFD937CD04A84A834D9B4C8A9B0D0DEFCBF4AC7736CD12E55'
$expectedCloneGateSha = '087F8933E9929F7C5AE7CDB650BEBCC9562E87E81902DE821BC2DF7C76847088'
...
function Assert-Gate([string]$Path, [string]$Sha256, [string]$State) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { throw "Required OLS2 gate is missing: $Path" }
    if ((Get-Sha $Path) -ne $Sha256) { throw "Required OLS2 gate hash changed: $Path" }
    $value = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
    if ([string]$value.state -ne $State) { throw "Required OLS2 gate state changed: $Path" }
    return $value
}

foreach ($path in @($sourceRoot, $pathTool, $publicCertificatePath, $definitionPath)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "OLS2 package prerequisite is missing: $path" }
}
if ((Get-Sha $definitionPath) -ne $expectedDefinitionSha) { throw 'OLS2 maintenance definition changed.' }
$signedSourceGate = Get-Content -LiteralPath $signedSourceGatePath -Raw | ConvertFrom-Json
if ([string]$signedSourceGate.schema -ne 'argos_ols2_signed_source_gate_v1' -or [string]$signedSourceGate.state -ne 'PASS_OLS2_SIGNED_SOURCE_GATE' -or [string]$signedSourceGate.requestId -ne $requestId -or -not ([IO.Path]::GetFullPath([string]$signedSourceGate.signedSourcePath)).Equals([IO.Path]::GetFullPath($sourceRoot), [StringComparison]::OrdinalIgnoreCase) -or [string]$signedSourceGate.maintenanceDefinitionSha256 -ne $expectedDefinitionSha -or [string]$signedSourceGate.entrypointSha256 -ne $expectedEntrypointSha -or [string]$signedSourceGate.workerSha256 -ne $expectedWorkerSha -or [int]$signedSourceGate.payloadFileCount -ne 2 -or [string]$signedSourceGate.signatureAlgorithm -ne 'RSA-SHA256-PKCS1' -or -not [bool]$signedSourceGate.reviewOnly -or [bool]$signedSourceGate.productionRoutingEnabled) { throw 'OLS2 signed-source gate contract changed.' }
...
if ($expectedManifestSha -notmatch '^[0-9A-F]{64}$' -or $expectedSignatureSha -notmatch '^[0-9A-F]{64}$') { throw 'OLS2 signed-source gate hashes are invalid.' }
if (Test-Path -LiteralPath $finalRoot) { throw "OLS2 final root already exists: $finalRoot" }
if (Test-Path -LiteralPath $partialRoot) { throw "OLS2 partial final root already exists: $partialRoot" }
...
if ($sourceFiles.Count -ne $expectedFiles.Count) { throw "OLS2 signed source file count changed: $($sourceFiles.Count)" }
foreach ($entry in $expectedFiles.GetEnumerator()) {
    $path = Join-Path $sourceRoot $entry.Key.Replace('/', '\')
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or (Get-Sha $path) -ne [string]$entry.Value) { throw "OLS2 signed source changed: $($entry.Key)" }
}

$providerGate = Assert-Gate $providerGatePath $expectedProviderGateSha 'PASS_OLS2_PROVIDER_TEST_GATE'
$entrypointGate = Assert-Gate $entrypointGatePath $expectedEntrypointGateSha 'PASS_OLS2_ENTRYPOINT_TEST_GATE'
...
if ([string]$pathGate.state -ne 'PASS_PATH_BUDGET') { throw "OLS2 final package path gate failed: $($pathGate.state)" }

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ols2_final_package_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OLS2_FINAL_PACKAGE_PREFLIGHT'
        requestId = $requestId
...
if ($extracted.Count -ne $expectedFiles.Count) { throw "OLS2 extracted file count changed: $($extracted.Count)" }
foreach ($entry in $expectedFiles.GetEnumerator()) {
    $path = Join-Path $partialExtract $entry.Key.Replace('/', '\')
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or (Get-Sha $path) -ne [string]$entry.Value) { throw "OLS2 exact final ZIP file changed: $($entry.Key)" }
}
...
if (-not $signaturePassed) { throw 'OLS2 exact final ZIP signature verification failed.' }

$manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
if ([string]$manifest.requestId -ne $requestId -or [string]$manifest.targetRole -ne 'JBOD' -or [string]$manifest.jobClass -ne 'MAINTENANCE_PATCH' -or @($manifest.files).Count -ne 2 -or @($manifest.changes).Count -ne 1 -or @($manifest.entryPointMutations).Count -ne 1 -or @($manifest.entryPointOutputs).Count -ne 1 -or @($manifest.allowedTaskActions).Count -ne 0 -or @($manifest.allowedProcessActions).Count -ne 0) { throw 'OLS2 exact final ZIP manifest contract changed.' }
$parserPassed = 0
foreach ($relative in @('payload/E.ps1', 'payload/W.ps1')) {
    ...
    if (@($errors).Count -ne 0) { throw "OLS2 exact final ZIP parser failure: $relative" }
    $parserPassed++
}
...
$gate = [ordered]@{
    schema = 'argos_ols2_final_package_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_OLS2_FINAL_PACKAGE_GATE'
    requestId = $requestId
    requestZip = 'work/OPENCV_OLS2/final/REQ_OLS2.ready.zip'
