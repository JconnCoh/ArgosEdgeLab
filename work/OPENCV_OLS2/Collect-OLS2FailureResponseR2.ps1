...
$optionalAbsent = [pscustomobject]@{ state = 'FAILED' }
$optionalPresent = [pscustomobject]@{ state = 'FAILED'; detail = 'CONTROL_DETAIL' }
if ($null -ne (Get-OptionalString $optionalAbsent 'detail') -or (Get-OptionalString $optionalPresent 'detail') -ne 'CONTROL_DETAIL') { throw 'OLS2 R2 optional-property controls failed.' }
...
if ([string]$invocation.schema -ne 'argos_ols2_failure_response_collection_r2_invocation_v1' -or [string]$invocation.requestId -ne $requestId) { throw 'OLS2 R2 failure-response invocation contract changed.' }
...
if (-not $sourceZip.StartsWith($approvedResponseRoot + '\', [StringComparison]::OrdinalIgnoreCase) -or -not ([IO.Path]::GetDirectoryName($sourceZip)).Equals($approvedResponseRoot, [StringComparison]::OrdinalIgnoreCase) -or [IO.Path]::GetExtension($sourceZip) -ine '.zip') { throw 'OLS2 R2 failure response is outside the exact approved response root.' }
$responseToken = [IO.Path]::GetFileNameWithoutExtension($sourceZip)
if ($responseToken -notmatch '^R_[0-9A-F]{12}_[0-9]{17}_[0-9a-f]{8}\.ready$') { throw 'OLS2 R2 failure-response token format changed.' }

$expectedZipBytes = [int64]$invocation.expectedZipBytes
$expectedZipSha256 = ([string]$invocation.expectedZipSha256).ToUpperInvariant()
if ($expectedZipBytes -lt 1 -or $expectedZipBytes -gt 2097152 -or $expectedZipSha256 -notmatch '^[0-9A-F]{64}$') { throw 'OLS2 R2 failure-response size or hash is invalid.' }
$collectionRoot = [IO.Path]::GetFullPath([string]$invocation.collectionRoot).TrimEnd('\')
if (-not $collectionRoot.Equals('C:\AS2G', [StringComparison]::OrdinalIgnoreCase)) { throw 'OLS2 R2 failure-response collection root changed.' }
$terminalGatePath = [IO.Path]::GetFullPath([string]$invocation.terminalGatePath)
$expectedTerminalGatePath = Join-Path $project 'work\OPENCV_OLS2\OLS2_SIGNED_FAILURE_GATE_R2.json'
if (-not $terminalGatePath.Equals($expectedTerminalGatePath, [StringComparison]::OrdinalIgnoreCase)) { throw 'OLS2 R2 failure-response terminal gate path changed.' }
...
foreach ($path in @($sourceZip,$responseVerifier,$endpointCertificate,$pathTool)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "OLS2 R2 failure-response prerequisite is missing: $path" }
}
if ((Get-Item -LiteralPath $sourceZip).Length -ne $expectedZipBytes -or (Get-FileHash -LiteralPath $sourceZip -Algorithm SHA256).Hash -ne $expectedZipSha256) { throw 'OLS2 R2 source failure-response ZIP changed.' }
foreach ($path in @($collectionRoot,$localZip,$readyRoot,$partialRoot,$terminalGatePath)) { if (Test-Path -LiteralPath $path) { throw "OLS2 R2 failure-response output already exists: $path" } }

Add-Type -AssemblyName System.IO.Compression.FileSystem
$archive = [IO.Compression.ZipFile]::OpenRead($sourceZip)
try {
    $entries = @($archive.Entries)
    if ($entries.Count -ne 5) { throw 'OLS2 R2 bounded failure-response ZIP entry count changed.' }
    $manifestEntry = $archive.GetEntry('PORTAL_RESPONSE_MANIFEST.json')
    if ($null -eq $manifestEntry -or $manifestEntry.Length -gt 1048576) { throw 'OLS2 R2 bounded failure-response manifest entry is missing or too large.' }
...
if ([string]$manifest.requestId -ne $requestId -or [string]$manifest.sourceRole -ne 'JBOD' -or [string]$manifest.state -ne 'FAILED' -or -not [bool]$manifest.reviewOnly -or [bool]$manifest.productionRoutingEnabled) { throw 'OLS2 R2 failure-response terminal contract changed.' }
...
if (@(Compare-Object -ReferenceObject $expectedFiles -DifferenceObject $declaredFiles).Count -ne 0) { throw 'OLS2 R2 failure-response file set changed.' }
...
if ([string]$pathGate.state -ne 'PASS_PATH_BUDGET') { throw "OLS2 R2 failure-response collection path gate failed: $($pathGate.state)" }

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ols2_failure_response_collection_r2_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OLS2_FAILURE_RESPONSE_COLLECTION_R2_PREFLIGHT'
        requestId = $requestId
...
if ((Get-Item -LiteralPath $localZip).Length -ne $expectedZipBytes -or (Get-FileHash -LiteralPath $localZip -Algorithm SHA256).Hash -ne $expectedZipSha256) { throw 'OLS2 R2 local failure-response ZIP changed during copy.' }
...
if ($LASTEXITCODE -ne 0) { throw 'OLS2 R2 signed failure-response verification failed.' }
$extractedManifest = Get-Content -LiteralPath (Join-Path $partialRoot 'PORTAL_RESPONSE_MANIFEST.json') -Raw | ConvertFrom-Json
if ([string]$extractedManifest.state -ne 'FAILED' -or @($extractedManifest.files).Count -ne 3) { throw 'OLS2 R2 extracted signed terminal-failure contract changed.' }
$responseRows = New-Object Collections.Generic.List[object]
foreach ($name in $expectedFiles) {
    $path = Join-Path $partialRoot $name
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or (Get-Item -LiteralPath $path).Length -gt 1048576) { throw "OLS2 R2 bounded failure-response file is missing or too large: $name" }
...
$failureDetail = Get-OptionalString $failure 'detail'
Move-Item -LiteralPath $partialRoot -Destination $readyRoot

$gate = [ordered]@{
    schema = 'argos_ols2_terminal_failure_gate_r2_v1'
    collectedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_OLS2_SIGNED_TERMINAL_FAILURE_R2_COLLECTED'
