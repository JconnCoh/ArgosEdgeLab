...
if ([string]$manifest.schema -ne 'argos_ols2_entrypoint_test_invocation_v1') { throw 'OLS2 test invocation schema mismatch.' }
...
foreach ($path in @($entrypoint, $priorWorker, $targetWorker)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "OLS2 test prerequisite missing: $path" }
}
if ((Get-FileHash -LiteralPath $priorWorker -Algorithm SHA256).Hash -ne $priorHash) { throw 'OLS2 test predecessor hash changed.' }
if ((Get-FileHash -LiteralPath $targetWorker -Algorithm SHA256).Hash -ne $targetHash) { throw 'OLS2 test target hash changed.' }
if ((Test-Path -LiteralPath $testRoot) -or (Test-Path -LiteralPath $dataRoot) -or (Test-Path -LiteralPath $gatePath)) { throw 'OLS2 test requires fresh test, data, and gate paths.' }
...
if ($maximumCanonicalEffectiveLength -ge 230 -or $aliasEffectiveLength -ge 200) { throw 'OLS2 planned canonical or process-local alias path budget failed before test write.' }

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ols2_entrypoint_test_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OLS2_ENTRYPOINT_TEST_PREFLIGHT'
        testRoot = $testRoot
        dataRoot = $dataRoot
        gatePath = $gatePath
...
function New-Case {
    ...
    $outputPath = Join-Path $processor 'OCV00_OLS2_LOT_INVENTORY.json'
    ...
    Write-Json -Path $entryInvocation -Value ([ordered]@{
        schema = 'argos_ols2_entrypoint_invocation_v1'
        portalRoot = $portal
...
Assert-True ([string]$entryPreflightResult.state -eq 'PASS_OLS2_ENTRYPOINT_PREFLIGHT') 'OLS2 entrypoint preflight failed.'
Assert-True (-not(Test-Path -LiteralPath $oldCase.outputPath)) 'OLS2 entrypoint preflight mutated output.'
Assert-True (-not[bool](Get-PSDrive -Name F -ErrorAction SilentlyContinue)) 'OLS2 entrypoint preflight created process-local alias F.'
$oldResult = & $entrypoint -Rehearsal -InvocationManifest $oldCase.entryInvocation | ConvertFrom-Json
Assert-True ([string]$oldResult.state -eq 'PASS_OCV00_BOUNDED_LOT_SUBTREE_INVENTORY_OLS2') 'OLS2 old-predecessor rehearsal failed.'
Assert-True ((Get-FileHash -LiteralPath $oldCase.workerPath -Algorithm SHA256).Hash -eq $targetHash) 'OLS2 old predecessor did not install target.'
Assert-True (Test-Path -LiteralPath $oldCase.outputPath -PathType Leaf) 'OLS2 old-predecessor case omitted capability output.'
Assert-True (-not[bool](Get-PSDrive -Name F -ErrorAction SilentlyContinue)) 'OLS2 unexpectedly created process-local alias F.'
...
Assert-True ([string]$oldInventory.state -eq 'COMPLETE' -and [bool]$oldInventory.complete -and [bool]$oldInventory.rootExists -and -not [bool]$oldInventory.truncated) 'OLS2 bounded lot subtree inventory did not complete.'
Assert-True ($oldBmpLeaves.Count -eq 2 -and [int]$oldInventory.bmpLeafCount -eq 2 -and [int]$signedInventory.bmpLeafCount -eq 2 -and [int]$oldInventory.otherLeafCount -eq 1) 'OLS2 bounded lot subtree inventory did not return the expected BF/DF metadata rows.'
Assert-True ([string]$oldOutput.inventory.processLocalAlias.name -eq 'F' -and [bool]$oldOutput.inventory.processLocalAlias.created -and [bool]$oldOutput.inventory.processLocalAlias.removed -and -not [bool]$oldOutput.inventory.processLocalAlias.persistent) 'OLS2 process-local alias cleanup failed.'
...
Assert-True ([bool]$oldResult.metadataOnly -and [bool]$oldResult.pathsEnumerated -and -not[bool]$oldResult.filesRead -and -not[bool]$oldResult.imageBytesRead -and -not[bool]$oldResult.sourceHashingPerformed) 'OLS2 entrypoint result did not preserve signed-response metadata-only controls.'
...
Assert-True ([string]$targetResult.state -eq 'PASS_OCV00_BOUNDED_LOT_SUBTREE_INVENTORY_OLS2') 'OLS2 target-idempotent rehearsal failed.'
Assert-True (-not[bool]$targetResult.workerChanged) 'OLS2 target-idempotent case reported a change.'
Assert-True ((Get-FileHash -LiteralPath $targetCase.workerPath -Algorithm SHA256).Hash -eq $targetHash) 'OLS2 target-idempotent worker changed unexpectedly.'
...
Assert-True ([string]$zeroResult.state -eq 'PASS_OCV00_BOUNDED_LOT_SUBTREE_INVENTORY_OLS2' -and -not [bool]$zeroResult.boundedSubtreeInventory.rootExists -and [int]$zeroResult.boundedSubtreeInventory.bmpLeafCount -eq 0) 'OLS2 absent-subtree collection case failed.'
...
Assert-True ([string]$oneResult.state -eq 'PASS_OCV00_BOUNDED_LOT_SUBTREE_INVENTORY_OLS2' -and [bool]$oneResult.boundedSubtreeInventory.rootExists -and [int]$oneResult.boundedSubtreeInventory.bmpLeafCount -eq 1) 'OLS2 one-BMP collection case failed.'
...
Assert-True $unapprovedFailed 'OLS2 unapproved predecessor was not refused.'
Assert-True (-not(Test-Path -LiteralPath $unapprovedCase.outputPath)) 'OLS2 unapproved case created output.'
...
catch { $rollbackFailed = $_.Exception.Message -like '*INJECTED_OLS2_FAILURE_AFTER_SWAP*' }
Assert-True $rollbackFailed 'OLS2 injected rollback case did not fail as expected.'
Assert-True ((Get-FileHash -LiteralPath $rollbackCase.workerPath -Algorithm SHA256).Hash -eq $priorHash) 'OLS2 rollback did not restore predecessor.'
Assert-True (-not(Test-Path -LiteralPath $rollbackCase.outputPath)) 'OLS2 rollback case left capability output.'
...
Assert-True $unsafeFailed 'OLS2 unsafe subtree root was not refused.'
Assert-True (-not(Test-Path -LiteralPath $unsafeCase.outputPath)) 'OLS2 unsafe-root preflight created output.'
$cases.Add([pscustomobject]@{caseId='UNSAFE_SUBTREE_ROOT_REFUSAL';state='PASS';mutationsPerformed=$false})

$truncatedCase = New-Case -CaseId 'TRUNCATED' -WorkerSource $targetWorker -FailAfterSwap $false
$truncatedProbe = Get-Content -LiteralPath $truncatedCase.probePath -Raw | ConvertFrom-Json
$truncatedProbe.outputPath = (Join-Path $truncatedCase.processor 'TRUNCATED.json')
$truncatedProbe.parameters.environmentInventory.boundedSubtreeInventory.maximumBmpLeaves = 1
$truncatedProbePath = Join-Path $truncatedCase.root 'TRUNCATED_PROBE.json'
Write-Json -Path $truncatedProbePath -Value $truncatedProbe
$truncatedResult = & $targetWorker -ConfigPath $truncatedCase.configPath -EnvironmentProbeManifest $truncatedProbePath | ConvertFrom-Json
$truncatedInventory = $truncatedResult.inventory.boundedSubtreeInventory
Assert-True ([string]$truncatedResult.state -eq 'PASS_JBOD_ENVIRONMENT_INVENTORY' -and [string]$truncatedInventory.state -eq 'HOLD_INCOMPLETE' -and [bool]$truncatedInventory.truncated -and -not [bool]$truncatedInventory.complete -and [int]$truncatedInventory.bmpLeafCount -eq 1 -and -not [bool]$truncatedInventory.filesRead -and -not [bool]$truncatedInventory.imageBytesRead -and -not [bool]$truncatedInventory.sourceHashingPerformed) 'OLS2 truncation hold case failed.'
...
Assert-True $rootFailed 'OLS2 unapproved data root was not refused.'
Assert-True (-not(Test-Path -LiteralPath $unapprovedRootCase.outputPath)) 'OLS2 unapproved-root preflight created output.'
$cases.Add([pscustomobject]@{caseId='UNAPPROVED_ROOT_REFUSAL';state='PASS';mutationsPerformed=$false})

$gate = [ordered]@{
    schema = 'argos_ols2_entrypoint_test_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_OLS2_ENTRYPOINT_TEST_GATE'
