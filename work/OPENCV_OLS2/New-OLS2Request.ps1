...
$signedSourceGatePath = Join-Path $PSScriptRoot 'OLS2_SIGNED_SOURCE_GATE.json'
...
if ([string]$definition.targetRole -ne 'JBOD' -or [string]$definition.jobClass -ne 'MAINTENANCE_PATCH') { throw 'OLS2 maintenance definition contract refused.' }
if (@($definition.allowedTaskActions).Count -ne 0 -or @($definition.allowedProcessActions).Count -ne 0) { throw 'OLS2 task/process actions must remain empty.' }
if (@($definition.changes).Count -ne 1 -or @($definition.entryPointMutations).Count -ne 1 -or @($definition.entryPointOutputs).Count -ne 1) { throw 'OLS2 declared mutation/output cardinality changed.' }
$readContract = $definition.metadataReadContract
if ([string]$readContract.mode -ne 'CONFIG_ANCHORED_BOUNDED_SUBTREE_METADATA_ONLY' -or [string]$readContract.approvedDataRoot -ne 'JBOD_KLARF_EXPORT' -or [string]$readContract.processLocalAliasName -ne 'F' -or [string]$readContract.relativeRoot -ne 'PatternedFront/Lot_62619-433' -or [int]$readContract.maximumDepth -ne 8 -or [int]$readContract.maximumEntries -ne 20000 -or [int]$readContract.maximumDirectories -ne 2048 -or [int]$readContract.maximumBmpLeaves -ne 2048 -or -not [bool]$readContract.pathEnumerationAllowed -or -not [bool]$readContract.subtreeMetadataOnly -or [bool]$readContract.fileContentReadAllowed -or [bool]$readContract.imageBytesReadAllowed -or [bool]$readContract.sourceHashingAllowed -or [bool]$readContract.taskOrProcessActionAllowed) { throw 'OLS2 metadata-read contract changed.' }
...
if ($payloadItems.Count -ne 2) { throw "OLS2 payload file count changed: $($payloadItems.Count)" }
...
if ($entry.Count -ne 1 -or [string]$entry[0].sha256 -ne [string]$definition.changes[0].installedSha256) { throw 'OLS2 entrypoint target hash mismatch.' }
if ($worker.Count -ne 1 -or [string]$worker[0].sha256 -ne [string]$definition.entryPointMutations[0].installedSha256) { throw 'OLS2 worker target hash mismatch.' }
if ([string]$definition.entryPoint -ne 'payload/E.ps1') { throw 'OLS2 entrypoint path changed.' }

$ready = Join-Path $outputRoot ($requestId + '.ready')
$partial = Join-Path $outputRoot ($requestId + '.partial')
foreach ($path in @($ready, $partial, $signedSourceGatePath, $signedSourceGatePartial)) { if (Test-Path -LiteralPath $path) { throw "OLS2 request path already exists: $path" } }
...
if ([string]$pathGate.state -ne 'PASS_PATH_BUDGET') { throw "OLS2 signing path gate failed: $($pathGate.state)" }

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ols2_signing_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OLS2_SIGNING_PREFLIGHT'
        requestId = $requestId
...
$signedSourceGate = [ordered]@{
    schema = 'argos_ols2_signed_source_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_OLS2_SIGNED_SOURCE_GATE'
    requestId = $requestId
...
[ordered]@{
    schema = 'argos_ols2_signing_result_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_OLS2_SIGNED_REQUEST'
    requestId = $requestId
    packagePath = $ready
    requestManifestSha256 = $requestManifestSha256
    requestSignatureSha256 = $requestSignatureSha256
