...
function Get-EnvironmentInventory {
    ...
    $subtreeRequest=Get-MemberValue $request 'boundedSubtreeInventory' $null
    $subtreeEnabled=($null-ne$subtreeRequest-and[bool](Get-MemberValue $subtreeRequest 'enabled' $false))
    ...
    if($childNames.Count-eq0-and$relativeLeafPaths.Count-eq0-and-not$subtreeEnabled){throw 'Environment inventory requires at least one bounded metadata selector.'}
    ...
    $subtreeResult=$null
    if($subtreeEnabled){
        if($relativeLeafPaths.Count-gt0-or$childNames.Count-gt0){throw 'Environment inventory bounded subtree cannot be combined with another metadata selector.'}
        if($aliasName-ne'F'){throw 'Environment inventory bounded subtree metadata requires fixed process-local alias F.'}
        $relativeRoot=([string](Get-MemberValue $subtreeRequest 'relativeRoot' '')).Replace('/','\').Trim('\')
        $maximumDepth=[int](Get-MemberValue $subtreeRequest 'maximumDepth' 0)
        $maximumEntries=[int](Get-MemberValue $subtreeRequest 'maximumEntries' 0)
        $maximumDirectories=[int](Get-MemberValue $subtreeRequest 'maximumDirectories' 0)
        $maximumBmpLeaves=[int](Get-MemberValue $subtreeRequest 'maximumBmpLeaves' 0)
        if([string]::IsNullOrWhiteSpace($relativeRoot)-or[IO.Path]::IsPathRooted($relativeRoot)-or$relativeRoot-match'(^|\\)\.\.?($|\\)'-or$relativeRoot.IndexOfAny([char[]]'*?[]')-ge0){throw 'Environment inventory bounded subtree relative root is unsafe.'}
        if($maximumDepth-lt1-or$maximumDepth-gt8){throw 'Environment inventory bounded subtree depth must be 1 through 8.'}
        if($maximumEntries-lt1-or$maximumEntries-gt20000){throw 'Environment inventory bounded subtree entry limit must be 1 through 20000.'}
        if($maximumDirectories-lt1-or$maximumDirectories-gt2048){throw 'Environment inventory bounded subtree directory limit must be 1 through 2048.'}
        if($maximumBmpLeaves-lt1-or$maximumBmpLeaves-gt2048){throw 'Environment inventory bounded subtree BMP limit must be 1 through 2048.'}
        $subtreeRoot=Get-SafeChildPath -Root $configuredRoot -Relative $relativeRoot
        $aliasRoot=$aliasName+':\'+$relativeRoot
        $canonicalRootBudget=Get-PortalPathBudget -Path $subtreeRoot -ReservedSuffixCharacters 32
        $aliasRootBudget=Get-PortalPathBudget -Path $aliasRoot -ReservedSuffixCharacters 32
        if([string]$canonicalRootBudget.state-eq'HARD_STOP_SHORTEN_BEFORE_WRITE_OR_LAUNCH'-or[string]$aliasRootBudget.state-ne'PASS_PATH_BUDGET'){throw "Environment inventory bounded subtree root path budget failed: $subtreeRoot"}
        if($Preflight){
            $subtreeResult=[ordered]@{schema='argos_bounded_subtree_inventory_v1';state='UNOBSERVED_PREFLIGHT';relativeRoot=$relativeRoot;normalizedRoot=$subtreeRoot;aliasReadRoot=$aliasRoot;rootExists=$null;rootAttributes=$null;rootLastWriteTimeUtc=$null;canonicalRootEffectiveLength=[int]$canonicalRootBudget.effectiveLength;aliasRootEffectiveLength=[int]$aliasRootBudget.effectiveLength;maximumDepth=$maximumDepth;maximumEntries=$maximumEntries;maximumDirectories=$maximumDirectories;maximumBmpLeaves=$maximumBmpLeaves;entriesVisited=0;directoriesEnumerated=0;directories=@();directoryCount=0;bmpLeaves=@();bmpLeafCount=0;otherLeafCount=0;depthBoundaryDirectoryCount=0;truncated=$false;accessErrorCount=0;accessErrors=@();skippedReparseSubtrees=0;skippedUnsafePathSubtrees=0;complete=$false;pathsEnumerated=$false;filesRead=$false;imageBytesRead=$false;sourceHashingPerformed=$false;mutationsPerformed=$false}
        }else{
            if(Get-PSDrive -Name $aliasName -ErrorAction SilentlyContinue){throw "Environment inventory process-local alias is already in use: $aliasName"}
            $rootExists=$false
            $rootAttributes=$null
            $rootLastWriteTimeUtc=$null
            $directoryRows=New-Object 'Collections.Generic.List[object]'
            ...
            try{
                ...
                $current=$aliasName+':'
                $missing=$false
                foreach($segment in @($relativeRoot.Split('\')|Where-Object{-not[string]::IsNullOrWhiteSpace($_)})){
                    $current=$current+'\'+$segment
                    if(-not(Test-Path -LiteralPath $current)){$missing=$true;break}
                    $currentItem=Get-Item -LiteralPath $current -Force -ErrorAction Stop
                    if(($currentItem.Attributes-band[IO.FileAttributes]::ReparsePoint)-ne0){throw 'Environment inventory bounded subtree root has a reparse ancestor.'}
                }
                $rootExists=(-not$missing-and(Test-Path -LiteralPath $aliasRoot -PathType Container))
                if($rootExists){
                    $rootItem=Get-Item -LiteralPath $aliasRoot -Force -ErrorAction Stop
                    $rootAttributes=[string]$rootItem.Attributes
                    $rootLastWriteTimeUtc=$rootItem.LastWriteTimeUtc.ToString('o')
                    $pending=New-Object 'Collections.Generic.Queue[object]'
                    $pending.Enqueue([pscustomobject]@{readPath=$aliasRoot;canonicalPath=$subtreeRoot;depth=0})
                    while($pending.Count-gt0-and-not$truncated){
                        $node=$pending.Dequeue()
                        $childrenToVisit=@()
                        try{$childrenToVisit=@(Get-ChildItem -LiteralPath ([string]$node.readPath) -Force -ErrorAction Stop)}
                        catch{
                            $message=[string]$_.Exception.Message
                            if($accessErrors.Count-lt16){$accessErrors.Add([pscustomobject]@{path=[string]$node.canonicalPath;message=$message.Substring(0,[Math]::Min(240,$message.Length))})}
                            continue
                        }
                        $directoriesEnumerated++
                        foreach($child in $childrenToVisit){
                            if($entriesVisited-ge$maximumEntries){$truncated=$true;break}
                            $entriesVisited++
                            $full=[IO.Path]::GetFullPath((Join-Path ([string]$node.canonicalPath) ([string]$child.Name)))
                            $aliasRead=Join-Path ([string]$node.readPath) ([string]$child.Name)
                            $approvedPrefix=$configuredRoot+'\'
                            $subtreePrefix=$subtreeRoot+'\'
                            if(-not$full.StartsWith($approvedPrefix,[StringComparison]::OrdinalIgnoreCase)-or-not$full.StartsWith($subtreePrefix,[StringComparison]::OrdinalIgnoreCase)){throw 'Environment inventory bounded subtree result escaped the approved root.'}
                            $relative=$full.Substring($approvedPrefix.Length)
                            $subtreeRelative=$full.Substring($subtreePrefix.Length)
                            ...
                            $canonicalBudget=Get-PortalPathBudget -Path $full -ReservedSuffixCharacters 32
                            $aliasBudget=Get-PortalPathBudget -Path $aliasRead -ReservedSuffixCharacters 32
                            if([string]$canonicalBudget.state-eq'HARD_STOP_SHORTEN_BEFORE_WRITE_OR_LAUNCH'-or[string]$aliasBudget.state-ne'PASS_PATH_BUDGET'){$skippedUnsafePathSubtrees++;continue}
                            if($reparse-and-not$isContainer){$skippedReparseSubtrees++}
                            if($isContainer){
                                if($directoryRows.Count-ge$maximumDirectories){$truncated=$true;break}
                                $directoryRows.Add([pscustomobject]@{relativePath=$relative;subtreeRelativePath=$subtreeRelative;normalizedFullPath=$full;aliasReadPath=$aliasRead;name=[string]$child.Name;depth=$depth;pathType='CONTAINER';attributes=[string]$child.Attributes;reparsePoint=$reparse;lastWriteTimeUtc=$child.LastWriteTimeUtc.ToString('o');canonicalEffectiveLength=[int]$canonicalBudget.effectiveLength;aliasEffectiveLength=[int]$aliasBudget.effectiveLength;containedByApprovedRoot=$true;filesRead=$false;imageBytesRead=$false;sourceHashingPerformed=$false;mutationsPerformed=$false})
                                if($reparse){$skippedReparseSubtrees++;continue}
                                if($depth-lt$maximumDepth){$pending.Enqueue([pscustomobject]@{readPath=$aliasRead;canonicalPath=$full;depth=$depth})}else{$depthBoundaryDirectoryCount++}
                            }elseif([string]$child.Extension-eq'.bmp'){
                                if($bmpRows.Count-ge$maximumBmpLeaves){$truncated=$true;break}
                                $bmpRows.Add([pscustomobject]@{relativePath=$relative;subtreeRelativePath=$subtreeRelative;normalizedFullPath=$full;aliasReadPath=$aliasRead;name=[string]$child.Name;depth=$depth;pathType='LEAF';extension='.bmp';attributes=[string]$child.Attributes;reparsePoint=$reparse;length=[int64]$child.Length;lastWriteTimeUtc=$child.LastWriteTimeUtc.ToString('o');canonicalEffectiveLength=[int]$canonicalBudget.effectiveLength;aliasEffectiveLength=[int]$aliasBudget.effectiveLength;containedByApprovedRoot=$true;filesRead=$false;imageBytesRead=$false;sourceHashingPerformed=$false;mutationsPerformed=$false})
                            }else{$otherLeafCount++}
                        }
                    }
                }
            ...
            if(-not$aliasRemoved){throw 'Environment inventory process-local alias was not removed.'}
            $directoryArray=$directoryRows.ToArray()
            $bmpArray=$bmpRows.ToArray()
            $errorArray=$accessErrors.ToArray()
            $complete=(-not$truncated-and$errorArray.Count-eq0-and$skippedReparseSubtrees-eq0-and$skippedUnsafePathSubtrees-eq0-and$depthBoundaryDirectoryCount-eq0)
            $subtreeResult=[ordered]@{schema='argos_bounded_subtree_inventory_v1';state=if($complete){'COMPLETE'}else{'HOLD_INCOMPLETE'};relativeRoot=$relativeRoot;normalizedRoot=$subtreeRoot;aliasReadRoot=$aliasRoot;rootExists=$rootExists;rootAttributes=$rootAttributes;rootLastWriteTimeUtc=$rootLastWriteTimeUtc;canonicalRootEffectiveLength=[int]$canonicalRootBudget.effectiveLength;aliasRootEffectiveLength=[int]$aliasRootBudget.effectiveLength;maximumDepth=$maximumDepth;maximumEntries=$maximumEntries;maximumDirectories=$maximumDirectories;maximumBmpLeaves=$maximumBmpLeaves;entriesVisited=$entriesVisited;directoriesEnumerated=$directoriesEnumerated;directories=$directoryArray;directoryCount=$directoryArray.Count;bmpLeaves=$bmpArray;bmpLeafCount=$bmpArray.Count;otherLeafCount=$otherLeafCount;depthBoundaryDirectoryCount=$depthBoundaryDirectoryCount;truncated=$truncated;accessErrorCount=$errorArray.Count;accessErrors=$errorArray;skippedReparseSubtrees=$skippedReparseSubtrees;skippedUnsafePathSubtrees=$skippedUnsafePathSubtrees;complete=$complete;pathsEnumerated=$true;filesRead=$false;imageBytesRead=$false;sourceHashingPerformed=$false;mutationsPerformed=$false}
        }
    }
    ...
    return [ordered]@{
        schema=if($subtreeEnabled){'argos_project_portal_environment_inventory_v3'}else{'argos_project_portal_environment_inventory_v2'}
        ...
        boundedSubtreeInventory=$subtreeResult
        ...
        commands=$commands
        pathsEnumerated=($subtreeEnabled-and-not$Preflight)
        filesRead=$false
