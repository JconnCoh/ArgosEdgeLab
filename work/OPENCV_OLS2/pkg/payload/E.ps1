...
if ($Preflight -and $Rehearsal) { throw 'OLS2 cannot combine Preflight and Rehearsal.' }
...
if ($Preflight -or $Rehearsal) {
    if ([string]::IsNullOrWhiteSpace($InvocationManifest)) { throw 'OLS2 Preflight/Rehearsal requires InvocationManifest.' }
    ...
    if ([string]$invocation.schema -ne 'argos_ols2_entrypoint_invocation_v1') { throw 'OLS2 entrypoint invocation schema mismatch.' }
...
function Assert-PathBudget {
    ...
    if (($full.Length + $Reserve) -ge 200 -or $longest -gt 80) { throw "OLS2 path budget refused: effective=$($full.Length + $Reserve) component=$longest path=$full" }
    return $full
}

function Write-Utf8JsonCreateNew {
    param([Parameter(Mandatory = $true)][string]$Path, [Parameter(Mandatory = $true)][object]$Value)
    if (Test-Path -LiteralPath $Path) { throw "OLS2 refuses overwrite: $Path" }
...
function Invoke-WorkerJson {
    ...
    if ($LASTEXITCODE -ne 0) { throw "OLS2 installed worker probe failed with exit code $LASTEXITCODE. $text" }
    try { return ($text | ConvertFrom-Json) }
    catch { throw "OLS2 installed worker probe did not return bounded JSON. $text" }
}
...
$outputPath = Assert-PathBudget (Join-Path $processorRoot 'OCV00_OLS2_LOT_INVENTORY.json')
$tempProbePath = Assert-PathBudget (Join-Path $processorRoot '.OLS2.probe.json')

foreach ($path in @($workerPath, $configPath, $payloadWorker)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "OLS2 prerequisite is missing: $path" }
}
if ((Get-ArgosFileHash -LiteralPath $payloadWorker -Algorithm SHA256).Hash -ne $targetWorkerSha256) { throw 'OLS2 payload worker hash changed.' }
$tokens = $null
$errors = $null
[void][Management.Automation.Language.Parser]::ParseFile($payloadWorker, [ref]$tokens, [ref]$errors)
if ($errors.Count -ne 0) { throw "OLS2 payload worker parser failure: $($errors[0].Message)" }

$installedBefore = (Get-ArgosFileHash -LiteralPath $workerPath -Algorithm SHA256).Hash
if ($installedBefore -ne $priorWorkerSha256 -and $installedBefore -ne $targetWorkerSha256) { throw "OLS2 installed endpoint worker predecessor is not approved: $installedBefore" }
if (Test-Path -LiteralPath $outputPath) { throw "OLS2 requires a fresh capability output namespace: $outputPath" }
if (Test-Path -LiteralPath $tempProbePath) { throw "OLS2 temporary probe path collision: $tempProbePath" }

if ($Preflight) {
    if (-not (Test-Path -LiteralPath $probeInvocationPath -PathType Leaf)) { throw "OLS2 preflight probe invocation is missing: $probeInvocationPath" }
    $probe = Get-Content -LiteralPath $probeInvocationPath -Raw | ConvertFrom-Json
    if ([string]$probe.schema -ne 'argos_project_portal_environment_probe_invocation_v1') { throw 'OLS2 preflight probe schema mismatch.' }
    if (-not ([IO.Path]::GetFullPath([string]$probe.outputPath)).Equals($outputPath, [StringComparison]::OrdinalIgnoreCase)) { throw 'OLS2 preflight output path mismatch.' }
    ...
    if ([string]$probeResult.state -ne 'PASS_JBOD_ENVIRONMENT_INVENTORY_PREFLIGHT' -or [bool]$probeResult.mutationsPerformed) { throw 'OLS2 payload worker non-mutating preflight failed.' }
    [ordered]@{
        schema = 'argos_ols2_entrypoint_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OLS2_ENTRYPOINT_PREFLIGHT'
...
$evidenceRoot = Assert-PathBudget (Join-Path $portalRoot ('state\maintenance_bootstrap\OLS2_' + [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffZ')))
if (Test-Path -LiteralPath $evidenceRoot) { throw "OLS2 evidence root collision: $evidenceRoot" }
...
$stagePath = Assert-PathBudget (Join-Path (Split-Path -Parent $workerPath) 'OLS2.stage.ps1')
$restorePath = Assert-PathBudget (Join-Path (Split-Path -Parent $workerPath) 'OLS2.restore.ps1')
foreach ($path in @($stagePath, $restorePath)) { if (Test-Path -LiteralPath $path) { throw "OLS2 short swap path collision: $path" } }
...
try {
    if ($installedBefore -eq $priorWorkerSha256) {
        ...
        if ((Get-ArgosFileHash -LiteralPath $priorPath -Algorithm SHA256).Hash -ne $priorWorkerSha256) { throw 'OLS2 predecessor archive hash mismatch.' }
        Copy-Item -LiteralPath $payloadWorker -Destination $stagePath -ErrorAction Stop
        if ((Get-ArgosFileHash -LiteralPath $stagePath -Algorithm SHA256).Hash -ne $targetWorkerSha256) { throw 'OLS2 staged worker hash mismatch.' }
        [IO.File]::Replace($stagePath, $workerPath, $backupPath, $true)
        $swapped = $true
        if ((Get-ArgosFileHash -LiteralPath $workerPath -Algorithm SHA256).Hash -ne $targetWorkerSha256) { throw 'OLS2 installed worker target hash mismatch.' }
        if ((Get-ArgosFileHash -LiteralPath $backupPath -Algorithm SHA256).Hash -ne $priorWorkerSha256) { throw 'OLS2 atomic backup predecessor hash mismatch.' }
        $changed = $true
    }
    if ($failAfterSwap) { throw 'INJECTED_OLS2_FAILURE_AFTER_SWAP' }
    ...
    if ([string]$preflightResult.state -ne 'PASS_JBOD_ENVIRONMENT_INVENTORY_PREFLIGHT' -or [bool]$preflightResult.mutationsPerformed) { throw 'OLS2 installed producer preflight failed.' }
    $plannedInventory = $preflightResult.inventory.boundedSubtreeInventory
    if ([string]$preflightResult.inventory.schema -ne 'argos_project_portal_environment_inventory_v3' -or [string]$plannedInventory.schema -ne 'argos_bounded_subtree_inventory_v1' -or [string]$plannedInventory.state -ne 'UNOBSERVED_PREFLIGHT' -or [string]$plannedInventory.relativeRoot -ne 'PatternedFront\Lot_62619-433' -or [string]$plannedInventory.aliasReadRoot -ne 'F:\PatternedFront\Lot_62619-433' -or [int]$plannedInventory.maximumDepth -ne 8 -or [int]$plannedInventory.maximumEntries -ne 20000 -or [int]$plannedInventory.maximumDirectories -ne 2048 -or [int]$plannedInventory.maximumBmpLeaves -ne 2048 -or [bool]$preflightResult.inventory.pathsEnumerated -or [bool]$plannedInventory.pathsEnumerated -or [bool]$plannedInventory.filesRead -or [bool]$plannedInventory.imageBytesRead -or [bool]$plannedInventory.sourceHashingPerformed -or [bool]$plannedInventory.mutationsPerformed) { throw 'OLS2 installed producer preflight subtree contract failed.' }
    $producerResult = Invoke-WorkerJson -WorkerPath $workerPath -ConfigPath $configPath -ProbePath $tempProbePath
    if ([string]$producerResult.state -ne 'PASS_JBOD_ENVIRONMENT_INVENTORY') { throw 'OLS2 installed producer terminal status failed.' }
    if (-not (Test-Path -LiteralPath $outputPath -PathType Leaf)) { throw 'OLS2 installed producer did not create its output.' }
    $output = Get-Content -LiteralPath $outputPath -Raw | ConvertFrom-Json
    if ([string]$output.state -ne 'PASS_JBOD_ENVIRONMENT_INVENTORY') { throw 'OLS2 output terminal state failed.' }
    if ([string]$output.workerSha256 -ne $targetWorkerSha256) { throw 'OLS2 output worker revision mismatch.' }
    if ([bool]$output.inventory.mutationsPerformed -or -not [bool]$output.inventory.pathsEnumerated -or [bool]$output.inventory.filesRead -or [bool]$output.inventory.imageBytesRead -or [string]$output.inventory.processLocalAlias.name -ne 'F' -or -not [bool]$output.inventory.processLocalAlias.created -or -not [bool]$output.inventory.processLocalAlias.removed -or [bool]$output.inventory.processLocalAlias.persistent) { throw 'OLS2 output violated read-only inventory boundaries.' }
    $subtree = $output.inventory.boundedSubtreeInventory
    $directories = @($subtree.directories)
    $bmpLeaves = @($subtree.bmpLeaves)
    if ([string]$output.inventory.schema -ne 'argos_project_portal_environment_inventory_v3' -or [string]$subtree.schema -ne 'argos_bounded_subtree_inventory_v1' -or [string]$subtree.state -ne 'COMPLETE' -or -not [bool]$subtree.complete -or [string]$subtree.relativeRoot -ne 'PatternedFront\Lot_62619-433' -or [bool]$subtree.truncated -or [int]$subtree.accessErrorCount -ne 0 -or [int]$subtree.skippedReparseSubtrees -ne 0 -or [int]$subtree.skippedUnsafePathSubtrees -ne 0 -or [int]$subtree.depthBoundaryDirectoryCount -ne 0) { throw 'OLS2 output bounded subtree inventory did not complete safely.' }
    if ($directories.Count -ne [int]$subtree.directoryCount -or $bmpLeaves.Count -ne [int]$subtree.bmpLeafCount) { throw 'OLS2 output bounded subtree row counts changed.' }
    if (@($directories | Where-Object { -not [bool]$_.containedByApprovedRoot -or -not ([string]$_.relativePath).StartsWith('PatternedFront\Lot_62619-433\',[StringComparison]::OrdinalIgnoreCase) -or -not ([string]$_.aliasReadPath).StartsWith('F:\PatternedFront\Lot_62619-433\',[StringComparison]::OrdinalIgnoreCase) -or [int]$_.canonicalEffectiveLength -ge 230 -or [int]$_.aliasEffectiveLength -ge 200 -or [bool]$_.reparsePoint -or [bool]$_.filesRead -or [bool]$_.imageBytesRead -or [bool]$_.sourceHashingPerformed -or [bool]$_.mutationsPerformed }).Count -ne 0) { throw 'OLS2 output bounded subtree directory-row safety contract failed.' }
    if (@($bmpLeaves | Where-Object { -not [bool]$_.containedByApprovedRoot -or -not ([string]$_.relativePath).StartsWith('PatternedFront\Lot_62619-433\',[StringComparison]::OrdinalIgnoreCase) -or -not ([string]$_.aliasReadPath).StartsWith('F:\PatternedFront\Lot_62619-433\',[StringComparison]::OrdinalIgnoreCase) -or [int]$_.canonicalEffectiveLength -ge 230 -or [int]$_.aliasEffectiveLength -ge 200 -or [string]$_.extension -ne '.bmp' -or [bool]$_.reparsePoint -or [bool]$_.filesRead -or [bool]$_.imageBytesRead -or [bool]$_.sourceHashingPerformed -or [bool]$_.mutationsPerformed }).Count -ne 0) { throw 'OLS2 output bounded subtree BMP-row safety contract failed.' }

    [ordered]@{
        schema = 'argos_ols2_entrypoint_result_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OCV00_BOUNDED_LOT_SUBTREE_INVENTORY_OLS2'
    ...
    if ($swapped) {
        ...
        if ((Get-ArgosFileHash -LiteralPath $workerPath -Algorithm SHA256).Hash -ne $priorWorkerSha256) { throw 'OLS2 rollback failed to restore the approved predecessor.' }
    }
    throw $failure
}
finally {
