...
if ([string]$manifest.schema -ne 'argos_ols2_route_invocation_v1') { throw 'OLS2 route invocation schema mismatch.' }
...
if ($paths.Count -lt 20 -or $paths.Count -gt 128 -or @($paths | Sort-Object -Unique).Count -ne $paths.Count) { throw 'OLS2 route path set is incomplete or duplicated.' }

$rows = New-Object Collections.Generic.List[object]
foreach ($path in $paths) {
    $one = & $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
    if ([string]$one.state -ne 'PASS_PATH_BUDGET' -or @($one.candidates).Count -ne 1) { throw "OLS2 route path budget failed: $path" }
...
$result = [ordered]@{
    schema = 'argos_ols2_complete_route_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = if ($Preflight) { 'PASS_OLS2_COMPLETE_ROUTE_PREFLIGHT' } else { 'PASS_OLS2_COMPLETE_ROUTE_GATE' }
...
foreach ($path in @($zipPath, $requestManifestPath, $requestSignaturePath)) { if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "OLS2 exact route artifact is missing: $path" } }
$result.requestZipSha256 = Get-Sha $zipPath
$result.requestManifestSha256 = Get-Sha $requestManifestPath
$result.requestSignatureSha256 = Get-Sha $requestSignaturePath
...
if (-not$result.exactFinalZipExtractionPassed -or -not$result.exactFinalZipSignaturePassed) { throw 'OLS2 exact final ZIP verification is not PASS.' }
$outputPath = [IO.Path]::GetFullPath([string]$manifest.outputPath)
if (Test-Path -LiteralPath $outputPath) { throw "OLS2 route gate output already exists: $outputPath" }
