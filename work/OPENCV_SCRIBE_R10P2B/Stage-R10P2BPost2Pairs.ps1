...
$outputRoot = 'D:\KLARFExport\R10P2B'
...
Assert-True ($env:COMPUTERNAME.Equals($expectedComputer, [StringComparison]::OrdinalIgnoreCase)) "R10P2B wrong computer: $($env:COMPUTERNAME)"
Assert-True (-not (Test-Path -LiteralPath $outputRoot)) "R10P2B create-new output root exists: $outputRoot"
$planned = New-Object Collections.Generic.List[object]
foreach ($item in $sources) {
    Assert-True (Test-Path -LiteralPath $item.source -PathType Leaf) "R10P2B exact source is absent: $($item.identity) $($item.channel)"
    Assert-True ((Get-Item -LiteralPath $item.source).Length -eq $sourceBytes) "R10P2B source byte count changed: $($item.identity) $($item.channel)"
    Assert-True ((Get-Sha256 $item.source) -eq [string]$item.sha256) "R10P2B source SHA-256 changed: $($item.identity) $($item.channel)"
    ...
    foreach ($path in @($partial, $destination)) {
        Assert-True (($path.Length + 32) -lt 200) "R10P2B unsafe output path: $path"
        Assert-True (-not (Test-Path -LiteralPath $path)) "R10P2B create-new output exists: $path"
    }
    $planned.Add([ordered]@{identity=$item.identity;channel=$item.channel;source=$item.source;sourceSha256=$item.sha256;sourceBytes=$sourceBytes;destination=$destination})
}
$processorBefore = @(Get-ProcessorRows)
if ($Preflight) {
    [ordered]@{schema='argos_r10p2b_post2_pair_stage_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R10P2B_POST2_PAIR_STAGE_PREFLIGHT';sourceCount=$sources.Count;sources=@($planned);processorProcessCount=$processorBefore.Count;sourceMutationPerformed=$false;sourceDeletionPerformed=$false;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
foreach ($item in $sources) {
    ...
    Assert-True ((Get-Item -LiteralPath $partial).Length -eq $sourceBytes) "R10P2B staged byte count changed: $($item.identity) $($item.channel)"
    Assert-True ((Get-Sha256 $partial) -eq [string]$item.sha256) "R10P2B staged SHA-256 changed: $($item.identity) $($item.channel)"
    Move-Item -LiteralPath $partial -Destination $destination -ErrorAction Stop
    $outputs.Add([ordered]@{identity=$item.identity;channel=$item.channel;destination=$destination;destinationSha256=Get-Sha256 $destination;destinationBytes=(Get-Item -LiteralPath $destination).Length})
}
$processorAfter = @(Get-ProcessorRows)
Assert-True ((($processorBefore | ConvertTo-Json -Compress) -eq ($processorAfter | ConvertTo-Json -Compress))) 'R10P2B protected processor identity changed.'
[ordered]@{schema='argos_r10p2b_post2_pair_stage_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R10P2B_POST2_PAIRS_STAGED_CREATE_NEW';sourceCount=$sources.Count;outputs=@($outputs);processorProcessCount=$processorAfter.Count;sourceMutationPerformed=$false;sourceDeletionPerformed=$false;taskOrProcessRestarted=$false;providerActivated=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Compress -Depth 8
