...
ENGINE_REVISION = "ARGOS_OPENCV_SCRIBE_V1R9_20260902"
...
def perimeter_scribe_regions(
        ...
        for offset in offsets:
            ...
            for delta in orientation_deltas:
                ...
                for channel_index, channel in enumerate(channels):
                    ...
                    rows.append((score, center[0], center[1], angle, float(perimeter_angle), channel_index))
    rows.sort(key=lambda item: (-item[0], item[4], item[3]))

    accepted: list[tuple[float, float, float, float, float, int]] = []
    pool_size = max(96, maximum_candidates * 4)
    channel_counts = [0, 0]
    minimum_separation = preview_width * 0.20
    for row in rows:
        if channel_counts[row[5]] >= pool_size:
            continue
        if any(
            ...
            continue
        accepted.append(row)
        channel_counts[row[5]] += 1
        if all(value >= pool_size for value in channel_counts):
            break
    regions: list[Region] = []
    ...
    for index, row in enumerate(accepted):
        ...
        shift = _perimeter_along_track_shift(
            _rectified_preview(
                channels[channel_index], center, row[3], preview_width, preview_height
            ),
            target_grid_width,
        )
        ...
        structure_score = _dot_matrix_structure_score(
            _rectified_preview(
                (bf, df)[channel_index],
                full_center,
                row[3],
        ...
        regions.append(Region(
            region_id=f"PERIMETER_{('BF', 'DF')[channel_index]}_{index:03d}",
            source=f"MULTISCALE_{('BF', 'DF')[channel_index]}_PERIMETER_SWEEP_REVIEW_ONLY_DEVELOPMENT",
    ...
    channel_regions = [
        sorted(
            [item for item in regions if f"PERIMETER_{label}_" in item.region_id],
            key=lambda item: (-item.localization_score, item.region_id),
        )
        for label in ("BF", "DF")
    ]
    balanced: list[Region] = []
    per_channel = max(1, maximum_candidates // 2)
    for channel_rows in channel_regions:
        for rank, item in enumerate(channel_rows[:per_channel]):
            balanced.append(Region(
                ...
                localization_score=float(1.0 - rank * 0.02 + item.localization_score * 0.001),
            ))
    balanced.sort(key=lambda item: (-item.localization_score, item.region_id))
    return balanced[:maximum_candidates]
