...
function New-Json([string]$Path,[object]$Value){Require (-not(Test-Path -LiteralPath $Path)) "O3F15L2 create-new JSON exists: $Path";[IO.File]::WriteAllText($Path,(($Value|ConvertTo-Json -Depth 20)+[Environment]::NewLine),(New-Object Text.UTF8Encoding($false)))}
...
function Invoke-Captured([string]$Executable,[string[]]$Arguments,[string]$WorkingDirectory){
    ...
    Require $process.Start() 'O3F15L2 rehearsal child did not start.'
    $stdout=$process.StandardOutput.ReadToEnd()
    $stderr=$process.StandardError.ReadToEnd()
    $process.WaitForExit()
    [pscustomobject]@{exitCode=[int]$process.ExitCode;stdout=$stdout;stderr=$stderr}
}
function Parse-OneJson([string]$Text,[string]$Label){$lines=@($Text-split'\r?\n'|Where-Object{-not[string]::IsNullOrWhiteSpace($_)});Require ($lines.Count-eq1) "O3F15L2 $Label stdout line count changed.";try{return($lines[0]|ConvertFrom-Json)}catch{throw "O3F15L2 $Label stdout is not JSON: $($_.Exception.Message)"}}
function New-RehearsalInvocation([string]$Path,[string]$Mode,[string]$Python,[string]$PythonHash,[string]$Base){
    New-Json $Path ([ordered]@{schema='argos_ocv03_o3f15l2_rehearsal_invocation_v1';fixtureMode=$Mode;pythonPath=$Python;pythonSha256=$PythonHash;runtimeRoot=(Join-Path $Base 'r');gateRoot=(Join-Path $Base 'g');corpusRoot=(Join-Path $Base 'c');mirrorRoot=(Join-Path $Base 'm')})
}
function New-MaintenanceFixture([string]$TemplateRoot,[string]$RequestRoot,[string]$CaseRequestId,[string]$Destination,[object]$Signer){
    Require (-not(Test-Path -LiteralPath $RequestRoot)) "O3F15L2 installer fixture request exists: $RequestRoot"
...
function Invoke-MaintenanceCase([string]$Name,[string]$TemplateRoot,[string]$CasesRoot,[string]$Carrier,[bool]$Approved,[object]$Signer,[string]$Worker,[string]$RequestVerifier,[string]$ResponseVerifier,[string]$PublicCertificate){
    ...
    Require ($responses.Count-eq1) "O3F15L2 installer fixture response cardinality changed: $Name"
    ...
    Require ($installLeaves.Count-eq1-and$installLeaves[0].FullName-eq$target) "O3F15L2 installer fixture wrote an undeclared install leaf: $Name"
    if($Approved){
        Require ([string]$response.state-eq'PASS_MAINTENANCE_PATCH'-and$afterSha-eq(Sha $Carrier)) "O3F15L2 approved installer case failed: $Name"
        $result=Get-Content -LiteralPath (Join-Path $responses[0].FullName 'RESULT.json') -Raw|ConvertFrom-Json
        Require ([string]$result.state-eq'PASS_MAINTENANCE_PATCH'-and[int]$result.changedFiles-eq1-and[bool]$result.changes[0].predecessorExisted-and[string]$result.changes[0].installedSha256-eq$afterSha) "O3F15L2 maintenance result contract changed: $Name"
    }else{
        Require ([string]$response.state-eq'FAILED'-and([string]$response.detail)-like'*Installed predecessor is not approved*'-and$afterSha-eq$beforeSha-and-not(Test-Path -LiteralPath (Join-Path $responses[0].FullName 'RESULT.json'))) 'O3F15L2 unapproved predecessor was not refused before install mutation.'
    }
...
Require ([string]$PSVersionTable.PSEdition-eq'Desktop'-and[int]$PSVersionTable.PSVersion.Major-eq5-and[int]$PSVersionTable.PSVersion.Minor-eq1) 'O3F15L2 final-ZIP rehearsal requires exact Windows PowerShell 5.1.'
...
$gatePath=Join-Path $PSScriptRoot 'O3F15L2_FINAL_ZIP_REHEARSAL_GATE.json'
foreach($path in @($signGatePath,$certificate,$packageTester,$responseVerifier,$signingIdentityPath,$endpointWorker,$queueGate,$python,$windowsPowerShell)){Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L2 final-ZIP dependency absent: $path"}
...
Require (Test-Path -LiteralPath $zip -PathType Leaf) 'O3F15L2 signed ZIP absent.'
Require ((Get-Item -LiteralPath $zip).Length-eq[int64]$signGate.packageZipBytes-and(Sha $zip)-eq$zipHash) 'O3F15L2 signed ZIP changed.'
Require ((Sha $python)-eq'7D96A4ED35D6E596CD9DD8933FEAFC66349CC21F75BE3B15C89FD336E50140C1') 'O3F15L2 local rehearsal runtime changed.'
Require ((Sha $certificate)-eq'2B434D0CF6A0D6D69AAE3D280032EFF00807D423CA3B3D7F2EFC1C6BB628BFFF'-and(Sha $packageTester)-eq'6CA21D7DE97EDE88F2C41F91D5B7801C688982C8C5A422987EB9528E1E9A084B'-and(Sha $responseVerifier)-eq'4AF5901A7B9DFFF5A4DAF128960173D67501ABF6FF87C586BA526643B1C1449C'-and(Sha $signingIdentityPath)-eq'3FD8164D1869375156FB7566D206FEAB97AE8A9E7D377B0AD4E03739ED697289') 'O3F15L2 signed-package installer rehearsal dependencies changed.'
Require ((Sha $endpointWorker)-eq'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250'-and(Sha $queueGate)-eq'170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D') 'O3F15L2 inherited endpoint worker or queue-safety evidence changed.'
foreach($path in @($fixtureRoot,$gatePath)){Require (-not(Test-Path -LiteralPath $path)) "O3F15L2 create-new final-ZIP rehearsal target exists: $path"}
$planned=@((Join-Path $extract 'payload\Invoke-O3F15L2.ps1'),$normalInvocation,$immediateInvocation,(Join-Path $normalBase 'r\Run-O3F15FrontReconcile.py'),(Join-Path $normalBase 'c\PROGRESS.json'),(Join-Path $normalBase 'm\PROGRESS.json'),(Join-Path $immediateBase 'c\PROGRESS.json'),(Join-Path $fixtureRoot 'installer\UNAPPROVED\state\maintenance\REQ_O3F15_IR_UNAPPROVED\failed_new\M000_0123456789_0123456789.rollback'),$gatePath)
...
Require ([string]$pathGate.state-eq'PASS_PATH_BUDGET') 'O3F15L2 exact-package path gate failed.'

if($Preflight){[ordered]@{schema='argos_ocv03_o3f15l2_final_zip_rehearsal_preflight_v1';state='PASS_O3F15L2_FINAL_ZIP_REHEARSAL_PREFLIGHT';requestId=$requestId;packageZipSha256=$zipHash;fixtureRoot=$fixtureRoot;pathState=[string]$pathGate.state;endpointWorkerSha256=Sha $endpointWorker;queueSafetyGateSha256=Sha $queueGate;installerCaseCount=3;targetOrJbodAccessed=$false;sourceImageBytesRead=$false;existingProcessesQueried=$false;existingProcessOrTaskActionPerformed=$false;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 8;return}
...
Require ([string]$packageTest.State-eq'PASS_SIGNED_PORTAL_PACKAGE') 'O3F15L2 exact package signature failed.'
...
Require ([string]$manifest.requestId-eq$requestId-and@($manifest.files).Count-eq[int]$signGate.payloadFileCount-and@($manifest.changes).Count-eq1-and[string]$manifest.entryPoint-eq'payload/Invoke-O3F15L2.ps1') 'O3F15L2 signed manifest identity changed.'
Require ([bool]$manifest.reviewOnly-and-not[bool]$manifest.trainingEligible-and-not[bool]$manifest.xmlEligible-and-not[bool]$manifest.productionEligible-and-not[bool]$manifest.productionRoutingEnabled-and-not[bool]$manifest.requestRetryAuthorized-and@($manifest.allowedTaskActions).Count-eq0) 'O3F15L2 signed authority widened.'
...
$actualOutputs=@($manifest.entryPointOutputs|ForEach-Object{Require (-not[bool]$_.requiredAtLaunch) 'O3F15L2 output was incorrectly declared launch-required.';[string]$_.path})
Require ($actualOutputs.Count-eq$expectedOutputs.Count-and@($actualOutputs|Sort-Object).Count-eq@($expectedOutputs|Sort-Object).Count-and(@(Compare-Object ($actualOutputs|Sort-Object) ($expectedOutputs|Sort-Object)).Count-eq0)) 'O3F15L2 declared result/mirror outputs changed.'
Require (@($manifest.allowedProcessActions).Count-eq2-and[int]$manifest.timeoutContract.bootstrapProgressSeconds-eq600-and[string]$manifest.sourceProcessingContract.terminalFailureSchema-eq'argos_ocv03_o3f15_terminal_failure_v1'-and[string]$manifest.sourceProcessingContract.terminalFailureState-eq'HOLD_O3F15_ARTIFACT_COMMIT_FAILURE') 'O3F15L2 signed terminal-failure/process/timeout contract changed.'
foreach($record in @($manifest.files)){
    $relative=[string]$record.path
    Require (-not[IO.Path]::IsPathRooted($relative)-and$relative-notmatch'(^|[\\/])\.\.([\\/]|$)') 'O3F15L2 signed payload path is unsafe.'
    $file=Join-Path $extract $relative
    Require (Test-Path -LiteralPath $file -PathType Leaf) "O3F15L2 signed payload file absent: $relative"
    Require ((Get-Item -LiteralPath $file).Length-eq[int64]$record.bytes-and(Sha $file)-eq[string]$record.sha256) "O3F15L2 signed payload hash changed: $relative"
}
$change=@($manifest.changes)[0]
$carrier=Join-Path $payload 'OCV03_NotchReviewOpenCvV1.py'
$carrierHash=Sha $carrier
Require ([string]$change.source-eq'payload/OCV03_NotchReviewOpenCvV1.py'-and[string]$change.installedSha256-eq$carrierHash-and@($change.approvedPredecessorSha256).Count-eq1-and@($change.approvedPredecessorSha256)-contains$carrierHash-and-not[bool]$change.allowCreate) 'O3F15L2 signed same-bytes carrier contract changed.'
...
Require ([bool]$signer.HasPrivateKey) 'O3F15L2 installer-rehearsal signing key is absent.'
...
Require ($installerCases.Count-eq3-and@($installerCases|Where-Object{[string]$_.endpointState-eq'PASS_MAINTENANCE_PATCH'}).Count-eq2-and@($installerCases|Where-Object{[bool]$_.refusedBeforeMutation}).Count-eq1) 'O3F15L2 exact maintenance-validation case set changed.'
$endpoint=Join-Path $payload 'Invoke-O3F15L2.ps1'
$contract=Get-Content -LiteralPath (Join-Path $payload 'O3F15L2_LAUNCH_CONTRACT.json') -Raw|ConvertFrom-Json
Require ([string]$contract.schema-eq'argos_ocv03_o3f15l2_launch_contract_v1'-and[string]$contract.state-eq'FROZEN_FOR_BUILD'-and[int]$contract.expectedPairCount-eq978-and[string]$contract.side-eq'FRONT'-and[string]$contract.expectedTerminalFailureSchema-eq'argos_ocv03_o3f15_terminal_failure_v1'-and[string]$contract.expectedTerminalFailureState-eq'HOLD_O3F15_ARTIFACT_COMMIT_FAILURE') 'O3F15L2 extracted launch contract changed.'
Require ([string]$contract.inheritedRoute.endpointWorkerSha256-eq'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250'-and[string]$contract.inheritedRoute.installedRouteConfigEvidenceSha256-eq'465107D861E8D2C376A419B1C15841BE3A5757C6498C36CA56B9A95A2F0B76DB'-and[string]$contract.inheritedRoute.queueSafetyGateSha256-eq'170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D'-and-not[bool]$contract.inheritedRoute.routeImplementationChanged) 'O3F15L2 extracted inherited route pins changed.'
...
Require ($leafProcess.exitCode-eq0-and[string]$leaf.state-eq'PASS_O3F15L2_EXACT_PACKAGED_LAUNCH_LEAVES'-and-not[bool]$leaf.mutationsPerformed) 'O3F15L2 exact packaged leaf preflight failed.'
...
Require ($testProcess.exitCode-eq0-and[string]::IsNullOrWhiteSpace($testProcess.stderr)-and[string]$focused.schema-eq'argos_ocv03_o3f15_front_reconcile_focused_gate_v1'-and[string]$focused.state-eq'PASS_O3F15_FRONT_RECONCILE_FOCUSED_GATE') 'O3F15L2 exact packaged isolated Python test failed.'
Require ([bool]$focused.sizeChecks.resultsUnder2MiB-and[bool]$focused.sizeChecks.progressUnder2MiB-and[bool]$focused.sizeChecks.allKnownOutputLeavesPathSafe-and[bool]$focused.sizeChecks.allKnownOutputComponentsSafe-and[bool]$focused.sizeChecks.fullCountNotTerminalBeforeCommit-and[bool]$focused.sizeChecks.terminalOnlyAfterCommit) 'O3F15L2 packaged terminal-commit/path checks changed.'
...
Require ($normalPreflightProcess.exitCode-eq0-and[string]$normalPreflight.schema-eq'argos_ocv03_o3f15l2_rehearsal_preflight_v1'-and[string]$normalPreflight.state-eq'PASS_O3F15L2_REHEARSAL_PREFLIGHT'-and-not[bool]$normalPreflight.processStarted-and-not[bool]$normalPreflight.mutationsPerformed) 'O3F15L2 NORMAL endpoint preflight failed.'
...
Require ($normalProcess.exitCode-eq0-and[string]$normal.schema-eq'argos_ocv03_o3f15l2_rehearsal_launch_v1'-and[string]$normal.state-eq'PASS_O3F15L2_REHEARSAL_WORKER_LAUNCHED'-and[string]$normal.fixtureMode-eq'NORMAL') 'O3F15L2 NORMAL endpoint rehearsal failed.'
Require ([string]$normal.focusedTestState-eq'PASS_O3F15_FRONT_RECONCILE_FOCUSED_GATE'-and[string]$normal.selfTestState-eq'PASS_O3F15_FRONT_RECONCILE_SELF_TEST'-and[string]$normal.preflightState-eq'PASS_O3F15_FRONT_RECONCILE_PREFLIGHT'-and[string]$normal.gateState-eq'COMPLETE_O3F15_GATE'-and[int]$normal.pid-gt0-and-not[string]::IsNullOrWhiteSpace([string]$normal.creationTimeUtc)-and[int]$normal.workerStillRunningAfterSeconds-eq3) 'O3F15L2 NORMAL child identity/stage evidence changed.'
Require ([string]$normal.runtimeRoot-eq(Join-Path $normalBase 'r')-and[string]$normal.gateRoot-eq(Join-Path $normalBase 'g')-and[string]$normal.corpusRoot-eq(Join-Path $normalBase 'c')-and[string]$normal.mirrorRoot-eq(Join-Path $normalBase 'm')) 'O3F15L2 NORMAL rehearsal roots changed.'
Require ([string]$normal.progressPath-eq(Join-Path $normalBase 'c\PROGRESS.json')-and[string]$normal.summaryPath-eq(Join-Path $normalBase 'c\SUMMARY.json')-and[string]$normal.mirrorProgressPath-eq(Join-Path $normalBase 'm\PROGRESS.json')-and[string]$normal.bootstrapProgressState-eq'RUNNING_O3F15_FULL978'-and[int]$normal.bootstrapScheduledCount-eq978) 'O3F15L2 NORMAL bootstrap progress evidence changed.'
Require (-not[bool]$normal.imageBytesRead-and-not[bool]$normal.existingProcessesQueried-and-not[bool]$normal.existingProcessOrTaskActionPerformed-and[bool]$normal.ownedProcessStarted-and[bool]$normal.mutationsPerformed-and[bool]$normal.reviewOnly) 'O3F15L2 NORMAL rehearsal safety evidence changed.'
...
Require ($collisionProcess.exitCode-ne0-and[string]$collision.schema-eq'argos_ocv03_o3f15l2_rehearsal_launch_v1'-and[string]$collision.state-eq'HOLD_O3F15L2_REHEARSAL_CREATE_NEW_COLLISION'-and-not[bool]$collision.ownedProcessStarted) 'O3F15L2 create-new collision was not refused before process start.'
