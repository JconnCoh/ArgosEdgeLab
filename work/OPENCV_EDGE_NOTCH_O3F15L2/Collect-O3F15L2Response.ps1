...
function Required([object]$Object,[string]$Name){$property=$Object.PSObject.Properties[$Name];if($null-eq$property){throw "O3F15L2 required property absent: $Name"};$property.Value}
...
foreach($path in @($InvocationManifest,$signGatePath,$publishGatePath,$verifier,$certificate)){Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L2 collection dependency absent: $path"}
...
Require ([string]$invocation.schema-eq'argos_ocv03_o3f15l2_response_collection_invocation_v1'-and[string]$invocation.state-eq'FROZEN_FOR_COLLECTION') 'O3F15L2 collection invocation is not frozen.'
...
Require ([string]$signGate.state-eq'PASS_O3F15L2_SIGNED_EXACT_978_FRONT_LAUNCH_PACKAGE'-and[string]$signGate.requestId-eq$requestId) 'O3F15L2 sign gate does not match collection invocation.'
Require ([string]$publishGate.state-eq'PASS_O3F15L2_PUBLISHED_EXACTLY_ONCE_AWAITING_SIGNED_LAUNCH_RESPONSE'-and[string]$publishGate.requestId-eq$requestId) 'O3F15L2 publish gate does not match collection invocation.'
...
Require ($sourceZip.StartsWith(($responses.TrimEnd('\')+'\'),[StringComparison]::OrdinalIgnoreCase)) 'O3F15L2 response ZIP is outside the qualified response root.'
Require (Test-Path -LiteralPath $sourceZip -PathType Leaf) 'O3F15L2 response ZIP absent.'
Require ((Get-Item -LiteralPath $sourceZip).Length-eq$zipBytes-and(Sha $sourceZip)-eq$zipHash) 'O3F15L2 response ZIP changed.'
Add-Type -AssemblyName System.IO.Compression.FileSystem
$archive=[IO.Compression.ZipFile]::OpenRead($sourceZip)
try{
    $entry=$archive.GetEntry('PORTAL_RESPONSE_MANIFEST.json')
    Require ($null-ne$entry-and$entry.Length-le65536) 'O3F15L2 response manifest absent or too large.'
...
Require ([string](Required $manifest 'requestId')-eq$requestId-and[string](Required $manifest 'responseId')-eq$responseId-and[string](Required $manifest 'state')-eq'PASS_MAINTENANCE_PATCH'-and[string](Required $manifest 'sourceRole')-eq'JBOD'-and[bool](Required $manifest 'reviewOnly')-and-not[bool](Required $manifest 'productionRoutingEnabled')) 'O3F15L2 terminal response manifest contract changed.'
$ready=Join-Path $PSScriptRoot 'o3f15l2_response'
$partial=$ready+'.partial'
$gatePath=Join-Path $PSScriptRoot 'O3F15L2_SIGNED_LAUNCH_GATE.json'
$resumePartial=Test-Path -LiteralPath $partial -PathType Container
foreach($path in @($ready,$gatePath)){Require (-not(Test-Path -LiteralPath $path)) "O3F15L2 create-new collection target exists: $path"}
if($resumePartial){
    $partialManifestPath=Join-Path $partial 'PORTAL_RESPONSE_MANIFEST.json'
    Require (Test-Path -LiteralPath $partialManifestPath -PathType Leaf) 'O3F15L2 resumable partial response manifest is absent.'
    Require ((Sha $partialManifestPath)-eq$sourceManifestSha256) 'O3F15L2 partial response is not the exact pinned source response attempt.'
    $partialManifest=Get-Content -LiteralPath $partialManifestPath -Raw|ConvertFrom-Json
    Require ([string](Required $partialManifest 'requestId')-eq$requestId-and[string](Required $partialManifest 'responseId')-eq$responseId) 'O3F15L2 partial response identity changed.'
}

if($Preflight){[ordered]@{schema='argos_ocv03_o3f15l2_response_collection_preflight_v1';state='PASS_O3F15L2_RESPONSE_COLLECTION_PREFLIGHT';requestId=$requestId;responseId=$responseId;sourceZipBytes=$zipBytes;sourceZipSha256=$zipHash;sourceManifestSha256=$sourceManifestSha256;exactPartialResume=$resumePartial;partialManifestExact=$(if($resumePartial){$true}else{$null});mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 8;return}
...
Require ((Sha (Join-Path $partial 'PORTAL_RESPONSE_MANIFEST.json'))-eq$sourceManifestSha256) 'O3F15L2 extracted response manifest changed from the pinned source ZIP.'
