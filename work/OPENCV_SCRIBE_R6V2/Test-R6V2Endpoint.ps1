...
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 24) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "R6V2 test create-new JSON exists: $Path"
...
$gatePath = Join-Path $PSScriptRoot 'R6V2_ENTRYPOINT_TEST_GATE_R2.json'
$expectedEngineSha = '1E4C55AA4ECFB4DA3CEA9DF577DCFE86C4A2FADCB23D29F78719F3E0AC55E0E9'
$expectedConfigurationSha = 'C5343C53E94EB2297FBE0637D13E9684D1C11CC3134E856FACE5C57450CB3C92'
...
foreach ($path in @($endpoint,$engine,$configuration,$batchSource,$bundle,(Join-Path $runtime 'python.exe'),$sample)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "R6V2 test dependency absent: $path"
}
Assert-True ((Get-Sha256 $engine) -eq $expectedEngineSha) 'R6V2 test engine changed.'
Assert-True ((Get-Sha256 $configuration) -eq $expectedConfigurationSha) 'R6V2 test configuration changed.'
Assert-True ((Get-Sha256 $bundle) -eq $expectedBundleSha) 'R6V2 test reference bundle changed.'
Assert-True ((Get-Sha256 $sample) -eq $expectedSampleSha) 'R6V2 test sample changed.'
foreach ($path in @($root,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R6V2 test target exists: $path" }

if ($Preflight) {
    [ordered]@{schema='argos_r6v2_endpoint_test_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R6V2_ENDPOINT_TEST_PREFLIGHT';endpointSha256=Get-Sha256 $endpoint;engineSha256=$expectedEngineSha;configurationSha256=$expectedConfigurationSha;bundleSha256=$expectedBundleSha;sampleSha256=$expectedSampleSha;plannedCaseCount=4;mutationsPerformed=$false;processStarted=$false;sourceImageBytesRead=$false;providerActivated=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
try {
    ...
    Copy-Item -LiteralPath $configuration -Destination (Join-Path $payload 'R6V2_CONFIGURATION.json')
    ...
    Assert-True ([string]$endpointPreflight.state -eq 'PASS_R6V2_SCRIBE_BATCH_PREFLIGHT' -and [int]$endpointPreflight.caseCount -eq 4 -and -not [bool]$endpointPreflight.mutationsPerformed) 'R6V2 exact endpoint preflight changed.'
    ...
    Assert-True ([string]$result.state -eq 'PASS_R6V2_REAL_IMAGE_REVIEW_ONLY_BATCH' -and [int]$result.caseCount -eq 4 -and [int]$result.identityEligibleCount -eq 0) 'R6V2 normal four-case rehearsal changed.'
    Assert-True (@($result.results).Count -eq 4 -and @($result.results | Where-Object { @($_.holds | Where-Object { [string]$_.code -eq 'SCRIBE_REFERENCE_COVERAGE_HOLD' }).Count -eq 1 }).Count -eq 4) 'R6V2 normal reference holds changed.'
    Assert-True ([bool]$result.sourceAliasRemoved -and -not (Test-Path -LiteralPath 'X:\')) 'R6V2 normal alias cleanup changed.'
    ...
    Copy-Item -LiteralPath $configuration -Destination (Join-Path $failure 'R6V2_CONFIGURATION.json')
    ...
    Assert-True ($caught -and -not (Test-Path -LiteralPath (Join-Path $failure 'w')) -and -not (Test-Path -LiteralPath (Join-Path $failure 'o')) -and -not (Test-Path -LiteralPath 'X:\')) 'R6V2 injected source-hash failure did not fail before write with alias cleanup.'

    $gate = [ordered]@{schema='argos_r6v2_entrypoint_test_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R6V2_ENTRYPOINT_TEST_GATE_R2';endpointSha256=Get-Sha256 $endpoint;engineSha256=$expectedEngineSha;configurationSha256=$expectedConfigurationSha;minimumObservedHeightRatio=0.2061033678437273;bundleSha256=$expectedBundleSha;caseCount=4;identityEligibleCount=0;referenceCoverageHoldCount=4;serializedProviderChildren=$true;automaticRetryAllowed=$false;sourceAliasRemovedOnSuccess=$true;sourceHashMismatchFailedBeforeWrite=$true;sourceAliasRemovedOnFailure=$true;processorIdentityUnchanged=$true;taskOrProcessRestarted=$false;sourceMutationPerformed=$false;sourceDeletionPerformed=$false;waferActionPerformed=$false;holdsCleared=$false;providerActivated=$false;reviewOnly=$true;trainingEligible=$false;xmlEligible=$false;productionEligible=$false}
    ...
    if (Test-Path -LiteralPath $root) {
        ...
        Assert-True ($resolved -eq 'C:\R6V2T') 'R6V2 test cleanup root changed.'
