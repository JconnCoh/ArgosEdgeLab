...
function Assert-Pin([string]$Path, [string]$Sha256, [string]$RequiredState = '') {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "R6V2 build dependency absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "R6V2 build dependency changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($RequiredState)) {
        $value = Get-Content -Raw -LiteralPath $Path | ConvertFrom-Json
        Assert-True ([string]$value.state -eq $RequiredState) "R6V2 build dependency state changed: $Path"
    }
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 32) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "R6V2 build create-new JSON exists: $Path"
...
$endpointPath = Join-Path $root 'Invoke-R6V2ScribeBatch.ps1'
$enginePath = Join-Path $project 'work\OPENCV_SCRIBE_V1R6\ArgosOpenCvScribeV1R6.py'
$configurationPath = Join-Path $root 'R6V2_CONFIGURATION.json'
...
$preactionPath = Join-Path $root 'PREACTION_BUILD_SIGN_R6V2.json'
$testGatePath = Join-Path $root 'R6V2_ENTRYPOINT_TEST_GATE_R2.json'
$selfPinGatePath = Join-Path $root 'R6V2_SELF_PIN_GATE.json'
...
$gatePath = Join-Path $root 'R6V2_FINAL_PACKAGE_GATE.json'
...
$endpointSha = 'C28E25F19BA28B455913560AB37B86EBC51009E8ECC166FBAD5443A45A29E897'
...
$jobShas = @('61E89F7B55606314267D2A74887E747FE212B107937D6445B6B2052E547F4405','EA1F500F0B8F5ADF5C598C431547E8DD10EBF41E0DC0A755C27476F853D712E5','2D809A00A589CA056C523498C2111015B67398BEBEC6DBF11C770C05FC60B5E0','1C916CFE42D480E673DFA9348F7B445BEE6A7573A2B8457ECE2104527587C431')
$definitionSha = '0BDA1E537294FFF15C89D3C470C0888657BC975CF316F24A53A58B1C2E034C30'
$preactionSha = '1751F5F02F2526B2818F9020A025CE825ABB87B67CC53186043080338622DD6F'
$testGateSha = 'F6FAEA24F31DE229B3061D0E38C92F9C3D49B0D4A796C4C24DEEDB578A34EAC5'
$selfPinGateSha = '9DBEB105E44DB39A2A3B06B70C356EA01E58D563AE7EF9B0E619B714DB16D253'
...
Assert-Pin $testGatePath $testGateSha 'PASS_R6V2_ENTRYPOINT_TEST_GATE_R2'
Assert-Pin $selfPinGatePath $selfPinGateSha 'PASS_R6V2_SELF_PIN_AND_LIVE_BRANCH_GATE'
...
Assert-True ([string]$preaction.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'R6V2 build preaction changed.'
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH' -and [string]$definition.entryPoint -eq 'payload/Invoke-R6V2ScribeBatch.ps1') 'R6V2 definition route changed.'
Assert-True (@($definition.changes).Count -eq 1 -and [string]$definition.changes[0].installedSha256 -eq $endpointSha -and [bool]$definition.changes[0].allowCreate) 'R6V2 maintenance change changed.'
Assert-True (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R6V2 action cardinality changed.'
Assert-True ([int]$definition.timeoutContract.endpointWorkerOuterTimeoutSeconds -gt ([int]$definition.timeoutContract.opencvChildTimeoutSeconds * [int]$definition.timeoutContract.maximumSequentialChildren)) 'R6V2 outer timeout does not cover all children.'
Assert-True ([string]$definition.rehearsal.gateSha256 -eq $testGateSha -and [string]$definition.rehearsal.selfPinGateSha256 -eq $selfPinGateSha) 'R6V2 declared rehearsal evidence changed.'
Assert-True (-not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed -and -not [bool]$definition.sourceProcessingContract.holdClearanceAllowed -and -not [bool]$definition.sourceProcessingContract.providerActivationAllowed) 'R6V2 processing authority changed.'
foreach ($path in @($signedRoot,$partialSigned,$finalRoot,$partialFinal,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R6V2 fresh output exists: $path" }

$planned = @($readyRoot,(Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.json'),(Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.sig'),$zipPath,(Join-Path $partialFinal 'extract\payload\Invoke-R6V2ScribeBatch.ps1'),$gatePath,'C:\ProgramData\ArgosProjectPortalRO\endpoint_jbod\state\work\J_0123456789_00000000\package\payload\Invoke-R6V2ScribeBatch.ps1','D:\A2\w\ocv\R6V2A.partial\refs\glyphs_v5_confirmed_20260806\028_62630_456_SLOT25_P04_S.png','D:\A2\o\ocv\R6V2A\Slot25\RESULT.json')
...
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET' -and @($pathGate.candidates).Count -eq $planned.Count) 'R6V2 build path gate changed.'
...
try {
    ...
    Assert-True ($certificateMatches.Count -eq 1 -and $certificateMatches[0].HasPrivateKey) 'R6V2 signer certificate changed or private key is absent.'
    $certificate = $certificateMatches[0]
}
finally { $store.Close(); $store.Dispose() }

if ($Preflight) {
    [ordered]@{schema='argos_r6v2_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R6V2_BUILD_PREFLIGHT';requestId=$requestId;payloadFileCount=8;endpointSha256=$endpointSha;engineSha256=$engineSha;configurationSha256=$configurationSha;batchSha256=$batchSha;jobCount=4;definitionSha256=$definitionSha;pathState=[string]$pathGate.state;maximumEffectiveLength=$maximumEffectiveLength;signerThumbprint=$thumbprint;mutationsPerformed=$false;targetExecuted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}

$files = New-Object Collections.Generic.List[object]
$files.Add([ordered]@{source=$endpointPath;path='payload/Invoke-R6V2ScribeBatch.ps1';bytes=[int64](Get-Item -LiteralPath $endpointPath).Length;sha256=$endpointSha})
...
$files.Add([ordered]@{source=$configurationPath;path='payload/R6V2_CONFIGURATION.json';bytes=[int64](Get-Item -LiteralPath $configurationPath).Length;sha256=$configurationSha})
...
Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R6V2 signed package verification failed.'
...
$expected = @{'payload/Invoke-R6V2ScribeBatch.ps1'=$endpointSha;'payload/ArgosOpenCvScribeV1R6.py'=$engineSha;'payload/R6V2_CONFIGURATION.json'=$configurationSha;'payload/BATCH.json'=$batchSha;'payload/S22.json'=$jobShas[0];'payload/S23.json'=$jobShas[1];'payload/S24.json'=$jobShas[2];'payload/S25.json'=$jobShas[3];'PORTAL_REQUEST_MANIFEST.json'=Get-Sha256 (Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.json');'PORTAL_REQUEST_MANIFEST.sig'=Get-Sha256 (Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.sig')}
$extracted = @(Get-ChildItem -LiteralPath $extractRoot -Recurse -File)
Assert-True ($extracted.Count -eq 10) 'R6V2 final ZIP file count changed.'
foreach ($item in $expected.GetEnumerator()) {
    $path = Join-Path $extractRoot $item.Key.Replace('/', '\')
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "R6V2 final ZIP leaf absent: $($item.Key)"
    Assert-True ((Get-Sha256 $path) -eq [string]$item.Value) "R6V2 final ZIP leaf changed: $($item.Key)"
}
$extractedManifest = Get-Content -Raw -LiteralPath (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json') | ConvertFrom-Json
Assert-True (@($extractedManifest.files).Count -eq 8 -and [string]$extractedManifest.changes[0].installedSha256 -eq $endpointSha) 'R6V2 extracted manifest payload/change contract changed.'
Assert-True ([string]$extractedManifest.rehearsal.gateSha256 -eq $testGateSha -and [string]$extractedManifest.rehearsal.selfPinGateSha256 -eq $selfPinGateSha) 'R6V2 extracted manifest gate pins changed.'
$gate = [ordered]@{schema='argos_r6v2_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R6V2_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip=('work/OPENCV_SCRIBE_R6V2/final/'+$zipName);requestZipBytes=[int64](Get-Item -LiteralPath $partialZip).Length;requestZipSha256=Get-Sha256 $partialZip;requestManifestSha256=$expected['PORTAL_REQUEST_MANIFEST.json'];requestSignatureSha256=$expected['PORTAL_REQUEST_MANIFEST.sig'];maintenanceDefinitionSha256=$definitionSha;endpointSha256=$endpointSha;engineSha256=$engineSha;configurationSha256=$configurationSha;minimumObservedHeightRatio=0.2061033678437273;batchSha256=$batchSha;jobSha256=$jobShas;payloadFileCount=8;exactFinalZipExtractionPassed=$true;exactFinalZipSignaturePassed=$true;maintenanceInstalledShaMatchesPayload=$true;entrypointTestGateSha256=$testGateSha;selfPinGateSha256=$selfPinGateSha;buildPreactionSha256=$preactionSha;pathState=[string]$pathGate.state;maximumEffectiveLength=$maximumEffectiveLength;caseCount=4;maximumConcurrentProviderChildren=1;automaticIdentityAuthority=$false;maximumPublications=1;retryAuthorized=$false;publicationRequiresCompleteRouteGate=$true;publicationAuthorized=$false;targetExecuted=$false;sourceImageBytesRead=$false;providerActivated=$false;reviewOnly=$true;trainingEligible=$false;xmlEligible=$false;productionEligible=$false}
