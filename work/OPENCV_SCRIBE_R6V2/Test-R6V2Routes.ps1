...
function Add-UniquePath([Collections.Generic.List[string]]$List, [Collections.Generic.HashSet[string]]$Set, [string]$Path) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($Path)) 'R6V2 route path is empty.'
    Assert-True ($Set.Add($Path)) "R6V2 duplicate route path: $Path"
    $List.Add($Path)
}
...
$finalGatePath = Join-Path $root 'R6V2_FINAL_PACKAGE_GATE.json'
$zipPath = Join-Path $root ('final\' + $requestReadyName + '.zip')
$extractRoot = Join-Path $root 'final\extract'
$outputPath = Join-Path $root 'R6V2_PATH_ROUTE_GATE.json'
...
foreach ($dependency in @($finalGatePath,$zipPath,(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json'),(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig'),$pathTool,$packageTester,$publicCertificate,$queueGatePath,$inheritedRoutePath)) {
    Assert-True (Test-Path -LiteralPath $dependency -PathType Leaf) "R6V2 route dependency absent: $dependency"
}
Assert-True ((Get-Sha256 $finalGatePath) -eq 'AF461D346CF6A6FAFD9B3B63FF6F13E0DDB9EF84DEF0E3E1871D739D0B8545BA') 'R6V2 final package gate changed.'
Assert-True ((Get-Sha256 $queueGatePath) -eq '170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D') 'R6V2 inherited queue-safety gate changed.'
Assert-True ((Get-Sha256 $inheritedRoutePath) -eq 'D161C4891688D6A76F7AD84EBBC935B7D14488CF21D7CF91E6F9944197B8706E') 'R6V2 inherited route-root evidence changed.'
$finalGate = Get-Content -Raw -LiteralPath $finalGatePath | ConvertFrom-Json
Assert-True ([string]$finalGate.state -eq 'PASS_R6V2_FINAL_PACKAGE_GATE' -and [string]$finalGate.requestId -eq $requestId -and -not [bool]$finalGate.publicationAuthorized) 'R6V2 final package identity/authority changed.'
Assert-True ((Get-Sha256 $zipPath) -eq [string]$finalGate.requestZipSha256) 'R6V2 final ZIP changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json')) -eq [string]$finalGate.requestManifestSha256) 'R6V2 extracted manifest changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig')) -eq [string]$finalGate.requestSignatureSha256) 'R6V2 extracted signature changed.'
...
Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R6V2 exact signed package verification failed.'
$manifest = Get-Content -Raw -LiteralPath (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json') | ConvertFrom-Json
Assert-True ([string]$manifest.requestId -eq $requestId -and [int64]$manifest.maxResultBytes -eq 16777216 -and @($manifest.files).Count -eq 8) 'R6V2 manifest route/result bound changed.'
Assert-True ([int]$manifest.timeoutContract.maximumSequentialChildren -eq 4 -and [int]$manifest.timeoutContract.endpointWorkerOuterTimeoutSeconds -eq 3000) 'R6V2 timeout contract changed.'
Assert-True (@($manifest.allowedTaskActions).Count -eq 0 -and @($manifest.allowedProcessActions).Count -eq 1) 'R6V2 action contract changed.'

$requestLeaves = @('PORTAL_REQUEST_MANIFEST.json','PORTAL_REQUEST_MANIFEST.sig','payload\Invoke-R6V2ScribeBatch.ps1','payload\ArgosOpenCvScribeV1R6.py','payload\R6V2_CONFIGURATION.json','payload\BATCH.json','payload\S22.json','payload\S23.json','payload\S24.json','payload\S25.json')
...
foreach ($path in @(
    ...
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R6V2.ps1',
    'D:\A2\w\ocv\R6V2A.partial\refs\PORTABLE_GLYPH_REFERENCE_MANIFEST.json',
    'D:\A2\w\ocv\R6V2A\ArgosOpenCvScribeV1R6.py','D:\A2\w\ocv\R6V2A\BATCH.json','D:\A2\w\ocv\R6V2A\S25.json',
    'D:\A2\w\ocv\R6V2A\refs\glyphs_v5_confirmed_20260806\028_62630_456_SLOT25_P04_S.png',
    'D:\A2\o\ocv\R6V2A\Slot22\RESULT.json','D:\A2\o\ocv\R6V2A\Slot23\RESULT.json','D:\A2\o\ocv\R6V2A\Slot24\RESULT.json','D:\A2\o\ocv\R6V2A\Slot25\RESULT.json','D:\A2\o\ocv\R6V2A\BATCH_GATE.json','D:\A2\o\ocv\R6V2A\EXECUTION.json'
...
Assert-True ($paths.Count -ge 170 -and $paths.Count -le 200) "R6V2 route path cardinality changed: $($paths.Count)"
$rows = New-Object Collections.Generic.List[object]
foreach ($path in $paths) {
    $checked = & $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
    Assert-True ([string]$checked.state -eq 'PASS_PATH_BUDGET' -and @($checked.candidates).Count -eq 1) "R6V2 route path budget failed: $path"
