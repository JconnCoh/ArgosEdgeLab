...
Assert-True ((Get-Sha256 $definitionPath) -eq $definitionSha256) 'R17A definition changed.'
...
Assert-True (Test-Path -LiteralPath $publishGatePath -PathType Leaf) 'R17A has not been published.'
Assert-True (Test-Path -LiteralPath $routeGatePath -PathType Leaf) 'R17A route gate is missing.'
...
Assert-True ([string]$publishGate.state -eq 'PASS_R17A_EXACT_SIGNED_DATA_PULL_PUBLISHED_CREATE_NEW' -and [string]$publishGate.requestId -eq $requestId -and -not [bool]$publishGate.retryAuthorized) 'R17A publication evidence changed.'
Assert-True ([string]$routeGate.state -eq 'PASS_R17A_COMPLETE_ROUTE_GATE' -and [int]$routeGate.maximumEffectiveLength -lt 200) 'R17A route gate changed.'
...
Assert-True ($matches.Count -le 1) 'Multiple terminal responses match R17A.'
if ($matches.Count -eq 0) {
    if ($Collect) { throw 'No matching signed terminal response exists yet.' }
    [ordered]@{schema='argos_r17a_response_collection_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='WAIT_R17A_MATCHING_RESPONSE';requestId=$requestId;responseFilesScanned=$responseFiles.Count;matchingResponses=0;mutationsPerformed=$false;requestRetried=$false;pixelsDecoded=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}

$source = [string]$matches[0].file.FullName
$sourceZipBytes = [int64]$matches[0].file.Length
Assert-True ($sourceZipBytes -le 50331648) 'R17A response ZIP exceeds the signed bound.'
$sourceZipSha256 = Get-Sha256 $source
$zip = [IO.Compression.ZipFile]::OpenRead($source)
try {
    $names = @($zip.Entries | ForEach-Object { $_.FullName })
    Assert-True ($names.Count -eq 4 -and @(Compare-Object -ReferenceObject @('DATA_PULL_PAYLOAD.zip','PORTAL_RESPONSE_MANIFEST.json','PORTAL_RESPONSE_MANIFEST.sig','RESULT.json') -DifferenceObject $names).Count -eq 0) 'R17A response entry set changed.'
...
Assert-True ($signatureValid -and $certificate.Thumbprint.ToUpperInvariant() -eq $expectedSignerThumbprint) 'R17A response signature failed.'
...
Assert-True ([string]$result.schema -eq 'argos_project_portal_data_pull_result_v2' -and [string]$result.state -eq 'PASS_DATA_PULL' -and [string]$result.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'R17A DATA_PULL result changed.'
Assert-True ([string]$result.container -eq 'DATA_PULL_PAYLOAD.zip' -and [string]$result.containerSha256 -eq $payloadSha256 -and [int64]$result.containerBytes -eq $payloadBytes.Length) 'R17A payload container pin changed.'
Assert-True ($resultFiles.Count -eq 24 -and @(Compare-Object -ReferenceObject $expectedRelativePaths -DifferenceObject $observedRelativePaths).Count -eq 0 -and @(Compare-Object -ReferenceObject $expectedEntryPaths -DifferenceObject $observedEntryPaths).Count -eq 0) 'R17A exact returned file set changed.'
Assert-True ([int64]$result.totalSourceBytes -le 50331648 -and [bool]$result.sourcePathsPreservedAsZipEntries -and [bool]$result.filesystemReturnPathsFlattened) 'R17A returned path/byte contract changed.'
...
Assert-True ([string]$responseTest.State -eq 'PASS_SIGNED_PORTAL_RESPONSE' -and [string]$responseTest.EndpointState -eq 'PASS_DATA_PULL') 'R17A extracted signed response verification failed.'
...
Assert-True ($diskFiles.Count -eq 24) 'R17A extracted payload count changed.'
foreach ($row in $resultFiles) {
    $diskPath = Join-Path $payloadExtractRoot ([string]$row.entryPath).Replace('/', '\')
    Assert-True (Test-Path -LiteralPath $diskPath -PathType Leaf) "R17A extracted file missing: $($row.entryPath)"
    Assert-True ([int64](Get-Item -LiteralPath $diskPath).Length -eq [int64]$row.bytes -and (Get-Sha256 $diskPath) -eq [string]$row.sha256) "R17A extracted file changed: $($row.entryPath)"
}
