#!/usr/bin/env python3
"""No-image fixture for the exact O3F10 endpoint orchestration rehearsal."""
...
def fail_if(stage: str) -> None:
    if os.environ.get("ARGOS_O3F10_FIXTURE_FAIL_STAGE", "").upper() == stage:
        raise RuntimeError(f"INJECTED_O3F10_{stage}_FAILURE")


def gate(root: Path) -> dict[str, object]:
    fail_if("GATE")
    summary = {
        "schema": "argos_ocv03_o3f10_gate_result_v1",
...
def dev6(root: Path, prerequisite: Path, expected_sha256: str) -> dict[str, object]:
    ...
    for index in range(1, 7):
        ...
        manifest = {"schema": "argos_ocv03_full_perimeter_topology_manifest_v2", "revision": f"O3F10_FIXTURE_{index:04d}", "results": [observed]}
    ...
    summary = {
        "schema": "argos_ocv03_o3f10_staged_result_v1",
