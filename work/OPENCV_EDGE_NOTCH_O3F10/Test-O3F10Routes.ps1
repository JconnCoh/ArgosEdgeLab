...
function Write-O3F10Json([string]$Path, [object]$Value) { Assert-O3F10 (-not (Test-Path -LiteralPath $Path)) "O3F10 route gate exists: $Path"; [IO.File]::WriteAllText($Path, (($Value | ConvertTo-Json -Depth 12) + [Environment]::NewLine), (New-Object Text.UTF8Encoding($false))) }
...
$outputPath = Join-Path (Join-Path $PSScriptRoot 'final_o3f10') 'O3F10_PREPUBLICATION_PATH_GATE.json'
...
foreach ($path in @(
    (Join-Path $project "work/OPENCV_EDGE_NOTCH_O3F10/final_o3f10/$zipName"),
    ...
    'D:/O3F10G1/SUMMARY.json.partial', 'D:/O3F10G1.failed/SUMMARY.json',
    'D:/O3F10D1/cases/C0006/0123456789abcdef_df_r10_recovery_enhanced.png', 'D:/O3F10D1/SUMMARY.json.partial', 'D:/O3F10D1.failed/SUMMARY.json',
...
Write-O3F10Json $outputPath $result
