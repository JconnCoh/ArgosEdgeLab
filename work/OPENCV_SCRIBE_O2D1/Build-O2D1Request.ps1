...
function Assert-Pin([string]$LiteralPath, [string]$Sha256, [string]$RequiredState = '') {
    if (-not (Test-Path -LiteralPath $LiteralPath -PathType Leaf) -or (Get-Sha256 $LiteralPath) -ne $Sha256) { throw "O2D1 pinned dependency changed: $LiteralPath" }
    if (-not [string]::IsNullOrWhiteSpace($RequiredState)) {
        $value = Get-Content -Raw -LiteralPath $LiteralPath | ConvertFrom-Json
        if ([string]$value.state -ne $RequiredState) { throw "O2D1 pinned dependency state changed: $LiteralPath" }
    }
}
...
$root = $PSScriptRoot
$requestId = 'REQ_O2D1'
$endpointPath = Join-Path $root 'Invoke-O2D1ScribeEndpoint.ps1'
$enginePath = Join-Path $project 'work\OPENCV_SCRIBE_V1\ArgosOpenCvScribeV1.py'
$bundlePath = Join-Path $root 'O2D1_REFS.zip'
$jobPath = Join-Path $root 'O2D1_SLOT16_JOB.json'
...
$gatePath = Join-Path $root 'O2D1_FINAL_PACKAGE_GATE.json'
...
$endpointSha = 'A23CAE49FE255C7212F3765CBDE472FAF1CF38326A834A559EC6C298966B0A16'
$engineSha = '0756EC2FA9279AA7AF915BC3F6D8B2810AC1268A64FC626233FAFE3DC5DDB53D'
$bundleSha = '56DF00E37A195E7BC3E026E4950DFB5A0AA7E7AF49A6FF39020B071840CCFBD6'
$jobSha = '3056646A019D155908A125503E17E024FD0DA74AB744CB2F31502197AE14BD9B'
$definitionSha = 'AB3B4759090F5CAC9DBC2FE1696CF1D2A5B17AB0B1163D1899F0A0C76492101F'
...
Assert-Pin (Join-Path $root 'O2D1_REFS_GATE.json') '9C4B7BB6770226FE512D395C8B77DADBAB648503A83432BCD66F937C1229D2DC' 'PASS_O2D1_REFERENCE_BUNDLE_GATE'
Assert-Pin (Join-Path $root 'O2D1_ENTRYPOINT_TEST_GATE_R2.json') 'BBEAC7C099F42AF4AF4232360653112E232CF318D1EB384925DA2DCD3DED5C6A' 'PASS_O2D1_ENTRYPOINT_TEST_GATE_R2'
Assert-Pin (Join-Path $root 'O2D1_R2_ALIAS_POSTCONDITION_GATE.json') 'D23FF4EF73CE96E6B4B12229DF10312718CE7476FD74B3B151C28DB227DA831A' 'PASS_O2D1_REHEARSAL_ALIAS_REMOVED'
...
if ([string]$definition.targetRole -ne 'JBOD' -or [string]$definition.jobClass -ne 'MAINTENANCE_PATCH' -or [string]$definition.entryPoint -ne 'payload/Invoke-O2D1ScribeEndpoint.ps1' -or @($definition.changes).Count -ne 1 -or @($definition.entryPointMutations).Count -ne 2 -or @($definition.entryPointOutputs).Count -ne 2 -or @($definition.allowedTaskActions).Count -ne 0 -or @($definition.allowedProcessActions).Count -ne 1 -or [string]$definition.allowedProcessActions[0] -ne 'START_BOUNDED_PORTABLE_OPENCV_SCRIBE_DEVELOPMENT_SLOT16' -or -not [bool]$definition.reviewOnly -or [bool]$definition.productionRoutingEnabled) { throw 'O2D1 maintenance definition contract changed.' }
if ([string]$definition.changes[0].installedSha256 -ne $endpointSha -or -not [bool]$definition.changes[0].allowCreate -or [string]$definition.entryPointMutations[0].targetRoot -ne 'D:\A2\w\ocv\O2D1' -or [string]$definition.entryPointMutations[1].targetRoot -ne 'D:\A2\o\ocv\O2D1' -or [string]$definition.sourceProcessingContract.lotId -ne '62619-433' -or [string]$definition.sourceProcessingContract.slotId -ne 'Slot16' -or -not [bool]$definition.sourceProcessingContract.opencvPixelDecodeRequired -or -not [bool]$definition.sourceProcessingContract.boundedWholeImageScribeSearchRequired -or [bool]$definition.sourceProcessingContract.sourceMutationAllowed -or [bool]$definition.sourceProcessingContract.sourceDeletionAllowed -or [bool]$definition.sourceProcessingContract.holdClearanceAllowed -or [bool]$definition.sourceProcessingContract.providerActivationAllowed) { throw 'O2D1 exact processing boundary changed.' }
foreach ($path in @($signedRoot, $partialSigned, $finalRoot, $partialFinal, $gatePath)) { if (Test-Path -LiteralPath $path) { throw "O2D1 fresh output exists: $path" } }

$planned = @(
    $readyRoot,
    (Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.json'),
    (Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.sig'),
    (Join-Path $readyRoot 'payload\Invoke-O2D1ScribeEndpoint.ps1'),
    (Join-Path $readyRoot 'payload\ArgosOpenCvScribeV1.py'),
    (Join-Path $readyRoot 'payload\O2D1_REFS.zip'),
    (Join-Path $readyRoot 'payload\O2D1_SLOT16_JOB.json'),
    $zipPath,
    (Join-Path $partialFinal 'extract\payload\O2D1_REFS.zip'),
    $gatePath,
    'C:\O2D1T3\w',
    'C:\O2D1T3\o'
)
$pathGate = & $pathTool -CandidatePath $planned -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
if ([string]$pathGate.state -ne 'PASS_PATH_BUDGET') { throw 'O2D1 package path gate failed.' }
if ($Preflight) {
    [ordered]@{
        schema = 'argos_o2d1_build_preflight_v1'
        state = 'PASS_O2D1_BUILD_PREFLIGHT'
        requestId = $requestId
        payloadFileCount = 4
...
if (-not $certificate.HasPrivateKey) { throw 'O2D1 signer private key is unavailable.' }
$files = @(
    [ordered]@{source=$endpointPath;path='payload/Invoke-O2D1ScribeEndpoint.ps1';bytes=[int64](Get-Item -LiteralPath $endpointPath).Length;sha256=$endpointSha},
    ...
    [ordered]@{source=$bundlePath;path='payload/O2D1_REFS.zip';bytes=[int64](Get-Item -LiteralPath $bundlePath).Length;sha256=$bundleSha},
    [ordered]@{source=$jobPath;path='payload/O2D1_SLOT16_JOB.json';bytes=[int64](Get-Item -LiteralPath $jobPath).Length;sha256=$jobSha}
)
...
if ([string]$packageTest.State -ne 'PASS_SIGNED_PORTAL_PACKAGE') { throw 'O2D1 signed package verification failed.' }
...
$expected = @{
    'payload/Invoke-O2D1ScribeEndpoint.ps1'=$endpointSha
    'payload/ArgosOpenCvScribeV1.py'=$engineSha
    'payload/O2D1_REFS.zip'=$bundleSha
    'payload/O2D1_SLOT16_JOB.json'=$jobSha
...
if ($extracted.Count -ne 6) { throw 'O2D1 final ZIP file count changed.' }
foreach ($item in $expected.GetEnumerator()) {
    $path = Join-Path $extractRoot $item.Key.Replace('/', '\')
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or (Get-Sha256 $path) -ne [string]$item.Value) { throw "O2D1 final ZIP file changed: $($item.Key)" }
}
$tokens = $null
$errors = $null
$extractedEndpoint = Join-Path $extractRoot 'payload\Invoke-O2D1ScribeEndpoint.ps1'
[void][Management.Automation.Language.Parser]::ParseFile($extractedEndpoint, [ref]$tokens, [ref]$errors)
if (@($errors).Count -ne 0) { throw 'O2D1 extracted endpoint parser failed.' }
$runtimeRoot = Join-Path $project 'work\FIDUCIAL_OPENCV_V1\portable\stage'
$rehearsalJob = Join-Path $root 'O2D1_REHEARSAL_JOB.json'
$exactPreflight = (& $extractedEndpoint -Preflight -Rehearsal -PayloadRoot (Join-Path $extractRoot 'payload') -RuntimeRoot $runtimeRoot -WorkRoot 'C:\O2D1T3\w' -OutputRoot 'C:\O2D1T3\o' -RehearsalJobPath $rehearsalJob | Out-String) | ConvertFrom-Json
if ([string]$exactPreflight.state -ne 'PASS_O2D1_ENDPOINT_PREFLIGHT' -or [bool]$exactPreflight.mutationsPerformed -or [bool]$exactPreflight.processStarted) { throw 'O2D1 exact extracted endpoint preflight failed.' }

$zipSha = Get-Sha256 $partialZip
$gate = [ordered]@{
    schema = 'argos_o2d1_final_package_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O2D1_FINAL_PACKAGE_GATE'
    requestId = $requestId
    requestZip = 'work/OPENCV_SCRIBE_O2D1/final/' + $zipName
