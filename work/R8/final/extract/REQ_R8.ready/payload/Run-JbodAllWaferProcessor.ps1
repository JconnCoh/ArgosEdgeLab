    ...
    do{
        ...
        try{
            ...
            if($cooperativeHold){
                ...
                $metadataSnapshotRoot=if(($config.PSObject.Properties.Name-contains'metadataSnapshotRoot')-and-not[string]::IsNullOrWhiteSpace([string]$config.metadataSnapshotRoot)){
                    [IO.Path]::GetFullPath([string]$config.metadataSnapshotRoot)
                }else{[IO.Path]::GetFullPath((Join-Path ([string]$config.stateRoot) 'metadata\verified'))}
