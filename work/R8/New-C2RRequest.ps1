...
$requestId='REQ_R8'
$definitionPath=Join-Path $project 'work\R8\pkg\MAINTENANCE_DEFINITION.json'
$payloadRoot=Join-Path $project 'work\R8\pkg\payload'
$outputRoot=Join-Path $project 'work\R8\signed_short'
...
if($payloadFiles.Count-ne2){throw 'R8 payload file count changed.'}
$payloadNames=@($payloadFiles|ForEach-Object{[string]$_.path})
if($payloadNames-notcontains[string]$definition.entryPoint){throw 'R8 entry point is absent from the payload.'}
foreach($change in @($definition.changes)){
    ...
    if(@($change.approvedPredecessorSha256).Count-lt1-or@($change.approvedPredecessorSha256)-notcontains[string]$change.installedSha256){throw 'R8 predecessor set must include its idempotent target hash.'}
