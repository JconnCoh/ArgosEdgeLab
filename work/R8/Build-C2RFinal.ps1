...
$requestId='REQ_R8'
$sourceRoot=Join-Path $root 'signed_short\REQ_R8.ready'
$finalRoot=Join-Path $root 'final'
$partialRoot='C:\R8Z'
$zipName='REQ_R8.ready.zip'
...
$entrySha='3B5FE09081FC4545420B16874D675525A3647785EFD4B1294562ABEE18E6BEC4'
$endpointPath=Join-Path $root 'C2R_EXACT_ENDPOINT_GATE.json'
$routePath=Join-Path $root 'R8_COMPLETE_ROUTE_GATE.json'
$priorPath=Join-Path $project 'work\V40\C2V40_TERMINAL_RESPONSE_GATE.json'
...
$finalTestRoot='C:\R8F1'
$finalEndpointPath=Join-Path $root 'R8_FINAL_EXACT_ENDPOINT_GATE.json'
$finalInvocationPath=Join-Path $root 'R8_FINAL_ENDPOINT_INVOCATION.json'

foreach($path in @($sourceRoot,$endpointPath,$routePath,$priorPath,$requestVerifier,$public,$endpointTest,$pathTool,$wrapper)){if(-not(Test-Path -LiteralPath $path)){throw "R8 final prerequisite missing: $path"}}
...
$endpoint=Read-Gate $endpointPath 'PASS_R8_EXACT_ENDPOINT_GATE'
$route=Read-Gate $routePath 'PASS_R8_COMPLETE_ROUTE_GATE'
$prior=Read-Gate $priorPath 'PASS_C2V40_SIGNED_TERMINAL_RESPONSE'
if(-not[bool]$endpoint.approvedOldPredecessorExercised-or-not[bool]$endpoint.targetIdempotenceAccepted-or-not[bool]$endpoint.unapprovedPredecessorRefusedBeforeMutation-or-not[bool]$endpoint.runtimeVerifierFailureRolledBack-or-not[bool]$endpoint.controlAfterRuntimeFailurePassed-or[int]$endpoint.responseSignaturesVerified-ne5){throw 'R8 endpoint contract changed.'}
if([int]$route.maximumPlannedEffectiveLength-ge200-or[int]$route.maximumPlannedComponentLength-gt80-or[string]$route.laptopResponseExtractionRoot-ne'C:\R8S'-or-not[bool]$route.laptopResponseRootFreshAtFreeze){throw 'R8 route contract changed.'}
if(-not[bool]$prior.signatureVerified-or[string]$prior.responseId-ne'R_CB3CDBA6F0B8_20260821161419288_ddf4e3bc'-or-not[bool]$prior.catalogAcceptance-or[string]$prior.disposition-ne'PENDING_DASHBOARD_REFRESH'){throw 'R8 prior signed terminal prerequisite changed.'}
...
foreach($relative in @('New-C2RRequest.ps1','Test-C2RExactEndpoint.ps1','Test-C2RRoutes.ps1','Build-C2RFinal.ps1','pkg\payload\C2R.ps1','pkg\payload\Run-JbodAllWaferProcessor.ps1')){
    ...
    $scriptRows.Add([pscustomobject]@{file=('work/R8/'+$relative.Replace('\','/'));sha256=(Get-FileHash -LiteralPath $script -Algorithm SHA256).Hash})
}
...
if((Get-FileHash -LiteralPath $manifestPath -Algorithm SHA256).Hash-ne$manifestSha-or[string]$manifest.requestId-ne$requestId-or@($manifest.files).Count-ne2-or@($manifest.changes).Count-ne2-or@($manifest.allowedTaskActions).Count-ne1-or[string]$manifest.allowedTaskActions[0]-ne'RESTART:ArgosEdgeLab.AllWaferProcessor.ReviewOnly.V2'){throw 'R8 signed manifest contract changed.'}
if((Get-FileHash -LiteralPath (Join-Path $sourceRoot 'payload\C2R.ps1') -Algorithm SHA256).Hash-ne$entrySha-or(Get-FileHash -LiteralPath (Join-Path $sourceRoot 'payload\Run-JbodAllWaferProcessor.ps1') -Algorithm SHA256).Hash-ne$targetSha){throw 'R8 signed payload changed.'}

$planned=@((Join-Path $partialRoot $zipName),(Join-Path $partialRoot ($zipName+'.gate.json')),(Join-Path $partialRoot 'extract\REQ_R8.ready\PORTAL_REQUEST_MANIFEST.json'),(Join-Path $partialRoot 'extract\REQ_R8.ready\payload\C2R.ps1'),(Join-Path $partialRoot 'extract\REQ_R8.ready\payload\Run-JbodAllWaferProcessor.ps1'),(Join-Path $finalRoot $zipName),(Join-Path $finalRoot ($zipName+'.gate.json')),$finalInvocationPath,$finalEndpointPath,(Join-Path $finalTestRoot 'control_after_failure\responses\R_0123456789AB_20260820050505050_a1b2c3d4.ready\PORTAL_RESPONSE_MANIFEST.json'))
...
if($Preflight){
    [ordered]@{schema='argos_r8_final_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R8_FINAL_PACKAGE_PREFLIGHT';requestId=$requestId;manifestSha256=$manifestSha;targetSha256=$targetSha;entryPointSha256=$entrySha;priorTerminalResponseId=[string]$prior.responseId;pathState=[string]$pathGate.state;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 5
    return
}
...
$extract=Join-Path $partialRoot 'extract\REQ_R8.ready'
...
$invocation=[ordered]@{schema='argos_r8_endpoint_test_invocation_v1';requestRoot=$extract;testRoot=$finalTestRoot;gatePath=$finalEndpointPath}
...
$finalEndpoint=Read-Gate $finalEndpointPath 'PASS_R8_EXACT_ENDPOINT_GATE'
...
$gate=[ordered]@{
    schema='argos_r8_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R8_FINAL_PACKAGE_GATE';requestId=$requestId
    requestZip='work/R8/final/REQ_R8.ready.zip';requestZipBytes=$zipBytes;requestZipSha256=$zipHash
