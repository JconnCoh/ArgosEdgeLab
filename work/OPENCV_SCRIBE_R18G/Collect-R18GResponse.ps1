...
$expectedResponseId = 'R_5FAB56656B2D_20260903221148575_65bcc5ed'
$expectedResponseZipSha256 = 'EDE6F6BD15762499621B013613915EB29981CF48C67A4D771AE94FAC280F2DE9'
...
$outerExtractRoot = 'C:\R18GR'
$payloadExtractRoot = 'C:\R18G'
$metadataRoot = Join-Path $PSScriptRoot 'collected_metadata'
$terminalGatePath = Join-Path $PSScriptRoot 'R18G_TERMINAL_RESPONSE_GATE.json'
...
$publishGateSha256 = 'B1677C6A424C791B28F194C2A77B7071B75FA76CC7B14212C7931A258145C325'
...
Assert-True ((Get-Sha256 $definitionPath) -eq $definitionSha256) 'R18G definition changed.'
Assert-True ((Get-Sha256 $preactionPath) -eq $preactionSha256) 'R18G preaction changed.'
Assert-True ((Get-Sha256 $publishGatePath) -eq $publishGateSha256) 'R18G publish gate changed.'
& $preactionTool -AuditPath $historyPath -ContractPath $preactionPath -ProjectRoot $project -Preflight | Out-Null
Assert-True (Test-Path -LiteralPath $publishGatePath -PathType Leaf) 'R18G has not been published.'
Assert-True (Test-Path -LiteralPath $routeGatePath -PathType Leaf) 'R18G route gate is missing.'
...
Assert-True ([string]$publishGate.state -eq 'PASS_R18G_EXACT_SIGNED_DATA_PULL_PUBLISHED_CREATE_NEW' -and [string]$publishGate.requestId -eq $requestId -and -not [bool]$publishGate.retryAuthorized) 'R18G publication evidence changed.'
Assert-True ([string]$routeGate.state -eq 'PASS_R18G_COMPLETE_ROUTE_GATE' -and [int]$routeGate.maximumEffectiveLength -lt 200) 'R18G route gate changed.'
...
Assert-True ($matches.Count -le 1) 'Multiple terminal responses match R18G.'
if ($matches.Count -eq 0) {
    if ($Collect) { throw 'No matching signed terminal response exists yet.' }
    [ordered]@{schema='argos_r18g_response_collection_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='WAIT_R18G_MATCHING_RESPONSE';requestId=$requestId;responseFilesScanned=$responseFiles.Count;matchingResponses=0;mutationsPerformed=$false;requestRetried=$false;pixelsDecoded=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}

$source = [string]$matches[0].file.FullName
$sourceZipBytes = [int64]$matches[0].file.Length
Assert-True ($sourceZipBytes -le 50331648) 'R18G response ZIP exceeds the signed bound.'
$sourceZipSha256 = Get-Sha256 $source
Assert-True ([string]$matches[0].manifest.responseId -eq $expectedResponseId -and $sourceZipSha256 -eq $expectedResponseZipSha256) 'R18G exact response identity or ZIP changed.'
$zip = [IO.Compression.ZipFile]::OpenRead($source)
try {
    $names = @($zip.Entries | ForEach-Object { $_.FullName })
    Assert-True ($names.Count -eq 4 -and @(Compare-Object -ReferenceObject @('DATA_PULL_PAYLOAD.zip','PORTAL_RESPONSE_MANIFEST.json','PORTAL_RESPONSE_MANIFEST.sig','RESULT.json') -DifferenceObject $names).Count -eq 0) 'R18G response entry set changed.'
...
Assert-True ($signatureValid -and $certificate.Thumbprint.ToUpperInvariant() -eq $expectedSignerThumbprint) 'R18G response signature failed.'
...
Assert-True ([string]$result.schema -eq 'argos_project_portal_data_pull_result_v2' -and [string]$result.state -eq 'PASS_DATA_PULL' -and [string]$result.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'R18G DATA_PULL result changed.'
Assert-True ([string]$result.container -eq 'DATA_PULL_PAYLOAD.zip' -and [string]$result.containerSha256 -eq $payloadSha256 -and [int64]$result.containerBytes -eq $payloadBytes.Length) 'R18G payload container pin changed.'
Assert-True ($resultFiles.Count -eq 24 -and @(Compare-Object -ReferenceObject $expectedRelativePaths -DifferenceObject $observedRelativePaths).Count -eq 0 -and @(Compare-Object -ReferenceObject $expectedEntryPaths -DifferenceObject $observedEntryPaths).Count -eq 0) 'R18G exact returned file set changed.'
Assert-True ([int64]$result.totalSourceBytes -le 50331648 -and [bool]$result.sourcePathsPreservedAsZipEntries -and [bool]$result.filesystemReturnPathsFlattened) 'R18G returned path/byte contract changed.'
...
Assert-True ([string]$responseTest.State -eq 'PASS_SIGNED_PORTAL_RESPONSE' -and [string]$responseTest.EndpointState -eq 'PASS_DATA_PULL') 'R18G extracted signed response verification failed.'
...
Assert-True ($diskFiles.Count -eq 24) 'R18G extracted payload count changed.'
foreach ($row in $resultFiles) {
    $diskPath = Join-Path $payloadExtractRoot ([string]$row.entryPath).Replace('/', '\')
    Assert-True (Test-Path -LiteralPath $diskPath -PathType Leaf) "R18G extracted file missing: $($row.entryPath)"
    Assert-True ([int64](Get-Item -LiteralPath $diskPath).Length -eq [int64]$row.bytes -and (Get-Sha256 $diskPath) -eq [string]$row.sha256) "R18G extracted file changed: $($row.entryPath)"
}
