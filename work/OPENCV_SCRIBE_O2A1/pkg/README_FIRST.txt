ARGOS O2D4 DIRECT ADMIN READ-ONLY OBSERVATION - O2A1
...
2. Right-click RUN_O2A1.cmd and choose Run as administrator.
...
4. After PASS it performs one read-only observation, writes evidence only under
   C:\O2A1, and returns O2A1R.zip to InspectionRevs automatically.
5. Leave the final window visible. If the return share is unavailable, retain
   O2A1R_LOCAL.zip beside the launcher and report the displayed failure.

O2A1 installs nothing; starts, stops, or restarts no task or process; changes no
