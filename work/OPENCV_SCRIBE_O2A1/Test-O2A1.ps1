...
function New-CaseInvocation([string]$CaseRoot, [string]$CaseId, [int]$MatchCount) {
    ...
    return [ordered]@{
        schema='argos_o2a1_direct_admin_invocation_v1'
        ...
        localResultLeaf='O2A1R_LOCAL.zip'
        returnPath=(Join-Path $CaseRoot 'return\O2A1R.zip')
        refuseComputerNames=@()
...
foreach ($required in @('AUDIT_O2A1.ps1','RUN_O2A1.cmd','INVOCATION.json','README_FIRST.txt','PACKAGE_MANIFEST.json')) {
    Assert-True (Test-Path -LiteralPath (Join-Path $packageSource $required) -PathType Leaf) "O2A1 package source is absent: $required"
}
Assert-True (Test-Path -LiteralPath $windowsPowerShell -PathType Leaf) 'Windows PowerShell 5.1 is absent.'
Assert-True (-not (Test-Path -LiteralPath $testRoot)) "O2A1 test root must be fresh: $testRoot"
Assert-True (-not (Test-Path -LiteralPath 'C:\O2A1')) 'Live O2A1 output root exists before rehearsal.'
if ($Preflight) {
    [ordered]@{schema='argos_o2a1_test_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2A1_TEST_PREFLIGHT';testRoot=$testRoot;targetExecuted=$false;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 6
    return
}

Assert-True (-not [string]::IsNullOrWhiteSpace($GatePath)) 'Test mode requires GatePath.'
$resolvedGate = [IO.Path]::GetFullPath($GatePath)
Assert-True ($resolvedGate.StartsWith($project.TrimEnd('\') + '\', [StringComparison]::OrdinalIgnoreCase)) 'O2A1 test gate must remain inside the project.'
Assert-True (-not (Test-Path -LiteralPath $resolvedGate)) 'O2A1 test gate must be create-new.'
...
foreach ($case in $cases) {
    ...
    $audit = Join-Path $package 'AUDIT_O2A1.ps1'
    ...
    Assert-True ($exitCode -eq 0) ("O2A1 case failed: " + [string]$case.id + [Environment]::NewLine + ($output -join [Environment]::NewLine))
    $returnZip = Join-Path $caseRoot 'return\O2A1R.zip'
    Assert-True (Test-Path -LiteralPath $returnZip -PathType Leaf) "O2A1 case returned no ZIP: $($case.id)"
    $extract = Join-Path $caseRoot 'extract'
    [IO.Compression.ZipFile]::ExtractToDirectory($returnZip, $extract)
    $observationPath = Join-Path $extract 'O2A1_OBSERVATION.json'
    Assert-True (Test-Path -LiteralPath $observationPath -PathType Leaf) "O2A1 case returned no observation: $($case.id)"
    $observation = Get-Content -LiteralPath $observationPath -Raw | ConvertFrom-Json
    Assert-True ([string]$observation.state -eq 'PASS_O2A1_DIRECT_ADMIN_READ_ONLY_OBSERVATION') "O2A1 case did not pass: $($case.id)"
    Assert-True (-not [bool]$observation.targetMutationsPerformed -and -not [bool]$observation.imageBytesRead) "O2A1 case violated authority: $($case.id)"
    $responseSource = @($observation.sourceResults | Where-Object { [string]$_.id -eq 'RESP_PENDING' })
    Assert-True ($responseSource.Count -eq 1) "O2A1 response source cardinality changed: $($case.id)"
    Assert-True ([int]$responseSource[0].matchedStartCount -eq [int]$case.matchCount) "O2A1 response match count changed: $($case.id)"
    $bmpRows = @($observation.sourceResults.rows | Where-Object { [string]$_.extension -eq '.bmp' })
    Assert-True (@($bmpRows | Where-Object { [bool]$_.safeTextOrSignatureContentRead }).Count -eq 0) "O2A1 read fixture BMP bytes: $($case.id)"
...
Assert-True (-not (Test-Path -LiteralPath 'C:\O2A1')) 'O2A1 rehearsal created the live output root.'
$gate = [ordered]@{
    schema='argos_o2a1_test_gate_v1'
    createdUtc=[DateTime]::UtcNow.ToString('o')
    state='PASS_O2A1_DIRECT_ADMIN_READ_ONLY_TEST_GATE'
