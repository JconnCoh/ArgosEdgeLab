...
JOB_SCHEMA = "argos_ocv03_o3p8_front_split_notch_job_v1"
...
def refine_bf(
    ...
    require(clean.shape == (inward + outward, width), "O3P8 crop shape changed.")
...
def process_identity(row: dict[str, Any], topology: Any, renderer: Any, job: dict[str, Any]) -> dict[str, Any]:
    ...
    require(1 <= len(seeds) <= int(job["corroboration"]["maximumSeedCountPerWafer"]), "O3P8 seed count is outside the bound.")
    for channel in ("bf", "df"):
        source = row[channel]
        source_path = Path(str(source["path"])).resolve(strict=True)
        require(source_path.stat().st_size == int(source["bytes"]), f"O3P8 {channel.upper()} byte count changed: {source_path}")
        require(sha256_file(source_path) == str(source["sha256"]).upper(), f"O3P8 {channel.upper()} source hash changed: {source_path}")
    bf_path = Path(str(row["bf"]["path"])).resolve(strict=True)
    bf_image = cv2.imread(str(bf_path), cv2.IMREAD_GRAYSCALE)
    require(bf_image is not None, f"O3P8 BF decode failed: {bf_path}")
    bf_fit = dict(seed_result["bf"]["fit"])
    require(
        bf_image.shape == (int(seed_result["bf"]["heightPx"]), int(seed_result["bf"]["widthPx"])),
        "O3P8 BF dimensions changed.",
    )
...
def run(job_path: Path, job: dict[str, Any], runtime_root: Path, topology_path: Path, renderer_path: Path, output_path: Path) -> dict[str, Any]:
    topology = load_module("argos_o3p8_topology", topology_path)
    renderer = load_module("argos_o3p8_crop", renderer_path)
    rows = [process_identity(row, topology, renderer, job) for row in job["inputs"]]
    require(len(rows) == int(job["expectedInputCount"]), "O3P8 input count changed.")
    result = {
        "schema": "argos_ocv03_o3p8_front_split_notch_result_v1",
        "revision": str(job["revision"]),
        "createdUtc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "state": "COMPLETE_O3P8_POST2_BF_TOPOLOGY_DF_RADIAL_REVIEW_ONLY",
...
def main() -> int:
    ...
    for row in job["inputs"]:
        ...
        for channel in ("bf", "df"):
            ...
            require(source.stat().st_size == int(row[channel]["bytes"]), f"O3P8 source byte count changed: {source}")
            require(sha256_file(source) == str(row[channel]["sha256"]).upper(), f"O3P8 source hash changed: {source}")
    if arguments.preflight:
        result = {
            "state": "PASS_O3P8_FRONT_SPLIT_NOTCH_PREFLIGHT",
            "jobSha256": sha256_file(job_path),
