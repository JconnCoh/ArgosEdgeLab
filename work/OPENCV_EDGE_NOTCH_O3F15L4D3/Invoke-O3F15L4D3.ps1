...
function Assert-PayloadAndTargetPins([object]$Contract, [string]$Root) {
    ...
    foreach ($required in @('Invoke-O3F15L4D3.ps1', 'Run-O3F15L4FrontReconcile.py', 'Test-O3F15L4PathHolds.py', 'FullPerimeterWaferTopologyOpenCvR11.py', 'OCV03_NotchReviewOpenCvV1.py')) {
...
function Assert-Classification([object]$Classification, [int64]$MaximumCoreBytes) {
    ...
    foreach ($className in $classNames) {
        ...
        foreach ($row in $rows) {
            ...
            Require ($rawLength -eq $canonicalText.Length -and $effectiveLength -eq $rawLength + 32 -and $maximumComponentLength -eq $computedMaximum -and $computedMaximum -le 255) 'Canonical path metrics changed.'
            ...
            if ($className -cne 'DIRECT_SAFE') {
                ...
                Require ($aliasRaw -eq $aliasText.Length -and $aliasEffective -eq $aliasRaw + 32 -and $aliasEffective -lt 200 -and $aliasMaximum -eq $computedAliasMaximum -and $computedAliasMaximum -le 255) 'Alias path metrics changed.'
            }
...
function New-FailureEnvelope([string]$Code, [string]$Detail, [object]$Contract) {
    ...
    [ordered]@{
        schema = 'argos_ocv03_o3f15l4d3_metadata_diagnostic_v1'
        state = 'HOLD_O3F15L4D3_METADATA_DIAGNOSTIC'
        failureCode = $Code
...
try {
    ...
    $contractPath = Join-Path $effectivePackageRoot 'O3F15L4D3_DIAGNOSTIC_CONTRACT.json'
    ...
    if ($Rehearsal) {
        $draftContract = [string]$contract.lifecycle -ceq 'DRAFT' -and [string]$contract.state -ceq 'DRAFT_O3F15L4D3_METADATA_DIAGNOSTIC_CONTRACT'
        $frozenContract = [string]$contract.lifecycle -ceq 'FROZEN' -and [string]$contract.state -ceq 'FROZEN_O3F15L4D3_METADATA_DIAGNOSTIC_CONTRACT'
        Require ($draftContract -or $frozenContract) 'D2 rehearsal requires a consistently paired draft or frozen contract.'
    } else {
        Require ([string]$contract.lifecycle -ceq 'FROZEN' -and [string]$contract.state -ceq 'FROZEN_O3F15L4D3_METADATA_DIAGNOSTIC_CONTRACT') 'D2 live execution requires the once-frozen contract.'
    }
    ...
    if ($Preflight) {
        Write-TerminalJson ([ordered]@{
            schema = 'argos_ocv03_o3f15l4d3_entrypoint_preflight_v1'
            state = 'PASS_O3F15L4D3_ENTRYPOINT_PREFLIGHT'
    ...
    $result = [ordered]@{
        schema = 'argos_ocv03_o3f15l4d3_metadata_diagnostic_v1'
        state = 'COMPLETE_O3F15L4D3_METADATA_DIAGNOSTIC'
