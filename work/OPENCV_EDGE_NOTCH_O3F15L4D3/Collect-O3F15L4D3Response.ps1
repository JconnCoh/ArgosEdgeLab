...
function Confirm-Classification([object]$Classification, [object]$Contract) {
    ...
    foreach ($name in $classNames) {
        ...
        foreach ($row in $rows) {
            ...
            Assert-True ($rawLength -eq $canonical.Length -and $effectiveLength -eq $rawLength + 32 -and $componentLength -eq (Get-MaximumComponentLength $canonical) -and $componentLength -le 255) 'D2 canonical source metrics changed.'
...
$expectedInvocation = Join-Path $PSScriptRoot 'O3F15L4D3_RESPONSE_COLLECTION_INVOCATION.json'
...
Assert-True ([string](Get-RequiredProperty $invocation 'schema') -ceq 'argos_ocv03_o3f15l4d3_response_collection_invocation_v1' -and [string](Get-RequiredProperty $invocation 'state') -ceq 'FROZEN_O3F15L4D3_RESPONSE_COLLECTION_INVOCATION') 'D2 response collection invocation changed.'
...
Assert-True ([string]$contract.schema -ceq 'argos_ocv03_o3f15l4d3_diagnostic_contract_v1' -and [string]$contract.lifecycle -ceq 'FROZEN') 'D2 frozen contract changed.'
...
Assert-True ([string]$publishGate.state -ceq 'PASS_O3F15L4D3_PUBLISHED_EXACTLY_ONCE_AWAITING_MATCHING_SIGNED_RESPONSE' -and [string]$publishGate.requestId -ceq $requestId -and [int]$publishGate.publicationCount -eq 1 -and -not [bool]$publishGate.automaticRetryAuthorized) 'D2 publication gate changed.'
...
Assert-True ($localRoot -ceq 'C:\O3F15D3C' -and $gatePath.Equals((Join-Path $PSScriptRoot 'O3F15L4D3_RESPONSE_COLLECTION_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 create-new response destinations changed.'
...
if ($entryBytes.ContainsKey('MAINTENANCE.stdout.txt') -and ([byte[]]$entryBytes['MAINTENANCE.stdout.txt']).Length -gt 0) {
    ...
    Assert-True ([string](Get-RequiredProperty $diagnostic 'schema') -ceq 'argos_ocv03_o3f15l4d3_metadata_diagnostic_v1') 'D2 diagnostic schema changed.'
    $diagnosticState = [string](Get-RequiredProperty $diagnostic 'state')
    Assert-True ($diagnosticState -cin @('COMPLETE_O3F15L4D3_METADATA_DIAGNOSTIC', 'HOLD_O3F15L4D3_METADATA_DIAGNOSTIC')) 'D2 diagnostic terminal state changed.'
    Confirm-DiagnosticAuthority $diagnostic
    if ($diagnosticState -ceq 'COMPLETE_O3F15L4D3_METADATA_DIAGNOSTIC') {
        ...
        Assert-True ((Normalize-WindowsPath ([string]$child.executable)).Equals((Normalize-WindowsPath 'D:/AFCV1/rt/python.exe'), [StringComparison]::OrdinalIgnoreCase) -and (@($child.arguments) -join '|') -ceq '-I|-B|Run-O3F15L4FrontReconcile.py|PREFLIGHT') 'D2 sole child invocation changed.'
...
if ($endpointState -ceq 'PASS_MAINTENANCE_PATCH') {
    ...
    Assert-True ([string]$maintenance.schema -ceq 'argos_project_portal_maintenance_result_v1' -and [string]$maintenance.state -ceq 'PASS_MAINTENANCE_PATCH' -and [int]$maintenance.exitCode -eq 0 -and [int]$maintenance.changedFiles -eq 1 -and [string]$maintenance.entryPoint -ceq 'payload/Invoke-O3F15L4D3.ps1' -and [bool]$maintenance.reviewOnly -and -not [bool]$maintenance.productionRoutingEnabled) 'D2 maintenance result changed.'
} else {
...
$preflightResult = [ordered]@{
    schema = 'argos_ocv03_o3f15l4d3_response_collection_preflight_v1'
    observedUtc = [DateTime]::UtcNow.ToString('o')
    state = $(if ($classificationValidated) { 'PASS_O3F15L4D3_RESPONSE_COLLECTION_PREFLIGHT' } else { 'HOLD_O3F15L4D3_SIGNED_TERMINAL_RESPONSE_PREFLIGHT' })
    requestId = $requestId
...
$gate = [ordered]@{
    schema = 'argos_ocv03_o3f15l4d3_response_collection_gate_v1'
    collectedUtc = [DateTime]::UtcNow.ToString('o')
    state = $(if ($classificationValidated) { 'PASS_O3F15L4D3_MATCHING_SIGNED_METADATA_CLASSIFICATION_COLLECTED' } else { 'HOLD_O3F15L4D3_MATCHING_SIGNED_TERMINAL_RESPONSE_COLLECTED' })
