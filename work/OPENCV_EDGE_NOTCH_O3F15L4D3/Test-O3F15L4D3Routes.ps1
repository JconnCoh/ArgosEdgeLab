...
$expectedInvocation = Join-Path $PSScriptRoot 'O3F15L4D3_ROUTE_GATE_INVOCATION.json'
...
Assert-True ([string](Get-RequiredProperty $invocation 'schema') -ceq 'argos_ocv03_o3f15l4d3_route_gate_invocation_v1') 'D2 route invocation schema changed.'
Assert-True ([string](Get-RequiredProperty $invocation 'state') -ceq 'FROZEN_O3F15L4D3_ROUTE_GATE_INVOCATION') 'D2 route invocation is not frozen.'
Assert-True ([string](Get-RequiredProperty $invocation 'revision') -ceq 'O3F15L4D3') 'D2 route invocation revision changed.'
...
Assert-True ([string]$contract.schema -ceq 'argos_ocv03_o3f15l4d3_diagnostic_contract_v1' -and [string]$contract.lifecycle -ceq 'FROZEN' -and [string]$contract.revision -ceq 'O3F15L4D3') 'D2 frozen diagnostic contract changed.'
...
Assert-True ([string]$routeInventory.state -ceq 'PASS_O3F15L4D3_WORKER_HASH_BOUND_ROUTE_CAPABILITY_INVENTORY') 'D2 route capability inventory changed.'
...
Assert-True ($gateOutputPath.Equals((Join-Path $PSScriptRoot 'final_o3f15l4d3\O3F15L4D3_PREPUBLICATION_PATH_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 route gate output is not beside the final ZIP.'
Assert-True ($publishGatePath.Equals((Join-Path $PSScriptRoot 'O3F15L4D3_PUBLISH_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication gate path changed.'
Assert-True ($publicationAttemptPath.Equals((Join-Path $PSScriptRoot 'O3F15L4D3_PUBLISH_ATTEMPT.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication-attempt receipt path changed.'
Assert-True ($collectionGatePath.Equals((Join-Path $PSScriptRoot 'O3F15L4D3_RESPONSE_COLLECTION_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 collection gate path changed.'
Assert-True ($localCollectionRoot -ceq 'C:\O3F15D3C') 'D2 response collection root is not the frozen short root.'
...
Assert-True ([int64]$manifest.maxResultBytes -eq 8388608 -and [string]$manifest.entryPoint -ceq 'payload/Invoke-O3F15L4D3.ps1' -and -not [bool]$manifest.requestRetryAuthorized) 'D2 request execution/result contract changed.'
$manifestFiles = @($manifest.files)
Assert-True ($manifestFiles.Count -eq (@($contract.payloadFiles).Count + 1)) 'D2 signed payload cardinality changed.'
Assert-True (@($manifestFiles | Where-Object { [string]$_.path -ceq 'payload/O3F15L4D3_DIAGNOSTIC_CONTRACT.json' }).Count -eq 1) 'D2 signed payload lacks its frozen contract.'
...
$candidatePaths = @($distinctItems | ForEach-Object { [string]$_.path })
...
$result = [ordered]@{
    schema = 'argos_ocv03_o3f15l4d3_complete_route_path_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    lifecycle = $(if ($Preflight) { 'DRAFT_PREFLIGHT' } else { 'FROZEN_GATE_OUTPUT' })
    state = $(if ($Preflight) { 'PASS_O3F15L4D3_COMPLETE_ROUTE_PATH_PREFLIGHT' } else { 'PASS_O3F15L4D3_COMPLETE_ROUTE_PATH_GATE' })
    revision = 'O3F15L4D3'
    requestId = $requestId
