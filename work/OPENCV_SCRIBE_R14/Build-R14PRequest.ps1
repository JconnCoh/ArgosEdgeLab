...
Assert-True (Test-Path -LiteralPath $InvocationManifest -PathType Leaf) 'R14P build invocation manifest is absent.'
...
Assert-True ([string]$invocation.schema -eq 'argos_r14p_build_invocation_v1') 'R14P build invocation schema changed.'
Assert-True ([string]$invocation.requestId -eq 'REQ_20260902T204408092Z_R14P') 'R14P build request identity changed.'
Assert-True ([string]$invocation.requiredBranch -eq 'codex/opencv-scribe-deciphering') 'R14P build branch changed.'
Assert-True ([bool]$invocation.authority.reviewOnly -and -not [bool]$invocation.authority.automaticIdentityAuthority -and -not [bool]$invocation.authority.trainingEligible -and -not [bool]$invocation.authority.xmlEligible -and -not [bool]$invocation.authority.productionEligible -and -not [bool]$invocation.authority.productionRoutingEnabled -and -not [bool]$invocation.authority.providerActivationAllowed) 'R14P build authority widened.'
Assert-True ([int]$invocation.authority.maximumPublications -eq 1 -and -not [bool]$invocation.authority.retryAuthorized) 'R14P publication count or retry boundary changed.'
Assert-True ((Get-Sha256 $MyInvocation.MyCommand.Path) -eq [string]$invocation.builderSha256) 'R14P builder self-pin changed.'
...
Assert-PinnedFile $contractPath ([string]$invocation.requestContract.sha256) 'R14P request contract'
Assert-PinnedFile $definitionPath ([string]$invocation.maintenanceDefinition.sha256) 'R14P maintenance definition'
$contract = Get-Content -Raw -LiteralPath $contractPath | ConvertFrom-Json
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Assert-True ([string]$contract.requestId -eq [string]$invocation.requestId -and [string]$contract.classification -match '^FROZEN') 'R14P request contract is not the frozen request.'
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH') 'R14P maintenance route changed.'
Assert-True ([string]$definition.entryPoint -eq [string]$invocation.entryPoint) 'R14P maintenance entrypoint changed.'
Assert-True ([int64]$definition.maxResultBytes -eq 16777216) 'R14P maximum portal result bytes changed.'

$payloadRows = @($invocation.payloads)
Assert-True ($payloadRows.Count -eq 7) 'R14P payload cardinality changed.'
...
foreach ($row in $payloadRows) {
    ...
    Assert-PinnedFile $source ([string]$row.sha256) 'R14P payload'
    $packagePath = ([string]$row.packagePath).Replace('\', '/')
    Assert-True ($packagePath -match '^payload/[A-Za-z0-9._-]+$') "R14P payload package path is not a single safe leaf: $packagePath"
    Assert-True ($packagePath -notmatch '(?i)(__pycache__|\.pyc$)') "R14P cache bytecode is forbidden: $packagePath"
    Assert-True ($seenPackagePaths.Add($packagePath)) "R14P duplicate payload package path: $packagePath"
...
Assert-True ($seenPackagePaths.Contains([string]$definition.entryPoint)) 'R14P entrypoint is not one exact payload.'

$endpointRow = @($payloadRows | Where-Object { ([string]$_.packagePath).Replace('\','/') -eq [string]$definition.entryPoint })
Assert-True ($endpointRow.Count -eq 1) 'R14P endpoint payload cardinality changed.'
Assert-True (@($definition.changes).Count -eq 1) 'R14P maintenance change cardinality changed.'
$change = $definition.changes[0]
Assert-True ([string]$change.source -eq [string]$definition.entryPoint -and [string]$change.destination -eq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R14P.ps1') 'R14P installed endpoint identity changed.'
Assert-True ([bool]$change.allowCreate -and [string]$change.installedSha256 -eq [string]$endpointRow[0].sha256) 'R14P installed endpoint hash does not match payload.'
Assert-True (@($change.approvedPredecessorSha256).Count -eq 1 -and [string]$change.approvedPredecessorSha256[0] -eq '159BD82528E505DE69BDEEE4C3A3B29402BA9281939C57F6187587E9047D9740') 'R14P approved installed predecessor changed.'
Assert-True (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R14P task/process action boundary changed.'
Assert-True ([int]$definition.timeoutContract.maximumSequentialChildren -eq 4 -and [int]$definition.timeoutContract.maximumConcurrentChildren -eq 1 -and [int]$definition.timeoutContract.opencvChildTimeoutSeconds -eq 600 -and [int]$definition.timeoutContract.endpointWorkerOuterTimeoutSeconds -eq 3000) 'R14P timeout contract changed.'
Assert-True ([int]$definition.timeoutContract.endpointWorkerOuterTimeoutSeconds -gt ([int]$definition.timeoutContract.maximumSequentialChildren * [int]$definition.timeoutContract.opencvChildTimeoutSeconds)) 'R14P outer timeout does not cover all serialized children.'
Assert-True (-not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed -and -not [bool]$definition.sourceProcessingContract.holdClearanceAllowed -and -not [bool]$definition.sourceProcessingContract.providerActivationAllowed -and -not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.fullSourceImageReturnAllowed) 'R14P source-processing authority widened.'
...
foreach ($gatePin in @($invocation.requiredGates)) {
    ...
    Assert-PinnedFile $gatePath ([string]$gatePin.sha256) 'R14P required gate'
    $gate = Get-Content -Raw -LiteralPath $gatePath | ConvertFrom-Json
    $propertyName = [string]$gatePin.stateProperty
    Assert-True ($gate.PSObject.Properties.Name -contains $propertyName) "R14P gate state property is absent: $propertyName"
    Assert-True ([string]$gate.$propertyName -eq [string]$gatePin.requiredState) "R14P required gate state changed: $($gatePin.path)"
...
$routeGatePins = @($requiredGateEvidence.ToArray() | Where-Object { [string]$_.requiredState -eq 'PASS_R14P_COMPLETE_ROUTE_GATE' })
Assert-True ($routeGatePins.Count -eq 1) 'R14P build requires exactly one complete route gate pin.'
...
Assert-PinnedFile $identityPath ([string]$invocation.signing.identitySha256) 'R14P signing identity'
Assert-PinnedFile $publicCertificatePath ([string]$invocation.signing.publicCertificateSha256) 'R14P public certificate'
Assert-PinnedFile $packageTesterPath ([string]$invocation.signing.packageTesterSha256) 'R14P package tester'
$identity = Get-Content -Raw -LiteralPath $identityPath | ConvertFrom-Json
$thumbprint = ([string]$identity.thumbprint).Replace(' ', '').ToUpperInvariant()
Assert-True ($thumbprint -eq ([string]$invocation.signing.expectedThumbprint).Replace(' ', '').ToUpperInvariant()) 'R14P laptop signing thumbprint changed.'
...
foreach ($path in @($signedRoot,$signedPartial,$finalRoot,$finalPartial,$finalGatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R14P build create-new target exists: $path" }

$pathTool = Join-Path $project 'utilities\Confirm-ArgosPathBudget.ps1'
Assert-True (Test-Path -LiteralPath $pathTool -PathType Leaf) 'R14P path-budget tool is absent.'
...
Assert-True ($remoteFinalLeaves -contains 'D:\A2\w\ocv\R14P\jobs\JQ20V.json') 'R14P runtime-job route changed.'
Assert-True ($remoteFinalLeaves -contains 'C:\R14PR\R14P_RETURN.zip') 'R14P short decoded-bundle staging route changed.'
...
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'R14P exact package path budget failed.'
$maximumEffectiveLength = [int]((@($pathGate.candidates) | Measure-Object effectiveLength -Maximum).Maximum)
Assert-True ($maximumEffectiveLength -lt 200) 'R14P package effective path length reached 200.'
...
try {
    ...
    Assert-True ($matches.Count -eq 1 -and $matches[0].HasPrivateKey) 'R14P signing certificate/private key is unavailable or ambiguous.'
    $certificate = $matches[0]
}
finally { $store.Close(); $store.Dispose() }

$preflightResult = [ordered]@{
    schema='argos_r14p_build_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R14P_BUILD_PREFLIGHT';requestId=[string]$invocation.requestId
...
try {
    ...
    foreach ($payload in $payloadFiles) {
        ...
        Assert-True ((Get-Sha256 $destination) -eq [string]$payload.sha256) "R14P copied payload changed: $($payload.path)"
    }
    ...
    Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R14P signed package verification failed.'
    ...
    Assert-True ($extractedFiles.Count -eq $expectedFiles.Count) 'R14P final ZIP file cardinality changed.'
    foreach ($expected in $expectedFiles.GetEnumerator()) {
        $leaf = Get-SafePackageChild $extractRoot ([string]$expected.Key)
        Assert-True (Test-Path -LiteralPath $leaf -PathType Leaf) "R14P final ZIP leaf is absent: $($expected.Key)"
        Assert-True ((Get-Sha256 $leaf) -eq [string]$expected.Value) "R14P final ZIP leaf changed: $($expected.Key)"
    }
    ...
    Assert-True ([string]$extractedTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R14P exact final ZIP signature verification failed.'

    [IO.Directory]::Move($signedPartial, $signedRoot)
    $signedPartialCreated = $false
    [IO.Directory]::Move($finalPartial, $finalRoot)
    $finalPartialCreated = $false
    Assert-True (Test-Path -LiteralPath $zipPath -PathType Leaf) 'R14P final ZIP disappeared after commit.'
    $gate = [ordered]@{
        schema='argos_r14p_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R14P_FINAL_SIGNED_PACKAGE';classification='FROZEN_REVIEW_ONLY';requestId=[string]$invocation.requestId
