...
else { Assert-True ([string]$invocation.schema -eq 'argos_r14p_response_collection_r2_invocation_v1') 'R14P R2 response collection invocation schema changed.' }
...
try {
    ...
    Assert-True ($responseFiles.Count -le [int]$invocation.limits.maximumResponseFiles) 'R14P response file scan bound was exceeded.'
    ...
    Assert-True ($matches.Count -eq 1) "R14P requires exactly one matching signed response; found $($matches.Count)."
    $match = $matches[0]
    ...
    Assert-True ($shortRoot -eq 'C:\R14R') 'R14P short extraction root changed.'
