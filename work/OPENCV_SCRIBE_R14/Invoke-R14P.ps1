...
$providerRevision = 'ARGOS_OPENCV_SCRIBE_ALPHABET_CROP_R14P_20260902'
$requestId = 'REQ_20260902T204408092Z_R14P'
...
$liveProviderSha256 = 'E87230012CB4AEB2D5BDACBB46162270244BFAE4FBB64CDAA47C78C5B1948DD5'
...
function Get-RequiredProperty([object]$Object, [string]$Name, [string]$Context) {
    Assert-True ($null -ne $Object) "R14P missing object: $Context"
    $property = $Object.PSObject.Properties[$Name]
    Assert-True ($null -ne $property) "R14P missing property: $Context.$Name"
    return $property.Value
}

function Assert-Sha256Text([string]$Value, [string]$Context) {
    Assert-True ($Value -match '^[0-9A-F]{64}$') "R14P invalid SHA-256: $Context"
}
...
function Get-FullPath([string]$Path, [string]$Context) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($Path)) "R14P empty path: $Context"
    Assert-True ($Path.IndexOf('"') -lt 0 -and $Path.IndexOf("`r") -lt 0 -and $Path.IndexOf("`n") -lt 0) "R14P unsafe path text: $Context"
    $full = [IO.Path]::GetFullPath($Path)
    Assert-True ([IO.Path]::IsPathRooted($full)) "R14P path is not rooted: $Context"
    return $full
}
...
function Assert-PathComponents([string]$Path, [string]$Context) {
    ...
    Assert-True ($longest -le 80) "R14P path component exceeds 80 characters: $Context"
}
...
function Assert-PathBudget([string]$Path, [string]$Context, [int]$Reserve = 32) {
    ...
    Assert-True (($full.Length + $Reserve) -lt 200) "R14P unsafe effective path: $Context"
}

function Assert-CanonicalSourcePath([string]$Path, [string]$Context) {
    Assert-PathComponents $Path $Context
    Assert-True ($Path.Length -lt 230) "R14P canonical source path reaches hard stop: $Context"
}
...
function Get-SubstRows([string]$SubstPath) {
    ...
    Assert-True ($LASTEXITCODE -eq 0) ('R14P subst inventory failed: ' + $output.Trim())
...
function Assert-XAliasAbsent([string]$SubstPath) {
    ...
    Assert-True ($matching.Count -eq 0) 'R14P X: subst alias is already in use.'
    Assert-True ($null -eq (Get-PSDrive -Name X -ErrorAction SilentlyContinue)) 'R14P X: PowerShell drive is already in use.'
    Assert-True (-not (Test-Path -LiteralPath 'X:\')) 'R14P X: filesystem path is already in use.'
}

function Assert-Authority([object]$Authority, [string]$Context, [string[]]$FalseFields) {
    Assert-True ([bool](Get-RequiredProperty $Authority 'reviewOnly' $Context)) "R14P review-only authority changed: $Context"
    foreach ($field in $FalseFields) {
        Assert-True (-not [bool](Get-RequiredProperty $Authority $field $Context)) "R14P forbidden authority enabled: $Context.$field"
    }
}
...
function Assert-TargetPositions([object[]]$Actual, [object[]]$Expected, [string]$Context) {
    ...
    Assert-True ($actualRows.Count -eq $expectedRows.Count) "R14P target-position count changed: $Context"
    for ($index = 0; $index -lt $expectedRows.Count; $index++) {
        Assert-True ([int]$actualRows[$index].position -eq [int]$expectedRows[$index].position) "R14P target position changed: $Context"
        Assert-True ([string]$actualRows[$index].label -ceq [string]$expectedRows[$index].label) "R14P target label changed: $Context"
    }
}
...
function Invoke-BoundedPython([string]$Python, [string]$Provider, [string]$Job, [string]$OutputRoot, [int]$TimeoutSeconds) {
    foreach ($path in @($Python,$Provider,$Job,$OutputRoot)) {
        Assert-True ($path.IndexOf('"') -lt 0 -and $path.IndexOf("`r") -lt 0 -and $path.IndexOf("`n") -lt 0) 'R14P process argument contains unsafe text.'
    }
...
function Get-SafeArtifactPath([string]$CaseRoot, [string]$RelativePath) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($RelativePath)) 'R14P artifact relative path is empty.'
    Assert-True (-not [IO.Path]::IsPathRooted($RelativePath)) 'R14P artifact path is rooted.'
    Assert-True ($RelativePath.IndexOf(':') -lt 0 -and $RelativePath.IndexOf('"') -lt 0) 'R14P artifact path contains forbidden text.'
    $normalizedRelative = $RelativePath.Replace('/', '\')
    $segments = @($normalizedRelative.Split([char[]]@('\'), [StringSplitOptions]::RemoveEmptyEntries))
    Assert-True ($segments.Count -gt 0 -and @($segments | Where-Object { $_ -eq '..' -or $_ -eq '.' }).Count -eq 0) 'R14P artifact path traverses its case root.'
    $full = [IO.Path]::GetFullPath((Join-WindowsPath $CaseRoot $normalizedRelative))
    $prefix = [IO.Path]::GetFullPath($CaseRoot).TrimEnd('\') + '\'
    Assert-True ($full.StartsWith($prefix, [StringComparison]::OrdinalIgnoreCase)) 'R14P artifact escaped its case root.'
...
Assert-True (-not [bool]$Rehearsal -or $explicitInvocation) 'R14P rehearsal requires explicit -InvocationManifest.'
if ($explicitInvocation) {
    ...
    $longDefault = Join-Path $PSScriptRoot 'R14P_LIVE_INVOCATION.json'
    $invocationPath = if (Test-Path -LiteralPath $shortDefault -PathType Leaf) { $shortDefault } else { $longDefault }
}
Assert-True (Test-Path -LiteralPath $invocationPath -PathType Leaf) "R14P invocation manifest absent: $invocationPath"
Assert-True ((Get-Item -LiteralPath $invocationPath).Length -le 262144) 'R14P invocation manifest exceeds 256 KiB.'
...
Assert-True ([string]$invocation.schema -eq 'argos_opencv_scribe_alphabet_crop_live_invocation_v1') 'R14P invocation schema changed.'
Assert-True ([string]$invocation.revision -eq $revision -and [string]$invocation.requestId -eq $requestId) 'R14P invocation identity changed.'
Assert-True ([string]$invocation.state -eq 'FROZEN') 'R14P invocation is not frozen.'
...
Assert-True ([int]$invocation.authority.maximumPublications -eq 1) 'R14P publication count changed.'

$expectedComputerName = [string]$invocation.expectedComputerName
Assert-True (-not [string]::IsNullOrWhiteSpace($expectedComputerName)) 'R14P expected computer is absent.'
if (-not $Rehearsal) { Assert-True ($expectedComputerName -eq $liveComputerName) 'R14P live computer contract changed.' }
Assert-True ($env:COMPUTERNAME.Equals($expectedComputerName, [StringComparison]::OrdinalIgnoreCase)) "R14P wrong computer: $($env:COMPUTERNAME)"
...
Assert-True ($pythonPath.Equals((Join-WindowsPath $runtimeRoot 'python.exe'), [StringComparison]::OrdinalIgnoreCase)) 'R14P Python is outside the exact runtime root.'
...
if ($Rehearsal) {
    Assert-True ($runtimeInstallationSha256 -eq $rehearsalRuntimeInstallationSha256) 'R14P rehearsal runtime installation fixture pin changed.'
}
else {
    Assert-True ($runtimeInstallationSha256 -eq $liveRuntimeInstallationSha256) 'R14P live runtime installation pin changed.'
}
Assert-True ([string]$invocation.referenceBundle.sha256 -eq $referenceBundleSha256) 'R14P reference bundle pin changed.'
Assert-True ([string]$invocation.referenceBundle.manifestSha256 -eq $referenceManifestSha256 -and [int]$invocation.referenceBundle.referenceCount -eq 456 -and [string]$invocation.referenceBundle.missingLabels -eq 'IJKOQVWXYZ') 'R14P reference contract changed.'
foreach ($path in @($pythonPath,$installationPath,$referenceBundlePath)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "R14P dependency absent: $path"
    Assert-PathBudget $path 'runtime/reference dependency' 32
}
Assert-True ((Get-Sha256 $installationPath) -eq $runtimeInstallationSha256) 'R14P runtime installation evidence changed.'
Assert-True ((Get-Sha256 $referenceBundlePath) -eq $referenceBundleSha256) 'R14P reference bundle bytes changed.'
$pythonCommand = Get-Command -Name $pythonPath -CommandType Application -ErrorAction Stop
Assert-True ([IO.Path]::GetFullPath($pythonCommand.Source).Equals($pythonPath, [StringComparison]::OrdinalIgnoreCase)) 'R14P Python command resolution changed.'

$substPath = Join-Path $env:SystemRoot 'System32\subst.exe'
Assert-True (Test-Path -LiteralPath $substPath -PathType Leaf) 'R14P subst.exe is absent.'
$substCommand = Get-Command -Name $substPath -CommandType Application -ErrorAction Stop
Assert-True ([IO.Path]::GetFullPath($substCommand.Source).Equals([IO.Path]::GetFullPath($substPath), [StringComparison]::OrdinalIgnoreCase)) 'R14P subst command resolution changed.'
...
Assert-True ([string]$invocation.roots.sourceAlias -eq 'X:') 'R14P source alias changed.'
Assert-True ($workPartial.Equals(($workRoot + '.partial'), [StringComparison]::OrdinalIgnoreCase) -and $workFailed.Equals(($workRoot + '.failed'), [StringComparison]::OrdinalIgnoreCase)) 'R14P work lifecycle roots changed.'
Assert-True ($outputPartial.Equals(($outputRoot + '.partial'), [StringComparison]::OrdinalIgnoreCase)) 'R14P output lifecycle root changed.'
if (-not $Rehearsal) {
    Assert-True ($workRoot -eq 'D:\A2\w\ocv\R14P' -and $outputRoot -eq 'D:\A2\o\ocv\R14P' -and $bundlePath -eq 'D:\A2\o\ocv\R14P_RETURN.zip') 'R14P live roots changed.'
    foreach ($path in @($workRoot,$workPartial,$workFailed,$outputRoot,$outputPartial,$bundlePath,$bundlePartial)) {
        Assert-True ($path.StartsWith('D:\', [StringComparison]::OrdinalIgnoreCase)) 'R14P live create-new path left D:.'
    }
}
Assert-True ([IO.Path]::GetPathRoot($workRoot).Equals([IO.Path]::GetPathRoot($workPartial), [StringComparison]::OrdinalIgnoreCase)) 'R14P work atomic move crosses volumes.'
Assert-True ([IO.Path]::GetPathRoot($outputRoot).Equals([IO.Path]::GetPathRoot($outputPartial), [StringComparison]::OrdinalIgnoreCase)) 'R14P output atomic move crosses volumes.'
foreach ($path in @($workRoot,$workPartial,$workFailed,$outputRoot,$outputPartial,$bundlePath,$bundlePartial)) {
    Assert-PathBudget $path 'create-new root' 32
    Assert-True (-not (Test-Path -LiteralPath $path)) "R14P create-new root exists: $path"
}
foreach ($parent in @((Split-Path -Parent $workRoot),(Split-Path -Parent $outputRoot),(Split-Path -Parent $bundlePath))) {
    Assert-True (Test-Path -LiteralPath $parent -PathType Container) "R14P required parent root absent: $parent"
}

Assert-True ([int]$invocation.execution.maximumSequentialProviderChildren -eq 4 -and [int]$invocation.execution.maximumConcurrentProviderChildren -eq 1) 'R14P provider serialization changed.'
Assert-True ([int]$invocation.execution.providerChildTimeoutSeconds -eq 600 -and [int]$invocation.execution.endpointWorkerOuterTimeoutSeconds -eq 3000 -and -not [bool]$invocation.execution.automaticRetryAllowed -and -not [bool]$invocation.execution.caseFailureStopsBatch) 'R14P provider timeout/retry contract changed.'
Assert-True ([bool]$invocation.execution.sourceHashBeforeOpenCvDecodeRequired -and [bool]$invocation.execution.sourceImagesProcessedReadOnlyInPlace -and -not [bool]$invocation.execution.fullSourceImageReturnAllowed) 'R14P source-read contract changed.'
Assert-True ([int64]$invocation.outputContract.maximumBundleBytes -eq $maximumBundleBytes -and [int]$invocation.outputContract.maximumBundleBase64Characters -eq $maximumBundleBase64Characters) 'R14P bundle bounds changed.'
Assert-True ([string]$invocation.outputContract.portalResponseTransport -eq 'MAINTENANCE_STDOUT_COMPACT_JSON_BASE64' -and [bool]$invocation.outputContract.sourceToGridToCellToTargetSha256Required) 'R14P output provenance contract changed.'

$providerPin = $invocation.payload.provider
$configurationPin = $invocation.payload.configuration
Assert-True ([string]$providerPin.file -eq 'R14P.py' -and [string]$configurationPin.file -eq 'CFG.json') 'R14P short payload filenames changed.'
...
if (-not $Rehearsal) { Assert-True ([string]$providerPin.sha256 -eq $liveProviderSha256) 'R14P live provider pin changed.' }
Assert-True ([string]$configurationPin.sha256 -eq $configurationSha256) 'R14P configuration pin changed.'
...
foreach ($expectedDependency in $dependencyExpectations) {
    ...
    Assert-True ([string]$dependency.file -eq [string]$expectedDependency.file -and [string]$dependency.sha256 -eq [string]$expectedDependency.sha256) "R14P dependency pin changed: $($expectedDependency.id)"
...
foreach ($pin in $payloadPins) {
    Assert-True (Test-Path -LiteralPath $pin.path -PathType Leaf) "R14P payload absent: $($pin.id)"
    Assert-PathBudget $pin.path "payload $($pin.id)" 32
    Assert-True ((Get-Sha256 $pin.path) -eq $pin.sha256) "R14P payload hash changed: $($pin.id)"
}
$configuration = Get-Content -Raw -LiteralPath $configurationSource | ConvertFrom-Json
Assert-True ([string]$configuration.schema -eq 'argos_opencv_scribe_alphabet_crop_configuration_v1' -and [string]$configuration.revision -eq $revision) 'R14P configuration contract changed.'
...
Assert-True ($invocationCases.Count -eq 4) 'R14P exact case count changed.'
...
for ($caseIndex = 0; $caseIndex -lt $expectedCases.Count; $caseIndex++) {
    ...
    foreach ($field in @('caseId','physicalIdentity','purpose','confirmedScribe','aliasAnchorCanonicalPath','bfRelativePath','dfRelativePath')) {
        Assert-True ([string]$actualCase.$field -ceq [string]$expectedCase.$field) "R14P case descriptor changed: $($expectedCase.caseId)/$field"
    }
    $caseSourceBytes = [int64]$actualCase.sourceBytesPerImage
    if ($Rehearsal) {
        Assert-True ($caseSourceBytes -ge 1 -and $caseSourceBytes -le $maximumRehearsalSourceBytesPerImage) "R14P rehearsal source-byte fixture bound changed: $($expectedCase.caseId)"
    }
    else {
        Assert-True ($caseSourceBytes -eq $liveSourceBytesPerImage) "R14P live source-byte contract changed: $($expectedCase.caseId)"
    }
    Assert-True (-not [bool]$actualCase.referenceAdmissionEligible) "R14P case admission contract changed: $($expectedCase.caseId)"
    ...
    if ($null -ne $sourceRootProperty -and -not [string]::IsNullOrWhiteSpace([string]$sourceRootProperty.Value)) {
        Assert-True ([bool]$Rehearsal -or ([string]$sourceRootProperty.Value -eq [string]$expectedCase.aliasAnchorCanonicalPath)) "R14P live source-root override refused: $($expectedCase.caseId)"
        $sourceRoot = Get-FullPath ([string]$sourceRootProperty.Value) "source root $($expectedCase.caseId)"
    }
    Assert-True (Test-Path -LiteralPath $sourceRoot -PathType Container) "R14P source root absent: $($expectedCase.caseId)"
    ...
    foreach ($sourceSpec in @([pscustomobject]@{channel='bf';path=$bfCanonical;relative=[string]$expectedCase.bfRelativePath},[pscustomobject]@{channel='df';path=$dfCanonical;relative=[string]$expectedCase.dfRelativePath})) {
        Assert-True (Test-Path -LiteralPath $sourceSpec.path -PathType Leaf) "R14P source absent: $($expectedCase.caseId)/$($sourceSpec.channel)"
        $item = Get-Item -LiteralPath $sourceSpec.path
        Assert-True ([int64]$item.Length -eq $caseSourceBytes) "R14P source byte metadata changed: $($expectedCase.caseId)/$($sourceSpec.channel)"
        Assert-True (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0) "R14P source is a reparse point: $($expectedCase.caseId)/$($sourceSpec.channel)"
...
if ($Preflight) {
    [ordered]@{
        schema = 'argos_opencv_scribe_r14p_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R14P_STRICTLY_NONMUTATING_PREFLIGHT'
        revision = $revision
        requestId = $requestId
...
try {
    ...
    foreach ($pin in $payloadPins) {
        ...
        Assert-True ((Get-Sha256 (Join-Path $workPartial $pin.file)) -eq $pin.sha256) "R14P staged payload changed: $($pin.id)"
    }
    Copy-Item -LiteralPath $invocationPath -Destination (Join-Path $workPartial 'LIVE.json')
    Assert-True ((Get-Sha256 (Join-Path $workPartial 'LIVE.json')) -eq $invocationSha256) 'R14P staged live manifest changed.'
    $stagedReferenceManifest = Join-Path $workPartial 'refs\PORTABLE_GLYPH_REFERENCE_MANIFEST.json'
    Assert-True (Test-Path -LiteralPath $stagedReferenceManifest -PathType Leaf) 'R14P extracted reference manifest is absent.'
    Assert-True ((Get-Sha256 $stagedReferenceManifest) -eq $referenceManifestSha256) 'R14P extracted reference manifest changed.'
    foreach ($relativeRoot in @('refs\glyphs','refs\glyphs_v5_confirmed_20260806')) {
        Assert-True (Test-Path -LiteralPath (Join-Path $workPartial $relativeRoot) -PathType Container) "R14P extracted reference root absent: $relativeRoot"
    }
    ...
    foreach ($entry in $validatedCases) {
        ...
        foreach ($path in @($caseOutput,$caseOutputPartial,$caseAttemptQuarantine,$runtimeJobPath)) {
            Assert-True (-not (Test-Path -LiteralPath $path)) "R14P case create-new path exists: $caseId/$path"
        ...
        $caseState = 'HOLD_R14P_CASE_LAUNCH_FAILURE'
        $caseDetail = ''
        $artifactRows = @()
        try {
            Assert-XAliasAbsent $substPath
            $createOutput = & $substPath 'X:' ([string]$entry.sourceRoot) 2>&1 | Out-String
            Assert-True ($LASTEXITCODE -eq 0 -and (Test-Path -LiteralPath 'X:\' -PathType Container)) ('R14P alias creation failed: ' + $createOutput.Trim())
            $aliasCreated = $true
            $mappingRows = @(Get-SubstRows $substPath)
            $matchingRows = @($mappingRows | Where-Object { $_ -match '(?i)^\s*X:\\:\s*=>\s*(.+?)\s*$' })
            Assert-True ($matchingRows.Count -eq 1) 'R14P alias mapping cardinality changed.'
            $mappedTarget = [regex]::Match($matchingRows[0], '(?i)^\s*X:\\:\s*=>\s*(.+?)\s*$').Groups[1].Value.Trim().TrimEnd('\')
            Assert-True ($mappedTarget.Equals(([string]$entry.sourceRoot).TrimEnd('\'), [StringComparison]::OrdinalIgnoreCase)) "R14P alias target changed: $caseId"
            ...
            foreach ($sourcePath in @($bfAliasPath,$dfAliasPath)) {
                Assert-True (Test-Path -LiteralPath $sourcePath -PathType Leaf) "R14P alias source absent: $caseId"
                Assert-True ([int64](Get-Item -LiteralPath $sourcePath).Length -eq [int64]$entry.sourceBytes) "R14P alias source byte metadata changed: $caseId"
            }
            ...
            $providerRun = Invoke-BoundedPython -Python $pythonPath -Provider (Join-Path $workRoot 'R14P.py') -Job $runtimeJobPath -OutputRoot $caseOutput -TimeoutSeconds 600
            Assert-True ([bool]$providerRun.started) "R14P provider launch failed: $caseId`: $($providerRun.launchError)"
            Assert-True (-not [bool]$providerRun.timedOut) "R14P provider timed out: $caseId"
            Assert-True ($null -ne $providerRun.exitCode -and [int]$providerRun.exitCode -in @(0,2)) "R14P provider exit code changed: $caseId`: $($providerRun.stderr)"
            Assert-True (Test-Path -LiteralPath $caseResultPath -PathType Leaf) "R14P CASE_RESULT absent: $caseId"

            $caseResult = Get-Content -Raw -LiteralPath $caseResultPath | ConvertFrom-Json
            Assert-True ([string]$caseResult.schema -eq 'argos_opencv_scribe_alphabet_crop_case_result_v1' -and [string]$caseResult.revision -eq $providerRevision) "R14P case-result contract changed: $caseId"
            Assert-True ([string]$caseResult.caseId -eq $caseId -and [string]$caseResult.physicalIdentity -eq [string]$entry.expected.physicalIdentity) "R14P case-result identity changed: $caseId"
            ...
            Assert-True ([string]$caseResult.provenance.sources.bf.sha256 -eq $bfSha256 -and [string]$caseResult.provenance.sources.df.sha256 -eq $dfSha256) "R14P case-result source hashes changed: $caseId"
            Assert-True ([string]$caseResult.provenance.sourcePairSha256 -eq $sourcePairSha256) "R14P case-result source-pair hash changed: $caseId"
            $artifactRows = @($caseResult.artifacts)
            Assert-True ($artifactRows.Count -le 15) "R14P artifact count exceeds contract: $caseId"
            if ($artifactRows.Count -eq 0) {
                Assert-True ([string]$caseResult.state -like 'HOLD_*') "R14P empty-artifact result is not a hold: $caseId"
                ...
                Assert-True ([int]$providerRun.exitCode -eq 0) "R14P nonzero provider exit returned artifacts: $caseId"
                ...
                Assert-True ($gridRows.Count -eq 1 -and $cellRows.Count -eq 12 -and $targetRows.Count -eq @($entry.expected.targetPositions).Count) "R14P evaluable artifact cardinality changed: $caseId"
                Assert-True ([string]$gridRows[0].relativePath -eq 'selected_grid.png') "R14P grid filename changed: $caseId"
                Assert-True ([string]$gridRows[0].sourceChannel -eq 'BF_DF_DERIVED' -and [string]$gridRows[0].sourceSha256 -eq $sourcePairSha256) "R14P grid source binding changed: $caseId"
                ...
                for ($position = 1; $position -le 12; $position++) {
                    ...
                    Assert-True ($cell.Count -eq 1 -and [string]$cell[0].sourceChannel -eq 'SELECTED_GRID' -and [string]$cell[0].sourceSha256 -eq $gridSha256) "R14P cell lineage changed: $caseId/P$position"
                }
                foreach ($targetPosition in @($entry.expected.targetPositions)) {
                    ...
                    Assert-True ($target.Count -eq 1 -and [string]$target[0].sourceChannel -eq 'AUDIT_CELL' -and [string]$target[0].sourceSha256 -eq [string]$cell[0].sha256) "R14P target lineage changed: $caseId/$($targetPosition.label)"
                }
                foreach ($artifact in $artifactRows) {
                    ...
                    Assert-True (Test-Path -LiteralPath $artifactPath -PathType Leaf) "R14P artifact absent: $caseId/$($artifact.relativePath)"
                    Assert-True ((Get-Sha256 $artifactPath) -eq [string]$artifact.sha256) "R14P artifact hash changed: $caseId/$($artifact.relativePath)"
                }
            }
            ...
            Assert-True ($actualFiles.Count -le 16) "R14P case file count exceeds contract: $caseId"
            foreach ($file in $actualFiles) {
                Assert-True ($file.Extension -in @('.json','.png')) "R14P forbidden case output type: $caseId/$($file.Name)"
                Assert-True ([int64]$file.Length -le $maximumBundleBytes) "R14P case output exceeds bundle bound: $caseId/$($file.Name)"
                if (-not $file.FullName.Equals($caseResultPath, [StringComparison]::OrdinalIgnoreCase)) {
                    ...
                    Assert-True ($declaredPaths -contains $relative) "R14P undeclared case output: $caseId/$relative"
                }
            }
            ...
            if ($aliasCreated) {
                ...
                if ($LASTEXITCODE -ne 0 -or (Test-Path -LiteralPath 'X:\')) { $aliasCleanupFailure = 'R14P X: alias removal failed: ' + $removeOutput.Trim() }
                $aliasCreated = $false
            }
        }
    ...
    Assert-True ($processorAfterJson -ceq $processorBeforeJson) 'R14P protected processor rows changed.'
    $failureCount = @($caseRows.ToArray() | Where-Object { [string]$_.state -like 'HOLD_R14P_CASE_LAUNCH_FAILURE' }).Count
    ...
    $batchGate = [ordered]@{
        schema='argos_opencv_scribe_r14p_batch_gate_v1';revision=$revision;createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R14P_BATCH_COMPLETE';disposition='PENDING_GATE';requestId=$requestId;rehearsal=[bool]$Rehearsal
    ...
    Write-JsonNew $executionPath ([ordered]@{
        schema='argos_opencv_scribe_r14p_execution_v1';revision=$revision;createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R14P_EXECUTION';requestId=$requestId;computerName=$env:COMPUTERNAME;rehearsal=[bool]$Rehearsal
    ...
    Assert-True ($bundleBytes -le $maximumBundleBytes) "R14P return bundle exceeds $maximumBundleBytes bytes."
    ...
    Assert-True ($bundleBase64.Length -le $maximumBundleBase64Characters) "R14P return Base64 exceeds $maximumBundleBase64Characters characters."
    [ordered]@{
        schema='argos_opencv_scribe_r14p_maintenance_envelope_v1';revision=$revision;state='PASS_R14P_SIGNED_RETURN_READY';requestId=$requestId;rehearsal=[bool]$Rehearsal
