...
function Test-BundleBytes([byte[]]$BundleBytes, [object]$Envelope, [object]$Limits, [string[]]$ExpectedCaseIds) {
    ...
    try {
        ...
        foreach ($archiveEntry in $allEntries) {
            ...
            [void](Get-SafeChildPath 'C:\R14R4\b' $archiveName)
        }
        ...
        foreach ($caseId in $ExpectedCaseIds) {
            ...
            $caseBytes = Read-ZipEntryBytes $archive ($caseId + '/CASE_RESULT.json') ([int64]$Limits.maximumCaseResultJsonBytes) 'case result'
...
try {
    ...
    Assert-True ($shortRoot -eq 'C:\R14R4') 'R14P short extraction root changed.'
