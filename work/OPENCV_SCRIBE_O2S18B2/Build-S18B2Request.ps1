...
$definitionPath = Join-Path $PSScriptRoot 'S18B2_DATA_PULL_DEFINITION.json'
$intentPath = Join-Path $PSScriptRoot 'S18B2_RECOVERY_INTENT.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_S18B2_BUILD.json'
...
$terminalPath = Join-Path $project 'work\OPENCV_SCRIBE_O2S18B1\S18B1_TERMINAL_RESPONSE_GATE.json'
...
$expectedDefinitionSha256 = '1AA3BF7118D17B0E1C96D91617055F16682512DFC18F046820570D1EB814A4AE'
$expectedIntentSha256 = 'D72F67CB22494366ED65A0CD11BEF31C6FD90CA4EA433680962773D78AECA596'
$expectedPreactionSha256 = '919A97635BDA9AD780372F4C2452A4A36EC36DB5B8635D8535ADA5564DCF2430'
...
$expectedTerminalSha256 = 'DE799E4183DEB75E9CEA1D82D13A8F8E86CE00D785D6A4AA1C236269924636B3'
...
function Assert-Pin([string]$Path, [string]$Sha256) {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "S18B2 dependency is absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "S18B2 dependency changed: $Path"
}
...
foreach ($path in @($identityPath,$publicCertificatePath,$packageTester,$intentTool,$preactionTool,$pathTool)) { Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "S18B2 input is absent: $path" }
...
$expectedRelativePaths = @(
    'identity/proposals/62619-433_20260824005735_Slot18/scribe/BF_SCRIBE_ORIENTED_DETECTOR_INPUT.png',
    'identity/proposals/62619-433_20260824005735_Slot18/scribe/DF_SCRIBE_ORIENTED_DETECTOR_INPUT.png'
)
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'DATA_PULL' -and [string]$definition.parameters.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'S18B2 DATA_PULL route changed.'
Assert-True ($relativePaths.Count -eq 2 -and @(Compare-Object -ReferenceObject $expectedRelativePaths -DifferenceObject $relativePaths).Count -eq 0) 'S18B2 exact relative path set changed.'
Assert-True ([int]$definition.parameters.maximumFiles -eq 2 -and [int64]$definition.parameters.maximumBytes -eq 4194304 -and [int64]$definition.maxResultBytes -eq 4194304) 'S18B2 bounds changed.'
...
Assert-True ([string]$routeHealth.state -eq 'PASS_O2D12_COMPLETE_ROUTE_GATE' -and [bool]$routeHealth.publicationAuthorized -and -not [bool]$routeHealth.retryAuthorized) 'S18B2 inherited route health changed.'
Assert-True ([int]$routeHealth.unresolvedEarlierAcceptedRequestCount -eq 0 -and [bool]$routeHealth.argosInboundRelayCurrentHealthProved) 'S18B2 route has unresolved earlier work.'
Assert-True ([string]$routeHealth.endpointWorkerSha256 -eq $expectedWorkerSha256 -and [string]$routeHealth.installedEndpointConfigSha256 -eq $expectedConfigSha256) 'S18B2 installed route pins changed.'
Assert-True ([string]$terminal.state -eq 'PASS_S18B1_SIGNED_SLOT18_SOURCE_METADATA_COLLECTED_HASH_BINDING_PENDING' -and [bool]$terminal.signedResponseVerified) 'S18B2 predecessor terminal evidence changed.'
...
Assert-True ($currentBranch -eq $branch -and $localTip -eq $remoteTip) 'S18B2 requires matching local/origin branch tips.'

foreach ($path in @($partialRoot,$signedRoot,$finalRoot,$packageGatePath,$routeGatePath,$pathSidecar)) { Assert-True (-not (Test-Path -LiteralPath $path)) "S18B2 fresh output exists: $path" }
...
$routePaths = @(
    ...
    'C:\S18B2R.zip',
    'C:\S18B2R\PORTAL_RESPONSE_MANIFEST.json',
    'C:\S18B2R\DATA_PULL_PAYLOAD.zip',
    ('C:\S18B2D\' + $longestRelative)
)
$pathGate = & $pathTool -CandidatePath $routePaths -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'S18B2 complete route path budget failed.'
...
Assert-True ([bool]$certificate.HasPrivateKey) 'S18B2 signer private key is unavailable.'

if ($Preflight) {
    [ordered]@{schema='argos_s18b2_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S18B2_BUILD_PREFLIGHT';requestId=$requestId;relativePathCount=2;maximumFiles=2;maximumBytes=4194304;routePathCount=$routePaths.Count;maximumEffectiveLength=$maximumEffective;maximumComponentLength=$maximumComponent;branch=$branch;localTip=$localTip;remoteTip=$remoteTip;mutationsPerformed=$false;jbodContacted=$false;imageBytesRequested=2;pixelsDecoded=$false;slots22Through25Exposed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
Assert-True ([string]$folderTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'S18B2 signed folder validation failed.'
...
Assert-True ([string]$extractTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE' -and @(Get-ChildItem -LiteralPath $extractRoot -File).Count -eq 2) 'S18B2 exact ZIP extraction validation failed.'
...
[ordered]@{schema='argos_s18b2_build_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S18B2_EXACT_SIGNED_DATA_PULL_READY';requestId=$requestId;requestZip=$finalZip;requestZipBytes=$zipBytes;requestZipSha256=$zipSha256;packageGate=$packageGatePath;routeGate=$routeGatePath;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
