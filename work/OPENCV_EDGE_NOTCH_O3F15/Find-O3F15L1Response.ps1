...
if(-not$Preflight){throw 'O3F15L1 response discovery is preflight-only.'}
...
foreach($path in @($signGatePath,$publishGatePath)){Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L1 response-discovery dependency absent: $path"}
...
Require ([string]$publishGate.state-eq'PASS_O3F15L1_PUBLISHED_EXACTLY_ONCE_AWAITING_SIGNED_LAUNCH_RESPONSE'-and[string]$publishGate.requestId-eq$requestId-and[string]$publishGate.publishedSha256-eq[string]$signGate.packageZipSha256) 'O3F15L1 publication gate changed.'
...
Require (Test-Path -LiteralPath $root -PathType Container) 'O3F15L1 response root unavailable.'
...
Require ($candidates.Count-le20) 'O3F15L1 post-publication response cohort exceeded 20 ZIPs.'
