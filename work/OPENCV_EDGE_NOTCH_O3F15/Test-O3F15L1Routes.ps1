...
function Required([object]$Object,[string]$Name){$property=$Object.PSObject.Properties[$Name];if($null-eq$property){throw "O3F15L1 required route property absent: $Name"};$property.Value}
function New-Json([string]$Path,[object]$Value){Require (-not(Test-Path -LiteralPath $Path)) "O3F15L1 create-new route gate exists: $Path";[IO.File]::WriteAllText($Path,(($Value|ConvertTo-Json -Depth 20)+[Environment]::NewLine),(New-Object Text.UTF8Encoding($false)))}
...
$outputPath=Join-Path $finalRoot 'O3F15L1_PREPUBLICATION_PATH_GATE.json'
foreach($path in @($contractPath,$signGatePath)){Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F15L1 route dependency absent: $path"}
$contract=Get-Content -LiteralPath $contractPath -Raw|ConvertFrom-Json
$signGate=Get-Content -LiteralPath $signGatePath -Raw|ConvertFrom-Json
Require ([string]$contract.schema-eq'argos_ocv03_o3f15l1_launch_contract_v1'-and[string]$contract.state-eq'FROZEN_FOR_BUILD') 'O3F15L1 route contract is not frozen.'
Require ([string]$signGate.state-eq'PASS_O3F15L1_SIGNED_EXACT_978_FRONT_LAUNCH_PACKAGE') 'O3F15L1 route sign gate changed.'
Require ([IO.Path]::GetFullPath([string]$signGate.finalRoot)-eq[IO.Path]::GetFullPath($finalRoot)-and[IO.Path]::GetDirectoryName([IO.Path]::GetFullPath([string]$signGate.packageZipPath))-eq[IO.Path]::GetFullPath($finalRoot)) 'O3F15L1 final ZIP is not in the path-gate directory.'
...
Require ([int](Required $contract 'expectedPairCount')-eq978-and[int](Required $route 'canonicalSourceLeafCount')-eq1956-and[int](Required $route 'aliasSourceLeafCount')-eq1956) 'O3F15L1 route source cardinality changed.'
...
Require ($suffixReserve-eq32-and$maximumCanonicalLength-eq197-and$maximumAliasLength-eq167-and$maximumCanonicalEffectiveLength-eq229-and$maximumAliasEffectiveLength-eq199) 'O3F15L1 source path hard bounds changed.'
Require ($maximumCanonicalEffectiveLength-eq($maximumCanonicalLength+$suffixReserve)-and$maximumAliasEffectiveLength-eq($maximumAliasLength+$suffixReserve)-and$maximumGeneratedComponentLength-le80) 'O3F15L1 path reserve/component arithmetic changed.'
Require ($representativeCanonical.Replace('\','/').StartsWith('D:/KLARFExport/',[StringComparison]::OrdinalIgnoreCase)-and$representativeAlias.Replace('\','/').StartsWith('Q:/',[StringComparison]::OrdinalIgnoreCase)) 'O3F15L1 representative source route roots changed.'
Require ($representativeCanonical.Length-le$maximumCanonicalLength-and$representativeAlias.Length-le$maximumAliasLength) 'O3F15L1 representative source path exceeded its declared hard bound.'
Require ($runFixedLeafCount-eq10-and$runPerCaseLeafCount-eq16-and$runCaseCount-eq978-and$runEnumeratedLeafCount-eq($runFixedLeafCount+($runPerCaseLeafCount*$runCaseCount))-and$runEnumeratedLeafCount-eq15658) 'O3F15L1 RUN leaf accounting changed.'
Require ($maximumCorpusPathLength-eq105-and$maximumCorpusEffectivePathLength-eq137-and$maximumCorpusEffectivePathLength-eq($maximumCorpusPathLength+$suffixReserve)-and[string](Required $route 'maximumGeneratedLeaf')-eq('D:/O3F15C/'+$maximumCorpusRelative.TrimStart('/','\'))) 'O3F15L1 corpus path hard bound changed.'
Require ($mirrorFinalLeafCount-eq4-and$maximumMirrorLeafBytesExclusive-eq2097152-and[string](Required $route 'maximumAliasLeaf')-eq$representativeAlias-and[string](Required $route 'maximumMirrorRelativeLeaf')-eq'TERMINAL_FAILURE.json.partial') 'O3F15L1 result/mirror maximum contract changed.'
$inheritedRoute=Required $contract 'inheritedRoute'
Require ([string](Required $inheritedRoute 'endpointWorkerSha256')-eq'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250'-and[string](Required $inheritedRoute 'installedRouteConfigEvidenceSha256')-eq'465107D861E8D2C376A419B1C15841BE3A5757C6498C36CA56B9A95A2F0B76DB'-and[string](Required $inheritedRoute 'queueSafetyGateSha256')-eq'170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D'-and-not[bool](Required $inheritedRoute 'routeImplementationChanged')) 'O3F15L1 inherited portal route pins changed.'

$packagePath=[string]$signGate.packagePath
$manifestPath=Join-Path $packagePath 'PORTAL_REQUEST_MANIFEST.json'
Require (Test-Path -LiteralPath $manifestPath -PathType Leaf) 'O3F15L1 signed manifest absent.'
$manifest=Get-Content -LiteralPath $manifestPath -Raw|ConvertFrom-Json
Require ([string]$manifest.requestId-eq$requestId-and@($manifest.files).Count-eq[int]$signGate.payloadFileCount) 'O3F15L1 signed route manifest changed.'
...
foreach($path in @(
    ...
    "U:/ProjectPortalRO/responses/$responseName.zip","C:/O3F15L1C/$responseName.zip",
    "C:/O3F15L1C/$responseName/PORTAL_RESPONSE_MANIFEST.json","C:/O3F15L1C/$responseName/MAINTENANCE.stdout.txt"
...
Require ([string]$actionableGate.state-eq'PASS_PATH_BUDGET'-and@($actionableGate.candidates).Count-eq$actionable.Count) 'O3F15L1 actionable route path gate failed.'
...
Require ($canonicalExit-le1-and[string]$canonicalGate.state-in@('PASS_PATH_BUDGET','SHORT_ALIAS_REQUIRED_BEFORE_WRITE_OR_LAUNCH')) 'O3F15L1 representative canonical source path crossed a hard path stop.'
