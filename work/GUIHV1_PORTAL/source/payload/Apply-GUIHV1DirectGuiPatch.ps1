...
$targetSpecs = @(
    [pscustomobject]@{
        ...
        predecessorName = '0'
        targetSha256 = '73C2289B58F6F6B23DD2FA12E847AFF171B3FAC45153202E93EE00E0B7533FBA'
        approvedPredecessors = @(
            'F73AC7C38139098B079A23CC9C7653A93D7464BE1D4376708B25B3346B704E5D',
...
function Assert-PathBudget {
    ...
    if (($fullPath.Length + 32) -ge 200 -or $maximumComponentLength -gt 80) {
        throw "GUIHV1 path budget refused: $fullPath"
    }
...
elseif ($Preflight -and -not [string]::IsNullOrWhiteSpace($InvocationManifest)) {
    ...
    $fixturePath = [Environment]::GetEnvironmentVariable('ARGOS_GUIHV1_REHEARSAL_MANIFEST', 'Process')
}
...
if ($isFixture) {
    ...
    if ([string]$fixture.schema -ne 'argos_guihv1_direct_patch_rehearsal_v1' -or -not [bool]$fixture.rehearsal) {
        throw 'GUIHV1 rehearsal fixture changed.'
    }
...
$payloadRows = @(
    foreach ($spec in $targetSpecs) {
        ...
        if (-not (Test-Path -LiteralPath $payloadPath -PathType Leaf)) {
            throw "GUIHV1 payload is missing: $($spec.payloadName)"
        }
        if ((Get-Sha256 -Path $payloadPath) -ne [string]$spec.targetSha256) {
            throw "GUIHV1 payload hash changed: $($spec.payloadName)"
        }

        $destination = Assert-PathBudget -Path (Join-Path $script:InstallRoot $spec.relativePath)
        if (-not (Test-Path -LiteralPath $destination -PathType Leaf)) {
            throw "GUIHV1 destination is missing: $($spec.relativePath)"
        }

        $installedSha256 = Get-Sha256 -Path $destination
        if (@($spec.approvedPredecessors) -notcontains $installedSha256) {
            throw "GUIHV1 destination predecessor is unapproved: $($spec.relativePath)"
        }
...
$preflightResult = [ordered]@{
    schema = 'argos_guihv1_direct_gui_patch_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_GUIHV1_DIRECT_GUI_PATCH_PREFLIGHT'
    requestId = $requestId
    rehearsal = $isFixture
...
try {
    ...
    if ($isFixture -and [bool](Get-OptionalValue -Object $fixture -Name 'failAfterTargetVerification' -Default $false)) {
        throw 'INJECTED_GUIHV1_POST_TARGET_FAILURE'
    }
    ...
    if (
        ...
        throw 'Healthy processor changed during GUIHV1.'
    }
    ...
    foreach ($row in $priorRows) {
        if ((Get-Sha256 -Path $row.destination) -ne [string]$row.targetSha256) {
            throw "GUIHV1 target changed after tray activation: $([string]$row.relativePath)"
        }
    }

    [ordered]@{
        schema = 'argos_guihv1_direct_gui_patch_result_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_GUIHV1_DIRECT_GUI_PATCH_AND_TRAY_ACTIVATION'
        requestId = $requestId
        rehearsal = $isFixture
        targetFiles = @(
            $priorRows | ForEach-Object {
    ...
    $failureState = if ($rollbackVerified) {
        'FAIL_GUIHV1_DIRECT_GUI_PATCH_ROLLED_BACK_BEFORE_TRAY_RESTART'
    }
    else {
        'FAIL_GUIHV1_DIRECT_GUI_PATCH_RECOVERY_FAILED'
    }

    [ordered]@{
        schema = 'argos_guihv1_direct_gui_patch_failure_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = $failureState
        requestId = $requestId
        rehearsal = $isFixture
    ...
    if ($rollbackVerified) {
        throw "GUIHV1 failed; all endpoint predecessors were restored before tray relaunch. $originalFailure"
    }

    throw "GUIHV1 failed and recovery failed. Original: $originalFailure Recovery: $recoveryFailure"
}
