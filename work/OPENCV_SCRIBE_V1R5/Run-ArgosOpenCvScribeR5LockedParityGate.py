#!/usr/bin/env python3
"""Run locked, review-only R5 scribe recognition and semantic gates."""
...
def load_engine(path: Path):
    specification = importlib.util.spec_from_file_location("argos_opencv_scribe_r5", path)
    if specification is None or specification.loader is None:
        raise RuntimeError("R5 engine module could not be loaded.")
...
def main() -> int:
    ...
    engine_path = script_path.with_name("ArgosOpenCvScribeV1R5.py")
    ...
    semantic_job = {
        "jobId": "R5_NEGATIVE_UNQUALIFIED_EXCEPTION_TEXTURE",
        "search": {
            "expectedRegions": [],
    ...
    auto_job = {
        "jobId": "R5_POSITIVE_REVIEW_ONLY_AUTO_LOCALIZED_DEVELOPMENT",
        ...
        "search": {
            ...
            "maximumCandidates": 12,
            "orientationStepDegrees": 15,
            "developmentMaximumRegions": 2,
            "developmentMinimumLocalizationScore": 0.0,
            "developmentMinimumBandWidthPixels": 500,
            "developmentOcrRegionWidthPixels": 1600,
            "developmentOcrRegionHeightPixels": 400,
        },
        "references": {"excludedPhysicalIdentity": "62607_215_SLOT25"},
    }
    ...
    gate = {
        "schema": "argos_opencv_scribe_r4_locked_parity_gate_v1",
        "createdUtc": datetime.now(timezone.utc).isoformat(),
        "state": "PASS_OCV02_R5_LOCKED_READER_AND_SEMANTIC_PARITY" if passed else "FAIL_OCV02_R5_LOCKED_READER_AND_SEMANTIC_PARITY",
        "engine": {
            "path": "work/OPENCV_SCRIBE_V1R5/ArgosOpenCvScribeV1R5.py",
