...
def main() -> int:
    ...
    require(job.get("schema") == "argos_raster_audit_opencv_job_v1", "Raster audit job schema changed.")
    require(job.get("state") == "FROZEN_EXACT_REVIEW_ONLY_RASTER_AUDIT", "Raster audit job state changed.")
    ...
    for _, _, candidate in groups:
        ...
        for role in ("clean", "overlay", "mask"):
            validate_record(asset_root, dict(assets[role]))

    if args.preflight:
        print(json.dumps({
            "schema": "argos_raster_audit_opencv_preflight_v1",
            "state": "PASS_OPENCV_RASTER_AUDIT_PREFLIGHT",
    ...
    for channel_name, index, candidate in groups:
        ...
        require(clean.shape == overlay.shape, f"Clean/overlay shape changed: {channel_name} {index}")
        require(mask.shape[:2] == clean.shape[:2], f"Mask shape changed: {channel_name} {index}")
        changed = np.any(clean != overlay, axis=2) if clean.ndim == 3 else clean != overlay
        current_mask = mask > 0
        ...
        require(changed_outside == 0, f"Overlay changed pixels outside mask: {channel_name} {index}")
        ...
        rows.append({
            ...
            "mask": mask_record,
            "widthPx": int(clean.shape[1]),
            "heightPx": int(clean.shape[0]),
    ...
    gate = {
        "schema": "argos_raster_audit_opencv_gate_v1",
        "state": "PASS_OPENCV_RETURNED_RASTER_PROVENANCE_AUDIT",
        "providerSha256": str(job["providerSha256"]),
        "openCvVersion": str(cv2.__version__),
