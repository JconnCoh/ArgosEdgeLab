...
Assert-True ([string]$invocation.schema-eq'argos_o3n1_route_invocation_v1') 'O3N1 route invocation schema changed.'
...
Assert-True ($result.requestZipSha256-eq[string]$invocation.requestZipSha256-and$result.requestManifestSha256-eq[string]$invocation.requestManifestSha256-and$result.requestSignatureSha256-eq[string]$invocation.requestSignatureSha256) 'O3N1 exact route artifact hash changed.'
$result.exactFinalZipExtractionPassed=[bool]$invocation.exactFinalZipExtractionPassed;$result.exactFinalZipSignaturePassed=[bool]$invocation.exactFinalZipSignaturePassed;Assert-True ($result.exactFinalZipExtractionPassed-and$result.exactFinalZipSignaturePassed) 'O3N1 final ZIP route proof changed.'
$output=[IO.Path]::GetFullPath([string]$invocation.outputPath);Assert-True (-not(Test-Path -LiteralPath $output)) 'O3N1 route output exists.'
