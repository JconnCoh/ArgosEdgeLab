...
$expectedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'O3N1_DATA_PULL_BUILD_INVOCATION.json'))
$definitionPath = Join-Path $PSScriptRoot 'O3N1_DATA_PULL_DEFINITION.json'
$intentPath = Join-Path $PSScriptRoot 'O3N1_RECOVERY_INTENT.json'
$renderTerminalPath = Join-Path $PSScriptRoot 'O3N1_RENDER_RESPONSE_COLLECTION_R3_GATE.json'
$inheritedRoutePath = Join-Path $PSScriptRoot 'O3N1_COMPLETE_ROUTE_GATE.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_O3N1_BUILD_DATA_PULL.json'
...
$packageGatePath = Join-Path $PSScriptRoot 'O3N1_DATA_PULL_FINAL_PACKAGE_GATE.json'
$routeGatePath = Join-Path $PSScriptRoot 'O3N1_DATA_PULL_COMPLETE_ROUTE_GATE.json'
$expectedDefinitionSha256 = '3CF69D6AB06E9F50F4109BDD9EF7F8FEAF806442C7689305A779825B63C8912E'
$expectedIntentSha256 = '993FC81D736B271F6CA96510BB31D56EEBA53EEFD3AB3FCF641159FDEEE45FE6'
$expectedRenderTerminalSha256 = '4ED5AA2ABC5FCFD8F07CB7BC766AA2A02070FAA93780AEBA456D81BD9451E8BC'
$expectedInheritedRouteSha256 = 'C46F4C24150E4799A373224DF421A8A08872FC7921309F8F9DD87C033C86E94D'
...
$expectedExportZipSha256 = '93B2A18D70CAD29B321818EDCD5F1F39C0F189C44DA9F2495BF4AE5AAECD7B2D'
$expectedExportZipBytes = [int64]15166219
$expectedRelativePath = 'OCV03ReviewExports/O3N1_20260827T231100Z/O3N1_SLOT16_REVIEW.zip'
...
function Assert-Pin([string]$Path, [string]$Sha256) {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "O3N1 DATA_PULL dependency is absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "O3N1 DATA_PULL dependency changed: $Path"
}
...
function Get-RoutePaths([string]$RequestId) {
    ...
    return @(
        ...
        'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03ReviewExports\O3N1_20260827T231100Z\O3N1_SLOT16_REVIEW.zip',
        ...
        'C:\O3N1P\R.zip',
        'C:\O3N1P\R\PORTAL_RESPONSE_MANIFEST.json',
        'C:\O3N1P\R\DATA_PULL_PAYLOAD.zip',
        ('C:\O3N1D\' + $returnedLeaf)
    )
}

Assert-True ($invocationPath.Equals($expectedInvocationPath,[StringComparison]::OrdinalIgnoreCase)) 'O3N1 DATA_PULL invocation manifest path changed.'
$invocation = Get-Content -Raw -LiteralPath $invocationPath | ConvertFrom-Json
Assert-True ([string]$invocation.schema -eq 'argos_o3n1_data_pull_build_invocation_v1' -and [string]$invocation.state -eq 'FROZEN_EXACT_BUILD_INVOCATION') 'O3N1 DATA_PULL invocation manifest state changed.'
Assert-True ([string]$invocation.powerShellScriptSha256 -eq (Get-Sha256 $PSCommandPath)) 'O3N1 DATA_PULL invocation does not pin the exact builder.'
Assert-True ([string]$invocation.definitionSha256 -eq $expectedDefinitionSha256 -and [string]$invocation.renderTerminalResponseSha256 -eq $expectedRenderTerminalSha256) 'O3N1 DATA_PULL invocation dependency pins changed.'
Assert-True ([int]$invocation.maximumFiles -eq 1 -and [int64]$invocation.maximumBytes -eq 16777216 -and -not [bool]$invocation.requestRetryAuthorized) 'O3N1 DATA_PULL invocation bounds changed.'
...
foreach ($path in @($preactionPath,$historyPath,$intentTool,$preactionTool,$pathTool)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "O3N1 DATA_PULL gate dependency is absent: $path"
}

$intentResult = & $intentTool -IntentPath $intentPath -Preflight -AsJson | ConvertFrom-Json
Assert-True ([string]$intentResult.state -eq 'PASS_ARGOS_RECOVERY_INTENT') 'O3N1 DATA_PULL recovery intent failed.'
...
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'DATA_PULL') 'O3N1 DATA_PULL route changed.'
Assert-True ([string]$definition.parameters.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'O3N1 DATA_PULL approved root changed.'
Assert-True ($relativePaths.Count -eq 1 -and $relativePaths[0] -eq $expectedRelativePath) 'O3N1 DATA_PULL exact file set changed.'
Assert-True ([int]$definition.parameters.maximumFiles -eq 1) 'O3N1 DATA_PULL maximum file count changed.'
Assert-True ([int64]$definition.parameters.maximumBytes -eq 16777216 -and [int64]$definition.maxResultBytes -eq 16777216) 'O3N1 DATA_PULL byte bounds changed.'
...
Assert-True ([string]$intent.mode -eq 'MUTATE' -and [int]$intent.authorizationBoundary.liveEndpointRequestsMaximum -eq 2) 'O3N1 two-step authorization changed.'
$sequence = @($intent.authorizationBoundary.liveEndpointRequestSequence | ForEach-Object { [string]$_ })
Assert-True ($sequence.Count -eq 2 -and $sequence[1] -eq 'ONE_DATA_PULL_AFTER_THE_MATCHING_RENDER_TERMINAL_RESPONSE_TO_RETURN_THAT_EXACT_ZIP') 'O3N1 DATA_PULL sequence authorization changed.'
Assert-True (-not [bool]$intent.requestRetryAuthorized) 'O3N1 retry authority changed.'
Assert-True ([string]$terminal.state -eq 'PASS_O3N1_MATCHING_SIGNED_RENDER_RESPONSE_COLLECTED_R3' -and [bool]$terminal.signatureVerified -and [bool]$terminal.dataPullNowEligible) 'O3N1 matching signed render terminal proof is absent.'
Assert-True ([string]$terminal.exportRelativePath -eq $expectedRelativePath -and [int64]$terminal.exportZipBytes -eq $expectedExportZipBytes -and [string]$terminal.exportZipSha256 -eq $expectedExportZipSha256) 'O3N1 rendered export identity changed.'
Assert-True ([string]$inheritedRoute.state -eq 'PASS_O3N1_COMPLETE_ROUTE_GATE') 'O3N1 inherited route gate changed.'
Assert-True ([string]$inheritedRoute.endpointWorkerSha256 -eq $expectedWorkerSha256 -and [string]$inheritedRoute.installedConfigEvidenceSha256 -eq $expectedConfigSha256 -and [string]$inheritedRoute.inheritedQueueSafetyGateSha256 -eq $expectedQueueSafetySha256) 'O3N1 installed route pins changed.'
Assert-True (-not [bool]$inheritedRoute.requestRetryAuthorized -and -not [bool]$inheritedRoute.gatewayAcceptanceIsExecutionEvidence) 'O3N1 route evidence semantics changed.'
...
foreach ($required in @('DefinitionPath','PayloadRoot','OutputRoot','IdentityStatePath')) {
    Assert-True ($builderParameters -contains $required) "O3N1 package builder argument is absent: $required"
}
...
foreach ($required in @('PackagePath','SignerCertificatePath','ExpectedTargetRole','ExpectedJobClass')) {
    Assert-True ($testerParameters -contains $required) "O3N1 package tester argument is absent: $required"
}
...
Assert-True ($currentBranch -eq $branch -and $localTip -eq $remoteTip) 'O3N1 DATA_PULL requires matching local/origin branch tips.'
foreach ($path in @($signedRoot,$finalRoot,$finalPartial,$packageGatePath,$routeGatePath)) {
    Assert-True (-not (Test-Path -LiteralPath $path)) "O3N1 DATA_PULL create-new output exists: $path"
}

$placeholderRequestId = 'REQ_20260827T203000111Z_62629419O3N1P'
...
Assert-True ([string]$plannedPathGate.state -eq 'PASS_PATH_BUDGET') 'O3N1 DATA_PULL complete round-trip path budget failed.'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_o3n1_data_pull_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o')
        state='PASS_O3N1_DATA_PULL_BUILD_PREFLIGHT';relativePath=$expectedRelativePath;maximumFiles=1
...
Assert-True ([string]$built.State -eq 'SIGNED_PORTAL_PACKAGE_READY') 'O3N1 DATA_PULL signed package builder failed.'
$requestId = [string]$built.RequestId
$readyRoot = [IO.Path]::GetFullPath([string]$built.PackagePath)
Assert-True ($requestId -match '^REQ_[0-9]{8}T[0-9]{9}Z_[A-F0-9]{12}$') 'O3N1 DATA_PULL generated request ID is invalid.'
Assert-True ((Split-Path -Leaf $readyRoot) -eq ($requestId + '.ready')) 'O3N1 DATA_PULL ready root does not match its request ID.'
...
Assert-True ([string]$folderTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'O3N1 DATA_PULL signed folder validation failed.'
...
Assert-True ([string]$extractTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'O3N1 DATA_PULL exact ZIP signature validation failed.'
Assert-True (@(Get-ChildItem -LiteralPath $extractRoot -File).Count -eq 2) 'O3N1 DATA_PULL exact ZIP entry cardinality changed.'
...
Assert-True ([string]$actualPathGate.state -eq 'PASS_PATH_BUDGET') 'O3N1 DATA_PULL exact route path budget failed.'
...
$requestZipRelative = 'work/OPENCV_EDGE_NOTCH_O3N1/dpf/' + $requestId + '.ready.zip'
$routeGate = [ordered]@{
    schema='argos_o3n1_data_pull_complete_route_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O3N1_DATA_PULL_COMPLETE_ROUTE_GATE'
...
$packageGate = [ordered]@{
    schema='argos_o3n1_data_pull_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O3N1_DATA_PULL_FINAL_PACKAGE_GATE'
...
[ordered]@{
    schema='argos_o3n1_data_pull_build_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O3N1_EXACT_SIGNED_DATA_PULL_READY'
