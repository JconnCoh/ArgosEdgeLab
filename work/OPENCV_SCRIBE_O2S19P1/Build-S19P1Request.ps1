...
$definitionPath = Join-Path $PSScriptRoot 'S19P1_DATA_PULL_DEFINITION.json'
$intentPath = Join-Path $PSScriptRoot 'S19P1_RECOVERY_INTENT.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_S19P1_BUILD.json'
...
$routeHealthPath = Join-Path $project 'work\OPENCV_SCRIBE_O2D13\O2D13_COMPLETE_ROUTE_GATE_R2.json'
$terminalPath = Join-Path $project 'work\OPENCV_SCRIBE_O2S19B1\S19B1_SIGNED_TERMINAL_FAILURE_GATE.json'
...
$expectedDefinitionSha256 = '4F5751BB0D326FCA0F71A725B495B613413DFE29DA6E419700935EF3113DB52F'
$expectedIntentSha256 = '197B11135378E3E1C714E8CEBE3A46666E4D16B6ED9FC4BCCDB113CC85DF3B89'
$expectedPreactionSha256 = '86772630ABB1B988613729D8A73A10CCB980F4EC86F0BD10AAF7CDC18B92CB86'
$expectedCheckpointSha256 = '1009F3149E1826F88200369BF716BB0E975C41CC0954A2B43526368715A047FF'
$expectedRouteHealthSha256 = 'F7F65BCAF809D586E7E55ED86C83BCD9149703AD399262EAFC28C09E65FE9116'
$expectedTerminalSha256 = '86F005A975046AF9EEAD48E906B664BE8BD61A2AA801D6C0490DB1D1E622E9D3'
...
function Assert-Pin([string]$Path, [string]$Sha256) {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "S19P1 dependency is absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "S19P1 dependency changed: $Path"
}
...
foreach ($path in @($identityPath,$publicCertificatePath,$packageTester,$intentTool,$preactionTool,$pathTool)) { Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "S19P1 input is absent: $path" }
...
$expectedRelativePaths = @(
    'identity/proposals/62619-433_20260824005735_Slot19/SCRIBE_PROPOSAL.json'
)
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'DATA_PULL' -and [string]$definition.parameters.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'S19P1 DATA_PULL route changed.'
Assert-True ($relativePaths.Count -eq 1 -and @(Compare-Object -ReferenceObject $expectedRelativePaths -DifferenceObject $relativePaths).Count -eq 0) 'S19P1 exact relative path set changed.'
Assert-True ([int]$definition.parameters.maximumFiles -eq 1 -and [int64]$definition.parameters.maximumBytes -eq 2097152 -and [int64]$definition.maxResultBytes -eq 2097152) 'S19P1 bounds changed.'
...
Assert-True ([string]$routeHealth.state -eq 'PASS_O2D13_COMPLETE_ROUTE_GATE' -and [bool]$routeHealth.publicationAuthorized -and -not [bool]$routeHealth.retryAuthorized) 'S19P1 inherited route health changed.'
Assert-True ([int]$routeHealth.unresolvedEarlierAcceptedRequestCount -eq 0 -and [bool]$routeHealth.argosInboundRelayCurrentHealthProved) 'S19P1 route has unresolved earlier work.'
Assert-True ([string]$routeHealth.endpointWorkerSha256 -eq $expectedWorkerSha256 -and [string]$routeHealth.installedEndpointConfigSha256 -eq $expectedConfigSha256) 'S19P1 installed route pins changed.'
Assert-True ([string]$terminal.state -eq 'FAIL_S19B1_SIGNED_TERMINAL_SOURCE_PATH_PREMISE' -and [string]$terminal.responseState -eq 'FAILED' -and [bool]$terminal.signatureValid -and [bool]$terminal.manifestFailureRecordVerified -and [bool]$terminal.directPostFailureObservationRequired) 'S19P1 predecessor terminal evidence changed.'
...
Assert-True ($currentBranch -eq $branch -and $localTip -eq $remoteTip) 'S19P1 requires matching local/origin branch tips.'

foreach ($path in @($partialRoot,$signedRoot,$finalRoot,$packageGatePath,$routeGatePath,$pathSidecar)) { Assert-True (-not (Test-Path -LiteralPath $path)) "S19P1 fresh output exists: $path" }
$responseId = 'R_0123456789AB_20260826235959999_a1b2c3d4'
$responseReady = $responseId + '.ready'
$longestRelative = 'data\JBOD_PROCESSOR_REVIEW\' + $expectedRelativePaths[0].Replace('/','\')
$routePaths = @(
    ...
    'C:\S19P1R.zip',
    'C:\S19P1R\PORTAL_RESPONSE_MANIFEST.json',
    'C:\S19P1R\DATA_PULL_PAYLOAD.zip',
    ('C:\S19P1D\' + $longestRelative)
)
$pathGate = & $pathTool -CandidatePath $routePaths -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'S19P1 complete route path budget failed.'
...
Assert-True ([bool]$certificate.HasPrivateKey) 'S19P1 signer private key is unavailable.'

if ($Preflight) {
    [ordered]@{schema='argos_s19p1_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19P1_BUILD_PREFLIGHT';requestId=$requestId;relativePathCount=1;maximumFiles=1;maximumBytes=2097152;routePathCount=$routePaths.Count;maximumEffectiveLength=$maximumEffective;maximumComponentLength=$maximumComponent;branch=$branch;localTip=$localTip;remoteTip=$remoteTip;signedPremiseFailures=1;directPostFailureObservation=$true;mutationsPerformed=$false;jbodContacted=$false;imageBytesRequested=0;slots22Through25Exposed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
Assert-True ([string]$folderTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'S19P1 signed folder validation failed.'
...
Assert-True ([string]$extractTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE' -and @(Get-ChildItem -LiteralPath $extractRoot -File).Count -eq 2) 'S19P1 exact ZIP extraction validation failed.'
...
$packageGate = [ordered]@{schema='argos_s19p1_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19P1_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip=('work/OPENCV_SCRIBE_O2S19P1/final/'+$requestId+'.ready.zip');requestZipBytes=$zipBytes;requestZipSha256=$zipSha256;requestManifestSha256=$manifestSha256;requestSignatureSha256=$signatureSha256;definitionSha256=$expectedDefinitionSha256;recoveryIntentSha256=$expectedIntentSha256;preactionSha256=$expectedPreactionSha256;exactFinalZipExtractionPassed=$true;exactFinalZipSignaturePassed=$true;requestedFileCount=1;requestedImageFiles=0;sourceHashingRequested=$false;publicationAuthorized=$false;reviewOnly=$true;productionRoutingEnabled=$false}
$routeGate = [ordered]@{schema='argos_s19p1_complete_route_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19P1_COMPLETE_ROUTE_GATE';requestId=$requestId;jobClass='DATA_PULL';requestZipSha256=$zipSha256;requestManifestSha256=$manifestSha256;requestSignatureSha256=$signatureSha256;endpointWorkerSha256=$expectedWorkerSha256;installedEndpointConfigSha256=$expectedConfigSha256;inheritedRouteHealthSha256=$expectedRouteHealthSha256;priorTerminalResponseSha256=$expectedTerminalSha256;approvedRoot='JBOD_PROCESSOR_REVIEW';relativePaths=$relativePaths;maximumFiles=1;maximumBytes=2097152;maxResultBytes=2097152;longestReturnedZipEntry=$longestRelative.Replace('\','/');routePathCount=$routeRows.Count;routeRows=$routeRows;maximumEffectiveLength=$maximumEffective;maximumComponentLength=$maximumComponent;reservedSuffixCharacters=32;exactFinalZipExtractionPassed=$true;exactFinalZipSignaturePassed=$true;unresolvedEarlierAcceptedRequestCount=0;publicationAuthorized=$true;maximumRequestsAuthorized=1;retryOnFailure=$false;matchingSignedTerminalResponseCollectionOnly=$true;directPostFailureObservation=$true;signedPremiseFailures=1;imageBytesRequested=0;slots22Through25Exposed=$false;reviewOnly=$true;productionRoutingEnabled=$false}
...
[ordered]@{schema='argos_s19p1_build_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19P1_EXACT_SIGNED_DATA_PULL_READY';requestId=$requestId;requestZip=$finalZip;requestZipBytes=$zipBytes;requestZipSha256=$zipSha256;packageGate=$packageGatePath;routeGate=$routeGatePath;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
