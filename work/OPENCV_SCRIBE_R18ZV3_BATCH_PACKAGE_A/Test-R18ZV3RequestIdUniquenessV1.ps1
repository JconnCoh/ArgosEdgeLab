...
$requestId = 'REQ_RZV3'
$bindingRecordSha256 = '07315387419E5396A73D23C79228682EF547E813CDC32000AAE242448B37B4E6'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_r18zv3_request_id_uniqueness_preflight_v1'
        checkedUtc=[DateTime]::UtcNow.ToString('o')
        state='PASS_R18ZV3_REQUEST_ID_UNIQUENESS_PREFLIGHT'
        requestId=$requestId
...
$preSignatureOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV3_REQUEST_ID_COLLISION_GATE_V2.json'))
$prePublicationOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV3_P8_COLLISION.json'))
Assert-True (
...
$result = [ordered]@{
    schema='argos_r18zv3_request_id_uniqueness_gate_v1'
    checkedUtc=[DateTime]::UtcNow.ToString('o')
    state=$(if($scanPassed){'PASS_R18ZV3_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS'}else{'FAIL_R18ZV3_REQUEST_ID_COLLISION_OR_SCAN_ERROR'})
    requestId=$requestId
    bindingRecordSha256=$bindingRecordSha256
    scannerSha256=$scannerSha256
