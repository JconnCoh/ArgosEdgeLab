...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV3_PUBLICATION_INVOCATION.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zv3_publication_invocation_v1') 'R18ZV3 invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZV3_PUBLICATION_INVOCATION_V1') 'R18ZV3 invocation is not frozen.'
Require ([string]$invocation.requestId -ceq 'REQ_RZV3') 'R18ZT publication request identity changed.'
...
Require ([string]$postFreezeContract.path -ceq 'work/OPENCV_SCRIBE_R18ZV3_BATCH_PACKAGE_A/R18ZV3_P8_FINAL_TOOL.json') 'R18ZT post-freeze tooling-gate path changed.'
...
Require ([string]$postFreezeTooling.schema -ceq [string]$postFreezeContract.expectedSchema -and
    ...
    [string]$postFreezeTooling.requestId -ceq 'REQ_RZV3' -and
...
Require ([string]$invocation.packageBinding.expectedSchema -ceq 'argos_opencv_scribe_r18zv3_package_bindings_v1') 'Fresh R18ZV3 package binding schema declaration changed.'
Require ([string]$binding.schema -ceq [string]$invocation.packageBinding.expectedSchema) 'Fresh package binding schema changed.'
Require ([string]$binding.state -ceq [string]$invocation.packageBinding.expectedState -and [string]$binding.requestId -ceq 'REQ_RZV3') 'Fresh package binding identity/state changed.'
...
foreach ($consumer in @($invocation.bindingConsumers)) {
    ...
    Require ([string](Get-NestedValue $consumerObject ([string]$consumer.requestIdProperty) "Binding consumer $($consumer.dependency)") -ceq 'REQ_RZV3') "Binding consumer request ID changed: $($consumer.dependency)"
}
...
Require ($bindingAdapterRows.Count -eq 1 -and $scienceAdapterRows.Count -eq 1 -and
    ...
    [string]$scienceAdapterGateRows[0].sha256 -ceq [string]$adapterQualificationDependency.sha256) 'R18ZV3 adapter dependency pins changed.'
Require ([string]$adapterQualification.state -ceq 'PASS_R18ZV2_BATCH_ADAPTER_EXACT_SEAM_ALGORITHM_UNCHANGED' -and
    ...
    -not [bool]$adapterQualification.behavioralBoundary.referenceLogicChanged -and
    ...
    -not [bool]$adapterQualification.tests.providerRunPerformed) 'R18ZV3 batch adapter qualification changed.'
$predecessorDependency = Get-ValidatedDependency 'r18zv2RunningCoexistenceGate'
$predecessorGate = Read-JsonFile ([string]$predecessorDependency.path) 1048576
Require ([string]$predecessorGate.state -ceq 'PASS_R18ZV2_RUNNING_STATE_BOUND_FOR_LOCAL_SUCCESSOR_PREPARATION' -and
    [string]$predecessorGate.predecessorRequestId -ceq 'REQ_RZV2' -and
    [string]$predecessorGate.successorRequestId -ceq 'REQ_RZV3' -and
    [bool]$predecessorGate.latestAuthenticatedDirectObservation.pending -and
    [string]$predecessorGate.latestAuthenticatedDirectObservation.workerState -ceq 'RUNNING' -and
    [bool]$predecessorGate.coexistenceContract.r18zv2MustNotBeStoppedBeforeCompleteR18zv3PackagePass -and
    -not [bool]$predecessorGate.coexistenceContract.successorMayBePublishedFromThisGate -and
    [bool]$predecessorGate.coexistenceContract.freshDirectQueueAndEndpointHealthAuditRequiredBeforeAnyPublication -and
    -not [bool]$predecessorGate.r18zv2StopPerformed) 'R18ZV2 running predecessor coexistence boundary changed.'
...
try {
    ...
    Require ([string]$manifest.schema -ceq 'argos_project_portal_request_manifest_v1' -and [string]$manifest.requestId -ceq 'REQ_RZV3' -and
    ...
    Require ([string]$change.source -ceq 'payload/Invoke-R18ZTBatchLaunch.ps1' -and
        [string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZV3.ps1' -and
...
Require ($requestRoot -ceq 'U:\ProjectPortalRO\requests' -and $responseRoot -ceq 'U:\ProjectPortalRO\responses' -and
    $readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZV3.ready.zip' -and $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZV3.ready.zip.upload' -and
    $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_RZV3.ready.zip') 'R18ZT exact publication route changed.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv3_publication_apply_intent_v1'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZV3_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
    requestId = 'REQ_RZV3'
...
Require ([string]$collision.requestId -ceq 'REQ_RZV3' -and
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv3_publish_gate_v1'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZV3_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZV3_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_RZV3'
