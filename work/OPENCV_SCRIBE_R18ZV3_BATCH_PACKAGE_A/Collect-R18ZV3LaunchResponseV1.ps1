...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zv3_publication_invocation_v1' -and
    [string]$invocation.state -ceq 'FROZEN_R18ZV3_PUBLICATION_INVOCATION_V1' -and [string]$invocation.lifecycle -ceq 'FROZEN') 'R18ZV3 invocation is not frozen.'
Require ([string]$invocation.requestId -ceq 'REQ_RZV3' -and -not [bool]$invocation.authority.retryAuthorized -and
...
Require ([string]$applyIntent.schema -ceq 'argos_opencv_scribe_r18zv3_publication_apply_intent_v1' -and
    [string]$applyIntent.state -ceq 'FROZEN_R18ZV3_APPLY_INTENT_BEFORE_EXTERNAL_WRITE' -and
    [string]$applyIntent.requestId -ceq 'REQ_RZV3' -and [string]$applyIntent.invocationManifestSha256 -ceq $invocationSha256 -and
...
if ($publishGateExists) {
    ...
    Require ([string]$publishGate.schema -ceq 'argos_opencv_scribe_r18zv3_publish_gate_v1' -and
        [string]$publishGate.state -ceq 'PASS_R18ZV3_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' -and
        [string]$publishGate.requestId -ceq 'REQ_RZV3') 'R18ZT publisher gate identity changed.'
...
if (-not $publishGateExists) {
    ...
    Require ($readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZV3.ready.zip' -and
        $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_RZV3.ready.zip' -and
        $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZV3.ready.zip.upload') 'R18ZT missing-gate recovery route leaves changed.'
    $routeRows = @(
...
foreach ($candidate in $candidates) {
    ...
    try {
        ...
        if ([string]$manifest.requestId -cne 'REQ_RZV3') { continue }
...
$preflightResult = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv3_launch_response_collection_gate_v1'
    collectedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZV3_ONE_SIGNED_LAUNCH_RESPONSE_PREFLIGHT' } else { 'PASS_R18ZV3_SIGNED_LAUNCH_RESPONSE_COLLECTED' })
    requestId = 'REQ_RZV3'
...
$verificationRows = @(& ([string]$responseVerifierDependency.path) -PackagePath $verificationRoot -EndpointCertificatePath ([string]$responseSignerDependency.path) -ExpectedSourceRole JBOD -ExpectedRequestId 'REQ_RZV3')
