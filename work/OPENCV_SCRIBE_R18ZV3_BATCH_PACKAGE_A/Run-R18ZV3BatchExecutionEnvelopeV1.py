#!/usr/bin/env python3
"""Durable asynchronous envelope for the fixed-parallel R18ZV3 batch runner."""
...
REVISION = "R18ZV3_FIXED_PARALLEL_ASYNC_REVIEW_ONLY_20260911A"
COMPLETE_STATE = "COMPLETE_R18ZT_EXISTING_ORIENTED_CROPS_REVIEW_ONLY"
RUNNER_REVISION = "R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260908D"
COORDINATOR_REVISION = "R18ZV3_FIXED_PROCESS_ISOLATED_BATCH_20260911A"; COORDINATOR_BASENAME = "r18zv3_parallel_batch.py"; BASE_RUNNER_BASENAME = "Run-R18ZTExistingOrientedCropsV2.py"
BASE_RUNNER_SHA256 = "F3B465F433ADB58F1C37C5673999FD03A4315EFA5A3128EFF8E192E9C3D58AE2"; FIXED_WORKER_COUNT = 8; MAXIMUM_WORKER_COUNT = 16
...
def exact_complete_contract(
    ...
    expected_scalars = {
        ...
        "parallelCoordinatorRevision": COORDINATOR_REVISION, "parallelExecutionMode": "FIXED_PROCESS_ISOLATED_SHARDS",
    }
    ...
    cache = complete.get("referenceBankCache"); shards = cache.get("shards") if isinstance(cache, dict) else None
    if type(complete.get("workerCount")) is not int or complete.get("workerCount") != FIXED_WORKER_COUNT:
        return False, "COMPLETE_FIXED_WORKER_COUNT_INVALID", complete
    if not isinstance(cache, dict) or cache.get("state") != "PASS_FIXED_PROCESS_ISOLATED_REFERENCE_BANK_CACHES" or cache.get("parallelism") != "FIXED_PROCESS_ISOLATED_SHARDS" or cache.get("workerCount") != FIXED_WORKER_COUNT or not isinstance(shards, list) or len(shards) != FIXED_WORKER_COUNT:
        return False, "COMPLETE_FIXED_SHARD_CACHE_INVALID", complete
    if any(not isinstance(row, dict) or row.get("state") != "PASS_EXACT_PROCESS_LOCAL_REFERENCE_BANK_CACHE" or not SHA256_PATTERN.fullmatch(str(row.get("bindingSha256", ""))) or not isinstance(row.get("stages"), dict) for row in shards) or len({str(row["bindingSha256"]) for row in shards}) != 1:
        return False, "COMPLETE_FIXED_SHARD_CACHE_ROW_INVALID", complete
    ...
    for field in ("parallelCoordinatorRevision", "parallelExecutionMode", "workerCount", "referenceBankCache"):
        if aggregate.get(field) != complete.get(field):
            return False, f"AGGREGATE_PARALLEL_CONTRACT_MISMATCH:{field}", complete
...
def run(arguments: argparse.Namespace) -> int:
    ...
    context: dict[str, object] = {
        ...
        "delegateExecutionMode": "IN_PROCESS_PUBLIC_MAIN_WITH_FIXED_PROCESS_ISOLATED_CHILDREN",
        "maximumDelegateChildProcessCount": FIXED_WORKER_COUNT, "delegateChildrenMustExitBeforeReturn": True,
    }
    try:
        ...
        if delegate.name != COORDINATOR_BASENAME:
            raise RuntimeError("Fixed-parallel coordinator basename mismatch.")
        base_runner = delegate.with_name(BASE_RUNNER_BASENAME)
        if not base_runner.is_file() or sha256_file(base_runner) != BASE_RUNNER_SHA256:
            raise RuntimeError("Adjacent frozen base-runner basename or hash mismatch.")
        ...
        with stdout_path.open("x", encoding="utf-8", buffering=1) as stdout_log, stderr_path.open(
            ...
            if (getattr(delegate_module, "REVISION", ""), getattr(delegate_module, "BASE_RUNNER_SHA256", ""), getattr(delegate_module, "DEFAULT_WORKER_COUNT", None), getattr(delegate_module, "MAXIMUM_WORKER_COUNT", None)) != (COORDINATOR_REVISION, BASE_RUNNER_SHA256, FIXED_WORKER_COUNT, MAXIMUM_WORKER_COUNT):
                raise RuntimeError("Fixed-parallel coordinator contract mismatch.")
            ...
            delegate_arguments = [
                    ...
                    "--output-root",
                    str(output_root),
                    "--worker-count", str(FIXED_WORKER_COUNT),
                    "--base-runner", str(base_runner),
                ]
            baseline_child_pids = {child.pid for child in multiprocessing.active_children() if child.pid is not None}
            with redirect_stdout(stdout_log), redirect_stderr(stderr_log):
                try:
                    try:
                        returned = delegate_module.main(delegate_arguments); delegate_exit_code = 0 if returned is None else int(returned)
                    except SystemExit as signal:
                        delegate_exit_code = 0 if signal.code is None else int(signal.code)
                finally:
                    children = [child for child in multiprocessing.active_children() if child.pid is not None and child.pid not in baseline_child_pids]
                    for child in children: child.join(0.25)
                    for child in children:
                        if child.is_alive(): child.terminate(); child.join(5.0)
                    residual = [{"name": child.name, "pid": child.pid} for child in children if child.is_alive()]
                    context["observedLiveDelegateChildrenAfterReturn"] = len(residual); context["residualDelegateChildren"] = residual
                    if residual: raise RuntimeError("Fixed-parallel delegate left live child processes.")
        ...
        if exact_complete and context.get("observedLiveDelegateChildrenAfterReturn") == 0:
            sys.stderr.write(
