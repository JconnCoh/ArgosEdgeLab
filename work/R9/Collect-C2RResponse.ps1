...
if([string]$publish.requestId-ne$requestId-or[string]$publish.state-ne'PASS_R9_EXACT_SIGNED_REQUEST_PUBLISHED_CREATE_NEW'-or[bool]$publish.overwritePerformed-or[int]$publish.pendingRequestsAtApply-ne0){throw 'R9 publication contract changed.'}
...
if($candidates.Count-eq0){if($Preflight){[ordered]@{schema='argos_r9_response_pending_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PENDING_R9_SIGNED_TERMINAL_RESPONSE';requestId=$requestId;pendingRequests=$pending.Count;responseCandidates=0;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 5;return};throw 'R9 response unavailable.'}
...
$pass=(
    ...
    [int]$stdout.overlayPhysicalMatches-eq10-and[int]$stdout.metadataMatchedCatalogRows-eq10-and
...
$terminal=[ordered]@{
    schema='argos_r9_terminal_response_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R9_SIGNED_TERMINAL_RESPONSE';disposition='PENDING_GATE'
