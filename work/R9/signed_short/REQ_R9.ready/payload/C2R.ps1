...
function Read-LotState([string]$Phase){
    if($isRehearsal){
        $property=if($Phase-eq'before'){'lotStateBefore'}else{'lotStateAfter'}
        $fixtureState=Get-Optional $fixture $property $null
        if($null-eq$fixtureState-or[int]$fixtureState.queueRows-ne10-or[int]$fixtureState.catalogRows-ne10){throw "C2R rehearsal expected ten current queue and FRONT catalog rows for $Phase."}
        return $fixtureState
    }
...
if($runnerBeforeSha-notin@($approvedRunnerBefore,$expectedRunner)){throw "R9 installed runner predecessor is not approved: $runnerBeforeSha"}
...
if($runnerBeforeSha-eq$approvedRunnerBefore){
    $runnerEvidence=Get-BudgetedPath (Join-Path $processorRoot ('state\maintenance_runner_fix\R9_'+[DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffZ')))
    if(Test-Path -LiteralPath $runnerEvidence){throw 'R9 runner evidence collision.'}
    ...
    $stage=Get-BudgetedPath (Join-Path $processorRoot 'R9.stage.ps1');$restore=Get-BudgetedPath (Join-Path $processorRoot 'R9.restore.ps1')
    foreach($path in @($stage,$restore)){if(Test-Path -LiteralPath $path){throw "R9 runner swap collision: $path"}}
    [void](New-Item -ItemType Directory -Path $runnerEvidence -Force);Copy-Item -LiteralPath $runnerPath -Destination $runnerPrior
    if((Get-Sha $runnerPrior)-ne$approvedRunnerBefore){throw 'R9 archived predecessor changed.'}
    $swapped=$false
    try{
        Copy-Item -LiteralPath $runnerPayload -Destination $stage;if((Get-Sha $stage)-ne$expectedRunner){throw 'R9 staged runner changed.'}
        [IO.File]::Replace($stage,$runnerPath,$backup,$true);$swapped=$true
        if((Get-Sha $runnerPath)-ne$expectedRunner){throw 'R9 repaired runner absent after swap.'}
        if($isRehearsal-and[bool](Get-Optional $fixture 'failAfterSwap' $false)){throw 'INJECTED_R9_FAILURE_AFTER_SWAP'}
        $runnerChanged=$true
    }catch{
        $failure=$_
        if($swapped){Copy-Item -LiteralPath $runnerPrior -Destination $restore;[IO.File]::Replace($restore,$runnerPath,$failed,$true);if((Get-Sha $runnerPath)-ne$approvedRunnerBefore){throw 'R9 runner rollback failed.'}}
        throw $failure
...
if((Get-Sha $runnerPath)-ne$expectedRunner){throw 'R9 repaired runner is not installed.'}
...
if(
    [int]$beforeState.queueRows-ne10-or[int]$beforeState.catalogRows-ne10-or
...
){throw 'R9 signed V40 pre-refresh state is not the exact ten-row FRONT contract.'}
...
if($beforeRoutes.Count-ne2-or$beforeAppearance.Count-ne1-or[int]$beforeAppearance[0].count-ne7-or$beforeScribe.Count-ne1-or[int]$beforeScribe[0].count-ne3){throw 'R9 signed V40 stale catalog route-state contract changed.'}
...
if([int]$afterState.queueRows-ne10-or[int]$afterState.catalogRows-ne10-or[int]$afterState.confirmedPhysical-ne10-or[int]$afterState.overlayPhysicalMatches-ne10-or[int]$afterState.metadataMatchedCatalogRows-ne10-or[int]$afterState.staleMatchedInsiteRows-ne0-or[int]$afterState.scribeHoldRows-ne0-or[int]$afterState.notReadyMatchedRows-ne0){throw 'R9 downstream catalog did not admit all ten confirmed FRONT wafers.'}
...
if($afterRoutes.Count-ne1-or$afterReady.Count-ne1-or[int]$afterReady[0].count-ne10){throw 'R9 downstream catalog did not emit the exact ten-row FRONT ready state.'}
$tasksAfter=@(Read-Tasks 'after');Compare-ProtectedTasks -Before $tasksBefore -After $tasksAfter

[ordered]@{
    schema='argos_r9_processor_runner_fix_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_JBOD_PROCESSOR_RUNNER_FIX_AND_REFRESH_R9';rehearsal=$isRehearsal
