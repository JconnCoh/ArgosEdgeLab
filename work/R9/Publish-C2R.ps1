...
if(-not[string]::IsNullOrWhiteSpace($InvocationManifest)){
    ...
    if([string]$invocation.schema-ne'argos_r9_publish_rehearsal_v1'-or-not[bool]$invocation.rehearsal){throw 'R9 publisher rehearsal manifest changed.'}
    ...
    foreach($path in @($uncRequestRoot,$publishGate)){if(-not$path.StartsWith('C:\R9P\',[StringComparison]::OrdinalIgnoreCase)){throw "R9 publisher rehearsal path escaped: $path"}}
    $isRehearsal=$true
}
...
if([string]$finalGate.state-ne'PASS_R9_FINAL_PACKAGE_GATE'-or-not[bool]$finalGate.publicationAuthorized-or@($finalGate.allowedTaskActions).Count-ne1-or[string]$finalGate.allowedTaskActions[0]-ne'RESTART:ArgosEdgeLab.AllWaferProcessor.ReviewOnly.V2'){throw 'R9 final contract changed.'}
...
try{
    ...
    $state=if($isRehearsal){'PASS_R9_EXACT_PUBLISHER_CREATE_NEW_REHEARSAL'}elseif([string]$queue.state-eq'NEW'){'PASS_R9_EXACT_SIGNED_REQUEST_PUBLISHED_CREATE_NEW'}elseif([string]$queue.state-eq'EXACT_UPLOAD'){'PASS_R9_EXACT_SIGNED_REQUEST_PUBLISHED_RESUMED_UPLOAD'}else{'PASS_R9_EXACT_SIGNED_REQUEST_PUBLISHED_RECOVERED_READY'}
    $gate=[ordered]@{schema='argos_r9_publish_gate_v1';publishedUtc=[DateTime]::UtcNow.ToString('o');state=$state;disposition='PENDING_GATE';requestId=$requestId;source=$source;publishedPath=$ready;bytes=$requestBytes;sha256=$requestSha;finalGateSha256=$finalGateSha;createNew=$true;initialQueueState=[string]$queue.state;overwritePerformed=$false;pendingRequestsBeforePublication=[int]$queue.foreign;pendingRequestsAtApply=[int]$applyState.foreign;pathState=[string]$pathGate.state;allowedTaskActions=@('RESTART:ArgosEdgeLab.AllWaferProcessor.ReviewOnly.V2');priorTerminalResponseId='R_0FE56F0676DA_20260821163245034_fbc68fa1';sourceDeletionPerformed=$false;otherInspectionTasksChanged=$false;waferAborted=$false;reviewOnly=$true;productionRoutingEnabled=$false}
