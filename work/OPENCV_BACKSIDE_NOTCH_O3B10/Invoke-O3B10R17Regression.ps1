...
$engine='C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03_BacksideNotchDevelopment_O3B10R17.py'
$engineHash='B480B40435EE80527B00074FC4E7A69F423033D5AF02DA659E9193145B6CD713'
...
$configPath='C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03CorpusR1\BACKSIDE_NOTCH_CONFIG_R5.json'
$configHash='C697CFF492B3A64EBDA95978E9A9F803BC06E155B0E2FC29157FDB8872C6E377'
$casesPath=Join-Path $PSScriptRoot 'R17_REGRESSION_CASES.json'
$casesHash='021D1098BD3EA0CF9A89CA3C0238F384FC75CD5FD2D7A495B8E2872838F4E6BF'
Require ($env:COMPUTERNAME-eq'A1025645101') 'R17 regression reached the wrong computer.'
...
Require ($cases.Count-eq10) 'R17 regression case cardinality changed.'
...
if($Preflight){[ordered]@{state='PASS_O3B10_R17_REGRESSION_PREFLIGHT';caseCount=$cases.Count;processStarted=$false;imageDecoded=$false;reviewOnly=$true}|ConvertTo-Json -Compress;return}
...
foreach($case in $cases){
  ...
  $process=New-Object Diagnostics.Process;$process.StartInfo=$start;Require $process.Start() "R17 detector did not start: $($case.id)";$stdout=$process.StandardOutput.ReadToEndAsync();$stderr=$process.StandardError.ReadToEndAsync();if(-not$process.WaitForExit(900000)){try{$process.Kill()}catch{};throw "R17 detector timed out: $($case.id)"};Require ($process.ExitCode-eq0) ("R17 detector failed: $($case.id): "+$stderr.Result)
  $resultPath=Join-Path ([string]$case.output) 'RESULT.json';$bfReview=Join-Path ([string]$case.output) 'BF_review.jpg';$dfReview=Join-Path ([string]$case.output) 'DF_review.jpg';foreach($path in @($resultPath,$bfReview,$dfReview)){Require (Test-Path -LiteralPath $path -PathType Leaf) "R17 output missing: $path"}
  $detector=Get-Content -LiteralPath $resultPath -Raw|ConvertFrom-Json;$pairCount=[int]$detector.pairedCandidateCount
  if([string]$case.expected-eq'UNIQUE'){Require ($pairCount-eq1-and[string]$detector.state-ne'HOLD_BACKSIDE_NOTCH_ANALYSIS_FAILED') "R17 unique control failed: $($case.id)"}
  if([string]$case.expected-eq'ZERO'){Require ($pairCount-eq0) "R17 damaged negative produced a notch: $($case.id)"}
...
[ordered]@{schema='argos_ocv03_o3b10_r17_actual_wafer_regression_v1';state='PASS_O3B10_R17_ACTUAL_WAFER_REGRESSION_EXECUTED';results=$results;sourceMutationPerformed=$false;existingProcessActionPerformed=$false;ownedChildProcessCount=10;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 32 -Compress
