...
$engine = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03_BacksideNotchDevelopment_O3B10R7.py'
$engineHash = 'FD2ADA366024996E8979660616F32C785E126405C1C0AC1D5FC1520DFB7493DE'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R7 regression reached the wrong computer.'
Require ((Sha $python) -eq $pythonHash) 'Pinned Python runtime changed.'
Require ((Sha $engine) -eq $engineHash) 'R7 detector changed.'
...
if ($Preflight) {
    [ordered]@{ state = 'PASS_O3B10_R7_REGRESSION_PREFLIGHT'; caseCount = $cases.Count; processStarted = $false; imageBytesRead = $false; reviewOnly = $true } | ConvertTo-Json -Compress
    return
}
...
foreach ($case in $cases) {
    ...
    Require $process.Start() "R7 detector did not start: $($case.id)"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(900000)) { try { $process.Kill() } catch {}; throw "R7 detector timed out: $($case.id)" }
    $stderr = $stderrTask.Result
    $null = $stdoutTask.Result
    Require ($process.ExitCode -eq 0) ("R7 detector failed: $($case.id): " + $stderr)
    ...
    foreach ($path in @($resultPath,$bfReview,$dfReview)) { Require (Test-Path -LiteralPath $path -PathType Leaf) "R7 output missing: $path" }
    $results += [ordered]@{
        id = $case.id
...
[ordered]@{ schema='argos_ocv03_o3b10_r7_regression_v1'; state='PASS_O3B10_R7_REGRESSION_EXECUTED'; results=$results; sourceMutationPerformed=$false; existingProcessActionPerformed=$false; ownedChildProcessCount=3; reviewOnly=$true; productionRoutingEnabled=$false } | ConvertTo-Json -Depth 32 -Compress
