...
$back = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03_BacksideNotchDevelopment_O3B10R11.py'
$backHash = 'E56C185E65E1CAECC6604519082F039279F0A12DA9ED1318AD38DBC5413C3B4B'
...
$output = Join-Path $reviewRoot 'C11RUN1'
...
if ($Preflight) {
    [ordered]@{state='PASS_O3B10_R11_BACKSIDE_CORPUS25_PREFLIGHT';pairLimit=25;side='BACK';output=$output;processStarted=$false;imageBytesRead=$false;reviewOnly=$true} | ConvertTo-Json -Compress
    return
}
...
[ordered]@{
    schema='argos_ocv03_r11_backside_corpus25_launch_v1'
    state='PASS_O3B10_R11_BACKSIDE_CORPUS25_LAUNCHED'
    pairLimit=25
    side='BACK'
    pid=$process.Id
    creationTimeUtc=$process.StartTime.ToUniversalTime().ToString('o')
    output=$output
    progressRelativePath='_ArgosReview/C11RUN1/SUMMARY.json'
