...
$back = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03_BacksideNotchDevelopment_O3B10R20.py'
$backHash = 'B10AC6F0E0500AC3D14713232FCEDC5C209FBACF56AD7826AF87443C2587C46C'
$backConfig = Join-Path $r1Root 'BACKSIDE_NOTCH_CONFIG_R8.json'
$backConfigHash = 'CEECD910C2EE7CFE0DA1486B72DEC5F59CE3F66C4219CB55BB4C10263D436E2A'
$reviewRoot = 'D:\KLARFExport\_ArgosReview'
$output = Join-Path $reviewRoot 'C15RUN4'

Require ($env:COMPUTERNAME -eq 'A1025645101') 'R20 backside corpus launch reached the wrong computer.'
foreach ($dependency in @(
    ...
    Require (Test-Path -LiteralPath $dependency.Path -PathType Leaf) "R20 corpus dependency absent: $($dependency.Path)"
    Require ((Sha $dependency.Path) -eq $dependency.Hash) "R20 corpus dependency changed: $($dependency.Path)"
}
Require (-not (Test-Path -LiteralPath $output)) 'Create-new R20 backside corpus output already exists.'
if ($Preflight) {
    [ordered]@{state='PASS_O3B10_R20_BACKSIDE_CORPUS_ALL_PREFLIGHT';expectedPairCount=953;side='BACK';output=$output;processStarted=$false;imageBytesRead=$false;reviewOnly=$true} | ConvertTo-Json -Compress
    return
}
...
Require $process.Start() 'R20 backside corpus worker did not start.'
Start-Sleep -Seconds 3
Require (-not $process.HasExited) ("R20 backside corpus worker exited immediately with code $($process.ExitCode).")
[ordered]@{
    schema='argos_ocv03_r20_backside_corpus_all_launch_v1'
    state='PASS_O3B10_R20_BACKSIDE_CORPUS_ALL_LAUNCHED'
    expectedPairCount=953
    side='BACK'
    pid=$process.Id
    creationTimeUtc=$process.StartTime.ToUniversalTime().ToString('o')
    output=$output
    progressRelativePath='_ArgosReview/C15RUN4/SUMMARY.json'
