...
$output = Join-Path $reviewRoot 'C15RUN1'
...
if ($Preflight) {
    [ordered]@{state='PASS_O3B10_R14_BACKSIDE_CORPUS_ALL_PREFLIGHT';expectedPairCount=943;side='BACK';output=$output;processStarted=$false;imageBytesRead=$false;reviewOnly=$true} | ConvertTo-Json -Compress
    return
}
...
[ordered]@{
    schema='argos_ocv03_r14_backside_corpus_all_launch_v1'
    state='PASS_O3B10_R14_BACKSIDE_CORPUS_ALL_LAUNCHED'
    expectedPairCount=943
    side='BACK'
    pid=$process.Id
    creationTimeUtc=$process.StartTime.ToUniversalTime().ToString('o')
    output=$output
    progressRelativePath='_ArgosReview/C15RUN1/SUMMARY.json'
