...
$engine = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03_BacksideNotchDevelopment_O3B10R11.py'
$engineHash = 'E56C185E65E1CAECC6604519082F039279F0A12DA9ED1318AD38DBC5413C3B4B'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R11 regression reached the wrong computer.'
Require ((Sha $python) -eq $pythonHash) 'Pinned Python runtime changed.'
Require ((Sha $engine) -eq $engineHash) 'R11 detector changed.'
...
if ($Preflight) {
    [ordered]@{ state = 'PASS_O3B10_R11_REGRESSION_PREFLIGHT'; caseCount = $cases.Count; processStarted = $false; imageBytesRead = $false; reviewOnly = $true } | ConvertTo-Json -Compress
    return
}
...
foreach ($case in $cases) {
    ...
    Require $process.Start() "R11 detector did not start: $($case.id)"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(900000)) { try { $process.Kill() } catch {}; throw "R11 detector timed out: $($case.id)" }
    $stderr = $stderrTask.Result
    $null = $stdoutTask.Result
    Require ($process.ExitCode -eq 0) ("R11 detector failed: $($case.id): " + $stderr)
    ...
    foreach ($path in @($resultPath,$bfReview,$dfReview)) { Require (Test-Path -LiteralPath $path -PathType Leaf) "R11 output missing: $path" }
    $results += [ordered]@{
        id = $case.id
...
[ordered]@{ schema='argos_ocv03_o3b10_r11_regression_v1'; state='PASS_O3B10_R11_REGRESSION_EXECUTED'; results=$results; sourceMutationPerformed=$false; existingProcessActionPerformed=$false; ownedChildProcessCount=3; reviewOnly=$true; productionRoutingEnabled=$false } | ConvertTo-Json -Depth 32 -Compress
