...
$engine = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV03_BacksideNotchDevelopment_O3B10R6.py'
$engineHash = '8CDDB66409845322139A04B20B3037013A8596EFFB630426E131D9B65D68F355'
...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R6 regression reached the wrong computer.'
Require ((Sha $python) -eq $pythonHash) 'Pinned Python runtime changed.'
Require ((Sha $engine) -eq $engineHash) 'R6 detector changed.'
...
if ($Preflight) {
    [ordered]@{ state = 'PASS_O3B10_R6_REGRESSION_PREFLIGHT'; caseCount = $cases.Count; processStarted = $false; imageBytesRead = $false; reviewOnly = $true } | ConvertTo-Json -Compress
    return
}
...
foreach ($case in $cases) {
    ...
    Require $process.Start() "R6 detector did not start: $($case.id)"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(900000)) { try { $process.Kill() } catch {}; throw "R6 detector timed out: $($case.id)" }
    $stderr = $stderrTask.Result
    $null = $stdoutTask.Result
    Require ($process.ExitCode -eq 0) ("R6 detector failed: $($case.id): " + $stderr)
    ...
    foreach ($path in @($resultPath,$bfReview,$dfReview)) { Require (Test-Path -LiteralPath $path -PathType Leaf) "R6 output missing: $path" }
    $results += [ordered]@{
        id = $case.id
...
[ordered]@{ schema='argos_ocv03_o3b10_r6_regression_v1'; state='PASS_O3B10_R6_REGRESSION_EXECUTED'; results=$results; sourceMutationPerformed=$false; existingProcessActionPerformed=$false; ownedChildProcessCount=2; reviewOnly=$true; productionRoutingEnabled=$false } | ConvertTo-Json -Depth 32 -Compress
