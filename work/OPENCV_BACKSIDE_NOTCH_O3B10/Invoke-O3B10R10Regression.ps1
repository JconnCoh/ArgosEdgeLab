...
Require ($env:COMPUTERNAME -eq 'A1025645101') 'R10 regression reached the wrong computer.'
...
if ($Preflight) {
    [ordered]@{ state = 'PASS_O3B10_R10_REGRESSION_PREFLIGHT'; caseCount = $cases.Count; processStarted = $false; imageBytesRead = $false; reviewOnly = $true } | ConvertTo-Json -Compress
    return
}
...
foreach ($case in $cases) {
    ...
    Require $process.Start() "R10 detector did not start: $($case.id)"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(900000)) { try { $process.Kill() } catch {}; throw "R10 detector timed out: $($case.id)" }
    $stderr = $stderrTask.Result
    $null = $stdoutTask.Result
    Require ($process.ExitCode -eq 0) ("R10 detector failed: $($case.id): " + $stderr)
    ...
    foreach ($path in @($resultPath,$bfReview,$dfReview)) { Require (Test-Path -LiteralPath $path -PathType Leaf) "R10 output missing: $path" }
    $results += [ordered]@{
        id = $case.id
...
[ordered]@{ schema='argos_ocv03_o3b10_r10_regression_v1'; state='PASS_O3B10_R10_REGRESSION_EXECUTED'; results=$results; sourceMutationPerformed=$false; existingProcessActionPerformed=$false; ownedChildProcessCount=3; reviewOnly=$true; productionRoutingEnabled=$false } | ConvertTo-Json -Depth 32 -Compress
