...
def radial_profile(mask: np.ndarray, cx: float, cy: float, radius: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    if float(np.mean(valid)) < 0.995:
        raise RuntimeError("Backside perimeter coverage is incomplete.")
    reversed_index = np.argmax(inside[:, ::-1], axis=1)
    last_index = inside.shape[1] - 1 - reversed_index
    return angles, samples[last_index]
...
def analyze(path: Path, channel: str, maximum_dimension: int) -> tuple[dict, np.ndarray]:
    ...
    angles, radii = radial_profile(mask, cx, cy, radius)
...
def analyze_df_radial(
    ...
    native = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
    if native is None:
        raise RuntimeError(f"DF decode failed: {path}")
    native_shape = [int(native.shape[0]), int(native.shape[1])]
    parameters = radial_engine.Parameters.from_json(radial_parameters)
    radial = radial_engine.localize_channel(native, "DF", parameters)
    scale = min(1.0, maximum_dimension / max(native.shape))
    gray = cv2.resize(native, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA)
    enhanced = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(8, 8)).apply(gray)
    overlay = cv2.cvtColor(enhanced, cv2.COLOR_GRAY2BGR)
    ...
    if not radial.get("qualified") or not fit:
        raise RuntimeError(
            f"Frozen R6 DF radial channel did not qualify: {radial.get('state', 'UNKNOWN')}"
        )
    circle = {
...
def main() -> int:
    ...
    for channel, path in (("BF", bf_path), ("DF", df_path)):
        try:
            analyses[channel] = (
                analyze(path, channel, args.maximum_dimension)
                if channel == "BF"
                else analyze_df_radial(path, args.maximum_dimension, radial_engine, radial_parameters)
            )
    ...
    if failures:
        result = {
            ...
            "patternSuppression": "FILLED_EXTERNAL_WAFER_CONTOUR_BEFORE_RADIAL_PROFILE",
            "failures": failures,
