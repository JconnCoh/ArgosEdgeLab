...
def analyze_radial(
    ...
    if not fit:
        qualification = {
            "state": radial.get("state", "UNKNOWN"),
            "fit": radial.get("fit", {}),
            "search": radial.get("search", {}),
        }
        raise RuntimeError(
            f"Frozen R6 {channel} radial channel did not qualify: "
            + json.dumps(qualification, separators=(",", ":"), sort_keys=True)
        )
    circle = {
    ...
    gc.collect()
    if channel == "BF":
        boundary = trace_boundary.astype(np.float32) * scale
        if not np.all(trace_supported):
            supported = np.flatnonzero(trace_supported)
            extended_indices = np.concatenate((supported - len(boundary), supported, supported + len(boundary)))
            extended_boundary = np.tile(boundary[supported], 3)
            boundary[~trace_supported] = np.interp(
                np.flatnonzero(~trace_supported), extended_indices, extended_boundary
            )
        _, profile_threshold, source_candidates = candidates_from_profile(boundary)
    else:
        profile_threshold = float(radial["candidateDepthThresholdPx"]) * scale
        source_candidates = radial.get("candidates", [])
    candidates = []
    ...
    result = {
        ...
        "patternSuppression": (
            "FULL_360_BOUNDARY_WITH_BOUNDED_GAP_INTERPOLATION_AND_LOCAL_HIGH_PASS"
            if channel == "BF" else "FULL_360_OUTERMOST_DARK_EXTERIOR_BOUNDARY"
        ),
        "circle": circle,
        "profileThresholdPx": profile_threshold,
        ...
        "radialQualification": {
            "qualified": bool(radial.get("qualified")),
            "state": radial["state"],
            "fit": radial["fit"],
...
def pair_candidates(bf: dict, df: dict) -> list[dict[str, float]]:
    ...
    return sorted(pairs, key=lambda row: (-row["score"], row["meanAngleDegrees"]))
...
def main() -> int:
    ...
    for channel, (analysis, overlay) in analyses.items():
        if not cv2.imwrite(str(review_paths[channel]), overlay, [cv2.IMWRITE_JPEG_QUALITY, 90]):
            raise RuntimeError(f"{channel} diagnostic trace JPEG write failed.")
        qualification = analysis["radialQualification"]
        fit = qualification["fit"]
        inlier_passed = float(fit["inlierFraction"]) >= float(radial_parameters["minimumFitInlierFraction"])
        coverage_bin_tolerance = 0.5 / 72.0
        coverage_passed = (
            float(fit["angularCoverageFraction"]) + coverage_bin_tolerance
            >= float(radial_parameters["minimumAngularCoverageFraction"])
        )
        rms_passed = (
            float(fit["rmsResidualPx"]) <= float(radial_parameters["maximumFitRmsResidualPx"])
        )
        bf_low_frequency_shape_suppressed = channel == "BF" and not rms_passed
        strict_trace_usable = inlier_passed and coverage_passed and (rms_passed or bf_low_frequency_shape_suppressed)
        coverage_deferred_to_unique_pair = (
            channel == "DF"
            and inlier_passed
            and rms_passed
            and not coverage_passed
            and float(fit["angularCoverageFraction"])
            >= float(radial_parameters["minimumCrossChannelCompensatedDfCoverageFraction"])
        )
        trace_usable = strict_trace_usable or coverage_deferred_to_unique_pair
        analysis["backsideTraceQualification"] = {
            "usable": trace_usable,
            "strictUsable": strict_trace_usable,
            "inlierPassed": inlier_passed,
            "coveragePassedWithHalfBinTolerance": coverage_passed,
            "coverageHalfBinToleranceFraction": coverage_bin_tolerance,
            "coverageDeferredToUniqueCrossChannelPair": coverage_deferred_to_unique_pair,
            "minimumCrossChannelCompensatedDfCoverageFraction": float(
                radial_parameters["minimumCrossChannelCompensatedDfCoverageFraction"]
            ),
            "rawRmsPassed": rms_passed,
            "bfLowFrequencyShapeSuppressed": bf_low_frequency_shape_suppressed,
            "manufacturedMorphologyAndCrossChannelPairingStillRequired": True,
        }
        if not trace_usable:
            failures.append({
                "channel": channel,
                "stage": "WAFER_MASK_OR_RADIAL_PROFILE",
                "reason": (
                    f"Backside {channel} trace did not qualify: "
                    + json.dumps(analysis["backsideTraceQualification"], separators=(",", ":"), sort_keys=True)
                ),
            })
    pairs: list[dict[str, float]] = []
    if "BF" in analyses and "DF" in analyses:
        bf, bf_overlay = analyses["BF"]
        df, df_overlay = analyses["DF"]
        pairs = pair_candidates(bf, df)
        df_trace = df["backsideTraceQualification"]
        if df_trace["coverageDeferredToUniqueCrossChannelPair"]:
            compensation_passed = len(pairs) == 1
            df_trace["coverageCompensatedByUniqueCrossChannelPair"] = compensation_passed
            df_trace["uniqueCrossChannelPairCountAtCompensation"] = len(pairs)
            df_trace["usable"] = compensation_passed
            if not compensation_passed:
                failures.append({
                    "channel": "DF",
                    "stage": "WAFER_MASK_OR_RADIAL_PROFILE",
                    "reason": (
                        "Backside DF coverage compensation requires exactly one unchanged "
                        f"manufactured BF/DF pair; observed {len(pairs)}."
                    ),
                })
    if failures:
        result = {
