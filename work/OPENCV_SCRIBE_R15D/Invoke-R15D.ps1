...
$providerRevision = 'ARGOS_OPENCV_SCRIBE_GRID_DIAGNOSTIC_R15D_20260903'
$requestId = 'REQ_20260903T130000000Z_R15D'
...
$liveProviderSha256 = 'EAD634E14F133C36AF498BD1B4713416D075A0FD6542F03A1C4C583C5157E2B7'
...
function Get-RequiredProperty([object]$Object, [string]$Name, [string]$Context) {
    Assert-True ($null -ne $Object) "R15D missing object: $Context"
    $property = $Object.PSObject.Properties[$Name]
    Assert-True ($null -ne $property) "R15D missing property: $Context.$Name"
    return $property.Value
}

function Assert-Sha256Text([string]$Value, [string]$Context) {
    Assert-True ($Value -match '^[0-9A-F]{64}$') "R15D invalid SHA-256: $Context"
}
...
function Get-FullPath([string]$Path, [string]$Context) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($Path)) "R15D empty path: $Context"
    Assert-True ($Path.IndexOf('"') -lt 0 -and $Path.IndexOf("`r") -lt 0 -and $Path.IndexOf("`n") -lt 0) "R15D unsafe path text: $Context"
    $full = [IO.Path]::GetFullPath($Path)
    Assert-True ([IO.Path]::IsPathRooted($full)) "R15D path is not rooted: $Context"
    return $full
}
...
function Assert-PathComponents([string]$Path, [string]$Context) {
    ...
    Assert-True ($longest -le 80) "R15D path component exceeds 80 characters: $Context"
}
...
function Assert-PathBudget([string]$Path, [string]$Context, [int]$Reserve = 32) {
    ...
    Assert-True (($full.Length + $Reserve) -lt 200) "R15D unsafe effective path: $Context"
}

function Assert-CanonicalSourcePath([string]$Path, [string]$Context) {
    Assert-PathComponents $Path $Context
    Assert-True ($Path.Length -lt 230) "R15D canonical source path reaches hard stop: $Context"
}
...
function Get-SubstRows([string]$SubstPath) {
    ...
    Assert-True ($LASTEXITCODE -eq 0) ('R15D subst inventory failed: ' + $output.Trim())
...
function Assert-XAliasAbsent([string]$SubstPath) {
    ...
    Assert-True ($matching.Count -eq 0) 'R15D X: subst alias is already in use.'
    Assert-True ($null -eq (Get-PSDrive -Name X -ErrorAction SilentlyContinue)) 'R15D X: PowerShell drive is already in use.'
    Assert-True (-not (Test-Path -LiteralPath 'X:\')) 'R15D X: filesystem path is already in use.'
}

function Assert-Authority([object]$Authority, [string]$Context, [string[]]$FalseFields) {
    Assert-True ([bool](Get-RequiredProperty $Authority 'reviewOnly' $Context)) "R15D review-only authority changed: $Context"
    foreach ($field in $FalseFields) {
        Assert-True (-not [bool](Get-RequiredProperty $Authority $field $Context)) "R15D forbidden authority enabled: $Context.$field"
    }
}
...
function Assert-TargetPositions([object[]]$Actual, [object[]]$Expected, [string]$Context) {
    ...
    Assert-True ($actualRows.Count -eq $expectedRows.Count) "R15D target-position count changed: $Context"
    for ($index = 0; $index -lt $expectedRows.Count; $index++) {
        Assert-True ([int]$actualRows[$index].position -eq [int]$expectedRows[$index].position) "R15D target position changed: $Context"
        Assert-True ([string]$actualRows[$index].label -ceq [string]$expectedRows[$index].label) "R15D target label changed: $Context"
    }
}
...
function Invoke-BoundedPython([string]$Python, [string]$Provider, [string]$Job, [string]$OutputRoot, [int]$TimeoutSeconds) {
    foreach ($path in @($Python,$Provider,$Job,$OutputRoot)) {
        Assert-True ($path.IndexOf('"') -lt 0 -and $path.IndexOf("`r") -lt 0 -and $path.IndexOf("`n") -lt 0) 'R15D process argument contains unsafe text.'
    }
...
function Get-SafeArtifactPath([string]$CaseRoot, [string]$RelativePath) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($RelativePath)) 'R15D artifact relative path is empty.'
    Assert-True (-not [IO.Path]::IsPathRooted($RelativePath)) 'R15D artifact path is rooted.'
    Assert-True ($RelativePath.IndexOf(':') -lt 0 -and $RelativePath.IndexOf('"') -lt 0) 'R15D artifact path contains forbidden text.'
    $normalizedRelative = $RelativePath.Replace('/', '\')
    $segments = @($normalizedRelative.Split([char[]]@('\'), [StringSplitOptions]::RemoveEmptyEntries))
    Assert-True ($segments.Count -gt 0 -and @($segments | Where-Object { $_ -eq '..' -or $_ -eq '.' }).Count -eq 0) 'R15D artifact path traverses its case root.'
    $full = [IO.Path]::GetFullPath((Join-WindowsPath $CaseRoot $normalizedRelative))
    $prefix = [IO.Path]::GetFullPath($CaseRoot).TrimEnd('\') + '\'
    Assert-True ($full.StartsWith($prefix, [StringComparison]::OrdinalIgnoreCase)) 'R15D artifact escaped its case root.'
...
Assert-True (-not [bool]$Rehearsal -or $explicitInvocation) 'R15D rehearsal requires explicit -InvocationManifest.'
if ($explicitInvocation) {
    ...
    $longDefault = Join-Path $PSScriptRoot 'R15D_LIVE_INVOCATION.json'
    $invocationPath = if (Test-Path -LiteralPath $shortDefault -PathType Leaf) { $shortDefault } else { $longDefault }
}
Assert-True (Test-Path -LiteralPath $invocationPath -PathType Leaf) "R15D invocation manifest absent: $invocationPath"
Assert-True ((Get-Item -LiteralPath $invocationPath).Length -le 262144) 'R15D invocation manifest exceeds 256 KiB.'
...
Assert-True ([string]$invocation.schema -eq 'argos_opencv_scribe_alphabet_crop_live_invocation_v1') 'R15D invocation schema changed.'
Assert-True ([string]$invocation.revision -eq $revision -and [string]$invocation.requestId -eq $requestId) 'R15D invocation identity changed.'
Assert-True ([string]$invocation.state -eq 'FROZEN') 'R15D invocation is not frozen.'
...
Assert-True ([int]$invocation.authority.maximumPublications -eq 1) 'R15D publication count changed.'

$expectedComputerName = [string]$invocation.expectedComputerName
Assert-True (-not [string]::IsNullOrWhiteSpace($expectedComputerName)) 'R15D expected computer is absent.'
if (-not $Rehearsal) { Assert-True ($expectedComputerName -eq $liveComputerName) 'R15D live computer contract changed.' }
Assert-True ($env:COMPUTERNAME.Equals($expectedComputerName, [StringComparison]::OrdinalIgnoreCase)) "R15D wrong computer: $($env:COMPUTERNAME)"
...
Assert-True ($pythonPath.Equals((Join-WindowsPath $runtimeRoot 'python.exe'), [StringComparison]::OrdinalIgnoreCase)) 'R15D Python is outside the exact runtime root.'
...
if ($Rehearsal) {
    Assert-True ($runtimeInstallationSha256 -eq $rehearsalRuntimeInstallationSha256) 'R15D rehearsal runtime installation fixture pin changed.'
}
else {
    Assert-True ($runtimeInstallationSha256 -eq $liveRuntimeInstallationSha256) 'R15D live runtime installation pin changed.'
}
Assert-True ([string]$invocation.referenceBundle.sha256 -eq $referenceBundleSha256) 'R15D reference bundle pin changed.'
Assert-True ([string]$invocation.referenceBundle.manifestSha256 -eq $referenceManifestSha256 -and [int]$invocation.referenceBundle.referenceCount -eq 456 -and [string]$invocation.referenceBundle.missingLabels -eq 'IJKOQVWXYZ') 'R15D reference contract changed.'
foreach ($path in @($pythonPath,$installationPath,$referenceBundlePath)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "R15D dependency absent: $path"
    Assert-PathBudget $path 'runtime/reference dependency' 32
}
Assert-True ((Get-Sha256 $installationPath) -eq $runtimeInstallationSha256) 'R15D runtime installation evidence changed.'
Assert-True ((Get-Sha256 $referenceBundlePath) -eq $referenceBundleSha256) 'R15D reference bundle bytes changed.'
$pythonCommand = Get-Command -Name $pythonPath -CommandType Application -ErrorAction Stop
Assert-True ([IO.Path]::GetFullPath($pythonCommand.Source).Equals($pythonPath, [StringComparison]::OrdinalIgnoreCase)) 'R15D Python command resolution changed.'

$substPath = Join-Path $env:SystemRoot 'System32\subst.exe'
Assert-True (Test-Path -LiteralPath $substPath -PathType Leaf) 'R15D subst.exe is absent.'
$substCommand = Get-Command -Name $substPath -CommandType Application -ErrorAction Stop
Assert-True ([IO.Path]::GetFullPath($substCommand.Source).Equals([IO.Path]::GetFullPath($substPath), [StringComparison]::OrdinalIgnoreCase)) 'R15D subst command resolution changed.'
...
Assert-True ([string]$invocation.roots.sourceAlias -eq 'X:') 'R15D source alias changed.'
Assert-True ($workPartial.Equals(($workRoot + '.partial'), [StringComparison]::OrdinalIgnoreCase) -and $workFailed.Equals(($workRoot + '.failed'), [StringComparison]::OrdinalIgnoreCase)) 'R15D work lifecycle roots changed.'
Assert-True ($outputPartial.Equals(($outputRoot + '.partial'), [StringComparison]::OrdinalIgnoreCase)) 'R15D output lifecycle root changed.'
if (-not $Rehearsal) {
    Assert-True ($workRoot -eq 'D:\A2\w\ocv\R15D' -and $outputRoot -eq 'D:\A2\o\ocv\R15D' -and $bundlePath -eq 'D:\A2\o\ocv\R15D_RETURN.zip') 'R15D live roots changed.'
    foreach ($path in @($workRoot,$workPartial,$workFailed,$outputRoot,$outputPartial,$bundlePath,$bundlePartial)) {
        Assert-True ($path.StartsWith('D:\', [StringComparison]::OrdinalIgnoreCase)) 'R15D live create-new path left D:.'
    }
}
Assert-True ([IO.Path]::GetPathRoot($workRoot).Equals([IO.Path]::GetPathRoot($workPartial), [StringComparison]::OrdinalIgnoreCase)) 'R15D work atomic move crosses volumes.'
Assert-True ([IO.Path]::GetPathRoot($outputRoot).Equals([IO.Path]::GetPathRoot($outputPartial), [StringComparison]::OrdinalIgnoreCase)) 'R15D output atomic move crosses volumes.'
foreach ($path in @($workRoot,$workPartial,$workFailed,$outputRoot,$outputPartial,$bundlePath,$bundlePartial)) {
    Assert-PathBudget $path 'create-new root' 32
    Assert-True (-not (Test-Path -LiteralPath $path)) "R15D create-new root exists: $path"
}
foreach ($parent in @((Split-Path -Parent $workRoot),(Split-Path -Parent $outputRoot),(Split-Path -Parent $bundlePath))) {
    Assert-True (Test-Path -LiteralPath $parent -PathType Container) "R15D required parent root absent: $parent"
}

Assert-True ([int]$invocation.execution.maximumSequentialProviderChildren -eq 4 -and [int]$invocation.execution.maximumConcurrentProviderChildren -eq 1) 'R15D provider serialization changed.'
Assert-True ([int]$invocation.execution.providerChildTimeoutSeconds -eq 600 -and [int]$invocation.execution.endpointWorkerOuterTimeoutSeconds -eq 3000 -and -not [bool]$invocation.execution.automaticRetryAllowed -and -not [bool]$invocation.execution.caseFailureStopsBatch) 'R15D provider timeout/retry contract changed.'
Assert-True ([bool]$invocation.execution.sourceHashBeforeOpenCvDecodeRequired -and [bool]$invocation.execution.sourceImagesProcessedReadOnlyInPlace -and -not [bool]$invocation.execution.fullSourceImageReturnAllowed) 'R15D source-read contract changed.'
Assert-True ([int64]$invocation.outputContract.maximumBundleBytes -eq $maximumBundleBytes -and [int]$invocation.outputContract.maximumBundleBase64Characters -eq $maximumBundleBase64Characters) 'R15D bundle bounds changed.'
Assert-True ([string]$invocation.outputContract.portalResponseTransport -eq 'MAINTENANCE_STDOUT_COMPACT_JSON_BASE64' -and [bool]$invocation.outputContract.sourceToRegionToHypothesisSha256Required) 'R15D output provenance contract changed.'

$providerPin = $invocation.payload.provider
$configurationPin = $invocation.payload.configuration
Assert-True ([string]$providerPin.file -eq 'R15D.py' -and [string]$configurationPin.file -eq 'CFG.json') 'R15D short payload filenames changed.'
...
if (-not $Rehearsal) { Assert-True ([string]$providerPin.sha256 -eq $liveProviderSha256) 'R15D live provider pin changed.' }
Assert-True ([string]$configurationPin.sha256 -eq $configurationSha256) 'R15D configuration pin changed.'
...
foreach ($expectedDependency in $dependencyExpectations) {
    ...
    Assert-True ([string]$dependency.file -eq [string]$expectedDependency.file -and [string]$dependency.sha256 -eq [string]$expectedDependency.sha256) "R15D dependency pin changed: $($expectedDependency.id)"
...
foreach ($pin in $payloadPins) {
    Assert-True (Test-Path -LiteralPath $pin.path -PathType Leaf) "R15D payload absent: $($pin.id)"
    Assert-PathBudget $pin.path "payload $($pin.id)" 32
    Assert-True ((Get-Sha256 $pin.path) -eq $pin.sha256) "R15D payload hash changed: $($pin.id)"
}
$configuration = Get-Content -Raw -LiteralPath $configurationSource | ConvertFrom-Json
Assert-True ([string]$configuration.schema -eq 'argos_opencv_scribe_alphabet_crop_configuration_v1' -and [string]$configuration.revision -eq $revision) 'R15D configuration contract changed.'
...
Assert-True ($invocationCases.Count -eq 4) 'R15D exact case count changed.'
...
for ($caseIndex = 0; $caseIndex -lt $expectedCases.Count; $caseIndex++) {
    ...
    foreach ($field in @('caseId','physicalIdentity','purpose','confirmedScribe','diagnosticRegionId','aliasAnchorCanonicalPath','bfRelativePath','dfRelativePath')) {
        Assert-True ([string]$actualCase.$field -ceq [string]$expectedCase.$field) "R15D case descriptor changed: $($expectedCase.caseId)/$field"
    }
    $caseSourceBytes = [int64]$actualCase.sourceBytesPerImage
    if ($Rehearsal) {
        Assert-True ($caseSourceBytes -ge 1 -and $caseSourceBytes -le $maximumRehearsalSourceBytesPerImage) "R15D rehearsal source-byte fixture bound changed: $($expectedCase.caseId)"
    }
    else {
        Assert-True ($caseSourceBytes -eq $liveSourceBytesPerImage) "R15D live source-byte contract changed: $($expectedCase.caseId)"
    }
    Assert-True (-not [bool]$actualCase.referenceAdmissionEligible) "R15D case admission contract changed: $($expectedCase.caseId)"
    ...
    if ($null -ne $sourceRootProperty -and -not [string]::IsNullOrWhiteSpace([string]$sourceRootProperty.Value)) {
        Assert-True ([bool]$Rehearsal -or ([string]$sourceRootProperty.Value -eq [string]$expectedCase.aliasAnchorCanonicalPath)) "R15D live source-root override refused: $($expectedCase.caseId)"
        $sourceRoot = Get-FullPath ([string]$sourceRootProperty.Value) "source root $($expectedCase.caseId)"
    }
    Assert-True (Test-Path -LiteralPath $sourceRoot -PathType Container) "R15D source root absent: $($expectedCase.caseId)"
    ...
    foreach ($sourceSpec in @([pscustomobject]@{channel='bf';path=$bfCanonical;relative=[string]$expectedCase.bfRelativePath},[pscustomobject]@{channel='df';path=$dfCanonical;relative=[string]$expectedCase.dfRelativePath})) {
        Assert-True (Test-Path -LiteralPath $sourceSpec.path -PathType Leaf) "R15D source absent: $($expectedCase.caseId)/$($sourceSpec.channel)"
        $item = Get-Item -LiteralPath $sourceSpec.path
        Assert-True ([int64]$item.Length -eq $caseSourceBytes) "R15D source byte metadata changed: $($expectedCase.caseId)/$($sourceSpec.channel)"
        Assert-True (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0) "R15D source is a reparse point: $($expectedCase.caseId)/$($sourceSpec.channel)"
...
if ($Preflight) {
    [ordered]@{
        schema = 'argos_opencv_scribe_r15d_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R15D_STRICTLY_NONMUTATING_PREFLIGHT'
        revision = $revision
        requestId = $requestId
...
try {
    ...
    foreach ($pin in $payloadPins) {
        ...
        Assert-True ((Get-Sha256 (Join-Path $workPartial $pin.file)) -eq $pin.sha256) "R15D staged payload changed: $($pin.id)"
    }
    Copy-Item -LiteralPath $invocationPath -Destination (Join-Path $workPartial 'LIVE.json')
    Assert-True ((Get-Sha256 (Join-Path $workPartial 'LIVE.json')) -eq $invocationSha256) 'R15D staged live manifest changed.'
    $stagedReferenceManifest = Join-Path $workPartial 'refs\PORTABLE_GLYPH_REFERENCE_MANIFEST.json'
    Assert-True (Test-Path -LiteralPath $stagedReferenceManifest -PathType Leaf) 'R15D extracted reference manifest is absent.'
    Assert-True ((Get-Sha256 $stagedReferenceManifest) -eq $referenceManifestSha256) 'R15D extracted reference manifest changed.'
    foreach ($relativeRoot in @('refs\glyphs','refs\glyphs_v5_confirmed_20260806')) {
        Assert-True (Test-Path -LiteralPath (Join-Path $workPartial $relativeRoot) -PathType Container) "R15D extracted reference root absent: $relativeRoot"
    }
    ...
    foreach ($entry in $validatedCases) {
        ...
        foreach ($path in @($caseOutput,$caseOutputPartial,$caseAttemptQuarantine,$runtimeJobPath)) {
            Assert-True (-not (Test-Path -LiteralPath $path)) "R15D case create-new path exists: $caseId/$path"
        ...
        $caseState = 'HOLD_R15D_CASE_LAUNCH_FAILURE'
        $caseDetail = ''
        $artifactRows = @()
        try {
            Assert-XAliasAbsent $substPath
            $createOutput = & $substPath 'X:' ([string]$entry.sourceRoot) 2>&1 | Out-String
            Assert-True ($LASTEXITCODE -eq 0 -and (Test-Path -LiteralPath 'X:\' -PathType Container)) ('R15D alias creation failed: ' + $createOutput.Trim())
            $aliasCreated = $true
            $mappingRows = @(Get-SubstRows $substPath)
            $matchingRows = @($mappingRows | Where-Object { $_ -match '(?i)^\s*X:\\:\s*=>\s*(.+?)\s*$' })
            Assert-True ($matchingRows.Count -eq 1) 'R15D alias mapping cardinality changed.'
            $mappedTarget = [regex]::Match($matchingRows[0], '(?i)^\s*X:\\:\s*=>\s*(.+?)\s*$').Groups[1].Value.Trim().TrimEnd('\')
            Assert-True ($mappedTarget.Equals(([string]$entry.sourceRoot).TrimEnd('\'), [StringComparison]::OrdinalIgnoreCase)) "R15D alias target changed: $caseId"
            ...
            foreach ($sourcePath in @($bfAliasPath,$dfAliasPath)) {
                Assert-True (Test-Path -LiteralPath $sourcePath -PathType Leaf) "R15D alias source absent: $caseId"
                Assert-True ([int64](Get-Item -LiteralPath $sourcePath).Length -eq [int64]$entry.sourceBytes) "R15D alias source byte metadata changed: $caseId"
            }
            ...
            $providerRun = Invoke-BoundedPython -Python $pythonPath -Provider (Join-Path $workRoot 'R15D.py') -Job $runtimeJobPath -OutputRoot $caseOutput -TimeoutSeconds 600
            Assert-True ([bool]$providerRun.started) "R15D provider launch failed: $caseId`: $($providerRun.launchError)"
            Assert-True (-not [bool]$providerRun.timedOut) "R15D provider timed out: $caseId"
            Assert-True ($null -ne $providerRun.exitCode -and [int]$providerRun.exitCode -in @(0,2)) "R15D provider exit code changed: $caseId`: $($providerRun.stderr)"
            Assert-True (Test-Path -LiteralPath $caseResultPath -PathType Leaf) "R15D CASE_RESULT absent: $caseId"

            $caseResult = Get-Content -Raw -LiteralPath $caseResultPath | ConvertFrom-Json
            Assert-True ([string]$caseResult.schema -eq 'argos_opencv_scribe_grid_diagnostic_case_result_v1' -and [string]$caseResult.revision -eq $providerRevision) "R15D case-result contract changed: $caseId"
            Assert-True ([string]$caseResult.caseId -eq $caseId -and [string]$caseResult.physicalIdentity -eq [string]$entry.expected.physicalIdentity) "R15D case-result identity changed: $caseId"
            ...
            Assert-True ([string]$caseResult.provenance.sources.bf.sha256 -eq $bfSha256 -and [string]$caseResult.provenance.sources.df.sha256 -eq $dfSha256) "R15D case-result source hashes changed: $caseId"
            Assert-True ([string]$caseResult.provenance.sourcePairSha256 -eq $sourcePairSha256) "R15D case-result source-pair hash changed: $caseId"
            $artifactRows = @($caseResult.artifacts)
            Assert-True ($artifactRows.Count -le 15) "R15D artifact count exceeds contract: $caseId"
            if ($artifactRows.Count -eq 0) {
                Assert-True ([string]$caseResult.state -like 'HOLD_*') "R15D empty-artifact result is not a hold: $caseId"
                ...
                Assert-True ([int]$providerRun.exitCode -eq 0) "R15D nonzero provider exit returned artifacts: $caseId"
                ...
                Assert-True ($artifactRows.Count -eq 6 -and $regionRows.Count -eq 2 -and $sheetRows.Count -eq 4) "R15D diagnostic artifact cardinality changed: $caseId"
                Assert-True ([string]$caseResult.diagnosticRegion.regionId -eq [string]$entry.expected.diagnosticRegionId) "R15D diagnostic region changed: $caseId"
                foreach ($channel in @('BF','DF')) {
                    ...
                    Assert-True ($regionArtifact.Count -eq 1) "R15D rectified region artifact changed: $caseId/$channel"
                    $expectedSourceHash = if ($channel -eq 'BF') { $bfSha256 } else { $dfSha256 }
                    Assert-True ([string]$regionArtifact[0].sourceSha256 -eq $expectedSourceHash) "R15D rectified source binding changed: $caseId/$channel"
                    foreach ($direction in @('FORWARD','REVERSE_180')) {
                        ...
                        Assert-True ($sheet.Count -eq 1) "R15D hypothesis sheet changed: $caseId/$channel/$direction"
                    }
                }
                foreach ($artifact in $artifactRows) {
                    ...
                    Assert-True (Test-Path -LiteralPath $artifactPath -PathType Leaf) "R15D artifact absent: $caseId/$($artifact.relativePath)"
                    Assert-True ((Get-Sha256 $artifactPath) -eq [string]$artifact.sha256) "R15D artifact hash changed: $caseId/$($artifact.relativePath)"
                }
            }
            ...
            Assert-True ($actualFiles.Count -le 16) "R15D case file count exceeds contract: $caseId"
            foreach ($file in $actualFiles) {
                Assert-True ($file.Extension -in @('.json','.png')) "R15D forbidden case output type: $caseId/$($file.Name)"
                Assert-True ([int64]$file.Length -le $maximumBundleBytes) "R15D case output exceeds bundle bound: $caseId/$($file.Name)"
                if (-not $file.FullName.Equals($caseResultPath, [StringComparison]::OrdinalIgnoreCase)) {
                    ...
                    Assert-True ($declaredPaths -contains $relative) "R15D undeclared case output: $caseId/$relative"
                }
            }
            ...
            if ($aliasCreated) {
                ...
                if ($LASTEXITCODE -ne 0 -or (Test-Path -LiteralPath 'X:\')) { $aliasCleanupFailure = 'R15D X: alias removal failed: ' + $removeOutput.Trim() }
                $aliasCreated = $false
            }
        }
    ...
    Assert-True ($processorAfterJson -ceq $processorBeforeJson) 'R15D protected processor rows changed.'
    $failureCount = @($caseRows.ToArray() | Where-Object { [string]$_.state -like 'HOLD_R15D_CASE_LAUNCH_FAILURE' }).Count
    ...
    $batchGate = [ordered]@{
        schema='argos_opencv_scribe_r15d_batch_gate_v1';revision=$revision;createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15D_BATCH_COMPLETE';disposition='PENDING_GATE';requestId=$requestId;rehearsal=[bool]$Rehearsal
    ...
    Write-JsonNew $executionPath ([ordered]@{
        schema='argos_opencv_scribe_r15d_execution_v1';revision=$revision;createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15D_EXECUTION';requestId=$requestId;computerName=$env:COMPUTERNAME;rehearsal=[bool]$Rehearsal
    ...
    Assert-True ($bundleBytes -le $maximumBundleBytes) "R15D return bundle exceeds $maximumBundleBytes bytes."
    ...
    Assert-True ($bundleBase64.Length -le $maximumBundleBase64Characters) "R15D return Base64 exceeds $maximumBundleBase64Characters characters."
    [ordered]@{
        schema='argos_opencv_scribe_r15d_maintenance_envelope_v1';revision=$revision;state='PASS_R15D_SIGNED_RETURN_READY';requestId=$requestId;rehearsal=[bool]$Rehearsal
