...
Assert-True (Test-Path -LiteralPath $InvocationManifest -PathType Leaf) 'R15D build invocation manifest is absent.'
...
Assert-True ([string]$invocation.schema -eq 'argos_r15d_build_invocation_v1') 'R15D build invocation schema changed.'
Assert-True ([string]$invocation.requestId -eq 'REQ_20260903T130000000Z_R15D') 'R15D build request identity changed.'
Assert-True ([string]$invocation.requiredBranch -eq 'codex/opencv-scribe-deciphering') 'R15D build branch changed.'
Assert-True ([bool]$invocation.authority.reviewOnly -and -not [bool]$invocation.authority.automaticIdentityAuthority -and -not [bool]$invocation.authority.trainingEligible -and -not [bool]$invocation.authority.xmlEligible -and -not [bool]$invocation.authority.productionEligible -and -not [bool]$invocation.authority.productionRoutingEnabled -and -not [bool]$invocation.authority.providerActivationAllowed) 'R15D build authority widened.'
Assert-True ([int]$invocation.authority.maximumPublications -eq 1 -and -not [bool]$invocation.authority.retryAuthorized) 'R15D publication count or retry boundary changed.'
Assert-True ((Get-Sha256 $MyInvocation.MyCommand.Path) -eq [string]$invocation.builderSha256) 'R15D builder self-pin changed.'
...
Assert-PinnedFile $contractPath ([string]$invocation.requestContract.sha256) 'R15D request contract'
Assert-PinnedFile $definitionPath ([string]$invocation.maintenanceDefinition.sha256) 'R15D maintenance definition'
$contract = Get-Content -Raw -LiteralPath $contractPath | ConvertFrom-Json
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Assert-True ([string]$contract.requestId -eq [string]$invocation.requestId -and [string]$contract.classification -match '^FROZEN') 'R15D request contract is not the frozen request.'
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH') 'R15D maintenance route changed.'
Assert-True ([string]$definition.entryPoint -eq [string]$invocation.entryPoint) 'R15D maintenance entrypoint changed.'
Assert-True ([int64]$definition.maxResultBytes -eq 16777216) 'R15D maximum portal result bytes changed.'

$payloadRows = @($invocation.payloads)
Assert-True ($payloadRows.Count -eq 7) 'R15D payload cardinality changed.'
...
foreach ($row in $payloadRows) {
    ...
    Assert-PinnedFile $source ([string]$row.sha256) 'R15D payload'
    $packagePath = ([string]$row.packagePath).Replace('\', '/')
    Assert-True ($packagePath -match '^payload/[A-Za-z0-9._-]+$') "R15D payload package path is not a single safe leaf: $packagePath"
    Assert-True ($packagePath -notmatch '(?i)(__pycache__|\.pyc$)') "R15D cache bytecode is forbidden: $packagePath"
    Assert-True ($seenPackagePaths.Add($packagePath)) "R15D duplicate payload package path: $packagePath"
...
Assert-True ($seenPackagePaths.Contains([string]$definition.entryPoint)) 'R15D entrypoint is not one exact payload.'

$endpointRow = @($payloadRows | Where-Object { ([string]$_.packagePath).Replace('\','/') -eq [string]$definition.entryPoint })
Assert-True ($endpointRow.Count -eq 1) 'R15D endpoint payload cardinality changed.'
Assert-True (@($definition.changes).Count -eq 1) 'R15D maintenance change cardinality changed.'
$change = $definition.changes[0]
Assert-True ([string]$change.source -eq [string]$definition.entryPoint -and [string]$change.destination -eq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R15D.ps1') 'R15D installed endpoint identity changed.'
Assert-True ([bool]$change.allowCreate -and [string]$change.installedSha256 -eq [string]$endpointRow[0].sha256) 'R15D installed endpoint hash does not match payload.'
Assert-True (@($change.approvedPredecessorSha256).Count -eq 1 -and [string]$change.approvedPredecessorSha256[0] -eq '159BD82528E505DE69BDEEE4C3A3B29402BA9281939C57F6187587E9047D9740') 'R15D approved installed predecessor changed.'
Assert-True (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R15D task/process action boundary changed.'
Assert-True ([int]$definition.timeoutContract.maximumSequentialChildren -eq 4 -and [int]$definition.timeoutContract.maximumConcurrentChildren -eq 1 -and [int]$definition.timeoutContract.opencvChildTimeoutSeconds -eq 600 -and [int]$definition.timeoutContract.endpointWorkerOuterTimeoutSeconds -eq 3000) 'R15D timeout contract changed.'
Assert-True ([int]$definition.timeoutContract.endpointWorkerOuterTimeoutSeconds -gt ([int]$definition.timeoutContract.maximumSequentialChildren * [int]$definition.timeoutContract.opencvChildTimeoutSeconds)) 'R15D outer timeout does not cover all serialized children.'
Assert-True (-not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed -and -not [bool]$definition.sourceProcessingContract.holdClearanceAllowed -and -not [bool]$definition.sourceProcessingContract.providerActivationAllowed -and -not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.fullSourceImageReturnAllowed) 'R15D source-processing authority widened.'
...
foreach ($gatePin in @($invocation.requiredGates)) {
    ...
    Assert-PinnedFile $gatePath ([string]$gatePin.sha256) 'R15D required gate'
    $gate = Get-Content -Raw -LiteralPath $gatePath | ConvertFrom-Json
    $propertyName = [string]$gatePin.stateProperty
    Assert-True ($gate.PSObject.Properties.Name -contains $propertyName) "R15D gate state property is absent: $propertyName"
    Assert-True ([string]$gate.$propertyName -eq [string]$gatePin.requiredState) "R15D required gate state changed: $($gatePin.path)"
...
$routeGatePins = @($requiredGateEvidence.ToArray() | Where-Object { [string]$_.requiredState -eq 'PASS_R15D_COMPLETE_ROUTE_GATE' })
Assert-True ($routeGatePins.Count -eq 1) 'R15D build requires exactly one complete route gate pin.'
...
Assert-PinnedFile $identityPath ([string]$invocation.signing.identitySha256) 'R15D signing identity'
Assert-PinnedFile $publicCertificatePath ([string]$invocation.signing.publicCertificateSha256) 'R15D public certificate'
Assert-PinnedFile $packageTesterPath ([string]$invocation.signing.packageTesterSha256) 'R15D package tester'
$identity = Get-Content -Raw -LiteralPath $identityPath | ConvertFrom-Json
$thumbprint = ([string]$identity.thumbprint).Replace(' ', '').ToUpperInvariant()
Assert-True ($thumbprint -eq ([string]$invocation.signing.expectedThumbprint).Replace(' ', '').ToUpperInvariant()) 'R15D laptop signing thumbprint changed.'
...
foreach ($path in @($signedRoot,$signedPartial,$finalRoot,$finalPartial,$finalGatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R15D build create-new target exists: $path" }

$pathTool = Join-Path $project 'utilities\Confirm-ArgosPathBudget.ps1'
Assert-True (Test-Path -LiteralPath $pathTool -PathType Leaf) 'R15D path-budget tool is absent.'
...
Assert-True ($remoteFinalLeaves -contains 'D:\A2\w\ocv\R15D\jobs\JQ20V.json') 'R15D runtime-job route changed.'
Assert-True ($remoteFinalLeaves -contains 'C:\R15DR\R15D_RETURN.zip') 'R15D short decoded-bundle staging route changed.'
...
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'R15D exact package path budget failed.'
$maximumEffectiveLength = [int]((@($pathGate.candidates) | Measure-Object effectiveLength -Maximum).Maximum)
Assert-True ($maximumEffectiveLength -lt 200) 'R15D package effective path length reached 200.'
...
try {
    ...
    Assert-True ($matches.Count -eq 1 -and $matches[0].HasPrivateKey) 'R15D signing certificate/private key is unavailable or ambiguous.'
    $certificate = $matches[0]
}
finally { $store.Close(); $store.Dispose() }

$preflightResult = [ordered]@{
    schema='argos_r15d_build_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15D_BUILD_PREFLIGHT';requestId=[string]$invocation.requestId
...
try {
    ...
    foreach ($payload in $payloadFiles) {
        ...
        Assert-True ((Get-Sha256 $destination) -eq [string]$payload.sha256) "R15D copied payload changed: $($payload.path)"
    }
    ...
    Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R15D signed package verification failed.'
    ...
    Assert-True ($extractedFiles.Count -eq $expectedFiles.Count) 'R15D final ZIP file cardinality changed.'
    foreach ($expected in $expectedFiles.GetEnumerator()) {
        $leaf = Get-SafePackageChild $extractRoot ([string]$expected.Key)
        Assert-True (Test-Path -LiteralPath $leaf -PathType Leaf) "R15D final ZIP leaf is absent: $($expected.Key)"
        Assert-True ((Get-Sha256 $leaf) -eq [string]$expected.Value) "R15D final ZIP leaf changed: $($expected.Key)"
    }
    ...
    Assert-True ([string]$extractedTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R15D exact final ZIP signature verification failed.'

    [IO.Directory]::Move($signedPartial, $signedRoot)
    $signedPartialCreated = $false
    [IO.Directory]::Move($finalPartial, $finalRoot)
    $finalPartialCreated = $false
    Assert-True (Test-Path -LiteralPath $zipPath -PathType Leaf) 'R15D final ZIP disappeared after commit.'
    $gate = [ordered]@{
        schema='argos_r15d_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15D_FINAL_SIGNED_PACKAGE';classification='FROZEN_REVIEW_ONLY';requestId=[string]$invocation.requestId
        invocationManifestSha256=Get-Sha256 $invocationPath;requestZip=('work/OPENCV_SCRIBE_R14/final/' + $zipName);requestZipBytes=[int64](Get-Item -LiteralPath $zipPath).Length;requestZipSha256=Get-Sha256 $zipPath
