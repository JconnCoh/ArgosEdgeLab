...
Revision: `ARGOS_FIDUCIAL_MODEL_WORKFLOW_V2`
...
fiducial-model creation, automatic calibration, and site-bound validation. It
does not by
itself grant alignment, inspection, XML, production, or reject authority.

V1 remains preserved byte-for-byte as
`work/ARGOS_FIDUCIAL_MODEL_WORKFLOW_V1_LOCKED.md`,
`work/ARGOS_FIDUCIAL_MODEL_WORKFLOW_V1_LOCKED.json`, and
`work/templates/ARGOS_FIDUCIAL_MODEL_RUN_V1_LOCKED.template.json`. V2 adds the
mandatory designated-site binding and count-aware short-line semantics exposed
by the exact PFC004 seven-wafer audit; it does not weaken the V1 geometry,
corner, polarity, or no-tuning requirements.
...
- Before any validation outcome is inspected, derive and freeze the intended
  fiducial-site prediction and a deterministic eligibility radius from the
  qualified notch pose, exact map/designation relationship, crop transform,
  and calibration localization error. This binds topology to site identity;
  it is not merely a search hint.
...
Support and gap semantics must be internally consistent for short lines. If a
calibrated rule permits one unsupported sample, it must express that permission
as an explicit integer accepted-sample requirement and a maximum contiguous
unsupported run. A fractional support threshold that makes the declared gap
mathematically impossible on a three- or seven-sample line is invalid. Every
residual and response-width statistic must declare its population (`FOUND`,
`ACCEPTED`, or another exact set). A sample cannot be allowed as unsupported
and then silently reintroduced through a found-sample percentile that vetoes
the same allowance.
...
- designated-site binding mutation: a geometrically complete repeated instance
  outside the frozen site radius remains ineligible, while the complete model
  at the designated site retains its result;
- site-radius ambiguity: zero or multiple refined candidates inside the frozen
  radius produce a hold rather than selecting the highest correlation.
...
- Validation eligibility is site-bound. Require exactly one refined candidate
  inside the frozen radius of the notch/map-predicted operator-designated site.
  A complete model elsewhere in a wide crop is an ineligible repeated instance
  or lookalike and can never substitute for the designated site.
...
operator judgment raster, and that positive must be the eligible candidate at
the designated site. Use the full available lot when practical to expose
wafer-to-wafer variation; preserve physical identities and do not treat later
scans of the same wafer as independent wafers. Whole-crop counts of complete
geometry are diagnostic only; they are never the site-bound pass criterion.
...
- no whole-crop `exactly one complete geometry` result without binding that
  result to the frozen operator-designated site;
- no fractional-support rule that silently nullifies a declared short-line gap
  allowance;
- no ambiguous percentile population for residual or width gates;
