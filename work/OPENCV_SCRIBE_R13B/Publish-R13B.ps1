...
Assert-True ([string]$invocation.schema -eq 'argos_r13b_publication_invocation_v1') 'R13B publication invocation schema changed.'
...
$preflightResult = [ordered]@{
    ...
    pathState=[string]$pathGate.state;maximumEffectiveLength=$maximumEffectiveLength;mutationsPerformed=$false;maximumPublications=1;retryAuthorized=$false;matchingSignedTerminalResponseOnly=$true
