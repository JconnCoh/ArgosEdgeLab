...
function Read-ResponseCandidate([string]$Path, [string]$ExpectedRequestId, [Security.Cryptography.X509Certificates.X509Certificate2]$Certificate, [object]$Limits) {
    ...
    try {
        ...
        Assert-True ($entries.Count -le [int]$Limits.maximumResponseZipEntries) 'Matching response ZIP entry bound was exceeded.'
...
else { Assert-True ([string]$invocation.schema -eq 'argos_r13b_response_collection_r3_invocation_v1') 'R13B R3 response collection invocation schema changed.' }
