...
$expectedRequestId = 'REQ_20260902T204408092Z_R13B'
...
$expectedInvocationLeaf = 'Get-R13BCurrentShareObservationR2.invocation.json'
$expectedOutputLeaf = 'R13B_CURRENT_SHARE_OBSERVATION.json'
...
Assert-True (Test-Path -LiteralPath $InvocationManifest -PathType Leaf) 'R13B invocation manifest is absent.'
$invocationPath = [IO.Path]::GetFullPath((Resolve-Path -LiteralPath $InvocationManifest).Path)
Assert-True ($invocationPath.Equals($expectedInvocationPath, [StringComparison]::OrdinalIgnoreCase)) 'R13B requires its exact adjacent invocation manifest.'
$invocationText = Read-BoundedUtf8File -Path $invocationPath -MaximumBytes 65536 -Label 'R13B invocation manifest'
$invocation = $invocationText | ConvertFrom-Json

Assert-True ([string]$invocation.schema -ceq 'argos_r13b_current_share_observation_invocation_v2') 'R13B R2 invocation schema changed.'
Assert-True ([string]$invocation.responseScanScope -ceq 'POST_PROVEN_ZERO_UNRESOLVED_BOUNDARY_ONLY') 'R13B R2 response scan scope changed.'
Assert-True ([string]$invocation.responseCorrelationMode -ceq 'SIGNED_EXACT_REQUEST_ID') 'R13B R2 response correlation mode changed.'
Assert-True ([string]$invocation.actionClassification -ceq 'OBSERVE_REMOTE_READ_ONLY_LOCAL_EVIDENCE_CREATE_NEW') 'R13B action classification changed.'
Assert-True ([string]$invocation.requestId -ceq $expectedRequestId) 'R13B request ID changed.'
Assert-True ([string]$invocation.driveName -ceq 'U') 'R13B drive name changed.'
Assert-True ((Normalize-Root ([string]$invocation.expectedInspectionRevsUnc)).Equals((Normalize-Root $expectedUncRoot), [StringComparison]::OrdinalIgnoreCase)) 'R13B InspectionRevs UNC changed.'
Assert-True ([string]$invocation.portalRelativeRoot -ceq 'ProjectPortalRO') 'R13B portal relative root changed.'
Assert-True ([string]$invocation.requestsRelativeRoot -ceq 'ProjectPortalRO\requests') 'R13B requests relative root changed.'
Assert-True ([string]$invocation.processedRelativeRoot -ceq 'ProjectPortalRO\requests\processed') 'R13B processed relative root changed.'
Assert-True ([string]$invocation.responsesRelativeRoot -ceq 'ProjectPortalRO\responses') 'R13B responses relative root changed.'
Assert-True ([string]$invocation.acceptedSinceUtc -ceq $expectedAcceptedSinceUtc) 'R13B accepted-request floor changed.'
Assert-True ([string]$invocation.acceptedSinceBoundaryMode -ceq $expectedAcceptedBoundaryMode) 'R13B accepted-request boundary mode changed.'
$declaredTerminalStates = @($invocation.terminalResponseStates | ForEach-Object { [string]$_ })
Assert-True ($declaredTerminalStates.Count -eq $expectedTerminalResponseStates.Count) 'R13B terminal response-state cardinality changed.'
for ($terminalStateIndex = 0; $terminalStateIndex -lt $expectedTerminalResponseStates.Count; $terminalStateIndex++) {
    Assert-True ($declaredTerminalStates[$terminalStateIndex] -ceq $expectedTerminalResponseStates[$terminalStateIndex]) 'R13B explicit terminal response-state set changed.'
}
Assert-True ([string]$invocation.outputLeaf -ceq $expectedOutputLeaf) 'R13B output leaf changed.'
Assert-True ([int]$invocation.scanBounds.maximumTopLevelRequestFiles -eq 1000) 'R13B top-level request bound changed.'
Assert-True ([int]$invocation.scanBounds.maximumProcessedFiles -eq 2000) 'R13B processed bound changed.'
Assert-True ([int]$invocation.scanBounds.maximumResponseFiles -eq 2000) 'R13B response bound changed.'
Assert-True ([int]$invocation.scanBounds.maximumZipEntriesPerPackage -eq 256) 'R13B ZIP-entry bound changed.'
Assert-True ([int64]$invocation.scanBounds.maximumManifestBytes -eq 1048576) 'R13B manifest byte bound changed.'
Assert-True ([int64]$invocation.scanBounds.maximumSignatureBytes -eq 8192) 'R13B signature byte bound changed.'
Assert-True ([int]$invocation.scanBounds.maximumCorrelationRows -eq 512) 'R13B correlation-row bound changed.'
Assert-True ([int]$invocation.scanBounds.maximumRecordedErrorRows -eq 64) 'R13B error-row bound changed.'
Assert-True ([bool]$invocation.requirements.requireZeroPendingRequestFiles) 'R13B zero-pending requirement changed.'
Assert-True ([bool]$invocation.requirements.requireZeroUnresolvedEarlierAcceptedRequests) 'R13B zero-unresolved requirement changed.'
Assert-True ([bool]$invocation.requirements.requireStableQueueSnapshot) 'R13B stable-snapshot requirement changed.'
Assert-True ([bool]$invocation.requirements.requireTargetAbsentFromPendingProcessedAndResponses) 'R13B target-absence requirement changed.'
Assert-True ([bool]$invocation.requirements.requirePriorZeroUnresolvedBoundary) 'R13B prior zero-unresolved boundary requirement changed.'
Assert-True ([bool]$invocation.requirements.requireOnlyExplicitTerminalResponseStates) 'R13B explicit terminal-state requirement changed.'
Assert-True (-not [bool]$invocation.requirements.shareWritesAllowed -and -not [bool]$invocation.requirements.mappingChangesAllowed -and -not [bool]$invocation.requirements.automaticRetryAllowed) 'R13B read-only/no-retry boundary changed.'
Assert-True ([bool]$invocation.authority.reviewOnly -and -not [bool]$invocation.authority.publicationAuthorizedByObservation -and -not [bool]$invocation.authority.automaticIdentityAuthority -and -not [bool]$invocation.authority.trainingEligible -and -not [bool]$invocation.authority.xmlEligible -and -not [bool]$invocation.authority.productionEligible -and -not [bool]$invocation.authority.productionRoutingEnabled -and -not [bool]$invocation.authority.mayClearHolds) 'R13B observation authority widened.'
Assert-True ($null -eq $invocation.publicationParent) 'R13B observation must not declare a publication parent.'

$referenceRows = @($invocation.referencePatterns)
Assert-True ($referenceRows.Count -eq 2) 'R13B reference-pattern cardinality changed.'
foreach ($reference in $referenceRows) {
    Assert-True ([bool]$reference.referenceOnly -and -not [bool]$reference.publicationParent) 'R13B pattern was promoted beyond reference-only use.'
    $referencePath = Resolve-ProjectFile -RelativePath ([string]$reference.path) -Label 'R13B reference pattern'
    Assert-True ((Get-Sha256 $referencePath) -ceq ([string]$reference.sha256).ToUpperInvariant()) "R13B reference pattern changed: $referencePath"
}

$priorBoundaryPath = Resolve-ProjectFile -RelativePath ([string]$invocation.priorZeroUnresolvedBoundary.path) -Label 'R13B prior zero-unresolved boundary'
Assert-True ((Get-Sha256 $priorBoundaryPath) -ceq ([string]$invocation.priorZeroUnresolvedBoundary.sha256).ToUpperInvariant()) 'R13B prior zero-unresolved boundary hash changed.'
$priorBoundary = (Read-BoundedUtf8File -Path $priorBoundaryPath -MaximumBytes 1048576 -Label 'R13B prior zero-unresolved boundary') | ConvertFrom-Json
Assert-True ([string]$invocation.priorZeroUnresolvedBoundary.stateProperty -ceq 'state') 'R13B prior boundary state selector changed.'
Assert-True ([string]$invocation.priorZeroUnresolvedBoundary.requiredState -ceq $expectedBoundaryGateState) 'R13B prior boundary required state changed.'
Assert-True ([string]$priorBoundary.state -ceq $expectedBoundaryGateState) 'R13B prior zero-unresolved boundary state changed.'
Assert-True ([string]$priorBoundary.createdUtc -ceq $expectedAcceptedSinceUtc -and [string]$invocation.priorZeroUnresolvedBoundary.createdUtc -ceq $expectedAcceptedSinceUtc) 'R13B prior boundary timestamp changed.'
Assert-True ([int]$priorBoundary.queueObservation.unresolvedEarlierAcceptedRequestCount -eq 0 -and [int]$invocation.priorZeroUnresolvedBoundary.requiredUnresolvedEarlierAcceptedRequestCount -eq 0) 'R13B prior boundary did not prove zero unresolved accepted requests.'

$requestCertificatePath = Resolve-ProjectFile -RelativePath ([string]$invocation.requestSigner.certificateRelativePath) -Label 'R13B request signer certificate'
$responseCertificatePath = Resolve-ProjectFile -RelativePath ([string]$invocation.responseSigner.certificateRelativePath) -Label 'R13B response signer certificate'
Assert-True ((Get-Sha256 $requestCertificatePath) -ceq ([string]$invocation.requestSigner.certificateSha256).ToUpperInvariant()) 'R13B request signer certificate hash changed.'
Assert-True ((Get-Sha256 $responseCertificatePath) -ceq ([string]$invocation.responseSigner.certificateSha256).ToUpperInvariant()) 'R13B response signer certificate hash changed.'
...
try {
    ...
    Assert-True ($requestCertificate.Thumbprint.Replace(' ', '').ToUpperInvariant() -ceq $expectedRequestThumbprint) 'R13B request signer certificate thumbprint changed.'
    Assert-True ($responseCertificate.Thumbprint.Replace(' ', '').ToUpperInvariant() -ceq $expectedResponseThumbprint) 'R13B response signer certificate thumbprint changed.'
    Assert-True ([string]$invocation.responseSigner.sourceRole -ceq 'JBOD') 'R13B response source role changed.'
    ...
    Assert-True ($outputPath.StartsWith(($PSScriptRoot.TrimEnd('\') + '\'), [StringComparison]::OrdinalIgnoreCase)) 'R13B output escaped its namespace.'
    Assert-True (-not (Test-Path -LiteralPath $outputPath)) 'R13B create-new observation already exists.'
    ...
    Assert-True ($logicalRows.Count -eq 1) 'R13B persistent U: logical-disk cardinality changed.'
    ...
    Assert-True ($displayRoot.Equals((Normalize-Root $expectedUncRoot), [StringComparison]::OrdinalIgnoreCase)) 'R13B U: PowerShell DisplayRoot changed.'
    Assert-True ($providerRoot.Equals((Normalize-Root $expectedUncRoot), [StringComparison]::OrdinalIgnoreCase)) 'R13B U: Win32 ProviderName changed.'
    Assert-True ([int]$logical.DriveType -eq 4) 'R13B U: is not a persistent network logical disk.'
    ...
    Assert-True (Test-Path -LiteralPath $portalRoot -PathType Container) 'R13B portal root is unreadable.'
    Assert-True (Test-Path -LiteralPath $requestsRoot -PathType Container) 'R13B requests root is unreadable.'
    Assert-True (Test-Path -LiteralPath $processedRoot -PathType Container) 'R13B processed root is unreadable.'
    Assert-True (Test-Path -LiteralPath $responsesRoot -PathType Container) 'R13B responses root is unreadable.'

    $observationStartedUtc = [DateTime]::UtcNow.ToString('o')
    $topLevelBefore = @(Get-BoundedFiles -Path $requestsRoot -Maximum ([int]$invocation.scanBounds.maximumTopLevelRequestFiles) -Label 'R13B top-level request scan')
    $processedBefore = @(Get-BoundedFiles -Path $processedRoot -Maximum ([int]$invocation.scanBounds.maximumProcessedFiles) -Label 'R13B processed-request scan')
    $responsesBefore = @(Get-BoundedFiles -Path $responsesRoot -Maximum ([int]$invocation.scanBounds.maximumResponseFiles) -Label 'R13B response scan')
    ...
    foreach ($file in $processedReadyRows) {
        try {
            $signed = Read-SignedZipManifest -PackagePath $file.FullName -ManifestEntryName 'PORTAL_REQUEST_MANIFEST.json' -SignatureEntryName 'PORTAL_REQUEST_MANIFEST.sig' -Certificate $requestCertificate -MaximumEntries ([int]$invocation.scanBounds.maximumZipEntriesPerPackage) -MaximumManifestBytes ([int64]$invocation.scanBounds.maximumManifestBytes) -MaximumSignatureBytes ([int64]$invocation.scanBounds.maximumSignatureBytes) -Label ('R13B processed request ' + $file.Name)
    ...
    Assert-True ($acceptedAfterBoundaryRows.Count -le [int]$invocation.scanBounds.maximumCorrelationRows) 'R13B accepted-after-boundary correlations exceeded their bound.'
    ...
    $topLevelAfter = @(Get-BoundedFiles -Path $requestsRoot -Maximum ([int]$invocation.scanBounds.maximumTopLevelRequestFiles) -Label 'R13B final top-level request scan')
    $processedAfter = @(Get-BoundedFiles -Path $processedRoot -Maximum ([int]$invocation.scanBounds.maximumProcessedFiles) -Label 'R13B final processed-request scan')
    $responsesAfter = @(Get-BoundedFiles -Path $responsesRoot -Maximum ([int]$invocation.scanBounds.maximumResponseFiles) -Label 'R13B final response scan')
    ...
    Assert-True ($processedErrorCount -eq 0) "R13B processed-request scan had $processedErrorCount error(s)$processedErrorSummary"
    Assert-True ($responseErrorCount -eq 0) "R13B response-manifest scan had $responseErrorCount error(s)$responseErrorSummary"
    $pendingSummary = (@($pendingRows + $finalPendingRows | Select-Object -First 10 | ForEach-Object { $_.Name }) -join ',')
    Assert-True ($pendingRows.Count -eq 0 -and $finalPendingRows.Count -eq 0) "R13B share has a pending request or upload: $pendingSummary"
    Assert-True ($targetPendingRows.Count -eq 0 -and $targetProcessedByLeaf.Count -eq 0 -and $targetProcessedByManifest.Count -eq 0 -and $targetResponseByManifest.Count -eq 0) 'R13B exact request ID already exists in a portal namespace.'
    Assert-True ($acceptedDuplicateGroups.Count -eq 0) 'R13B processed requests after the proven boundary contain a duplicate signed request ID.'
    ...
    Assert-True ($unresolvedRows.Count -eq 0) "R13B has an earlier accepted request without one matching signed terminal response: $unresolvedSummary"
    Assert-True ($ambiguousRows.Count -eq 0) "R13B has an earlier accepted request with multiple matching signed terminal responses: $ambiguousSummary"
    Assert-True ($topLevelStable -and $processedStable -and $responsesStable) 'R13B queue changed during the bounded observation.'
    ...
    $gate = [ordered]@{
        schema = 'argos_r13b_current_share_observation_v1'
    ...
    if ($Preflight) {
        $gate.schema = 'argos_r13b_current_share_observation_preflight_v1'
        $gate.state = 'PASS_R13B_CURRENT_SHARE_OBSERVATION_PREFLIGHT'
    ...
    [ordered]@{
        schema = 'argos_r13b_current_share_observation_result_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R13B_CURRENT_SHARE_OBSERVATION_WRITTEN_CREATE_NEW'
        requestId = $expectedRequestId
