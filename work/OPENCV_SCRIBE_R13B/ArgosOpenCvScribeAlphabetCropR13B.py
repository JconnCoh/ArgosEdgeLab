#!/usr/bin/env python3
"""Review-only R13B source-to-glyph crop provider.

The provider consumes one hash-pinned BF/DF acquisition, runs the frozen R11
analysis in-process, evaluates every bounded paired-channel localization
region and both orientations with the frozen R12B topology diagnostic, and
returns a deterministic candidate grid plus audit cells.  Canonical truth is
used only to select the most plausible grid for reference harvesting.  It is
never an automatic identity, reference-admission, training, XML, production,
...
REVISION = "ARGOS_OPENCV_SCRIBE_ALPHABET_CROP_R13B_20260902"
...
def validate_job(job: dict[str, Any], r11: Any) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    require(job.get("schema") == JOB_SCHEMA, "R13B job schema mismatch")
    ...
    require(job.get("inputMode") == "DEVELOPMENT_AUTO_LOCALIZED_WHOLE_IMAGE", "R13B requires R11 development automatic localization")
    search = job.get("search")
    require(isinstance(search, dict), "search must be an object")
    require(search.get("expectedRegions") == [], "R13B refuses preselected expected regions")
    ...
    require(256 <= int(search.get("maximumWorkingDimension", 0)) <= 2048, "maximumWorkingDimension is outside the R13B bound")
    ...
    maximum_paired = int(blob.get("maximumPairedRegionsEvaluated", 0))
    require(0.0 < minimum_dot_size < maximum_dot_size <= 64.0, "Blob diameter bounds are invalid")
    require(1 <= maximum_paired <= 16, "maximumPairedRegionsEvaluated must be from 1 through 16")
    require(maximum_paired >= int(search["maximumCandidates"]), "Paired-region bound must cover every bounded R11 candidate")
    require(blob.get("directions") == ["FORWARD", "REVERSE_180"], "R13B direction contract mismatch")
    ...
    return r11_job, targets, {
        ...
        "maximumPairedRegionsEvaluated": maximum_paired,
    }
...
def paired_regions(r11_result: dict[str, Any], maximum: int) -> list[dict[str, Any]]:
    ...
    selected = [
        row for row in rows
        if isinstance(row, dict) and "PAIRED_BF_DF" in str(row.get("source", ""))
    ]
    require(len(selected) <= maximum, "R11 emitted more paired regions than the declared complete-evaluation bound")
    return sorted(
        selected,
        key=lambda row: (
...
def run_case(job: dict[str, Any], job_path: Path, job_sha256: str) -> tuple[dict[str, Any], dict[str, bytes]]:
    ...
    regions = paired_regions(r11_result, int(blob["maximumPairedRegionsEvaluated"]))
    ...
    if not regions:
        state = "HOLD_R13B_NO_PAIRED_LOCALIZATION"
        holds.append({"code": "SCRIBE_ALPHABET_LOCALIZATION_HOLD", "detail": "R11 produced no bounded paired BF/DF localization region."})
    elif selected is None:
        state = "HOLD_R13B_NO_EVALUABLE_GRID"
        holds.append({"code": "SCRIBE_ALPHABET_GRID_HOLD", "detail": "R12B produced no evaluable twelve-cell grid in any bounded paired region, channel, or direction."})
    else:
        ...
        if selected["nonTargetCanonicalMismatchPositions"]:
            state = "HOLD_R13B_GRID_NON_TARGET_TRUTH_MISMATCH"
            holds.append({
            ...
            state = "HOLD_R13B_REVIEW_ONLY_GRID_SELECTED"
    holds.append({
    ...
    result = {
        ...
        "blobGridEvaluation": {
            ...
            "pairedRegionCount": len(regions),
            "pairedRegions": regions,
...
def validate_output_path(output_root: Path) -> tuple[Path, Path]:
    ...
    require(
        ...
        "Case output exceeds the R13B path budget",
    )
...
def failure_result(job: dict[str, Any] | None, job_path: Path, error: Exception) -> dict[str, Any]:
    ...
    return {
        ...
        "revision": REVISION,
        "classification": "PENDING_GATE",
        "state": "HOLD_R13B_INPUT_OR_PROVIDER_GATE_FAILED",
