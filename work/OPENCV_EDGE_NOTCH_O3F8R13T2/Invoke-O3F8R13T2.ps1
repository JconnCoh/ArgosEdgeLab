...
function Start-Python([string]$Python,[string[]]$Arguments,[string]$Working,[int]$Probe,[string]$Mode){
    ...
    $start.UseShellExecute=$false;$start.CreateNoWindow=$true;$start.WindowStyle=[Diagnostics.ProcessWindowStyle]::Hidden
    $start.EnvironmentVariables['PYTHONDONTWRITEBYTECODE']='1';$start.EnvironmentVariables['PYTHONNOUSERSITE']='1';$start.EnvironmentVariables['PYTHONUTF8']='1'
    if($Mode){$start.EnvironmentVariables['ARGOS_O3F8R13T2_FIXTURE_MODE']=$Mode}
...
function Contract([object]$Value){
    Require ([string]$Value.schema-eq'argos_ocv03_o3f8r13t2_launch_contract_v1'-and[string]$Value.state-eq'FROZEN_FOR_BUILD') 'Contract identity changed'
    ...
    Require ([string]$Value.runtimeRoot-eq'D:/O3F8R13T2RT'-and[string]$Value.corpusRoot-eq'D:/O3F8R13T2C'-and[string]$Value.mirrorRoot-eq'D:/KLARFExport/_ArgosReview/F8R13T2S') 'Target roots changed'
    ...
    Require ([string]$Value.expectedLaunchState-eq'PASS_O3F8R13T2_TARGETED_WORKER_LAUNCHED'-and[string]$Value.expectedRehearsalState-eq'PASS_O3F8R13T2_REHEARSAL_WORKER_LAUNCHED') 'Launch states changed'
...
function Main{
    ...
    $contractPath=Join-Path $PSScriptRoot 'O3F8R13T2_LAUNCH_CONTRACT.json';Require (Test-Path -LiteralPath $contractPath -PathType Leaf) 'Contract missing'
    $contract=Get-Content -LiteralPath $contractPath -Raw|ConvertFrom-Json;Contract $contract;$payload=@(Payload $contract)
    if($PackageLeafPreflight){[ordered]@{schema='argos_ocv03_o3f8r13t2_package_leaf_preflight_v1';state='PASS_O3F8R13T2_PACKAGE_LEAVES';payloadPinCount=$payload.Count;expectedPairCount=11;imageBytesRead=$false;mutationsPerformed=$false;reviewOnly=$true}|ConvertTo-Json -Compress;return}
    if($Rehearsal){
        ...
        Require ([string]$inv.schema-eq'argos_ocv03_o3f8r13t2_rehearsal_invocation_v1'-and[string]$inv.fixtureMode-in@('NORMAL','IMMEDIATE_EXIT')) 'Rehearsal invocation changed'
        ...
        $runnerName='O3F8R13T2LaunchFixture.py'
    }else{
    ...
    if($Preflight){[ordered]@{schema='argos_ocv03_o3f8r13t2_target_preflight_v1';state='PASS_O3F8R13T2_TARGET_PREFLIGHT';expectedPairCount=11;hotspotCount=10;providerErrorCount=5;imageBytesRead=$false;processStarted=$false;mutationsPerformed=$false;reviewOnly=$true}|ConvertTo-Json -Compress;return}
    ...
    }else{$testJson=[pscustomobject]@{state='PASS_O3F8_R13_DF_CANDIDATE_LIMIT_T2'};$selfJson=[pscustomobject]@{state='PASS_O3F8_R13_TARGETED_SELF_TEST'};$preJson=[pscustomobject]@{state='PASS_O3F8_R13_TARGETED_PREFLIGHT'}}
    ...
    [ordered]@{schema=$(if($Rehearsal){'argos_ocv03_o3f8r13t2_rehearsal_launch_v1'}else{'argos_ocv03_o3f8r13t2_launch_v1'});state=$(if($Rehearsal){[string]$contract.expectedRehearsalState}else{[string]$contract.expectedLaunchState});fixtureMode=$(if($Rehearsal){$script:fixtureMode}else{$null});testState=[string]$testJson.state;selfTestState=[string]$selfJson.state;preflightState=[string]$preJson.state;pid=$worker.Pid;creationTimeUtc=$worker.CreationTimeUtc;runtimeRoot=$runtime;corpusRoot=$corpus;mirrorRoot=$mirror;bootstrapProgressState=[string]$bootstrap.state;bootstrapScheduledCount=[int]$bootstrap.scheduledCount;sourceImagesReadByEndpoint=$false;existingProcessesQueried=$false;existingProcessOrTaskActionPerformed=$false;ownedProcessStarted=$true;automaticRetryAuthorized=$false;holdsCleared=$false;mutationsPerformed=$true;reviewOnly=$true;trainingEligible=$false;xmlEligible=$false;productionEligible=$false;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 6 -Compress
}
try{Main}catch{if($Rehearsal){$message=[string]$_.Exception.Message;[ordered]@{schema='argos_ocv03_o3f8r13t2_rehearsal_launch_v1';state=$(if($message-match'Create-new root exists'){'HOLD_O3F8R13T2_REHEARSAL_CREATE_NEW_COLLISION'}elseif($message-match'exited immediately'){'HOLD_O3F8R13T2_REHEARSAL_WORKER_EXITED_IMMEDIATELY'}else{'HOLD_O3F8R13T2_REHEARSAL_FAILURE'});fixtureMode=$script:fixtureMode;error=$message;imageBytesRead=$false;reviewOnly=$true}|ConvertTo-Json -Compress;exit 2};throw}
