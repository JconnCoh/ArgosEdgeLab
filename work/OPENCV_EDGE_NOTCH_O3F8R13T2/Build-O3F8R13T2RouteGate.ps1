...
$outputPath=Join-Path $PSScriptRoot 'final_o3f8r13t2\O3F8R13T2_PREPUBLICATION_PATH_R4_GATE.json'
...
Need ([string]$sign.state-eq'PASS_O3F8R13T2_SIGNED_TARGETED_PACKAGE'-and[int]$sign.payloadFileCount-eq25) 'Target sign gate changed'
...
$manifest=Get-Content -Raw -LiteralPath $manifestPath|ConvertFrom-Json;Need (@($manifest.files).Count-eq25) 'Signed payload cardinality changed'
...
foreach($row in @($old.routePaths)){
    $path=([string]$row.path).Replace($oldId,$newId).Replace('O3F15L4E3','O3F8R13T2').Replace('o3f15l4e3','o3f8r13t2')
