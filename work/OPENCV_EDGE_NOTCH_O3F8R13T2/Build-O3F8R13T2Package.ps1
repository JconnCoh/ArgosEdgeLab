...
$contractPath=Join-Path $PSScriptRoot 'O3F8R13T2_LAUNCH_CONTRACT.json';$contractHash='8CD7A7385DE53F2A394160ABD61B5E0689987C0667FE4B878738F3369F4BBCC6'
$definitionSource=Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION.json';$root='C:\O3F8R13T2PKR2';$payload=Join-Path $root 'payload';$definitionTarget=Join-Path $root 'DEFINITION.json';$gatePath=Join-Path $PSScriptRoot 'O3F8R13T2_BUILD_GATE_R2.json'
...
Require ([string]$contract.schema-eq'argos_ocv03_o3f8r13t2_launch_contract_v1'-and[string]$contract.state-eq'FROZEN_FOR_BUILD'-and[int]$contract.expectedPairCount-eq11) 'Contract identity changed'
Require ([string]$definition.schema-eq'argos_ocv03_o3f8r13t2_maintenance_definition_v1'-and[string]$definition.state-eq'FROZEN_FOR_SIGNING'-and[string]$definition.entryPoint-eq'payload/Invoke-O3F8R13T2.ps1') 'Definition identity changed'
...
$rows.Add([pscustomobject]@{path='O3F8R13T2_LAUNCH_CONTRACT.json';source=$contractPath;bytes=[int64](Get-Item $contractPath).Length;sha256=$contractHash;copyToRuntime=$false});$sources=$rows.ToArray()
...
foreach($required in @('Invoke-O3F8R13T2.ps1','Run-O3F8R13Targeted.py','FullPerimeterWaferTopologyOpenCvR13.py','FullPerimeterWaferTopologyOpenCvR11.py','Run-O3F15L4FrontReconcile.py','Test-O3F8R13DfCandidateLimitT2.py','OCV03_NotchReviewOpenCvV1.py')){Require (@($sources|Where-Object{[string]$_.path-eq$required}).Count-eq1) "Required payload absent: $required"}
