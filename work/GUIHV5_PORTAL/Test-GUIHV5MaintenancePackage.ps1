...
param(
    ...
    [string]$PackageRoot = 'C:\G5P5\REQ_GUIHV5_TEST5.ready',
    [string]$SuccessRoot = 'C:\G5S5',
    [string]$FailureRoot = 'C:\G5F5',
    [string]$RefusalRoot = 'C:\G5R5',
...
function New-FixtureTree([string]$Root, [bool]$InstallTargets, [bool]$UnapprovedViewer) {
    ...
    foreach ($row in @($ledger.rows | Where-Object { [string]$_.state -eq 'COMPLETED' })) {
        ...
        $result = [ordered]@{
            state = 'PASS_GUIHV5_FIXTURE_REVIEW_ONLY'
            reviewOnly = $true
            xmlWritten = $false
...
foreach ($required in @($signedPullZip, (Join-Path $payloadSource 'Apply-GUIHV5DirectGuiPatch.ps1'), (Join-Path $payloadSource 'p\d'), (Join-Path $payloadSource 'p\s'), (Join-Path $payloadSource 'p\i'))) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "Required GUIHV5 rehearsal input is missing: $required" }
}
if (-not (Test-Path -LiteralPath $definitionSource -PathType Leaf)) {
    foreach ($signedLeaf in @($signedManifestSource, $signedSignatureSource)) {
        if (-not (Test-Path -LiteralPath $signedLeaf -PathType Leaf)) { throw "Signed GUIHV5 package input is missing: $signedLeaf" }
    }
}
if ((Get-Sha256 $signedPullZip) -ne $signedPullZipSha256) { throw 'Signed GUIPR2 pull ZIP changed.' }
foreach ($path in @($resolvedPackageRoot, $roots.success, $roots.failure, $roots.refusal)) {
    if (Test-Path -LiteralPath $path) { throw "Fresh GUIHV5 rehearsal path already exists: $path" }
}

$preflightResult = [ordered]@{
    schema = 'argos_guihv5_maintenance_package_rehearsal_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_GUIHV5_MAINTENANCE_PACKAGE_REHEARSAL_PREFLIGHT'
...
$entryPoint = Join-Path $resolvedPackageRoot 'payload\Apply-GUIHV5DirectGuiPatch.ps1'
...
if ($entryPreflightResult.exitCode -ne 0 -or $entryPreflightResult.stdout -notmatch 'PASS_GUIHV5_SCRIBE_HOLD_PROJECTION_MAINTENANCE_PATCH_PREFLIGHT') { throw "GUIHV5 exact preflight failed: $($entryPreflightResult.stderr)" }
$success = Invoke-Child @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $entryPoint, '-Rehearsal', '-InvocationManifest', $fixturePaths.success)
if ($success.exitCode -ne 0 -or $success.stdout -notmatch 'PASS_GUIHV5_SCRIBE_HOLD_PROJECTION_PRODUCED_VALIDATED_AND_TRAY_ACTIVATED') { throw "GUIHV5 success rehearsal failed: $($success.stderr)" }
...
if ($failure.exitCode -eq 0 -or ($failure.stdout + $failure.stderr) -notmatch 'INJECTED_GUIHV5_POST_PRODUCER_VALIDATION_FAILURE') { throw 'GUIHV5 injected failure control did not fail exactly.' }
...
if ($refusal.exitCode -eq 0 -or ($refusal.stdout + $refusal.stderr) -notmatch 'destination predecessor is unapproved') { throw 'GUIHV5 unapproved predecessor was not refused.' }
if ((Get-Sha256 (Join-Path $roots.refusal 'dashboard_manifest.json')) -ne $refusalDashboardBefore) { throw 'GUIHV5 refusal mutated the dashboard.' }

$result = [ordered]@{
    schema = 'argos_guihv5_maintenance_package_rehearsal_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_GUIHV5_MAINTENANCE_PACKAGE_REHEARSAL'
    windowsPowerShell51Preflight = 'PASS_GUIHV5_SCRIBE_HOLD_PROJECTION_MAINTENANCE_PATCH_PREFLIGHT'
    successState = 'PASS_GUIHV5_SCRIBE_HOLD_PROJECTION_PRODUCED_VALIDATED_AND_TRAY_ACTIVATED'
    injectedFailure = 'INJECTED_GUIHV5_POST_PRODUCER_VALIDATION_FAILURE'
    installedSourceRollbackVerified = $true
    producerOutputRollbackVerified = $true
    unapprovedPredecessorRefusedBeforeMutation = $true
    exactRegression = [ordered]@{ acquisitionRows = 380; physicalRows = 190; stateSplit = @(126, 55, 5, 4); readyRows = 5; missingJoins = 0; unrelatedRows = 1342 }
