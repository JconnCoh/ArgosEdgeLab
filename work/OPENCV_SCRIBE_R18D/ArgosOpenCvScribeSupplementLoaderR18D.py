#!/usr/bin/env python3
"""Hash-locked loader for the operator-confirmed R18D W/Z supplement."""
...
EXPECTED_COUNTS = {"J": 2, "K": 1, "Q": 2, "W": 1, "X": 1, "Z": 1}
...
def load_supplemental_prototypes(r11: Any, manifest_path: Path, expected_sha256: str) -> tuple[list[Any], dict[str, Any]]:
    ...
    if manifest.get("schema") != SCHEMA or manifest.get("disposition") != "DIAGNOSTIC_ONLY":
        raise ValueError("R18D supplemental manifest contract mismatch.")
    ...
    if manifest.get("operatorConfirmedLabels") != "WZ" or manifest.get("remainingMissingLabelsAfterSupplement") != "IOVY":
        raise ValueError("R18D W/Z confirmation or remaining-gap contract changed.")

    prototypes = []
    counts: dict[str, int] = {}
    seen: set[tuple[str, str, int]] = set()
    for row in manifest.get("references", []):
        label, case_id, position = str(row.get("label", "")), str(row.get("caseId", "")), int(row.get("position", 0))
    ...
    declared = {str(k): int(v) for k, v in dict(manifest.get("labelCounts", {})).items()}
    if counts != EXPECTED_COUNTS or declared != EXPECTED_COUNTS:
        raise ValueError(f"R18D supplemental label counts changed: actual={counts}, declared={declared}")
    return prototypes, {"manifestSha256": actual, "referenceCount": len(prototypes), "prototypeLabels": "JKQWXZ", "labelCounts": counts, "operatorConfirmedLabels": "WZ", "singleExampleProvisionalLabels": "KWXZ"}
...
def combine_reference_prototypes(r11: Any, base_prototypes: list[Any], base_evidence: dict[str, Any], manifest_path: Path, expected_sha256: str) -> tuple[list[Any], dict[str, Any]]:
    ...
    if missing != "IOVY":
        raise ValueError(f"R18D combined missing-label contract changed: {missing}")
    return combined, {"base": dict(base_evidence), "supplement": evidence, "combinedReferenceCount": len(combined), "combinedPrototypeLabels": labels, "missingBodyReferenceLabels": missing, "referenceCoverageComplete": False, "providerActivationAuthorized": False, "identityAdmissionAuthorized": False}
