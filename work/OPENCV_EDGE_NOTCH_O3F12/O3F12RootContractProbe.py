...
def load_runner(path: Path) -> ModuleType:
    spec = importlib.util.spec_from_file_location("argos_o3f12_exact_o3f9_runner", path)
...
def exercise_owned_alias(module: ModuleType, root: Path) -> dict[str, object]:
    ...
    try:
        with module.owned_case_alias(plan, injected):
            raise RuntimeError("INJECTED_O3F12_ALIAS_FIXTURE_EXCEPTION")
    except RuntimeError as exc:
        need(str(exc) == "INJECTED_O3F12_ALIAS_FIXTURE_EXCEPTION", "Injected alias exception changed")
...
def main() -> int:
    ...
    need(args.gate_root.replace("\\", "/") == "D:/O3F9G12", "O3F12 exact GATE contract root changed")
    need(args.dev_root.replace("\\", "/") == "D:/O3F9D12", "O3F12 exact DEV6 contract root changed")
    need(args.gate_root != args.dev_root, "O3F12 exact contract roots collapsed")
    ...
    rejected = False
    try:
        validate(module, "D:/O3F12_REJECT", True)
    except RuntimeError as exc:
        rejected = "D:\\O3F9*" in str(exc)
    need(rejected, "Exact runner did not reject the incompatible O3F12 root prefix")
    ...
    print(
        json.dumps(
            {
                "schema": "argos_ocv03_o3f12_exact_runner_root_contract_v1",
                "state": "PASS_O3F12_EXACT_REAL_RUNNER_ROOT_CONTRACT",
                ...
                "incompatibleO3F12PrefixRejected": True,
