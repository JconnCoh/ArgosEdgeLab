...
function Write-O3F12Json([string]$Path,[object]$Value){Assert-O3F12 (-not(Test-Path -LiteralPath $Path)) "O3F12 share observation exists: $Path";[IO.File]::WriteAllText($Path,(($Value|ConvertTo-Json -Depth 8)+[Environment]::NewLine),(New-Object Text.UTF8Encoding($false)))}
...
Assert-O3F12 ([string]$drive.DisplayRoot-eq$expected-and[string]$disk.ProviderName-eq$expected-and[int]$disk.DriveType-eq4) 'O3F12 persistent U: mapping changed.'
$requestRoot='U:\ProjectPortalRO\requests';$responseRoot='U:\ProjectPortalRO\responses'
Assert-O3F12 (Test-Path -LiteralPath $requestRoot -PathType Container) 'O3F12 request share is unavailable.'
Assert-O3F12 (Test-Path -LiteralPath $responseRoot -PathType Container) 'O3F12 response share is unavailable.'
$pending=@(Get-ChildItem -LiteralPath $requestRoot -File -ErrorAction Stop)
Assert-O3F12 ($pending.Count-eq0) 'O3F12 request share is not zero-pending.'
...
Assert-O3F12 ((Get-O3F12Hash $o3f9Failure)-eq$o3f9FailureHash-and(Get-O3F12Hash $o3f10Failure)-eq$o3f10FailureHash-and(Get-O3F12Hash $o3f11Failure)-eq$o3f11FailureHash-and(Get-O3F12Hash $launchGate)-eq$launchHash-and(Get-O3F12Hash $checkpoint)-eq$checkpointHash) 'O3F12 prior accepted-request closure evidence changed.'
$failureValue=Get-Content -LiteralPath $o3f9Failure -Raw|ConvertFrom-Json
Assert-O3F12 ([string]$failureValue.requestId-eq'REQ_O3F9_20260902A'-and[string]$failureValue.responseState-eq'FAILED'-and[bool]$failureValue.signatureVerified-and-not[bool]$failureValue.requestRetryAuthorized) 'O3F12 O3F9 signed terminal closure changed.'
$o3f10FailureValue=Get-Content -LiteralPath $o3f10Failure -Raw|ConvertFrom-Json
Assert-O3F12 ([string]$o3f10FailureValue.requestId-eq'REQ_O3F10_20260902A'-and[string]$o3f10FailureValue.responseState-eq'FAILED'-and[bool]$o3f10FailureValue.signatureVerified-and-not[bool]$o3f10FailureValue.requestRetryAuthorized-and[string]$o3f10FailureValue.classification-eq'SIGNED_ARTIFACT_OUTPUT_ROOT_PREFIX_CONTRACT_FAILURE_NOT_LIVE_STATE_PREMISE_FAILURE') 'O3F12 O3F10 signed terminal closure changed.'
$o3f11FailureValue=Get-Content -LiteralPath $o3f11Failure -Raw|ConvertFrom-Json
Assert-O3F12 ([string]$o3f11FailureValue.requestId-eq'REQ_O3F11_20260902A'-and[string]$o3f11FailureValue.responseState-eq'FAILED'-and[bool]$o3f11FailureValue.signatureVerified-and-not[bool]$o3f11FailureValue.requestRetryAuthorized-and[string]$o3f11FailureValue.classification-eq'SIGNED_ARTIFACT_SOURCE_PATH_ALIAS_CONTRACT_FAILURE_NOT_LIVE_STATE_PREMISE_FAILURE') 'O3F12 O3F11 signed terminal closure changed.'
$result=[ordered]@{schema='argos_ocv03_o3f12_current_share_observation_v1';state='PASS_O3F12_CURRENT_SHARE_ZERO_PENDING_REQUESTS';displayRoot=$expected;logicalDiskProvider=[string]$disk.ProviderName;driveType=[int]$disk.DriveType;pendingRequestCount=0;unresolvedAcceptedRequestCount=0;acceptedRequestClosureEvidence=@([ordered]@{path='work/OPENCV_EDGE_NOTCH_O3F9/O3F9_SIGNED_TERMINAL_FAILURE_GATE.json';sha256=$o3f9FailureHash;requestId='REQ_O3F9_20260902A';terminalState='FAILED';signatureVerified=$true},[ordered]@{path='work/OPENCV_EDGE_NOTCH_O3F10/O3F10_SIGNED_TERMINAL_FAILURE_GATE.json';sha256=$o3f10FailureHash;requestId='REQ_O3F10_20260902A';terminalState='FAILED';signatureVerified=$true},[ordered]@{path='work/OPENCV_EDGE_NOTCH_O3F11/O3F11_SIGNED_TERMINAL_FAILURE_GATE.json';sha256=$o3f11FailureHash;requestId='REQ_O3F11_20260902A';terminalState='FAILED';signatureVerified=$true},[ordered]@{path='work/O3B21/R32C953L1_SIGNED_TERMINAL_LAUNCH_GATE.json';sha256=$launchHash},[ordered]@{path='work/FRONTSIDE_INSPECTION_REVIEW_ONLY/OCV03_O3F6_R8_FULL_978_COMPLETE_STITCH_HOLDER_REVIEW_PENDING_CHECKPOINT_20260902.md';sha256=$checkpointHash});mutationsPerformed=$false;mappingChanged=$false;reviewOnly=$true;productionRoutingEnabled=$false}
if($Preflight){$result.state='PASS_O3F12_CURRENT_SHARE_OBSERVATION_PREFLIGHT';$result|ConvertTo-Json -Depth 8;return}
Write-O3F12Json (Join-Path $PSScriptRoot 'O3F12_CURRENT_SHARE_OBSERVATION.json') $result
