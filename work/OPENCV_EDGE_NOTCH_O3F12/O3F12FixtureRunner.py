#!/usr/bin/env python3
"""No-image fixture for the exact O3F12 endpoint orchestration rehearsal."""
...
def fail_if(stage: str) -> None:
    if os.environ.get("ARGOS_O3F12_FIXTURE_FAIL_STAGE", "").upper() == stage:
        raise RuntimeError(f"INJECTED_O3F12_{stage}_FAILURE")


def timeout_with_owned_alias() -> None:
    if os.environ.get("ARGOS_O3F12_FIXTURE_FAIL_STAGE", "").upper() != "DEV6_TIMEOUT_ALIAS":
        return
    subst_text = os.environ.get("ARGOS_O3F12_FIXTURE_SUBST_PATH", "")
    anchor_text = os.environ.get("ARGOS_O3F12_FIXTURE_ALIAS_ANCHOR", "")
    ...
    if not subst.is_absolute() or not subst.is_file() or not anchor.is_absolute() or not anchor.is_dir():
        raise RuntimeError("O3F12 timeout-alias fixture environment is invalid")
    ...
    def mappings() -> dict[str, str]:
        ...
        if child.returncode != 0 or child.stderr:
            raise RuntimeError("O3F12 timeout-alias subst query failed")
        result: dict[str, str] = {}
    ...
    if "Q:" in mappings():
        raise RuntimeError("O3F12 timeout-alias Q: is already mapped")
    ...
    if create.returncode != 0 or create.stderr:
        raise RuntimeError("O3F12 timeout-alias creation failed")
    ...
    if target is None or os.path.normcase(os.path.normpath(target)) != os.path.normcase(os.path.normpath(str(anchor))):
        raise RuntimeError("O3F12 timeout-alias target verification failed")
    time.sleep(3600)  # Endpoint timeout/backstop must own cleanup for this injected case.
    raise RuntimeError("O3F12 timeout-alias fixture unexpectedly resumed")


def gate(root: Path) -> dict[str, object]:
    fail_if("GATE")
    summary = {
        "schema": "argos_ocv03_o3f12_gate_result_v1",
...
def dev6(root: Path, prerequisite: Path, expected_sha256: str) -> dict[str, object]:
    ...
    for index in range(1, 7):
        ...
        manifest = {"schema": "argos_ocv03_full_perimeter_topology_manifest_v2", "revision": f"O3F12_FIXTURE_{index:04d}", "results": [observed]}
    ...
    summary = {
        "schema": "argos_ocv03_o3f12_staged_result_v1",
        "state": "COMPLETE_O3F12_DEV6",
        "stage": "DEV6",
        "selectedCount": 6,
        "executedCount": 6,
