...
$endpointSha256 = '8B49AA580B1F81F95F97B51296978A2AFC2B1E8A3533687B23DE7DDC56F7EB9A'
...
function Copy-Pinned([string]$Source, [string]$Destination, [string]$Sha256) {
    Assert-True ((Get-Sha256 $Source) -eq $Sha256) "R15E endpoint-test source pin changed: $Source"
    [IO.File]::Copy($Source, $Destination, $false)
    Assert-True ((Get-Sha256 $Destination) -eq $Sha256) "R15E endpoint-test copy changed: $Destination"
}
...
function New-Scenario(
    ...
    Copy-Pinned $Endpoint (Join-Path $payloadRoot 'Invoke-R15E.ps1') $endpointSha256
    ...
    Write-TextNew (Join-Path $payloadRoot 'R15E.py') $StubSource
    $providerSha256 = Get-Sha256 (Join-Path $payloadRoot 'R15E.py')
    ...
    Assert-True ($payloadLeaves.Count -eq 7) "R15E $Name fixture does not contain exactly seven payload leaves."
    foreach ($expectedLeaf in @('Invoke-R15E.ps1','R15E.py','R11.py','R12A.py','R12B.py','CFG.json','LIVE.json')) {
        Assert-True (@($payloadLeaves | Where-Object { $_.Name -ceq $expectedLeaf }).Count -eq 1) "R15E $Name fixture leaf changed: $expectedLeaf"
    }
    return [pscustomobject]@{
        name=$Name
        root=$ScenarioRoot
        endpoint=(Join-Path $payloadRoot 'Invoke-R15E.ps1')
        invocation=$livePath
...
function Invoke-EndpointJson([object]$Scenario, [switch]$OnlyPreflight) {
    ...
    Assert-True (-not [string]::IsNullOrWhiteSpace($text)) "R15E $($Scenario.name) endpoint emitted no JSON."
    return $text | ConvertFrom-Json
}

Assert-True ([bool]$Preflight -xor [bool]$Gate) 'Specify exactly one of -Preflight or -Gate.'
$projectRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'))
$endpointPath = Join-Path $PSScriptRoot 'Invoke-R15E.ps1'
$configurationPath = Join-Path $PSScriptRoot 'R15E_CONFIGURATION.json'
$baseInvocationPath = Join-Path $PSScriptRoot 'R15E_LIVE_INVOCATION.json'
...
$successEvidencePath = Join-Path $PSScriptRoot 'R15E_ENDPOINT_SUCCESS_REHEARSAL_INVOCATION.json'
$failureEvidencePath = Join-Path $PSScriptRoot 'R15E_ENDPOINT_FAILURE_REHEARSAL_INVOCATION.json'
$gatePath = Join-Path $PSScriptRoot 'R15E_ENDPOINT_REHEARSAL_GATE.json'

foreach ($path in @($endpointPath,$configurationPath,$baseInvocationPath,$installationPath,$referenceBundlePath,$pythonPath)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "R15E endpoint-test dependency absent: $path"
}
Assert-True ((Get-Sha256 $endpointPath) -eq $endpointSha256) 'R15E endpoint changed after the endpoint-test self pin.'
Assert-True ((Get-Sha256 $configurationPath) -eq $configurationSha256) 'R15E configuration changed before endpoint test.'
Assert-True ((Get-Sha256 $installationPath) -eq $installationSha256) 'R15E rehearsal installation fixture changed.'
Assert-True ((Get-Sha256 $referenceBundlePath) -eq $referenceBundleSha256) 'R15E rehearsal reference ZIP changed.'
foreach ($pin in $dependencyPins) { Assert-True ((Get-Sha256 (Join-Path $projectRoot ([string]$pin.source))) -eq [string]$pin.sha256) "R15E dependency changed: $($pin.name)" }

if ($Preflight) {
    [ordered]@{
        schema='argos_opencv_scribe_r15e_endpoint_test_preflight_v1';revision=$revision;checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15E_ENDPOINT_TEST_PREFLIGHT'
...
foreach ($path in @($successEvidencePath,$failureEvidencePath,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R15E endpoint-test create-new evidence exists: $path" }
$substPath = Join-Path $env:SystemRoot 'System32\subst.exe'
$substBefore = & $substPath 2>&1 | Out-String
Assert-True ($LASTEXITCODE -eq 0 -and $substBefore -notmatch '(?im)^\s*X:\\:\s*=>') 'R15E endpoint test requires X: to be unused.'
$testRoot = 'C:\R15EG_' + $PID + '_' + [DateTime]::UtcNow.ToString('HHmmss')
Assert-True ($testRoot -match '^C:\\R15EG_[0-9]+_[0-9]{6}$' -and -not (Test-Path -LiteralPath $testRoot)) 'R15E endpoint-test root is not fresh and bounded.'
...
try {
    ...
    Assert-True ([string]$successPreflight.state -eq 'PASS_R15E_STRICTLY_NONMUTATING_PREFLIGHT' -and [int]$successPreflight.caseCount -eq 4 -and -not [bool]$successPreflight.sourceImageBytesRead -and -not [bool]$successPreflight.filesystemMutationPerformed) 'R15E success rehearsal preflight contract failed.'
    $successResponse = Invoke-EndpointJson $success
    Assert-True ([string]$successResponse.state -eq 'PASS_R15E_SIGNED_RETURN_READY' -and [int]$successResponse.caseCount -eq 4 -and [int]$successResponse.caseLaunchFailureCount -eq 0 -and [int]$successResponse.providerCompletedCount -eq 4) 'R15E success endpoint envelope failed.'
    ...
    Assert-True ([int]$successBatch.caseCount -eq 4 -and [int]$successBatch.providerCompletedCount -eq 4 -and [int]$successBatch.pixelsDecodedCaseCount -eq 4 -and [int]$successBatch.caseLaunchFailureCount -eq 0) 'R15E success batch gate failed.'
    foreach ($case in @($successBatch.cases)) {
        Assert-True ([bool]$case.providerCompleted -and [bool]$case.pixelsDecodedByOpenCv -and -not [bool]$case.attemptQuarantined) "R15E success case state failed: $($case.caseId)"
        $result = Get-Content -Raw -LiteralPath (Join-Path $success.output (([string]$case.caseId) + '\CASE_RESULT.json')) | ConvertFrom-Json
        Assert-True ([string]$result.state -eq 'HOLD_R15E_DIAGNOSTIC_EVIDENCE_RETURNED' -and @($result.artifacts).Count -eq 6) "R15E success result failed: $($case.caseId)"
    }
    ...
    Assert-True ([string]$failurePreflight.state -eq 'PASS_R15E_STRICTLY_NONMUTATING_PREFLIGHT' -and [int]$failurePreflight.caseCount -eq 4 -and -not [bool]$failurePreflight.sourceImageBytesRead -and -not [bool]$failurePreflight.filesystemMutationPerformed) 'R15E failure rehearsal preflight contract failed.'
    $failureResponse = Invoke-EndpointJson $failure
    Assert-True ([string]$failureResponse.state -eq 'PASS_R15E_SIGNED_RETURN_READY' -and [int]$failureResponse.caseCount -eq 4 -and [int]$failureResponse.caseLaunchFailureCount -eq 4 -and [int]$failureResponse.providerCompletedCount -eq 0) 'R15E injected-failure endpoint envelope failed.'
    ...
    Assert-True ([int]$failureBatch.caseCount -eq 4 -and [int]$failureBatch.providerCompletedCount -eq 0 -and [int]$failureBatch.pixelsDecodedCaseCount -eq 0 -and [int]$failureBatch.caseLaunchFailureCount -eq 4) 'R15E injected-failure batch gate failed.'
    foreach ($case in @($failureBatch.cases)) {
        $caseId = [string]$case.caseId
        Assert-True (-not [bool]$case.providerCompleted -and -not [bool]$case.pixelsDecodedByOpenCv -and [bool]$case.attemptQuarantined) "R15E injected-failure quarantine state failed: $caseId"
        $caseFiles = @(Get-ChildItem -LiteralPath (Join-Path $failure.output $caseId) -File)
        Assert-True ($caseFiles.Count -eq 1 -and $caseFiles[0].Name -eq 'CASE_RESULT.json') "R15E injected-failure compact output failed: $caseId"
        $result = Get-Content -Raw -LiteralPath $caseFiles[0].FullName | ConvertFrom-Json
        Assert-True ([string]$result.state -eq 'HOLD_R15E_CASE_LAUNCH_FAILURE' -and @($result.artifacts).Count -eq 0) "R15E injected-failure result failed: $caseId"
        Assert-True (Test-Path -LiteralPath (Join-Path $failure.work ('failed\' + $caseId + '\committed\CASE_RESULT.json')) -PathType Leaf) "R15E injected-failure recoverable quarantine absent: $caseId"
    }

    foreach ($source in $sourceRows.ToArray()) {
        Assert-True ([int64](Get-Item -LiteralPath $source.path).Length -eq [int64]$source.bytes -and (Get-Sha256 $source.path) -eq [string]$source.sha256) "R15E source fixture changed: $($source.caseId)/$($source.channel)"
    }
    $substAfter = & $substPath 2>&1 | Out-String
    Assert-True ($LASTEXITCODE -eq 0 -and $substAfter -notmatch '(?im)^\s*X:\\:\s*=>') 'R15E endpoint left the X: alias mapped.'
    ...
    $gateResult = [ordered]@{
        schema='argos_opencv_scribe_r15e_endpoint_rehearsal_gate_v1';revision=$revision;classification='PENDING_GATE';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15E_ENDPOINT_REHEARSAL'
        endpoint=[ordered]@{path='work/OPENCV_SCRIBE_R15E/Invoke-R15E.ps1';sha256=$endpointSha256;windowsPowerShellVersion=$PSVersionTable.PSVersion.ToString();sevenPayloadPackageShapePassed=$true;liveRuntimeAndSourcePinsRemainRequired=$true}
        ...
        success=[ordered]@{invocationEvidence='work/OPENCV_SCRIBE_R15E/R15E_ENDPOINT_SUCCESS_REHEARSAL_INVOCATION.json';invocationSha256=Get-Sha256 $successEvidencePath;providerStubSha256=[string]$success.providerSha256;bundleSha256=[string]$successResponse.bundleSha256;caseCount=4;providerCompletedCount=4;caseLaunchFailureCount=0;artifactAndLineageValidationPassed=$true}
        injectedFailure=[ordered]@{invocationEvidence='work/OPENCV_SCRIBE_R15E/R15E_ENDPOINT_FAILURE_REHEARSAL_INVOCATION.json';invocationSha256=Get-Sha256 $failureEvidencePath;providerStubSha256=[string]$failure.providerSha256;bundleSha256=[string]$failureResponse.bundleSha256;caseCount=4;providerCompletedCount=0;caseLaunchFailureCount=4;compactFailureConstructionPassed=$true;recoverableQuarantinePassed=$true;laterCasesProcessed=$true}
    ...
    if (Test-Path -LiteralPath $testRoot) {
        Assert-True ($testRoot -match '^C:\\R15EG_[0-9]+_[0-9]{6}$') 'R15E endpoint-test cleanup target escaped its exact root.'
