...
Assert-True (Test-Path -LiteralPath $InvocationManifest -PathType Leaf) 'R15E publication invocation manifest is absent.'
...
Assert-True ([string]$invocation.schema -eq 'argos_r15e_publication_r2_invocation_v1') 'R15E R2 publication invocation schema changed.'
Assert-True ([string]$invocation.requestId -eq 'REQ_20260903T124500000Z_R15E') 'R15E publication request identity changed.'
Assert-True ([string]$invocation.requiredBranch -eq 'codex/opencv-scribe-deciphering') 'R15E publication branch changed.'
Assert-True ([bool]$invocation.authority.publicationAuthorized -and [int]$invocation.authority.maximumPublications -eq 1 -and -not [bool]$invocation.authority.retryAuthorized -and [bool]$invocation.authority.matchingSignedTerminalResponseOnly) 'R15E publication authority/count boundary changed.'
Assert-True ([bool]$invocation.authority.reviewOnly -and -not [bool]$invocation.authority.automaticIdentityAuthority -and -not [bool]$invocation.authority.trainingEligible -and -not [bool]$invocation.authority.xmlEligible -and -not [bool]$invocation.authority.productionEligible -and -not [bool]$invocation.authority.productionRoutingEnabled -and -not [bool]$invocation.authority.providerActivationAllowed) 'R15E publication authority widened.'
Assert-True ((Get-Sha256 $MyInvocation.MyCommand.Path) -eq [string]$invocation.publisherSha256) 'R15E publisher self-pin changed.'

$sourceZip = Resolve-ProjectFile $project ([string]$invocation.requestZip.path) 'request ZIP'
Assert-PinnedFile $sourceZip ([string]$invocation.requestZip.sha256) 'R15E request ZIP'
Assert-True ((Get-Item -LiteralPath $sourceZip).Length -eq [int64]$invocation.requestZip.bytes) 'R15E request ZIP byte count changed.'
foreach ($gatePin in @($invocation.requiredGates)) {
    ...
    Assert-PinnedFile $gatePath ([string]$gatePin.sha256) 'R15E publication gate dependency'
    $gate = Get-Content -Raw -LiteralPath $gatePath | ConvertFrom-Json
    $property = [string]$gatePin.stateProperty
    Assert-True ($gate.PSObject.Properties.Name -contains $property) "R15E publication gate property is absent: $property"
    Assert-True ([string]$gate.$property -eq [string]$gatePin.requiredState) "R15E publication gate state changed: $($gatePin.path)"
}
...
Assert-PinnedFile $responseCollectorScript ([string]$invocation.responseCollector.scriptSha256) 'R15E response collector script'
Assert-PinnedFile $responseCollectorInvocation ([string]$invocation.responseCollector.invocationSha256) 'R15E response collector invocation'
Assert-PinnedFile $responseCollectorRehearsalGate ([string]$invocation.responseCollector.rehearsalGateSha256) 'R15E response collector rehearsal gate'
$responseCollectorGate = Get-Content -Raw -LiteralPath $responseCollectorRehearsalGate | ConvertFrom-Json
Assert-True ([string]$responseCollectorGate.state -eq 'PASS_R15E_COLLECTOR_ZIP_LOOKUP_REHEARSAL' -and [string]$responseCollectorGate.collectorSha256 -eq [string]$invocation.responseCollector.scriptSha256 -and [bool]$responseCollectorGate.exactNestedBytesRead -and [bool]$responseCollectorGate.exactNormalizedFileSetClosed -and [bool]$responseCollectorGate.undeclaredExtraEntryRejected -and [bool]$responseCollectorGate.directoryEntryRejected -and [bool]$responseCollectorGate.normalizedDuplicateEntryRejected) 'R15E response collector rehearsal binding changed.'
...
Assert-PinnedFile $observerScript ([string]$invocation.freshQueueObservation.scriptSha256) 'R15E queue observer script'
Assert-PinnedFile $observerInvocation ([string]$invocation.freshQueueObservation.invocationSha256) 'R15E queue observer invocation'
...
Assert-True ([string]$observation.state -eq 'PASS_R15E_CURRENT_SHARE_OBSERVATION_PREFLIGHT' -and [string]$observation.requestId -eq [string]$invocation.requestId) 'R15E fresh queue observation did not pass for this request.'
Assert-True ([bool]$observation.safety.targetAbsentFromPendingProcessedAndResponses -and [bool]$observation.safety.zeroPendingRequests -and [bool]$observation.safety.zeroUnresolvedEarlierAcceptedRequests -and [bool]$observation.safety.stableBoundedSnapshot) 'R15E fresh queue observation is not clear.'
Assert-True ([int]$observation.scan.pendingRequestFileCount -eq 0 -and [int]$observation.scan.finalPendingRequestFileCount -eq 0 -and [int]$observation.scan.unresolvedEarlierAcceptedRequestCount -eq 0 -and [int]$observation.scan.ambiguousEarlierAcceptedRequestCount -eq 0) 'R15E fresh queue counts are not clear.'
Assert-True ([int]$observation.scan.exactRequestIdPendingMatchCount -eq 0 -and [int]$observation.scan.exactRequestIdProcessedLeafMatchCount -eq 0 -and [int]$observation.scan.exactRequestIdProcessedManifestMatchCount -eq 0 -and [int]$observation.scan.exactRequestIdResponseManifestMatchCount -eq 0) 'R15E exact request identity already exists on the route.'
...
Assert-True ($observationAgeSeconds -ge -5 -and $observationAgeSeconds -le [int]$invocation.freshQueueObservation.maximumAgeSeconds) 'R15E queue observation is stale or future-dated.'
...
Assert-True ($currentBranch -eq [string]$invocation.requiredBranch -and $localTip -eq $remoteTip) 'R15E publication requires matching local/origin branch tips.'
$worktreeRows = @(& git -C $project status --porcelain=v1)
Assert-True ($worktreeRows.Count -eq 0) 'R15E publication requires a clean worktree.'

$shareRoot = [string]$invocation.route.inspectionRevsUnc
Assert-True ($shareRoot -eq '\\shm-cifs\Department\DE-1302_FAB_BE_Engineering\60_Saw_VI_Sort\600_General\Joshua.conn\AVI_Images\Argos\Uploads\InspectionRevs') 'R15E InspectionRevs route changed.'
...
Assert-True ((Normalize-Root ([string]$psDrive.DisplayRoot)).Equals((Normalize-Root $shareRoot), [StringComparison]::OrdinalIgnoreCase)) 'R15E U: PowerShell mapping changed.'
Assert-True ([int]$logicalDisk.DriveType -eq 4 -and (Normalize-Root ([string]$logicalDisk.ProviderName)).Equals((Normalize-Root $shareRoot), [StringComparison]::OrdinalIgnoreCase)) 'R15E U: is not the exact persistent network mapping.'
$requestRoot = 'U:\ProjectPortalRO\requests'
Assert-True (Test-Path -LiteralPath $requestRoot -PathType Container) 'R15E request route is unavailable.'
...
Assert-True ($pendingNow.Count -eq 0) ('R15E another request appeared after observation: ' + (($pendingNow | ForEach-Object { $_.Name }) -join ','))
foreach ($path in @($uploadPath,$readyPath,$processedPath,$publishGatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R15E create-new publication target exists: $path" }
...
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'R15E exact publication path budget failed.'
$maximumEffectiveLength = [int]((@($pathGate.candidates) | Measure-Object effectiveLength -Maximum).Maximum)
Assert-True ($maximumEffectiveLength -lt 200) 'R15E publication path reached effective length 200.'

$preflightResult = [ordered]@{
    schema='argos_r15e_publish_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15E_EXACT_ONE_TIME_PUBLISH_PREFLIGHT';requestId=[string]$invocation.requestId
...
try {
    ...
    $uploadCreated = $true
    Assert-True ((Get-Item -LiteralPath $uploadPath).Length -eq [int64]$invocation.requestZip.bytes -and (Get-Sha256 $uploadPath) -eq [string]$invocation.requestZip.sha256) 'R15E upload copy changed.'
    [IO.File]::Move($uploadPath, $readyPath)
    $uploadCreated = $false
    Assert-True ((Get-Item -LiteralPath $readyPath).Length -eq [int64]$invocation.requestZip.bytes -and (Get-Sha256 $readyPath) -eq [string]$invocation.requestZip.sha256) 'R15E published request changed.'
    $gate = [ordered]@{
        schema='argos_r15e_publish_gate_v1';publishedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15E_EXACT_SIGNED_REQUEST_PUBLISHED_CREATE_NEW';disposition='PENDING_GATE';requestId=[string]$invocation.requestId
