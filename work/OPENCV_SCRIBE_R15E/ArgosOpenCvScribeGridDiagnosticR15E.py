#!/usr/bin/env python3
"""Read-only R15E diagnostic for a pinned R11 perimeter region.
...
REVISION = "ARGOS_OPENCV_SCRIBE_GRID_DIAGNOSTIC_R15E_20260903"
...
def load_dependencies() -> tuple[Any, Any, Any, dict[str, Any]]:
    ...
    for key in ("r11", "r12a", "r12b"):
        ...
        modules.append(import_module(f"argos_r15e_{key}", path))
...
def validate_job(job: dict[str, Any], r11: Any) -> str:
    require(job.get("schema") == JOB_SCHEMA, "R15E job schema mismatch")
...
def run_case(job: dict[str, Any], job_path: Path, job_sha256: str) -> tuple[dict[str, Any], dict[str, bytes]]:
    ...
    require(len(artifacts) == 6, "R15E artifact cardinality changed")
    result = {
        "schema": RESULT_SCHEMA,
        "revision": REVISION,
        "classification": "DIAGNOSTIC_ONLY",
        "state": "HOLD_R15E_DIAGNOSTIC_EVIDENCE_RETURNED",
        "caseId": job["caseId"],
        "jobId": job["jobId"],
...
def validate_output_path(output_root: Path) -> tuple[Path, Path]:
    ...
    require(not root.exists() and not partial.exists(), "R15E output root already exists")
    require(root.parent.is_dir(), "R15E output parent is absent")
    planned = [partial / "CASE_RESULT.json", partial / "hypotheses_DF_REVERSE_180.png"]
    require(max(len(str(path)) + PATH_SUFFIX_RESERVE for path in planned) <= MAXIMUM_SAFE_EFFECTIVE_PATH, "R15E output path budget failed")
    return root, partial
...
def failure_result(job: dict[str, Any] | None, error: Exception) -> dict[str, Any]:
    ...
    return {"schema": RESULT_SCHEMA, "revision": REVISION, "classification": "PENDING_GATE", "state": "HOLD_R15E_INPUT_OR_PROVIDER_GATE_FAILED", "caseId": safe.get("caseId"), "physicalIdentity": safe.get("physicalIdentity"), "eligibleIdentity": False, "referenceAdmissionEligible": False, "provenance": {"sources": {"bf": {"sha256": safe.get("inputs", {}).get("bf", {}).get("sha256"), "verified": False}, "df": {"sha256": safe.get("inputs", {}).get("df", {}).get("sha256"), "verified": False}}}, "artifacts": [], "failure": {"errorType": type(error).__name__, "detail": str(error)}, "holds": [{"code": "SCRIBE_GRID_DIAGNOSTIC_GATE_HOLD", "detail": "R15E failed closed and emitted no diagnostic raster."}], "authority": dict(SAFE_AUTHORITY)}
