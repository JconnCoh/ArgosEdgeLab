...
function Read-ResponseCandidate([string]$Path, [string]$ExpectedRequestId, [Security.Cryptography.X509Certificates.X509Certificate2]$Certificate, [object]$Limits) {
    ...
    try {
        ...
        Assert-True ($actualNames.Count -eq $expectedNames.Count -and @(Compare-Object -ReferenceObject $expectedNames -DifferenceObject $actualNames).Count -eq 0) 'R15E response ZIP exact entry set changed.'
        ...
        Assert-True $signatureValid 'R15E matching response signature is invalid.'
        Assert-True ([string]$manifest.schema -eq 'argos_project_portal_response_manifest_v1' -and [string]$manifest.sourceRole -eq 'JBOD') 'R15E matching response schema/source role changed.'
        Assert-True ([string]$manifest.signatureAlgorithm -eq 'RSA-SHA256-PKCS1' -and ([string]$manifest.signerThumbprint).Replace(' ','').ToUpperInvariant() -eq $Certificate.Thumbprint.Replace(' ','').ToUpperInvariant()) 'R15E matching response signer changed.'
        Assert-True ([bool]$manifest.reviewOnly -and -not [bool]$manifest.trainingEligible -and -not [bool]$manifest.xmlEligible -and -not [bool]$manifest.productionEligible -and -not [bool]$manifest.productionRoutingEnabled -and -not [bool]$manifest.credentialsIncluded) 'R15E matching response authority widened.'
        $records = @($manifest.files)
        Assert-True ($records.Count -eq 3) 'R15E matching response signed-file cardinality changed.'
        ...
        Assert-True (@(Compare-Object -ReferenceObject $expectedSignedNames -DifferenceObject $signedNames).Count -eq 0) 'R15E matching response signed-file names changed.'
        foreach ($record in $records) {
            ...
            Assert-True ([int64]$bytes.Length -eq [int64]$record.bytes -and (Get-ByteSha256 $bytes) -eq ([string]$record.sha256).ToUpperInvariant()) "R15E matching signed response leaf changed: $name"
        }
...
function Test-BundleBytes([byte[]]$BundleBytes, [object]$Envelope, [object]$Limits, [string[]]$ExpectedCaseIds) {
    Assert-True ([int64]$BundleBytes.Length -eq [int64]$Envelope.bundleBytes) 'R15E decoded bundle byte count changed.'
    Assert-True ((Get-ByteSha256 $BundleBytes) -eq ([string]$Envelope.bundleSha256).ToUpperInvariant()) 'R15E decoded bundle hash changed.'
    $memory = New-Object IO.MemoryStream
    ...
    try {
        ...
        Assert-True ($allEntries.Count -le [int]$Limits.maximumBundleEntries) 'R15E bundle entry bound was exceeded.'
        foreach ($archiveEntry in $allEntries) {
            $archiveName = ([string]$archiveEntry.FullName).Replace('\','/').TrimEnd('/')
            Assert-True (-not [string]::IsNullOrWhiteSpace($archiveName)) 'R15E bundle contains an empty archive path.'
            Assert-True (-not [string]::IsNullOrWhiteSpace([string]$archiveEntry.Name)) "R15E bundle directory entries are not allowed: $archiveName"
            [void](Get-SafeChildPath 'C:\R15ER\b' $archiveName)
        }
        ...
        foreach ($entry in $entries) {
            ...
            Assert-True ($seen.Add($name)) "R15E duplicate bundle path: $name"
            Assert-True ([int64]$entry.Length -le [int64]$Limits.maximumBundleEntryBytes) "R15E bundle entry is too large: $name"
            $total += [int64]$entry.Length
            Assert-True ($total -le [int64]$Limits.maximumBundleExpandedBytes) 'R15E expanded bundle byte bound was exceeded.'
        }
        foreach ($required in @('BATCH_GATE.json','EXECUTION.json')) { Assert-True ($seen.Contains($required)) "R15E bundle required leaf is absent: $required" }
        foreach ($caseId in $ExpectedCaseIds) { Assert-True ($seen.Contains($caseId + '/CASE_RESULT.json')) "R15E case result is absent: $caseId" }
        ...
        Assert-True ([string]$batch.schema -eq 'argos_opencv_scribe_r15e_batch_gate_v1' -and [string]$batch.state -eq 'PASS_R15E_BATCH_COMPLETE' -and [string]$batch.requestId -eq 'REQ_20260903T124500000Z_R15E' -and -not [bool]$batch.rehearsal) 'R15E returned batch gate changed.'
        Assert-True ([string]$execution.schema -eq 'argos_opencv_scribe_r15e_execution_v1' -and [string]$execution.state -eq 'PASS_R15E_EXECUTION' -and [string]$execution.requestId -eq 'REQ_20260903T124500000Z_R15E' -and -not [bool]$execution.rehearsal) 'R15E returned execution evidence changed.'
        Assert-True ([string]$execution.batchGateSha256 -eq (Get-ByteSha256 $batchBytes)) 'R15E execution does not bind the returned batch gate.'
        Assert-True ([int]$batch.caseCount -eq $ExpectedCaseIds.Count -and @($batch.cases).Count -eq $ExpectedCaseIds.Count -and [int]$execution.caseCount -eq $ExpectedCaseIds.Count) 'R15E returned case cardinality changed.'
        foreach ($object in @($batch,$execution)) { Assert-True (-not [bool]$object.sourceMutationPerformed -and -not [bool]$object.sourceDeletionPerformed -and -not [bool]$object.taskOrProcessRestarted -and -not [bool]$object.providerActivated -and [bool]$object.reviewOnly -and -not [bool]$object.trainingEligible -and -not [bool]$object.xmlEligible -and -not [bool]$object.productionEligible -and -not [bool]$object.productionRoutingEnabled) 'R15E returned execution authority widened.' }
        foreach ($caseId in $ExpectedCaseIds) {
            $row = @($batch.cases | Where-Object { [string]$_.caseId -eq $caseId })
            Assert-True ($row.Count -eq 1) "R15E batch case identity is absent or duplicated: $caseId"
            $caseBytes = Read-ZipEntryBytes $archive ($caseId + '/CASE_RESULT.json') ([int64]$Limits.maximumJsonBytes) 'case result'
            Assert-True ((Get-ByteSha256 $caseBytes) -eq ([string]$row[0].caseResultSha256).ToUpperInvariant()) "R15E batch does not bind case result: $caseId"
            $case = (New-Object Text.UTF8Encoding($false, $true)).GetString($caseBytes) | ConvertFrom-Json
            Assert-True ([string]$case.schema -eq 'argos_opencv_scribe_grid_diagnostic_case_result_v1' -and [string]$case.caseId -eq $caseId -and [string]$case.state -like 'HOLD_*') "R15E case contract changed: $caseId"
            if ([string]$case.state -eq 'HOLD_R15E_CASE_LAUNCH_FAILURE') {
                Assert-True ([bool]$case.authority.reviewOnly -and -not [bool]$case.authority.automaticIdentityAuthority -and -not [bool]$case.authority.automaticReferenceAdmissionAllowed -and -not [bool]$case.authority.trainingEligible -and -not [bool]$case.authority.xmlEligible -and -not [bool]$case.authority.productionEligible -and -not [bool]$case.authority.productionRoutingEnabled -and -not [bool]$case.authority.mayClearHolds) "R15E launch-failure case authority widened: $caseId"
            }
            else {
                Assert-True ([string]$case.classification -eq 'PENDING_GATE' -and -not [bool]$case.eligibleIdentity -and -not [bool]$case.referenceAdmissionEligible) "R15E case authority widened: $caseId"
            }
            foreach ($artifact in @($case.artifacts)) {
                $artifactName = $caseId + '/' + ([string]$artifact.relativePath).Replace('\','/')
                Assert-True ($expectedFiles.Add($artifactName)) "R15E declared artifact path is duplicated: $artifactName"
                Assert-True ($seen.Contains($artifactName)) "R15E declared artifact is absent: $artifactName"
                $artifactBytes = Read-ZipEntryBytes $archive $artifactName ([int64]$Limits.maximumBundleEntryBytes) 'case artifact'
                Assert-True ((Get-ByteSha256 $artifactBytes) -eq ([string]$artifact.sha256).ToUpperInvariant()) "R15E declared artifact hash changed: $artifactName"
            }
        }
        Assert-True ($seen.SetEquals($expectedFiles)) ('R15E bundle normalized file set is not exactly closed; expected=' + $expectedFiles.Count + ';actual=' + $seen.Count)
...
function Invoke-CollectorZipLookupRehearsal([string]$ProjectRoot, [object]$Invocation, [string]$InvocationPath, [string]$CollectorPath) {
    Assert-True ([string]$PSVersionTable.PSEdition -eq 'Desktop' -and [int]$PSVersionTable.PSVersion.Major -eq 5 -and [int]$PSVersionTable.PSVersion.Minor -eq 1) 'R15E collector ZIP lookup rehearsal requires Windows PowerShell 5.1 Desktop.'
    Assert-True ([string]$Invocation.rehearsal.normalizedNestedEntry -eq 'CASE01/CASE_RESULT.json' -and [string]$Invocation.rehearsal.rawNestedEntry -eq 'CASE01\CASE_RESULT.json') 'R15E collector ZIP lookup rehearsal entry contract changed.'
    $gatePath = Resolve-ProjectFile $ProjectRoot ([string]$Invocation.rehearsal.gatePath) 'collector ZIP lookup rehearsal gate'
    Assert-True (-not (Test-Path -LiteralPath $gatePath)) "R15E collector ZIP lookup rehearsal gate already exists: $gatePath"
    $tempBase = [IO.Path]::GetFullPath([IO.Path]::GetTempPath()).TrimEnd('\')
    $testRoot = Join-Path $tempBase ('ArgosR15ECollectorZipLookup_' + [Guid]::NewGuid().ToString('N'))
    Assert-True ($testRoot.StartsWith($tempBase + '\', [StringComparison]::OrdinalIgnoreCase) -and [IO.Path]::GetFileName($testRoot) -match '^ArgosR15ECollectorZipLookup_[0-9a-f]{32}$') 'R15E collector ZIP lookup rehearsal temporary root is unsafe.'
    ...
    $expectedBytes = (New-Object Text.UTF8Encoding($false)).GetBytes('R15E_BACKSLASH_ENTRY_BYTES_20260902')
    $extraRejected = $false
    ...
    try {
        ...
        $caseObject = [ordered]@{schema='argos_opencv_scribe_grid_diagnostic_case_result_v1';caseId='CASE01';state='HOLD_R15E_REVIEW_REQUIRED';classification='PENDING_GATE';eligibleIdentity=$false;referenceAdmissionEligible=$false;artifacts=@([ordered]@{relativePath='preview.bin';sha256=$artifactSha})}
        ...
        $batchObject = [ordered]@{schema='argos_opencv_scribe_r15e_batch_gate_v1';state='PASS_R15E_BATCH_COMPLETE';requestId='REQ_20260903T124500000Z_R15E';rehearsal=$false;caseCount=1;cases=@([ordered]@{caseId='CASE01';caseResultSha256=$caseSha})}
        ...
        $executionObject = [ordered]@{schema='argos_opencv_scribe_r15e_execution_v1';state='PASS_R15E_EXECUTION';requestId='REQ_20260903T124500000Z_R15E';rehearsal=$false;caseCount=1;batchGateSha256=(Get-ByteSha256 $batchBytes)}
        ...
        try {
            ...
            Assert-True ($rawMatches.Count -eq 1) 'R15E collector ZIP lookup rehearsal nested entry cardinality changed.'
            $rawEntryName = [string]$rawMatches[0].FullName
            Assert-True ([string]::Equals($rawEntryName, [string]$Invocation.rehearsal.rawNestedEntry, [StringComparison]::Ordinal)) 'R15E WinPS 5.1 CreateFromDirectory did not emit the expected raw backslash entry name.'
            Assert-True ($null -eq $archive.GetEntry([string]$Invocation.rehearsal.normalizedNestedEntry)) 'R15E native forward-slash GetEntry unexpectedly resolved the raw backslash entry.'
        ...
        Assert-True ((Get-ByteSha256 $readBytes) -eq (Get-ByteSha256 $caseBytes) -and [int64]$readBytes.Length -eq [int64]$caseBytes.Length) 'R15E normalized collector lookup did not read the exact nested bytes.'
        ...
        catch { $extraRejected = $_.Exception.Message -like 'R15E bundle normalized file set is not exactly closed*' }
        Assert-True $extraRejected 'R15E collector ZIP lookup rehearsal did not reject an undeclared normalized file entry.'
        ...
        catch { $directoryRejected = $_.Exception.Message -like 'R15E bundle directory entries are not allowed*' }
        Assert-True $directoryRejected 'R15E collector ZIP lookup rehearsal did not reject a directory entry.'
        ...
        catch { $duplicateRejected = $_.Exception.Message -like 'R15E duplicate bundle path:*' }
        Assert-True $duplicateRejected 'R15E collector ZIP lookup rehearsal did not reject a normalized duplicate entry.'
    }
    finally {
        if (Test-Path -LiteralPath $testRoot -PathType Container) { [IO.Directory]::Delete($testRoot, $true) }
    }
    Assert-True (-not (Test-Path -LiteralPath $testRoot)) 'R15E collector ZIP lookup rehearsal temporary root remains.'
    $gate = [ordered]@{
        schema='argos_r15e_collector_zip_lookup_rehearsal_gate_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15E_COLLECTOR_ZIP_LOOKUP_REHEARSAL';classification='FROZEN_REVIEW_ONLY'
        collectorPath='work/OPENCV_SCRIBE_R15E/Collect-R15EResponseR2.ps1';collectorSha256=Get-Sha256 $CollectorPath;rehearsalInvocationPath='work/OPENCV_SCRIBE_R15E/Test-R15ECollectorR2.invocation.json';rehearsalInvocationSha256=Get-Sha256 $InvocationPath
...
Assert-True (Test-Path -LiteralPath $InvocationManifest -PathType Leaf) 'R15E response collection invocation is absent.'
...
if ($Rehearsal) { Assert-True ([string]$invocation.schema -eq 'argos_r15e_collector_r2_rehearsal_invocation_v1') 'R15E collector rehearsal invocation schema changed.' }
else { Assert-True ([string]$invocation.schema -eq 'argos_r15e_response_collection_r2_invocation_v1') 'R15E R2 response collection invocation schema changed.' }
Assert-True ([string]$invocation.requestId -eq 'REQ_20260903T124500000Z_R15E') 'R15E response collection request identity changed.'
Assert-True ((Get-Sha256 $MyInvocation.MyCommand.Path) -eq [string]$invocation.collectorSha256) 'R15E response collector self-pin changed.'
Assert-True ([int]$invocation.maximumMatchingResponses -eq 1 -and -not [bool]$invocation.requestRetryAuthorized -and [bool]$invocation.matchingSignedTerminalResponseOnly) 'R15E response correlation/retry boundary changed.'
Assert-True ([bool]$invocation.authority.reviewOnly -and -not [bool]$invocation.authority.automaticIdentityAuthority -and -not [bool]$invocation.authority.trainingEligible -and -not [bool]$invocation.authority.xmlEligible -and -not [bool]$invocation.authority.productionEligible -and -not [bool]$invocation.authority.productionRoutingEnabled -and -not [bool]$invocation.authority.providerActivationAllowed) 'R15E response collection authority widened.'
...
foreach ($gatePin in @($invocation.requiredGates)) {
    ...
    Assert-PinnedFile $gatePath ([string]$gatePin.sha256) 'R15E collection gate dependency'
    $gate = Get-Content -Raw -LiteralPath $gatePath | ConvertFrom-Json
    $property = [string]$gatePin.stateProperty
    Assert-True ([string]$gate.$property -eq [string]$gatePin.requiredState) "R15E collection dependency state changed: $($gatePin.path)"
}

$publishGatePath = Resolve-ProjectFile $project ([string]$invocation.publicationGate.path) 'R15E publication gate'
Assert-True (Test-Path -LiteralPath $publishGatePath -PathType Leaf) 'R15E publication gate is absent; response collection cannot precede exact one-time publication.'
...
Assert-PinnedFile $certificatePath ([string]$invocation.responseSigner.certificateSha256) 'R15E JBOD response signer certificate'
Assert-PinnedFile $verifierPath ([string]$invocation.responseSigner.verifierSha256) 'R15E signed response verifier'
$certificate = New-Object Security.Cryptography.X509Certificates.X509Certificate2($certificatePath)
try {
    Assert-True ($certificate.Thumbprint.Replace(' ','').ToUpperInvariant() -eq ([string]$invocation.responseSigner.thumbprint).Replace(' ','').ToUpperInvariant()) 'R15E JBOD response signer thumbprint changed.'
    $shareRoot = [string]$invocation.route.inspectionRevsUnc
    Assert-True ($shareRoot -eq '\\shm-cifs\Department\DE-1302_FAB_BE_Engineering\60_Saw_VI_Sort\600_General\Joshua.conn\AVI_Images\Argos\Uploads\InspectionRevs') 'R15E response route changed.'
    ...
    Assert-True ((Normalize-Root ([string]$psDrive.DisplayRoot)).Equals((Normalize-Root $shareRoot), [StringComparison]::OrdinalIgnoreCase)) 'R15E response U: PowerShell mapping changed.'
    Assert-True ([int]$logicalDisk.DriveType -eq 4 -and (Normalize-Root ([string]$logicalDisk.ProviderName)).Equals((Normalize-Root $shareRoot), [StringComparison]::OrdinalIgnoreCase)) 'R15E response U: is not the exact persistent network mapping.'
    $responseRoot = 'U:\ProjectPortalRO\responses'
    Assert-True (Test-Path -LiteralPath $responseRoot -PathType Container) 'R15E response share is unavailable.'
    ...
    Assert-True ($responseFiles.Count -le [int]$invocation.limits.maximumResponseFiles) 'R15E response file scan bound was exceeded.'
    ...
    Assert-True ($matches.Count -eq 1) "R15E requires exactly one matching signed response; found $($matches.Count)."
    $match = $matches[0]
    $manifest = $match.Manifest
    Assert-True (@($invocation.allowedTerminalStates) -contains [string]$manifest.state) "R15E response state is not an allowed terminal state: $($manifest.state)"
    Assert-True (-not [string]::IsNullOrWhiteSpace([string]$manifest.responseId)) 'R15E matching response ID is empty.'
    $responseSnapshot = Get-Item -LiteralPath $match.Path
    Assert-True ([int64]$responseSnapshot.Length -eq [int64]$match.ZipBytes -and (Get-Sha256 $match.Path) -eq [string]$match.ZipSha256) 'R15E matching response changed during bounded collection preflight.'
    ...
    if ([string]$manifest.state -eq 'PASS_MAINTENANCE_PATCH') {
        ...
        Assert-True ([string]$envelope.schema -eq 'argos_opencv_scribe_r15e_maintenance_envelope_v1' -and [string]$envelope.state -eq 'PASS_R15E_SIGNED_RETURN_READY' -and [string]$envelope.requestId -eq [string]$invocation.requestId -and -not [bool]$envelope.rehearsal) 'R15E maintenance envelope changed.'
        Assert-True ([int]$envelope.caseCount -eq 4 -and [int64]$envelope.bundleBytes -le [int64]$invocation.limits.maximumBundleBytes -and [int]$envelope.bundleBase64Characters -le [int]$invocation.limits.maximumBundleBase64Characters) 'R15E maintenance envelope bounds/cardinality changed.'
        Assert-True ([string]$envelope.bundleBase64 -ne '' -and ([string]$envelope.bundleBase64).Length -eq [int]$envelope.bundleBase64Characters) 'R15E maintenance envelope Base64 length changed.'
        Assert-True (-not [bool]$envelope.automaticRetryAllowed -and -not [bool]$envelope.sourceMutationPerformed -and -not [bool]$envelope.sourceDeletionPerformed -and -not [bool]$envelope.taskOrProcessRestarted -and -not [bool]$envelope.providerActivated -and -not [bool]$envelope.holdsCleared -and [bool]$envelope.reviewOnly -and -not [bool]$envelope.trainingEligible -and -not [bool]$envelope.xmlEligible -and -not [bool]$envelope.productionEligible -and -not [bool]$envelope.productionRoutingEnabled) 'R15E maintenance envelope authority widened.'
    ...
    $collectionGatePath = Join-Path $collectionRoot 'R15E_EXACT_RESPONSE_COLLECTION_GATE.json'
    $shortRoot = [string]$invocation.shortExtractionRoot
    Assert-True ($shortRoot -eq 'C:\R15ER') 'R15E short extraction root changed.'
    foreach ($path in @($collectionRoot,$partialRoot,$shortRoot)) { Assert-True (-not (Test-Path -LiteralPath $path)) "R15E create-new response collection target exists: $path" }
    $plannedPaths = New-Object Collections.Generic.List[string]
    foreach ($path in @((Join-Path $shortRoot 'z.zip'),(Join-Path $shortRoot 'r\PORTAL_RESPONSE_MANIFEST.json'),(Join-Path $shortRoot 'b\BATCH_GATE.json'),(Join-Path $partialRoot 'response\transport.ready.zip'),(Join-Path $partialRoot 'response\PORTAL_RESPONSE_MANIFEST.json'),(Join-Path $partialRoot 'evidence\R15E_RETURN.zip'),$collectionGatePath)) { $plannedPaths.Add($path) }
    ...
    Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'R15E response collection path budget failed.'
    $maximumEffectiveLength = [int]((@($pathGate.candidates) | Measure-Object effectiveLength -Maximum).Maximum)
    Assert-True ($maximumEffectiveLength -lt 200) 'R15E response collection path reached effective length 200.'

    $preflightResult = [ordered]@{
        schema='argos_r15e_response_collection_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R15E_EXACT_RESPONSE_COLLECTION_PREFLIGHT';requestId=[string]$invocation.requestId;responseId=[string]$manifest.responseId
    ...
    try {
        ...
        Assert-True ((Get-Sha256 $shortZip) -eq [string]$match.ZipSha256) 'R15E short response copy changed.'
        ...
        Assert-True ([string]$verification.State -eq 'PASS_SIGNED_PORTAL_RESPONSE' -and [string]$verification.EndpointState -eq [string]$manifest.state -and ([string]$verification.SignerThumbprint).Replace(' ','').ToUpperInvariant() -eq $certificate.Thumbprint.Replace(' ','').ToUpperInvariant()) 'R15E official signed response verification failed.'
        $shortBundle = $null
        if ($null -ne $bundleEvidence) {
            $shortBundle = Join-Path $shortRoot 'R15E_RETURN.zip'
            [IO.File]::WriteAllBytes($shortBundle, [byte[]]$bundleBytes)
            Assert-True ((Get-Sha256 $shortBundle) -eq [string]$envelope.bundleSha256) 'R15E short decoded bundle changed.'
        ...
        if ($null -ne $bundleEvidence) {
            [IO.File]::Copy($shortBundle, (Join-Path $partialRoot 'evidence\R15E_RETURN.zip'), $false)
        ...
        $shortCreated = $false
        $gate = [ordered]@{
            schema='argos_r15e_exact_response_collection_gate_v1';collectedUtc=[DateTime]::UtcNow.ToString('o');state=if ([string]$manifest.state -eq 'PASS_MAINTENANCE_PATCH') {'PASS_R15E_EXACT_SIGNED_TERMINAL_RESPONSE_AND_BUNDLE_COLLECTED'} else {'PASS_R15E_EXACT_SIGNED_TERMINAL_FAILURE_COLLECTED'}
        ...
        Write-JsonCreateNew (Join-Path $collectionRoot 'R15E_EXACT_RESPONSE_COLLECTION_GATE.json') $gate
