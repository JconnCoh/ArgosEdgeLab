...
function Assert-Pin([string]$Path, [string]$Sha256, [string]$RequiredState = '') {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "O2D23 build dependency absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "O2D23 build dependency changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($RequiredState)) {
        $value = Get-Content -Raw -LiteralPath $Path | ConvertFrom-Json
        Assert-True ([string]$value.state -eq $RequiredState) "O2D23 build dependency state changed: $Path"
    }
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 32) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "O2D23 build create-new JSON exists: $Path"
...
function New-FinalPackageGate([int64]$RequestZipBytes, [string]$RequestZipSha256, [string]$RequestManifestSha256, [string]$RequestSignatureSha256) {
    return [ordered]@{
        schema='argos_o2d23_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D23_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip='work/OPENCV_SCRIBE_O2D23/final/'+$zipName
        ...
        entrypointTestGateState='PASS_O2D23_ENTRYPOINT_TEST_GATE_R2';entrypointTestGateSha256=$testGateSha;declaredRehearsalGateState=[string]$definition.rehearsal.gateState;declaredRehearsalGateSha256=[string]$definition.rehearsal.gateSha256
        selfPinGateState='PASS_O2D23_SELF_PIN_AND_LIVE_BRANCH_GATE';selfPinGateSha256=$selfPinGateSha;declaredSelfPinGateState=[string]$definition.rehearsal.selfPinGateState;declaredSelfPinGateSha256=[string]$definition.rehearsal.selfPinGateSha256;endpointSelfPinCount=6;endpointSelfPinMatchCount=6;liveAssertionBranchCaseCount=3
        ...
        normalRequiredState='PASS_O2D23_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED';outerTimeoutCoversChild=$true;uniqueRequestId=$true;installedReferenceBundleReusedByExactHash=$true
...
$endpointPath = Join-Path $root 'Invoke-O2D23ScribeEndpoint.ps1'
$enginePath = Join-Path $project 'work\OPENCV_SCRIBE_V1R5\ArgosOpenCvScribeV1R5.py'
$jobPath = Join-Path $root 'O2D23_SLOT25_JOB.json'
...
$gatePath = Join-Path $root 'O2D23_FINAL_PACKAGE_GATE.json'
...
$cloneManifestPath = Join-Path $root 'O2D23_CLONE_LITERAL_REMEDIATION_BUILD_FINAL.json'
$cloneGatePath = Join-Path $root 'O2D23_CLONE_LITERAL_GATE_FINAL_R3.json'
$developmentParentTerminalGatePath = Join-Path $project 'work\OPENCV_SCRIBE_O2D22\O2D22_TERMINAL_RESPONSE_GATE.json'
$selfPinGatePath = Join-Path $root 'O2D23_SELF_PIN_AND_LIVE_BRANCH_GATE.json'

$endpointSha = '159BD82528E505DE69BDEEE4C3A3B29402BA9281939C57F6187587E9047D9740'
$engineSha = 'F61F5954A77E6F730A2BF0D110A468535C4595D25DB21AFFE1573EF08B8139AB'
$jobSha = '08B84D4FD74F96E47548DDB5C38FA7F808D6002ADFC22BF8159F2654E74A3029'
$definitionSha = '17C3D8046E4C74383B3985AACDFDEA1E8FDF61EE215B290A44587083D2AA95AF'
$testGateSha = '17EFFBE2B9557688BFD4181B3DC6F8F3E01687E0FF9FBC59DDA0967370B40CF1'
$selfPinGateSha = 'F00DED7060A9E8FB269670B0D89AEEA50241A82D1E6C0D5B74571B9AB91E1FD5'
$noArgumentGateSha = 'E83611A70DE1DEF20DC11F8036AB2224E088E96C410817B5D38723ECB2D07B7E'
$developmentParentTerminalGateSha = '90ABE4E37A74EED80EC7D2F82296D1B101E2F655A2AB7F909D5D525F5B34D7F2'
$preactionSha = '58F7D5412EB39872C19F31332E261EFA5E99B4161EDDDF6498F07BD70651FF35'
...
Assert-Pin (Join-Path $root 'O2D23_ENTRYPOINT_TEST_GATE_R2.json') $testGateSha 'PASS_O2D23_ENTRYPOINT_TEST_GATE_R2'
Assert-Pin $selfPinGatePath $selfPinGateSha 'PASS_O2D23_SELF_PIN_AND_LIVE_BRANCH_GATE'
Assert-Pin (Join-Path $root 'O2D23_NO_ARGUMENT_FILE_GATE.json') $noArgumentGateSha 'PASS_O2D23_EXACT_NO_ARGUMENT_WINDOWS_POWERSHELL_51_FILE'
Assert-Pin $cloneManifestPath '4219177E0B2F1CBE9FAF36B53F5108D749D302E09A9A2806CBDA066EB9CEDBD7'
Assert-Pin $developmentParentTerminalGatePath $developmentParentTerminalGateSha 'PASS_O2D22_EXACT_SIGNED_SLOT24_BLIND_VALIDATION_RESPONSE'
Assert-Pin (Join-Path $root 'PREACTION_BUILD_SIGN_O2D23.json') $preactionSha 'PASS_PREACTION_CONTRACT'
...
foreach ($tool in @($packageTester,$pathTool,$preactionTool,$windowsPowerShell,$identityPath,$publicCertificate)) { Assert-True (Test-Path -LiteralPath $tool -PathType Leaf) "O2D23 build tool absent: $tool" }

Assert-True (Test-Path -LiteralPath $cloneGatePath -PathType Leaf) 'O2D23 clone gate absent.'
$cloneGate = Get-Content -Raw -LiteralPath $cloneGatePath | ConvertFrom-Json
Assert-True ([string]$cloneGate.state -eq 'PASS_ARGOS_CLONE_LITERAL_REMEDIATION' -and [string]$cloneGate.mode -eq 'GATE') 'O2D23 clone gate state changed.'
Assert-True ([string]$cloneGate.manifestSha256 -eq (Get-Sha256 $cloneManifestPath)) 'O2D23 clone gate manifest pin changed.'
$builderPair = @($cloneGate.pairs | Where-Object { [string]$_.generated -eq 'work/OPENCV_SCRIBE_O2D23/Build-O2D23Request.ps1' })
Assert-True ($builderPair.Count -eq 1 -and [string]$builderPair[0].generatedSha256 -eq (Get-Sha256 $MyInvocation.MyCommand.Path)) 'O2D23 clone gate does not bind the exact builder.'
$cloneGateSha = Get-Sha256 $cloneGatePath

$preactionJson = & $preactionTool -AuditPath (Join-Path $project 'work\ARGOS_HISTORY_NO_REPEAT_AUDIT_20260820.json') -ContractPath (Join-Path $root 'PREACTION_BUILD_SIGN_O2D23.json') -ProjectRoot $project -Preflight | Out-String
$preaction = $preactionJson | ConvertFrom-Json
Assert-True ([string]$preaction.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'O2D23 preaction state changed.'

$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH' -and [string]$definition.entryPoint -eq 'payload/Invoke-O2D23ScribeEndpoint.ps1') 'O2D23 definition route changed.'
Assert-True (@($definition.changes).Count -eq 1 -and @($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'O2D23 definition action cardinality changed.'
Assert-True ([string]$definition.changes[0].source -eq 'payload/Invoke-O2D23ScribeEndpoint.ps1' -and [string]$definition.changes[0].destination -eq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_O2D23.ps1') 'O2D23 exact maintenance change identity changed.'
Assert-True (@($definition.changes[0].approvedPredecessorSha256).Count -eq 1 -and [string]$definition.changes[0].approvedPredecessorSha256[0] -eq '08B6E76548CEE99EA11FC6245FB07C05F2E31E53B23562F8A1B15B1AC6EF6A32') 'O2D23 approved installed predecessor changed.'
Assert-True ([string]$definition.changes[0].installedSha256 -eq $endpointSha) 'O2D23 maintenance installedSha256 does not equal the exact endpoint payload.'
Assert-True ([string]$definition.rehearsal.requiredState -eq 'PASS_O2D23_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED' -and [int]$definition.timeoutContract.endpointWorkerOuterTimeoutSeconds -gt [int]$definition.timeoutContract.opencvChildTimeoutSeconds) 'O2D23 timeout or normal required-state contract changed.'
Assert-True ([string]$definition.rehearsal.gateState -eq 'PASS_O2D23_ENTRYPOINT_TEST_GATE_R2' -and [string]$definition.rehearsal.gateSha256 -eq $testGateSha) 'O2D23 declared entrypoint rehearsal evidence changed.'
Assert-True ([string]$definition.rehearsal.selfPinGateState -eq 'PASS_O2D23_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [string]$definition.rehearsal.selfPinGateSha256 -eq $selfPinGateSha) 'O2D23 declared self-pin/live-branch evidence changed.'
Assert-True ([string]$definition.sourceProcessingContract.inputMode -eq 'DEVELOPMENT_AUTO_LOCALIZED_WHOLE_IMAGE' -and [bool]$definition.sourceProcessingContract.upstreamNotchHoldDoesNotSkipScribeDevelopment -and [bool]$definition.sourceProcessingContract.boundedExceptionSearchAllowed -and -not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed -and -not [bool]$definition.sourceProcessingContract.holdClearanceAllowed -and -not [bool]$definition.sourceProcessingContract.providerActivationAllowed) 'O2D23 processing boundary changed.'
foreach ($path in @($signedRoot,$partialSigned,$finalRoot,$partialFinal,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "O2D23 fresh output exists: $path" }

$planned = @(
    $readyRoot,
    (Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.json'),
    (Join-Path $readyRoot 'PORTAL_REQUEST_MANIFEST.sig'),
    (Join-Path $readyRoot 'payload\Invoke-O2D23ScribeEndpoint.ps1'),
    (Join-Path $readyRoot 'payload\ArgosOpenCvScribeV1R5.py'),
    (Join-Path $readyRoot 'payload\O2D23_SLOT25_JOB.json'),
    $zipPath,
    (Join-Path $partialFinal 'extract\payload\Invoke-O2D23ScribeEndpoint.ps1'),
    $gatePath,
    'C:\ProgramData\ArgosProjectPortalRO\endpoint_jbod\state\work\J_0123456789_00000000\package\payload\Invoke-O2D23ScribeEndpoint.ps1',
...
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'O2D23 build path gate failed.'
$candidateRows = @($pathGate.candidates)
Assert-True ($candidateRows.Count -eq $planned.Count) 'O2D23 build path candidate cardinality changed.'
...
try {
    ...
    Assert-True ($certificateMatches.Count -eq 1) 'O2D23 signer certificate cardinality changed.'
...
Assert-True ($certificate.HasPrivateKey) 'O2D23 signer private key unavailable.'
...
if ($Preflight) {
    ...
    Assert-True ([string]$projectionRoundTrip.schema -eq 'argos_o2d23_final_package_gate_v1' -and [string]$projectionRoundTrip.cloneLiteralGateSha256 -eq $cloneGateSha) 'O2D23 final gate construction rehearsal failed.'
    [ordered]@{schema='argos_o2d23_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D23_BUILD_PREFLIGHT';requestId=$requestId;payloadFileCount=3;endpointSha256=$endpointSha;engineSha256=$engineSha;jobSha256=$jobSha;definitionSha256=$definitionSha;pathState=[string]$pathGate.state;signerThumbprint=$thumbprint;finalGateConstructionRehearsed=$true;mutationsPerformed=$false;targetExecuted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 7
    return
}

$files = @(
    [ordered]@{source=$endpointPath;path='payload/Invoke-O2D23ScribeEndpoint.ps1';bytes=[int64](Get-Item -LiteralPath $endpointPath).Length;sha256=$endpointSha},
    ...
    [ordered]@{source=$jobPath;path='payload/O2D23_SLOT25_JOB.json';bytes=[int64](Get-Item -LiteralPath $jobPath).Length;sha256=$jobSha}
)
...
Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'O2D23 signed package verification failed.'
...
$expected = @{
    'payload/Invoke-O2D23ScribeEndpoint.ps1'=$endpointSha
    'payload/ArgosOpenCvScribeV1R5.py'=$engineSha
    'payload/O2D23_SLOT25_JOB.json'=$jobSha
...
Assert-True ($extracted.Count -eq 5) 'O2D23 final ZIP file count changed.'
foreach ($item in $expected.GetEnumerator()) {
    $path = Join-Path $extractRoot $item.Key.Replace('/', '\')
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "O2D23 final ZIP leaf absent: $($item.Key)"
    Assert-True ((Get-Sha256 $path) -eq [string]$item.Value) "O2D23 final ZIP leaf changed: $($item.Key)"
}
$tokens = $null
$parserErrors = $null
[void][Management.Automation.Language.Parser]::ParseFile((Join-Path $extractRoot 'payload\Invoke-O2D23ScribeEndpoint.ps1'), [ref]$tokens, [ref]$parserErrors)
Assert-True (@($parserErrors).Count -eq 0) 'O2D23 extracted endpoint parser failed.'
$extractedManifest = Get-Content -Raw -LiteralPath (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json') | ConvertFrom-Json
Assert-True (@($extractedManifest.changes).Count -eq 1 -and [string]$extractedManifest.changes[0].source -eq 'payload/Invoke-O2D23ScribeEndpoint.ps1') 'O2D23 extracted signed manifest change identity changed.'
Assert-True ([string]$extractedManifest.changes[0].installedSha256 -eq $endpointSha) 'O2D23 extracted signed manifest installedSha256 does not equal endpoint payload.'
Assert-True ([string]$extractedManifest.rehearsal.gateState -eq 'PASS_O2D23_ENTRYPOINT_TEST_GATE_R2' -and [string]$extractedManifest.rehearsal.gateSha256 -eq $testGateSha) 'O2D23 extracted signed manifest rehearsal evidence changed.'
Assert-True ([string]$extractedManifest.rehearsal.selfPinGateState -eq 'PASS_O2D23_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [string]$extractedManifest.rehearsal.selfPinGateSha256 -eq $selfPinGateSha) 'O2D23 extracted signed manifest self-pin evidence changed.'
