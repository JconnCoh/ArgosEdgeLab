...
function Add-UniquePath([Collections.Generic.List[string]]$List, [Collections.Generic.HashSet[string]]$Set, [string]$Path) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($Path)) 'O2D21 route path is empty.'
    Assert-True ($Set.Add($Path)) "O2D21 duplicate route path: $Path"
    $List.Add($Path)
}
...
$finalGatePath = Join-Path $root 'O2D21_FINAL_PACKAGE_GATE.json'
$zipPath = Join-Path $root ('final\' + $requestReadyName + '.zip')
$extractRoot = Join-Path $root 'final\extract'
$outputPath = Join-Path $root 'O2D21_COMPLETE_ROUTE_GATE.json'
...
foreach ($dependency in @($finalGatePath,$zipPath,(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json'),(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig'),$pathTool,$packageTester,$publicCertificate,$jeoPath,$queueGatePath)) {
    Assert-True (Test-Path -LiteralPath $dependency -PathType Leaf) "O2D21 route dependency absent: $dependency"
}
Assert-True ((Get-Sha256 $finalGatePath) -eq '04DC48EB53A807E37DEEB27EA5591E48DDB698C1DE6F1B1B5E0B912FCFDABEEB') 'O2D21 final package gate changed.'
Assert-True ((Get-Sha256 $jeoPath) -eq '91B5A9219F72845187D7CB17DAB0F1D74AF223E7DDA11E380EEA26148C60CD01') 'O2D21 direct JBOD observation changed.'
Assert-True ((Get-Sha256 $queueGatePath) -eq '170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D') 'O2D21 inherited queue-safety gate changed.'

$finalGate = Get-Content -Raw -LiteralPath $finalGatePath | ConvertFrom-Json
Assert-True ([string]$finalGate.state -eq 'PASS_O2D21_FINAL_PACKAGE_GATE' -and [string]$finalGate.requestId -eq $requestId) 'O2D21 final package identity changed.'
Assert-True (-not [bool]$finalGate.publicationAuthorized -and [bool]$finalGate.publicationRequiresCompleteRouteGate) 'O2D21 final package publication boundary changed.'
Assert-True ([bool]$finalGate.maintenanceInstalledShaMatchesPayload -and [string]$finalGate.endpointPayloadSha256 -eq '7862116EA2520FE6E3EE1AF03DCFED1A9E926BB8375D80BCE658206DC3E48CEC' -and [string]$finalGate.declaredInstalledSha256 -eq [string]$finalGate.endpointPayloadSha256) 'O2D21 endpoint payload and declared installed hashes diverged.'
Assert-True ([string]$finalGate.entrypointTestGateState -eq 'PASS_O2D21_ENTRYPOINT_TEST_GATE_R2' -and [string]$finalGate.entrypointTestGateSha256 -eq '6737C358531CD06AFB4D513C3A3333E85905116292A581B5EFE3425275AAC2FB') 'O2D21 entrypoint test gate changed.'
Assert-True ([string]$finalGate.selfPinGateState -eq 'PASS_O2D21_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [string]$finalGate.selfPinGateSha256 -eq '81CC2310488026B8F30FB9F66C4C66CEBC17389AA4866E71E176135199789B44' -and [int]$finalGate.endpointSelfPinCount -eq 6 -and [int]$finalGate.endpointSelfPinMatchCount -eq 6 -and [int]$finalGate.liveAssertionBranchCaseCount -eq 3) 'O2D21 self-pin or live-branch gate changed.'
Assert-True ((Get-Sha256 $zipPath) -eq [string]$finalGate.requestZipSha256) 'O2D21 final ZIP changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json')) -eq [string]$finalGate.requestManifestSha256) 'O2D21 extracted request manifest changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig')) -eq [string]$finalGate.requestSignatureSha256) 'O2D21 extracted request signature changed.'
...
Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'O2D21 exact signed package verification failed.'

$manifest = Get-Content -Raw -LiteralPath (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json') | ConvertFrom-Json
Assert-True ([string]$manifest.requestId -eq $requestId -and [string]$manifest.targetRole -eq 'JBOD' -and [string]$manifest.jobClass -eq 'MAINTENANCE_PATCH') 'O2D21 request route changed.'
Assert-True ([int64]$manifest.maxResultBytes -eq 4194304 -and @($manifest.files).Count -eq 3) 'O2D21 request file or result bound changed.'
Assert-True ([string]$manifest.entryPoint -eq 'payload/Invoke-O2D21ScribeEndpoint.ps1' -and [string]$manifest.rehearsal.requiredState -eq 'PASS_O2D21_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED') 'O2D21 entrypoint contract changed.'
Assert-True ([string]$manifest.rehearsal.gateState -eq 'PASS_O2D21_ENTRYPOINT_TEST_GATE_R2' -and [string]$manifest.rehearsal.gateSha256 -eq '6737C358531CD06AFB4D513C3A3333E85905116292A581B5EFE3425275AAC2FB') 'O2D21 signed rehearsal declaration changed.'
Assert-True ([string]$manifest.rehearsal.selfPinGateState -eq 'PASS_O2D21_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [string]$manifest.rehearsal.selfPinGateSha256 -eq '81CC2310488026B8F30FB9F66C4C66CEBC17389AA4866E71E176135199789B44') 'O2D21 signed self-pin declaration changed.'
Assert-True (@($manifest.changes).Count -eq 1 -and [string]$manifest.changes[0].source -eq 'payload/Invoke-O2D21ScribeEndpoint.ps1' -and [string]$manifest.changes[0].installedSha256 -eq '7862116EA2520FE6E3EE1AF03DCFED1A9E926BB8375D80BCE658206DC3E48CEC') 'O2D21 signed maintenance source or installed hash changed.'
Assert-True ([int]$manifest.timeoutContract.endpointWorkerOuterTimeoutSeconds -eq 900 -and [int]$manifest.timeoutContract.opencvChildTimeoutSeconds -eq 600) 'O2D21 timeout contract changed.'
Assert-True (@($manifest.allowedTaskActions).Count -eq 0 -and @($manifest.allowedProcessActions).Count -eq 1) 'O2D21 action contract changed.'

$jeo = Get-Content -Raw -LiteralPath $jeoPath | ConvertFrom-Json
Assert-True ([string]$jeo.state -eq 'PASS_JEO1_DIRECT_ADMIN_READ_ONLY_OBSERVATION' -and [string]$jeo.computerName -eq 'A1025645101') 'O2D21 direct JBOD observation identity changed.'
$endpointWorkerRows = @($jeo.sourceResults | Where-Object { [string]$_.id -eq 'ENDPOINT_WORKER' })
Assert-True ($endpointWorkerRows.Count -eq 1 -and @($endpointWorkerRows[0].rows).Count -eq 1 -and [string]$endpointWorkerRows[0].rows[0].sha256 -eq 'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250') 'O2D21 observed endpoint worker pin changed.'
...
Assert-True ($endpointProcess.Count -eq 1 -and $senderProcess.Count -ge 1) 'O2D21 observed JBOD portal process evidence changed.'

$requestLeaves = @(
    'PORTAL_REQUEST_MANIFEST.json',
    'PORTAL_REQUEST_MANIFEST.sig',
    'payload\Invoke-O2D21ScribeEndpoint.ps1',
    'payload\ArgosOpenCvScribeV1R5.py',
    'payload\O2D21_SLOT23_JOB.json'
)
$responseLeaves = @(
...
foreach ($path in @(
    ...
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_O2D21.ps1',
    'D:\A2\w\ocv\O2D21_20260827T023200000Z_8A9CFF90.partial\refs\PORTABLE_GLYPH_REFERENCE_MANIFEST.json',
    'D:\A2\w\ocv\O2D21_20260827T023200000Z_8A9CFF90\ArgosOpenCvScribeV1R5.py',
    'D:\A2\w\ocv\O2D21_20260827T023200000Z_8A9CFF90\JOB.json',
    'D:\A2\w\ocv\O2D21_20260827T023200000Z_8A9CFF90\refs\glyphs_v5_confirmed_20260806\028_62630_456_SLOT23_P04_S.png',
    'D:\A2\o\ocv\O2D21_20260827T023200000Z_8A9CFF90\RESULT.json',
    'D:\A2\o\ocv\O2D21_20260827T023200000Z_8A9CFF90\RUN_GATE.json',
...
Assert-True ($paths.Count -ge 120 -and $paths.Count -le 160) "O2D21 route path cardinality changed: $($paths.Count)"

$rows = New-Object Collections.Generic.List[object]
foreach ($path in $paths) {
    $checked = & $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
    Assert-True ([string]$checked.state -eq 'PASS_PATH_BUDGET' -and @($checked.candidates).Count -eq 1) "O2D21 route path budget failed: $path"
...
Assert-True ([int]$longest.effectiveLength -lt 200 -and $maximumComponentLength -le 80) 'O2D21 complete route exceeds the safe materialized path budget.'

$state = $(if ($Preflight) { 'HOLD_O2D21_COMPLETE_ROUTE_PREFLIGHT_ARGOS_INBOUND_RELAY_UNPROVEN' } else { 'HOLD_O2D21_COMPLETE_ROUTE_GATE_ARGOS_INBOUND_RELAY_UNPROVEN' })
$result = [ordered]@{
    schema = 'argos_o2d21_complete_route_gate_v1'
    ...
    finalPackageGateSha256 = '04DC48EB53A807E37DEEB27EA5591E48DDB698C1DE6F1B1B5E0B912FCFDABEEB'
    endpointPayloadSha256 = '7862116EA2520FE6E3EE1AF03DCFED1A9E926BB8375D80BCE658206DC3E48CEC'
    declaredInstalledSha256 = '7862116EA2520FE6E3EE1AF03DCFED1A9E926BB8375D80BCE658206DC3E48CEC'
    maintenanceInstalledShaMatchesPayload = $true
    declaredRehearsalGateState = 'PASS_O2D21_ENTRYPOINT_TEST_GATE_R2'
    declaredRehearsalGateSha256 = '6737C358531CD06AFB4D513C3A3333E85905116292A581B5EFE3425275AAC2FB'
    declaredSelfPinGateState = 'PASS_O2D21_SELF_PIN_AND_LIVE_BRANCH_GATE'
    declaredSelfPinGateSha256 = '81CC2310488026B8F30FB9F66C4C66CEBC17389AA4866E71E176135199789B44'
...
Assert-True (-not (Test-Path -LiteralPath $outputPath)) 'O2D21 route gate output already exists.'
