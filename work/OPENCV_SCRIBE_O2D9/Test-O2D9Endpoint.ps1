...
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 16) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "O2D9 test create-new JSON exists: $Path"
...
$endpoint = Join-Path $PSScriptRoot 'Invoke-O2D9ScribeEndpoint.ps1'
$engine = Join-Path $project 'work\OPENCV_SCRIBE_V1R3\ArgosOpenCvScribeV1R3.py'
$liveJob = Join-Path $PSScriptRoot 'O2D9_SLOT16_JOB.json'
...
foreach ($path in @($endpoint,$engine,$liveJob,$bundle,(Join-Path $runtime 'python.exe'),$sampleBf,$sampleDf)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "O2D9 test dependency absent: $path"
}
Assert-True ((Get-Sha256 $engine) -eq '8A6DE04B7DD08EFA717AF606FD0D04622ABE84C753B690C4590B0E95D8B31BAB') 'O2D9 test engine changed.'
Assert-True ((Get-Sha256 $bundle) -eq '56DF00E37A195E7BC3E026E4950DFB5A0AA7E7AF49A6FF39020B071840CCFBD6') 'O2D9 test bundle changed.'
Assert-True ((Get-Sha256 $sampleBf) -eq $bfSha -and (Get-Sha256 $sampleDf) -eq $dfSha) 'O2D9 test sample changed.'
foreach ($path in @($root,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "O2D9 test target exists: $path" }
$tokens = $null
$parserErrors = $null
[void][Management.Automation.Language.Parser]::ParseFile($endpoint, [ref]$tokens, [ref]$parserErrors)
Assert-True (@($parserErrors).Count -eq 0) 'O2D9 endpoint Windows PowerShell parser failed.'

if ($Preflight) {
    [ordered]@{schema='argos_o2d9_entrypoint_test_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D9_ENTRYPOINT_TEST_PREFLIGHT';endpointSha256=Get-Sha256 $endpoint;engineSha256=Get-Sha256 $engine;bundleSha256=Get-Sha256 $bundle;mutationsPerformed=$false;processStarted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 6
    return
}
...
try {
    ...
    $job.revision = 'OCV02_O2D9_REHEARSAL_85B91FC3'
    $job.jobId = 'O2D9_REHEARSAL_85B91FC3'
    ...
    $jobPath = Join-Path $payload 'O2D9_SLOT16_JOB.json'
    ...
    Assert-True ([string]$endpointPreflight.state -eq 'PASS_O2D9_ENDPOINT_PREFLIGHT' -and -not [bool]$endpointPreflight.processStarted -and -not [bool]$endpointPreflight.mutationsPerformed) 'O2D9 endpoint preflight changed.'
    ...
    Assert-True ([string]$normalResult.state -eq 'PASS_O2D9_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED') 'O2D9 normal rehearsal state changed.'
    Assert-True (@($normalResult.holds | Where-Object { [string]$_.code -eq 'SCRIBE_REFERENCE_COVERAGE_HOLD' }).Count -eq 1) 'O2D9 normal rehearsal coverage hold changed.'
    Assert-True (-not [bool]$normalResult.installedProposalEligibleIdentity -and [string]$normalResult.installedConsensusState -eq 'MULTIPLE_IMAGE_SUPPORTED_M12_CANDIDATES') 'O2D9 installed ambiguity preservation changed.'
    Assert-True (-not [bool]$normalResult.taskOrProcessRestarted -and -not [bool]$normalResult.providerActivated -and -not [bool]$normalResult.productionEligible) 'O2D9 normal rehearsal authority changed.'
    ...
    Assert-True ($caught -and -not (Test-Path -LiteralPath (Join-Path $failure 'w')) -and -not (Test-Path -LiteralPath (Join-Path $failure 'o'))) 'O2D9 injected source-hash failure did not fail before write.'

    $gate = [ordered]@{
        schema='argos_o2d9_entrypoint_test_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D9_ENTRYPOINT_TEST_GATE';revision='O2D9_20260826T014322494Z_85B91FC3'
    ...
    if (Test-Path -LiteralPath $root) {
        ...
        Assert-True ($resolved -eq 'C:\O2D9T_85B91FC3') 'O2D9 test cleanup root changed.'
