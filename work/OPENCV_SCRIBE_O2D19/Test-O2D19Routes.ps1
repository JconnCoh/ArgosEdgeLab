...
function Add-UniquePath([Collections.Generic.List[string]]$List, [Collections.Generic.HashSet[string]]$Set, [string]$Path) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($Path)) 'O2D19 route path is empty.'
    Assert-True ($Set.Add($Path)) "O2D19 duplicate route path: $Path"
    $List.Add($Path)
}
...
$finalGatePath = Join-Path $root 'O2D19_FINAL_PACKAGE_GATE.json'
$zipPath = Join-Path $root ('final\' + $requestReadyName + '.zip')
$extractRoot = Join-Path $root 'final\extract'
$outputPath = Join-Path $root 'O2D19_COMPLETE_ROUTE_GATE.json'
...
foreach ($dependency in @($finalGatePath,$zipPath,(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json'),(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig'),$pathTool,$packageTester,$publicCertificate,$jeoPath,$queueGatePath)) {
    Assert-True (Test-Path -LiteralPath $dependency -PathType Leaf) "O2D19 route dependency absent: $dependency"
}
Assert-True ((Get-Sha256 $finalGatePath) -eq '96158335F247702B030FACBB63785279D5D8DF9E2300E9EF1ECF8B69773AEBDC') 'O2D19 final package gate changed.'
Assert-True ((Get-Sha256 $jeoPath) -eq '91B5A9219F72845187D7CB17DAB0F1D74AF223E7DDA11E380EEA26148C60CD01') 'O2D19 direct JBOD observation changed.'
Assert-True ((Get-Sha256 $queueGatePath) -eq '170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D') 'O2D19 inherited queue-safety gate changed.'

$finalGate = Get-Content -Raw -LiteralPath $finalGatePath | ConvertFrom-Json
Assert-True ([string]$finalGate.state -eq 'PASS_O2D19_FINAL_PACKAGE_GATE' -and [string]$finalGate.requestId -eq $requestId) 'O2D19 final package identity changed.'
Assert-True (-not [bool]$finalGate.publicationAuthorized -and [bool]$finalGate.publicationRequiresCompleteRouteGate) 'O2D19 final package publication boundary changed.'
Assert-True ([bool]$finalGate.maintenanceInstalledShaMatchesPayload -and [string]$finalGate.endpointPayloadSha256 -eq '6961BC0D73216CC661BA8B5ED9FC814D56B5374AB3882DDBFDD5C1783CF8ED2D' -and [string]$finalGate.declaredInstalledSha256 -eq [string]$finalGate.endpointPayloadSha256) 'O2D19 endpoint payload and declared installed hashes diverged.'
Assert-True ([string]$finalGate.entrypointTestGateState -eq 'PASS_O2D19_ENTRYPOINT_TEST_GATE_R2' -and [string]$finalGate.entrypointTestGateSha256 -eq '6CF3D8E25B21CFE93613668733813B659083D0F56D3232ED8CAB3A72E52BEEDE') 'O2D19 entrypoint test gate changed.'
Assert-True ([string]$finalGate.selfPinGateState -eq 'PASS_O2D19_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [string]$finalGate.selfPinGateSha256 -eq 'CA309879CDBA1C71CC616DE74458A9FF3C583A45318CD9F50912FA7C60A643B5' -and [int]$finalGate.endpointSelfPinCount -eq 6 -and [int]$finalGate.endpointSelfPinMatchCount -eq 6 -and [int]$finalGate.liveAssertionBranchCaseCount -eq 3) 'O2D19 self-pin or live-branch gate changed.'
Assert-True ((Get-Sha256 $zipPath) -eq [string]$finalGate.requestZipSha256) 'O2D19 final ZIP changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json')) -eq [string]$finalGate.requestManifestSha256) 'O2D19 extracted request manifest changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig')) -eq [string]$finalGate.requestSignatureSha256) 'O2D19 extracted request signature changed.'
...
Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'O2D19 exact signed package verification failed.'

$manifest = Get-Content -Raw -LiteralPath (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json') | ConvertFrom-Json
Assert-True ([string]$manifest.requestId -eq $requestId -and [string]$manifest.targetRole -eq 'JBOD' -and [string]$manifest.jobClass -eq 'MAINTENANCE_PATCH') 'O2D19 request route changed.'
Assert-True ([int64]$manifest.maxResultBytes -eq 4194304 -and @($manifest.files).Count -eq 3) 'O2D19 request file or result bound changed.'
Assert-True ([string]$manifest.entryPoint -eq 'payload/Invoke-O2D19ScribeEndpoint.ps1' -and [string]$manifest.rehearsal.requiredState -eq 'PASS_O2D19_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED') 'O2D19 entrypoint contract changed.'
Assert-True ([string]$manifest.rehearsal.gateState -eq 'PASS_O2D19_ENTRYPOINT_TEST_GATE_R2' -and [string]$manifest.rehearsal.gateSha256 -eq '6CF3D8E25B21CFE93613668733813B659083D0F56D3232ED8CAB3A72E52BEEDE') 'O2D19 signed rehearsal declaration changed.'
Assert-True ([string]$manifest.rehearsal.selfPinGateState -eq 'PASS_O2D19_SELF_PIN_AND_LIVE_BRANCH_GATE' -and [string]$manifest.rehearsal.selfPinGateSha256 -eq 'CA309879CDBA1C71CC616DE74458A9FF3C583A45318CD9F50912FA7C60A643B5') 'O2D19 signed self-pin declaration changed.'
Assert-True (@($manifest.changes).Count -eq 1 -and [string]$manifest.changes[0].source -eq 'payload/Invoke-O2D19ScribeEndpoint.ps1' -and [string]$manifest.changes[0].installedSha256 -eq '6961BC0D73216CC661BA8B5ED9FC814D56B5374AB3882DDBFDD5C1783CF8ED2D') 'O2D19 signed maintenance source or installed hash changed.'
Assert-True ([int]$manifest.timeoutContract.endpointWorkerOuterTimeoutSeconds -eq 900 -and [int]$manifest.timeoutContract.opencvChildTimeoutSeconds -eq 600) 'O2D19 timeout contract changed.'
Assert-True (@($manifest.allowedTaskActions).Count -eq 0 -and @($manifest.allowedProcessActions).Count -eq 1) 'O2D19 action contract changed.'

$jeo = Get-Content -Raw -LiteralPath $jeoPath | ConvertFrom-Json
Assert-True ([string]$jeo.state -eq 'PASS_JEO1_DIRECT_ADMIN_READ_ONLY_OBSERVATION' -and [string]$jeo.computerName -eq 'A1025645101') 'O2D19 direct JBOD observation identity changed.'
$endpointWorkerRows = @($jeo.sourceResults | Where-Object { [string]$_.id -eq 'ENDPOINT_WORKER' })
Assert-True ($endpointWorkerRows.Count -eq 1 -and @($endpointWorkerRows[0].rows).Count -eq 1 -and [string]$endpointWorkerRows[0].rows[0].sha256 -eq 'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250') 'O2D19 observed endpoint worker pin changed.'
...
Assert-True ($endpointProcess.Count -eq 1 -and $senderProcess.Count -ge 1) 'O2D19 observed JBOD portal process evidence changed.'

$requestLeaves = @(
    'PORTAL_REQUEST_MANIFEST.json',
    'PORTAL_REQUEST_MANIFEST.sig',
    'payload\Invoke-O2D19ScribeEndpoint.ps1',
    'payload\ArgosOpenCvScribeV1R5.py',
    'payload\O2D19_SLOT21_JOB.json'
)
$responseLeaves = @(
...
foreach ($path in @(
    ...
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_O2D19.ps1',
    'D:\A2\w\ocv\O2D19_20260827T012505000Z_73C073D0.partial\refs\PORTABLE_GLYPH_REFERENCE_MANIFEST.json',
    'D:\A2\w\ocv\O2D19_20260827T012505000Z_73C073D0\ArgosOpenCvScribeV1R5.py',
    'D:\A2\w\ocv\O2D19_20260827T012505000Z_73C073D0\JOB.json',
    'D:\A2\w\ocv\O2D19_20260827T012505000Z_73C073D0\refs\glyphs_v5_confirmed_20260806\028_62630_456_SLOT21_P04_S.png',
    'D:\A2\o\ocv\O2D19_20260827T012505000Z_73C073D0\RESULT.json',
    'D:\A2\o\ocv\O2D19_20260827T012505000Z_73C073D0\RUN_GATE.json',
...
Assert-True ($paths.Count -ge 120 -and $paths.Count -le 160) "O2D19 route path cardinality changed: $($paths.Count)"

$rows = New-Object Collections.Generic.List[object]
foreach ($path in $paths) {
    $checked = & $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
    Assert-True ([string]$checked.state -eq 'PASS_PATH_BUDGET' -and @($checked.candidates).Count -eq 1) "O2D19 route path budget failed: $path"
...
Assert-True ([int]$longest.effectiveLength -lt 200 -and $maximumComponentLength -le 80) 'O2D19 complete route exceeds the safe materialized path budget.'

$state = $(if ($Preflight) { 'HOLD_O2D19_COMPLETE_ROUTE_PREFLIGHT_ARGOS_INBOUND_RELAY_UNPROVEN' } else { 'HOLD_O2D19_COMPLETE_ROUTE_GATE_ARGOS_INBOUND_RELAY_UNPROVEN' })
$result = [ordered]@{
    schema = 'argos_o2d19_complete_route_gate_v1'
    ...
    finalPackageGateSha256 = '96158335F247702B030FACBB63785279D5D8DF9E2300E9EF1ECF8B69773AEBDC'
    endpointPayloadSha256 = '6961BC0D73216CC661BA8B5ED9FC814D56B5374AB3882DDBFDD5C1783CF8ED2D'
    declaredInstalledSha256 = '6961BC0D73216CC661BA8B5ED9FC814D56B5374AB3882DDBFDD5C1783CF8ED2D'
    maintenanceInstalledShaMatchesPayload = $true
    declaredRehearsalGateState = 'PASS_O2D19_ENTRYPOINT_TEST_GATE_R2'
    declaredRehearsalGateSha256 = '6CF3D8E25B21CFE93613668733813B659083D0F56D3232ED8CAB3A72E52BEEDE'
    declaredSelfPinGateState = 'PASS_O2D19_SELF_PIN_AND_LIVE_BRANCH_GATE'
    declaredSelfPinGateSha256 = 'CA309879CDBA1C71CC616DE74458A9FF3C583A45318CD9F50912FA7C60A643B5'
...
Assert-True (-not (Test-Path -LiteralPath $outputPath)) 'O2D19 route gate output already exists.'
