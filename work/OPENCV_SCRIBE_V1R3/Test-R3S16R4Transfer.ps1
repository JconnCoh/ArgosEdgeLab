...
$manifestPath=Join-Path $PSScriptRoot 'R3S16R4_TRANSFER_MANIFEST.json'
$scriptRoot=Join-Path $PSScriptRoot 'transfer_r4'
$gatePath=Join-Path $PSScriptRoot 'R3S16R4_TRANSFER_GATE.json'
...
Assert-Exact ([string]$manifest.schema-eq'argos_r3s16r4_short_transfer_manifest_v1') 'Manifest schema changed.'
...
try{foreach($row in @($manifest.payloadParts)){[void](Assert-Script $row);$source=[IO.File]::ReadAllText((Join-Path $scriptRoot ([string]$row.script)));$match=[regex]::Match($source,"-Part '(p[0-9]{3}\.bin)' -Data '([^']+)' -ExpectedSha '([A-F0-9]{64})'");Assert-Exact $match.Success "Payload part literal absent: $($row.script)";$bytes=[Convert]::FromBase64String($match.Groups[2].Value);Assert-Exact ($match.Groups[1].Value-eq[string]$row.part-and$match.Groups[3].Value-eq[string]$row.partSha256) "Payload part identity changed: $($row.script)";Assert-Exact ($bytes.Length-eq[int]$row.partBytes-and(Get-BytesSha $bytes)-eq[string]$row.partSha256) "Payload part content changed: $($row.script)";$payloadMemory.Write($bytes,0,$bytes.Length)};$payload=$payloadMemory.ToArray()}finally{$payloadMemory.Dispose()}
