...
namespace ArgosEdgeLab.ReviewApp
{
    ...
    internal sealed class MainWindow : Form
    {
        ...
        public MainWindow(DashboardManifest dashboardManifest, string loadedManifestPath)
        {
            ...
            holdList.Columns.Add(TextColumn("Domain", 145)); holdList.Columns.Add(TextColumn("Exact hold reason", 330));
            holdList.Columns.Add(TextColumn("Detail", 430));
        ...
        {
            holdList.Rows.Clear();
            foreach (HeldAcquisition held in manifest.heldAcquisitions.OrderByDescending(item => item.scanTimestampLocal))
                holdList.Rows.Add(held.scanTimestampLocal, held.lot, held.slot, held.waferId,
                    held.domain, held.holdReason, held.holdDetail);
        }
            ...
            try
            {
                    ...
                    StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    using (MainWindow window = new MainWindow(manifest, manifestPath))
                    {
                        ...
                        if (window.HeldAcquisitionCount != manifest.heldAcquisitions.Count)
