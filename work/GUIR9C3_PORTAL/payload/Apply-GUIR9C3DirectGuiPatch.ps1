...
$targetSpecs = @(
    [pscustomobject]@{
        ...
        predecessorName = '0'
        targetSha256 = 'CF8C229A9F0EC5C26D88F800849DF96C9EC6AAA3039FE81F01971E477F4A3828'
    ...
    [pscustomobject]@{
        ...
        predecessorName = '1'
        targetSha256 = 'FD2ABADC0494C0FABA81452B18FB7D860AF7B4F32CA086018D0907E2885B1225'
        approvedPredecessors = @(
    ...
    [pscustomobject]@{
        ...
        predecessorName = '3'
        targetSha256 = 'CA69813161A862FF1623AE1989D18F8E6B1394901B2C1BAC279F97307AA07B2C'
        approvedPredecessors = @(
...
function Assert-PathBudget {
    ...
    if (($fullPath.Length + 32) -ge 200 -or $maximumComponentLength -gt 80) {
        throw "GUIR9 path budget refused: $fullPath"
    }
...
elseif ($Preflight -and -not [string]::IsNullOrWhiteSpace($InvocationManifest)) {
    ...
    $fixturePath = [Environment]::GetEnvironmentVariable('ARGOS_GUIR9_REHEARSAL_MANIFEST', 'Process')
}
...
if ($isFixture) {
    ...
    if ([string]$fixture.schema -ne 'argos_guir9_direct_patch_rehearsal_v1' -or -not [bool]$fixture.rehearsal) {
        throw 'GUIR9 rehearsal fixture changed.'
    }
...
$payloadRows = @(
    foreach ($spec in $targetSpecs) {
        ...
        if (-not (Test-Path -LiteralPath $payloadPath -PathType Leaf)) {
            throw "GUIR9 payload is missing: $($spec.payloadName)"
        }
        if ((Get-Sha256 -Path $payloadPath) -ne [string]$spec.targetSha256) {
            throw "GUIR9 payload hash changed: $($spec.payloadName)"
        }

        $destination = Assert-PathBudget -Path (Join-Path $script:InstallRoot $spec.relativePath)
        if (-not (Test-Path -LiteralPath $destination -PathType Leaf)) {
            throw "GUIR9 destination is missing: $($spec.relativePath)"
        }

        $installedSha256 = Get-Sha256 -Path $destination
        if (@($spec.approvedPredecessors) -notcontains $installedSha256) {
            throw "GUIR9 destination predecessor is unapproved: $($spec.relativePath)"
        }
...
$preflightResult = [ordered]@{
    schema = 'argos_guir9_direct_gui_patch_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_GUIR9_DIRECT_GUI_PATCH_PREFLIGHT'
    requestId = $requestId
    rehearsal = $isFixture
...
try {
    ...
    if ($isFixture -and [bool](Get-OptionalValue -Object $fixture -Name 'failAfterTargetVerification' -Default $false)) {
        throw 'INJECTED_GUIR9_POST_TARGET_FAILURE'
    }
    ...
    if (
        ...
        throw 'Healthy processor changed during GUIR9.'
    }
    ...
    foreach ($row in $priorRows) {
        if ((Get-Sha256 -Path $row.destination) -ne [string]$row.targetSha256) {
            throw "GUIR9 target changed after tray activation: $([string]$row.relativePath)"
        }
    }

    [ordered]@{
        schema = 'argos_guir9_direct_gui_patch_result_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_GUIR9_DIRECT_GUI_PATCH_AND_TRAY_ACTIVATION'
        requestId = $requestId
        rehearsal = $isFixture
        targetFiles = @(
            $priorRows | ForEach-Object {
    ...
    $failureState = if ($rollbackVerified) {
        'FAIL_GUIR9_DIRECT_GUI_PATCH_ROLLED_BACK_BEFORE_TRAY_RESTART'
    }
    else {
        'FAIL_GUIR9_DIRECT_GUI_PATCH_RECOVERY_FAILED'
    }

    [ordered]@{
        schema = 'argos_guir9_direct_gui_patch_failure_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = $failureState
        requestId = $requestId
        rehearsal = $isFixture
    ...
    if ($rollbackVerified) {
        throw "GUIR9 failed; all endpoint predecessors were restored before tray relaunch. $originalFailure"
    }

    throw "GUIR9 failed and recovery failed. Original: $originalFailure Recovery: $recoveryFailure"
}
