...
RUN_ROOT = Path(r"D:\O3F8R13T5C")
MIRROR_ROOT = Path(r"D:\KLARFExport\_ArgosReview\F8R13T5S")
...
def activate_r13_execution(l4: Any, frozen: Any) -> None:
    """Switch only the fresh child runner after historical R11 validation."""
    need(Path(l4.R11).resolve() == R13_PATH.resolve() and l4.R11_SHA256 == R13_SHA256, "L4 fresh engine is not R13")
    need(Path(frozen.R11).name == "FullPerimeterWaferTopologyOpenCvR11.py" and frozen.R11_SHA256 == R11_SHA256, "Historical engine was not R11 before activation")
    frozen.R11 = R13_PATH
    frozen.R11_SHA256 = R13_SHA256
...
def run(output_root: Path, mirror_root: Path) -> dict[str, Any]:
    ...
    selected = select_targets(context)
    targets = describe_targets(selected)
    activate_r13_execution(l4, frozen)
    output_root.mkdir()
    mirror_root.mkdir()
