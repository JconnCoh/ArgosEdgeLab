...
Assert-True ([string]$invocation.schema -eq 'argos_o2d15_response_collection_invocation_v1') 'O2D15 invocation schema changed.'
Assert-True ([string]$invocation.revision -eq $revision -and [string]$invocation.requestId -eq $requestId -and [string]$invocation.responseId -eq $responseId) 'O2D15 collection identity changed.'
Assert-True ([int64]$invocation.expectedZipBytes -eq $expectedZipBytes -and [string]$invocation.expectedZipSha256 -eq $expectedZipSha256 -and [string]$invocation.expectedResponseManifestSha256 -eq $expectedManifestSha256) 'O2D15 response pins changed.'
...
Assert-True ((Normalize-Root ([string]$psDrive.DisplayRoot)).Equals((Normalize-Root $shareRoot), [StringComparison]::OrdinalIgnoreCase)) 'O2D15 U: mapping changed.'
$sourceZip = [IO.Path]::GetFullPath([string]$invocation.sourceZip)
Assert-True ([IO.Path]::GetDirectoryName($sourceZip).Equals('U:\ProjectPortalRO\responses', [StringComparison]::OrdinalIgnoreCase)) 'O2D15 response source root changed.'
Assert-True ([IO.Path]::GetFileName($sourceZip) -eq ($responseId + '.ready.zip')) 'O2D15 response source leaf changed.'
Assert-True ((Test-Path -LiteralPath $sourceZip -PathType Leaf) -and (Get-Item -LiteralPath $sourceZip).Length -eq $expectedZipBytes -and (Get-Sha256 $sourceZip) -eq $expectedZipSha256) 'O2D15 response ZIP changed.'
...
Assert-True ($outputRoot.StartsWith($project + '\', [StringComparison]::OrdinalIgnoreCase) -and $terminalGate.StartsWith($project + '\', [StringComparison]::OrdinalIgnoreCase)) 'O2D15 collection target escaped the project.'
$localZip = Join-Path $outputRoot 'response.zip'
$partialRoot = Join-Path $outputRoot 'partial'
$readyRoot = Join-Path $outputRoot 'ready'
foreach ($fresh in @($outputRoot, $terminalGate)) { Assert-True (-not (Test-Path -LiteralPath $fresh)) "O2D15 collection target is not fresh: $fresh" }
...
foreach ($required in @($verifier, $certificate, $pathTool)) { Assert-True (Test-Path -LiteralPath $required -PathType Leaf) "O2D15 verification dependency is absent: $required" }
...
try {
    ...
    Assert-True ($entryNames.Count -eq 5 -and @(Compare-Object $expectedEntries $entryNames).Count -eq 0) 'O2D15 response entry set changed.'
...
Assert-True ((Get-BytesSha256 $manifestBytes) -eq $expectedManifestSha256) 'O2D15 manifest hash changed.'
Assert-True ((Get-BytesSha256 $resultBytes) -eq '99F41A5E892C666171984C92F0A31BCD8A23E6A96723C50A344641BEB2F5B59D') 'O2D15 maintenance result hash changed.'
Assert-True ((Get-BytesSha256 $stdoutBytes) -eq 'B5C1C24EAFB72D8B750831ADF3443BC5F5420926D45745CEC90C30C53E6DF255' -and @($stderrBytes).Count -eq 0) 'O2D15 maintenance output changed.'
...
Assert-True ([string]$manifest.schema -eq 'argos_project_portal_response_manifest_v1' -and [string]$manifest.requestId -eq $requestId -and [string]$manifest.responseId -eq $responseId) 'O2D15 signed manifest identity changed.'
Assert-True ([string]$manifest.sourceRole -eq 'JBOD' -and [string]$manifest.state -eq 'PASS_MAINTENANCE_PATCH' -and [string]$manifest.signerThumbprint -eq 'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC') 'O2D15 signed source/state/signer changed.'
Assert-True ([bool]$manifest.reviewOnly -and -not [bool]$manifest.trainingEligible -and -not [bool]$manifest.xmlEligible -and -not [bool]$manifest.productionEligible -and -not [bool]$manifest.productionRoutingEnabled -and -not [bool]$manifest.credentialsIncluded) 'O2D15 signed authority widened.'
Assert-True (@($manifest.files).Count -eq 3) 'O2D15 manifest file count changed.'
foreach ($record in @($manifest.files)) {
    $bytes = if ([string]$record.path -eq 'RESULT.json') { $resultBytes } elseif ([string]$record.path -eq 'MAINTENANCE.stdout.txt') { $stdoutBytes } elseif ([string]$record.path -eq 'MAINTENANCE.stderr.txt') { $stderrBytes } else { throw "O2D15 unexpected manifest file: $($record.path)" }
    Assert-True (@($bytes).Count -eq [int64]$record.bytes -and (Get-BytesSha256 ([byte[]]$bytes)) -eq [string]$record.sha256) "O2D15 response file changed: $($record.path)"
}
...
Assert-True ($signatureValid -and $cert.Thumbprint -eq 'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC') 'O2D15 response signature failed.'

Assert-True ([string]$maintenance.schema -eq 'argos_project_portal_maintenance_result_v1' -and [string]$maintenance.state -eq 'PASS_MAINTENANCE_PATCH' -and [int]$maintenance.changedFiles -eq 1 -and [string]$maintenance.entryPoint -eq 'payload/Invoke-O2D15ScribeEndpoint.ps1' -and [int]$maintenance.exitCode -eq 0) 'O2D15 maintenance result changed.'
Assert-True ([bool]$maintenance.reviewOnly -and -not [bool]$maintenance.productionRoutingEnabled) 'O2D15 maintenance authority widened.'
Assert-True ([string]$runGate.schema -eq 'argos_o2d15_opencv_scribe_development_gate_v1' -and [string]$runGate.state -eq 'PASS_O2D15_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED' -and [string]$runGate.revision -eq $revision -and -not [bool]$runGate.rehearsal) 'O2D15 run identity changed.'
Assert-True ([string]$runGate.engineSha256 -eq '8A6DE04B7DD08EFA717AF606FD0D04622ABE84C753B690C4590B0E95D8B31BAB' -and [string]$runGate.jobSha256 -eq 'B0C4C36354A222F38A57A5A581213B5687578D44E91FDBA1F9AB2DA55FE105AB' -and [string]$runGate.referenceBundleSha256 -eq '56DF00E37A195E7BC3E026E4950DFB5A0AA7E7AF49A6FF39020B071840CCFBD6') 'O2D15 dependency binding changed.'
Assert-True ([string]$runGate.resultSha256 -eq 'C20B7F9196F0CA4F010CBAEC14E253F5DDE90CBCA631D613019114AE151D67E1' -and [string]$runGate.resultState -eq 'SCRIBE_M12_AMBIGUOUS_MULTIPLE_VALID') 'O2D15 result state changed.'
Assert-True ([string]$runGate.imageFirstString -eq '1443R071SUF5' -and [string]$runGate.proposedString -eq '1443R071SUF5' -and [string]$runGate.checksumState -eq 'SCRIBE_M12_IMAGE_FIRST_CHECKSUM_VALID_REVIEW_ONLY') 'O2D15 result proposal changed.'
Assert-True (@($runGate.candidates).Count -eq 7 -and @($runGate.holds).Count -eq 2 -and (@($runGate.holds.code) -contains 'SCRIBE_M12_AMBIGUOUS_MULTIPLE_VALID') -and (@($runGate.holds.code) -contains 'SCRIBE_REFERENCE_COVERAGE_HOLD')) 'O2D15 candidates/holds changed.'
Assert-True ([int]$runGate.localization.qualifiedInstalledDetectorInputCount -eq 2 -and [string]$runGate.localization.selectedRegionSource -eq 'HASH_PINNED_INSTALLED_REVIEW_ONLY_PROCESSOR_OUTPUT' -and -not [bool]$runGate.installedProposalEligibleIdentity -and [string]$runGate.installedConsensusState -eq 'MULTIPLE_IMAGE_SUPPORTED_M12_CANDIDATES') 'O2D15 installed-proposal evidence changed.'
Assert-True (-not [bool]$runGate.taskOrProcessRestarted -and -not [bool]$runGate.sourceMutationPerformed -and -not [bool]$runGate.sourceDeletionPerformed -and -not [bool]$runGate.waferActionPerformed -and -not [bool]$runGate.holdsCleared -and -not [bool]$runGate.providerActivated -and [bool]$runGate.reviewOnly -and -not [bool]$runGate.productionEligible) 'O2D15 protected invariants changed.'
