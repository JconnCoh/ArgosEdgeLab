...
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 16) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "O2D15 test create-new JSON exists: $Path"
...
$endpoint = Join-Path $PSScriptRoot 'Invoke-O2D15ScribeEndpoint.ps1'
$engine = Join-Path $project 'work\OPENCV_SCRIBE_V1R5\ArgosOpenCvScribeV1R5.py'
$liveJob = Join-Path $PSScriptRoot 'O2D15_SLOT19_JOB.json'
...
foreach ($path in @($endpoint,$engine,$liveJob,$bundle,(Join-Path $runtime 'python.exe'),$sampleBf,$sampleDf)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "O2D15 test dependency absent: $path"
}
Assert-True ((Get-Sha256 $endpoint) -eq '748C9DCB2E20F653A367D091A588BDA3AC93CCCE653F20F3D0C047D0580E8F61') 'O2D15 endpoint changed.'
Assert-True ((Get-Sha256 $engine) -eq 'F61F5954A77E6F730A2BF0D110A468535C4595D25DB21AFFE1573EF08B8139AB') 'O2D15 test engine changed.'
Assert-True ((Get-Sha256 $bundle) -eq '56DF00E37A195E7BC3E026E4950DFB5A0AA7E7AF49A6FF39020B071840CCFBD6') 'O2D15 test bundle changed.'
Assert-True ((Get-Sha256 $sampleBf) -eq $bfSha -and (Get-Sha256 $sampleDf) -eq $dfSha) 'O2D15 test sample changed.'
foreach ($path in @($root,$gatePath)) { Assert-True (-not (Test-Path -LiteralPath $path)) "O2D15 test target exists: $path" }
$tokens = $null
$parserErrors = $null
[void][Management.Automation.Language.Parser]::ParseFile($endpoint, [ref]$tokens, [ref]$parserErrors)
Assert-True (@($parserErrors).Count -eq 0) 'O2D15 endpoint Windows PowerShell parser failed.'

if ($Preflight) {
    [ordered]@{schema='argos_o2d15_entrypoint_test_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D15_ENTRYPOINT_TEST_PREFLIGHT';endpointSha256=Get-Sha256 $endpoint;engineSha256=Get-Sha256 $engine;bundleSha256=Get-Sha256 $bundle;mutationsPerformed=$false;processStarted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 6
    return
}
...
try {
    ...
    $job.revision = 'OCV02_O2D15_REHEARSAL_9A8661E9'
    $job.jobId = 'O2D15_REHEARSAL_9A8661E9'
    ...
    $jobPath = Join-Path $payload 'O2D15_SLOT19_JOB.json'
    ...
    Assert-True ([string]$endpointPreflight.state -eq 'PASS_O2D15_ENDPOINT_PREFLIGHT' -and -not [bool]$endpointPreflight.processStarted -and -not [bool]$endpointPreflight.mutationsPerformed) 'O2D15 endpoint preflight changed.'
    ...
    Assert-True ([string]$normalResult.state -eq 'PASS_O2D15_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED') 'O2D15 normal rehearsal state changed.'
    Assert-True (-not [string]::IsNullOrWhiteSpace([string]$normalResult.imageFirstString) -and [string]$normalResult.checksumState -ne 'NOT_EVALUATED' -and [int]$normalResult.localization.autoLocalizedPromotedCandidateCount -gt 0) 'O2D15 normal rehearsal did not evaluate an auto-localized scribe.'
    Assert-True (@($normalResult.holds | Where-Object { [string]$_.code -eq 'SCRIBE_REFERENCE_COVERAGE_HOLD' }).Count -eq 1) 'O2D15 normal rehearsal coverage hold changed.'
    Assert-True ([bool]$normalResult.upstreamNotchHoldDidNotSkipScribe -and [string]$normalResult.upstreamProposalState -eq 'SCRIBE_IDENTITY_CONFIRMATION_HOLD') 'O2D15 upstream-hold boundary changed.'
    Assert-True ([bool]$normalResult.sourceAliasRemoved -and -not (Test-Path -LiteralPath 'X:\')) 'O2D15 source alias was not removed.'
    Assert-True (-not [bool]$normalResult.taskOrProcessRestarted -and -not [bool]$normalResult.providerActivated -and -not [bool]$normalResult.productionEligible) 'O2D15 normal rehearsal authority changed.'
    ...
    Assert-True ($caught -and -not (Test-Path -LiteralPath (Join-Path $failure 'w')) -and -not (Test-Path -LiteralPath (Join-Path $failure 'o'))) 'O2D15 injected source-hash failure did not fail before write.'

    $gate = [ordered]@{
        schema='argos_o2d15_entrypoint_test_gate_v3';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_O2D15_ENTRYPOINT_TEST_GATE_R3';revision='O2D15_20260826T225708001Z_9A8661E9'
    ...
    if (Test-Path -LiteralPath $root) {
        ...
        Assert-True ($resolved -eq 'C:\O2D15T_9A8661E9') 'O2D15 test cleanup root changed.'
