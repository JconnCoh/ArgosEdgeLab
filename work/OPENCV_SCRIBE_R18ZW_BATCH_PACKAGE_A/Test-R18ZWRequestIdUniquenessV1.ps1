...
$requestId = 'REQ_RZW1'
$bindingRecordSha256 = 'DE0B07B9DA8700A61CAF52D9786210ECDA6E363292D17922D5303AD69FAEE89A'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_r18zw_request_id_uniqueness_preflight_v1'
        checkedUtc=[DateTime]::UtcNow.ToString('o')
        state='PASS_R18ZW_REQUEST_ID_UNIQUENESS_PREFLIGHT'
        requestId=$requestId
...
$preSignatureOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZW_REQUEST_ID_COLLISION_GATE.json'))
$prePublicationOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZW_P8_COLLISION.json'))
Assert-True (
...
$result = [ordered]@{
    schema='argos_r18zw_request_id_uniqueness_gate_v1'
    checkedUtc=[DateTime]::UtcNow.ToString('o')
    state=$(if($scanPassed){'PASS_R18ZW_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS'}else{'FAIL_R18ZW_REQUEST_ID_COLLISION_OR_SCAN_ERROR'})
    requestId=$requestId
    bindingRecordSha256=$bindingRecordSha256
    scannerSha256=$scannerSha256
