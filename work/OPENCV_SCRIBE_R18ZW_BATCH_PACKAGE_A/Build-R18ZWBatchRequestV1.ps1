...
function Assert-ExactLauncherChange([object[]]$Changes, [string]$LauncherSha256, [string]$Label) {
    ...
    Require ([string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZW.ps1') "$Label launcher destination changed."
...
$requestId = 'REQ_RZW1'
$revision = 'R18ZW_EXISTING_ORIENTED_CROPS_PORTAL_DETACHED_20260912A'
$runnerRevision = 'R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260908D'
$bindingPath = Join-Path $PSScriptRoot 'R18ZW_FROZEN_PACKAGE_BINDINGS_V1.json'
$launcherPath = Join-Path $PSScriptRoot 'Invoke-R18ZWBatchLaunchV1.ps1'
$payloadManifestPath = Join-Path $PSScriptRoot 'R18ZW_PAYLOAD_MANIFEST.json'
$definitionPath = Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION_R18ZW.json'
$configurationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A\R18ZW_BATCH_CONFIGURATION.json'
$designPath = Join-Path $PSScriptRoot 'R18ZW_BATCH_PACKAGE_DESIGN_V1.json'
$importSmokePath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B\Test-R18ZTPackagedImportClosure.py'
$payloadCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZW_CLONE_LITERAL_REMEDIATION.json'
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZW_CLONE_LITERAL_REMEDIATION.json'
$publisherCloneRemediationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A\R18ZW_CLONE_LITERAL_REMEDIATION.json'
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZW_CLONE_RAW_GATE_V3.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZW_CLONE_GATE_V2.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZW_PATH_PLAN_GATE.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZW_POWERSHELL_SAFETY_GATE_V2.json'
$scienceGatePath = Join-Path $PSScriptRoot 'R18ZW_SCIENCE_GATE.json'
$junctionGatePath = Join-Path $PSScriptRoot 'R18ZW_PORTAL_DETACH_GATE.json'
$preactionPath = Join-Path $PSScriptRoot 'R18ZW_PREACTION_GATE_V2.json'
$collisionGatePath = Join-Path $PSScriptRoot 'R18ZW_REQUEST_ID_COLLISION_GATE.json'
$collisionScannerPath = Join-Path $PSScriptRoot 'Test-R18ZWRequestIdUniquenessV1.ps1'
...
$testRoot = 'C:\RZW1A\T'
$testWorkRoot = 'C:\RZW1A\TW'
$testOutputRoot = 'C:\RZW1A\TO'
$stageRoot = 'C:\RZW1A\P'
$readyRoot = Join-Path $stageRoot ($requestId + '.ready')
$stageZip = Join-Path $stageRoot ($requestId + '.ready.zip')
$verifyRoot = 'C:\RZW1A\V'
$finalRoot = Join-Path $PSScriptRoot 'final'
$finalPartial = Join-Path $PSScriptRoot 'final.partial'
$zipName = $requestId + '.ready.zip'
$finalGatePath = Join-Path $PSScriptRoot 'R18ZW_FINAL_PACKAGE_GATE.json'
$productionWorkRoot = 'D:\A2\w\ocv\R18ZW1'
$productionOutputRoot = 'D:\A2\o\ocv\R18ZW1'
$productionProposalAlias = 'D:\A2\w\ocv\R18ZW1\p'
$canonicalProposalRoot = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals'
$productionInstalledLauncher = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZW.ps1'
$bindingSha = 'DE0B07B9DA8700A61CAF52D9786210ECDA6E363292D17922D5303AD69FAEE89A'

Require-Pin $bindingPath $bindingSha -1 'FROZEN_R18ZW_PACKAGE_A_V1_BINDINGS'
$binding = Get-Content -LiteralPath $bindingPath -Raw | ConvertFrom-Json
Require ([string]$binding.schema -eq 'argos_opencv_scribe_r18zw_package_bindings_v1' -and [string]$binding.requestId -eq $requestId -and [string]$binding.packageRevision -eq $revision -and [string]$binding.runnerRevision -eq $runnerRevision) 'R18ZW binding changed.'
...
Require ([string]$boundLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$boundLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZW1\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$boundLongestOutput.basePathLength -eq 62 -and [int]$boundLongestOutput.suffixReserveCharacters -eq 52 -and [int]$boundLongestOutput.effectivePathLength -eq 114 -and [int]$boundLongestOutput.effectiveComponentLength -eq 68) 'R18ZT exact longest atomic case-output binding changed.'
$boundConservativeTemp = $binding.pathContract.launcherConservativeProviderTempBudget
Require ([string]$boundConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$boundConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZW1\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$boundConservativeTemp.basePathLength -eq 74 -and [int]$boundConservativeTemp.baseComponentLength -eq 28 -and [int]$boundConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$boundConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$boundConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$boundConservativeTemp.trueAtomicWriteTarget) 'R18ZT conservative provider-temp launcher budget changed.'
...
$requiredArtifactNames = @(
    ...
    'heldOutZDiagnosticGate','r18zuReviewBatchScienceGate','approvedSourceGuard','r18zv3TerminalPredecessorGate','r18zv3SignedLaunchResponseGate','r18zv3BatchCompletionObservation','r18zuProvider','r18zv1DevelopmentFreeze','r18zv1Slot21Job','r18zv1Slot21Result','r18zv1Slot21CompactGate','r18zv1Slot21CorrectionGate','collectorFailure','collectorRemediationBinding','packageAPreflightWithdrawal','batchAdapter','adapterQualificationGate','r18zv1Provider','r18zwDevelopmentFreeze','r18zwPairedManifest','r18zwBfReference','r18zwDfReference','r18zwFocusedGate','portalDetachGate'
)
...
foreach ($row in $artifactRows) {
    ...
    $embeddedState = [string]$row.state
    if ([string]$row.name -ceq 'batchConfiguration') {
        Require ($embeddedState -ceq 'FROZEN_UNPUBLISHED') 'R18ZW batch-configuration lifecycle annotation changed.'
        $embeddedState = ''
    }
    Require-Pin $path ([string]$row.sha256) ([int64]$row.bytes) $embeddedState
    $resolvedArtifacts[[string]$row.name] = $path
}
Require ($resolvedArtifacts['launcher'] -eq [IO.Path]::GetFullPath($launcherPath) -and $resolvedArtifacts['payloadManifest'] -eq [IO.Path]::GetFullPath($payloadManifestPath) -and $resolvedArtifacts['maintenanceDefinition'] -eq [IO.Path]::GetFullPath($definitionPath) -and $resolvedArtifacts['batchConfiguration'] -eq [IO.Path]::GetFullPath($configurationPath) -and $resolvedArtifacts['packageDesign'] -eq [IO.Path]::GetFullPath($designPath) -and $resolvedArtifacts['packageImportSmokeTest'] -eq [IO.Path]::GetFullPath($importSmokePath) -and $resolvedArtifacts['executionEnvelope'] -eq [IO.Path]::GetFullPath((Join-Path $project 'work\OPENCV_SCRIBE_R18ZV3_BATCH_PACKAGE_A\Run-R18ZV3BatchExecutionEnvelopeV1.py')) -and $resolvedArtifacts['parallelCoordinator'] -eq [IO.Path]::GetFullPath((Join-Path $project 'work\OPENCV_SCRIBE_R18ZV3\r18zv3_parallel_batch.py')) -and -not [bool](@($artifactRows|Where-Object name -eq 'packageImportSmokeTest')[0].executedForCurrentPackage)) 'R18ZW core binding changed.'
Require ($resolvedArtifacts['batchAdapter'] -eq [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'ArgosOpenCvScribeV1R18ZWBatchAdapter.py')) -and $resolvedArtifacts['adapterQualificationGate'] -eq [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZW_ADAPTER_QUALIFICATION_GATE.json')) -and $resolvedArtifacts['provider'] -eq [IO.Path]::GetFullPath((Join-Path $project 'work\OPENCV_SCRIBE_R18ZW\ArgosOpenCvScribeV1R18ZW.py'))) 'R18ZW batch-adapter binding changed.'
...
$cloneGate = Read-BoundGate $cloneGatePath 'PASS_R18ZW_PACKAGE_A_CLONE_LITERAL_GATE'
$pathGate = Read-BoundGate $pathGatePath 'PASS_R18ZW_PACKAGE_A_PATH_BUDGET'
$powerShellGate = Read-BoundGate $powerShellGatePath 'PASS_R18ZW_PACKAGE_A_POWERSHELL_SAFETY'
$scienceGate = Read-BoundGate $scienceGatePath 'PASS_R18ZW_BATCH_SCIENCE_AND_ADAPTER_EQUIVALENCE_GATE'
$junctionGate = Get-Content -LiteralPath $junctionGatePath -Raw | ConvertFrom-Json
$preaction = Read-BoundGate $preactionPath 'PASS_PREACTION_CONTRACT'
$collisionGate = $null
if ($Build) { $collisionGate = Read-BoundGate $collisionGatePath 'PASS_R18ZW_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS' }

$builderPath = [IO.Path]::GetFullPath($MyInvocation.MyCommand.Path)
$builderSha = Get-Sha256 $builderPath
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Build-R18ZWBatchRequestV1.ps1' = $builderSha
    'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Collect-R18ZWLaunchResponseV1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZWLaunchResponseV1.ps1')
    'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Invoke-R18ZWBatchLaunchV1.ps1' = Get-Sha256 $launcherPath
    'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Publish-R18ZWV1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZWV1.ps1')
    'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Test-R18ZWRequestIdUniquenessV1.ps1' = Get-Sha256 $collisionScannerPath
    'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/ArgosOpenCvScribeV1R18ZWBatchAdapter.py' = Get-Sha256 (Join-Path $PSScriptRoot 'ArgosOpenCvScribeV1R18ZWBatchAdapter.py')
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/R18ZW_CLONE_RAW_GATE_V3.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @(
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Invoke-R18ZWBatchLaunchV1.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Build-R18ZWBatchRequestV1.ps1';sha=$builderSha},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Publish-R18ZWV1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZWV1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Collect-R18ZWLaunchResponseV1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZWLaunchResponseV1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Test-R18ZWRequestIdUniquenessV1.ps1';sha=(Get-Sha256 $collisionScannerPath)}
)) {
...
Require ([string]$junctionGate.state -eq 'PASS_R18ZW_PARALLEL_ENVELOPE_RESOURCE_PRESERVATION_AND_PORTAL_DETACH_GATE' -and [bool]$junctionGate.activeEnvelopePreservedWithoutParentOnlyKill -and [bool]$junctionGate.aliasAndWorkPreservedWhileEnvelopeActive -and [int]$junctionGate.maximumDelegateChildProcessCount -eq 8 -and [bool]$junctionGate.delegateSubprocessAllowed -and [bool]$junctionGate.portalCaptureReleasedBeforeWorkerExit -and [bool]$junctionGate.productionLaunchUsesShellExecute -and [bool]$junctionGate.productionLaunchWindowHidden -and -not [bool]$junctionGate.longLivedChildInheritedEndpointPipes -and [string]$junctionGate.launcherSha256 -eq [string]($artifactRows | Where-Object name -eq 'launcher').sha256) 'R18ZW parallel resource and portal-detach gate changed.'
Require ([int]$scienceGate.focusedCaseCount -eq 7 -and [int]$scienceGate.focusedExactCount -eq 7 -and [int]$scienceGate.providerRunCount -eq 7 -and [int]$scienceGate.normalHypothesisAttemptCount -eq 56 -and [int]$scienceGate.wrongPairAdmissionCount -eq 0 -and [bool]$scienceGate.runtimeRestorationPassed -and -not [bool]$scienceGate.truthAvailableToProvider -and -not [bool]$scienceGate.identityAccepted -and -not [bool]$scienceGate.publicationAuthorized -and ([string[]]$scienceGate.exactImageFirstStrings -join '|') -ceq '13HFX135SUE3|1471D081SUE4|148A2106SUC4|147SL145SUE3|148AW103SUD5|K9663019FEB7|L0430010FEB1') 'R18ZW focused science changed.'
foreach ($sciencePin in @('provider','batchAdapter','adapterQualificationGate','r18zwDevelopmentFreeze','r18zwPairedManifest','r18zwBfReference','r18zwDfReference','r18zwFocusedGate','batchRunner','parallelCoordinator','executionEnvelope','executionEnvelopeTest','parallelQualificationGate','runnerNonImageTestGate','r18ztProvider','r18zuProvider','r18zv1Provider')) {
...
$predecessor = Get-Content -LiteralPath $resolvedArtifacts['r18zv3TerminalPredecessorGate'] -Raw | ConvertFrom-Json
Require ([string]$predecessor.state -ceq 'PASS_R18ZV3_ENDPOINT_TERMINAL_AND_BATCH_OUTPUT_COMPLETE' -and [string]$predecessor.predecessorRequestId -ceq 'REQ_RZV3' -and [bool]$predecessor.predecessorEndpointTransactionTerminal -and [bool]$predecessor.predecessorBatchOutputTerminal -and [int]$predecessor.predecessorBatchCompletionEvidence.qualifiedCases -eq 1044 -and [int]$predecessor.predecessorBatchCompletionEvidence.completedCases -eq 1044 -and -not [bool]$predecessor.evidenceSemantics.signedCorpusCompletionClaimed -and -not [bool]$predecessor.evidenceSemantics.predecessorWorkerExitAsserted -and [bool]$predecessor.freshDirectQueueAndEndpointHealthAuditRequiredBeforePublication -and -not [bool]$predecessor.publicationAuthorizedByThisGate) 'R18ZV3 terminal predecessor boundary changed.'
$adapterGate = Get-Content -LiteralPath $resolvedArtifacts['adapterQualificationGate'] -Raw | ConvertFrom-Json
Require ([string]$adapterGate.state -ceq 'PASS_R18ZW_BATCH_ADAPTER_EXACT_SEAM_AND_RESOURCE_CAP' -and [string]$adapterGate.requestId -ceq 'REQ_RZW1' -and
    ...
    -not [bool]$adapterGate.behavioralBoundary.envelopeLogicChanged -and -not [bool]$adapterGate.behavioralBoundary.resultLogicChanged -and
    -not [bool]$adapterGate.behavioralBoundary.directStructuralEvaluatorCalled -and [int]$adapterGate.resourceContract.workerCount -eq 8 -and [int]$adapterGate.resourceContract.openCvThreadsPerWorker -eq 1 -and [bool]$adapterGate.resourceContract.openClGpuDisabled -and
    -not [bool]$adapterGate.tests.imageBytesRead -and -not [bool]$adapterGate.tests.providerRunPerformed) 'R18ZW adapter qualification changed.'
...
Require ([string]$pathLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$pathLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZW1\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$pathLongestOutput.basePathLength -eq 62 -and [int]$pathLongestOutput.suffixReserveCharacters -eq 52 -and [int]$pathLongestOutput.effectivePathLength -eq 114 -and [int]$pathLongestOutput.effectiveComponentLength -eq 68) 'R18ZT path-gate true atomic longest-output fact changed.'
$pathConservativeTemp = $pathGate.launcherConservativeProviderTempBudget
Require ([string]$pathConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$pathConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZW1\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$pathConservativeTemp.basePathLength -eq 74 -and [int]$pathConservativeTemp.baseComponentLength -eq 28 -and [int]$pathConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$pathConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$pathConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$pathConservativeTemp.trueAtomicWriteTarget) 'R18ZT path-gate conservative provider-temp fact changed.'
...
$bindingDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/R18ZW_FROZEN_PACKAGE_BINDINGS_V1.json' })
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/Build-R18ZWBatchRequestV1.ps1' })
...
if ($Build) {
    Require ([string]$collisionGate.schema -ceq 'argos_r18zw_request_id_uniqueness_gate_v1' -and [string]$collisionGate.requestId -ceq $requestId -and [string]$collisionGate.scannerSha256 -ceq (Get-Sha256 $collisionScannerPath) -and [string]$collisionGate.scanPurpose -ceq 'PRE_SIGNATURE') 'R18ZW collision gate changed.'
...
foreach ($outputPin in @(
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZW1\LAUNCH.json';schema='argos_opencv_scribe_r18zt_batch_launch_v2'},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZW1\RUNNING.json';schema=[string]$binding.outputSchemas.progress},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZW1\COMPLETE.json';schema=[string]$binding.outputSchemas.complete},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZW1\STATUS.json';schema=[string]$binding.outputSchemas.status}
)) {
...
$runningOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZW1\RUNNING.json' })[0]
...
$failureOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZW1\FAILURE.json' })[0]
...
$expectedLeafSetSha = '246426CFE75BEDC333E2FBF62C53322B44A6BE40C32207EFB590C1C39F5CD210'
...
if ($Preflight) {
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zw_batch_build_preflight_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZW_PACKAGE_A_BUILD_PREFLIGHT'
        requestId = $requestId
...
$stagedPreflight = (& (Join-Path $activePayload 'Invoke-R18ZTBatchLaunch.ps1') -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $activePayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\RZW1A\PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
...
$stagedPackageHashClosure = [ordered]@{
    state = 'PASS_R18ZW_STAGED_PACKAGED_HASH_CLOSURE_NO_IMAGE'
...
if ($Test) {
    ...
    Require ([string]$stagedJunctionGate.state -eq 'PASS_R18ZW_PARALLEL_ENVELOPE_RESOURCE_PRESERVATION_AND_PORTAL_DETACH_GATE' -and [string]$stagedJunctionGate.launcherSha256 -eq $launcherSha -and [bool]$stagedJunctionGate.activeEnvelopeHoldCasePassed -and [bool]$stagedJunctionGate.fastCompleteReportingFailureCasePassed -and [bool]$stagedJunctionGate.invalidCompletePreservationCasePassed -and [bool]$stagedJunctionGate.preexistingRootCollisionRejectedBeforeProcess -and [bool]$stagedJunctionGate.secondFreshControlPassed -and [int]$stagedJunctionGate.completeFailureCoexistenceCount -eq 0 -and [int]$stagedJunctionGate.maximumDelegateChildProcessCount -eq 8 -and [bool]$stagedJunctionGate.delegateSubprocessAllowed -and [bool]$stagedJunctionGate.portalCaptureReleasedBeforeWorkerExit -and [bool]$stagedJunctionGate.productionLaunchUsesShellExecute -and [bool]$stagedJunctionGate.productionLaunchWindowHidden -and -not [bool]$stagedJunctionGate.longLivedChildInheritedEndpointPipes -and -not [bool]$stagedJunctionGate.residualTestRootPresent) 'R18ZW staged production launcher gate failed.'
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zw_batch_unsigned_test_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZW_PACKAGE_A_UNSIGNED_TEST'
        requestId = $requestId
        bindingRecordSha256 = $bindingSha
...
$extractedProvider = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZW\ArgosOpenCvScribeV1R18ZWBatchAdapter.py'
$extractedWrappedProvider = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZW\ArgosOpenCvScribeV1R18ZW.py'
...
$routeGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zw_complete_route_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZW_COMPLETE_ROUTE_GATE_SIGNED_UNPUBLISHED'
    requestId = $requestId
    bindingRecordSha256 = $bindingSha
...
$finalGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zw_final_package_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZW_SIGNED_UNPUBLISHED_PACKAGE_GATE'
    requestId = $requestId
    revision = $revision
    bindingRecordSha256 = $bindingSha
    requestZip = 'work/OPENCV_SCRIBE_R18ZW_BATCH_PACKAGE_A/final/REQ_RZW1.ready.zip'
    ...
    proposalJunctionGateSha256 = Get-Sha256 $junctionGatePath
    r18zv3TerminalPredecessorGateSha256 = Get-Sha256 $resolvedArtifacts['r18zv3TerminalPredecessorGate']
