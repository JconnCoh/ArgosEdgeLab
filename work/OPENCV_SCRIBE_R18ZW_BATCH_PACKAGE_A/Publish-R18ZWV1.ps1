...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zw_publication_invocation_v1') 'R18ZW invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZW_PUBLICATION_INVOCATION_V1') 'R18ZW invocation is not frozen.'
Require ([string]$invocation.requestId -ceq 'REQ_RZW1') 'R18ZT publication request identity changed.'
...
Require ([string]$invocation.packageBinding.expectedSchema -ceq 'argos_opencv_scribe_r18zw_package_bindings_v1') 'Fresh R18ZW package binding schema declaration changed.'
Require ([string]$binding.schema -ceq [string]$invocation.packageBinding.expectedSchema) 'Fresh package binding schema changed.'
Require ([string]$binding.state -ceq [string]$invocation.packageBinding.expectedState -and [string]$binding.requestId -ceq 'REQ_RZW1') 'Fresh package binding identity/state changed.'
...
foreach ($consumer in @($invocation.bindingConsumers)) {
    ...
    Require ([string](Get-NestedValue $consumerObject ([string]$consumer.requestIdProperty) "Binding consumer $($consumer.dependency)") -ceq 'REQ_RZW1') "Binding consumer request ID changed: $($consumer.dependency)"
}
...
Require ([string](Get-NestedValue $scienceGate ([string]$scienceContract.imageFirstStringProperty) 'R18ZW science gate') -ceq '13HFX135SUE3' -and
    [bool](Get-NestedValue $scienceGate ([string]$scienceContract.exactFinishPassedProperty) 'R18ZW science gate') -and
    [int](Get-NestedValue $scienceGate ([string]$scienceContract.wrongAcceptedCountProperty) 'R18ZW science gate') -eq 0 -and
    -not [bool](Get-NestedValue $scienceGate ([string]$scienceContract.providerRerunProperty) 'R18ZW science gate')) 'R18ZW science gate semantic contract changed.'
Require ([int]$scienceGate.focusedCaseCount -eq 7 -and [int]$scienceGate.focusedExactCount -eq 7 -and
    [int]$scienceGate.providerRunCount -eq 7 -and [int]$scienceGate.normalHypothesisAttemptCount -eq 56 -and
    [int]$scienceGate.wrongPairAdmissionCount -eq 0 -and [bool]$scienceGate.runtimeRestorationPassed -and
    -not [bool]$scienceGate.truthAvailableToProvider -and -not [bool]$scienceGate.identityAccepted -and
    ([string[]]$scienceGate.exactImageFirstStrings -join '|') -ceq '13HFX135SUE3|1471D081SUE4|148A2106SUC4|147SL145SUE3|148AW103SUD5|K9663019FEB7|L0430010FEB1') 'R18ZW seven-case science boundary changed.'
foreach($name in @('provider','r18zwDevelopmentFreeze','r18zwPairedManifest','r18zwBfReference','r18zwDfReference','r18zwFocusedGate')){$d=Get-ValidatedDependency $name;$br=@($binding.artifacts|Where-Object name -ceq $name);$sr=@($scienceGate.artifacts|Where-Object name -ceq $name);Require($br.Count -eq 1 -and $sr.Count -eq 1 -and [string]$br[0].sha256 -ceq [string]$d.sha256 -and [string]$sr[0].sha256 -ceq [string]$d.sha256) "R18ZW science pin changed: $name"}

$batchAdapterDependency = Get-ValidatedDependency 'batchAdapter'
$adapterQualificationDependency = Get-ValidatedDependency 'adapterQualificationGate'
$adapterQualification = Read-JsonFile ([string]$adapterQualificationDependency.path) 1048576
$bindingAdapterRows = @($binding.artifacts | Where-Object { [string]$_.name -ceq 'batchAdapter' })
$scienceAdapterRows = @($scienceGate.artifacts | Where-Object { [string]$_.name -ceq 'batchAdapter' })
$bindingAdapterGateRows = @($binding.artifacts | Where-Object { [string]$_.name -ceq 'adapterQualificationGate' })
$scienceAdapterGateRows = @($scienceGate.artifacts | Where-Object { [string]$_.name -ceq 'adapterQualificationGate' })
Require ($bindingAdapterRows.Count -eq 1 -and $scienceAdapterRows.Count -eq 1 -and
    [string]$bindingAdapterRows[0].sha256 -ceq [string]$batchAdapterDependency.sha256 -and
    [string]$scienceAdapterRows[0].sha256 -ceq [string]$batchAdapterDependency.sha256 -and
    $bindingAdapterGateRows.Count -eq 1 -and $scienceAdapterGateRows.Count -eq 1 -and
    [string]$bindingAdapterGateRows[0].sha256 -ceq [string]$adapterQualificationDependency.sha256 -and
    [string]$scienceAdapterGateRows[0].sha256 -ceq [string]$adapterQualificationDependency.sha256) 'R18ZW adapter dependency pins changed.'
Require ([string]$adapterQualification.state -ceq 'PASS_R18ZW_BATCH_ADAPTER_EXACT_SEAM_AND_RESOURCE_CAP' -and
    [bool]$adapterQualification.entryProviderTranslation.runJobIdentityPreserved -and
    [bool]$adapterQualification.entryProviderTranslation.r18zvIdentityPreserved -and
    [bool]$adapterQualification.entryProviderTranslation.r18hIdentityPreserved -and
    -not [bool]$adapterQualification.behavioralBoundary.rankingLogicChanged -and
    -not [bool]$adapterQualification.behavioralBoundary.envelopeLogicChanged -and
    -not [bool]$adapterQualification.behavioralBoundary.resultLogicChanged -and
    -not [bool]$adapterQualification.behavioralBoundary.directStructuralEvaluatorCalled -and
    [int]$adapterQualification.resourceContract.workerCount -eq 8 -and
    [int]$adapterQualification.resourceContract.openCvThreadsPerWorker -eq 1 -and
    [bool]$adapterQualification.resourceContract.openClGpuDisabled -and
    -not [bool]$adapterQualification.tests.imageBytesRead -and
    -not [bool]$adapterQualification.tests.providerRunPerformed) 'R18ZW batch adapter qualification changed.'
$predecessorDependency = Get-ValidatedDependency 'r18zv3TerminalPredecessorGate'
$predecessorGate = Read-JsonFile ([string]$predecessorDependency.path) 1048576
Require ([string]$predecessorGate.state -ceq 'PASS_R18ZV3_ENDPOINT_TERMINAL_AND_BATCH_OUTPUT_COMPLETE' -and
    [string]$predecessorGate.predecessorRequestId -ceq 'REQ_RZV3' -and
    [bool]$predecessorGate.predecessorEndpointTransactionTerminal -and
    [bool]$predecessorGate.predecessorBatchOutputTerminal -and
    [int]$predecessorGate.predecessorBatchCompletionEvidence.qualifiedCases -eq 1044 -and
    [int]$predecessorGate.predecessorBatchCompletionEvidence.completedCases -eq 1044 -and
    -not [bool]$predecessorGate.evidenceSemantics.signedCorpusCompletionClaimed -and
    -not [bool]$predecessorGate.evidenceSemantics.predecessorWorkerExitAsserted -and
    [bool]$predecessorGate.freshDirectQueueAndEndpointHealthAuditRequiredBeforePublication -and
    -not [bool]$predecessorGate.publicationAuthorizedByThisGate) 'R18ZV3 terminal predecessor boundary changed.'
$detachDependency = Get-ValidatedDependency 'portalDetachGate'
$detachGate = Read-JsonFile ([string]$detachDependency.path) 1048576
Require ([string]$detachGate.state -ceq 'PASS_R18ZW_PARALLEL_ENVELOPE_RESOURCE_PRESERVATION_AND_PORTAL_DETACH_GATE' -and
    [string]$detachGate.launcherSha256 -ceq [string](Get-ValidatedDependency 'launcher').sha256 -and
    [bool]$detachGate.portalCaptureReleasedBeforeWorkerExit -and [bool]$detachGate.productionLaunchUsesShellExecute -and
    [bool]$detachGate.productionLaunchWindowHidden -and -not [bool]$detachGate.longLivedChildInheritedEndpointPipes) 'R18ZW portal-detach gate changed.'
$healthDependency = Get-ValidatedDependency 'prepublicationHealthGate'
$healthGate = Read-JsonFile ([string]$healthDependency.path) 1048576
Require ([string]$healthGate.state -ceq 'PASS_R18ZW_FRESH_DIRECT_QUEUE_AND_ENDPOINT_HEALTH' -and [string]$healthGate.requestId -ceq 'REQ_RZW1' -and
    [string]$healthGate.bindingRecordSha256 -ceq [string]$bindingDependency.sha256 -and [string]$healthGate.terminalPredecessorGateSha256 -ceq [string]$predecessorDependency.sha256 -and
    [string]$healthGate.computerName -ceq 'A1025645101' -and [bool]$healthGate.predecessorRequestTerminal -and
    [bool]$healthGate.endpointHealthy -and [bool]$healthGate.queueHeadClear -and [int]$healthGate.pendingRequestCount -eq 0 -and
    -not [bool]$healthGate.taskOrProcessMutationPerformed -and -not [bool]$healthGate.mutationsPerformed) 'R18ZW fresh direct health gate changed.'
$healthCheckedUtc = [DateTimeOffset]::Parse([string]$healthGate.checkedUtc)
Require ($healthCheckedUtc -ge [DateTimeOffset]::UtcNow.AddMinutes(-30) -and $healthCheckedUtc -le [DateTimeOffset]::UtcNow.AddMinutes(5)) 'R18ZW direct health gate is stale or future-dated.'
...
foreach($name in @('r18zv1Provider','r18zv1DevelopmentFreeze','r18zv1Slot21Job','r18zv1Slot21Result','r18zv1Slot21CompactGate','r18zv1Slot21CorrectionGate')){$d=Get-ValidatedDependency $name;$br=@($binding.artifacts|Where-Object name -ceq $name);Require($br.Count -eq 1 -and [string]$br[0].sha256 -ceq [string]$d.sha256) "R18ZV1 pin changed: $name"}
...
try {
    ...
    Require ([string]$manifest.schema -ceq 'argos_project_portal_request_manifest_v1' -and [string]$manifest.requestId -ceq 'REQ_RZW1' -and
    ...
    Require ([string]$change.source -ceq 'payload/Invoke-R18ZTBatchLaunch.ps1' -and
        [string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZW.ps1' -and
...
Require ($requestRoot -ceq 'U:\ProjectPortalRO\requests' -and $responseRoot -ceq 'U:\ProjectPortalRO\responses' -and
    $readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZW1.ready.zip' -and $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_RZW1.ready.zip.upload' -and
    $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_RZW1.ready.zip') 'R18ZT exact publication route changed.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zw_publication_apply_intent_v1'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZW_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
    requestId = 'REQ_RZW1'
    ...
    prepublicationHealthGateSha256 = [string]$healthDependency.sha256
...
Require ([string]$collision.requestId -ceq 'REQ_RZW1' -and
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zw_publish_gate_v1'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZW_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZW_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_RZW1'
