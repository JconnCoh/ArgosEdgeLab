...
param(
    ...
    [switch]$JunctionGate,
    [string]$PayloadRoot = '',
    [string]$WorkRoot = 'D:\A2\w\ocv\R18ZW1',
    [string]$OutputRoot = 'D:\A2\o\ocv\R18ZW1',
...
$packageRevision = 'R18ZW_EXISTING_ORIENTED_CROPS_PORTAL_DETACHED_20260912A'
$runnerRevision = 'R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260908D'
$envelopeRevision = 'R18ZV3_FIXED_PARALLEL_ASYNC_REVIEW_ONLY_20260911A'
$payloadManifestSha = '8C5606802F96A62F3F222F9C26A5688C63E8BACC672AD1EE2BD2418F90C49C40'
$configurationSha = 'F9E7CF1E14EBE02053E19CB6D176B962AAD02235125A1DB84565FA10EAD6F35A'
$envelopeSha = '88CF2BC2DEE757B9F789E0D03881DC397D1C30A9857380DC460C43A48822A510'
$delegateSha = '423C15AF407E8A7637104765EE5D315113F914876E59F3440308AC9316766C6E'; $baseRunnerSha = 'F3B465F433ADB58F1C37C5673999FD03A4315EFA5A3128EFF8E192E9C3D58AE2'
$providerSha = '5A124F9F793C44CD3631956FA0330BD33B59298AAF653DE25C0F31572EC16E45'
$providerRevision = 'ARGOS_OPENCV_SCRIBE_V1R18ZW_CANONICAL_SPARSE_RESCUE_DIAGNOSTIC_20260912'
...
$installedLauncher = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZW.ps1'
$configuredWorkRoot = 'D:\A2\w\ocv\R18ZW1'
$configuredOutputRoot = 'D:\A2\o\ocv\R18ZW1'
$configuredCanonicalProposalRoot = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals'
$configuredProposalAlias = 'D:\A2\w\ocv\R18ZW1\p'
$configuredProviderPath = 'D:\A2\w\ocv\R18ZW1\OPENCV_SCRIBE_R18ZW\ArgosOpenCvScribeV1R18ZWBatchAdapter.py'
$configuredR11AnalyzerPath = 'D:\A2\w\ocv\R18ZW1\OPENCV_SCRIBE_R11A\ArgosOpenCvScribeV1R11.py'
...
function Get-R18ZTExactCompleteValidation([string]$OutputRoot, [string]$ConfigurationPath) {
    ...
    try {
        ...
        $expectedScalars = [ordered]@{
            ...
            parallelCoordinatorRevision = 'R18ZV3_FIXED_PROCESS_ISOLATED_BATCH_20260911A'; parallelExecutionMode = 'FIXED_PROCESS_ISOLATED_SHARDS'
        }
        ...
        $parallelCache = $complete.referenceBankCache; if ($complete.workerCount -ne 8 -or $null -eq $parallelCache -or [string]$parallelCache.state -cne 'PASS_FIXED_PROCESS_ISOLATED_REFERENCE_BANK_CACHES' -or [string]$parallelCache.parallelism -cne 'FIXED_PROCESS_ISOLATED_SHARDS' -or $parallelCache.workerCount -ne 8 -or @($parallelCache.shards).Count -ne 8) { return [pscustomobject]@{present=$true;exact=$false;detail='COMPLETE_FIXED_PARALLEL_CONTRACT_INVALID'} }
        $pinnedValues = @{}
        foreach ($pin in @(
        ...
        if ([string]$aggregate.parallelCoordinatorRevision -cne [string]$complete.parallelCoordinatorRevision -or [string]$aggregate.parallelExecutionMode -cne [string]$complete.parallelExecutionMode -or $aggregate.workerCount -ne 8 -or @($aggregate.referenceBankCache.shards).Count -ne 8) { return [pscustomobject]@{present=$true;exact=$false;detail='AGGREGATE_FIXED_PARALLEL_CONTRACT_MISMATCH'} }
...
function New-R18ZTExactCompleteGateFixture([string]$OutputRoot, [string]$ConfigurationPath, [string]$ProposalRoot, [string]$CanonicalProposalRoot) {
    ...
    $cacheRows = @(1..8 | ForEach-Object { [ordered]@{state='PASS_EXACT_PROCESS_LOCAL_REFERENCE_BANK_CACHE';bindingSha256=('A' * 64);parallelism='NONE_SERIAL_PROCESS_LOCAL';stages=[ordered]@{baseAppearance=[ordered]@{hits=0;misses=1};combinedAppearanceSupplement=[ordered]@{hits=0;misses=1};topology=[ordered]@{hits=0;misses=1};runStructure=[ordered]@{hits=0;misses=1}}} }); $parallelCache = [ordered]@{state='PASS_FIXED_PROCESS_ISOLATED_REFERENCE_BANK_CACHES';parallelism='FIXED_PROCESS_ISOLATED_SHARDS';workerCount=8;shards=$cacheRows}
    Write-JsonNew $aggregatePath ([ordered]@{
        schema='argos_opencv_scribe_r18zt_batch_aggregate_v2';createdUtc='2026-09-06T00:00:00Z';state='PASS_R18ZT_EXISTING_ORIENTED_CROPS_REVIEW_ONLY_AGGREGATE';disposition='COMPLETE_ALL_QUALIFIED_CASES_COMPARABLE';revision=$revision;batchId=$batchId;proposalRoot=$ProposalRoot;canonicalProposalRoot=$CanonicalProposalRoot;proposalAliasResolvedTarget=$CanonicalProposalRoot;outputRoot=$OutputRoot;directChildCount=1;qualifiedCaseCount=1;inventoryHoldCount=0;completedCount=1;providerInvocationAttemptCount=1;providerRunJobEnteredCount=1;providerRunJobReturnedCount=1;providerRunCount=1;comparableResultCount=1;noncomparableOrFailedCount=0;parallelCoordinatorRevision='R18ZV3_FIXED_PROCESS_ISOLATED_BATCH_20260911A';parallelExecutionMode='FIXED_PROCESS_ISOLATED_SHARDS';workerCount=8;referenceBankCache=$parallelCache;inventory=$inventoryPin;caseIndex=$indexPin;exactProgressPointer=$runningPath;recursiveResultScanRequired=$false;wholeWaferImagesRead=$false;truthOrChecksumUsedByRunnerForSelection=$false;identityAcceptedCount=0;sourceMutationPerformed=$false;automaticRetryPerformed=$false;reviewOnly=$true
    }) 8
    ...
    Write-JsonNew (Join-Path $OutputRoot 'COMPLETE.json') ([ordered]@{
        schema='argos_opencv_scribe_r18zt_batch_complete_v2';createdUtc='2026-09-06T00:00:00Z';state='COMPLETE_R18ZT_EXISTING_ORIENTED_CROPS_REVIEW_ONLY';disposition='COMPLETE_ALL_QUALIFIED_CASES_COMPARABLE';revision=$revision;batchId=$batchId;proposalRoot=$ProposalRoot;canonicalProposalRoot=$CanonicalProposalRoot;proposalAliasResolvedTarget=$CanonicalProposalRoot;qualifiedCaseCount=1;inventoryHoldCount=0;completedCount=1;providerInvocationAttemptCount=1;providerRunJobEnteredCount=1;providerRunJobReturnedCount=1;providerRunCount=1;comparableResultCount=1;noncomparableOrFailedCount=0;parallelCoordinatorRevision='R18ZV3_FIXED_PROCESS_ISOLATED_BATCH_20260911A';parallelExecutionMode='FIXED_PROCESS_ISOLATED_SHARDS';workerCount=8;referenceBankCache=$parallelCache;aggregate=$aggregatePin;inventory=$inventoryPin;caseIndex=$indexPin;exactProgressPointer=$runningPath;recursiveResultScanRequired=$false;truthOrChecksumUsedByRunnerForSelection=$false;identityAcceptedCount=0;sourceMutationPerformed=$false;automaticRetryPerformed=$false;reviewOnly=$true
    }) 8
}
...
function Resolve-R18ZTLaunchFailure {
    ...
    if ($null -ne $OwnedProcess) {
        try {
            ...
            if ($OwnedProcess.HasExited) { Require ($OwnedProcess.WaitForExit(10000)) 'R18ZT owned envelope exit wait failed.'; $workerExitConfirmed = $true }
            else { $Detail = $Detail + '; OWNED_PARALLEL_ENVELOPE_STILL_RUNNING_RESOURCES_PRESERVED' }
        }
        catch { $Detail = $Detail + '; OWNED_WORKER_EXIT_NOT_CONFIRMED: ' + $_.Exception.Message }
        finally { if ($workerExitConfirmed) { $OwnedProcess.Dispose() } }
    }
    ...
    elseif ($workerExitConfirmed -and -not $OwnedProcessStarted) {
        if ($completePresent) {
    ...
    return [pscustomobject]@{
        ...
        resourcesPreserved = ($completeExact -or $OwnedProcessStarted -or -not $workerExitConfirmed)
...
function Invoke-R18ZTProposalJunctionGate {
    ...
    $detachRoot = Join-Path $junctionGateRoot 'detach'
    $detachWorkerScript = Join-Path $detachRoot 'worker.py'
    $detachParentScript = Join-Path $detachRoot 'parent.ps1'
    $detachPidPath = Join-Path $detachRoot 'worker.pid'
    $detachParentProcess = $null
    $detachWorkerProcess = $null
    $detachCaptureReleased = $false
    try {
        [void](New-Item -ItemType Directory -Path $detachRoot)
        [IO.File]::WriteAllText($detachWorkerScript, "import time`ntime.sleep(60)`n", (New-Object Text.UTF8Encoding($false)))
        [IO.File]::WriteAllText($detachParentScript, @'
param([string]$PythonPath,[string]$WorkerPath,[string]$PidPath)
$s=New-Object Diagnostics.ProcessStartInfo;$s.FileName=$PythonPath;$s.Arguments=$WorkerPath;$s.UseShellExecute=$true;$s.CreateNoWindow=$true;$s.WindowStyle=[Diagnostics.ProcessWindowStyle]::Hidden
$p=New-Object Diagnostics.Process;$p.StartInfo=$s;if(-not $p.Start()){throw 'detached child start failed'}
[IO.File]::WriteAllText($PidPath,[string]$p.Id);[ordered]@{childPid=[int]$p.Id}|ConvertTo-Json -Compress
'@, (New-Object Text.UTF8Encoding($false)))
        $detachStartInfo = New-Object Diagnostics.ProcessStartInfo
        $detachStartInfo.FileName = Join-Path $PSHOME 'powershell.exe'
        $detachStartInfo.Arguments = '-NoProfile -ExecutionPolicy Bypass -File ' + (Quote-ProcessArgument $detachParentScript) + ' -PythonPath ' + (Quote-ProcessArgument $GatePythonPath) + ' -WorkerPath ' + (Quote-ProcessArgument $detachWorkerScript) + ' -PidPath ' + (Quote-ProcessArgument $detachPidPath)
        $detachStartInfo.UseShellExecute = $false; $detachStartInfo.CreateNoWindow = $true; $detachStartInfo.RedirectStandardOutput = $true; $detachStartInfo.RedirectStandardError = $true
        $detachParentProcess = New-Object Diagnostics.Process; $detachParentProcess.StartInfo = $detachStartInfo
        Require ($detachParentProcess.Start()) 'R18ZW captured-parent detach rehearsal did not start.'
        $detachOutTask = $detachParentProcess.StandardOutput.ReadToEndAsync(); $detachErrTask = $detachParentProcess.StandardError.ReadToEndAsync()
        Require ($detachParentProcess.WaitForExit(10000)) 'R18ZW captured parent did not exit while its child remained active.'
        Require ($detachOutTask.Wait(1000) -and $detachErrTask.Wait(1000) -and [int]$detachParentProcess.ExitCode -eq 0) ('R18ZW captured-parent streams did not drain: ' + $detachErrTask.Result)
        $detachObservation = $detachOutTask.Result.Trim() | ConvertFrom-Json
        $detachWorkerProcess = [Diagnostics.Process]::GetProcessById([int]$detachObservation.childPid); $detachWorkerProcess.Refresh()
        Require (-not $detachWorkerProcess.HasExited) 'R18ZW detached worker did not remain alive after parent stream closure.'
        $detachCaptureReleased = $true
        ...
        Require (-not [bool]$sleepResolution.workerExitConfirmed -and -not [bool]$sleepResolution.workerTerminatedByLauncher -and [bool]$sleepResolution.resourcesPreserved -and -not [bool]$sleepResolution.proposalAliasRemoved -and -not [bool]$sleepResolution.workQuarantined -and -not [bool]$sleepResolution.failureWritten -and (Test-Path -LiteralPath $sleepAlias)) 'R18ZT production catch active-envelope preservation case failed.'
        $sleepProcess.Kill(); Require ($sleepProcess.WaitForExit(10000)) 'R18ZT gate-only sleeping envelope cleanup failed.'; $sleepProcess.Dispose(); $sleepProcess = $null
        ...
        Write-JsonNew $invalidConfiguration ([ordered]@{batchId='R18ZW1';proposalRoot=$invalidAlias;canonicalProposalRoot=$invalidCanonical}) 4
        ...
        $invalidProcess = $null
        Require ([bool]$invalidResolution.workerExitConfirmed -and [bool]$invalidResolution.completeOriginallyPresent -and [bool]$invalidResolution.completePresent -and -not [bool]$invalidResolution.completeExact -and -not [bool]$invalidResolution.invalidCompleteQuarantined -and -not [bool]$invalidResolution.proposalAliasRemoved -and -not [bool]$invalidResolution.workQuarantined -and -not [bool]$invalidResolution.failureWritten -and [bool]$invalidResolution.resourcesPreserved) 'R18ZT production catch invalid-COMPLETE resource-preservation case failed.'
        ...
        if ($null -eq $detachWorkerProcess -and (Test-Path -LiteralPath $detachPidPath -PathType Leaf)) { try { $detachWorkerProcess = [Diagnostics.Process]::GetProcessById([int][IO.File]::ReadAllText($detachPidPath)) } catch {} }
        foreach ($detachProcess in @($detachWorkerProcess,$detachParentProcess)) { if ($null -ne $detachProcess) { try { if (-not $detachProcess.HasExited) { $detachProcess.Kill(); [void]$detachProcess.WaitForExit(10000) } } catch {}; $detachProcess.Dispose() } }
        ...
        if (Test-Path -LiteralPath $sleepAlias) { try { Remove-OwnedProposalJunction $sleepAlias $sleepCanonical } catch {} }
        if (Test-Path -LiteralPath $invalidAlias) {
            try { Remove-OwnedProposalJunction $invalidAlias $invalidCanonical } catch {}
        }
        foreach ($file in @($detachWorkerScript,$detachParentScript,$detachPidPath,$sleepScript,$sleepFailure,$sleepConfiguration,$fastScript,$fastConfiguration,(Join-Path $fastOutput 'COMPLETE.json'),(Join-Path $fastOutput 'LAUNCH_REPORTING_ERROR.json'),(Join-Path $fastOutput 'AGGREGATE.json'),(Join-Path $fastOutput 'INVENTORY.json'),(Join-Path $fastOutput 'CASE_INDEX.json'),$invalidScript,$invalidConfiguration,$invalidFailure,(Join-Path $invalidOutput 'COMPLETE.json'),(Join-Path $invalidOutput 'COMPLETE.invalid.json'))) {
            if (Test-Path -LiteralPath $file -PathType Leaf) { [IO.File]::Delete($file) }
        }
        foreach ($directory in @($detachRoot,$sleepFailed,$sleepWork,$sleepOutput,$sleepCanonical,$sleepRoot,$fastWork,$fastOutput,$fastCanonical,$fastRoot,$invalidFailed,$invalidWork,$invalidOutput,$invalidCanonical,$invalidRoot,$collisionRoot,$junctionGateRoot)) {
    ...
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zw_parallel_resource_gate_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZW_PARALLEL_ENVELOPE_RESOURCE_PRESERVATION_AND_PORTAL_DETACH_GATE'
        ...
        fixedParallelEnvelopeStarted = $true
        activeEnvelopePreservedWithoutParentOnlyKill = [bool]$sleepResolution.resourcesPreserved
        aliasAndWorkPreservedWhileEnvelopeActive = (-not [bool]$sleepResolution.proposalAliasRemoved -and -not [bool]$sleepResolution.workQuarantined)
        activeFailureJunctionPreservationExercised = $true
        actualProductionCatchFunctionExercised = $true
        activeEnvelopeHoldCasePassed = $true
        fastCompleteReportingFailureCasePassed = $true
        postCompleteFailurePreservedExactComplete = $true
        invalidCompletePreservationCasePassed = $true
        invalidCompleteFailureCommitSuppressed = $true
        ...
        maximumDelegateChildProcessCount = 8
        delegateSubprocessAllowed = $true
        portalCaptureReleasedBeforeWorkerExit = $detachCaptureReleased
        productionLaunchUsesShellExecute = $true
        productionLaunchWindowHidden = $true
        longLivedChildInheritedEndpointPipes = $false
...
foreach ($pin in @(
    ...
    [pscustomobject]@{value=$delegateSha;label='parallel coordinator'}, [pscustomobject]@{value=$baseRunnerSha;label='base batch runner'},
...
$delegateRelative = 'OPENCV_SCRIBE_R18ZV3\r18zv3_parallel_batch.py'; $baseRunnerRelative = 'OPENCV_SCRIBE_R18ZV3\Run-R18ZTExistingOrientedCropsV2.py'
$providerRelative = 'OPENCV_SCRIBE_R18ZW\ArgosOpenCvScribeV1R18ZWBatchAdapter.py'
...
$delegatePath = Join-Path $WorkRoot $delegateRelative; $baseRunnerPath = Join-Path $WorkRoot $baseRunnerRelative
...
$requiredPayloadHashes = [ordered]@{
    ...
    $delegateRelative = $delegateSha; $baseRunnerRelative = $baseRunnerSha
...
Require ([string]$configuration.schema -eq 'argos_opencv_scribe_r18zt_batch_configuration_v2' -and [string]$configuration.batchId -eq 'R18ZW1' -and [string]$configuration.revision -eq $runnerRevision) 'R18ZT configuration contract changed.'
...
try {
    ...
    Require ([int]$manifest.finalization.openCvThreadsPerWorkerPinned -eq 1 -and [bool]$manifest.finalization.openClGpuDisabled -and [bool]$manifest.finalization.portalDetachedChildLaunchPinned) 'R18ZW resource and detached-launch manifest pins changed.'
...
if ($Preflight) {
    [ordered]@{
        ...
        sourceInventoryAndHashingDeferredToOwnedWorker = $true; parallelCoordinatorRevision = 'R18ZV3_FIXED_PROCESS_ISOLATED_BATCH_20260911A'; fixedWorkerCount = 8; maximumDelegateChildProcessCount = 8
...
try {
    ...
    Require ((Get-Sha256 $delegatePath) -eq $delegateSha) 'R18ZT staged parallel coordinator changed.'; Require ((Get-Sha256 $baseRunnerPath) -eq $baseRunnerSha) 'R18ZT staged base runner changed.'
    ...
    $env:OMP_NUM_THREADS = '1'; $env:OPENBLAS_NUM_THREADS = '1'; $env:MKL_NUM_THREADS = '1'; $env:NUMEXPR_NUM_THREADS = '1'; $env:CUDA_VISIBLE_DEVICES = '-1'
    ...
    $launch = [ordered]@{
        ...
        delegateSha256 = $delegateSha; baseRunnerSha256 = $baseRunnerSha
        ...
        ownedProcessStarted = $true; delegateExecutionMode = 'IN_PROCESS_PUBLIC_MAIN_WITH_FIXED_PROCESS_ISOLATED_CHILDREN'; fixedWorkerCount = 8; maximumDelegateChildProcessCount = 8
        automaticRetryAllowed = $false
        publicationAuthorized = $false
        publicationPerformed = $false
        sourceMutationPerformed = $false
