...
$expectedResponseId = 'R_D1D4956AA344_20260903194752479_47abca21'
$expectedResponseZipSha256 = '72FA6467CC8DF2020B7D21027BCEFEFC50B93AA0035FD74DF6B4619498C747C2'
...
$outerExtractRoot = 'C:\R18ER'
$payloadExtractRoot = 'C:\R18E'
$metadataRoot = Join-Path $PSScriptRoot 'collected_metadata'
$terminalGatePath = Join-Path $PSScriptRoot 'R18E_TERMINAL_RESPONSE_GATE.json'
...
$publishGateSha256 = '484FF80082F4C3CB97BA73F207A7157336FE67D851777C15019A459042B5E1A8'
...
Assert-True ([string]$publishGate.state -eq 'PASS_R18E_EXACT_SIGNED_DATA_PULL_PUBLISHED_CREATE_NEW' -and [string]$publishGate.requestId -eq $requestId -and -not [bool]$publishGate.retryAuthorized) 'R18E publication evidence changed.'
Assert-True ([string]$routeGate.state -eq 'PASS_R18E_COMPLETE_ROUTE_GATE' -and [int]$routeGate.maximumEffectiveLength -lt 200) 'R18E route gate changed.'
...
if ($matches.Count -eq 0) {
    ...
    [ordered]@{schema='argos_r18e_response_collection_preflight_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='WAIT_R18E_MATCHING_RESPONSE';requestId=$requestId;responseFilesScanned=$responseFiles.Count;matchingResponses=0;mutationsPerformed=$false;requestRetried=$false;pixelsDecoded=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
