...
$QueryPath=Join-Path $PSScriptRoot 'Q3.ps1'
...
function New-Rows { return [ordered]@{ schema='argos_r18uq3_sql_row_rehearsal_v1'; directRows=@(); issueHistoryRows=@(); associateHistoryRows=@(); disassociateHistoryRows=@() } }

$tempRoot=Join-Path ([IO.Path]::GetTempPath()) ('ArgosR18UQ3_'+[Guid]::NewGuid().ToString('N'))
...
try {
    ...
    Assert-True ($preflight.state -ceq 'PASS_R18UQ3_QUERY_PREFLIGHT') 'Preflight state failed.'
    ...
    for ($i=0; $i -lt $keys.Count; $i++) {
        $key=[string]$keys[$i]
        if ($key -ceq '62546-481' -or $key -ceq '62617-215D') { continue }
    ...
    $complete.issueHistoryRows=@([pscustomobject]@{
        ParentContainer='62546-481F'; IssuedWaferContainer='62546-481-010'; Scribe='13HFX135SUE3'; SourceEpiContainer='112204-079H-2'
        IssueTxnDate='2026-07-01T01:02:03.004'; IssueTxnServiceName='ComponentIssue'; IssueTxnType='6440'
    })
    $complete.associateHistoryRows=@([pscustomobject]@{
        ParentContainer='62627-193'; UnitContainer='62627-193B'; Scribe=$null
        MembershipTxnDate='2026-08-01T00:00:00.000'; MembershipTxnServiceName='Associate'; MembershipTxnType='Associate'; MembershipEventId='2'
    })
    ...
    Assert-True ($completeResult.disposition -ceq 'COMPLETE' -and $completeResult.state -ceq 'PASS_R18UQ3_EXHAUSTIVE_ROSTER') '50-lot COMPLETE result failed.'
    ...
    Assert-True ($completeResult.counts.invalidOrNullRows -eq 0 -and $completeResult.counts.ignoredHistoricalRows -eq 1) 'Ignored non-wafer historical diagnostics failed.'
    Assert-True (@($completeResult.ignoredHistoricalRows | Where-Object { $_.queryLot -ceq '62627-193' -and $_.code -ceq 'IGNORED_NON_WAFER_HISTORICAL_CHILD' }).Count -eq 1) 'Ignored non-wafer historical row missing.'
    $issuedLot=@($completeResult.lots | Where-Object { $_.queryLot -ceq '62546-481' })[0]
    Assert-True ($issuedLot.resolvedMembers[0].unitContainer -ceq '62546-481-010' -and $issuedLot.resolvedMembers[0].resolvedScribe -ceq '13HFX135SUE3') 'Exact issued-unit catalog identity did not resolve 62546-481.'
    Assert-True (@($issuedLot.resolvedMembers[0].evidence | Where-Object { $_.membershipEvidenceBasis -ceq 'EXACT_ISSUED_UNIT_CATALOG_KEY' }).Count -eq 1) 'Exact issued-unit evidence basis missing.'
    Assert-True (@($issuedLot.resolvedMembers[0].parentContainers) -contains '62546-481F') 'Derivative current parent evidence was not preserved.'
    ...
    $withHold.associateHistoryRows=$complete.associateHistoryRows
    $withHold.issueHistoryRows=@($complete.issueHistoryRows) + @([pscustomobject]@{
    ...
    Assert-True ($holdResult.disposition -ceq 'USABLE_WITH_HOLDS' -and $holdResult.state -ceq 'PASS_R18UQ3_USABLE_ROSTER_WITH_EXPLICIT_HELD_MEMBERS') '50-lot usable-with-holds state failed.'
    ...
    $noSuffixD=New-Rows
    $noSuffixD.directRows=$complete.directRows
    $noSuffixD.disassociateHistoryRows=$complete.disassociateHistoryRows
    $noSuffixD.associateHistoryRows=$complete.associateHistoryRows
    $noSuffixD.issueHistoryRows=@($complete.issueHistoryRows) + @([pscustomobject]@{ ParentContainer=$null; IssuedWaferContainer='62617-215-050'; Scribe='11111111SUA1'; SourceEpiContainer='SOURCE-1' })
    $noSuffixDResult=Invoke-Rehearsal $tempRoot 'no_suffix_d' $noSuffixD
    $noSuffixDLot=@($noSuffixDResult.lots | Where-Object { $_.queryLot -ceq '62617-215D' })[0]
    Assert-True (@($noSuffixDLot.resolvedMembers | Where-Object { $_.unitContainer -ceq '62617-215-050' }).Count -eq 0) 'Unsuffixed unit was synthesized into suffixed D lot.'
    Assert-True (@($noSuffixDResult.invalidOrNullRows | Where-Object { $_.code -ceq 'NO_EXACT_DB_PARENT_MEMBERSHIP_EVIDENCE' }).Count -eq 1) 'Unsuffixed D-candidate diagnostic missing.'
    Assert-True ($noSuffixDResult.disposition -ceq 'HOLD') 'Unmapped unsuffixed D-candidate did not hold global result.'

    $waferIssue=New-Rows
    $waferIssue.directRows=$complete.directRows
    $waferIssue.disassociateHistoryRows=$complete.disassociateHistoryRows
    $waferIssue.associateHistoryRows=$complete.associateHistoryRows
    $waferIssue.issueHistoryRows=@($complete.issueHistoryRows) + @([pscustomobject]@{ ParentContainer=$null; IssuedWaferContainer='62546-481-020Wafer'; Scribe='22222222SUA2'; SourceEpiContainer='SOURCE-2' })
    $waferResult=Invoke-Rehearsal $tempRoot 'wafer_issue' $waferIssue
    $waferLot=@($waferResult.lots | Where-Object { $_.queryLot -ceq '62546-481' })[0]
    Assert-True (@($waferLot.resolvedMembers | Where-Object { $_.unitContainer -ceq '62546-481-020WAFER' }).Count -eq 0) 'Issue WAFER suffix was admitted as exact issued-unit identity.'
    Assert-True (@($waferResult.invalidOrNullRows | Where-Object { $_.code -ceq 'NO_EXACT_DB_PARENT_MEMBERSHIP_EVIDENCE' }).Count -eq 1) 'Issue WAFER suffix diagnostic missing.'

    $contradiction=New-Rows
    $contradiction.directRows=@($complete.directRows | Where-Object { $_.ParentContainer -cne '62613-842A' })
    $contradiction.disassociateHistoryRows=$complete.disassociateHistoryRows
    $contradiction.associateHistoryRows=$complete.associateHistoryRows
    $contradiction.issueHistoryRows=@($complete.issueHistoryRows) + @([pscustomobject]@{ ParentContainer='62613-842A'; IssuedWaferContainer='62607-215-099'; Scribe='67055042SUC1'; SourceEpiContainer='SOURCE-3' })
    ...
    Assert-True ($receipt.state -ceq 'PASS_R18UQ3_RESULT_WRITTEN_CREATE_NEW' -and -not $receipt.overwritePermitted -and -not $receipt.credentialsReturned) 'Create-new receipt failed.'
    ...
    $secret='R18UQ3_DO_NOT_RETURN_SECRET'; $secure=ConvertTo-SecureString $secret -AsPlainText -Force
    ...
    [pscustomobject][ordered]@{
        schema='argos_r18uq3_focused_test_v1'; state='PASS_R18UQ3_FOCUSED_LOCAL_REHEARSAL'
        assertions=39; queryLots=50; completeDisposition=$completeResult.disposition
    ...
    if ($full.StartsWith($temp,[StringComparison]::OrdinalIgnoreCase) -and [IO.Path]::GetFileName($full).StartsWith('ArgosR18UQ3_')) { Remove-Item -LiteralPath $full -Recurse -Force }
}
