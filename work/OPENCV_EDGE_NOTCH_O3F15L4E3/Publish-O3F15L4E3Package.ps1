...
$expectedInvocation = Join-Path $PSScriptRoot 'O3F15L4E3_PUBLISH_P2_INVOCATION.json'
...
Assert-True ([string](Get-RequiredProperty $invocation 'schema') -ceq 'argos_ocv03_o3f15l4e3_publish_p2_invocation_v1') 'D2 publication invocation schema changed.'
Assert-True ([string](Get-RequiredProperty $invocation 'state') -ceq 'FROZEN_O3F15L4E3_EXACTLY_ONCE_PUBLICATION_P2_INVOCATION') 'D2 publication invocation is not frozen.'
...
foreach ($row in $closureRows) {
    ...
    Assert-True ($priorId -ceq 'REQ_20260903T160913268Z_F8A07CFA345E' -and [string](Get-RequiredProperty $row 'responseId') -ceq 'R_BE83F07DE6AD_20260903161350667_e63dc3b0') 'E3 immediate prior serial request is not terminal O3F15L4E2.'
...
Assert-True ($publicationGatePath.Equals((Join-Path $PSScriptRoot 'O3F15L4E3_PUBLISH_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication gate path changed.'
Assert-True ($publicationAttemptPath.Equals((Join-Path $PSScriptRoot 'O3F15L4E3_PUBLISH_ATTEMPT.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication-attempt receipt path changed.'
...
if ($attemptReceiptExists) {
    ...
    Assert-True ([string](Get-RequiredProperty $attemptReceipt 'schema') -ceq 'argos_ocv03_o3f15l4e3_publish_attempt_v1' -and [string](Get-RequiredProperty $attemptReceipt 'state') -ceq 'STARTED_O3F15L4E3_SINGLE_PUBLICATION_ATTEMPT') 'D2 publication-attempt receipt identity changed.'
...
$preflightResult = [ordered]@{
    schema = 'argos_ocv03_o3f15l4e3_publish_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O3F15L4E3_EXACTLY_ONCE_PUBLICATION_PREFLIGHT'
    requestId = $requestId
    sourceZip = $sourceZip
...
if ($queueState -ceq 'NEW') {
    $attemptReceipt = [ordered]@{
        schema = 'argos_ocv03_o3f15l4e3_publish_attempt_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'STARTED_O3F15L4E3_SINGLE_PUBLICATION_ATTEMPT'
        requestId = $requestId
        sourceZip = $sourceZip
        sourceZipBytes = $sourceBytes
        sourceZipSha256 = $sourceHash
...
$gate = [ordered]@{
    schema = 'argos_ocv03_o3f15l4e3_publish_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O3F15L4E3_PUBLISHED_EXACTLY_ONCE_AWAITING_MATCHING_SIGNED_RESPONSE'
