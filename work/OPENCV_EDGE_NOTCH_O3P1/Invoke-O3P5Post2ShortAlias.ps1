...
if ([string]$invocation.schema -ne 'argos_ocv03_o3p5_short_alias_launch_invocation_v1') {
    throw 'O3P5 launch invocation schema changed.'
}
if (-not [bool]$invocation.reviewOnly -or [bool]$invocation.productionRoutingEnabled -or [bool]$invocation.sourceMutationAllowed -or [bool]$invocation.networkAllowed -or [bool]$invocation.liveProviderActivation) {
    throw 'O3P5 launch authority widened.'
}
...
if ($workspaceRoot -cne 'C:\Users\joshua.conn\Desktop\ArgosDev\ArgosEdgeLab' -or $aliasDrive -cne 'R:' -or $outputFull -cne 'C:\Users\joshua.conn\Desktop\ArgosDev\ArgosEdgeLab\work\OPENCV_EDGE_NOTCH_O3P1\O3P5_POST2_CORROBORATION.json') {
    throw 'O3P5 workspace, alias, or output contract changed.'
}
...
Require-ExactFile -Path $engineFull -Sha256 ([string]$invocation.engine.sha256) -Label 'O3P5 engine'
Require-ExactFile -Path $jobFull -Sha256 ([string]$invocation.job.sha256) -Label 'O3P5 job'
Require-ExactFile -Path $syntheticGateFull -Sha256 ([string]$invocation.syntheticGate.sha256) -Label 'O3P4 synthetic gate'
$syntheticGate = Get-Content -Raw -LiteralPath $syntheticGateFull | ConvertFrom-Json
if ([string]$syntheticGate.state -ne 'PASS_O3P4_FRONT_NOTCH_SYNTHETIC_GATE') {
    throw 'O3P4 synthetic gate is not PASS.'
}
if (Test-Path -LiteralPath $outputFull) {
    throw "Create-new O3P5 output already exists: $outputFull"
}
if (Test-Path -LiteralPath ($outputFull + '.partial')) {
    throw "Create-new O3P5 partial output already exists: $outputFull.partial"
}
if (Test-Path -LiteralPath $launchGateFull) {
    throw "Create-new O3P5 launch gate already exists: $launchGateFull"
}
if (Test-Path -LiteralPath ($launchGateFull + '.partial')) {
    throw "Create-new O3P5 partial launch gate already exists: $launchGateFull.partial"
}
...
if ($null -ne $existingTarget -or (Test-Path -LiteralPath $aliasRoot)) {
    throw "O3P5 alias drive is not unused: $aliasDrive"
}

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ocv03_o3p5_short_alias_launch_preflight_v1'
        state = 'PASS_O3P5_SHORT_ALIAS_LAUNCH_PREFLIGHT'
...
try {
    ...
    if ($null -eq $mappedTarget -or [IO.Path]::GetFullPath($mappedTarget).TrimEnd('\') -cne $workspaceRoot) {
        throw "O3P5 alias maps to the wrong target: $mappedTarget"
    }
    ...
    try {
        ...
        if ($preflightExit -ne 0 -or $preflightRows.Count -ne 1) {
            throw "O3P5 alias Python preflight failed with exit $preflightExit and $($preflightRows.Count) rows."
        }
        $pythonPreflight = [string]$preflightRows[0] | ConvertFrom-Json
        if ([string]$pythonPreflight.state -ne 'PASS_O3P4_FRONT_NOTCH_CORROBORATION_PREFLIGHT' -or [bool]$pythonPreflight.imageBytesDecoded -or [bool]$pythonPreflight.outputCreated) {
            throw 'O3P5 alias Python preflight did not pass exactly.'
        }
    ...
    if ($engineExit -ne 0 -or $engineRows.Count -ne 1) {
        throw "O3P5 engine failed with exit $engineExit and $($engineRows.Count) rows."
    }
    $engineResult = [string]$engineRows[0] | ConvertFrom-Json
    if ([string]$engineResult.state -ne 'COMPLETE_O3P4_POST2_TOPOLOGY_CORROBORATION_REVIEW_ONLY') {
        throw 'O3P5 engine terminal state changed.'
    }
    $engineCommand = [string]$engineRows[0]
    Require-ExactFile -Path $outputFull -Sha256 ([string]$engineResult.outputSha256) -Label 'O3P5 engine output'
    $aliasOutput = [IO.Path]::GetFullPath((Join-Path $aliasRoot ([string]$invocation.output.relativePath)))
    Require-ExactFile -Path $aliasOutput -Sha256 ([string]$engineResult.outputSha256) -Label 'O3P5 alias output'
} finally {
    if ($aliasCreated) {
        ...
        if ($null -eq $mappedBeforeDelete -or [IO.Path]::GetFullPath($mappedBeforeDelete).TrimEnd('\') -cne $workspaceRoot) {
            throw "O3P5 refuses to remove a non-matching alias: $mappedBeforeDelete"
        }
...
if ($null -ne (Get-SubstTarget -SubstPath $substPath -Drive $aliasDrive) -or (Test-Path -LiteralPath $aliasRoot)) {
    throw 'O3P5 alias remained after bounded execution.'
}
...
$launchResult = [ordered]@{
    schema = 'argos_ocv03_o3p5_short_alias_launch_gate_v1'
    state = 'PASS_O3P5_POST2_SHORT_ALIAS_EXECUTION'
    invocationPath = [IO.Path]::GetFullPath($InvocationManifest)
    invocationSha256 = Get-Sha256 -Path $InvocationManifest
    workspaceRoot = $workspaceRoot
    aliasDrive = $aliasDrive
