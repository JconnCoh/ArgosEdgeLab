...
$contractPath=Join-Path $PSScriptRoot 'O3F8R13T3_LAUNCH_CONTRACT.json';$contractHash='EAC7A757ADC8953C6925C27650183CFCA4198561D0AFA7DC567148E99F37899F'
$definitionSource=Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION.json';$root='C:\O3F8R13T3PKR2';$payload=Join-Path $root 'payload';$definitionTarget=Join-Path $root 'DEFINITION.json';$gatePath=Join-Path $PSScriptRoot 'O3F8R13T3_BUILD_GATE_R2.json'
...
Require ([string]$contract.schema-eq'argos_ocv03_o3f8r13t3_launch_contract_v1'-and[string]$contract.state-eq'FROZEN_FOR_BUILD'-and[int]$contract.expectedPairCount-eq11) 'Contract identity changed'
Require ([string]$definition.schema-eq'argos_ocv03_o3f8r13t3_maintenance_definition_v1'-and[string]$definition.state-eq'FROZEN_FOR_SIGNING'-and[string]$definition.entryPoint-eq'payload/Invoke-O3F8R13T3.ps1') 'Definition identity changed'
...
$rows.Add([pscustomobject]@{path='O3F8R13T3_LAUNCH_CONTRACT.json';source=$contractPath;bytes=[int64](Get-Item $contractPath).Length;sha256=$contractHash;copyToRuntime=$false});$sources=$rows.ToArray()
Require ($sources.Count-eq26-and@($sources.path|Sort-Object -Unique).Count-eq26-and@($sources|Where-Object{[bool]$_.copyToRuntime}).Count-eq23) 'Payload cardinality changed'
foreach($required in @('Invoke-O3F8R13T3.ps1','Run-O3F8R13Targeted.py','FullPerimeterWaferTopologyOpenCvR13.py','FullPerimeterWaferTopologyOpenCvR11.py','Run-O3F15L4FrontReconcile.py','Test-O3F8R13DfCandidateLimitT2.py','Test-O3F8R13TargetedPinIsolationT3.py','OCV03_NotchReviewOpenCvV1.py')){Require (@($sources|Where-Object{[string]$_.path-eq$required}).Count-eq1) "Required payload absent: $required"}
