...
function New-Bytes([string]$Path,[byte[]]$Bytes){Require (-not(Test-Path -LiteralPath $Path)) "O3F8R13T3 create-new path exists: $Path";[IO.File]::WriteAllBytes($Path,$Bytes)}
function New-Json([string]$Path,[object]$Value){Require (-not(Test-Path -LiteralPath $Path)) "O3F8R13T3 create-new JSON exists: $Path";[IO.File]::WriteAllText($Path,(($Value|ConvertTo-Json -Depth 20)+[Environment]::NewLine),(New-Object Text.UTF8Encoding($false)))}

$project=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'))
$root='C:\O3F8R13T3PKR2'
$payload=Join-Path $root 'payload'
$definitionPath=Join-Path $root 'DEFINITION.json'
$buildGatePath=Join-Path $PSScriptRoot 'O3F8R13T3_BUILD_GATE_R2.json'
$signGatePath=Join-Path $PSScriptRoot 'O3F8R13T3_SIGN_GATE.json'
$finalRoot=Join-Path $PSScriptRoot 'final_o3f8r13t3'
...
foreach($path in @($definitionPath,$buildGatePath,$identityPath,$certificatePath,$verifier)){Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F8R13T3 signing dependency absent: $path"}
$buildGate=Get-Content -LiteralPath $buildGatePath -Raw|ConvertFrom-Json
$definition=Get-Content -LiteralPath $definitionPath -Raw|ConvertFrom-Json
Require ([string]$buildGate.state-eq'PASS_O3F8R13T3_UNSIGNED_TARGETED_PACKAGE_BUILT'-and[string]$buildGate.definitionSha256-eq(Sha $definitionPath)) 'O3F8R13T3 build evidence changed.'
Require ([string]$definition.state-eq'FROZEN_FOR_SIGNING'-and[string]$definition.targetRole-eq'JBOD'-and[string]$definition.jobClass-eq'MAINTENANCE_PATCH'-and[string]$definition.entryPoint-eq'payload/Invoke-O3F8R13T3.ps1') 'O3F8R13T3 definition is not frozen for signing.'
Require (@($definition.changes).Count-eq1-and@($definition.allowedTaskActions).Count-eq0-and@($definition.allowedProcessActions).Count-eq2-and@($definition.entryPointOutputs).Count-eq4-and[int]$definition.sourceProcessingContract.expectedPairCount-eq11-and[int]$definition.sourceProcessingContract.hotspotCount-eq10-and[int]$definition.sourceProcessingContract.providerErrorCount-eq5-and[string]$definition.sourceProcessingContract.side-eq'FRONT') 'O3F8R13T3 maintenance bounds changed.'
$change=@($definition.changes)[0]
$carrierHash='6B7CC6E643265545297BA4DED1E6B37AB262349572194B13F79264EF77D8DCE4'
Require ([string]$change.source-eq'payload/OCV03_NotchReviewOpenCvV1.py'-and[string]$change.destination-eq'C:/ProgramData/ArgosEdgeLabRO/AllWaferProcessorV2/OCV03_NotchReviewOpenCvV1.py'-and[string]$change.installedSha256-eq$carrierHash-and@($change.approvedPredecessorSha256).Count-eq1-and([string]@($change.approvedPredecessorSha256)[0])-eq$carrierHash-and-not[bool]$change.allowCreate) 'O3F8R13T3 same-bytes carrier contract changed.'
Require ([string]$buildGate.endpointWorkerSha256-eq'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250'-and[string]$buildGate.installedRouteConfigEvidenceSha256-eq'465107D861E8D2C376A419B1C15841BE3A5757C6498C36CA56B9A95A2F0B76DB'-and[string]$buildGate.queueSafetyGateSha256-eq'170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D') 'O3F8R13T3 inherited portal route evidence changed.'
$actual=@(Get-ChildItem -LiteralPath $payload -File|Sort-Object Name)
Require ($actual.Count-eq[int]$buildGate.payloadFileCount) 'O3F8R13T3 payload cardinality changed.'
foreach($row in @($buildGate.payloadFiles)){$path=Join-Path $payload ([string]$row.path);Require (Test-Path -LiteralPath $path -PathType Leaf) "O3F8R13T3 payload absent: $($row.path)";Require ((Get-Item -LiteralPath $path).Length-eq[int64]$row.bytes-and(Sha $path)-eq[string]$row.sha256) "O3F8R13T3 payload changed: $($row.path)"}
...
Require ([bool]$certificate.HasPrivateKey) 'O3F8R13T3 signer private key unavailable.'
...
Require ($files.Count-eq$actual.Count) 'O3F8R13T3 signed payload cardinality changed.'
