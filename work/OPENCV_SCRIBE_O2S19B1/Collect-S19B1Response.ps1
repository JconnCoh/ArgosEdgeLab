...
Assert-True($invocationPath.StartsWith($project+'\',[StringComparison]::OrdinalIgnoreCase))'S19B1 invocation escaped the project.'
...
Assert-True([string]$invocation.schema-eq'argos_s19b1_response_collection_invocation_v1' -and [string]$invocation.requestId-eq$requestId -and [string]$invocation.responseId-eq$responseId)'S19B1 collection identity changed.'
Assert-True([int64]$invocation.expectedZipBytes-eq$expectedZipBytes -and [string]$invocation.expectedZipSha256-eq$expectedZipSha256 -and [string]$invocation.expectedResponseManifestSha256-eq$expectedManifestSha256)'S19B1 response pins changed.'
Assert-True([bool]$invocation.singleCollectionAuthorized -and -not[bool]$invocation.requestRetryAuthorized -and [bool]$invocation.flattenNestedPayloadToSignedShortNames -and [bool]$invocation.reviewOnly -and -not[bool]$invocation.productionRoutingEnabled)'S19B1 collection authority changed.'
Assert-True((Get-Sha256 $invocationPath)-eq$expectedInvocationSha256)'S19B1 invocation bytes changed.'
$preactionPath=Join-Path $project 'work\OPENCV_SCRIBE_O2S19B1\PREACTION_S19B1_COLLECT.json'
$publishGatePath=Join-Path $project 'work\OPENCV_SCRIBE_O2S19B1\S19B1_PUBLISH_GATE.json'
Assert-True((Get-Sha256 $preactionPath)-eq$expectedPreactionSha256 -and (Get-Sha256 $publishGatePath)-eq$expectedPublishGateSha256)'S19B1 collection dependencies changed.'
...
Assert-True ($null-ne$logicalDisk -and [int]$logicalDisk.DriveType-eq4 -and (Normalize-Root ([string]$logicalDisk.ProviderName)).Equals((Normalize-Root $shareRoot),[StringComparison]::OrdinalIgnoreCase)) 'S19B1 persistent U: mapping changed.'
$sourceZip=[IO.Path]::GetFullPath([string]$invocation.sourceZip)
Assert-True([IO.Path]::GetDirectoryName($sourceZip).Equals('U:\ProjectPortalRO\responses',[StringComparison]::OrdinalIgnoreCase) -and [IO.Path]::GetFileName($sourceZip)-eq($responseId+'.ready.zip'))'S19B1 response source changed.'
Assert-True((Test-Path -LiteralPath $sourceZip -PathType Leaf) -and (Get-Item -LiteralPath $sourceZip).Length-eq$expectedZipBytes -and (Get-Sha256 $sourceZip)-eq$expectedZipSha256)'S19B1 response ZIP changed.'
...
Assert-True($outputRoot.StartsWith($project+'\',[StringComparison]::OrdinalIgnoreCase) -and $terminalGate.StartsWith($project+'\',[StringComparison]::OrdinalIgnoreCase))'S19B1 collection target escaped the project.'
$localZip=Join-Path $outputRoot 'response.zip';$partialRoot=Join-Path $outputRoot 'response.partial';$readyRoot=Join-Path $outputRoot 'response';$decodedPartial=Join-Path $partialRoot 'decoded';$proposalLocal=Join-Path $decodedPartial 'slot19_proposal.json';$summaryLocal=Join-Path $decodedPartial 'slot19_multi_channel_summary.json';$mappingLocal=Join-Path $decodedPartial 'source_to_local_mapping.json'
foreach($fresh in @($outputRoot,$terminalGate)){Assert-True(-not(Test-Path -LiteralPath $fresh))"S19B1 collection target is not fresh: $fresh"}
...
try{
    $expectedEntries=@('DATA_PULL_PAYLOAD.zip','PORTAL_RESPONSE_MANIFEST.json','PORTAL_RESPONSE_MANIFEST.sig','RESULT.json');$entryNames=@($archive.Entries|ForEach-Object{$_.FullName});Assert-True($entryNames.Count-eq4 -and @(Compare-Object $expectedEntries $entryNames).Count-eq0)'S19B1 response entry set changed.'
...
Assert-True((Get-BytesSha256 $manifestBytes)-eq$expectedManifestSha256 -and (Get-BytesSha256 $resultBytes)-eq$expectedResultSha256 -and (Get-BytesSha256 $payloadBytes)-eq$expectedPayloadSha256)'S19B1 response content hash changed.'
...
Assert-True([string]$manifest.schema-eq'argos_project_portal_response_manifest_v1' -and [string]$manifest.requestId-eq$requestId -and [string]$manifest.responseId-eq$responseId -and [string]$manifest.sourceRole-eq'JBOD' -and [string]$manifest.state-eq'PASS_DATA_PULL')'S19B1 signed manifest identity/state changed.'
Assert-True([string]$manifest.signerThumbprint-eq'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC' -and [bool]$manifest.reviewOnly -and -not[bool]$manifest.trainingEligible -and -not[bool]$manifest.xmlEligible -and -not[bool]$manifest.productionEligible -and -not[bool]$manifest.productionRoutingEnabled -and -not[bool]$manifest.credentialsIncluded)'S19B1 signed authority changed.'
Assert-True(@($manifest.files).Count-eq2 -and @(Compare-Object @('DATA_PULL_PAYLOAD.zip','RESULT.json') @($manifest.files.path)).Count-eq0)'S19B1 manifest file set changed.'
foreach($record in @($manifest.files)){$bytes=if([string]$record.path-eq'DATA_PULL_PAYLOAD.zip'){$payloadBytes}else{$resultBytes};Assert-True (@($bytes).Count-eq[int64]$record.bytes -and (Get-BytesSha256 ([byte[]]$bytes))-eq[string]$record.sha256) "S19B1 declared file changed: $($record.path)"}
...
Assert-True($signatureValid -and $cert.Thumbprint-eq'DF46FA4B81065AB273A88F4E1FA8AC0F2EE518CC')'S19B1 response signature failed.'

$proposalEntry='data/JBOD_PROCESSOR_REVIEW/identity/proposals/62619-433_20260824005735_Slot19/SCRIBE_PROPOSAL.json';$summaryEntry='data/JBOD_PROCESSOR_REVIEW/identity/proposals/62619-433_20260824005735_Slot19/scribe/multi_channel/MULTI_CHANNEL_READER_SUMMARY.json'
...
Assert-True((Get-BytesSha256 $proposalBytes)-eq$expectedProposalSha256 -and (Get-BytesSha256 $summaryBytes)-eq$expectedSummarySha256)'S19B1 nested source hashes changed.'
Assert-True([string]$result.schema-eq'argos_project_portal_data_pull_result_v2' -and [string]$result.state-eq'PASS_DATA_PULL' -and [string]$result.approvedRoot-eq'JBOD_PROCESSOR_REVIEW' -and [string]$result.containerSha256-eq$expectedPayloadSha256 -and @($result.files).Count-eq2)'S19B1 DATA_PULL result changed.'
foreach($row in @($result.files)){$expected=if([string]$row.entryPath-eq$proposalEntry){$expectedProposalSha256}elseif([string]$row.entryPath-eq$summaryEntry){$expectedSummarySha256}else{throw "S19B1 unexpected result path: $($row.entryPath)"};Assert-True([string]$row.sha256-eq$expected)"S19B1 result hash changed: $($row.entryPath)"}
...
Assert-True([string]$proposal.schema-eq'argos_jbod_scribe_proposal_v1' -and [string]$proposal.physicalIdentity-eq'62619-433_20260824005735_Slot19' -and [string]$summary.schema-eq'argos_scribe_multi_channel_polarity_reader_v1' -and [string]$summary.acquisitionKey-eq'62619-433_20260824005735_SLOT18')'S19B1 source identity changed.'
$bfPath=[string]$proposal.bfOrientedReviewPath;$dfPath=[string]$proposal.dfOrientedReviewPath
Assert-True($bfPath.EndsWith('\62619-433_20260824005735_Slot19\scribe\BF_SCRIBE_ORIENTED_DETECTOR_INPUT.png',[StringComparison]::OrdinalIgnoreCase) -and $dfPath.EndsWith('\62619-433_20260824005735_Slot19\scribe\DF_SCRIBE_ORIENTED_DETECTOR_INPUT.png',[StringComparison]::OrdinalIgnoreCase))'S19B1 oriented input paths changed.'
...
Assert-True([string]$pathGate.state-eq'PASS_PATH_BUDGET')'S19B1 collection path budget failed.'
$preflightResult=[ordered]@{schema='argos_s19b1_response_collection_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19B1_RESPONSE_COLLECTION_PREFLIGHT';requestId=$requestId;responseId=$responseId;endpointState=[string]$manifest.state;sourceZipBytes=$expectedZipBytes;sourceZipSha256=$expectedZipSha256;responseManifestSha256=$expectedManifestSha256;signedResponseVerified=$signatureValid;proposalSha256=$expectedProposalSha256;summarySha256=$expectedSummarySha256;bfOrientedReviewPath=$bfPath;dfOrientedReviewPath=$dfPath;orientedInputHashMetadataPresent=$hashMetadataPresent;pathState=[string]$pathGate.state;mutationsPerformed=$false;jbodContacted=$false;requestRetried=$false;imageBytesRead=$false;reviewOnly=$true;productionRoutingEnabled=$false}
if($Preflight){$preflightResult|ConvertTo-Json -Depth 10;return}

[void][IO.Directory]::CreateDirectory($outputRoot);[IO.File]::Copy($sourceZip,$localZip,$false);Assert-True((Get-Sha256 $localZip)-eq$expectedZipSha256)'S19B1 response changed during copy.';[void][IO.Directory]::CreateDirectory($partialRoot);[IO.Compression.ZipFile]::ExtractToDirectory($localZip,$partialRoot)
...
Assert-True([string]$verified.State-eq'PASS_SIGNED_PORTAL_RESPONSE' -and [string]$verified.EndpointState-eq'PASS_DATA_PULL')'S19B1 extracted response verification failed.'
