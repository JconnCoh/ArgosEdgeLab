...
$definitionPath = Join-Path $PSScriptRoot 'S19B1_DATA_PULL_DEFINITION.json'
$intentPath = Join-Path $PSScriptRoot 'S19B1_RECOVERY_INTENT.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_S19B1_BUILD.json'
...
$terminalPath = Join-Path $project 'work\OPENCV_SCRIBE_O2D13\O2D13_TERMINAL_RESPONSE_GATE.json'
...
$expectedDefinitionSha256 = '4C84BB0877E1452CF96597C00FA0726F3994F526CC8FE741E9653E67F82267C1'
$expectedIntentSha256 = 'FC8C5DF56C2E370D80A55992DC45C1325E6DA2F5E07B40FEA92A4BE5D8B35DD4'
$expectedPreactionSha256 = 'AA00B9F0E0C8B95461B9DEB8DAD5E68CF707FB54BB39AF921734552A7B9D5C65'
...
$expectedTerminalSha256 = '59817EF309D84DA447CEF05D231CC0D1BAD6125B3257909442957E28B1B293FC'
...
function Assert-Pin([string]$Path, [string]$Sha256) {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "S19B1 dependency is absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "S19B1 dependency changed: $Path"
}
...
foreach ($path in @($identityPath,$publicCertificatePath,$packageTester,$intentTool,$preactionTool,$pathTool)) { Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "S19B1 input is absent: $path" }
...
$expectedRelativePaths = @(
    ...
    'identity/proposals/62619-433_20260824005735_Slot19/scribe/multi_channel/MULTI_CHANNEL_READER_SUMMARY.json'
)
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'DATA_PULL' -and [string]$definition.parameters.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'S19B1 DATA_PULL route changed.'
Assert-True ($relativePaths.Count -eq 2 -and @(Compare-Object -ReferenceObject $expectedRelativePaths -DifferenceObject $relativePaths).Count -eq 0) 'S19B1 exact relative path set changed.'
Assert-True ([int]$definition.parameters.maximumFiles -eq 2 -and [int64]$definition.parameters.maximumBytes -eq 4194304 -and [int64]$definition.maxResultBytes -eq 4194304) 'S19B1 bounds changed.'
...
Assert-True ([string]$routeHealth.state -eq 'PASS_O2D13_COMPLETE_ROUTE_GATE' -and [bool]$routeHealth.publicationAuthorized -and -not [bool]$routeHealth.retryAuthorized) 'S19B1 inherited route health changed.'
Assert-True ([int]$routeHealth.unresolvedEarlierAcceptedRequestCount -eq 0 -and [bool]$routeHealth.argosInboundRelayCurrentHealthProved) 'S19B1 route has unresolved earlier work.'
Assert-True ([string]$routeHealth.endpointWorkerSha256 -eq $expectedWorkerSha256 -and [string]$routeHealth.installedEndpointConfigSha256 -eq $expectedConfigSha256) 'S19B1 installed route pins changed.'
Assert-True ([string]$terminal.state -eq 'PASS_O2D13_SIGNED_SLOT18_DEVELOPMENT_RESULT' -and [bool]$terminal.signedResponseVerified) 'S19B1 predecessor terminal evidence changed.'
...
Assert-True ($currentBranch -eq $branch -and $localTip -eq $remoteTip) 'S19B1 requires matching local/origin branch tips.'

foreach ($path in @($partialRoot,$signedRoot,$finalRoot,$packageGatePath,$routeGatePath,$pathSidecar)) { Assert-True (-not (Test-Path -LiteralPath $path)) "S19B1 fresh output exists: $path" }
...
$routePaths = @(
    ...
    'C:\S19B1R.zip',
    'C:\S19B1R\PORTAL_RESPONSE_MANIFEST.json',
    'C:\S19B1R\DATA_PULL_PAYLOAD.zip',
    ('C:\S19B1D\' + $longestRelative)
)
$pathGate = & $pathTool -CandidatePath $routePaths -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'S19B1 complete route path budget failed.'
...
Assert-True ([bool]$certificate.HasPrivateKey) 'S19B1 signer private key is unavailable.'

if ($Preflight) {
    [ordered]@{schema='argos_s19b1_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19B1_BUILD_PREFLIGHT';requestId=$requestId;relativePathCount=2;maximumFiles=2;maximumBytes=4194304;routePathCount=$routePaths.Count;maximumEffectiveLength=$maximumEffective;maximumComponentLength=$maximumComponent;branch=$branch;localTip=$localTip;remoteTip=$remoteTip;mutationsPerformed=$false;jbodContacted=$false;imageBytesRequested=0;slots22Through25Exposed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
Assert-True ([string]$folderTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'S19B1 signed folder validation failed.'
...
Assert-True ([string]$extractTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE' -and @(Get-ChildItem -LiteralPath $extractRoot -File).Count -eq 2) 'S19B1 exact ZIP extraction validation failed.'
...
[ordered]@{schema='argos_s19b1_build_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S19B1_EXACT_SIGNED_DATA_PULL_READY';requestId=$requestId;requestZip=$finalZip;requestZipBytes=$zipBytes;requestZipSha256=$zipSha256;packageGate=$packageGatePath;routeGate=$routeGatePath;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
