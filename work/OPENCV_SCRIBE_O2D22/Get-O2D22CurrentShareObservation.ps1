...
$priorResponse = Join-Path $responsesRoot 'R_0625466C6A6C_20260827024747655_661acb16.ready.zip'
$observationPath = Join-Path $PSScriptRoot 'O2D22_CURRENT_SHARE_OBSERVATION.json'
...
function Write-NewJson([string]$Path, [object]$Value) {
    Assert-True (-not (Test-Path -LiteralPath $Path)) "O2D22 create-new share evidence exists: $Path"
...
Assert-True ($null -ne $logical) 'O2D22 persistent U: logical disk is absent.'
$displayRoot = ([string]$drive.DisplayRoot).TrimEnd('\')
$providerRoot = ([string]$logical.ProviderName).TrimEnd('\')
Assert-True ($displayRoot.Equals($uncRoot, [StringComparison]::OrdinalIgnoreCase)) 'O2D22 U: PowerShell DisplayRoot changed.'
Assert-True ($providerRoot.Equals($uncRoot, [StringComparison]::OrdinalIgnoreCase) -and [int]$logical.DriveType -eq 4) 'O2D22 U: persistent logical-disk mapping changed.'
Assert-True (Test-Path -LiteralPath $requestsRoot -PathType Container) 'O2D22 requests root is unreadable.'
Assert-True (Test-Path -LiteralPath $responsesRoot -PathType Container) 'O2D22 responses root is unreadable.'

$requestFiles = @(Get-ChildItem -LiteralPath $requestsRoot -File -Force -ErrorAction Stop | Select-Object -First 1001)
Assert-True ($requestFiles.Count -le 1000) 'O2D22 top-level request observation exceeded 1000 files.'
...
Assert-True ($readyRows.Count -eq 0 -and $uploadRows.Count -eq 0) 'O2D22 share has a pending request or upload.'
Assert-True (-not (Test-Path -LiteralPath $target) -and -not (Test-Path -LiteralPath $upload)) 'O2D22 target request or upload already exists.'
Assert-True (Test-Path -LiteralPath $priorResponse -PathType Leaf) 'O2D22 prior O2D21 response is absent.'
Assert-True ((Get-Item -LiteralPath $priorResponse).Length -eq 3686) 'O2D22 prior O2D21 response byte count changed.'
Assert-True ((Get-Sha256 $priorResponse) -eq '8A249AD8ACDFCFBA75F2815FF1EFCC1B0A9762C447BB9DC7A777F39629FCF491') 'O2D22 prior O2D21 response hash changed.'
...
Assert-True ([string]$aliasBudget.state -eq 'PASS_PATH_BUDGET' -and @($aliasBudget.candidates).Count -eq 4) 'O2D22 alias path budget failed.'
...
$observation = [ordered]@{
    schema='argos_o2d22_current_share_observation_v1';observedUtc=$now;state='PASS_O2D22_CURRENT_SHARE_ZERO_PENDING'
    ...
    priorO2D21ResponsePresent=$true;priorO2D21ResponseBytes=3686;priorO2D21ResponseSha256='8A249AD8ACDFCFBA75F2815FF1EFCC1B0A9762C447BB9DC7A777F39629FCF491'
...
$aliasGate = [ordered]@{
    schema='argos_o2d22_publication_alias_gate_v1';createdUtc=$now;state='PASS_O2D22_EXACT_INSPECTIONREVS_U_ALIAS_GATE'
...
if ($Preflight) {
    [ordered]@{schema='argos_o2d22_current_share_observation_preflight_v1';checkedUtc=$now;state='PASS_O2D22_CURRENT_SHARE_OBSERVATION_PREFLIGHT';pendingRequestCount=0;targetAbsent=$true;uploadAbsent=$true;persistentMappingVerified=$true;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 5
    return
}

Write-NewJson $observationPath $observation
Write-NewJson $aliasGatePath $aliasGate
[ordered]@{schema='argos_o2d22_current_share_observation_result_v1';createdUtc=$now;state='PASS_O2D22_CURRENT_SHARE_OBSERVATION_WRITTEN';observationSha256=Get-Sha256 $observationPath;aliasGateSha256=Get-Sha256 $aliasGatePath;mutationsPerformed=$false;mappingRemoved=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 5
