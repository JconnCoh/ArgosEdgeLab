...
$historicalCompletedRowsIncluded=0
foreach ($catalogAcquisition in @($catalog.acquisitions)) {
    ...
    $resultFingerprintState='CURRENT_ACQUISITION_FINGERPRINT'
    if($currentRows.Count-eq0){
        if([string]$catalogAcquisition.domain-eq'FRONTSIDE' -and $identityRows.Count-eq1){
            $currentRows=@($identityRows)
            $resultFingerprintState='HISTORICAL_COMPLETED_SUPERSEDED_CURRENT_FINGERPRINT'
            $historicalCompletedRowsIncluded++
        }elseif($identityRows.Count-gt0){
    ...
    $included.Add([pscustomobject]@{
        ...
        resultFingerprintState=$resultFingerprintState
        currentAcquisitionFingerprint=$fingerprint
        ledgerJobKey=[string]$ledgerRow.jobKey
    })
}
...
if ($included.Count -eq 0) {
    Write-AtomicJson $readinessPath ([ordered]@{
        ...
        historicalCompletedRowsIncluded=$historicalCompletedRowsIncluded
        reviewOnly=$true
        xmlExportEnabled=$false
    })
    return
    ...
    foreach ($entry in @($group.Group | Sort-Object { [string]$_.acquisition.slot })) {
        ...
        $waferRows.Add([ordered]@{
            ...
            metadata=[ordered]@{
                ...
                resultFingerprintState=[string]$entry.resultFingerprintState
                currentAcquisitionFingerprint=[string]$entry.currentAcquisitionFingerprint
                ledgerJobKey=[string]$entry.ledgerJobKey
                reviewOnly='true'
