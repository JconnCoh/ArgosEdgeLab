...
function Verify-Package {
    ...
    Assert-True ([string]$package.schema -eq 'argos_o2a2_direct_admin_package_manifest_v1') 'O2A2 package schema changed.'
    Assert-True ([string]$package.revision -eq 'O2A2') 'O2A2 package revision changed.'
    Assert-True ([bool]$package.reviewOnly -and -not [bool]$package.productionRoutingEnabled) 'O2A2 package safety flags changed.'
...
function Read-Invocation {
    ...
    Assert-True ([string]$value.schema -eq 'argos_o2a2_direct_admin_invocation_v1') 'O2A2 invocation schema changed.'
    Assert-True ([bool]$value.rehearsal -eq $IsRehearsal) 'O2A2 invocation rehearsal mode changed.'
    Assert-True ([bool]$value.reviewOnly -and -not [bool]$value.productionRoutingEnabled) 'O2A2 invocation safety flags changed.'
    Assert-True ([string]$value.incidentId -eq 'OCV02_O2D4_TERMINAL_STATE_20260825') 'O2A2 incident identity changed.'
    Assert-True ([string]$value.requestId -eq 'REQ_O2D4') 'O2A2 request identity changed.'
    Assert-True ([string]$value.expectedResponsePrefix -eq 'R_A2A87054A416_') 'O2A2 response prefix changed.'
    Assert-True (@($value.pathSources).Count -ge 1 -and @($value.pathSources).Count -le 32) 'O2A2 path-source count is outside 1..32.'
    return $value
}
...
function Get-PathRows {
    ...
    Assert-True ($sourceId -match '^[A-Z0-9_]{1,32}$') "Unsafe O2A2 source id: $sourceId"
    $rootText = [string]$Source.root
    Assert-True (-not [string]::IsNullOrWhiteSpace($rootText)) "O2A2 source root is empty: $sourceId"
    Assert-True ($rootText.IndexOfAny([char[]]'*?') -lt 0) "O2A2 source root contains a wildcard: $sourceId"
    Assert-True ([IO.Path]::IsPathRooted($rootText)) "O2A2 source root is not absolute: $sourceId"
    $root = [IO.Path]::GetFullPath($rootText).TrimEnd('\')
    $selection = [string]$Source.selection
    Assert-True ($selection -in @('EXACT_PATH', 'CHILD_NAME_EXACT', 'CHILD_NAME_PREFIX')) "O2A2 source selection changed: $sourceId"
    $maximumDepth = [int]$Source.maximumDepth
    $maximumRows = [int]$Source.maximumRows
    Assert-True ($maximumDepth -ge 0 -and $maximumDepth -le 8) "O2A2 source depth is outside 0..8: $sourceId"
    Assert-True ($maximumRows -ge 1 -and $maximumRows -le 1000) "O2A2 source row bound is outside 1..1000: $sourceId"
    ...
    if ($selection -eq 'EXACT_PATH') {
        ...
        if (Test-Path -LiteralPath $root -PathType Container) {
            ...
            Assert-True (-not [string]::IsNullOrWhiteSpace($matchText)) "O2A2 child match is empty: $sourceId"
...
function Invoke-O2A2Preflight {
    ...
    if (-not $IsRehearsal) {
        foreach ($blocked in @($Invocation.refuseComputerNames)) {
            Assert-True (-not $env:COMPUTERNAME.Equals([string]$blocked, [StringComparison]::OrdinalIgnoreCase)) "O2A2 refuses this computer: $env:COMPUTERNAME"
        }
        $identity = [Security.Principal.WindowsIdentity]::GetCurrent()
        $principal = New-Object Security.Principal.WindowsPrincipal($identity)
        Assert-True ($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) 'O2A2 live observation requires an elevated administrator token.'
        $configPath = Join-Path ([string]$Invocation.portalRoot) 'config\endpoint_jbod.json'
        Assert-True (Test-Path -LiteralPath $configPath -PathType Leaf) 'JBOD endpoint config is absent; O2A2 refuses a non-JBOD host.'
        $config = Get-Content -LiteralPath $configPath -Raw | ConvertFrom-Json
        Assert-True ([string]$config.schema -eq 'argos_project_portal_endpoint_config_v1' -and [string]$config.role -eq 'JBOD') 'O2A2 endpoint config is not the JBOD role.'
        Assert-True ([bool]$config.reviewOnly -and -not [bool]$config.productionRoutingEnabled) 'O2A2 endpoint config safety flags changed.'
    }
    ...
    Assert-True (-not (Test-Path -LiteralPath $outputRoot)) "O2A2 output root must be fresh: $outputRoot"
    Assert-True (-not (Test-Path -LiteralPath $localZip)) "O2A2 local result ZIP must be fresh: $localZip"
    Assert-True (-not (Test-Path -LiteralPath $returnPath)) "O2A2 return ZIP must be fresh: $returnPath"
    Assert-True (Test-Path -LiteralPath (Split-Path -Parent $returnPath) -PathType Container) 'O2A2 return root is unavailable.'
    Assert-True ([string]$Invocation.localResultLeaf -eq 'O2A2R_LOCAL.zip') 'O2A2 local result leaf changed.'
    Assert-True ((Split-Path -Leaf $returnPath) -eq 'O2A2R.zip') 'O2A2 return leaf changed.'
    return [pscustomobject]@{
        schema = 'argos_o2a2_direct_admin_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_O2A2_DIRECT_ADMIN_READ_ONLY_PREFLIGHT'
...
$preflightResult = Invoke-O2A2Preflight -Invocation $invocation -PackageRoot $packageRoot -IsRehearsal ([bool]$Rehearsal)
if ($Preflight) {
...
$transcriptPath = Join-Path $outputRoot 'O2A2_TRANSCRIPT.log'
...
try {
    ...
    Assert-True ($copyCandidates.Count -le [int]$invocation.maximumCopyFiles) 'O2A2 safe-evidence copy count exceeded its bound.'
    [int64]$copyBytes = 0
    foreach ($copyCandidate in $copyCandidates.ToArray()) {
        $copyBytes += [int64]$copyCandidate.bytes
    }
    Assert-True ($copyBytes -le [int64]$invocation.maximumCopyBytes) 'O2A2 safe-evidence copy bytes exceeded its bound.'
    ...
    foreach ($candidate in $copyCandidates.ToArray()) {
        ...
        Assert-True (-not (Test-Path -LiteralPath $destination)) "O2A2 evidence destination collision: $destination"
        [IO.File]::Copy([string]$candidate.sourcePath, $destination, $false)
        Assert-True ((Get-Sha256 $destination) -eq [string]$candidate.sha256) "O2A2 evidence copy hash changed: $destination"
    ...
    $observation = [ordered]@{
        schema = 'argos_o2a2_direct_admin_observation_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = if ($truncatedSources.Count -eq 0 -and $accessErrorCount -eq 0) { 'PASS_O2A2_DIRECT_ADMIN_READ_ONLY_OBSERVATION' } else { 'HOLD_O2A2_OBSERVATION_INCOMPLETE' }
    ...
    $observationPath = Join-Path $outputRoot 'O2A2_OBSERVATION.json'
    ...
    $failurePath = Join-Path $outputRoot 'O2A2_FAILURE.json'
    if (-not (Test-Path -LiteralPath $failurePath)) {
        Write-Utf8CreateNewJson -Path $failurePath -Value ([ordered]@{
            schema = 'argos_o2a2_direct_admin_failure_v1'
            createdUtc = [DateTime]::UtcNow.ToString('o')
            state = 'FAIL_O2A2_DIRECT_ADMIN_READ_ONLY_OBSERVATION'
    ...
    if (Test-Path -LiteralPath $returnPath -PathType Leaf) {
        Assert-True ((Get-Sha256 $returnPath) -eq (Get-Sha256 $localResultZip)) 'O2A2 returned ZIP hash changed.'
    }
}

Assert-True $completed 'O2A2 observation did not complete.'
[ordered]@{
    schema = 'argos_o2a2_direct_admin_return_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O2A2_DIRECT_ADMIN_READ_ONLY_OBSERVATION_RETURNED'
