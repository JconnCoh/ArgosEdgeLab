...
$partialRoot = Join-Path $PSScriptRoot 'final2.partial'
...
$negativeArguments = @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $extractedAudit, '-InvocationManifest', $extractedInvocation, '-Preflight')
$priorPreference = $ErrorActionPreference
try {
    $ErrorActionPreference = 'Continue'
    $negativeRows = @(& $windowsPowerShell @negativeArguments 2>&1)
