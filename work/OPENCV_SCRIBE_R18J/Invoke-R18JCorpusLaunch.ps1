...
param(
    ...
    [switch]$Rehearsal,
    [string]$PayloadRoot = '',
    [string]$WorkRoot = 'D:\A2\w\ocv\R18J1',
    [string]$OutputRoot = 'D:\A2\o\ocv\R18J1',
    [string]$SourceRoot = 'D:\KLARFExport',
    [string]$ProposalRoot = 'D:\AFCV1\review\identity\proposals',
...
$manifestSha = 'BF2D0E809915ADEB503E2B7ADEFB3DCAC30903025D851B26067C9FB5B90DC57D'
...
$runnerSha = 'E8C193024317C683EA0E8DC999F3D632BF113B65C1E6F528949AE5B33F83A069'
...
$payloadManifestPath = Join-Path $payload 'R18J_PAYLOAD_MANIFEST.json'
$partial = $WorkRoot + '.partial'
$failed = $WorkRoot + '.failed'
$runtimeConfiguration = Join-Path $WorkRoot 'RUNTIME_CONFIGURATION.json'
$runner = Join-Path $WorkRoot 'OPENCV_SCRIBE_R18J\Run-R18JScribeCorpus.py'
...
function Get-CorpusRows {
    return @(Get-CimInstance Win32_Process -Filter "Name='python.exe' OR Name='pythonw.exe'" -ErrorAction Stop |
        Where-Object { [string]$_.CommandLine -like '*Run-R18JScribeCorpus.py*' } |
        Sort-Object ProcessId |
        ForEach-Object { [pscustomobject]@{processId=[uint32]$_.ProcessId;creationDate=[string]$_.CreationDate;commandLine=[string]$_.CommandLine} })
}
...
foreach ($path in @($payloadManifestPath,$PythonPath,$ReferenceBundlePath,$SourceRoot,$ProposalRoot)) {
...
Require ([string]$manifest.schema -eq 'argos_opencv_scribe_r18j_payload_manifest_v1' -and [string]$manifest.revision -eq $revision) 'R18J payload manifest contract changed.'
...
foreach ($path in @($WorkRoot,$partial,$failed,$OutputRoot,$runtimeConfiguration,$runner,$launchPath,$failurePath,(Join-Path $OutputRoot 'c\0123456789ABCDEF\fallback\crops\R02_DF_SCRIBE_ORIENTED_DETECTOR_INPUT.png'))) {
    Assert-PathBudget $path 32
}
...
if ($Preflight) {
    [ordered]@{
        schema='argos_opencv_scribe_r18j_launch_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
        state='PASS_R18J_CORPUS_LAUNCH_PREFLIGHT';revision=$revision;rehearsal=[bool]$Rehearsal;
        payloadFileCount=$files.Count;payloadManifestSha256=$manifestSha;pythonSha256=$ExpectedPythonSha256;
        referenceBundleSha256=$refsSha;sourceRoot=$SourceRoot;proposalRoot=$ProposalRoot;workRoot=$WorkRoot;
...
try {
    ...
    $launch = [ordered]@{
        schema='argos_opencv_scribe_r18j_corpus_launch_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
        state='PASS_R18J_CORPUS_WORKER_STARTED';revision=$revision;computerName=$env:COMPUTERNAME;
        ...
        sourceRoot=$SourceRoot;proposalRoot=$ProposalRoot;payloadManifestSha256=$manifestSha;providerSha256=$providerSha;
    ...
    if ((Test-Path -LiteralPath $OutputRoot -PathType Container) -and -not (Test-Path -LiteralPath $failurePath)) {
        Write-JsonNew $failurePath ([ordered]@{schema='argos_opencv_scribe_r18j_launch_failure_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='HOLD_R18J_CORPUS_LAUNCH_FAILURE';detail=$detail;ownedChildTerminated=$true;automaticRetryAllowed=$false;sourceMutationPerformed=$false;identityAccepted=$false;reviewOnly=$true;productionRoutingEnabled=$false}) 8
    }
    throw
}
