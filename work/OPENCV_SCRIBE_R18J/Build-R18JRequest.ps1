...
function Require-Pin([string]$Path, [string]$Sha256, [string]$State = '') {
    Require (Test-Path -LiteralPath $Path -PathType Leaf) "R18J build dependency absent: $Path"
    Require ((Get-Sha256 $Path) -eq $Sha256) "R18J build dependency changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($State)) {
        $value = Get-Content -Raw -LiteralPath $Path | ConvertFrom-Json
        Require ([string]$value.state -eq $State) "R18J build dependency state changed: $Path"
    }
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 32) {
    Require (-not (Test-Path -LiteralPath $Path)) "R18J build create-new JSON exists: $Path"
...
$revision = 'R18J_FULL_KLARF_JBOD_REVIEW_ONLY_20260904A'
$entrypoint = Join-Path $PSScriptRoot 'Invoke-R18JCorpusLaunch.ps1'
$payloadManifestPath = Join-Path $PSScriptRoot 'R18J_PAYLOAD_MANIFEST.json'
$definitionPath = Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION.json'
$localGatePath = Join-Path $PSScriptRoot 'R18J_CORPUS_LOCAL_GATE.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18J2_PATH_GATE.json'
...
$finalGatePath = Join-Path $PSScriptRoot 'R18J_FINAL_PACKAGE_GATE.json'
...
$definitionSha = '8FDEA2DF772703011B82B797161330B829CA39DD76D424E8CC373F434B247A56'
...
Require (Test-Path -LiteralPath $localSource -PathType Container) 'R18J local rehearsal source absent.'
Require (Test-Path -LiteralPath $localProposals -PathType Container) 'R18J local rehearsal proposal root absent.'
...
Require ([string]$preactionResult.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'R18J preaction gate changed.'
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Require ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH' -and [string]$definition.entryPoint -eq 'payload/Invoke-R18JCorpusLaunch.ps1') 'R18J maintenance route changed.'
Require (@($definition.changes).Count -eq 1 -and [string]$definition.changes[0].installedSha256 -eq $entrypointSha -and [bool]$definition.changes[0].allowCreate) 'R18J maintenance change changed.'
Require (@($definition.changes[0].approvedPredecessorSha256).Count -eq 1 -and [string]$definition.changes[0].approvedPredecessorSha256[0] -eq $entrypointSha) 'R18J create-only idempotent installed-hash boundary changed.'
Require (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R18J process/task action cardinality changed.'
Require ([bool]$definition.reviewOnly -and -not [bool]$definition.productionRoutingEnabled -and -not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed) 'R18J authority changed.'
$payloadManifest = Get-Content -Raw -LiteralPath $payloadManifestPath | ConvertFrom-Json
Require ([string]$payloadManifest.schema -eq 'argos_opencv_scribe_r18j_payload_manifest_v1' -and [string]$payloadManifest.revision -eq $revision -and @($payloadManifest.files).Count -eq 23) 'R18J payload manifest shape changed.'
...
foreach ($file in $payloadFiles) {
    ...
    Require ((Get-Item -LiteralPath $source).Length -eq [int64]$file.bytes) "R18J payload length changed: $($file.sourcePath)"
}
...
try {
    ...
    Require ($matches.Count -eq 1 -and $matches[0].HasPrivateKey) 'R18J signer certificate or private key changed.'
...
foreach ($path in @($stageRoot,$readyRoot,$stageZip,$verifyRoot,$finalRoot,$finalPartial,$finalGatePath)) {
    Require (-not (Test-Path -LiteralPath $path)) "R18J build fresh output exists: $path"
}
if ($Preflight) {
    [ordered]@{schema='argos_opencv_scribe_r18j_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R18J_BUILD_PREFLIGHT';requestId=$requestId;payloadFileCount=25;entrypointSha256=$entrypointSha;payloadManifestSha256=$payloadManifestSha;definitionSha256=$definitionSha;localGateSha256=$localGateSha;pathGateSha256=$pathGateSha;preactionSha256=$preactionSha;signerThumbprint=$thumbprint;mutationsPerformed=$false;targetExecuted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
Require ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18J signed package verification failed.'
...
Require ($extracted.Count -eq 27) 'R18J extracted final ZIP file count changed.'
foreach ($row in $manifestFiles) {
    $path = Join-Path $verifyRoot ([string]$row.path).Replace('/','\')
    Require-Pin $path ([string]$row.sha256)
    Require ((Get-Item -LiteralPath $path).Length -eq [int64]$row.bytes) "R18J extracted payload length changed: $($row.path)"
}
...
Require ([string]$exactPackageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18J extracted signed package verification failed.'
$packagedPreflight = (& (Join-Path $verifyRoot 'payload\Invoke-R18JCorpusLaunch.ps1') -Preflight -Rehearsal -PayloadRoot (Join-Path $verifyRoot 'payload') -WorkRoot 'C:\R18JFW2' -OutputRoot 'C:\R18JFO2' -SourceRoot $localSource -ProposalRoot $localProposals -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localRefs -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
Require ([string]$packagedPreflight.state -eq 'PASS_R18J_CORPUS_LAUNCH_PREFLIGHT' -and -not [bool]$packagedPreflight.mutationsPerformed -and -not [bool]$packagedPreflight.processStarted) 'R18J exact packaged entrypoint preflight failed.'
...
$finalGate = [ordered]@{
    schema='argos_opencv_scribe_r18j_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
    state='PASS_R18J_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip=('work/OPENCV_SCRIBE_R18J/final/'+$zipName);
