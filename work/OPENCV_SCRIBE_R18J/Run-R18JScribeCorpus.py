#!/usr/bin/env python3
"""Review-only, resumable scribe deciphering over the JBOD KLARF corpus.

Existing processor scribe crops are evaluated first.  Only a missing, blank,
or structurally unusable crop falls back to the R18J bounded whole-wafer
source-pixel crop sweep.  The frozen R18H reader and reference bytes are never
changed.  Results are proposals/holds only and cannot accept an identity.
"""
...
REVISION = "ARGOS_OPENCV_SCRIBE_R18J_FULL_KLARF_REVIEW_ONLY_20260903"
...
def run_case(
    ...
    fallback_states = {
        ...
        "HOLD_SCRIBE_GRID_BOUNDARY_INCOMPLETE",
        "HOLD_SCRIBE_IMAGE_SCORE_BELOW_FROZEN_MINIMUM",
    }
    if existing["state"] not in fallback_states:
        selected = existing
        fallback = None
    else:
        bf_path, df_path = Path(str(case["bfPath"])), Path(str(case["dfPath"]))
        bf_sha, df_sha = sha256_file(bf_path), sha256_file(df_path)
        job = whole_wafer_job(config, case, bf_sha, df_sha)
        fallback_root = case_root / "fallback"
        fallback = r18j.analyze(job, Path(str(config["providerPath"])), fallback_root)
        selected = fallback
    return {
        "schema": "argos_opencv_scribe_r18j_corpus_case_v1",
        "createdUtc": utc_now(),
...
def main(argv: Iterable[str]) -> int:
    ...
    source_root = Path(str(config["sourceRoot"]))
    if not source_root.is_dir():
        raise ValueError(f"KLARF source root absent: {source_root}")
    cases, inventory_holds = discover_source_pairs(source_root)
    if not cases:
        raise ValueError("No paired frontside BF/DF full-wafer sources were discovered.")
    inventory_path = output_root / "INVENTORY.json"
    write_json_new(inventory_path, {
        "schema": "argos_opencv_scribe_r18j_corpus_inventory_v1",
        "createdUtc": utc_now(), "sourceRoot": str(source_root),
        "caseCount": len(cases), "cases": cases, "holds": inventory_holds,
        "sourceImagesRead": False, "reviewOnly": True,
    })
    ...
    complete = {
        "schema": "argos_opencv_scribe_r18j_corpus_complete_v1",
        "createdUtc": utc_now(), "state": "PASS_R18J_FULL_KLARF_REVIEW_ONLY_COMPLETE",
