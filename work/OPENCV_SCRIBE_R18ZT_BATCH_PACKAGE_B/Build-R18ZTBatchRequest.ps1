...
if ([string]$PSVersionTable.PSEdition -ne 'Desktop' -or $PSVersionTable.PSVersion.Major -ne 5 -or $PSVersionTable.PSVersion.Minor -ne 1) {
    throw 'R18ZT package B builder requires Windows PowerShell 5.1 exactly.'
}
...
function Require-Pin([string]$Path, [string]$Sha256, [int64]$Bytes = -1, [string]$State = '') {
    Require ($Sha256 -cmatch '^[A-F0-9]{64}$') "R18ZT package B pin is not final: $Path"
    Require (Test-Path -LiteralPath $Path -PathType Leaf) "R18ZT package B dependency absent: $Path"
    if ($Bytes -ge 0) { Require ((Get-Item -LiteralPath $Path).Length -eq $Bytes) "R18ZT package B dependency length changed: $Path" }
    Require ((Get-Sha256 $Path) -eq $Sha256) "R18ZT package B dependency hash changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($State)) {
        $record = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
        Require ([string]$record.state -eq $State) "R18ZT package B dependency state changed: $Path"
    }
}
...
$revision = 'R18ZT_EXISTING_ORIENTED_CROPS_ASYNC_REVIEW_ONLY_20260906B'
$runnerRevision = 'R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260906C'
$bindingPath = Join-Path $PSScriptRoot 'R18ZT_FROZEN_PACKAGE_BINDINGS_V3.json'
...
$definitionPath = Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION_V2.json'
$configurationPath = Join-Path $PSScriptRoot 'R18ZT_BATCH_CONFIGURATION.json'
$designPath = Join-Path $PSScriptRoot 'R18ZT_BATCH_PACKAGE_DESIGN_V3.json'
$importSmokePath = Join-Path $PSScriptRoot 'Test-R18ZTPackagedImportClosure.py'
$cloneRemediationPath = Join-Path $PSScriptRoot 'R18ZT_PACKAGE_B_CLONE_REMEDIATION_V2.json'
$publisherCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZT_PUBLISHER_CLONE_REMEDIATION.json'
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZT_PACKAGE_B_CLONE_LITERAL_RAW_GATE_V2.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZT_PACKAGE_B_CLONE_GATE_V2.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZT_PATH_PLAN_GATE_V2.json'
...
$junctionGatePath = Join-Path $PSScriptRoot 'R18ZT_PROPOSAL_JUNCTION_GATE.json'
$recoveryIntentPath = Join-Path $PSScriptRoot 'R18ZT_RECOVERY_INTENT.json'
...
$bindingSha = 'FF7B2C75B0A3C7DD29A5249D92E947424DDB407243B52C5B04FAC36DCF9AE3F1'

Require-Pin $bindingPath $bindingSha -1 'FROZEN_R18ZT_PACKAGE_B_V3_BINDINGS'
$binding = Get-Content -LiteralPath $bindingPath -Raw | ConvertFrom-Json
Require ([string]$binding.schema -eq 'argos_opencv_scribe_r18zt_package_bindings_v3' -and [string]$binding.requestId -eq $requestId -and [string]$binding.packageRevision -eq $revision -and [string]$binding.runnerRevision -eq $runnerRevision) 'R18ZT V3 binding identity changed.'
...
$requiredArtifactNames = @(
    ...
    'packageCloneRemediation','publisherCloneRemediation','packageV1Withdrawal','packageV2Withdrawal','executionEnvelope','executionEnvelopeTest','batchRunner','batchRunnerTest','batchRunnerBWithdrawal','provider','r11Analyzer',
...
Require ($resolvedArtifacts['packageCloneRemediation'] -eq [IO.Path]::GetFullPath($cloneRemediationPath) -and $resolvedArtifacts['publisherCloneRemediation'] -eq [IO.Path]::GetFullPath($publisherCloneRemediationPath)) 'R18ZT clone-remediation binding paths changed.'
...
function Read-BoundGate([string]$Path, [string]$State) {
    Require (Test-Path -LiteralPath $Path -PathType Leaf) "R18ZT package B gate absent: $Path"
    $record = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
    Require ([string]$record.state -eq $State -and [string]$record.requestId -eq $requestId -and [string]$record.bindingRecordSha256 -eq $bindingSha) "R18ZT package B gate binding changed: $Path"
    return $record
}

$cloneGate = Read-BoundGate $cloneGatePath 'PASS_R18ZT_PACKAGE_B_CLONE_LITERAL_GATE'
$pathGate = Read-BoundGate $pathGatePath 'PASS_R18ZT_PACKAGE_B_PATH_BUDGET'
$powerShellGate = Read-BoundGate $powerShellGatePath 'PASS_R18ZT_PACKAGE_B_POWERSHELL_SAFETY'
...
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/Invoke-R18ZTBatchLaunch.ps1' = [string]($artifactRows | Where-Object name -eq 'launcher').sha256
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/Build-R18ZTBatchRequest.ps1' = $builderSha
    'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/Run-R18ZTBatchExecutionEnvelope.py' = [string]($artifactRows | Where-Object name -eq 'executionEnvelope').sha256
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/R18ZT_PACKAGE_B_CLONE_LITERAL_RAW_GATE_V2.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
Require-Pin $cloneRawGatePath ([string]$cloneGate.rawUtilityGateSha256) -1 'PASS_ARGOS_CLONE_LITERAL_REMEDIATION'
$cloneRemediation = Get-Content -LiteralPath $cloneRemediationPath -Raw | ConvertFrom-Json
$rawCloneGate = Get-Content -LiteralPath $cloneRawGatePath -Raw | ConvertFrom-Json
Require ([string]$rawCloneGate.manifestSha256 -eq (Get-Sha256 $cloneRemediationPath) -and [int]$rawCloneGate.pairCount -eq $expectedClonePairs.Count -and @($rawCloneGate.violations).Count -eq 0 -and [bool]$rawCloneGate.metadataOnly -and -not [bool]$rawCloneGate.targetExecuted) 'R18ZT raw clone utility gate changed.'
...
foreach ($scriptPin in @([pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/Invoke-R18ZTBatchLaunch.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256}, [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/Build-R18ZTBatchRequest.ps1';sha=$builderSha})) {
...
Require ([bool]$finish.exact -and [string]$finish.criterion -ceq 'EXACT_IMAGE_FIRST_STRING_WITH_DEFENSIBLE_GENERIC_SUPPORT_AND_ZERO_WRONG_ACCEPTED' -and [string]$finish.imageFirstString -ceq '13HFX135SUE3' -and [string]$finish.truth -ceq '13HFX135SUE3' -and [string]$finish.selectedChannel -ceq 'BF' -and [string]$finish.selectedPolarity -ceq 'DARK' -and [string]$finish.selectedDirection -ceq 'FORWARD' -and [double]$finish.selectionScore -eq [double]0.9117836040446633 -and [bool]$finish.selectedGlyphEnvelopePassed -and [int]$finish.correctImageFirstCount -eq 1 -and [int]$finish.wrongImageFirstAcceptedCount -eq 0 -and [int]$finish.developmentAcceptedWrongCount -eq 0 -and [int]$finish.validationCaseCount -eq 1 -and [bool]$finish.truthComparedOnlyAfterProviderResult) 'R18ZT exact deciphering finish changed.'
...
Require ([string]$recoveryIntent.state -eq 'FROZEN_R18ZT_PACKAGE_B_RECOVERY_INTENT' -and [string]$recoveryIntent.requestId -eq $requestId -and [string]$recoveryIntent.bindingRecordSha256 -eq $bindingSha -and -not [bool]$recoveryIntent.mutation.publicationAuthorized -and -not [bool]$recoveryIntent.mutation.automaticRetryAuthorized) 'R18ZT recovery intent changed.'
...
Require ([string]$preactionResult.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'R18ZT package B preaction utility changed.'
$bindingDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/R18ZT_FROZEN_PACKAGE_BINDINGS_V3.json' })
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/Build-R18ZTBatchRequest.ps1' })
Require ($bindingDependency.Count -eq 1 -and [string]$bindingDependency[0].sha256 -eq $bindingSha -and $builderDependency.Count -eq 1 -and [string]$builderDependency[0].sha256 -eq $builderSha -and [string]$preaction.historyAuditSha256 -eq 'F3E7AF05017BF00ADEDFFA7A06D89155E2E5C1BF76A64E643E4972F00737BC9C') 'R18ZT package B preaction dependencies changed.'
...
if ($Preflight) {
    [ordered]@{
        ...
        state = 'PASS_R18ZT_PACKAGE_B_BUILD_PREFLIGHT'
        requestId = $requestId
...
$stagedPreflight = (& (Join-Path $activePayload 'Invoke-R18ZTBatchLaunch.ps1') -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $activePayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\R18ZT_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
...
if ($Test) {
    ...
    [ordered]@{
        ...
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZT_PACKAGE_B_UNSIGNED_TEST'
        requestId = $requestId
        bindingRecordSha256 = $bindingSha
...
$extractedPreflightText = (& $windowsPowerShell51 -NoProfile -ExecutionPolicy Bypass -File $extractedLauncher -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $extractedPayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\R18ZT_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String)
...
$finalGate = [ordered]@{
    ...
    revision = $revision
    bindingRecordSha256 = $bindingSha
    requestZip = 'work/OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B/final/REQ_R18ZT1.ready.zip'
