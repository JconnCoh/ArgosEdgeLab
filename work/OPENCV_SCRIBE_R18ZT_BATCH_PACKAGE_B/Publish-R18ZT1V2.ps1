...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZT_PUBLICATION_INVOCATION_V2.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v2') 'R18ZT publication invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION') 'R18ZT publication invocation is not frozen; execution refused.'
Require ([string]$invocation.requestId -ceq 'REQ_R18ZT1') 'R18ZT publication request identity changed.'
...
$currentBranch = (& git -C $projectRoot branch --show-current | Out-String).Trim()
$localTip = (& git -C $projectRoot rev-parse HEAD | Out-String).Trim()
$originTip = (& git -C $projectRoot rev-parse ('origin/' + [string]$invocation.git.branch) | Out-String).Trim()
...
Require ($currentBranch -ceq [string]$invocation.git.branch -and $localTip -ceq $originTip -and $statusRows.Count -eq 0) 'R18ZT publication requires the frozen clean dedicated branch matching recorded origin.'
...
Require ([string]$binding.state -ceq [string]$invocation.packageBinding.expectedState -and [string]$binding.requestId -ceq 'REQ_R18ZT1') 'Fresh package binding identity/state changed.'
...
foreach ($consumer in @($invocation.bindingConsumers)) {
    ...
    Require ([string](Get-NestedValue $consumerObject ([string]$consumer.requestIdProperty) "Binding consumer $($consumer.dependency)") -ceq 'REQ_R18ZT1') "Binding consumer request ID changed: $($consumer.dependency)"
}
...
Require ([string](Get-NestedValue $routeGate ([string]$routeContract.requestZipSha256Property) 'R18ZT route gate') -ceq [string]$requestZipForGates.sha256 -and
    [bool](Get-NestedValue $routeGate ([string]$routeContract.allRootsKnownProperty) 'R18ZT route gate') -and
...
Require ([string](Get-NestedValue $scienceGate ([string]$scienceContract.adjudicationSha256Property) 'R18ZT science gate') -ceq 'E526EDF07597D3F71A52FABAD7E93941A0FE3CD34003B6F0595C9D3992234A7D' -and
    [string](Get-NestedValue $scienceGate ([string]$scienceContract.imageFirstStringProperty) 'R18ZT science gate') -ceq '13HFX135SUE3' -and
...
Require ([string]$adjudicationDependency.sha256 -ceq 'E526EDF07597D3F71A52FABAD7E93941A0FE3CD34003B6F0595C9D3992234A7D') 'R18ZT Slot21 adjudication hash changed.'
$adjudicationBindingRows = @($binding.artifacts | Where-Object { [string]$_.name -ceq [string]$invocation.science.adjudicationBindingArtifactName })
Require ($adjudicationBindingRows.Count -eq 1 -and [string]$adjudicationBindingRows[0].sha256 -ceq [string]$adjudicationDependency.sha256) 'Fresh package binding does not mechanically pin the exact Slot21 adjudication.'
$adjudication = Read-JsonFile ([string]$adjudicationDependency.path) 1048576
Require ([string]$adjudication.schema -ceq 'argos_opencv_scribe_r18zt_slot21_post_result_adjudication_gate_v1' -and
    [string]$adjudication.state -ceq 'PASS_R18ZT_SLOT21_EXACT_DECIPHERING_FINISH_POST_RESULT_ADJUDICATION' -and
    [string]$adjudication.scope -ceq 'READ_ONLY_POST_RESULT_ADJUDICATION_NO_PROVIDER_RERUN') 'R18ZT Slot21 adjudication identity changed.'
Require ([bool]$adjudication.authority.operatorExactDecipheringFinishCriterionPassed -and
    [string]$adjudication.decipheringFinish.imageFirstString -ceq '13HFX135SUE3' -and
    [bool]$adjudication.decipheringFinish.exact -and [int]$adjudication.decipheringFinish.wrongImageFirstAcceptedCount -eq 0 -and
    [int]$adjudication.decipheringFinish.developmentAcceptedWrongCount -eq 0 -and
    [string]$adjudication.decipheringFinish.selectedChannel -ceq 'BF' -and
    [string]$adjudication.decipheringFinish.selectedPolarity -ceq 'DARK' -and
    [string]$adjudication.decipheringFinish.selectedDirection -ceq 'FORWARD' -and
    [Math]::Abs([double]$adjudication.decipheringFinish.selectionScore - 0.9117836040446633) -le 0.000000000000001 -and
    [string]$adjudication.decipheringFinish.truth -ceq '13HFX135SUE3' -and
    [bool]$adjudication.decipheringFinish.truthComparedOnlyAfterProviderResult) 'R18ZT exact image-first Slot21 finish evidence changed.'
Require ([int]$adjudication.executedRun.providerRunCount -eq 1 -and [int]$adjudication.executedRun.analyzeImagesCallCount -eq 1 -and
    [int]$adjudication.executedRun.attemptCount -eq 8 -and [bool]$adjudication.executedRun.orderedAttemptKeysExact -and
    [int]$adjudication.executedRun.directStructuralEvaluatorCallsByHarness -eq 0 -and -not [bool]$adjudication.executedRun.retryPerformed -and
    -not [bool]$adjudication.executedRun.providerRerunByThisAdjudication) 'R18ZT Slot21 full-chain execution evidence changed.'
Require (-not [bool]$adjudication.invariants.providerRerunAuthorized -and -not [bool]$adjudication.invariants.providerExecutedByThisAdjudication -and
    -not [bool]$adjudication.invariants.truthOrChecksumUsedForSelection -and -not [bool]$adjudication.invariants.wrongAccepted -and
    -not [bool]$adjudication.authority.publicationSelfAuthority) 'R18ZT Slot21 adjudication authority/invariants changed.'
Require ([bool]$adjudication.runtimeRestoration.analyze -and [bool]$adjudication.runtimeRestoration.apply -and
    [bool]$adjudication.runtimeRestoration.enforce -and [bool]$adjudication.runtimeRestoration.evaluate -and
    [bool]$adjudication.runtimeRestoration.loader -and [bool]$adjudication.runtimeRestoration.revision -and
    [bool]$adjudication.runtimeRestoration.validate -and [bool]$adjudication.runtimeRestoration.sharedLockAvailableAfterRun) 'R18ZT Slot21 runtime restoration/shared-lock evidence changed.'
Require (-not [bool]$adjudication.holdsAndAuthority.coverageHoldCleared -and -not [bool]$adjudication.holdsAndAuthority.eligibleIdentity -and
    @($adjudication.holdsAndAuthority.resultHoldsPreserved).Count -eq 1 -and
    [string]$adjudication.holdsAndAuthority.resultHoldsPreserved[0].code -ceq 'SCRIBE_REFERENCE_COVERAGE_HOLD') 'R18ZT Slot21 coverage hold was not preserved.'

foreach ($sciencePin in @($invocation.science.adjudicationPins)) {
    $scienceDependency = Get-ValidatedDependency ([string]$sciencePin.dependency)
    $rows = @($adjudication.pins | Where-Object { [string]$_.name -ceq [string]$sciencePin.pinName })
    Require ($rows.Count -eq 1 -and [string]$rows[0].path -ceq [string]$scienceDependency.row.path -and
        [string]$rows[0].sha256 -ceq [string]$scienceDependency.sha256) "R18ZT adjudication pin changed: $($sciencePin.pinName)"
    $bindingRows = @($binding.artifacts | Where-Object { [string]$_.name -ceq [string]$sciencePin.bindingArtifactName })
    Require ($bindingRows.Count -eq 1 -and [string]$bindingRows[0].sha256 -ceq [string]$scienceDependency.sha256) "Fresh package binding science pin changed: $($sciencePin.bindingArtifactName)"
}
...
Require ([string]$developmentGate.state -ceq 'PASS_R18ZT_GENERIC_HOLD_RESCUE_FROZEN_BEFORE_VALIDATION' -and
    [bool]$developmentGate.criteria.all475QueriesEvaluated -and [int]$developmentGate.criteria.exactFoldCount -eq 49 -and
    [int]$developmentGate.criteria.acceptedCorrect -eq 312 -and [int]$developmentGate.criteria.acceptedWrong -eq 0 -and
    [int]$developmentGate.leaveOneExactScribeLineageOut.referenceQueries -eq 475 -and
    [int]$developmentGate.leaveOneExactScribeLineageOut.exactFoldCount -eq 49 -and
    [int]$developmentGate.leaveOneExactScribeLineageOut.acceptedCorrect -eq 312 -and
    [int]$developmentGate.leaveOneExactScribeLineageOut.acceptedWrong -eq 0) 'R18ZT frozen 475-query/49-fold development evidence changed.'
Require ([bool]$developmentGate.criteria.k25vDiagnosticHoldPreserved -and [bool]$developmentGate.criteria.k25vRankingAndScoresUnchanged -and
    [bool]$developmentGate.criteria.k25vWrongPositions2And4NotRescued -and -not [bool]$developmentGate.controls.k25v.envelopePassed -and
    [string]$developmentGate.controls.k25v.imageFirstString -ceq '14DCK076SUG1' -and [bool]$developmentGate.controls.k25v.rankingAndScoresUnchanged -and
    -not [bool]$developmentGate.controls.k25v.wrongDiagnosticPositions2And4Rescued -and @($developmentGate.controls.k25v.rescuedPositions).Count -eq 0 -and
    @($developmentGate.controls.k25v.heldPositions | Where-Object { [int]$_.position -eq 2 }).Count -eq 1 -and
    @($developmentGate.controls.k25v.heldPositions | Where-Object { [int]$_.position -eq 4 }).Count -eq 1) 'R18ZT frozen K25V diagnostic hold changed.'
Require ([bool]$developmentGate.invariants.frozenBeforeValidationEvaluation -and [bool]$developmentGate.invariants.truthComparedOnlyAfterImageRanking -and
    -not [bool]$developmentGate.invariants.checksumUsedForSelectionOrAcceptance -and -not [bool]$developmentGate.invariants.lotSlotOrLabelExceptionUsed -and
    -not [bool]$developmentGate.criteria.resultAuthorityExpanded -and -not [bool]$developmentGate.invariants.publicationAuthorized) 'R18ZT development genericity/authority invariants changed.'

$slotResultDependency = Get-ValidatedDependency 'slot21Result'
$slotResult = Read-JsonFile ([string]$slotResultDependency.path) 2097152
Require ([string]$slotResult.state -ceq 'SCRIBE_M12_AMBIGUOUS_MULTIPLE_VALID' -and [string]$slotResult.imageFirstString -ceq '13HFX135SUE3') 'R18ZT frozen Slot21 result changed.'
Require ([string]$slotResult.ambiguityResolution.state -ceq 'PASS_REVIEW_ONLY_IMAGE_FIRST_SCRIBE' -and
    @($slotResult.ambiguityResolution.closeImageFirstStrings).Count -eq 1 -and
    [string]$slotResult.ambiguityResolution.closeImageFirstStrings[0] -ceq '13HFX135SUE3') 'R18ZT Slot21 unique current image-first evidence changed.'
Require (@($slotResult.holds).Count -eq 1 -and [string]$slotResult.holds[0].code -ceq 'SCRIBE_REFERENCE_COVERAGE_HOLD' -and
    -not [bool]$slotResult.eligibleIdentity) 'R18ZT frozen Slot21 result hold/identity boundary changed.'
$slotExecutionGateDependency = Get-ValidatedDependency 'slot21ExecutionGate'
$slotExecutionGate = Read-JsonFile ([string]$slotExecutionGateDependency.path) 1048576
Require ([string]$slotExecutionGate.state -ceq 'HOLD_R18ZT_SLOT21_FINISH_CRITERIA_NOT_MET_NO_RETRY' -and -not [bool]$slotExecutionGate.retryAuthorized) 'R18ZT frozen Slot21 legacy execution gate changed.'
Require ([string]$slotExecutionGate.resultLevel.imageFirstString -ceq '13HFX135SUE3' -and
    [string]$slotExecutionGate.ambiguityResolution.state -ceq 'PASS_REVIEW_ONLY_IMAGE_FIRST_SCRIBE' -and
    -not [bool]$slotExecutionGate.invariants.publicationAuthorized -and -not [bool]$slotExecutionGate.invariants.identityAccepted) 'R18ZT Slot21 legacy gate interpretation changed.'
...
try {
    ...
    Require ([string]$manifest.schema -ceq 'argos_project_portal_request_manifest_v1' -and [string]$manifest.requestId -ceq 'REQ_R18ZT1' -and
    ...
    Require ([string]$change.source -ceq 'payload/Invoke-R18ZTBatchLaunch.ps1' -and
        [string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZT1.ps1' -and
...
$priorTerminalDependency = Get-ValidatedDependency 'priorTerminalCheckpoint'
$priorTerminal = Read-JsonFile ([string]$priorTerminalDependency.path) 1048576
Require ([string]$priorTerminal.state -match '^PASS_' -and [string]$priorTerminal.requestId -ceq 'REQ_S21_20260905212322_S9RHWN0X00G4K59QB7Q1120VDR' -and
    [bool]$priorTerminal.signedResponseVerified) 'The prior same-endpoint request lacks its pinned signed terminal response.'
...
Require ($requestRoot -ceq 'U:\ProjectPortalRO\requests' -and $responseRoot -ceq 'U:\ProjectPortalRO\responses' -and
    $readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZT1.ready.zip' -and $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZT1.ready.zip.upload' -and
    $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_R18ZT1.ready.zip') 'R18ZT exact publication route changed.'
Require (-not (Test-Path -LiteralPath $publishGatePath)) 'R18ZT V2 publication gate already exists; republish refused.'
...
# This is intentionally the final read-only operation before the first external write.
# The separately frozen scanner must actually scan all four declared scopes and emit
# bounded JSON to stdout; an unavailable gateway inventory is never accepted.
$collisionScannerDependency = Get-ValidatedDependency ([string]$invocation.collision.scannerDependency)
$collisionInvocationDependency = Get-ValidatedDependency ([string]$invocation.collision.invocationDependency)
$scanStarted = [DateTimeOffset]::UtcNow
$collisionText = (& ([string]$collisionScannerDependency.path) -Preflight -InvocationManifest ([string]$collisionInvocationDependency.path) | Out-String)
...
Require ([string]$collision.schema -ceq [string]$invocation.collision.expectedSchema -and
    [string]$collision.state -ceq [string]$invocation.collision.expectedState -and [string]$collision.requestId -ceq 'REQ_R18ZT1') 'R18ZT fresh collision scan identity changed.'
Require ([bool]$collision.allFourScopesActuallyScanned -and -not [bool]$collision.gatewayInventoryUnavailable -and
    -not [bool]$collision.mutationsPerformed -and [int]$collision.totalMatchCount -eq 0) 'R18ZT fresh collision scan did not prove zero matches in all four scopes.'
$requiredScopeIds = @('GATEWAY_REQUEST_INVENTORY','LIVE_PROCESSED_ARCHIVE','LIVE_REQUEST_ROOT','LOCAL_PUBLICATION_ARCHIVE')
$scopeRows = @($collision.scopes)
Require ($scopeRows.Count -eq 4) 'R18ZT fresh collision scan scope count changed.'
foreach ($scopeId in $requiredScopeIds) {
    $matches = @($scopeRows | Where-Object { [string]$_.id -ceq $scopeId })
    Require ($matches.Count -eq 1 -and [bool]$matches[0].scanned -and -not [bool]$matches[0].unavailable -and [int]$matches[0].matchCount -eq 0) "R18ZT collision scope did not pass: $scopeId"
}
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_publish_gate_v2'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V2_PUBLICATION_PREFLIGHT' } else { 'PASS_R18ZT1V2_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    ...
    freshCollisionScan = $collision
    freshCollisionScanEvidenceSha256 = $collisionEvidenceSha256
    allFourCollisionScopesActuallyScanned = $true
    gatewayInventoryUnavailable = $false
    collisionMatchCount = 0
