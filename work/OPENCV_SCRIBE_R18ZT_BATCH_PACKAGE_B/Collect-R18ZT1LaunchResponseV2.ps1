...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zt_publication_invocation_v2' -and
    [string]$invocation.state -ceq 'FROZEN_R18ZT1_PUBLICATION_INVOCATION' -and [string]$invocation.lifecycle -ceq 'FROZEN') 'R18ZT publication invocation is not frozen; collection refused.'
...
Require (Test-Path -LiteralPath $publishGatePath -PathType Leaf) 'R18ZT publisher gate is absent; launch-response collection is not authorized.'
$publishGateSha256 = Get-Sha256 $publishGatePath
$publishGate = Read-JsonFile $publishGatePath 4194304
Require ([string]$publishGate.schema -ceq 'argos_opencv_scribe_r18zt1_publish_gate_v2' -and
    [string]$publishGate.state -ceq 'PASS_R18ZT1V2_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' -and
...
Require ([bool]$publishGate.signatureVerified -and [bool]$publishGate.allFourCollisionScopesActuallyScanned -and
    -not [bool]$publishGate.gatewayInventoryUnavailable -and [int]$publishGate.collisionMatchCount -eq 0 -and
    [bool]$publishGate.mutationsPerformed -and -not [bool]$publishGate.retryAuthorized -and
    [bool]$publishGate.asynchronousLaunchOnly -and -not [bool]$publishGate.completionClaimed) 'R18ZT publisher gate safety/launch-only contract changed.'
...
Require (-not (Test-Path -LiteralPath $partialRoot) -and -not (Test-Path -LiteralPath $finalRoot) -and
    -not (Test-Path -LiteralPath $collectionGatePath)) 'R18ZT response was already collected or a partial attempt exists; retry refused.'
...
foreach ($path in $snapshot) {
    ...
    Require ([int64]$item.Length -le [int64]$invocation.collector.maximumResponseZipBytes) "R18ZT new response candidate exceeds the byte cap: $path"
...
foreach ($candidate in $candidates) {
    $zip = [IO.Compression.ZipFile]::OpenRead([string]$candidate.path)
    try {
        Require ($zip.Entries.Count -le [int]$invocation.collector.maximumResponseEntries) "R18ZT response ZIP entry count exceeds the cap: $($candidate.path)"
...
$preflightResult = [ordered]@{
    schema = 'argos_opencv_scribe_r18zt1_launch_response_collection_gate_v2'
    collectedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZT1V2_ONE_SIGNED_LAUNCH_RESPONSE_PREFLIGHT' } else { 'PASS_R18ZT1V2_SIGNED_LAUNCH_RESPONSE_COLLECTED' })
...
# One local create-new extraction attempt. A partial directory is deliberately
# retained on any failure, making retry under this collector namespace impossible.
...
$verificationRows = @(& ([string]$responseVerifierDependency.path) -PackagePath $partialRoot -EndpointCertificatePath ([string]$responseSignerDependency.path) -ExpectedSourceRole JBOD -ExpectedRequestId 'REQ_R18ZT1')
...
$resultPath = Get-SafeExtractPath $partialRoot ([string]$invocation.collector.resultPayloadPath)
$stdoutPath = Get-SafeExtractPath $partialRoot ([string]$invocation.collector.stdoutPayloadPath)
...
Write-JsonCreateNew $collectionGatePath $preflightResult
$preflightResult | ConvertTo-Json -Depth 32
