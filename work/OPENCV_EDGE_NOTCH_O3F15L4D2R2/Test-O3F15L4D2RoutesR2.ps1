...
$expectedInvocation = Join-Path $PSScriptRoot 'O3F15L4D2_ROUTE_GATE_R2_INVOCATION.json'
...
Assert-True ([string](Get-RequiredProperty $invocation 'schema') -ceq 'argos_ocv03_o3f15l4d2_route_gate_r2_invocation_v1') 'D2 R2 route invocation schema changed.'
Assert-True ([string](Get-RequiredProperty $invocation 'state') -ceq 'FROZEN_O3F15L4D2_ROUTE_GATE_R2_INVOCATION') 'D2 R2 route invocation is not frozen.'
...
Assert-True ($gateOutputPath.Equals((Join-Path $projectRoot 'work\OPENCV_EDGE_NOTCH_O3F15L4D2\final_o3f15l4d2\O3F15L4D2_PREPUBLICATION_PATH_R2_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 R2 route gate output is not beside the final ZIP.'
...
$result = [ordered]@{
    schema = 'argos_ocv03_o3f15l4d2_complete_route_path_r2_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    lifecycle = $(if ($Preflight) { 'DRAFT_PREFLIGHT' } else { 'FROZEN_GATE_OUTPUT' })
    state = $(if ($Preflight) { 'PASS_O3F15L4D2_COMPLETE_ROUTE_PATH_R2_PREFLIGHT' } else { 'PASS_O3F15L4D2_COMPLETE_ROUTE_PATH_R2_GATE' })
    revision = 'O3F15L4D2'
    requestId = $requestId
