...
param(
    [switch]$Preflight,
    [switch]$Test,
    [string]$PackageRoot = 'C:\O3F15L4D2R2T'
)
...
function Invoke-DiagnosticCase([string]$Mode, [string]$CaseRoot, [string]$EntryPoint, [string]$Fixture, [string]$Contract, [string]$PowerShell, [string]$Python) {
    ...
    [IO.File]::Copy($Contract, (Join-Path $CaseRoot 'O3F15L4D2_DIAGNOSTIC_CONTRACT.json'), $false)
    ...
    ) (Split-Path -Parent $EntryPoint) @{O3F15L4D2_FIXTURE_MODE=$Mode}
    ...
    if ($success) {
        Require ([string]$value.state -ceq 'COMPLETE_O3F15L4D2_METADATA_DIAGNOSTIC' -and [int]$value.classification.identityCount -eq 978 -and [int]$value.classification.sourceLeafCount -eq 1956) "D2 success contract changed: $Mode"
    } else {
        Require ([string]$value.state -ceq 'HOLD_O3F15L4D2_METADATA_DIAGNOSTIC' -and [string]$value.failureCode -ceq [string]$expectedFailure[$Mode]) "D2 hold contract changed: $Mode"
    }
    ...
    [pscustomobject][ordered]@{mode=$Mode;state=[string]$value.state;failureCode=$(if($success){$null}else{[string]$value.failureCode});emittedJsonBytes=[Text.Encoding]::UTF8.GetByteCount($capture.stdout);combinedChildBytes=[int64]$value.child.combinedBytes}
}
function New-SignedWorkerFixture([string]$ExtractedPackage, [string]$RequestRoot, [string]$RequestId, [string]$Destination, [object]$Signer) {
    ...
    $fixtureEntry = Join-Path $payloadRoot 'O3F15L4D2MaintenanceFixture.ps1'
    $fixtureSource = "#Requires -Version 5.1`r`n[ordered]@{state='PASS_O3F15L4D2_LOCAL_MAINTENANCE_FIXTURE';taskActionCount=0;existingProcessActionCount=0;providerActivated=`$false;sourceImageBytesRead=`$false;reviewOnly=`$true;productionRoutingEnabled=`$false}|ConvertTo-Json -Compress`r`n"
    ...
    $manifest.entryPoint = 'payload/O3F15L4D2MaintenanceFixture.ps1'
    $manifest.changes[0].destination = $Destination.Replace('\','/')
    $manifest.rehearsal.requiredState = 'PASS_O3F15L4D2_LOCAL_MAINTENANCE_FIXTURE'
...
function Invoke-ExactWorkerCase([string]$Name, [string]$ExtractedPackage, [string]$CasesRoot, [string]$Carrier, [ValidateSet('APPROVED','IDEMPOTENT','UNAPPROVED','ABSENT')][string]$Disposition, [object]$Signer, [string]$Worker, [string]$RequestVerifier, [string]$ResponseVerifier, [string]$LocalPublicCertificate) {
    ...
    elseif ($Disposition -ceq 'UNAPPROVED') { [IO.File]::WriteAllText($target, 'O3F15L4D2_UNAPPROVED_PREDECESSOR', (New-Object Text.UTF8Encoding($false))) }
    $beforeExists = Test-Path -LiteralPath $target -PathType Leaf
    $beforeHash = if ($beforeExists) { Get-Sha256 $target } else { $null }
    $requestId = 'REQ_O3F15L4D2_LOCAL_' + $Name
...
Require ($resolvedPackageRoot -ceq 'C:\O3F15L4D2R2T') 'D2 R2 rehearsal root changed.'
$signGatePath = Join-Path $project 'work\OPENCV_EDGE_NOTCH_O3F15L4D2\final_o3f15l4d2\O3F15L4D2_SIGN_GATE.json'
...
$testRoot = Join-Path $resolvedPackageRoot 'rehearsal'
$extractRoot = Join-Path $testRoot 'signed'
$gatePath = Join-Path $PSScriptRoot 'O3F15L4D2_FINAL_ZIP_REHEARSAL_R2_GATE.json'
...
Require ([string]$signGate.state -ceq 'PASS_O3F15L4D2_SIGNED_METADATA_DIAGNOSTIC_PACKAGE' -and [int]$signGate.signatureCount -eq 1 -and [int]$signGate.payloadFileCount -eq 18) 'D2 sign gate changed.'
...
$planned = @(
    (Join-Path $extractRoot 'payload\Invoke-O3F15L4D2.ps1'),
    (Join-Path $testRoot 'diagnostic\CLASSIFICATION_OVERSIZE\O3F15L4D2_DIAGNOSTIC_CONTRACT.json'),
    (Join-Path $testRoot 'exact-worker\UNAPPROVED\incoming\REQ_O3F15L4D2_LOCAL_UNAPPROVED.ready\payload\O3F15L4D2_DIAGNOSTIC_CONTRACT.json'),
    (Join-Path $testRoot 'exact-worker\UNAPPROVED\state\maintenance\REQ_O3F15L4D2_LOCAL_UNAPPROVED\failed_new\M000_0123456789_0123456789.rollback'),
...
if ($Preflight) {
    [ordered]@{schema='argos_ocv03_o3f15l4d2_final_zip_rehearsal_r2_preflight_v1';state='PASS_O3F15L4D2_FINAL_ZIP_REHEARSAL_R2_PREFLIGHT';requestId=[string]$signGate.requestId;packageZipSha256=[string]$signGate.packageZipSha256;testRoot=$testRoot;pathState=[string]$pathResult.state;diagnosticCaseCount=11;exactEndpointWorkerCaseCount=4;releaseSignatureCount=[int]$signGate.signatureCount;plannedLocalRehearsalRequestSignatureCount=4;plannedLocalRehearsalResponseSignatureCount=4;caseRowRuntimeType='System.Management.Automation.PSCustomObject';mutationsPerformed=$false;targetOrJbodAccessed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
Require ([string]$manifest.entryPoint -ceq 'payload/Invoke-O3F15L4D2.ps1' -and [string]::Join('|',@($manifest.sourceProcessingContract.exactChildArguments)) -ceq '-I|-B|Run-O3F15L4FrontReconcile.py|PREFLIGHT') 'D2 signed entrypoint selector changed.'
...
$contractPath = Join-Path $payloadRoot 'O3F15L4D2_DIAGNOSTIC_CONTRACT.json'
$entryPoint = Join-Path $payloadRoot 'Invoke-O3F15L4D2.ps1'
$fixture = Join-Path $payloadRoot 'O3F15L4D2DiagnosticFixture.py'
$contract = Get-Content -LiteralPath $contractPath -Raw | ConvertFrom-Json
Require ([string]$contract.lifecycle -ceq 'FROZEN' -and [string]$contract.state -ceq 'FROZEN_O3F15L4D2_METADATA_DIAGNOSTIC_CONTRACT') 'D2 packaged contract is not frozen.'
