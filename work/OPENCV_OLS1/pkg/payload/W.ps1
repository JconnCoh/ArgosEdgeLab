...
function Get-EnvironmentInventory {
    ...
    Write-Verbose 'OLS1DBG Get-EnvironmentInventory entered'
    ...
    Write-Verbose 'OLS1DBG drive ready'
    ...
    $pathNameSearch=Get-MemberValue $request 'boundedPathNameSearch' $null
    $searchEnabled=($null-ne$pathNameSearch-and[bool](Get-MemberValue $pathNameSearch 'enabled' $false))
    ...
    if($childNames.Count-eq0-and$relativeLeafPaths.Count-eq0-and-not$searchEnabled){throw 'Environment inventory requires at least one bounded metadata selector.'}
    ...
    $pathNameSearchResult=$null
    if($searchEnabled){
        $literalToken=[string](Get-MemberValue $pathNameSearch 'literalToken' '')
        $maximumDepth=[int](Get-MemberValue $pathNameSearch 'maximumDepth' 0)
        $maximumEntries=[int](Get-MemberValue $pathNameSearch 'maximumEntries' 0)
        $maximumMatches=[int](Get-MemberValue $pathNameSearch 'maximumMatches' 0)
        if($literalToken-notmatch'^[A-Za-z0-9_-]{1,32}$'){throw 'Environment inventory bounded path-name token is unsafe.'}
        if($maximumDepth-lt1-or$maximumDepth-gt4){throw 'Environment inventory bounded path-name depth must be 1 through 4.'}
        if($maximumEntries-lt1-or$maximumEntries-gt50000){throw 'Environment inventory bounded path-name entry limit must be 1 through 50000.'}
        if($maximumMatches-lt1-or$maximumMatches-gt128){throw 'Environment inventory bounded path-name match limit must be 1 through 128.'}
        if($relativeLeafPaths.Count-gt0){throw 'Environment inventory bounded path-name search cannot be combined with exact-relative-leaf observation.'}
        if($Preflight){
            $pathNameSearchResult=[ordered]@{schema='argos_bounded_path_name_search_v1';state='UNOBSERVED_PREFLIGHT';literalToken=$literalToken;maximumDepth=$maximumDepth;maximumEntries=$maximumEntries;maximumMatches=$maximumMatches;entriesVisited=0;directoriesEnumerated=0;matches=@();matchCount=0;truncated=$false;accessErrorCount=0;accessErrors=@();skippedReparseSubtrees=0;skippedUnsafePathSubtrees=0;complete=$false;filesRead=$false;imageBytesRead=$false;sourceHashingPerformed=$false;mutationsPerformed=$false}
        }else{
            ...
            while($pending.Count-gt0-and-not$truncated){
                ...
                $children=@()
                try{$children=@(Get-ChildItem -LiteralPath ([string]$node.fullPath) -Force -ErrorAction Stop)}
                catch{
                    $message=[string]$_.Exception.Message
                    if($accessErrors.Count-lt16){$accessErrors.Add([pscustomobject]@{path=[string]$node.fullPath;message=$message.Substring(0,[Math]::Min(240,$message.Length))})}
                    continue
                }
                ...
                foreach($child in $children){
                    ...
                    if(-not$full.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)){throw 'Environment inventory bounded path-name result escaped the approved root.'}
                    ...
                    $nameMatches=([string]$child.Name).IndexOf($literalToken,[StringComparison]::OrdinalIgnoreCase)-ge0
                    if($nameMatches){
                        if($matches.Count-ge$maximumMatches){$truncated=$true;break}
                        $matches.Add([pscustomobject]@{relativePath=$relative;normalizedFullPath=$full;name=[string]$child.Name;depth=([int]$node.depth+1);pathType=if($isContainer){'CONTAINER'}else{'LEAF'};attributes=[string]$child.Attributes;reparsePoint=$reparse;length=if($isContainer){$null}else{[int64]$child.Length};lastWriteTimeUtc=$child.LastWriteTimeUtc.ToString('o');containedByApprovedRoot=$true;filesRead=$false;imageBytesRead=$false;sourceHashingPerformed=$false;mutationsPerformed=$false})
                    }
                    if($isContainer-and([int]$node.depth+1)-lt$maximumDepth){
                        if($reparse){$skippedReparseSubtrees++;continue}
                        $componentLength=([string]$child.Name).Length
                        if(($full.Length+32)-ge230-or$componentLength-gt80){$skippedUnsafePathSubtrees++;continue}
                        $pending.Enqueue([pscustomobject]@{fullPath=$full;depth=([int]$node.depth+1)})
                    }
                }
            }
    ...
    Write-Verbose 'OLS1DBG returning inventory'

    return [ordered]@{
        schema=if($searchEnabled){'argos_project_portal_environment_inventory_v3'}else{'argos_project_portal_environment_inventory_v2'}
        ...
        boundedPathNameSearch=$pathNameSearchResult
        ...
        commands=$commands
        pathsEnumerated=($searchEnabled-and-not$Preflight)
        filesRead=$false
...
Write-Verbose 'OLS1DBG config loaded'
...
if(-not[string]::IsNullOrWhiteSpace($EnvironmentProbeManifest)){
    ...
    Write-Verbose 'OLS1DBG writing output'
    Write-NewUtf8Json -Path $outputPath -Value $result
    Write-Verbose 'OLS1DBG output written'
