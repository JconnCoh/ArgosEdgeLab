...
$requestId = 'REQ_OLS1'
$sourceRoot = Join-Path $workRoot 'signed_short\REQ_OLS1.ready'
$finalRoot = Join-Path $workRoot 'final'
$partialRoot = Join-Path $workRoot 'final.partial'
$zipName = 'REQ_OLS1.ready.zip'
...
$signedSourceGatePath = Join-Path $workRoot 'OLS1_SIGNED_SOURCE_GATE.json'
$entrypointGatePath = Join-Path $workRoot 'OLS1_ENTRYPOINT_TEST_GATE.json'
...
function Assert-Gate([string]$Path, [string]$Sha256, [string]$State) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { throw "Required OLS1 gate is missing: $Path" }
    if ((Get-Sha $Path) -ne $Sha256) { throw "Required OLS1 gate hash changed: $Path" }
    $value = Get-Content -LiteralPath $Path -Raw | ConvertFrom-Json
    if ([string]$value.state -ne $State) { throw "Required OLS1 gate state changed: $Path" }
    return $value
}

foreach ($path in @($sourceRoot, $pathTool, $publicCertificatePath, $definitionPath)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "OLS1 package prerequisite is missing: $path" }
}
if ((Get-Sha $definitionPath) -ne $expectedDefinitionSha) { throw 'OLS1 maintenance definition changed.' }
$signedSourceGate = Get-Content -LiteralPath $signedSourceGatePath -Raw | ConvertFrom-Json
if ([string]$signedSourceGate.schema -ne 'argos_ols1_signed_source_gate_v1' -or [string]$signedSourceGate.state -ne 'PASS_OLS1_SIGNED_SOURCE_GATE' -or [string]$signedSourceGate.requestId -ne $requestId -or -not ([IO.Path]::GetFullPath([string]$signedSourceGate.signedSourcePath)).Equals([IO.Path]::GetFullPath($sourceRoot), [StringComparison]::OrdinalIgnoreCase) -or [string]$signedSourceGate.maintenanceDefinitionSha256 -ne $expectedDefinitionSha -or [string]$signedSourceGate.entrypointSha256 -ne $expectedEntrypointSha -or [string]$signedSourceGate.workerSha256 -ne $expectedWorkerSha -or [int]$signedSourceGate.payloadFileCount -ne 2 -or [string]$signedSourceGate.signatureAlgorithm -ne 'RSA-SHA256-PKCS1' -or -not [bool]$signedSourceGate.reviewOnly -or [bool]$signedSourceGate.productionRoutingEnabled) { throw 'OLS1 signed-source gate contract changed.' }
...
if ($expectedManifestSha -notmatch '^[0-9A-F]{64}$' -or $expectedSignatureSha -notmatch '^[0-9A-F]{64}$') { throw 'OLS1 signed-source gate hashes are invalid.' }
if (Test-Path -LiteralPath $finalRoot) { throw "OLS1 final root already exists: $finalRoot" }
if (Test-Path -LiteralPath $partialRoot) { throw "OLS1 partial final root already exists: $partialRoot" }
...
if ($sourceFiles.Count -ne $expectedFiles.Count) { throw "OLS1 signed source file count changed: $($sourceFiles.Count)" }
foreach ($entry in $expectedFiles.GetEnumerator()) {
    $path = Join-Path $sourceRoot $entry.Key.Replace('/', '\')
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or (Get-Sha $path) -ne [string]$entry.Value) { throw "OLS1 signed source changed: $($entry.Key)" }
}

$entrypointGate = Assert-Gate $entrypointGatePath $expectedEntrypointGateSha 'PASS_OLS1_ENTRYPOINT_TEST_GATE'
...
if ([string]$exactEndpointGate.endpointWorkerTargetSha256 -ne $expectedOldWorkerSha) { throw 'OLS1 inherited endpoint predecessor does not match the qualified FIC1 endpoint target.' }
...
if ([string]$pathGate.state -ne 'PASS_PATH_BUDGET') { throw "OLS1 final package path gate failed: $($pathGate.state)" }

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ols1_final_package_preflight_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_OLS1_FINAL_PACKAGE_PREFLIGHT'
        requestId = $requestId
...
if ($extracted.Count -ne $expectedFiles.Count) { throw "OLS1 extracted file count changed: $($extracted.Count)" }
foreach ($entry in $expectedFiles.GetEnumerator()) {
    $path = Join-Path $partialExtract $entry.Key.Replace('/', '\')
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or (Get-Sha $path) -ne [string]$entry.Value) { throw "OLS1 exact final ZIP file changed: $($entry.Key)" }
}
...
if (-not $signaturePassed) { throw 'OLS1 exact final ZIP signature verification failed.' }

$manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
if ([string]$manifest.requestId -ne $requestId -or [string]$manifest.targetRole -ne 'JBOD' -or [string]$manifest.jobClass -ne 'MAINTENANCE_PATCH' -or @($manifest.files).Count -ne 2 -or @($manifest.changes).Count -ne 1 -or @($manifest.entryPointMutations).Count -ne 1 -or @($manifest.entryPointOutputs).Count -ne 1 -or @($manifest.allowedTaskActions).Count -ne 0 -or @($manifest.allowedProcessActions).Count -ne 0) { throw 'OLS1 exact final ZIP manifest contract changed.' }
$parserPassed = 0
foreach ($relative in @('payload/E.ps1', 'payload/W.ps1')) {
    ...
    if (@($errors).Count -ne 0) { throw "OLS1 exact final ZIP parser failure: $relative" }
    $parserPassed++
}
...
$gate = [ordered]@{
    schema = 'argos_ols1_final_package_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_OLS1_FINAL_PACKAGE_GATE'
    requestId = $requestId
    requestZip = 'work/OPENCV_OLS1/final/REQ_OLS1.ready.zip'
