...
$requestId = 'REQ_RZV1'
$bindingRecordSha256 = 'DF4C66EA1E3E707E70D48F38D532EC331D5F840307578521F532A4B2D32F6B14'
...
if ($Preflight) {
    [ordered]@{
        schema='argos_r18zv1_request_id_uniqueness_preflight_v1'
        checkedUtc=[DateTime]::UtcNow.ToString('o')
        state='PASS_R18ZV1_REQUEST_ID_UNIQUENESS_PREFLIGHT'
        requestId=$requestId
...
$preSignatureOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV1_REQUEST_ID_COLLISION_GATE.json'))
$prePublicationOutput = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZV1_P8_COLLISION.json'))
Assert-True (
...
$result = [ordered]@{
    schema='argos_r18zv1_request_id_uniqueness_gate_v1'
    checkedUtc=[DateTime]::UtcNow.ToString('o')
    state=$(if($scanPassed){'PASS_R18ZV1_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS'}else{'FAIL_R18ZV1_REQUEST_ID_COLLISION_OR_SCAN_ERROR'})
    requestId=$requestId
    bindingRecordSha256=$bindingRecordSha256
    scannerSha256=$scannerSha256
