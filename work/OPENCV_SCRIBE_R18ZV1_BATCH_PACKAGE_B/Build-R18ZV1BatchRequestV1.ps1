...
function Assert-ExactLauncherChange([object[]]$Changes, [string]$LauncherSha256, [string]$Label) {
    ...
    Require ([string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZV1.ps1') "$Label launcher destination changed."
...
$requestId = 'REQ_RZV1'
$revision = 'R18ZV1_EXISTING_ORIENTED_CROPS_ASYNC_REVIEW_ONLY_20260910B'
$runnerRevision = 'R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260908D'
$bindingPath = Join-Path $PSScriptRoot 'R18ZV1_FROZEN_PACKAGE_BINDINGS_V1.json'
$launcherPath = Join-Path $PSScriptRoot 'Invoke-R18ZV1BatchLaunchV1.ps1'
$payloadManifestPath = Join-Path $PSScriptRoot 'R18ZV1_PAYLOAD_MANIFEST.json'
$definitionPath = Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION_R18ZV1.json'
$configurationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B\R18ZV1_BATCH_CONFIGURATION.json'
$designPath = Join-Path $PSScriptRoot 'R18ZV1_BATCH_PACKAGE_DESIGN_V1.json'
$importSmokePath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B\Test-R18ZTPackagedImportClosure.py'
$payloadCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZV1_B1_CLONE_REMEDIATION.json'
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZV1_B1_CLONE_REMEDIATION_V3.json'
$publisherCloneRemediationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B\R18ZV1_B1_CLONE_REMEDIATION.json'
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZV1_B1_CLONE_RAW_GATE_V3.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZV1_B1_CLONE_GATE_V3.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZV1_PATH_PLAN_GATE.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZV1_POWERSHELL_SAFETY_GATE_V3.json'
$scienceGatePath = Join-Path $PSScriptRoot 'R18ZV1_SCIENCE_GATE.json'
$junctionGatePath = Join-Path $PSScriptRoot 'R18ZV1_PROPOSAL_JUNCTION_GATE.json'
$recoveryIntentPath = Join-Path $PSScriptRoot 'R18ZV1_RECOVERY_INTENT_V3.json'
$recoveryGatePath = Join-Path $PSScriptRoot 'R18ZV1_RECOVERY_INTENT_GATE_V3.json'
$preactionPath = Join-Path $PSScriptRoot 'R18ZV1_PREACTION_GATE_V3.json'
$collisionGatePath = Join-Path $PSScriptRoot 'R18ZV1_REQUEST_ID_COLLISION_GATE.json'
$collisionScannerPath = Join-Path $PSScriptRoot 'Test-R18ZV1RequestIdUniquenessV1.ps1'
...
$testRoot = 'C:\RZV1B1T'
$testWorkRoot = 'C:\RZV1B1TW'
$testOutputRoot = 'C:\RZV1B1TO'
$stageRoot = 'C:\RZV1B1P'
$readyRoot = Join-Path $stageRoot ($requestId + '.ready')
$stageZip = Join-Path $stageRoot ($requestId + '.ready.zip')
$verifyRoot = 'C:\RZV1B1V'
$finalRoot = Join-Path $PSScriptRoot 'final'
$finalPartial = Join-Path $PSScriptRoot 'final.partial'
$zipName = $requestId + '.ready.zip'
$finalGatePath = Join-Path $PSScriptRoot 'R18ZV1_FINAL_PACKAGE_GATE.json'
$productionWorkRoot = 'D:\A2\w\ocv\R18ZV1'
$productionOutputRoot = 'D:\A2\o\ocv\R18ZV1'
$productionProposalAlias = 'D:\A2\w\ocv\R18ZV1\p'
$canonicalProposalRoot = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals'
$productionInstalledLauncher = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZV1.ps1'
$bindingSha = 'DF4C66EA1E3E707E70D48F38D532EC331D5F840307578521F532A4B2D32F6B14'

Require-Pin $bindingPath $bindingSha -1 'FROZEN_R18ZV1_PACKAGE_B_V1_BINDINGS'
$binding = Get-Content -LiteralPath $bindingPath -Raw | ConvertFrom-Json
Require ([string]$binding.schema -eq 'argos_opencv_scribe_r18zv1_package_bindings_v1' -and [string]$binding.requestId -eq $requestId -and [string]$binding.packageRevision -eq $revision -and [string]$binding.runnerRevision -eq $runnerRevision) 'R18ZV1 binding changed.'
...
Require ([string]$boundLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$boundLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZV1\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$boundLongestOutput.basePathLength -eq 62 -and [int]$boundLongestOutput.suffixReserveCharacters -eq 52 -and [int]$boundLongestOutput.effectivePathLength -eq 114 -and [int]$boundLongestOutput.effectiveComponentLength -eq 68) 'R18ZT exact longest atomic case-output binding changed.'
$boundConservativeTemp = $binding.pathContract.launcherConservativeProviderTempBudget
Require ([string]$boundConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$boundConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZV1\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$boundConservativeTemp.basePathLength -eq 74 -and [int]$boundConservativeTemp.baseComponentLength -eq 28 -and [int]$boundConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$boundConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$boundConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$boundConservativeTemp.trueAtomicWriteTarget) 'R18ZT conservative provider-temp launcher budget changed.'
...
$requiredArtifactNames = @(
    ...
    'heldOutZDiagnosticGate','r18zuReviewBatchScienceGate','approvedSourceGuard','priorRequestTerminalRouteGate','r18zuProvider','r18zv1DevelopmentFreeze','r18zv1Slot21Job','r18zv1Slot21Result','r18zv1Slot21CompactGate','r18zv1Slot21CorrectionGate','collectorFailure','collectorRemediationBinding','packageAPreflightWithdrawal'
)
...
$cloneGate = Read-BoundGate $cloneGatePath 'PASS_R18ZV1_PACKAGE_B_CLONE_LITERAL_GATE'
$pathGate = Read-BoundGate $pathGatePath 'PASS_R18ZV1_PACKAGE_B_PATH_BUDGET'
$powerShellGate = Read-BoundGate $powerShellGatePath 'PASS_R18ZV1_PACKAGE_B_POWERSHELL_SAFETY'
$scienceGate = Read-BoundGate $scienceGatePath 'PASS_R18ZV1_BATCH_SCIENCE_GATE'
$junctionGate = Read-BoundGate $junctionGatePath 'PASS_R18ZV1_PROPOSAL_JUNCTION_CREATION_AND_ROLLBACK'
...
$collisionGate = $null
if ($Build) { $collisionGate = Read-BoundGate $collisionGatePath 'PASS_R18ZV1_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS' }

$builderPath = [IO.Path]::GetFullPath($MyInvocation.MyCommand.Path)
$builderSha = Get-Sha256 $builderPath
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Build-R18ZV1BatchRequestV1.ps1' = $builderSha
    'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Collect-R18ZV1LaunchResponseV1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZV1LaunchResponseV1.ps1')
    'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Invoke-R18ZV1BatchLaunchV1.ps1' = Get-Sha256 $launcherPath
    'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Publish-R18ZV1V1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZV1V1.ps1')
    'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Test-R18ZV1RequestIdUniquenessV1.ps1' = Get-Sha256 $collisionScannerPath
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/R18ZV1_B1_CLONE_RAW_GATE_V3.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @(
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Invoke-R18ZV1BatchLaunchV1.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Build-R18ZV1BatchRequestV1.ps1';sha=$builderSha},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Publish-R18ZV1V1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZV1V1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Collect-R18ZV1LaunchResponseV1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZV1LaunchResponseV1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Test-R18ZV1RequestIdUniquenessV1.ps1';sha=(Get-Sha256 $collisionScannerPath)}
)) {
...
Require ([string]$priorRoute.state -ceq 'PASS_R18ZU1_ENDPOINT_REQUEST_TERMINAL_SIGNED_RETURN_AUTHENTICATED' -and [string]$priorRoute.priorRequestId -ceq 'REQ_R18ZU1' -and
    ...
    [bool]$priorRoute.conclusion.returnRouteHealthy -and [bool]$priorRoute.observation.signedResponseVerified -and
    [string]$priorRoute.result.state -ceq 'PASS_MAINTENANCE_PATCH' -and -not [bool]$priorRoute.externalMutationPerformed -and
    -not [bool]$priorRoute.imageBytesRead) 'R18ZU1 terminal route changed.'
...
Require ([string]$pathLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$pathLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZV1\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$pathLongestOutput.basePathLength -eq 62 -and [int]$pathLongestOutput.suffixReserveCharacters -eq 52 -and [int]$pathLongestOutput.effectivePathLength -eq 114 -and [int]$pathLongestOutput.effectiveComponentLength -eq 68) 'R18ZT path-gate true atomic longest-output fact changed.'
$pathConservativeTemp = $pathGate.launcherConservativeProviderTempBudget
Require ([string]$pathConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$pathConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZV1\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$pathConservativeTemp.basePathLength -eq 74 -and [int]$pathConservativeTemp.baseComponentLength -eq 28 -and [int]$pathConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$pathConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$pathConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$pathConservativeTemp.trueAtomicWriteTarget) 'R18ZT path-gate conservative provider-temp fact changed.'
...
Require ([string]$recoveryIntent.state -eq 'FROZEN_R18ZV1_PACKAGE_B_RECOVERY_INTENT' -and [string]$recoveryIntent.requestId -eq $requestId -and [string]$recoveryIntent.bindingRecordSha256 -eq $bindingSha -and -not [bool]$recoveryIntent.mutation.publicationAuthorized -and -not [bool]$recoveryIntent.mutation.automaticRetryAuthorized) 'R18ZV1 recovery intent changed.'
...
$bindingDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/R18ZV1_FROZEN_PACKAGE_BINDINGS_V1.json' })
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/Build-R18ZV1BatchRequestV1.ps1' })
...
if ($Build) {
    Require ([string]$collisionGate.schema -ceq 'argos_r18zv1_request_id_uniqueness_gate_v1' -and [string]$collisionGate.requestId -ceq $requestId -and [string]$collisionGate.scannerSha256 -ceq (Get-Sha256 $collisionScannerPath) -and [string]$collisionGate.scanPurpose -ceq 'PRE_SIGNATURE') 'R18ZV1 collision gate changed.'
...
foreach ($outputPin in @(
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV1\LAUNCH.json';schema='argos_opencv_scribe_r18zt_batch_launch_v2'},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV1\RUNNING.json';schema=[string]$binding.outputSchemas.progress},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV1\COMPLETE.json';schema=[string]$binding.outputSchemas.complete},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZV1\STATUS.json';schema=[string]$binding.outputSchemas.status}
)) {
...
$runningOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZV1\RUNNING.json' })[0]
...
$failureOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZV1\FAILURE.json' })[0]
...
$expectedLeafSetSha = '38C495FE4F848579F1951B7D0AF8AFD2106FEB8C73EB992681D62634BAEC011D'
...
if ($Preflight) {
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zv1_batch_build_preflight_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZV1_PACKAGE_B_BUILD_PREFLIGHT'
        requestId = $requestId
...
$stagedPreflight = (& (Join-Path $activePayload 'Invoke-R18ZTBatchLaunch.ps1') -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $activePayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\RZV1B1_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
...
$stagedPackageHashClosure = [ordered]@{
    state = 'PASS_R18ZV1_STAGED_PACKAGED_HASH_CLOSURE_NO_IMAGE'
...
if ($Test) {
    ...
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zv1_batch_unsigned_test_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZV1_PACKAGE_B_UNSIGNED_TEST'
        requestId = $requestId
        bindingRecordSha256 = $bindingSha
...
$extractedPreflightText = (& $windowsPowerShell51 -NoProfile -ExecutionPolicy Bypass -File $extractedLauncher -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $extractedPayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\RZV1B1_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String)
...
$extractedProvider = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZV1\ArgosOpenCvScribeV1R18ZV1.py'
...
$routeGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv1_complete_route_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZV1_COMPLETE_ROUTE_GATE_SIGNED_UNPUBLISHED'
    requestId = $requestId
    bindingRecordSha256 = $bindingSha
...
$finalGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zv1_final_package_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZV1_SIGNED_UNPUBLISHED_PACKAGE_GATE'
    requestId = $requestId
    revision = $revision
    bindingRecordSha256 = $bindingSha
    requestZip = 'work/OPENCV_SCRIBE_R18ZV1_BATCH_PACKAGE_B/final/REQ_RZV1.ready.zip'
