...
Assert-True ([string]$invocation.schema-eq'argos_o3q2_route_invocation_v1') 'O3Q2 route invocation schema changed.'
...
Assert-True ($result.requestZipSha256-eq[string]$invocation.requestZipSha256-and$result.requestManifestSha256-eq[string]$invocation.requestManifestSha256-and$result.requestSignatureSha256-eq[string]$invocation.requestSignatureSha256) 'O3Q2 exact route artifact hash changed.'
$result.exactFinalZipExtractionPassed=[bool]$invocation.exactFinalZipExtractionPassed;$result.exactFinalZipSignaturePassed=[bool]$invocation.exactFinalZipSignaturePassed;Assert-True ($result.exactFinalZipExtractionPassed-and$result.exactFinalZipSignaturePassed) 'O3Q2 final ZIP route proof changed.'
$output=[IO.Path]::GetFullPath([string]$invocation.outputPath);Assert-True (-not(Test-Path -LiteralPath $output)) 'O3Q2 route output exists.'
