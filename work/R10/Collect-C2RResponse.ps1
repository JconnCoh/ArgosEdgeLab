...
$requestId='REQ_R10'
$localRoot='C:\R10S'
$publishGatePath=Join-Path $root 'R10_PUBLISH_GATE.json'
$publishGateSha='9D83F604EDD676195EF2BE08A819DD8646A78680A7C751F85857D7CE2EE1DC50'
$routeGatePath=Join-Path $root 'R10_RESPONSE_ROUTE_GATE.json'
$terminalGatePath=Join-Path $root 'R10_TERMINAL_RESPONSE_GATE.json'
...
if([string]$publish.requestId-ne$requestId-or[string]$publish.state-ne'PASS_R10_EXACT_SIGNED_REQUEST_PUBLISHED_CREATE_NEW'-or[bool]$publish.overwritePerformed-or[int]$publish.pendingRequestsAtApply-ne0){throw 'R10 publication contract changed.'}
...
if($candidates.Count-eq0){if($Preflight){[ordered]@{schema='argos_r10_response_pending_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PENDING_R10_SIGNED_TERMINAL_RESPONSE';requestId=$requestId;pendingRequests=$pending.Count;responseCandidates=0;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 5;return};throw 'R10 response unavailable.'}
...
try{
    ...
    $declaredNames=@($candidate.manifest.files|ForEach-Object{[string]$_.path})
    $stdoutBytes=if($declaredNames-contains'MAINTENANCE.stdout.txt'){Read-Entry (Exact-Entry $archive 'MAINTENANCE.stdout.txt') 4194304}else{New-Object byte[] 0}
    $stdoutText=$utf8.GetString($stdoutBytes);$stdout=if([string]::IsNullOrWhiteSpace($stdoutText)){$null}else{$stdoutText|ConvertFrom-Json}
    $stderrBytes=if($declaredNames-contains'MAINTENANCE.stderr.txt'){Read-Entry (Exact-Entry $archive 'MAINTENANCE.stderr.txt') 4194304}else{New-Object byte[] 0}
    $stderrText=$utf8.GetString($stderrBytes)
    if([string]$candidate.manifest.state-eq'FAILED'-and$declaredNames-notcontains'FAILURE.json'){throw 'R10 signed failure response does not declare FAILURE.json.'}
...
if([string]$candidate.manifest.state-eq'FAILED'){
    $terminal=[ordered]@{schema='argos_r10_terminal_failure_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='FAIL_R10_SIGNED_TERMINAL_RESPONSE';disposition='WITHDRAWN';requestId=$requestId;responseId=[string]$candidate.manifest.responseId;responseState=[string]$candidate.manifest.state;responseZip=$targetZip;responseZipBytes=$zipBytes;responseZipSha256=$zipSha;signatureVerified=$true;failureState=if($null-eq$failure){''}else{[string]$failure.state};stdoutDeclared=($declaredNames-contains'MAINTENANCE.stdout.txt');stderrDeclared=($declaredNames-contains'MAINTENANCE.stderr.txt');stderrSha256=Bytes-Sha $stderrBytes;sourceDeletionPerformed=$false;otherInspectionTasksChanged=$false;waferAborted=$false;reviewOnly=$true;productionRoutingEnabled=$false}
...
$pass=(
    ...
    [string]$stdout.state-eq'PASS_JBOD_PROCESSOR_RUNNER_FIX_AND_REFRESH_R10'-and-not[bool]$stdout.rehearsal-and
    ...
    [string]$stdout.lot-eq'62631-586'-and[string]$stdout.scan-eq'20260819173317'-and[string]$stdout.scanDate-eq'2026-08-19'-and[string]$stdout.selectedDomain-eq'FRONTSIDE'-and
    [int]$stdout.currentQueueRows-eq10-and[int]$stdout.frontCatalogRows-eq10-and[int]$stdout.distinctFrontPhysicalIdentities-eq10-and[int]$stdout.sameIdentityNonFrontCompetitorRows-eq10-and
    [int]$stdout.confirmedPhysicalAcquisitions-eq10-and[int]$stdout.overlayPhysicalMatches-eq10-and[int]$stdout.metadataMatchedCatalogRows-eq10-and
    [int]$stdout.staleMatchedInsiteRowsBefore-eq0-and[int]$stdout.staleMatchedInsiteRowsAfter-eq0-and
    [int]$stdout.notReadyMatchedRowsBefore-eq10-and[int]$stdout.notReadyMatchedRowsAfter-eq0-and[int]$stdout.scribeHoldRowsAfter-eq0-and
    @($stdout.routeStatesAfter).Count-eq1-and[string]$stdout.routeStatesAfter[0].state-eq'READY_FRONTSIDE_SCRATCH_TEST_REVIEW_ONLY_PROCESSING'-and[int]$stdout.routeStatesAfter[0].count-eq10-and
    [int]$stdout.exactProcessCountAfter-eq1-and
    [bool]$stdout.processorTaskRestarted-and-not[bool]$stdout.restartSkippedFresh-and[int]$stdout.oldProcessId-gt0-and[int]$stdout.newProcessId-ne[int]$stdout.oldProcessId-and
...
$terminal=[ordered]@{
    schema='argos_r10_terminal_response_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R10_SIGNED_TERMINAL_RESPONSE';disposition='PENDING_GATE'
    ...
    lot=[string]$stdout.lot;scan=[string]$stdout.scan;scanDate=[string]$stdout.scanDate;selectedDomain=[string]$stdout.selectedDomain;currentQueueRows=[int]$stdout.currentQueueRows;frontCatalogRows=[int]$stdout.frontCatalogRows;distinctFrontPhysicalIdentities=[int]$stdout.distinctFrontPhysicalIdentities;sameIdentityNonFrontCompetitorRows=[int]$stdout.sameIdentityNonFrontCompetitorRows;confirmedPhysicalAcquisitions=[int]$stdout.confirmedPhysicalAcquisitions;overlayPhysicalMatches=[int]$stdout.overlayPhysicalMatches;metadataMatchedCatalogRows=[int]$stdout.metadataMatchedCatalogRows
