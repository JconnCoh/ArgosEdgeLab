...
$requestId='REQ_R10'
$definitionPath=Join-Path $project 'work\R10\pkg\MAINTENANCE_DEFINITION.json'
$payloadRoot=Join-Path $project 'work\R10\pkg\payload'
$outputRoot=Join-Path $project 'work\R10\signed_short'
...
if($payloadFiles.Count-ne2){throw 'R10 payload file count changed.'}
$payloadNames=@($payloadFiles|ForEach-Object{[string]$_.path})
if($payloadNames-notcontains[string]$definition.entryPoint){throw 'R10 entry point is absent from the payload.'}
foreach($change in @($definition.changes)){
    ...
    if(@($change.approvedPredecessorSha256).Count-lt1-or@($change.approvedPredecessorSha256)-notcontains[string]$change.installedSha256){throw 'R10 predecessor set must include its idempotent target hash.'}
...
if(@($definition.changes).Count-ne2-or@($definition.allowedTaskActions).Count-ne1-or[string]$definition.allowedTaskActions[0]-ne'RESTART:ArgosEdgeLab.AllWaferProcessor.ReviewOnly.V2'){throw 'R10 exact change or task-action contract changed.'}
if($Preflight){
    [ordered]@{schema='argos_r10_signing_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R10_SIGNING_PREFLIGHT';requestId=$requestId;requestIdLength=$requestId.Length;payloadFiles=$payloadFiles.Count;changes=@($definition.changes).Count;allowedTaskActions=@($definition.allowedTaskActions).Count;mutationsPerformed=$false;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 6
    return
}
...
[ordered]@{schema='argos_r10_signing_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R10_SIGNED_REQUEST';requestId=$requestId;packagePath=$ready;payloadFiles=$payloadFiles.Count;reviewOnly=$true;productionRoutingEnabled=$false}|ConvertTo-Json -Depth 6
