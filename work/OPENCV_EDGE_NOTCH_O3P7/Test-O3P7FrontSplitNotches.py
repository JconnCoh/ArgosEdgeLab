#!/usr/bin/env python3
"""Synthetic and structural controls for the O3P7 frontside channel split."""
...
SCHEMA = "argos_ocv03_o3p7_synthetic_invocation_v1"
...
def main() -> int:
    ...
    require(invocation.get("schema") == SCHEMA, "O3P7 synthetic invocation schema changed.")
    require(invocation.get("reviewOnly") is True and invocation.get("productionRoutingEnabled") is False, "O3P7 synthetic authority widened.")
    ...
    require(
        ...
        "O3P7 synthetic output must be create-new.",
    )
    ...
    if arguments.preflight:
        result = {
            "state": "PASS_O3P7_SYNTHETIC_PREFLIGHT",
        ...
        engine = load_module("argos_o3p7_synthetic_engine", engine_path)
        topology = load_module("argos_o3p7_synthetic_topology", topology_path)
        ...
        result = {
            "schema": "argos_ocv03_o3p7_front_split_notch_synthetic_gate_v1",
            "createdUtc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
            "state": "PASS_O3P7_FRONT_SPLIT_NOTCH_SYNTHETIC_GATE" if passed else "FAIL_O3P7_FRONT_SPLIT_NOTCH_SYNTHETIC_GATE",
