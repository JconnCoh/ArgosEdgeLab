...
JOB_SCHEMA = "argos_ocv03_o3p7_front_split_notch_job_v1"
...
def load_job(path: Path) -> tuple[dict[str, Any], Path, Path, Path, Path, Path]:
    ...
    require(job.get("schema") == JOB_SCHEMA, "O3P7 job schema changed.")
    for field in (
        "trainingEligible",
        "xmlEligible",
        "productionEligible",
        ...
        require(job.get(field) is False, f"Forbidden O3P7 authority changed: {field}")
    require(job.get("reviewOnly") is True, "O3P7 must remain review-only.")
    require(
        job.get("channelMethods") == {
            "BF": "O3L8_TOP_CONNECTED_TOPOLOGY_MEASURED_CONTOUR",
            "DF": "FROZEN_R6_OUTER_EDGE_RADIAL_FULL_360",
        },
        "O3P7 channel-method split changed.",
    )
    require(
        job.get("candidateLocalTopologyErrors") == sorted(CANDIDATE_LOCAL_TOPOLOGY_ERRORS),
        "O3P7 candidate-local exception contract changed.",
    )
    ...
    require(runtime_root.is_dir() and output_path.parent.is_dir(), "O3P7 runtime or output parent is absent.")
    require(
        not output_path.exists() and not output_path.with_name(output_path.name + ".partial").exists(),
        "O3P7 output must be create-new.",
    )
    for record, label in (
        ...
        require(sha256_file(resolved) == str(record["sha256"]).upper(), f"O3P7 {label} changed.")
    require(
        json.loads(runtime_gate.read_text(encoding="utf-8")).get("state") == "PASS_O3P2_LOCAL_RUNTIME_INSTALLED",
        "O3P7 runtime gate is not PASS.",
    )
    module_under_root(str(cv2.__file__), runtime_root, "OpenCV")
    module_under_root(str(np.__file__), runtime_root, "NumPy")
    require(cv2.__version__ == str(job["expectedOpenCvVersion"]), "O3P7 OpenCV version changed.")
    require(np.__version__ == str(job["expectedNumpyVersion"]), "O3P7 NumPy version changed.")
...
def refine_bf(
    ...
    require(clean.shape == (inward + outward, width), "O3P7 crop shape changed.")
...
def process_identity(row: dict[str, Any], topology: Any, renderer: Any, job: dict[str, Any]) -> dict[str, Any]:
    ...
    require(1 <= len(seeds) <= int(job["corroboration"]["maximumSeedCountPerWafer"]), "O3P7 seed count is outside the bound.")
    for channel in ("bf", "df"):
        source = row[channel]
        source_path = Path(str(source["path"])).resolve(strict=True)
        require(source_path.stat().st_size == int(source["bytes"]), f"O3P7 {channel.upper()} byte count changed: {source_path}")
        require(sha256_file(source_path) == str(source["sha256"]).upper(), f"O3P7 {channel.upper()} source hash changed: {source_path}")
    bf_path = Path(str(row["bf"]["path"])).resolve(strict=True)
    bf_image = cv2.imread(str(bf_path), cv2.IMREAD_GRAYSCALE)
    require(bf_image is not None, f"O3P7 BF decode failed: {bf_path}")
    bf_fit = dict(seed_result["bf"]["fit"])
    require(
        bf_image.shape == (int(seed_result["bf"]["heightPx"]), int(seed_result["bf"]["widthPx"])),
        "O3P7 BF dimensions changed.",
    )
...
def run(job_path: Path, job: dict[str, Any], runtime_root: Path, topology_path: Path, renderer_path: Path, output_path: Path) -> dict[str, Any]:
    topology = load_module("argos_o3p7_topology", topology_path)
    renderer = load_module("argos_o3p7_crop", renderer_path)
    rows = [process_identity(row, topology, renderer, job) for row in job["inputs"]]
    require(len(rows) == int(job["expectedInputCount"]), "O3P7 input count changed.")
    result = {
        "schema": "argos_ocv03_o3p7_front_split_notch_result_v1",
        "revision": str(job["revision"]),
        "createdUtc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "state": "COMPLETE_O3P7_POST2_BF_TOPOLOGY_DF_RADIAL_REVIEW_ONLY",
...
def main() -> int:
    ...
    for row in job["inputs"]:
        ...
        for channel in ("bf", "df"):
            ...
            require(source.stat().st_size == int(row[channel]["bytes"]), f"O3P7 source byte count changed: {source}")
            require(sha256_file(source) == str(row[channel]["sha256"]).upper(), f"O3P7 source hash changed: {source}")
    if arguments.preflight:
        result = {
            "state": "PASS_O3P7_FRONT_SPLIT_NOTCH_PREFLIGHT",
            "jobSha256": sha256_file(job_path),
