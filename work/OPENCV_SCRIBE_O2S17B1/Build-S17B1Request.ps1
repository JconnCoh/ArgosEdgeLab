...
$definitionPath = Join-Path $PSScriptRoot 'S17B1_DATA_PULL_DEFINITION.json'
$intentPath = Join-Path $PSScriptRoot 'S17B1_RECOVERY_INTENT.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_S17B1_BUILD.json'
...
$terminalPath = Join-Path $project 'work\OPENCV_SCRIBE_O2D11\O2D11_TERMINAL_RESPONSE_GATE_R26.json'
...
$expectedDefinitionSha256 = 'D50CD69CC7A1AD1FD1822D7C3AE63144E0F1FED637635CB8424D1B25207AD49C'
$expectedIntentSha256 = 'C8B977F3917E73A54DB03DC6F58ADC072BED9C972CCB32845A1480D090D151EA'
$expectedPreactionSha256 = '857707F7460A4721A614442D48F6A823D0C2AE922AD3D73AFF733BF8B353148E'
...
$expectedTerminalSha256 = '658678FE83586D79E7197A2D555AB5C7264B890686E6677D76BF90A423F17CD9'
...
function Assert-Pin([string]$Path, [string]$Sha256) {
    Assert-True (Test-Path -LiteralPath $Path -PathType Leaf) "S17B1 dependency is absent: $Path"
    Assert-True ((Get-Sha256 $Path) -eq $Sha256) "S17B1 dependency changed: $Path"
}
...
foreach ($path in @($identityPath,$publicCertificatePath,$packageTester,$intentTool,$preactionTool,$pathTool)) { Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "S17B1 input is absent: $path" }
...
$expectedRelativePaths = @(
    'identity/proposals/62619-433_20260824005735_Slot17/SCRIBE_PROPOSAL.json',
    'identity/proposals/62619-433_20260824005735_Slot17/scribe/multi_channel/MULTI_CHANNEL_READER_SUMMARY.json'
)
Assert-True ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'DATA_PULL' -and [string]$definition.parameters.approvedRoot -eq 'JBOD_PROCESSOR_REVIEW') 'S17B1 DATA_PULL route changed.'
Assert-True ($relativePaths.Count -eq 2 -and @(Compare-Object -ReferenceObject $expectedRelativePaths -DifferenceObject $relativePaths).Count -eq 0) 'S17B1 exact relative path set changed.'
Assert-True ([int]$definition.parameters.maximumFiles -eq 2 -and [int64]$definition.parameters.maximumBytes -eq 4194304 -and [int64]$definition.maxResultBytes -eq 4194304) 'S17B1 bounds changed.'
...
Assert-True ([string]$routeHealth.state -eq 'PASS_O2D11_COMPLETE_ROUTE_GATE' -and [bool]$routeHealth.publicationAuthorized -and -not [bool]$routeHealth.retryAuthorized) 'S17B1 inherited route health changed.'
Assert-True ([int]$routeHealth.unresolvedEarlierAcceptedRequestCount -eq 0 -and [bool]$routeHealth.argosInboundRelayCurrentHealthProved) 'S17B1 route has unresolved earlier work.'
Assert-True ([string]$routeHealth.endpointWorkerSha256 -eq $expectedWorkerSha256 -and [string]$routeHealth.installedEndpointConfigSha256 -eq $expectedConfigSha256) 'S17B1 installed route pins changed.'
Assert-True ([string]$terminal.state -eq 'PASS_O2D11_SIGNED_SLOT16_DEVELOPMENT_RESULT' -and [bool]$terminal.signedResponseVerified) 'S17B1 predecessor terminal evidence changed.'
...
Assert-True ($currentBranch -eq $branch -and $localTip -eq $remoteTip) 'S17B1 requires matching local/origin branch tips.'

foreach ($path in @($partialRoot,$signedRoot,$finalRoot,$packageGatePath,$routeGatePath,$pathSidecar)) { Assert-True (-not (Test-Path -LiteralPath $path)) "S17B1 fresh output exists: $path" }
...
$routePaths = @(
    ...
    'C:\S17B1R.zip',
    'C:\S17B1R\PORTAL_RESPONSE_MANIFEST.json',
    'C:\S17B1R\DATA_PULL_PAYLOAD.zip',
    ('C:\S17B1D\' + $longestRelative)
)
$pathGate = & $pathTool -CandidatePath $routePaths -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
Assert-True ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'S17B1 complete route path budget failed.'
...
Assert-True ([bool]$certificate.HasPrivateKey) 'S17B1 signer private key is unavailable.'

if ($Preflight) {
    [ordered]@{schema='argos_s17b1_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S17B1_BUILD_PREFLIGHT';requestId=$requestId;relativePathCount=2;maximumFiles=2;maximumBytes=4194304;routePathCount=$routePaths.Count;maximumEffectiveLength=$maximumEffective;maximumComponentLength=$maximumComponent;branch=$branch;localTip=$localTip;remoteTip=$remoteTip;mutationsPerformed=$false;jbodContacted=$false;imageBytesRequested=0;slots22Through25Exposed=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}
...
Assert-True ([string]$folderTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'S17B1 signed folder validation failed.'
...
Assert-True ([string]$extractTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE' -and @(Get-ChildItem -LiteralPath $extractRoot -File).Count -eq 2) 'S17B1 exact ZIP extraction validation failed.'
...
[ordered]@{schema='argos_s17b1_build_result_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_S17B1_EXACT_SIGNED_DATA_PULL_READY';requestId=$requestId;requestZip=$finalZip;requestZipBytes=$zipBytes;requestZipSha256=$zipSha256;packageGate=$packageGatePath;routeGate=$routeGatePath;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
