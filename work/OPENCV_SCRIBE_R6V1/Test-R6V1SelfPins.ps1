...
function Get-LiteralPin([string]$Text, [string]$Name) {
    ...
    Assert-True ($matches.Count -eq 1) "R6V1 endpoint requires exactly one literal pin: $Name"
...
$gatePath = Join-Path $PSScriptRoot 'R6V1_SELF_PIN_GATE.json'

foreach ($path in @($endpoint,$engine,$batchPath,$continuityPath,$o2d5InvocationPath,$goodFixture,$badBatchFixture,$badInstallationFixture)) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "R6V1 self-pin dependency absent: $path"
}
Assert-True (-not (Test-Path -LiteralPath $gatePath)) 'R6V1 self-pin gate exists.'
...
Assert-True ($installationMatches.Count -eq 1) 'R6V1 continuity installation pin cardinality changed.'
...
foreach ($name in @('engineSha','batchSha','refsSha','installationSha')) {
    $observed = Get-LiteralPin $endpointText $name
    $expectedValue = [string]$expected[$name]
    Assert-True ($observed -eq $expectedValue) "R6V1 endpoint self-pin mismatch: $name"
    $rows.Add([pscustomobject]@{id=$name;endpointValue=$observed;evidenceValue=$expectedValue;match=$true})
}
Assert-True (@($batch.cases).Count -eq 4) 'R6V1 batch case count changed.'
foreach ($case in @($batch.cases)) {
    $jobPath = Join-Path $PSScriptRoot ([string]$case.jobFile)
    Assert-True ((Get-Sha256 $jobPath) -eq [string]$case.jobSha256) "R6V1 batch job self-pin changed: $($case.slot)"
}

if ($Preflight) {
    [ordered]@{schema='argos_r6v1_self_pin_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R6V1_SELF_PIN_PREFLIGHT';endpointSha256=Get-Sha256 $endpoint;pinCount=$rows.Count;jobPinCount=4;plannedLiveBranchCases=3;mutationsPerformed=$false;processStarted=$false;sourceImageBytesRead=$false;providerActivated=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}

$good = (& $endpoint -Preflight -LiveContractFixtureManifest $goodFixture | Out-String) | ConvertFrom-Json
Assert-True ([string]$good.state -eq 'PASS_R6V1_LIVE_CONTRACT_FIXTURE' -and [bool]$good.batchAssertionExecuted -and [bool]$good.installationAssertionExecuted -and -not [bool]$good.mutationsPerformed) 'R6V1 positive live-contract fixture failed.'
$badBatchRefused = $false
try { & $endpoint -Preflight -LiveContractFixtureManifest $badBatchFixture 2>&1 | Out-Null }
catch { $badBatchRefused = $_.Exception.Message -eq 'R6V1 live batch manifest changed.' }
Assert-True $badBatchRefused 'R6V1 bad-batch fixture was not refused by the exact assertion.'
$badInstallationRefused = $false
try { & $endpoint -Preflight -LiveContractFixtureManifest $badInstallationFixture 2>&1 | Out-Null }
catch { $badInstallationRefused = $_.Exception.Message -eq 'R6V1 runtime installation changed.' }
Assert-True $badInstallationRefused 'R6V1 bad-installation fixture was not refused by the exact assertion.'

$gate = [ordered]@{schema='argos_r6v1_self_pin_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R6V1_SELF_PIN_AND_LIVE_BRANCH_GATE';endpointSha256=Get-Sha256 $endpoint;pins=$rows.ToArray();pinCount=$rows.Count;pinMatchCount=@($rows | Where-Object { $_.match }).Count;jobPinCount=4;liveBranchCases=@([pscustomobject]@{id='GOOD_BATCH_AND_INSTALLATION';state='PASS'},[pscustomobject]@{id='BAD_BATCH_HASH_REFUSED';state='PASS';failureBeforeImageRead=$true},[pscustomobject]@{id='BAD_INSTALLATION_HASH_REFUSED';state='PASS';failureBeforeImageRead=$true});normalLiveFixtureCallCount=3;rehearsalBranchAloneSufficient=$false;targetExecuted=$false;mutationsPerformed=$false;imageBytesRead=$false;providerActivated=$false;reviewOnly=$true;productionRoutingEnabled=$false}
