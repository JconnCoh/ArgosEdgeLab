...
function Require-Pin([string]$Path, [string]$Sha256, [string]$State = '') {
    Require (Test-Path -LiteralPath $Path -PathType Leaf) "R18N build dependency absent: $Path"
    Require ((Get-Sha256 $Path) -eq $Sha256) "R18N build dependency changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($State)) {
        $value = Get-Content -Raw -LiteralPath $Path | ConvertFrom-Json
        Require ([string]$value.state -eq $State) "R18N build dependency state changed: $Path"
    }
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 32) {
    Require (-not (Test-Path -LiteralPath $Path)) "R18N build create-new JSON exists: $Path"
...
$requestId = 'REQ_R18N1'
$revision = 'R18N_FULL_KLARF_JBOD_REVIEW_ONLY_20260904A'
$payloadRevision = 'R18J_FULL_KLARF_JBOD_REVIEW_ONLY_20260904A'
$entrypoint = Join-Path $PSScriptRoot 'Invoke-R18NCorpusLaunch.ps1'
...
$pathGatePath = Join-Path $PSScriptRoot 'R18N_PATH_PLAN_GATE.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_R18N_FULL_KLARF_PREPARATION.json'
$stageRoot = 'C:\R18NP'
...
$finalGatePath = Join-Path $PSScriptRoot 'R18N_FINAL_PACKAGE_GATE.json'
...
$entrypointSha = '0B75F178A4D73668DBD76E0F07A820E16897DA40D0466A6FBA84806A8AFCD746'
$payloadManifestSha = 'BF2D0E809915ADEB503E2B7ADEFB3DCAC30903025D851B26067C9FB5B90DC57D'
$definitionSha = '640230EECD8CA8C65767E52C9C436791EE10AEAA5EDB35F89B1A5D1D414292E1'
$localGateSha = '2803AEE7EF2C1C8993BF279F4F3C1F305B375E4018E1DFC344A8A678A76EA03E'
$pathGateSha = 'A2292407D871AE3464E7170866622714A81D0C3DF3BB0968D6237C10443835DF'
$preactionSha = 'AAF77F58FCF74002EDA06012207182D0A80459153D43A962684FE4A2D94E1460'
...
Require (Test-Path -LiteralPath $localSource -PathType Container) 'R18N local rehearsal source absent.'
Require (Test-Path -LiteralPath $localProposals -PathType Container) 'R18N local rehearsal proposal root absent.'
...
Require ([string]$preactionResult.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'R18N preaction gate changed.'
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Require ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH' -and [string]$definition.entryPoint -eq 'payload/Invoke-R18NCorpusLaunch.ps1') 'R18N maintenance route changed.'
Require (@($definition.changes).Count -eq 1 -and [string]$definition.changes[0].installedSha256 -eq $entrypointSha -and [bool]$definition.changes[0].allowCreate) 'R18N maintenance change changed.'
Require (@($definition.changes[0].approvedPredecessorSha256).Count -eq 1 -and [string]$definition.changes[0].approvedPredecessorSha256[0] -eq $entrypointSha) 'R18N create-only idempotent installed-hash boundary changed.'
Require (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R18N process/task action cardinality changed.'
Require ([string]$definition.sourceProcessingContract.proposalRoot -eq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals') 'R18N observed proposal root changed.'
Require ([string]$definition.entryPointMutations[0].targetRoot -eq 'D:\A2\w\ocv\R18N1' -and [string]$definition.entryPointMutations[1].targetRoot -eq 'D:\A2\o\ocv\R18N1') 'R18N declared roots changed.'
Require (-not [bool]$definition.publication.explicitOperatorAuthorityPresent) 'R18N preparation cannot claim publication authority.'
Require ([bool]$definition.reviewOnly -and -not [bool]$definition.productionRoutingEnabled -and -not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed) 'R18N authority changed.'
$payloadManifest = Get-Content -Raw -LiteralPath $payloadManifestPath | ConvertFrom-Json
Require ([string]$payloadManifest.schema -eq 'argos_opencv_scribe_r18j_payload_manifest_v1' -and [string]$payloadManifest.revision -eq $payloadRevision -and @($payloadManifest.files).Count -eq 23) 'R18N inherited payload manifest shape changed.'
...
foreach ($file in $payloadFiles) {
    ...
    Require ((Get-Item -LiteralPath $source).Length -eq [int64]$file.bytes) "R18N payload length changed: $($file.sourcePath)"
}
...
try {
    ...
    Require ($matches.Count -eq 1 -and $matches[0].HasPrivateKey) 'R18N signer certificate or private key changed.'
...
foreach ($path in @($stageRoot,$readyRoot,$stageZip,$verifyRoot,$finalRoot,$finalPartial,$finalGatePath)) {
    Require (-not (Test-Path -LiteralPath $path)) "R18N build fresh output exists: $path"
}
if ($Preflight) {
    [ordered]@{schema='argos_opencv_scribe_r18n_build_preflight_v1';createdUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R18N_BUILD_PREFLIGHT';requestId=$requestId;payloadFileCount=25;entrypointSha256=$entrypointSha;payloadManifestSha256=$payloadManifestSha;definitionSha256=$definitionSha;localGateSha256=$localGateSha;pathPlanGateSha256=$pathGateSha;preactionSha256=$preactionSha;signerThumbprint=$thumbprint;publicationAuthorized=$false;mutationsPerformed=$false;targetExecuted=$false;reviewOnly=$true;productionRoutingEnabled=$false} | ConvertTo-Json -Depth 8
    return
}

[void](New-Item -ItemType Directory -Path (Join-Path $readyRoot 'payload\files'))
Copy-Item -LiteralPath $entrypoint -Destination (Join-Path $readyRoot 'payload\Invoke-R18NCorpusLaunch.ps1')
...
$manifestFiles.Add([ordered]@{path='payload/Invoke-R18NCorpusLaunch.ps1';bytes=[int64](Get-Item -LiteralPath $entrypoint).Length;sha256=$entrypointSha})
...
Require ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18N signed package verification failed.'
...
Require ($extracted.Count -eq 27) 'R18N extracted final ZIP file count changed.'
foreach ($row in $manifestFiles) {
    $path = Join-Path $verifyRoot ([string]$row.path).Replace('/','\')
    Require-Pin $path ([string]$row.sha256)
    Require ((Get-Item -LiteralPath $path).Length -eq [int64]$row.bytes) "R18N extracted payload length changed: $($row.path)"
}
...
Require ([string]$exactPackageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18N extracted signed package verification failed.'
...
Require ($actualLeaves.Count -eq 27 -and $actualLeafSetSha -eq '02D888A6921BF16695C262B821FF894DE5BCF3AF42F5E192B544491C233003B8') 'R18N final ZIP membership differs from the path plan.'
$priorFailureLeaf = 'payload/files/OPENCV_SCRIBE_R18F/reference_bank/SUPPLEMENTAL_GLYPH_REFERENCE_MANIFEST.json'
Require ($actualLeaves -contains $priorFailureLeaf) 'R18N prior failing leaf is absent from exact enumeration.'
$expandedRoots = @($readyRoot,$verifyRoot,'C:\ProgramData\ArgosProjectPortalRO\endpoint_jbod\pending\REQ_R18N1.ready')
...
foreach ($workRoot in @('D:\A2\w\ocv\R18N1.partial','D:\A2\w\ocv\R18N1')) {
    foreach ($file in $payloadFiles) {
...
Require ($pathRows.Count -eq 127) 'R18N exact expanded path cardinality changed.'
Require ([int]$longest.effectiveLength -lt 200) "R18N exact final ZIP member exceeds path budget: $($longest.path)"
Require ([int](($pathRows | Measure-Object maximumComponentLength -Maximum).Maximum) -le 80) 'R18N exact final ZIP member component exceeds path budget.'
$packagedPreflight = (& (Join-Path $verifyRoot 'payload\Invoke-R18NCorpusLaunch.ps1') -Preflight -Rehearsal -PayloadRoot (Join-Path $verifyRoot 'payload') -WorkRoot 'C:\R18NFW' -OutputRoot 'C:\R18NFO' -SourceRoot $localSource -ProposalRoot $localProposals -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localRefs -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
Require ([string]$packagedPreflight.state -eq 'PASS_R18J_CORPUS_LAUNCH_PREFLIGHT' -and -not [bool]$packagedPreflight.mutationsPerformed -and -not [bool]$packagedPreflight.processStarted) 'R18N exact packaged entrypoint preflight failed.'
...
$exactRouteGate = [ordered]@{
    schema='argos_opencv_scribe_r18n_complete_route_gate_v1';checkedUtc=[DateTime]::UtcNow.ToString('o');state='PASS_R18N_COMPLETE_ROUTE_GATE';
...
$finalGate = [ordered]@{
    schema='argos_opencv_scribe_r18n_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
    state='PASS_R18N_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip=('work/OPENCV_SCRIBE_R18N/final/'+$zipName);
