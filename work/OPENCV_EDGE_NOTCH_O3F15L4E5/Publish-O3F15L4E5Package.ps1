...
$expectedInvocation = Join-Path $PSScriptRoot 'O3F15L4E5_PUBLISH_P2_INVOCATION.json'
...
Assert-True ([string](Get-RequiredProperty $invocation 'schema') -ceq 'argos_ocv03_o3f15l4e5_publish_p2_invocation_v1') 'D2 publication invocation schema changed.'
Assert-True ([string](Get-RequiredProperty $invocation 'state') -ceq 'FROZEN_O3F15L4E5_EXACTLY_ONCE_PUBLICATION_P2_INVOCATION') 'D2 publication invocation is not frozen.'
...
Assert-True ([string]$routeGate.schema -ceq 'argos_ocv03_o3f15l4e5_complete_route_path_r4_gate_v1' -and [string]$routeGate.state -ceq 'PASS_O3F15L4E5_COMPLETE_ROUTE_PATH_R4_GATE' -and [string]$routeGate.requestId -ceq $requestId -and [string]$routeGate.signedPackageZipSha256 -ceq $sourceHash -and [int]$routeGate.publicationCountMaximum -eq 1 -and -not [bool]$routeGate.requestRetryAuthorized) 'D2 prepublication route gate changed.'
...
foreach ($row in $closureRows) {
    ...
    Assert-True ($priorId -ceq 'REQ_20260903T164503516Z_EE1927560A46' -and [string](Get-RequiredProperty $row 'responseId') -ceq 'R_CAEC226461A1_20260903164953033_587db049') 'E5 immediate prior serial request is not terminal O3F15L4E4.'
...
Assert-True ($publicationGatePath.Equals((Join-Path $PSScriptRoot 'O3F15L4E5_PUBLISH_GATE.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication gate path changed.'
Assert-True ($publicationAttemptPath.Equals((Join-Path $PSScriptRoot 'O3F15L4E5_PUBLISH_ATTEMPT.json'), [StringComparison]::OrdinalIgnoreCase)) 'D2 publication-attempt receipt path changed.'
...
if ($attemptReceiptExists) {
    ...
    Assert-True ([string](Get-RequiredProperty $attemptReceipt 'schema') -ceq 'argos_ocv03_o3f15l4e5_publish_attempt_v1' -and [string](Get-RequiredProperty $attemptReceipt 'state') -ceq 'STARTED_O3F15L4E5_SINGLE_PUBLICATION_ATTEMPT') 'D2 publication-attempt receipt identity changed.'
...
$preflightResult = [ordered]@{
    schema = 'argos_ocv03_o3f15l4e5_publish_preflight_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O3F15L4E5_EXACTLY_ONCE_PUBLICATION_PREFLIGHT'
    requestId = $requestId
    sourceZip = $sourceZip
...
if ($queueState -ceq 'NEW') {
    $attemptReceipt = [ordered]@{
        schema = 'argos_ocv03_o3f15l4e5_publish_attempt_v1'
        createdUtc = [DateTime]::UtcNow.ToString('o')
        state = 'STARTED_O3F15L4E5_SINGLE_PUBLICATION_ATTEMPT'
        requestId = $requestId
        sourceZip = $sourceZip
        sourceZipBytes = $sourceBytes
        sourceZipSha256 = $sourceHash
...
$gate = [ordered]@{
    schema = 'argos_ocv03_o3f15l4e5_publish_gate_v1'
    createdUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_O3F15L4E5_PUBLISHED_EXACTLY_ONCE_AWAITING_MATCHING_SIGNED_RESPONSE'
