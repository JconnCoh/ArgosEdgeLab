...
def reference_root_key(roots: dict[str, Path]) -> tuple[tuple[str, str], ...]:
    return tuple(sorted((str(prefix), normalized_path(path)) for prefix, path in roots.items()))


class ReferenceBankCache:
    """Process-local cache for immutable, fully pinned reference banks."""

    STAGES = ("baseAppearance", "combinedAppearanceSupplement", "topology", "runStructure")
    ...
    def __init__(self, config: dict[str, Any], context: dict[str, Any]) -> None:
        ...
        self.base = (
            normalized_path(references["manifestPath"]),
            str(references["manifestSha256"]).upper(),
            reference_root_key(roots),
        )
        self.supplement = (
            normalized_path(references["supplementalManifestPath"]),
            str(references["supplementalManifestSha256"]).upper(),
        )
        binding = (
            context["providerSha256"], context["r11AnalyzerSha256"],
            self.base, self.supplement,
        )
    ...
    def clone(value: Any) -> Any:
        if isinstance(value, tuple) and len(value) == 2 and isinstance(value[0], list):
            return list(value[0]), copy.deepcopy(value[1])
        if isinstance(value, list):
            return list(value)
        raise TypeError("Reference loader returned an unsupported cache value.")

    def load(self, stage: str, observed: Any, expected: Any, loader: Any, *args: Any) -> Any:
        if stage not in self.STAGES or observed != expected:
            raise ValueError(f"{stage} reference-cache binding mismatch.")
        if stage in self.values:
            self.hits[stage] += 1
        else:
            self.misses[stage] += 1
            self.values[stage] = self.clone(loader(*args))
        return self.clone(self.values[stage])

    def snapshot(self) -> dict[str, Any]:
        return {
            "state": "PASS_EXACT_PROCESS_LOCAL_REFERENCE_BANK_CACHE",
            "bindingSha256": self.binding_sha256,
            "parallelism": "NONE_SERIAL_PROCESS_LOCAL",
            "stages": {stage: {"hits": self.hits[stage], "misses": self.misses[stage]} for stage in self.STAGES},
        }
...
def assert_lexical_alias_mapping(
    execution_path: Path,
    canonical_path: Path,
    execution_root: Path,
    canonical_root: Path,
    ...
    """Bind one alias leaf to canonical provenance without touching that canonical leaf."""

    try:
        execution_relative = execution_path.relative_to(execution_root)
        canonical_relative = canonical_path.relative_to(canonical_root)
    except ValueError as error:
        raise ValueError(f"Source leaf escaped its declared root: {label}") from error
    execution_text = os.path.normcase(os.path.normpath(str(execution_relative)))
    canonical_text = os.path.normcase(os.path.normpath(str(canonical_relative)))
    if execution_text != canonical_text:
        raise ValueError(f"Alias/canonical relative mapping changed: {label}")
    return str(canonical_relative)


def assert_alias_leaf_has_no_reparse_ancestor(
    execution_path: Path,
    execution_root: Path,
    label: str,
) -> None:
    """Reject every reparse component below the one intentional root alias."""

    try:
        relative = execution_path.relative_to(execution_root)
    except ValueError as error:
        raise ValueError(f"Execution source escaped the proposal-root alias: {label}") from error
    if not relative.parts:
        raise ValueError(f"Execution source has no bounded relative leaf: {label}")
    current = execution_root
    for component in relative.parts:
        current = current / component
        if is_link_or_reparse(current):
            raise ValueError(
                f"Execution source has an absent or reparse component below the root alias: {label}"
            )
...
def source_fingerprints(
    ...
    for name in SOURCE_NAMES:
        ...
        assert_alias_leaf_has_no_reparse_ancestor(execution_path, execution_root, name)
        relative_mapping = assert_lexical_alias_mapping(
            execution_path,
            canonical_path,
            execution_root,
            canonical_root,
        ...
        values[name] = {
            **fingerprint,
            ...
            "canonicalBindingBasis": "VERIFIED_ROOT_JUNCTION_TARGET_AND_EXACT_RELATIVE_MAPPING",
            "canonicalRelativePath": relative_mapping,
            "canonicalProvenanceLexicalOnly": True,
            "canonicalLeafFilesystemIoPerformed": False,
...
def validate_source_stability(case: dict[str, Any], context: dict[str, Any]) -> None:
    execution_paths = {
...
def run_public_provider_once_with_attempt_audit(
    ...
    reference_cache: ReferenceBankCache,
    ...
    audit.update({
        ...
        "referenceBankLoadersRestored": False,
    })
    try:
    ...
    try:
        supplement_owner = provider.R18ZV.R18Z_LOADER
        original_base = r11.load_reference_prototypes
        original_combined = supplement_owner.combine_reference_prototypes
        original_topology = provider.R17D.load_topology_prototypes
        original_run_structure = provider.R18H.load_run_structure_prototypes
    except AttributeError as error:
        raise ValueError("Provider reference-bank cache seam is unavailable.") from error

    def load_base(manifest: Path, manifest_sha: str, roots: dict[str, Path]) -> Any:
        observed = (normalized_path(manifest), str(manifest_sha).upper(), reference_root_key(roots))
        return reference_cache.load("baseAppearance", observed, reference_cache.base, original_base, manifest, manifest_sha, roots)

    def load_combined(active_r11: Any, base: list[Any], evidence: dict[str, Any], manifest: Path, manifest_sha: str) -> Any:
        observed = (normalized_path(manifest), str(manifest_sha).upper())
        return reference_cache.load("combinedAppearanceSupplement", observed, reference_cache.supplement, original_combined, active_r11, base, evidence, manifest, manifest_sha)

    def load_structural(stage: str, loader: Any, active_r11: Any, manifest: Path, manifest_sha: str, roots: dict[str, Path], supplement: Path | None = None, supplement_sha: str = "") -> Any:
        observed = (normalized_path(manifest), str(manifest_sha).upper(), reference_root_key(roots), normalized_path(supplement) if supplement else "", str(supplement_sha).upper())
        return reference_cache.load(stage, observed, reference_cache.base + reference_cache.supplement, loader, active_r11, manifest, manifest_sha, roots, supplement, supplement_sha)

    def load_topology(*args: Any, **kwargs: Any) -> Any:
        return load_structural("topology", original_topology, *args, **kwargs)

    def load_run_structure(*args: Any, **kwargs: Any) -> Any:
        return load_structural("runStructure", original_run_structure, *args, **kwargs)

    r11.load_reference_prototypes = load_base
    supplement_owner.combine_reference_prototypes = load_combined
    provider.R17D.load_topology_prototypes = load_topology
    provider.R18H.load_run_structure_prototypes = load_run_structure
    ...
    try:
        ...
        r11.load_reference_prototypes = original_base
        supplement_owner.combine_reference_prototypes = original_combined
        provider.R17D.load_topology_prototypes = original_topology
        provider.R18H.load_run_structure_prototypes = original_run_structure
        ...
        audit["referenceBankLoadersRestored"] = all((
            r11.load_reference_prototypes is original_base,
            supplement_owner.combine_reference_prototypes is original_combined,
            provider.R17D.load_topology_prototypes is original_topology,
            provider.R18H.load_run_structure_prototypes is original_run_structure,
        ))
        audit["referenceBankCache"] = reference_cache.snapshot()
    ...
    if not audit["referenceBankLoadersRestored"]:
        raise RuntimeError("Provider reference-bank loader restoration failed.")
    if analyze_calls != 1:
...
def run_case(
    ...
    validate_source_stability(case, context)
    try:
...
def execute_batch(
    ...
    *,
    ...
    try:
        ...
        reference_cache = ReferenceBankCache(config, context)
        ...
        for index, case in enumerate(inventory["cases"], 1):
            ...
            rows.append(run_case(config, context, case, cases_root, provider, reference_cache))
        ...
        aggregate = {
            ...
            "referenceBankCache": reference_cache.snapshot(),
            "provider": {
