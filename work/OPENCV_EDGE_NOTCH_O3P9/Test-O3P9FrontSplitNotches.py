#!/usr/bin/env python3
"""Synthetic and structural controls for the O3P9 frontside channel split."""
...
SCHEMA = "argos_ocv03_o3p9_synthetic_invocation_v1"
...
def pattern_suppression_controls(engine: Any, topology: Any, job: dict[str, Any]) -> dict[str, Any]:
    width, inward, radius = 1000, 420, 5150.0
    geometry = topology.circle_geometry(width, float(inward), radius)
    support = np.ones(width, dtype=np.float32)
    x = np.arange(width, dtype=np.float32)
    observations: list[dict[str, Any]] = []
    for center in (260, 500, 740):
        broad = 9.0 * np.exp(-0.5 * np.square((x - float(center)) / 58.0))
        pattern = 12.0 * np.exp(-0.5 * np.square((x - float(center + 20)) / 0.7))
        edge = geometry - broad - pattern
        expected_angle = engine.contour_angle(float(center), float(edge[center]), width, float(inward), radius, 120.0)
        feature = engine.best_topology_feature(edge, geometry, support, 120.0, inward, radius, expected_angle, 2.0, job)
        observations.append(
            {
                "syntheticNotchCenterX": center,
                "selectedTipX": None if feature is None else int(feature["tipX"]),
                "patternSuppressionWidthPx": None if feature is None else int(feature["patternSuppressionWidthPx"]),
                "passed": feature is not None and abs(int(feature["tipX"]) - center) <= 4,
            }
        )
    return {"observations": observations, "passed": all(bool(row["passed"]) for row in observations)}
...
def main() -> int:
    ...
    require(invocation.get("schema") == SCHEMA, "O3P9 synthetic invocation schema changed.")
    require(invocation.get("reviewOnly") is True and invocation.get("productionRoutingEnabled") is False, "O3P9 synthetic authority widened.")
    ...
    require(
        ...
        "O3P9 synthetic output must be create-new.",
    )
    ...
    if arguments.preflight:
        result = {
            "state": "PASS_O3P9_SYNTHETIC_PREFLIGHT",
        ...
        engine = load_module("argos_o3p9_synthetic_engine", engine_path)
        topology = load_module("argos_o3p9_synthetic_topology", topology_path)
        ...
        suppression = pattern_suppression_controls(engine, topology, job)
        passed = all(bool(case["passed"]) for case in cases) and decision_passed and bool(structural["passed"]) and bool(exceptions["passed"]) and bool(suppression["passed"])
        result = {
            "schema": "argos_ocv03_o3p9_front_split_notch_synthetic_gate_v1",
            "createdUtc": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
            "state": "PASS_O3P9_FRONT_SPLIT_NOTCH_SYNTHETIC_GATE" if passed else "FAIL_O3P9_FRONT_SPLIT_NOTCH_SYNTHETIC_GATE",
            ...
            "patternSuppressionControls": suppression,
