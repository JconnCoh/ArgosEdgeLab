...
function Assert-Pin([string]$Path,[string]$Sha){Assert-True(Test-Path -LiteralPath $Path -PathType Leaf)"O3C2 publisher dependency absent: $Path";Assert-True((Get-Sha256 $Path)-eq$Sha)"O3C2 publisher dependency changed: $Path"}
...
$checkpointPath=Join-Path $project 'work\FRONTSIDE_INSPECTION_REVIEW_ONLY\OCV03_O3C2_SOURCE_FREEZE_PUBLISH_READY_20260827.md'
$pathTool=Join-Path $project 'utilities\Confirm-ArgosPathBudget.ps1'
$publishGatePath=Join-Path $PSScriptRoot 'O3C2_PUBLISH_GATE.json'
...
$checkpointSha='F2EDC99A503A60B85FA9CF2AE2104E7EBA33B9D2D957EDBE0D684CF595338911'
$invocationSha='0738510BC1F7CED021A0778B4DDE845A146A1EDA71EB2B9FCE9B4D5152CE1E84'
...
Assert-True([string]$invocation.schema-eq'argos_o3c2_publish_invocation_v1'-and[string]$invocation.publisherRevision-eq'Publish-O3C2'-and[string]$invocation.requestId-eq$requestId)'O3C2 publisher invocation identity changed.'
Assert-True([string]$invocation.requestZipSha256-eq$zipSha-and[string]$invocation.checkpointSha256-eq$checkpointSha-and[string]$invocation.branch-eq$branch)'O3C2 publisher invocation pins changed.'
Assert-True([int]$invocation.maximumPublications-eq1-and-not[bool]$invocation.retryAuthorized-and[bool]$invocation.matchingSignedResponseCollectionOnly-and[bool]$invocation.requireCleanMatchingBranchTips-and[bool]$invocation.requireZeroPendingShareRequests)'O3C2 publisher one-shot boundary changed.'
Assert-True([bool]$invocation.reviewOnly-and-not[bool]$invocation.trainingEligible-and-not[bool]$invocation.xmlEligible-and-not[bool]$invocation.productionRoutingEnabled-and-not[bool]$invocation.sourceHashingPerformed-and-not[bool]$invocation.imageBytesDecoded-and-not[bool]$invocation.pixelProcessingPerformed-and-not[bool]$invocation.knownNotchLocationConsumed-and-not[bool]$invocation.notchAnglePriorConsumed-and-not[bool]$invocation.fixedAngularSearchWindowConsumed-and-not[bool]$invocation.providerActivated)'O3C2 publisher authority widened.'
...
Assert-True((Get-Item -LiteralPath $sourceZip).Length-eq$zipBytes)'O3C2 ZIP byte count changed.'
...
Assert-True($currentBranch-eq$branch-and$localTip-eq$remoteTip)'O3C2 publisher requires matching local/origin branch tips.'
$worktree=@(& git -C $project status --porcelain=v1)
Assert-True($worktree.Count-eq0)'O3C2 publisher requires a clean worktree.'
...
Assert-True($null-ne$logical-and[int]$logical.DriveType-eq4-and(Normalize-Root([string]$drive.DisplayRoot)).Equals((Normalize-Root $shareRoot),[StringComparison]::OrdinalIgnoreCase)-and(Normalize-Root([string]$logical.ProviderName)).Equals((Normalize-Root $shareRoot),[StringComparison]::OrdinalIgnoreCase))'O3C2 persistent U: mapping changed.'
Assert-True(Test-Path -LiteralPath $requestRoot -PathType Container)'O3C2 request share unavailable.'
...
Assert-True($pending.Count-eq0)('O3C2 another request is pending: '+(($pending|ForEach-Object{$_.Name})-join', '))
foreach($p in @($uploadPath,$readyPath,$processedPath,$publishGatePath)){Assert-True(-not(Test-Path -LiteralPath $p))"O3C2 create-new publication target exists: $p"}
