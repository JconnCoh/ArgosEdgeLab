...
$script:ExpectedInputSchema = 'argos_r18uq2_roster_query_input_v1'
...
function Write-NewUtf8JsonResult {
    ...
    return [pscustomobject][ordered]@{
        schema='argos_r18uq2_result_write_receipt_v1'
        state='PASS_R18UQ2_RESULT_WRITTEN_CREATE_NEW'
        path=$resolved
...
function Get-QueryInput {
    ...
    if ([string]$value.schema -cne $script:ExpectedInputSchema) { throw 'R18UQ2 input schema mismatch.' }
    if ([string]$value.sourceCatalogSha256 -cne $script:ExpectedCatalogSha256) { throw 'R18UQ2 source catalog hash mismatch.' }
    if ([string]$value.queryKeyFingerprintSha256 -cne $script:ExpectedKeyFingerprintSha256) { throw 'R18UQ2 declared key fingerprint mismatch.' }
    $keys = @($value.queryKeys | ForEach-Object { ([string]$_).Trim().ToUpperInvariant() })
    if ($keys.Count -ne $script:ExpectedKeyCount -or @($keys | Sort-Object -Unique).Count -ne $script:ExpectedKeyCount) { throw 'R18UQ2 requires exactly 50 unique query keys.' }
    foreach ($key in $keys) { if ($key -notmatch '^\d{5}-\d{3}[A-Z]?$') { throw "Malformed R18UQ2 query key: $key" } }
    $fingerprint = Get-Sha256Hex -Bytes ([Text.UTF8Encoding]::new($false).GetBytes(($keys -join "`n")))
    if ($fingerprint -cne $script:ExpectedKeyFingerprintSha256) { throw 'R18UQ2 exact ordered key set mismatch.' }
...
function Get-LiveSqlRows {
    ...
    try {
        ...
        $builder['Connect Timeout']=10; $builder['Application Name']='ArgosEdgeLabR18UQ2ReadOnlyRoster'
...
function Get-LiveOdbcRows {
    ...
    try {
        ...
        $builder['Pwd']=$SuppliedCredential.GetNetworkCredential().Password; $builder['APP']='ArgosEdgeLabR18UQ2ReadOnlyRoster'
...
function Get-RehearsalRows {
    ...
    if ([string]$snapshot.Value.schema -cne 'argos_r18uq2_sql_row_rehearsal_v1') { throw 'R18UQ2 rehearsal schema mismatch.' }
    ...
    foreach ($rows in @($direct,$issue,$associate,$disassociate)) { if ($rows.Count -gt $script:MaximumRowsPerSource) { throw 'R18UQ2 rehearsal row cap exceeded.' } }
    return [pscustomobject]@{
...
function Build-RosterResult {
    ...
    foreach ($row in @($SqlRows.IssueRows)) {
        ...
        $queryLot=$null; $membershipBasis='NO_EXACT_QUERY_PARENT_ON_ISSUE_ROW'
        if ($null -ne $parent -and $catalog.Contains($parent)) { $queryLot=$parent; $membershipBasis='EXACT_CURRENT_PARENT_FROM_ISSUE_ROW' }
    ...
    foreach ($sourceSpec in @(
        ...
        foreach ($row in @($sourceSpec.Rows)) {
            ...
            if (-not (Test-UnitContainerSyntax $unit)) { $codes.Add('HISTORICAL_CHILD_CONTAINER_INVALID') }
    ...
    if ($invalid.Count -gt $script:MaximumDiagnosticRows) { throw 'R18UQ2 invalid-row cap exceeded.' }
    ...
    if ($invalid.Count -gt $script:MaximumDiagnosticRows) { throw 'R18UQ2 diagnostic cap exceeded after reconciliation.' }
    ...
    if ($zeroHoldComplete) { $disposition='COMPLETE'; $state='PASS_R18UQ2_EXHAUSTIVE_ROSTER' }
    elseif ($usableWithHolds) { $disposition='USABLE_WITH_HOLDS'; $state='PASS_R18UQ2_USABLE_ROSTER_WITH_EXPLICIT_HELD_MEMBERS' }
    else { $disposition='HOLD'; $state='HOLD_R18UQ2_ROSTER_NOT_USABLE' }
    ...
    $ignoredHistorical=$historicalMembership.Count-$usedHistoricalRows.Count
    ...
    return [pscustomobject][ordered]@{
        ...
        counts=[ordered]@{
            ...
            ignoredHistoricalMembershipRows=$ignoredHistorical
        }
        lots=$lots.ToArray()
        ...
        holds=$holds.ToArray()
        invariants=[ordered]@{
            readOnlySql=$true; parameterizedLotKeys=$true; exactDatabaseParentEvidenceRequired=$true
...
if ($Preflight) {
    ...
    $preflightResult=[ordered]@{
        schema='argos_r18uq2_query_preflight_v1'; state='PASS_R18UQ2_QUERY_PREFLIGHT'
        ...
        exactDatabaseParentEvidenceRequired=$true; issuedUnitSuffixInferenceUsed=$false; historicalMembershipFallbackOnly=$true
