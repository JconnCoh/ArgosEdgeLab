...
function Add-UniquePath([Collections.Generic.List[string]]$List, [Collections.Generic.HashSet[string]]$Set, [string]$Path) {
    Assert-True (-not [string]::IsNullOrWhiteSpace($Path)) 'O2D13 route path is empty.'
    Assert-True ($Set.Add($Path)) "O2D13 duplicate route path: $Path"
    $List.Add($Path)
}
...
$finalGatePath = Join-Path $root 'O2D13_FINAL_PACKAGE_GATE.json'
$zipPath = Join-Path $root ('final\' + $requestReadyName + '.zip')
$extractRoot = Join-Path $root 'final\extract'
$outputPath = Join-Path $root 'O2D13_COMPLETE_ROUTE_GATE.json'
...
foreach ($dependency in @($finalGatePath,$zipPath,(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json'),(Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig'),$pathTool,$packageTester,$publicCertificate,$jeoPath,$queueGatePath)) {
    Assert-True (Test-Path -LiteralPath $dependency -PathType Leaf) "O2D13 route dependency absent: $dependency"
}
Assert-True ((Get-Sha256 $finalGatePath) -eq '320382D295E21CC51F24C6CDAC3D9242110ECC36F29EB554A0EBDAB335A98EDC') 'O2D13 final package gate changed.'
Assert-True ((Get-Sha256 $jeoPath) -eq '91B5A9219F72845187D7CB17DAB0F1D74AF223E7DDA11E380EEA26148C60CD01') 'O2D13 direct JBOD observation changed.'
Assert-True ((Get-Sha256 $queueGatePath) -eq '170015C259C3EC8FC0B369F070748A555336F8FB312636819B7F99EBCE85A65D') 'O2D13 inherited queue-safety gate changed.'

$finalGate = Get-Content -Raw -LiteralPath $finalGatePath | ConvertFrom-Json
Assert-True ([string]$finalGate.state -eq 'PASS_O2D13_FINAL_PACKAGE_GATE' -and [string]$finalGate.requestId -eq $requestId) 'O2D13 final package identity changed.'
Assert-True (-not [bool]$finalGate.publicationAuthorized -and [bool]$finalGate.publicationRequiresCompleteRouteGate) 'O2D13 final package publication boundary changed.'
Assert-True ((Get-Sha256 $zipPath) -eq [string]$finalGate.requestZipSha256) 'O2D13 final ZIP changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json')) -eq [string]$finalGate.requestManifestSha256) 'O2D13 extracted request manifest changed.'
Assert-True ((Get-Sha256 (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.sig')) -eq [string]$finalGate.requestSignatureSha256) 'O2D13 extracted request signature changed.'
...
Assert-True ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'O2D13 exact signed package verification failed.'

$manifest = Get-Content -Raw -LiteralPath (Join-Path $extractRoot 'PORTAL_REQUEST_MANIFEST.json') | ConvertFrom-Json
Assert-True ([string]$manifest.requestId -eq $requestId -and [string]$manifest.targetRole -eq 'JBOD' -and [string]$manifest.jobClass -eq 'MAINTENANCE_PATCH') 'O2D13 request route changed.'
Assert-True ([int64]$manifest.maxResultBytes -eq 4194304 -and @($manifest.files).Count -eq 3) 'O2D13 request file or result bound changed.'
Assert-True ([string]$manifest.entryPoint -eq 'payload/Invoke-O2D13ScribeEndpoint.ps1' -and [string]$manifest.rehearsal.requiredState -eq 'PASS_O2D13_OPENCV_SCRIBE_DEVELOPMENT_EXECUTED') 'O2D13 entrypoint contract changed.'
Assert-True ([int]$manifest.timeoutContract.endpointWorkerOuterTimeoutSeconds -eq 900 -and [int]$manifest.timeoutContract.opencvChildTimeoutSeconds -eq 600) 'O2D13 timeout contract changed.'
Assert-True (@($manifest.allowedTaskActions).Count -eq 0 -and @($manifest.allowedProcessActions).Count -eq 1) 'O2D13 action contract changed.'

$jeo = Get-Content -Raw -LiteralPath $jeoPath | ConvertFrom-Json
Assert-True ([string]$jeo.state -eq 'PASS_JEO1_DIRECT_ADMIN_READ_ONLY_OBSERVATION' -and [string]$jeo.computerName -eq 'A1025645101') 'O2D13 direct JBOD observation identity changed.'
$endpointWorkerRows = @($jeo.sourceResults | Where-Object { [string]$_.id -eq 'ENDPOINT_WORKER' })
Assert-True ($endpointWorkerRows.Count -eq 1 -and @($endpointWorkerRows[0].rows).Count -eq 1 -and [string]$endpointWorkerRows[0].rows[0].sha256 -eq 'CB6700714E20DAC2D3C097095A2800C92ECAAC75F29878F4C86326493B246250') 'O2D13 observed endpoint worker pin changed.'
...
Assert-True ($endpointProcess.Count -eq 1 -and $senderProcess.Count -ge 1) 'O2D13 observed JBOD portal process evidence changed.'

$requestLeaves = @(
    'PORTAL_REQUEST_MANIFEST.json',
    'PORTAL_REQUEST_MANIFEST.sig',
    'payload\Invoke-O2D13ScribeEndpoint.ps1',
    'payload\ArgosOpenCvScribeV1R3.py',
    'payload\O2D13_SLOT18_JOB.json'
)
$responseLeaves = @(
...
foreach ($path in @(
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals\62619-433_20260824005735_Slot18\SCRIBE_PROPOSAL.json',
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals\62619-433_20260824005735_Slot18\scribe\multi_channel\MULTI_CHANNEL_READER_SUMMARY.json',
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals\62619-433_20260824005735_Slot18\scribe\BF_SCRIBE_ORIENTED_DETECTOR_INPUT.png',
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals\62619-433_20260824005735_Slot18\scribe\DF_SCRIBE_ORIENTED_DETECTOR_INPUT.png',
    ...
    'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_O2D13.ps1',
    'D:\A2\w\ocv\O2D13_20260826T211907000Z_10B0E71B.partial\refs\PORTABLE_GLYPH_REFERENCE_MANIFEST.json',
    'D:\A2\w\ocv\O2D13_20260826T211907000Z_10B0E71B\ArgosOpenCvScribeV1R3.py',
    'D:\A2\w\ocv\O2D13_20260826T211907000Z_10B0E71B\JOB.json',
    'D:\A2\w\ocv\O2D13_20260826T211907000Z_10B0E71B\refs\glyphs_v5_confirmed_20260806\028_62630_456_SLOT21_P04_S.png',
    'D:\A2\o\ocv\O2D13_20260826T211907000Z_10B0E71B\RESULT.json',
    'D:\A2\o\ocv\O2D13_20260826T211907000Z_10B0E71B\RUN_GATE.json',
...
Assert-True ($paths.Count -ge 120 -and $paths.Count -le 160) "O2D13 route path cardinality changed: $($paths.Count)"

$rows = New-Object Collections.Generic.List[object]
foreach ($path in $paths) {
    $checked = & $pathTool -CandidatePath $path -ReservedSuffixCharacters 32 -AsJson | ConvertFrom-Json
    Assert-True ([string]$checked.state -eq 'PASS_PATH_BUDGET' -and @($checked.candidates).Count -eq 1) "O2D13 route path budget failed: $path"
...
Assert-True ([int]$longest.effectiveLength -lt 200 -and $maximumComponentLength -le 80) 'O2D13 complete route exceeds the safe materialized path budget.'

$state = $(if ($Preflight) { 'HOLD_O2D13_COMPLETE_ROUTE_PREFLIGHT_ARGOS_INBOUND_RELAY_UNPROVEN' } else { 'HOLD_O2D13_COMPLETE_ROUTE_GATE_ARGOS_INBOUND_RELAY_UNPROVEN' })
$result = [ordered]@{
    schema = 'argos_o2d13_complete_route_gate_v1'
...
Assert-True (-not (Test-Path -LiteralPath $outputPath)) 'O2D13 route gate output already exists.'
