...
function Require-Pin([string]$Path, [string]$Sha256, [string]$State = '') {
    Require (Test-Path -LiteralPath $Path -PathType Leaf) "R18K build dependency absent: $Path"
    Require ((Get-Sha256 $Path) -eq $Sha256) "R18K build dependency changed: $Path"
    if (-not [string]::IsNullOrWhiteSpace($State)) {
        $value = Get-Content -Raw -LiteralPath $Path | ConvertFrom-Json
        Require ([string]$value.state -eq $State) "R18K build dependency state changed: $Path"
    }
}
function Write-JsonNew([string]$Path, [object]$Value, [int]$Depth = 32) {
    Require (-not (Test-Path -LiteralPath $Path)) "R18K build create-new JSON exists: $Path"
...
$requestId = 'REQ_20260904T022000000Z_R18K'
$revision = 'R18K_FULL_KLARF_JBOD_REVIEW_ONLY_20260904A'
...
$pathGatePath = Join-Path $PSScriptRoot 'R18K_PATH_GATE.json'
$preactionPath = Join-Path $PSScriptRoot 'PREACTION_R18K_FULL_KLARF_PUBLICATION.json'
$stageRoot = 'C:\R18KP'
...
$finalGatePath = Join-Path $PSScriptRoot 'R18K_FINAL_PACKAGE_GATE.json'
...
$definitionSha = '7FFFFE14CCFEFF75B0352CEE1138DF17FF36023C2198AC497EB09646B5568E96'
$localGateSha = '2803AEE7EF2C1C8993BF279F4F3C1F305B375E4018E1DFC344A8A678A76EA03E'
$pathGateSha = '98AE55BC84C9BD0A3998C9E6DE5DEE38D20AACE3F308A58955BE0F64B363FBB9'
$preactionSha = '832A76BF528BD5B80A94A5E4360D1EBC6CF79A13F6D9AACB79EAC82FEA4129E9'
...
Require (Test-Path -LiteralPath $localSource -PathType Container) 'R18K local rehearsal source absent.'
Require (Test-Path -LiteralPath $localProposals -PathType Container) 'R18K local rehearsal proposal root absent.'
...
Require ([string]$preactionResult.state -eq 'PASS_ARGOS_ZERO_RECURRENCE_PREACTION') 'R18K preaction gate changed.'
$definition = Get-Content -Raw -LiteralPath $definitionPath | ConvertFrom-Json
Require ([string]$definition.targetRole -eq 'JBOD' -and [string]$definition.jobClass -eq 'MAINTENANCE_PATCH' -and [string]$definition.entryPoint -eq 'payload/Invoke-R18JCorpusLaunch.ps1') 'R18K maintenance route changed.'
Require (@($definition.changes).Count -eq 1 -and [string]$definition.changes[0].installedSha256 -eq $entrypointSha -and [bool]$definition.changes[0].allowCreate) 'R18K maintenance change changed.'
Require (@($definition.changes[0].approvedPredecessorSha256).Count -eq 1 -and [string]$definition.changes[0].approvedPredecessorSha256[0] -eq $entrypointSha) 'R18K create-only idempotent installed-hash boundary changed.'
Require (@($definition.allowedTaskActions).Count -eq 0 -and @($definition.allowedProcessActions).Count -eq 1) 'R18K process/task action cardinality changed.'
Require ([string]$definition.sourceProcessingContract.proposalRoot -eq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals') 'R18K observed proposal root changed.'
Require ([bool]$definition.reviewOnly -and -not [bool]$definition.productionRoutingEnabled -and -not [bool]$definition.sourceProcessingContract.automaticIdentityAuthority -and -not [bool]$definition.sourceProcessingContract.sourceMutationAllowed -and -not [bool]$definition.sourceProcessingContract.sourceDeletionAllowed) 'R18K authority changed.'
$payloadManifest = Get-Content -Raw -LiteralPath $payloadManifestPath | ConvertFrom-Json
Require ([string]$payloadManifest.schema -eq 'argos_opencv_scribe_r18j_payload_manifest_v1' -and [string]$payloadManifest.revision -eq $payloadRevision -and @($payloadManifest.files).Count -eq 23) 'R18K inherited payload manifest shape changed.'
...
foreach ($file in $payloadFiles) {
    ...
    Require ((Get-Item -LiteralPath $source).Length -eq [int64]$file.bytes) "R18K payload length changed: $($file.sourcePath)"
}
...
try {
    ...
    Require ($matches.Count -eq 1 -and $matches[0].HasPrivateKey) 'R18K signer certificate or private key changed.'
...
Require ([string]$packageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18K signed package verification failed.'
...
Require ($extracted.Count -eq 27) 'R18K extracted final ZIP file count changed.'
foreach ($row in $manifestFiles) {
    $path = Join-Path $verifyRoot ([string]$row.path).Replace('/','\')
    Require-Pin $path ([string]$row.sha256)
    Require ((Get-Item -LiteralPath $path).Length -eq [int64]$row.bytes) "R18K extracted payload length changed: $($row.path)"
}
...
Require ([string]$exactPackageTest.State -eq 'PASS_SIGNED_PORTAL_PACKAGE') 'R18K extracted signed package verification failed.'
$packagedPreflight = (& (Join-Path $verifyRoot 'payload\Invoke-R18JCorpusLaunch.ps1') -Preflight -Rehearsal -PayloadRoot (Join-Path $verifyRoot 'payload') -WorkRoot 'C:\R18KFW' -OutputRoot 'C:\R18KFO' -SourceRoot $localSource -ProposalRoot $localProposals -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localRefs -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
Require ([string]$packagedPreflight.state -eq 'PASS_R18J_CORPUS_LAUNCH_PREFLIGHT' -and -not [bool]$packagedPreflight.mutationsPerformed -and -not [bool]$packagedPreflight.processStarted) 'R18K exact packaged entrypoint preflight failed.'
...
$finalGate = [ordered]@{
    schema='argos_opencv_scribe_r18k_final_package_gate_v1';createdUtc=[DateTime]::UtcNow.ToString('o');
    state='PASS_R18K_FINAL_PACKAGE_GATE';requestId=$requestId;requestZip=('work/OPENCV_SCRIBE_R18K/final/'+$zipName);
