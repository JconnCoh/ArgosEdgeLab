#!/usr/bin/env python3
"""Argos review-only OpenCV scribe provider, semantic revision R11.
...
R11 adds complementary smooth-band and paired-channel periodic perimeter
specialists.  Their interleaved shortlist is refined in a wide rectified crop,
then mapped back to native coordinates with the exact inverse perspective
transform.  Glyph descriptors use interior quantile centering and area
resampling so small cell-origin changes do not alias the dot matrix.
"""
...
ENGINE_REVISION = "ARGOS_OPENCV_SCRIBE_V1R11_20260902"
...
def describe_exact(
    ...
    foreground_y, foreground_x = np.nonzero(interior >= 40.0)
    if foreground_x.size:
        center_x = x + margin_x + 0.5 * (
            float(np.quantile(foreground_x, 0.02))
            + float(np.quantile(foreground_x, 0.98))
        )
        center_y = y + margin_y + 0.5 * (
            float(np.quantile(foreground_y, 0.02))
            + float(np.quantile(foreground_y, 0.98))
        )
    else:
    ...
    sampled = cv2.GaussianBlur(sampled, (3, 3), 0.7)
    values = sampled.reshape(-1).astype(np.float32)
...
def find_best_grid(gray: np.ndarray, prototypes: PrototypeBank) -> dict[str, Any]:
    ...
    y_radius = 0 if gray.shape[0] <= 300 else 100
    ...
    for cell_width in cell_widths:
        ...
        for y in range(min_y, max_y + 1, coarse_step):
            for x in range(min_x, max_x + 1, coarse_step):
                best = better_grid(
                    best,
                    evaluate_grid(residual, prototypes, x, y, cell_width, cell_height),
                )
    if best is None:
        raise ValueError("No valid twelve-cell scribe grid could be evaluated.")
    cell_width = int(best["cellWidth"])
    refined: dict[str, Any] | None = None
    refinement_radius = max(1, coarse_step // 2)
    for y in range(max(0, int(best["y"]) - refinement_radius), min(gray.shape[0] - cell_height, int(best["y"]) + refinement_radius) + 1):
        for x in range(max(0, int(best["x"]) - refinement_radius), min(gray.shape[1] - 12 * cell_width, int(best["x"]) + refinement_radius) + 1):
...
def _rectified_preview(
    ...
    unit_x = np.array([math.cos(radians), math.sin(radians)], dtype=np.float32)
    unit_y = np.array([-math.sin(radians), math.cos(radians)], dtype=np.float32)
    native_center = np.array([float(center[0]), float(center[1])], dtype=np.float32)
    half_width = (width - 1) / 2.0
    half_height = (height - 1) / 2.0
    source = np.array([
        native_center - half_width * unit_x - half_height * unit_y,
        native_center + half_width * unit_x - half_height * unit_y,
        native_center - half_width * unit_x + half_height * unit_y,
    ], dtype=np.float32)
    target = np.array([[0, 0], [width - 1, 0], [0, height - 1]], dtype=np.float32)
...
def _top_fraction_mean(values: np.ndarray, fraction: float = 0.02) -> float:
    flat = values.reshape(-1)
    take = max(1, int(round(flat.size * fraction)))
    return float(np.mean(np.partition(flat, flat.size - take)[-take:]))


def _smooth_specialist_score(
    bf_patch: np.ndarray,
    df_patch: np.ndarray,
) -> tuple[float, int, str]:
    """Score sparse dot response while penalizing dense geometric texture."""
    best = (-math.inf, 0, "DARK")
    for channel_index, patch in enumerate((bf_patch, df_patch)):
        enhanced = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(6, 4)).apply(patch)
        gradient = cv2.magnitude(
            cv2.Sobel(patch, cv2.CV_32F, 1, 0, ksize=3),
            cv2.Sobel(patch, cv2.CV_32F, 0, 1, ksize=3),
        )
        for polarity, view in (("DARK", enhanced), ("BRIGHT", 255 - enhanced)):
            residual = np.maximum(
                0.0,
                cv2.GaussianBlur(view, (0, 0), 3.0).astype(np.float32)
                - view.astype(np.float32),
            )
            for y in range(0, max(1, patch.shape[0] - 79), 20):
                for x in range(0, max(1, patch.shape[1] - 599), 100):
                    response_window = residual[y:y + 80, x:x + 600]
                    gradient_window = gradient[y:y + 80, x:x + 600]
                    signal = _top_fraction_mean(response_window)
                    texture = float(np.median(gradient_window) + 0.15 * np.mean(gradient_window))
                    edge_fraction = float(np.mean(gradient_window > 100.0))
                    score = signal / (texture + 1.0) * math.exp(-3.0 * edge_fraction)
                    if score > best[0]:
                        best = (score, channel_index, polarity)
    return best


def _paired_dark_response(bf_patch: np.ndarray, df_patch: np.ndarray) -> np.ndarray:
    bf_enhanced = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(6, 4)).apply(bf_patch)
    df_enhanced = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(6, 4)).apply(df_patch)
    bf_dark = np.maximum(
        0.0,
        cv2.GaussianBlur(bf_enhanced, (0, 0), 3.0).astype(np.float32)
        - bf_enhanced.astype(np.float32),
    )
    df_dark = np.maximum(
        0.0,
        cv2.GaussianBlur(df_enhanced, (0, 0), 3.0).astype(np.float32)
        - df_enhanced.astype(np.float32),
    )
    shared = np.sqrt(bf_dark * df_dark)
    scale = max(1.0, float(np.percentile(shared, 99.5)))
    normalized = np.clip(shared * (255.0 / scale), 0.0, 255.0).astype(np.uint8)
    line_response = np.zeros_like(normalized)
    for angle in range(0, 180, 30):
        kernel = np.zeros((15, 15), dtype=np.uint8)
        center = 7
        radians = math.radians(float(angle))
        dx = int(round(math.cos(radians) * 7.0))
        dy = int(round(math.sin(radians) * 7.0))
        cv2.line(kernel, (center - dx, center - dy), (center + dx, center + dy), 1, 1)
        opened = cv2.morphologyEx(normalized, cv2.MORPH_OPEN, kernel)
        line_response = np.maximum(line_response, opened)
    line_response = cv2.dilate(line_response, np.ones((3, 3), np.uint8))
...
def _periodic_window(
    ...
    best = (-math.inf, 0, 0)
    total_response = float(np.sum(response)) + 1e-9
    maximum_x = max(0, response.shape[1] - window_width)
    maximum_y = max(0, response.shape[0] - band_height)
    for y in range(0, maximum_y + 1, y_step):
        projection = np.mean(response[y:y + band_height], axis=0)
        projection = cv2.GaussianBlur(
            projection.reshape(1, -1), (0, 0), 3.0
        ).reshape(-1)
        baseline = cv2.GaussianBlur(projection.reshape(1, -1), (0, 0), 18.0).reshape(-1)
        highpass = np.maximum(0.0, projection - baseline)
        for x in range(0, maximum_x + 1, x_step):
            values = highpass[x:x + window_width]
            centered = values - float(np.mean(values))
            denominator = float(np.dot(centered, centered)) + 1e-9
            periodicity = max(
                float(np.dot(centered[:-lag], centered[lag:])) / denominator
                for lag in range(lag_minimum, lag_maximum + 1)
            )
            signal = float(np.sum(values))
            band_fraction = float(
                np.sum(response[y:y + band_height, x:x + window_width])
            ) / total_response
            score = max(0.0, periodicity) * band_fraction * min(1.0, signal / 5000.0)
            if score > best[0]:
                best = (score, x, y)
    return best


def _rectified_point_to_native(
    center: tuple[float, float],
    angle_degrees: float,
    width: int,
    height: int,
    ...
    radians = math.radians(float(angle_degrees))
    along = float(point[0]) - (width - 1) / 2.0
    across = float(point[1]) - (height - 1) / 2.0
    return (
        float(center[0]) + along * math.cos(radians) - across * math.sin(radians),
        float(center[1]) + along * math.sin(radians) + across * math.cos(radians),
    )


def _refine_perimeter_candidate(
    bf: np.ndarray,
    df: np.ndarray,
    center: tuple[float, float],
    angle_degrees: float,
    ...
    wide_height = 500
    full_patches = [
        _rectified_preview(image, center, angle_degrees, wide_width, wide_height)
        for image in (bf, df)
    ]
    patches = [
        cv2.resize(image, (1600, 250), interpolation=cv2.INTER_AREA)
        for image in full_patches
    ]
    if specialist == "PAIRED":
        response = _paired_dark_response(patches[0], patches[1])
        _, x, y = _periodic_window(response, 600, 120, 45, 53, 20, 10)
        window = response[y:y + 120, x:x + 600]
    else:
        best_periodic = (-math.inf, 0, 0, 1.0)
        best_smooth = (
            -math.inf, 0, 0, np.zeros((100, 600), dtype=np.float32)
        )
        for patch in patches:
            raw_gradient = cv2.magnitude(
                cv2.Sobel(patch, cv2.CV_32F, 1, 0, ksize=3),
                cv2.Sobel(patch, cv2.CV_32F, 0, 1, ksize=3),
            )
            enhanced = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(12, 4)).apply(patch)
            for view in (enhanced, 255 - enhanced):
                response = np.maximum(
                    0.0,
                    cv2.GaussianBlur(view, (0, 0), 3.0).astype(np.float32)
                    - view.astype(np.float32),
                )
                score, x_value, y_value = _periodic_window(
                    response, 600, 120, 45, 53, 20, 10
                )
                edge_fraction = float(np.mean(
                    raw_gradient[y_value:y_value + 120, x_value:x_value + 600] > 100.0
                ))
                if score > best_periodic[0]:
                    best_periodic = (score, x_value, y_value, edge_fraction)
                for smooth_y in range(0, 151, 10):
                    for smooth_x in range(0, 1001, 20):
                        response_window = response[
                            smooth_y:smooth_y + 100, smooth_x:smooth_x + 600
                        ]
                        gradient_window = raw_gradient[
                            smooth_y:smooth_y + 100, smooth_x:smooth_x + 600
                        ]
                        signal = _top_fraction_mean(response_window)
                        texture = float(
                            np.median(gradient_window) + 0.15 * np.mean(gradient_window)
                        )
                        dense_edges = float(np.mean(gradient_window > 100.0))
                        smooth_score = signal / (texture + 1.0) * math.exp(-3.0 * dense_edges)
                        if smooth_score > best_smooth[0]:
                            best_smooth = (
                                smooth_score,
                                smooth_x,
                                smooth_y,
        ...
        _, x, y, periodic_edge_fraction = best_periodic
        if periodic_edge_fraction <= 0.20:
            window = np.zeros((120, 600), dtype=np.float32)
        else:
            _, x, y, window = best_smooth
            threshold = float(np.percentile(window, 97.0))
            yy, xx = np.nonzero(window >= threshold)
            if xx.size:
                edge_quantile = 0.98 if best_periodic[1] < x else 0.02
                return _rectified_point_to_native(
                    center,
                    angle_degrees,
                    wide_width,
                    wide_height,
                    (
                        2.0 * (x + float(np.quantile(xx, edge_quantile))),
                        2.0 * (y + float(np.median(yy))),
                    ),
                )
    refined_x = x + window.shape[1] / 2.0
    refined_y = y + window.shape[0] / 2.0
    return _rectified_point_to_native(
        center,
        angle_degrees,
        wide_width,
        wide_height,
        (2.0 * refined_x, 2.0 * refined_y),
    )


def _refine_region_angle(
    bf: np.ndarray,
    df: np.ndarray,
    center: tuple[float, float],
    angle_degrees: float,
    ...
    if specialist == "PAIRED":
        best = (-math.inf, angle_degrees)
        for candidate_angle in np.arange(angle_degrees - 15.0, angle_degrees + 15.1, 5.0):
            patches = [
                cv2.resize(
                    _rectified_preview(image, center, float(candidate_angle), 1600, 400),
                    (800, 200),
                    interpolation=cv2.INTER_AREA,
                )
                for image in (bf, df)
            ]
            score, _, _ = _periodic_window(
                _paired_dark_response(patches[0], patches[1]),
                600,
                80,
                22,
                27,
    ...
    count, _, stats, centroids = cv2.connectedComponentsWithStats(
        (residual >= 80.0).astype(np.uint8), 8
    )
    points: list[np.ndarray] = []
    for index in range(1, count):
        ...
        if (
            ...
            and 3 <= component_height <= 12
            and 7 <= area <= 100
            and 0.5 <= component_width / float(component_height) <= 2.0
            and fill >= 0.25
            and 80.0 <= x <= 1520.0
            and 30.0 <= y <= 370.0
        ):
    ...
    _, eigenvectors, _ = cv2.PCACompute2(np.asarray(points, dtype=np.float32), mean=None)
    correction = math.degrees(math.atan2(
        float(eigenvectors[0, 1]), float(eigenvectors[0, 0])
    ))
    while correction > 45.0:
    ...
    while correction < -45.0:
        correction += 180.0
    correction = float(np.clip(round(correction / 5.0) * 5.0, -15.0, 15.0))
    return angle_degrees + correction


def _spatial_shortlist(
    rows: list[tuple[float, float, float, float, int, str]],
    limit: int,
) -> list[tuple[float, float, float, float, int, str]]:
    accepted: list[tuple[float, float, float, float, int, str]] = []
    for row in sorted(rows, key=lambda item: (-item[0], item[1], item[2])):
        if any(math.hypot(row[1] - other[1], row[2] - other[2]) < 1200.0 for other in accepted):
            continue
        accepted.append(row)
...
def perimeter_scribe_regions(
    ...
    """Return interleaved smooth and paired-channel perimeter candidates."""
    height, width = bf.shape[:2]
    scale = min(1.0, maximum_dimension / float(max(height, width)))
    target = (max(1, int(round(width * scale))), max(1, int(round(height * scale))))
    locator_bf = cv2.resize(bf, target, interpolation=cv2.INTER_AREA) if scale < 1.0 else bf
    center_x, center_y, radius = _wafer_ellipse(locator_bf)
    center_x /= scale
    center_y /= scale
    radius /= scale
    smooth_rows: list[tuple[float, float, float, float, int, str]] = []
    paired_rows: list[tuple[float, float, float, float, int, str]] = []
    for perimeter_angle in np.arange(0.0, 360.0, 5.0):
        ...
        for inward_fraction in (0.05, 0.09, 0.13, 0.17, 0.21):
            center = (
                center_x + radius * (1.0 - inward_fraction) * unit_x,
                center_y + radius * (1.0 - inward_fraction) * unit_y,
            )
            angle = float(perimeter_angle + 90.0)
            bf_patch = cv2.resize(
                _rectified_preview(bf, center, angle, 1600, 400),
                (800, 200),
                interpolation=cv2.INTER_AREA,
            )
            df_patch = cv2.resize(
                _rectified_preview(df, center, angle, 1600, 400),
                (800, 200),
                interpolation=cv2.INTER_AREA,
            )
            smooth_score, channel_index, polarity = _smooth_specialist_score(bf_patch, df_patch)
            paired_response = _paired_dark_response(bf_patch, df_patch)
            paired_score, _, _ = _periodic_window(paired_response, 600, 80, 22, 27, 100, 20)
            smooth_rows.append((smooth_score, center[0], center[1], angle, channel_index, polarity))
            paired_rows.append((paired_score, center[0], center[1], angle, 0, "DARK"))

    specialist_limit = max(1, min(4, (maximum_candidates + 1) // 2))
    smooth = _spatial_shortlist(smooth_rows, specialist_limit)
    paired = _spatial_shortlist(paired_rows, specialist_limit)
    interleaved: list[tuple[str, tuple[float, float, float, float, int, str]]] = []
    for index in range(max(len(smooth), len(paired))):
        if index < len(smooth):
            interleaved.append(("SMOOTH", smooth[index]))
        if index < len(paired):
            interleaved.append(("PAIRED", paired[index]))

    regions: list[Region] = []
    for rank, (specialist, row) in enumerate(interleaved[:maximum_candidates]):
        refined_center = _refine_perimeter_candidate(
            bf,
            df,
            (row[1], row[2]),
            row[3],
        ...
        refined_angle = _refine_region_angle(
            bf, df, refined_center, row[3], specialist
        )
        channel = ("BF", "DF")[int(row[4])] if specialist == "SMOOTH" else "BF_DF"
        polarity_token = f"_{row[5]}" if specialist == "SMOOTH" else ""
        regions.append(Region(
            region_id=f"PERIMETER_{specialist}_{channel}{polarity_token}_{rank:02d}",
            source=f"{specialist}_{channel}{polarity_token}_FULL_PERIMETER_REVIEW_ONLY_DEVELOPMENT",
            center_x=refined_center[0],
            center_y=refined_center[1],
            width=float(ocr_width),
            height=float(ocr_height),
            angle_degrees=float(refined_angle),
            localization_score=float(1.0 - rank * 0.02),
        ))
    return regions
...
def rectify(gray: np.ndarray, region: Region) -> np.ndarray | None:
    ...
    patch = _rectified_preview(
        gray,
        (region.center_x, region.center_y),
        region.angle_degrees,
        output_width,
        output_height,
    )
...
def analyze_images(
    ...
    if auto_localized_development:
        ...
        standard_regions = [
            Region(
                ...
                source=f"AUTO_LOCALIZED_{region.source}",
    ...
    paired_enhancement_diagnostics: list[dict[str, Any]] = []
    ...
    for region in standard_regions:
        ...
        if bf_patch is None or df_patch is None:
            continue
        if auto_localized_development and "_PAIRED_" in region.source:
            small_bf = cv2.resize(bf_patch, (800, 200), interpolation=cv2.INTER_AREA)
            small_df = cv2.resize(df_patch, (800, 200), interpolation=cv2.INTER_AREA)
            ...
            paired_enhancement_diagnostics.append({
                ...
                "responseWidth": int(response.shape[1]),
                "responseHeight": int(response.shape[0]),
                "periodicWindowX": x,
                "periodicWindowY": y,
                "periodicWindowWidth": 600,
            ...
            continue
        channel_patches = (("BF", bf_patch), ("DF", df_patch))
        if auto_localized_development and "_SMOOTH_" in region.source:
            selected_channel = "DF" if "_DF_" in region.source else "BF"
            channel_patches = tuple(
                item for item in channel_patches if item[0] == selected_channel
            )
        for channel, patch in channel_patches:
            polarity_views = (("DARK", patch), ("BRIGHT", 255 - patch))
            if auto_localized_development and "_SMOOTH_" in region.source:
                selected_polarity = "BRIGHT" if "_BRIGHT_" in region.source else "DARK"
                polarity_views = tuple(
                    item for item in polarity_views if item[0] == selected_polarity
                )
            for polarity, view in polarity_views:
                for direction, oriented in (("FORWARD", view), ("REVERSE_180", cv2.rotate(view, cv2.ROTATE_180))):
                    try:
                        evaluated = evaluate_detector_input(oriented, prototypes, str(job.get("references", {}).get("excludedPhysicalIdentity", "")))
                    except ValueError:
    ...
    if auto_localized_development:
        hypotheses.sort(key=lambda row: (
            not (
                ...
                and float(row["selectionScore"]) >= AUTO_IDENTITY_MIN_SELECTION_SCORE
            ),
    ...
    for row in hypotheses:
        if bool(row["boundaryComplete"]) and (
            not auto_localized_development
            or (
                bool(row["checksumValid"])
                and float(row["selectionScore"]) >= AUTO_IDENTITY_MIN_SELECTION_SCORE
            )
    ...
    elif auto_localized_development and paired_enhancement_diagnostics and not candidates:
        state = "SCRIBE_PAIRED_ENHANCEMENT_IDENTITY_HOLD"
        holds.append({"code": state, "detail": "Paired BF/DF enhancement localized periodic scribe evidence, but no image-first checksum-valid identity cleared the automatic score gate."})
    ...
    if auto_localized_development:
        ...
        if paired_enhancement_diagnostics and not candidates:
            holds.append({"code": "SCRIBE_PAIRED_ENHANCEMENT_IDENTITY_HOLD", "detail": "Paired BF/DF enhancement localized periodic scribe evidence, but no image-first checksum-valid identity was established."})
    deduplicated_holds = []
    ...
    return {
        ...
        "checksumState": "NOT_EVALUATED" if best is None else (
            "SCRIBE_M12_CHECKSUM_OR_SCORE_GATE_FAILED" if auto_localized_development and not candidates
            else
        ...
        "localization": {
            ...
            "pairedEnhancementDiagnostics": paired_enhancement_diagnostics,
