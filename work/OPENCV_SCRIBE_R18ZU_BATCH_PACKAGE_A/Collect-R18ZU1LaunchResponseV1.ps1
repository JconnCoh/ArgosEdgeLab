...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zu_publication_invocation_v8' -and
    [string]$invocation.state -ceq 'FROZEN_R18ZU1_PUBLICATION_INVOCATION_V8' -and [string]$invocation.lifecycle -ceq 'FROZEN') 'R18ZT publication invocation is not frozen; collection refused.'
Require ([string]$invocation.requestId -ceq 'REQ_R18ZU1' -and -not [bool]$invocation.authority.retryAuthorized -and
...
Require ([string]$applyIntent.schema -ceq 'argos_opencv_scribe_r18zu1_publication_apply_intent_v8' -and
    [string]$applyIntent.state -ceq 'FROZEN_R18ZU1V8_APPLY_INTENT_BEFORE_EXTERNAL_WRITE' -and
    [string]$applyIntent.requestId -ceq 'REQ_R18ZU1' -and [string]$applyIntent.invocationManifestSha256 -ceq $invocationSha256 -and
...
if ($publishGateExists) {
    ...
    Require ([string]$publishGate.schema -ceq 'argos_opencv_scribe_r18zu1_publish_gate_v8' -and
        [string]$publishGate.state -ceq 'PASS_R18ZU1V8_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' -and
        [string]$publishGate.requestId -ceq 'REQ_R18ZU1') 'R18ZT publisher gate identity changed.'
...
if (-not $publishGateExists) {
    ...
    Require ($readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZU1.ready.zip' -and
        $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_R18ZU1.ready.zip' -and
        $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZU1.ready.zip.upload') 'R18ZT missing-gate recovery route leaves changed.'
    $routeRows = @(
...
foreach ($candidate in $candidates) {
    ...
    try {
        ...
        if ([string]$manifest.requestId -cne 'REQ_R18ZU1') { continue }
...
$preflightResult = [ordered]@{
    schema = 'argos_opencv_scribe_r18zu1_launch_response_collection_gate_v8'
    collectedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZU1V8_ONE_SIGNED_LAUNCH_RESPONSE_PREFLIGHT' } else { 'PASS_R18ZU1V8_SIGNED_LAUNCH_RESPONSE_COLLECTED' })
...
$verificationRows = @(& ([string]$responseVerifierDependency.path) -PackagePath $verificationRoot -EndpointCertificatePath ([string]$responseSignerDependency.path) -ExpectedSourceRole JBOD -ExpectedRequestId 'REQ_R18ZU1')
...
Require ([string](Get-NestedValue $resultObject ([string]$invocation.collector.resultRequestIdProperty) 'R18ZT endpoint RESULT') -ceq 'REQ_R18ZU1') 'R18ZT endpoint RESULT request identity changed.'
