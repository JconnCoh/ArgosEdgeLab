...
function Assert-ExactLauncherChange([object[]]$Changes, [string]$LauncherSha256, [string]$Label) {
    ...
    Require ([string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZU1.ps1') "$Label launcher destination changed."
...
$revision = 'R18ZU_EXISTING_ORIENTED_CROPS_ASYNC_REVIEW_ONLY_20260908A'
$runnerRevision = 'R18ZT_EXISTING_ORIENTED_CROPS_PUBLIC_PROVIDER_BATCH_20260908D'
$bindingPath = Join-Path $PSScriptRoot 'R18ZU_FROZEN_PACKAGE_BINDINGS_V5.json'
$launcherPath = Join-Path $PSScriptRoot 'Invoke-R18ZUBatchLaunchV1.ps1'
$payloadManifestPath = Join-Path $PSScriptRoot 'R18ZU_PAYLOAD_MANIFEST.json'
$definitionPath = Join-Path $PSScriptRoot 'MAINTENANCE_DEFINITION_R18ZU1.json'
$configurationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A\R18ZU_BATCH_CONFIGURATION.json'
$designPath = Join-Path $PSScriptRoot 'R18ZU_BATCH_PACKAGE_DESIGN_V1.json'
$importSmokePath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZT_BATCH_PACKAGE_B\Test-R18ZTPackagedImportClosure.py'
$payloadCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZU_A1_CLONE_REMEDIATION.json'
$builderCloneRemediationPath = Join-Path $PSScriptRoot 'R18ZU_A1_CLONE_REMEDIATION.json'
$publisherCloneRemediationPath = Join-Path $project 'work\OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A\R18ZU_A1_CLONE_REMEDIATION.json'
$cloneRawGatePath = Join-Path $PSScriptRoot 'R18ZU_A3_CLONE_RAW_GATE.json'
$cloneGatePath = Join-Path $PSScriptRoot 'R18ZU_A3_CLONE_GATE.json'
$pathGatePath = Join-Path $PSScriptRoot 'R18ZU_PATH_PLAN_GATE.json'
$powerShellGatePath = Join-Path $PSScriptRoot 'R18ZU_POWERSHELL_SAFETY_GATE_V3.json'
$scienceGatePath = Join-Path $PSScriptRoot 'R18ZU_SCIENCE_GATE.json'
$junctionGatePath = Join-Path $PSScriptRoot 'R18ZU_PROPOSAL_JUNCTION_GATE.json'
$recoveryIntentPath = Join-Path $PSScriptRoot 'R18ZU_RECOVERY_INTENT_V3.json'
$recoveryGatePath = Join-Path $PSScriptRoot 'R18ZU_RECOVERY_INTENT_GATE_V3.json'
$preactionPath = Join-Path $PSScriptRoot 'R18ZU_PREACTION_GATE_V2.json'
$collisionGatePath = Join-Path $PSScriptRoot 'R18ZU_REQUEST_ID_COLLISION_GATE.json'
$collisionScannerPath = Join-Path $PSScriptRoot 'Test-R18ZU1RequestIdUniquenessV1.ps1'
...
$testRoot = 'C:\R18ZU1A1T'
$testWorkRoot = 'C:\R18ZU1A1TW'
$testOutputRoot = 'C:\R18ZU1A1TO'
$stageRoot = 'C:\R18ZU1A1P'
$readyRoot = Join-Path $stageRoot ($requestId + '.ready')
$stageZip = Join-Path $stageRoot ($requestId + '.ready.zip')
$verifyRoot = 'C:\R18ZU1A1V'
$finalRoot = Join-Path $PSScriptRoot 'final'
$finalPartial = Join-Path $PSScriptRoot 'final.partial'
$zipName = $requestId + '.ready.zip'
$finalGatePath = Join-Path $PSScriptRoot 'R18ZU_FINAL_PACKAGE_GATE.json'
$productionWorkRoot = 'D:\A2\w\ocv\R18ZU1'
$productionOutputRoot = 'D:\A2\o\ocv\R18ZU1'
$productionProposalAlias = 'D:\A2\w\ocv\R18ZU1\p'
$canonicalProposalRoot = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\identity\proposals'
$productionInstalledLauncher = 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZU1.ps1'
$bindingSha = 'F47674D29746DBFBBCCC409367D3E308146ECCFEC16A5739EAE992261DC87BA0'

Require-Pin $bindingPath $bindingSha -1 'FROZEN_R18ZU_PACKAGE_A_V5_BINDINGS'
$binding = Get-Content -LiteralPath $bindingPath -Raw | ConvertFrom-Json
Require ([string]$binding.schema -eq 'argos_opencv_scribe_r18zu_package_bindings_v5' -and [string]$binding.requestId -eq $requestId -and [string]$binding.packageRevision -eq $revision -and [string]$binding.runnerRevision -eq $runnerRevision) 'R18ZT V5 binding identity changed.'
...
Require ([string]$boundLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$boundLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZU1\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$boundLongestOutput.basePathLength -eq 62 -and [int]$boundLongestOutput.suffixReserveCharacters -eq 52 -and [int]$boundLongestOutput.effectivePathLength -eq 114 -and [int]$boundLongestOutput.effectiveComponentLength -eq 68) 'R18ZT exact longest atomic case-output binding changed.'
$boundConservativeTemp = $binding.pathContract.launcherConservativeProviderTempBudget
Require ([string]$boundConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$boundConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZU1\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$boundConservativeTemp.basePathLength -eq 74 -and [int]$boundConservativeTemp.baseComponentLength -eq 28 -and [int]$boundConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$boundConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$boundConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$boundConservativeTemp.trueAtomicWriteTarget) 'R18ZT conservative provider-temp launcher budget changed.'
...
$requiredArtifactNames = @(
    ...
    'heldOutZDiagnosticGate','r18zuReviewBatchScienceGate','approvedSourceGuard','priorRequestTerminalRouteGate'
)
...
$cloneGate = Read-BoundGate $cloneGatePath 'PASS_R18ZU_PACKAGE_A_CLONE_LITERAL_GATE'
$pathGate = Read-BoundGate $pathGatePath 'PASS_R18ZU_PACKAGE_A_PATH_BUDGET'
$powerShellGate = Read-BoundGate $powerShellGatePath 'PASS_R18ZU_PACKAGE_A_POWERSHELL_SAFETY'
$scienceGate = Read-BoundGate $scienceGatePath 'PASS_R18ZU_BATCH_SCIENCE_GATE'
$junctionGate = Read-BoundGate $junctionGatePath 'PASS_R18ZU_PROPOSAL_JUNCTION_CREATION_AND_ROLLBACK'
...
$collisionGate = $null
if ($Build) { $collisionGate = Read-BoundGate $collisionGatePath 'PASS_R18ZU1_REQUEST_ID_UNIQUENESS_ZERO_COLLISIONS' }

$builderPath = [IO.Path]::GetFullPath($MyInvocation.MyCommand.Path)
$builderSha = Get-Sha256 $builderPath
$expectedClonePairs = [ordered]@{
    'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Build-R18ZUBatchRequestV1.ps1' = $builderSha
    'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Collect-R18ZU1LaunchResponseV1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZU1LaunchResponseV1.ps1')
    'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Invoke-R18ZUBatchLaunchV1.ps1' = Get-Sha256 $launcherPath
    'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Publish-R18ZU1V1.ps1' = Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZU1V1.ps1')
    'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Run-R18ZUBatchExecutionEnvelopeV1.py' = Get-Sha256 (Join-Path $PSScriptRoot 'Run-R18ZUBatchExecutionEnvelopeV1.py')
    'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Test-R18ZU1RequestIdUniquenessV1.ps1' = Get-Sha256 $collisionScannerPath
}
...
Require ([string]$cloneGate.rawUtilityGatePath -ceq 'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/R18ZU_A3_CLONE_RAW_GATE.json' -and [string]$cloneGate.rawUtilityGateSha256 -cmatch '^[A-F0-9]{64}$') 'R18ZT raw clone utility gate pin is absent.'
...
foreach ($scriptPin in @(
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Invoke-R18ZUBatchLaunchV1.ps1';sha=[string]($artifactRows | Where-Object name -eq 'launcher').sha256},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Build-R18ZUBatchRequestV1.ps1';sha=$builderSha},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Publish-R18ZU1V1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Publish-R18ZU1V1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Collect-R18ZU1LaunchResponseV1.ps1';sha=(Get-Sha256 (Join-Path $PSScriptRoot 'Collect-R18ZU1LaunchResponseV1.ps1'))},
    [pscustomobject]@{path='work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Test-R18ZU1RequestIdUniquenessV1.ps1';sha=(Get-Sha256 $collisionScannerPath)}
)) {
...
foreach ($sciencePin in @('runnerNonImageTestGate','canonicalZDevelopmentGate','r18zuNonImageRegressionGate','heldOutZDiagnosticGate','r18zuReviewBatchScienceGate','priorRequestTerminalRouteGate')) {
    $bindingRow = @($artifactRows | Where-Object { [string]$_.name -eq $sciencePin })[0]
    $scienceRows = @($scienceGate.artifacts | Where-Object { [string]$_.name -eq $sciencePin })
...
Require ([string]$priorRoute.state -ceq 'PASS_R18ZT1_ENDPOINT_TERMINAL_AND_SIGNED_RETURN_ROUTE_HEALTHY' -and
    [bool]$priorRoute.conclusion.priorEndpointRequestTerminal -and
    [bool]$priorRoute.conclusion.priorEndpointQueueHeadCleared -and
    [bool]$priorRoute.conclusion.signedTerminalResponseReturnedToShare -and
    [bool]$priorRoute.conclusion.returnRouteHealthy -and
    -not [bool]$priorRoute.mutationsPerformed -and -not [bool]$priorRoute.sourceImagesRead -and
    -not [bool]$priorRoute.resultPayloadsRead) 'R18ZT1 prior-request terminal route clearance changed.'
...
Require ([string]$pathLongestOutput.leaf -ceq 'CASE_RESULT.json' -and [string]$pathLongestOutput.path -ceq 'D:\A2\o\ocv\R18ZU1\cases\FFFFFFFFFFFFFFFFFFFF\CASE_RESULT.json' -and [int]$pathLongestOutput.basePathLength -eq 62 -and [int]$pathLongestOutput.suffixReserveCharacters -eq 52 -and [int]$pathLongestOutput.effectivePathLength -eq 114 -and [int]$pathLongestOutput.effectiveComponentLength -eq 68) 'R18ZT path-gate true atomic longest-output fact changed.'
$pathConservativeTemp = $pathGate.launcherConservativeProviderTempBudget
Require ([string]$pathConservativeTemp.leaf -ceq 'PROVIDER_RESULT.json.partial' -and [string]$pathConservativeTemp.path -ceq 'D:\A2\o\ocv\R18ZU1\cases\FFFFFFFFFFFFFFFFFFFF\PROVIDER_RESULT.json.partial' -and [int]$pathConservativeTemp.basePathLength -eq 74 -and [int]$pathConservativeTemp.baseComponentLength -eq 28 -and [int]$pathConservativeTemp.conservativeSuffixReserveCharacters -eq 52 -and [int]$pathConservativeTemp.conservativeEffectivePathLength -eq 126 -and [int]$pathConservativeTemp.conservativeEffectiveComponentLength -eq 80 -and -not [bool]$pathConservativeTemp.trueAtomicWriteTarget) 'R18ZT path-gate conservative provider-temp fact changed.'
...
Require ([string]$recoveryIntent.state -eq 'FROZEN_R18ZU_PACKAGE_A_RECOVERY_INTENT' -and [string]$recoveryIntent.requestId -eq $requestId -and [string]$recoveryIntent.bindingRecordSha256 -eq $bindingSha -and -not [bool]$recoveryIntent.mutation.publicationAuthorized -and -not [bool]$recoveryIntent.mutation.automaticRetryAuthorized) 'R18ZU recovery intent changed.'
...
$bindingDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/R18ZU_FROZEN_PACKAGE_BINDINGS_V5.json' })
$builderDependency = @($preaction.dependencies | Where-Object { [string]$_.path -eq 'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/Build-R18ZUBatchRequestV1.ps1' })
...
if ($Build) {
    Require ([string]$collisionGate.schema -ceq 'argos_r18zu1_request_id_uniqueness_gate_v3' -and [string]$collisionGate.requestId -ceq $requestId -and [string]$collisionGate.scannerSha256 -ceq (Get-Sha256 $collisionScannerPath) -and [string]$collisionGate.scanPurpose -ceq 'PRE_SIGNATURE') 'R18ZT collision gate identity changed.'
...
foreach ($outputPin in @(
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZU1\LAUNCH.json';schema='argos_opencv_scribe_r18zt_batch_launch_v2'},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZU1\RUNNING.json';schema=[string]$binding.outputSchemas.progress},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZU1\COMPLETE.json';schema=[string]$binding.outputSchemas.complete},
    [pscustomobject]@{path='D:\A2\o\ocv\R18ZU1\STATUS.json';schema=[string]$binding.outputSchemas.status}
)) {
...
$runningOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZU1\RUNNING.json' })[0]
...
$failureOutput = @($outputs | Where-Object { [string]$_.path -ceq 'D:\A2\o\ocv\R18ZU1\FAILURE.json' })[0]
...
$expectedLeafSetSha = '062F2E4B829C3857DC49F73D557BFC26C48B8F3225C35ACD1A380CADCBE3927B'
...
if ($Preflight) {
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zu_batch_build_preflight_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZU_PACKAGE_A_BUILD_PREFLIGHT'
        requestId = $requestId
...
$stagedPreflight = (& (Join-Path $activePayload 'Invoke-R18ZTBatchLaunch.ps1') -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $activePayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\R18ZU1A1_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String) | ConvertFrom-Json
...
$stagedPackageHashClosure = [ordered]@{
    state = 'PASS_R18ZU_STAGED_PACKAGED_HASH_CLOSURE_NO_IMAGE'
...
if ($Test) {
    ...
    [ordered]@{
        schema = 'argos_opencv_scribe_r18zu_batch_unsigned_test_v1'
        checkedUtc = [DateTime]::UtcNow.ToString('o')
        state = 'PASS_R18ZU_PACKAGE_A_UNSIGNED_TEST'
        requestId = $requestId
        bindingRecordSha256 = $bindingSha
...
$extractedPreflightText = (& $windowsPowerShell51 -NoProfile -ExecutionPolicy Bypass -File $extractedLauncher -Preflight -Rehearsal -PackageValidationOnly -PayloadRoot $extractedPayload -WorkRoot $testWorkRoot -OutputRoot $testOutputRoot -CanonicalProposalRoot 'C:\R18ZU1A1_PROPOSALS_NOT_ACCESSED' -PythonPath $localPython -ExpectedPythonSha256 $localPythonSha -ReferenceBundlePath $localReferenceBundle -ExpectedComputerName $env:COMPUTERNAME | Out-String)
...
$extractedProvider = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZU\ArgosOpenCvScribeV1R18ZU.py'
$extractedR18ztProvider = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZT\ArgosOpenCvScribeV1R18ZT.py'
$extractedCanonicalZGate = Join-Path $extractedPayload 'files\OPENCV_SCRIBE_R18ZU\R18ZU_CANONICAL_Z_DEVELOPMENT_GATE_V2.json'
Require ((Get-Sha256 $extractedRunner) -eq $runnerSha -and (Get-Sha256 $extractedProvider) -eq $providerSha -and
    (Get-Sha256 $extractedR18ztProvider) -eq [string]($artifactRows | Where-Object name -eq 'r18ztProvider').sha256 -and
    (Get-Sha256 $extractedCanonicalZGate) -eq [string]($artifactRows | Where-Object name -eq 'canonicalZDevelopmentGate').sha256) 'R18ZU extracted packaged hash closure changed.'
...
$routeGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zu_complete_route_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZU_COMPLETE_ROUTE_GATE_SIGNED_UNPUBLISHED'
    requestId = $requestId
    bindingRecordSha256 = $bindingSha
...
$finalGate = [ordered]@{
    schema = 'argos_opencv_scribe_r18zu_final_package_gate_v1'
    checkedUtc = [DateTime]::UtcNow.ToString('o')
    state = 'PASS_R18ZU_SIGNED_UNPUBLISHED_PACKAGE_GATE'
    requestId = $requestId
    revision = $revision
    bindingRecordSha256 = $bindingSha
    requestZip = 'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/final/REQ_R18ZU1.ready.zip'
