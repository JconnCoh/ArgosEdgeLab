...
$fixedInvocationPath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot 'R18ZU_PUBLICATION_INVOCATION_V8.json'))
...
Require ([string]$invocation.schema -ceq 'argos_opencv_scribe_r18zu_publication_invocation_v8') 'R18ZT publication invocation schema changed.'
Require ([string]$invocation.state -ceq 'FROZEN_R18ZU1_PUBLICATION_INVOCATION_V8') 'R18ZT publication invocation is not frozen; execution refused.'
Require ([string]$invocation.requestId -ceq 'REQ_R18ZU1') 'R18ZT publication request identity changed.'
...
Require ([string]$postFreezeContract.path -ceq 'work/OPENCV_SCRIBE_R18ZU_BATCH_PACKAGE_A/R18ZU_P8_FINAL_TOOL.json') 'R18ZT post-freeze tooling-gate path changed.'
...
Require ([string]$postFreezeTooling.schema -ceq [string]$postFreezeContract.expectedSchema -and
    ...
    [string]$postFreezeTooling.requestId -ceq 'REQ_R18ZU1' -and
...
Require ([string]$binding.state -ceq [string]$invocation.packageBinding.expectedState -and [string]$binding.requestId -ceq 'REQ_R18ZU1') 'Fresh package binding identity/state changed.'
...
foreach ($consumer in @($invocation.bindingConsumers)) {
    ...
    Require ([string](Get-NestedValue $consumerObject ([string]$consumer.requestIdProperty) "Binding consumer $($consumer.dependency)") -ceq 'REQ_R18ZU1') "Binding consumer request ID changed: $($consumer.dependency)"
}
...
$scienceAdjudicationRows = @($scienceGate.artifacts | Where-Object { [string]$_.name -ceq 'slot21Adjudication' })
Require ($scienceAdjudicationRows.Count -eq 1 -and
    [string]$scienceAdjudicationRows[0].sha256 -ceq 'E526EDF07597D3F71A52FABAD7E93941A0FE3CD34003B6F0595C9D3992234A7D') 'R18ZT science gate adjudication pin changed.'
Require ([string]$scienceGate.heldOutZImageFirstString -ceq '147Z6157SUA5' -and
    ...
    [int]$scienceGate.r18zuRenderedNonZFalseAcceptances -eq 0 -and
...
Require ([bool]$adjudication.authority.operatorExactDecipheringFinishCriterionPassed -and
    ...
    [decimal]$adjudication.decipheringFinish.selectionScore -eq [decimal]::Parse('0.9117836040446633', [Globalization.CultureInfo]::InvariantCulture) -and
...
Require ([string]$r18zuScience.state -ceq 'PASS_R18ZU_REVIEW_BATCH_SCIENCE_WITH_STRICT_RETENTION_HOLD' -and
    ...
    [int]$r18zuScience.boundedRegression.referenceQueries -eq 475 -and
    [int]$r18zuScience.boundedRegression.exactFolds -eq 49 -and
    ...
    -not [bool]$r18zuScience.adjudication.productionAuthorized) 'R18ZU direct review-batch science evidence changed.'
foreach ($pin in @(
    [pscustomobject]@{dependency='batchRunner';artifact='batchRunner';scienceField='batchRunner'},
    [pscustomobject]@{dependency='r18zuProvider';artifact='provider';scienceField='provider'},
    ...
    Require ($bindingRows.Count -eq 1 -and [string]$bindingRows[0].sha256 -ceq [string]$dependency.sha256 -and
        [string]$sciencePin.sha256 -ceq [string]$dependency.sha256) "R18ZU direct science dependency changed: $($pin.dependency)"
}
...
try {
    ...
    Require ([string]$manifest.schema -ceq 'argos_project_portal_request_manifest_v1' -and [string]$manifest.requestId -ceq 'REQ_R18ZU1' -and
    ...
    Require ([string]$change.source -ceq 'payload/Invoke-R18ZTBatchLaunch.ps1' -and
        [string]$change.destination -ceq 'C:\ProgramData\ArgosEdgeLabRO\AllWaferProcessorV2\OCV02_R18ZU1.ps1' -and
...
Require ([string]$priorTerminal.state -ceq 'PASS_R18ZT1_ENDPOINT_TERMINAL_AND_SIGNED_RETURN_ROUTE_HEALTHY' -and
    [string]$priorTerminal.requestId -ceq 'REQ_R18ZT1' -and
    [string]$priorTerminal.endpoint.state -ceq 'PASS_MAINTENANCE_PATCH' -and
    [string]$priorTerminal.returnedResponse.state -ceq 'PASS_MAINTENANCE_PATCH' -and
    [bool]$priorTerminal.returnedResponse.signatureValid -and
    [bool]$priorTerminal.conclusion.priorEndpointRequestTerminal -and
    [bool]$priorTerminal.conclusion.priorEndpointQueueHeadCleared -and
    [bool]$priorTerminal.conclusion.signedTerminalResponseReturnedToShare -and
    [bool]$priorTerminal.conclusion.returnRouteHealthy -and
    -not [bool]$priorTerminal.mutationsPerformed) 'The prior same-endpoint request or its signed return route is not terminal and healthy.'
...
Require ($requestRoot -ceq 'U:\ProjectPortalRO\requests' -and $responseRoot -ceq 'U:\ProjectPortalRO\responses' -and
    $readyPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZU1.ready.zip' -and $uploadPath -ceq 'U:\ProjectPortalRO\requests\REQ_R18ZU1.ready.zip.upload' -and
    $processedPath -ceq 'U:\ProjectPortalRO\requests\processed\REQ_R18ZU1.ready.zip') 'R18ZT exact publication route changed.'
...
$applyIntent = [ordered]@{
    schema = 'argos_opencv_scribe_r18zu1_publication_apply_intent_v8'
    createdUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = 'FROZEN_R18ZU1V8_APPLY_INTENT_BEFORE_EXTERNAL_WRITE'
...
Require ([string]$collision.requestId -ceq 'REQ_R18ZU1' -and
...
$result = [ordered]@{
    schema = 'argos_opencv_scribe_r18zu1_publish_gate_v8'
    publishedUtc = [DateTimeOffset]::UtcNow.ToString('o')
    state = $(if ($Preflight) { 'PASS_R18ZU1V8_PUBLICATION_PREFLIGHT_PENDING_APPLY_COLLISION_GATE' } else { 'PASS_R18ZU1V8_EXACT_SIGNED_MAINTENANCE_PUBLISHED_CREATE_NEW' })
    requestId = 'REQ_R18ZU1'
