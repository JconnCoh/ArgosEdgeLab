...
function Write-O3F9Json([string]$Path, [object]$Value) { Assert-O3F9 (-not (Test-Path -LiteralPath $Path)) "O3F9 create-new JSON exists: $Path"; [IO.File]::WriteAllText($Path, (($Value | ConvertTo-Json -Depth 32) + [Environment]::NewLine), (New-Object Text.UTF8Encoding($false))) }
...
foreach ($path in @($specPath, $definitionPath)) { Assert-O3F9 (Test-Path -LiteralPath $path -PathType Leaf) "O3F9 package dependency is absent: $path" }
$spec = Get-Content -LiteralPath $specPath -Raw | ConvertFrom-Json
$definition = Get-Content -LiteralPath $definitionPath -Raw | ConvertFrom-Json
Assert-O3F9 ([string]$spec.schema -eq 'argos_ocv03_o3f9_package_spec_v1' -and [string]$spec.requestId -eq 'REQ_O3F9_20260902A') 'O3F9 package identity changed.'
Assert-O3F9 ([string]$definition.entryPoint -eq [string]$spec.entryPoint -and @($definition.changes).Count -eq 1 -and @($definition.allowedTaskActions).Count -eq 0) 'O3F9 maintenance contract changed.'
Assert-O3F9 ([bool]$spec.reviewOnly -and -not [bool]$spec.productionRoutingEnabled -and -not [bool]$spec.requestRetryAuthorized) 'O3F9 package authority widened.'
...
$candidateRows = @(
    [pscustomobject]@{role='endpoint';source=$endpointSource;path='Invoke-O3F9StagedEndpoint.ps1';expected=''},
...
foreach ($row in $candidateRows) {
    Assert-O3F9 (Test-Path -LiteralPath $row.source -PathType Leaf) "O3F9 payload source is absent: $($row.source)"
    $hash = Get-O3F9Hash $row.source
    if (-not [string]::IsNullOrWhiteSpace([string]$row.expected)) { Assert-O3F9 ($hash -eq [string]$row.expected) "O3F9 inherited payload hash changed: $($row.source)" }
...
Assert-O3F9 ($sources.Count -eq 12 -and @($sources.role | Sort-Object -Unique).Count -eq 12 -and @($sources.path | Sort-Object -Unique).Count -eq 12) 'O3F9 payload source cardinality changed.'

$stageRoot = [IO.Path]::GetFullPath([string]$spec.stageRoot)
Assert-O3F9 ($stageRoot -eq 'C:\A9F') 'O3F9 staging root changed.'
...
$finalGate = Join-Path $PSScriptRoot 'O3F9_FINAL_PACKAGE_GATE.json'
foreach ($path in @($stageRoot, $requestPartial, $requestReady, $finalRoot, $finalZip, $finalGate)) { Assert-O3F9 (-not (Test-Path -LiteralPath $path)) "O3F9 create-new build target exists: $path" }
...
Assert-O3F9 ([string]$pathGate.state -eq 'PASS_PATH_BUDGET') 'O3F9 build path gate failed.'

if ($Preflight) {
    [ordered]@{
        schema = 'argos_ocv03_o3f9_build_preflight_v1'
        state = 'PASS_O3F9_BUILD_PREFLIGHT'
...
Assert-O3F9 ([string]$spec.state -eq 'FROZEN_FOR_BUILD' -and [string]$definition.state -eq 'FROZEN_FOR_BUILD') 'O3F9 draft spec/definition must be frozen only after all gates pass.'
...
$records = @(Get-ChildItem -LiteralPath (Join-Path $requestPartial 'payload') -File | Sort-Object Name | ForEach-Object { [ordered]@{path=('payload/' + $_.Name);bytes=[int64]$_.Length;sha256=Get-O3F9Hash $_.FullName} })
Assert-O3F9 ($records.Count -eq 13) 'O3F9 staged payload count changed.'
...
Assert-O3F9 ([bool]$certificate.HasPrivateKey) 'O3F9 signer private key is absent.'
...
$gate = [ordered]@{schema='argos_ocv03_o3f9_final_package_gate_v1';state='PASS_O3F9_FINAL_PACKAGE_GATE';requestId=[string]$spec.requestId;requestZip=$finalZip;requestZipBytes=[int64](Get-Item -LiteralPath $finalZip).Length;requestZipSha256=Get-O3F9Hash $finalZip;requestManifestSha256=Get-O3F9Hash (Join-Path $requestReady 'PORTAL_REQUEST_MANIFEST.json');requestSignatureSha256=Get-O3F9Hash (Join-Path $requestReady 'PORTAL_REQUEST_MANIFEST.sig');endpointSha256=[string]$endpointRow.sha256;runnerSha256=[string](@($sources | Where-Object {$_.role -eq 'runner'})[0].sha256);r10Sha256=[string](@($sources | Where-Object {$_.role -eq 'r10Detector'})[0].sha256);localGateSha256=[string](@($sources | Where-Object {$_.role -eq 'localGate'})[0].sha256);payloadFileCount=13;publicationAuthorized=$false;publicationCount=0;requestRetryAuthorized=$false;reviewOnly=$true;productionRoutingEnabled=$false}
Write-O3F9Json $finalGate $gate
